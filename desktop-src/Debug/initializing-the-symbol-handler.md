...
The following code demonstrates how to initialize the symbol handler. The [**SymSetOptions**](symsetoptions.md) function defers symbol loading until symbol information is requested. The code loads the symbols for each module in the specified process by passing a value of **TRUE** for the *bInvade* parameter of the [**SymInitialize**](syminitialize.md) function. (This function calls the [**SymLoadModule64**](symloadmodule64.md) function for each module the process has mapped into its address space.)

If the specified process is not the process that called [**SymInitialize**](syminitialize.md), the code passes a process identifier as the first parameter of **SymInitialize**.
