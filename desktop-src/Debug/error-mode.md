...
To handle these errors without user intervention, use [**SetErrorMode**](/windows/win32/WinBase/?branch=master) or the thread-specific [**SetThreadErrorMode**](/windows/win32/WinBase/?branch=master). After calling one of these functions and specifying appropriate flags, the system will not display the corresponding error message boxes.

A process can retrieve its error mode using [**GetErrorMode**](/windows/win32/WinBase/?branch=master) or [**GetThreadErrorMode**](/windows/win32/WinBase/?branch=master).

Best practice is that all applications call the process-wide [**SetErrorMode**](/windows/win32/WinBase/?branch=master) function with a parameter of **SEM\_FAILCRITICALERRORS** at startup. This is to prevent error mode dialogs from hanging the application.
