...
-   *Global spanning events* are events that contain information about a whole recording. An example of a global spanning event is the [**SBE2\_STREAM\_DESC**](/previous-versions/windows/desktop/api/sbe/ns-sbe-__midl___midl_itf_sbe_0000_0015_0001) event, which describes the state of streams in the recording.
...
SBE2 implements the [**IBroadcastEventEx**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ibroadcasteventex) interface to broadcast spanning events. To receive spanning events, an object calls [**IBroadcastEventEx::FireEx**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ibroadcasteventex-fireex) with the GUID of the spanning event and with context information that uniquely identifies the event. A value of 0 for the third parameter to **FireEx** indicates that this event is a global spanning event. A stream ID value for the third parameter to **FireEx** indicates that the event is an in-band spanning event.

An application can use the [**ISBE2GlobalEvent**](/previous-versions/windows/desktop/api/sbe/nn-sbe-isbe2globalevent) or [**ISBE2SpanningEvent**](/previous-versions/windows/desktop/api/sbe/nn-sbe-isbe2spanningevent) interface (new in Windows 7) to get data from a global or in-band spanning event, respectively. Both global and in-band spanning events can be received from the [**IBroadcastEventEx**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ibroadcasteventex) interface..

Applications can also receive in-band spanning events by parsing media samples in a stream. The media sample for a spanning event does not contain any media data but carries the event data. To get the event data, query the sample for the [**IAttributeGet**](/previous-versions/windows/desktop/api/dsattrib/nn-dsattrib-iattributeget) interface and call the [**IAttributeGet::GetAttrib**](/previous-versions/windows/desktop/api/dsattrib/nf-dsattrib-iattributeget-getattrib) or [**IAttributeGet::GetAttribIndexed**](/previous-versions/windows/desktop/api/dsattrib/nf-dsattrib-iattributeget-getattribindexed) method.
...
The handler method queries the [**Stream Buffer Source**](stream-buffer-source-filter.md) filter to obtain a pointer to the [**ISBE2GlobalEvent**](/previous-versions/windows/desktop/api/sbe/nn-sbe-isbe2globalevent) interface.

The handler then must perform the following actions:

1.  If the required buffer size is unknown, call the [**GetEvent**](/previous-versions/windows/desktop/api/sbe/nf-sbe-isbe2globalevent-getevent) method by using the interface pointer to get the size of the buffer required to hold the event data. The **GetEvent** call passes **NULL** for the *pb* parameter to get the required buffer size, in bytes, in the *pcb* return parameter. If the required buffer size is known, that value can be passed as the value of the *pcb* parameter.
2.  Allocate the buffer that will hold the event data.
3.  Call [**GetEvent**](/previous-versions/windows/desktop/api/sbe/nf-sbe-isbe2globalevent-getevent) again to return the actual event data in the buffer allocated in step 2.
...
[**IBroadcastEventEx**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ibroadcasteventex)
</dt> <dt>
...
[**ISBE2GlobalEvent::GetEvent**](/previous-versions/windows/desktop/api/sbe/nf-sbe-isbe2globalevent-getevent)
</dt> </dl>
