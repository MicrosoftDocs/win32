...
| Interface                                                            | Features                                                                                                                                                                                         |
|----------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**IESEvent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevent)                                         | Defines a generic event interface that can deliver and encapsulate events raised by devices in a media graph.                                                                                    |
| [**IESEvents**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevents)                                       | Defines event handling features for devices in a media graph. Devices register to receive specific types of events sent by other devices, and are required to handle only those types of events. |
| [**IESEventService**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventservice)                           | Defines a custom event service that is attached to the media graph with the service id of **SID\_ESEventService**.                                                                               |
| [**IESEventServiceConfiguration**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventserviceconfiguration) | Configures a custom event service.                                                                                                                                                               |



 

The rest of this document uses the term Event Service for a Windows event service that implements the [**IESEvent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevent) and [**IESEvents**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevents) interfaces. Although the Event Service was initially developed to support the Protected Broadcast Driver Architecture (PBDA) in Windows 7, it is a generic event interface that can be used with non-PBDA media graphs, and non-PBDA events in Windows 7 implement these interfaces. For more information about PBDA, download the specification from <http://go.microsoft.com/fwlink/p/?linkid=132926>.
...
2.  Query the media graph's service provider object to obtain a pointer to the event service object. A pointer to [**IESEventService**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventservice) interface is returned in the third parameter of the **QueryService** call.
3.  Use the [**IESEventService**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventservice) pointer from step 2 to obtain a pointer to the connection point container object. A pointer to the **IConnectionPointContainer** interface is returned in the second parameter of the **QueryInterface** call.
...
The following example shows how an object of class **CListener** would register to receive the [**OpenMMIEvent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesopenmmievent) event (error checking code omitted for brevity):
...
Once you have the registration process defined, the object can start listening for registered events by calling the registration method. The following example shows how a CListener object would start listening for an [**OpenMMIEvent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesopenmmievent) event by calling the **RegisterForEvents** method defined in the previous example:
...
To handle events from the Event Service, an object must implement the [**IESEvents**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevents) interface, which defines a single event handler method named [**OnESEventReceived**](/previous-versions/windows/desktop/api/tuner/nf-tuner-iesevents-oneseventreceived). The object must implement **OnESEventReceived** for each type of event it has registered to receive. The following example illustrates how the CListener object from the previous example would receive an [**OpenMMIEvent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesopenmmievent) event:
...
To obtain data from an event, a receiving object must implement the [**IESEvent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevent) interface. This interface defines the following methods for obtaining event data:



| Method                                          | Feature                                            |
|-------------------------------------------------|----------------------------------------------------|
| [**GetEventType**](/previous-versions/windows/desktop/api/tuner/nf-tuner-iesevent-geteventtype)   | Returns a GUID that indicates the event type.      |
| [**GetData**](/previous-versions/windows/desktop/api/dvbsiparser/nf-dvbsiparser-idvb_st-getdata)              | Returns a byte array that contains the event data. |
| [**GetStringData**](/previous-versions/windows/desktop/api/tuner/nf-tuner-iesevent-getstringdata) | Gets the event data in Unicode string format.      |
| [**GetEventId**](/previous-versions/windows/desktop/api/tuner/nf-tuner-iesevent-geteventid)       | Gets the unique identifier for the event.          |



 

The following example shows how an object would implement the [**IESEvent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevent) interface to obtain data from an event of type **MyEvent**:
...
1.  Query the media graph object to obtain a pointer to the media graph's service provider object. A pointer to the **IServiceProvider** interface is returned in the second parameter of the **QueryInterface** call.
2.  Query the media graph's service provider object to obtain a pointer to the event service object. A pointer to the [**IESEventService**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventservice) interface is returned in the third parameter of the **QueryService** call.
3.  Use the [**IESEventService**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventservice) pointer from step 1 to call the [**FireESEvent**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ieseventservice-fireesevent) method and raise the desired event.
...
2.  Instantiate an object that implements the [**IESEventService**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventservice) interface for your custom event service. A pointer to the **IESEventService** interface for the custom event service is returned in the last parameter of the **CoCreateInstance** call that creates the custom event service object.
3.  Query the event service object you created in step 2 to obtain a pointer to the event service configuration object for your custom event service. A pointer to the [**IESEventServiceConfiguration**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventservice) interface is returned in the second parameter of the **QueryInterface** call.
4.  Use the [**IESEventServiceConfiguration**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ieseventserviceconfiguration) pointer from step 3 to make the event service you are chaining to the parent or owner of your custom event service.
    -   Use the [**IESEventServiceConfiguration::SetParent**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ieseventserviceconfiguration-setparent) method to make the chained-to event service the parent. The *parent* event service can pass events to your custom event service so that you can pass them to your clients.
    -   If you make an object an *owner* of your custom event service, the owner object can examine data from any events it passes to your custom event service. Use the [**IESEventServiceConfiguration::SetOwner**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ieseventserviceconfiguration-setparent) method to make the object an owner. The object must implement the [**IESEvents**](/previous-versions/windows/desktop/api/tuner/nn-tuner-iesevents) interface so that it can handle events that the parent event service passes to it.

A parent event service calls the [**IESEvents::OnESEventReceived**](/previous-versions/windows/desktop/api/tuner/nf-tuner-iesevents-oneseventreceived) method of the owner object to allow the owner to examine the events. The can do can do any or any combination of the following:

-   Handle the event itself.
-   Pass the event to your custom event system without changes for handling. The owner returns "S\_OK" from the [**OnESEventReceived**](/previous-versions/windows/desktop/api/tuner/nf-tuner-iesevents-oneseventreceived) method to accomplish this.
-   Modify the event or raise a different event before passing it to your custom event system.
-   Prevent the event from passing through to your custom event system. The owner returns "S\_FALSE" from the [**OnESEventReceived**](/previous-versions/windows/desktop/api/tuner/nf-tuner-iesevents-oneseventreceived) method to accomplish this.
