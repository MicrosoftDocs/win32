...
Objects send events using COM connection points. To catch an event, the client implements an event interface. The object fires events by calling methods on that interface. For example, the Video Control sends events through the [**\_IMSVidCtlEvents**](/previous-versions/windows/desktop/api/msvidctl/) interface. The mouse-click event is [**\_IMSVidCtlEvents::Click**](/previous-versions/windows/desktop/api/msvidctl/). The following image illustrates this process.
