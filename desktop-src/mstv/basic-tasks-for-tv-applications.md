...
8.  **Create a default tune request.** (Required for script applications only) When Microsoft Internet Explorer encounters a "TV:" URL in a Web page, it automatically loads the Video Control and tunes it to the default tune request, which is stored in the system registry. This registry entry must be created by a C++ application, using the [**ICreatePropBagOnRegKey**](/previous-versions/windows/desktop/api/regbag/nn-regbag-icreatepropbagonregkey) interface.
