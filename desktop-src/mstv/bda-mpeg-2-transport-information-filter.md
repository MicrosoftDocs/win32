...
<td>[<strong>IBaseFilter</strong>](https://msdn.microsoft.com/library/windows/desktop/dd389526), <strong>IConnectionPointContainer</strong>, [<strong>IGuideData</strong>](/previous-versions/windows/desktop/api/Bdatif/nn-bdatif-iguidedata), [<strong>IPSITables</strong>](/windows/desktop/api/mpeg2psiparser/nn-mpeg2psiparser-ipsitables), [<strong>ITuneRequestInfo</strong>](/previous-versions/windows/desktop/api/Bdatif/nn-bdatif-itunerequestinfo)</td>
</tr>
<tr class="even">
...
Currently, the [**IGuideData**](/previous-versions/windows/desktop/api/Bdatif/nn-bdatif-iguidedata) interface is available only for DVB broadcasts. For other types, the filter returns E\_NOINTERFACE when queried for the interface.

This filter supports event notifications through the [**IGuideDataEvent**](/previous-versions/windows/desktop/api/Bdatif/nn-bdatif-iguidedataevent) connection point interface. To set up the event sink, use the **IConnectionPointContainer** interface on the filter.
