---
title: Video Control Reference
description: Video Control Reference
ms.assetid: 4d4559c1-ce2d-48a7-bf15-826cfb9512f1
ms.date: 05/31/2018
ms.topic: article
ms.author: windowssdkdev
ms.prod: windows
ms.technology: desktop
---

# Video Control Reference

This section contains the reference documentation for the [Video Control](video-control.md) application programming interfaces (APIs). For more information on using these APIs, see [Using the Video Control](using-the-video-control.md).

-   [Video Control Objects (C++)](video-control-objects--c.md)
-   [Video Control Interfaces](video-control-interfaces.md)
-   [Video Control Event Interfaces](video-control-event-interfaces.md)
-   [Video Control Enumerations](video-control-enumerations.md)
-   [MSVidWebDVD Visual Basic Reference](mstv-msvidwebdvd_visual_basic_reference)

## Related topics

<dl> <dt>

[Video Control](video-control.md)
</dt> </dl>

 

 




