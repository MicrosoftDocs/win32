...
4.  Query the Stream Buffer Sink filter for the [**IStreamBufferSink**](/previous-versions/windows/desktop/api/Sbe/nn-sbe-istreambuffersink) interface.
5.  Call the [**IStreamBufferSink::LockProfile**](/previous-versions/windows/desktop/api/Sbe/nf-sbe-istreambuffersink-lockprofile) method. This method does several things:
...
3.  Establish a source for the Stream Buffer Source filter, by doing one of the following:
    -   Call [**IStreamBufferSource::SetStreamSink**](/previous-versions/windows/desktop/api/Sbe/nf-sbe-istreambuffersource-setstreamsink) with a pointer to the sink filter's **IStreamBufferSink** interface. With this method, the sink graph and the source graph must share the same process space.
