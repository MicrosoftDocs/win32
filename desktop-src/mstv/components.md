...
To get the components list from a tune request, call the [**ITuneRequest::get\_Components**](itunerequest-get-components.md) method, which returns a pointer to the **IComponents** interface. Then call [**IComponents::EnumComponents**](icomponents-enumcomponents.md), which returns a pointer to the [**IEnumComponents**](ienumcomponents.md) interface. Use this interface to iterate through the collection:
...
After the tune request is submitted to the Network Provider and data is moving through the filter graph, you can get the updated tune request by calling the Network Provider filter's [**ITuner::get\_TuneRequest**](ituner-get-tunerequest.md) method. Use the returned **ITuneRequest** pointer to examine the updated component information. To change which components are active or inactive, update the component list and resubmit the tune request. [**IComponent::put\_Status**](icomponent-put-status.md) to change a component's active/inactive status.
...
4.  Set the desired properties on each component type. At a minimum, set the category (audio, video, and so forth) by calling the [**IComponentType::put\_Category**](icomponenttype-put-category.md) method.
5.  Add each component to the collection by calling [**IComponents::Add**](icomponents-add.md).
6.  Call [**ITuningSpace::put\_DefaultPreferredComponentTypes**](ituningspace-put-defaultpreferredcomponenttypes.md) with a pointer to the component types collection.
7.  Persist the changes by putting the tuning space back into the **SystemTuningSpaces** collection. Call the [**ITuningSpaceContainer::put\_Item**](ituningspacecontainer-put-item.md) method with a pointer to the tuning space object.
...
One reason to set the default component types is to specify a preferred language for an audio stream. Do this by calling the [**ILanguageComponentType::put\_LangID**](ilanguagecomponenttype-put-langid.md) method on the component type.
