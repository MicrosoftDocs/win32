...
Components are represented by the [**IComponent**](/previous-versions/windows/desktop/api/tuner/nn-tuner-icomponent) interface. Each component has a type associated with it, represented by the [**IComponentType**](/previous-versions/windows/desktop/api/tuner/nn-tuner-icomponenttype) interface. The component type describes the component, and may include the media type of the stream. The **IComponent** interface is also used to activate or deactivate a given component. For example, if a program has two audio streams (say, English and Spanish), typically only one stream is active at a time.
...
To get the components list from a tune request, call the [**ITuneRequest::get\_Components**](/previous-versions/windows/desktop/api/tuner/nf-tuner-itunerequest-get_components) method, which returns a pointer to the **IComponents** interface. Then call [**IComponents::EnumComponents**](/previous-versions/windows/desktop/api/tuner/nf-tuner-icomponents-enumcomponents), which returns a pointer to the [**IEnumComponents**](/previous-versions/windows/desktop/api/tuner/nn-tuner-ienumcomponents) interface. Use this interface to iterate through the collection:
...
After the tune request is submitted to the Network Provider and data is moving through the filter graph, you can get the updated tune request by calling the Network Provider filter's [**ITuner::get\_TuneRequest**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ituner-get_tunerequest) method. Use the returned **ITuneRequest** pointer to examine the updated component information. To change which components are active or inactive, update the component list and resubmit the tune request. [**IComponent::put\_Status**](/previous-versions/windows/desktop/api/tuner/nf-tuner-icomponent-put_status) to change a component's active/inactive status.
...
4.  Set the desired properties on each component type. At a minimum, set the category (audio, video, and so forth) by calling the [**IComponentType::put\_Category**](/previous-versions/windows/desktop/api/tuner/nf-tuner-icomponenttype-put_category) method.
5.  Add each component to the collection by calling [**IComponents::Add**](/previous-versions/windows/desktop/api/tuner/nf-tuner-icomponents-add).
6.  Call [**ITuningSpace::put\_DefaultPreferredComponentTypes**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ituningspace-put_defaultpreferredcomponenttypes) with a pointer to the component types collection.
7.  Persist the changes by putting the tuning space back into the **SystemTuningSpaces** collection. Call the [**ITuningSpaceContainer::put\_Item**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ituningspacecontainer-put_item) method with a pointer to the tuning space object.
...
One reason to set the default component types is to specify a preferred language for an audio stream. Do this by calling the [**ILanguageComponentType::put\_LangID**](/previous-versions/windows/desktop/api/tuner/nf-tuner-ilanguagecomponenttype-put_langid) method on the component type.
