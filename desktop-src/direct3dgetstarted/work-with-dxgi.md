...
-   Get a [**CoreWindow**](https://msdn.microsoft.com/library/windows/apps/br208225) for your app.
-   Get an interface for the Direct3D device and context.
-   Create the swap chain to display your rendered image in the [**CoreWindow**](https://msdn.microsoft.com/library/windows/apps/br208225).
-   Create a render target for drawing and populate it with pixels.
-   Present the swap chain!

## Create a window for your app

The first thing we need to do is create a window. First, create a window class by populating an instance of [**WNDCLASS**](https://msdn.microsoft.com/library/windows/desktop/ms633576), then register it using [**RegisterClass**](https://msdn.microsoft.com/library/windows/desktop/ms633586). The window class contains essential properties of the window, including the icon it uses, the static message processing function (more on this later), and a unique name for the window class.
...
Next, you create the window. We also need to provide size information for the window and the name of the window class we just created. When you call [**CreateWindow**](https://msdn.microsoft.com/library/windows/desktop/ms632679), you get back an opaque pointer to the window called an HWND; you'll need to keep the HWND pointer and use it any time you need to reference the window, including destroying or recreating it, and (especially important) when creating the DXGI swap chain you use to draw in the window.
...
This simple example only checks for program exit conditions: [**WM\_CLOSE**](https://msdn.microsoft.com/library/windows/desktop/ms632617), sent when the window is requested to be closed, and [**WM\_DESTROY**](https://msdn.microsoft.com/library/windows/desktop/ms632620), which is sent after the window is actually removed from the screen. A full, production app needs to handle other windowing events too—for the complete list of windowing events, see [Window Notifications](https://msdn.microsoft.com/library/windows/desktop/ff468922).
...
First, you tell DXGI what values to use for the properties of the swap chain. Do this using a [**DXGI\_SWAP\_CHAIN\_DESC**](https://msdn.microsoft.com/library/windows/desktop/bb173075) structure. Six fields are particularly important for desktop apps:
...
-   **SampleDesc**: This field controls multisampling. Set **Count** to 1 and **Quality** to 0 for flip-model swap chains. (To use multisampling with flip-model swap chains, draw on a separate multisampled render target and then resolve that target to the swap chain just before presenting it. Example code is provided in [Multisampling in Windows Store apps](https://msdn.microsoft.com/library/windows/apps/dn458384).)
...
Get the [**ID3D11Device**](https://msdn.microsoft.com/library/windows/desktop/dn280493) reference you created previously. Upcast it to [**IDXGIDevice3**](https://msdn.microsoft.com/library/windows/desktop/dn280345) (if you haven't already) and then call [**IDXGIDevice::GetAdapter**](https://msdn.microsoft.com/library/windows/desktop/bb174531) to acquire the DXGI adapter. Get the parent factory for that adapter by calling [**IDXGIFactory2::GetParent**](https://msdn.microsoft.com/library/windows/desktop/bb174542) ([**IDXGIFactory2**](https://msdn.microsoft.com/library/windows/desktop/hh404556) inherits from [**IDXGIObject**](https://msdn.microsoft.com/library/windows/desktop/bb174541))—now you can use that factory to create the swap chain by calling [**CreateSwapChainForHwnd**](https://msdn.microsoft.com/library/windows/desktop/hh404557), as seen in the following code sample.
...
The shader pipeline needs a resource to draw pixels into. The simplest way to create this resource is to define a [**ID3D11Texture2D**](https://msdn.microsoft.com/library/windows/desktop/ff476635) resource as a back buffer for the pixel shader to draw into, and then read that texture into the swap chain.
