...
When [**IDirect3DDevice9::ProcessVertices**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-processvertices) fails due to an incompatible destination vertex buffer FVF code, the expected code is printed to the debug output (debug builds only).
