...
Vertex buffers, represented by the [**IDirect3DVertexBuffer9**](idirect3dvertexbuffer9.md) interface, are memory buffers that contain vertex data. Vertex buffers can contain any vertex type - transformed or untransformed, lit or unlit - that can be rendered through the use of the rendering methods in the [**IDirect3DDevice9**](idirect3ddevice9.md) interface. You can process the vertices in a vertex buffer to perform operations such as transformation, lighting, or generating clipping flags. Transformation is always performed.
...
You can create vertex buffers with the [**IDirect3DDevice9::CreateVertexBuffer**](idirect3ddevice9--createvertexbuffer.md) method, which takes pool (memory class) and usage parameters. **IDirect3DDevice9::CreateVertexBuffer** can also be created with a specified FVF code for use in fixed function vertex processing, or as the output of process vertices. For details, see [FVF Vertex Buffers (Direct3D 9)](fvf-vertex-buffers.md).
