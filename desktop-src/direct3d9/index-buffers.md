...
Index buffers, represented by the [**IDirect3DIndexBuffer9**](/windows/desktop/api/d3d9helper/nn-d3d9-idirect3dindexbuffer9) interface, are memory buffers that contain index data. Index data, or indices, are integer offsets into vertex buffers and are used to render primitives using the [**IDirect3DDevice9::DrawIndexedPrimitive**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-drawindexedprimitive) method.

A vertex buffer contains vertices; therefore, you can draw a vertex buffer either with or without indexed primitives. However, because an index buffer contains indices, you cannot use an index buffer without a corresponding vertex buffer. (As a side note, [**IDirect3DDevice9::DrawIndexedPrimitiveUP**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-drawindexedprimitiveup) and [**IDirect3DDevice9::DrawPrimitiveUP**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitiveup) are the only draw methods that draw without an index or a vertex buffer.)
...
Setting the D3DUSAGE\_SOFTWAREPROCESSING behavior flag when calling the [**IDirect3DDevice9::CreateIndexBuffer**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-createindexbuffer) method specifies that the index buffer is to be used with software vertex processing. This flag is required in mixed mode vertex processing (D3DCREATE\_MIXED\_VERTEXPROCESSING) when software vertex processing is used.
...
Create an index buffer object by calling the [**IDirect3DDevice9::CreateIndexBuffer**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-createindexbuffer) method, which accepts six parameters.
...
-   The fourth parameter is a member of the [**D3DPOOL**](https://msdn.microsoft.com/en-us/library/Bb172584(v=VS.85).aspx) enumerated type that tells the system where in memory to place the new index buffer.
...
After you fill or read the index data, call the [**IDirect3DIndexBuffer9::Unlock**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dindexbuffer9-unlock) method, as shown in the following code example.
...
>
> For information about using D3DLOCK\_DISCARD or D3DLOCK\_NOOVERWRITE for the *Flags* parameter of the [**IDirect3DIndexBuffer9::Lock**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dindexbuffer9-lock) method, see [Performance Optimizations (Direct3D 9)](performance-optimizations.md).
...
Retrieve information about an index buffer by calling the [**IDirect3DIndexBuffer9::GetDesc**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dindexbuffer9-getdesc) method. This method fills the members of the [**D3DINDEXBUFFER\_DESC**](d3dindexbuffer-desc.md) structure with information about the index buffer.
