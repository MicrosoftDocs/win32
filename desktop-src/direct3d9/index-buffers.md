...
Index buffers, represented by the [**IDirect3DIndexBuffer9**](/windows/desktop/api) interface, are memory buffers that contain index data. Index data, or indices, are integer offsets into vertex buffers and are used to render primitives using the [**IDirect3DDevice9::DrawIndexedPrimitive**](/windows/desktop/api) method.

A vertex buffer contains vertices; therefore, you can draw a vertex buffer either with or without indexed primitives. However, because an index buffer contains indices, you cannot use an index buffer without a corresponding vertex buffer. (As a side note, [**IDirect3DDevice9::DrawIndexedPrimitiveUP**](/windows/desktop/api) and [**IDirect3DDevice9::DrawPrimitiveUP**](/windows/desktop/api) are the only draw methods that draw without an index or a vertex buffer.)
...
Setting the D3DUSAGE\_SOFTWAREPROCESSING behavior flag when calling the [**IDirect3DDevice9::CreateIndexBuffer**](/windows/desktop/api) method specifies that the index buffer is to be used with software vertex processing. This flag is required in mixed mode vertex processing (D3DCREATE\_MIXED\_VERTEXPROCESSING) when software vertex processing is used.
...
Create an index buffer object by calling the [**IDirect3DDevice9::CreateIndexBuffer**](/windows/desktop/api) method, which accepts six parameters.
...
-   The final parameter that [**IDirect3DDevice9::CreateIndexBuffer**](/windows/desktop/api) accepts is the address of a variable that is filled with a pointer to the new [**IDirect3DIndexBuffer9**](/windows/desktop/api) interface of the vertex buffer object, if the call succeeds.
...
After you fill or read the index data, call the [**IDirect3DIndexBuffer9::Unlock**](/windows/desktop/api) method, as shown in the following code example.
...
>
> For information about using D3DLOCK\_DISCARD or D3DLOCK\_NOOVERWRITE for the *Flags* parameter of the [**IDirect3DIndexBuffer9::Lock**](/windows/desktop/api) method, see [Performance Optimizations (Direct3D 9)](performance-optimizations.md).
