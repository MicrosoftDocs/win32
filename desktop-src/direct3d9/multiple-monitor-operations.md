...
When a device is successfully reset ([**IDirect3DDevice9::Reset**](idirect3ddevice9--reset.md)) or created ([**IDirect3D9::CreateDevice**](idirect3d9--createdevice.md)) in full-screen operations, the Direct3D object that created the device is marked as owning all adapters on that system. This state is known as exclusive mode, and the Direct3D object owns exclusive mode. Exclusive mode means that devices created by any other Direct3D object can neither assume full-screen operations nor allocate video memory. In addition, when a Direct3D object assumes exclusive mode, all devices other than the one that went full-screen are placed in lost state. For details, see [Lost Devices (Direct3D 9)](lost-devices.md).
