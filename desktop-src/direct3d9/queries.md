...
| EVENT             | [**D3DISSUE\_END**](d3dissue-end.md)                                            | BOOL                                                                        | Retail/Debug | [**CreateDevice**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3d9-createdevice) |
...
| RESOURCEMANAGER   | [**D3DISSUE\_END**](d3dissue-end.md)                                            | [**D3DDEVINFO\_ResourceManager**](d3ddevinfo-resourcemanager.md)           | Debug only   | [**Present**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-present)     |
...
| VCACHE            | [**D3DISSUE\_END**](d3dissue-end.md)                                            | [**D3DDEVINFO\_VCACHE**](d3ddevinfo-vcache.md)                             | Retail/Debug | [**CreateDevice**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3d9-createdevice) |
| VERTEXSTATS       | [**D3DISSUE\_END**](d3dissue-end.md)                                            | [**D3DDEVINFO\_D3DVERTEXSTATS**](d3ddevinfo-d3dvertexstats.md)             | Debug only   | [**Present**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-present)     |
...
This method returns a success code if a query can be created; otherwise it returns an error code. Once [**CreateQuery**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-createquery) succeeds, you can create a query object like this:
...
[**GetData**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dquery9-getdata) does two things:

1.  Returns the query state in the return code.
2.  Returns the answer to the query in *pData*.

From each of the three query states, here are the [**GetData**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dquery9-getdata) return codes:
...
-   Call [**GetData**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dquery9-getdata) to return the number of pixels drawn.
-   Specify [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md) to enable the resource to transition the query to the signaled state.
-   Poll the query resource by calling [**GetData**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dquery9-getdata) from a loop. As long as **GetData** returns S\_FALSE, this means the resource has not returned the answer yet.

The return value of [**GetData**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dquery9-getdata) essentially tells you in what state the query is. Possible values are S\_OK, S\_FALSE, and an error. Do not call **GetData** on a query that is in the building state.

-   S\_OK means the resource (GPU or driver, or runtime) is finished. The query is returning to the signaled state. The answer (if any) is being returned by [**GetData**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dquery9-getdata).
...
Instead of specifying [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md), which provides more up-to-date information, you could supply zero which is a more light-weight check if the query is in the issued state. Supplying zero will cause [**GetData**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dquery9-getdata) to not flush the command buffer. For this reason, care must be taken to avoid infinate loops (see **GetData** for details). Since the runtime queues up work in the command buffer, **D3DGETDATA\_FLUSH** is a mechanism for flushing the command buffer to the driver (and hence the GPU; see [Accurately Profiling Direct3D API Calls (Direct3D 9)](accurately-profiling-direct3d-api-calls.md)). During the command buffer flush, a query may transition to the signaled state.
