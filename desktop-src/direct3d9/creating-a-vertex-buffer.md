...
You create a vertex buffer object by calling the [**IDirect3DDevice9::CreateVertexBuffer**](/windows/desktop/api) method, which accepts five parameters. The first parameter specifies the vertex buffer length, in bytes. Use the sizeof operator to determine the size of a vertex format, in bytes. Consider the following custom vertex format.
...
The final parameter that [**IDirect3DDevice9::CreateVertexBuffer**](/windows/desktop/api) accepts is the address of a variable that will be filled with a pointer to the new [**IDirect3DVertexBuffer9**](/windows/desktop/api) interface of the vertex buffer object, if the call succeeds.
