...
You create a vertex buffer object by calling the [**IDirect3DDevice9::CreateVertexBuffer**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-createvertexbuffer) method, which accepts five parameters. The first parameter specifies the vertex buffer length, in bytes. Use the sizeof operator to determine the size of a vertex format, in bytes. Consider the following custom vertex format.
...
The final parameter that [**IDirect3DDevice9::CreateVertexBuffer**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-createvertexbuffer) accepts is the address of a variable that will be filled with a pointer to the new [**IDirect3DVertexBuffer9**](/windows/desktop/api/d3d9helper/nn-d3d9-idirect3dvertexbuffer9) interface of the vertex buffer object, if the call succeeds.
