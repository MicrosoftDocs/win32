...
You create a cubic environment map texture by calling the [**CreateCubeTexture**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-createcubetexture) method. Cubic environment map textures must be square, with dimensions that are a power of two.
...
You can navigate between faces of a cubic environment map by using the [**GetCubeMapSurface**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dcubetexture9-getcubemapsurface) method.

The following code example uses [**GetCubeMapSurface**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dcubetexture9-getcubemapsurface) to retrieve the cube-map surface used for the positive y-face (face 2).
