...
-   Multiple-element texture surfaces, when used as a texture, cannot be filtered. This limitation can be verified using [**CheckDeviceFormat**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3d9-checkdeviceformat).
...
-   [**Clear**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-clear) clears all elements of the multiple-element texture which is set as the render target.
...
3.  The application sets the surface as a render target using the [**SetRenderTarget**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-setrendertarget) call. The pixel shader provides output to the surfaces using the [mov - ps](https://msdn.microsoft.com/windows/desktop/be89b6e8-6f71-43f4-9970-0631adbaef80) instruction.
