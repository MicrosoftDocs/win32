...
The DirectX 10 HLSL compiler is now the default compiler. See [Effect-Compiler Tool](direct3dtools.fxc) for details.
...
Applying **compiler flags** or **assembler flags** to the incorrect API will fail shader validation. Check the Direct3D error code return value from the function with the DirectX Error Lookup Tool (DXErr.exe) to help track down this error. You can get DXErr.exe and learn about it from the DirectX SDK. For info about the DirectX SDK, see [Where is the DirectX SDK?](nodepage.directx_sdk__august_2009_).
