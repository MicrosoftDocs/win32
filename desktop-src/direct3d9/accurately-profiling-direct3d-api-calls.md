...
The profiler returns the number of CPU cycles required to process the work associated with each call (remember that the GPU isn't included in these numbers because the GPU hasn't started working on these commands yet). Because [**IDirect3DDevice9::DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) required almost a million cycles to process, you could conclude that it is not very efficient. However, you'll soon see why this conclusion is incorrect and how you can generate results that can be used for budgeting.

### Measuring State Changes Requires Careful Render Sequences

All calls other than [**IDirect3DDevice9::DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master), [**DrawIndexedPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawindexedprimitive?branch=master), or [**Clear**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-clear?branch=master) (such as [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master), [**SetVertexDeclaration**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-setvertexdeclaration?branch=master), and [**SetRenderState**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-setrenderstate?branch=master)) produce a state change. Each state change sets pipeline state that controls how rendering will be done.
...
In other words, it takes about 4568 clock cycles to process [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) and [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) on this 2 GHz machine. You could convert these values to the actual time it took to execute all the calls like this:
...
Using the render sequence from example 2, here are some typical timing measurements that illustrate the magnitude of a mode transition. Assuming that [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) and [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) calls do not cause a mode transition, an off-the-shelf instrumenting profiler could return results similar to these:
...
Now, look at what happens for the same example when a mode transition occurs. This time, assume [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) and [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) cause a mode transition. Once again, an off-the-shelf instrumenting profiler could return results similar to these:
...
The time measured for [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) is about the same, however, the dramatic increase in the amount of time spent in [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) is due to the mode transition. Here's what is happening:

1.  Assume the command buffer has room for one command before our render sequence starts.
2.  [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) is converted to a device-independent format and added to the command buffer. In this scenario, this call fills the command buffer.
3.  The runtime tries to add [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) to the command buffer but cannot, because it is full. Instead, the runtime empties the command buffer. This causes the kernel-mode transition. Assume the transition takes about 5000 cycles. This time contributes to time spent in **DrawPrimitive**.
4.  The driver then processes the work associated with all the commands that were emptied from the command buffer. Assume that the driver time to process the commands that nearly filled the command buffer is about 935,000 cycles. Assume that the driver work associated with [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) is about 2750 cycles. This time contributes to time spent in [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master).
...
6.  The render sequence finishes by converting [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) and adding it to the command buffer. Assume this takes about 900 cycles. This time contributes to time spent in **DrawPrimitive**.
...
Just like the measurement for [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) without the mode transition (900 cycles), the measurement for **DrawPrimitive** with the mode transition (947,950 cycles) is accurate but useless in terms of budgeting CPU work. The result contains the correct runtime work, the driver work for [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master), the driver work for any commands that preceded **SetTexture**, and two mode transitions. However, the measurement is missing the **DrawPrimitive** driver work.
...
2.  Add a query event marker to the command buffer by calling [**Issue**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-issue?branch=master)([**D3DISSUE\_END**](d3dissue-end.md)). This marker tells the driver to track when the GPU finishes executing whatever commands preceded the marker.
3.  The first call empties the command buffer because calling [**GetData**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-getdata?branch=master) with [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md) forces the command buffer to be emptied. Each subsequent call is checking the GPU to see when it finishes processing all the command-buffer work. This loop does not return S\_OK until the GPU is idle.
...
7.  The first call empties the command buffer because calling [**GetData**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-getdata?branch=master) with [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md) forces the command buffer to be emptied. When the GPU finishes processing all the command-buffer work, **GetData** returns S\_OK, and the loop is exited because the GPU is idle.
...
1.  The first call empties the command buffer by calling [**GetData**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-getdata?branch=master) with [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md). When the GPU finishes processing all the command-buffer work, **GetData** returns S\_OK, and the loop is exited because the GPU is idle.
2.  The render sequence starts by converting [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) to a device-independent format and adding it to the command buffer. Assume this takes about 100 cycles.
3.  [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) is converted and added to the command buffer. Assume this takes about 900 cycles.
4.  [**Issue**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-issue?branch=master) adds a query marker to the command buffer. Assume this takes about 200 cycles.
5.  [**GetData**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-getdata?branch=master) causes the command buffer to be emptied which forces the kernel-mode transition. Assume this takes about 5000 cycles.
6.  The driver then processes the work associated with all four calls. Assume that the driver time to process [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) is about 2964 cycles, [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) is about 3600 cycles, [**Issue**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-issue?branch=master) is about 200 cycles. So the total driver time for all four commands is about 6450 cycles.
    > [!Note]  
    > The driver also takes a little time to see what the status of the GPU is. Because the GPU work is trivial, the GPU should be done already. [**GetData**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-getdata?branch=master) will return S\_OK based on the likelihood that the GPU is finished.
...
The numbers for [**GetData**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-getdata?branch=master) include:
...
-   When one of the lock methods ([**Lock**](/windows/win32/d3d9helper/nf-d3d9-idirect3dvertexbuffer9-lock?branch=master)) is called on a vertex buffer, index buffer, or texture (under certain conditions with certain flags).
...
-   When [**ValidateDevice**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-validatedevice?branch=master) is called.
-   When [**Present**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-present?branch=master) is called.
-   When the command buffer fills up.
-   When [**GetData**](/windows/win32/d3d9helper/nf-d3d9-idirect3dquery9-getdata?branch=master) is called with D3DGETDATA\_FLUSH.
...
In other words, it takes about 6.9 million cycles on this 2 GHz machine to process the 1500 calls in the render loop. Of the 6.9 million cycles, the amount of time in the mode transitions is approximately 10k, so now the profile results are almost entirely measuring work associated with [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) and [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master).

Notice that the code sample requires an array of two textures. To avoid a runtime optimization that would remove [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) if it sets the same texture pointer every time it is called, simply use an array of two textures. That way, each time through the loop, the texture pointer changes, and the full work associated with **SetTexture** is performed. Be sure that both textures are the same size and format, so that no other state will change when the texture does.
...
So far, this paper has shown how to profile a render sequence. Each render sequence has been fairly simple, containing a single [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) call and a [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) call. This was done to focus on the command buffer and the use of the query mechanism to control it. Here is a brief summary of how to profile an arbitrary render sequence:
...
All of these techniques are used to profile state changes. Assuming that you have read and understood how to control the command buffer, and have successfully completed baseline measurements on [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master), you are ready to add state changes to your render sequences. There are a few additional profiling challenges when adding state changes to a render sequence. If you intend to add state changes to your render sequences, be sure to continue into the next section.
...
1.  Profile [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) first.
...
Starting with a render sequence that contains [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master), here is the code sequence for measuring the cost of adding [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master):
...
Notice that the loop contains two calls, [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) and [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master). The render sequence loops 1500 times and generates results similar to these:
...
Each iteration of the loop contains a state change and a draw call. Subtracting out the result of the [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) render sequence leaves:
...
This is the average number of cycles to add [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) to this render sequence. This same technique can be applied to other state changes.

Why is [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) called a simple state change? Because the state that is being set is constrained so that the pipeline does the same amount of work each time the state is changed. Constraining both textures to the same size and format assures the same amount of work for each **SetTexture** call.
...
2.  Profile a second render sequence that toggles the state change. The render sequence loop contains:
    ...
    -   [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) just like the original sequence.
    -   A state change to set the state into a "true" condition.
    -   A second [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) to force the second state change to be realized.
3.  Find the difference between the two render sequences. This is done by:
    -   Multiply the baseline [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) sequence by 2 because there are two **DrawPrimitive** calls in the new sequence.
...
The loop toggles the state by executing two [**SetRenderState**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-setrenderstate?branch=master) calls. The first **SetRenderState** call disables z-testing and the second **SetRenderState** enables z-testing. Each **SetRenderState** is followed by [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) so that the work associated with the state change is processed by the driver instead of only setting a dirty bit in the driver.
...
Profiling either one of these state changes in a render sequence with [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) generates results similar to these:
...
In general, the runtime will concatenate two or more back-to-back [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) calls when:

1.  The primitive type is a triangle list (D3DPT\_TRIANGLELIST).
2.  Each successive [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) call must reference consecutive vertices within the vertex buffer.

Similarly, the right conditions for concatenating two or more back-to-back [**DrawIndexedPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawindexedprimitive?branch=master) calls are:

1.  The primitive type is a triangle list (D3DPT\_TRIANGLELIST).
2.  Each successive [**DrawIndexedPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawindexedprimitive?branch=master) call must sequential reference consecutive indices within the index buffer.
3.  Each successive [**DrawIndexedPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawindexedprimitive?branch=master) call must use the same value for BaseVertexIndex.
...
-   When the previous call is [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master), if the next draw call:
    -   uses a triangle list, AND
    -   specifies the StartVertex = previous StartVertex + previous PrimitiveCount \* 3
-   When using [**DrawIndexedPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawindexedprimitive?branch=master), if the next draw call:
    -   uses a triangle list, AND
...
The loop iterates through 1500 triangles, setting a texture and drawing each triangle. This render loop takes approximately 2750 cycles for [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master) and 1100 cycles for [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) as shown in previous sections. You might intuitively expect that moving **SetTexture** outside the render loop should reduce the amount of work done by the driver by 1500 \* 2750 cycles, which is the amount of work associated with calling **SetTexture** 1500 times. The code snippet would look like this:
...
This gives you a baseline performance for [**DrawPrimitive**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-drawprimitive?branch=master) that can be used to build from. To profile one state change, follow these additional tips:
...
3.  For state changes that significantly change the pipeline (like [**SetTexture**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-settexture?branch=master)), subtract the difference between the two sequences to get the time for state change.
4.  For state changes that significantly change the pipeline (and therefore require toggling states like [**SetRenderState**](/windows/win32/d3d9helper/nf-d3d9-idirect3ddevice9-setrenderstate?branch=master)), subtract the difference between the render sequences and divide by 2. This will generate the average number of cycles for each state change.
