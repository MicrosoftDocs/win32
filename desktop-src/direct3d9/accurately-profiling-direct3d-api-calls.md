...
In other words, it takes about 4568 clock cycles to process [**SetTexture**](idirect3ddevice9--settexture.md) and [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) on this 2 GHz machine. You could convert these values to the actual time it took to execute all the calls like this:
...
Now, look at what happens for the same example when a mode transition occurs. This time, assume [**SetTexture**](idirect3ddevice9--settexture.md) and [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) cause a mode transition. Once again, an off-the-shelf instrumenting profiler could return results similar to these:
...
The time measured for [**SetTexture**](idirect3ddevice9--settexture.md) is about the same, however, the dramatic increase in the amount of time spent in [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) is due to the mode transition. Here's what is happening:

1.  Assume the command buffer has room for one command before our render sequence starts.
2.  [**SetTexture**](idirect3ddevice9--settexture.md) is converted to a device-independent format and added to the command buffer. In this scenario, this call fills the command buffer.
3.  The runtime tries to add [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) to the command buffer but cannot, because it is full. Instead, the runtime empties the command buffer. This causes the kernel-mode transition. Assume the transition takes about 5000 cycles. This time contributes to time spent in **DrawPrimitive**.
4.  The driver then processes the work associated with all the commands that were emptied from the command buffer. Assume that the driver time to process the commands that nearly filled the command buffer is about 935,000 cycles. Assume that the driver work associated with [**SetTexture**](idirect3ddevice9--settexture.md) is about 2750 cycles. This time contributes to time spent in [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md).
...
6.  The render sequence finishes by converting [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) and adding it to the command buffer. Assume this takes about 900 cycles. This time contributes to time spent in **DrawPrimitive**.
...
Just like the measurement for [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) without the mode transition (900 cycles), the measurement for **DrawPrimitive** with the mode transition (947,950 cycles) is accurate but useless in terms of budgeting CPU work. The result contains the correct runtime work, the driver work for [**SetTexture**](idirect3ddevice9--settexture.md), the driver work for any commands that preceded **SetTexture**, and two mode transitions. However, the measurement is missing the **DrawPrimitive** driver work.
...
2.  Add a query event marker to the command buffer by calling [**Issue**](idirect3dquery9--issue.md)([**D3DISSUE\_END**](d3dissue-end.md)). This marker tells the driver to track when the GPU finishes executing whatever commands preceded the marker.
3.  The first call empties the command buffer because calling [**GetData**](idirect3dquery9--getdata.md) with [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md) forces the command buffer to be emptied. Each subsequent call is checking the GPU to see when it finishes processing all the command-buffer work. This loop does not return S\_OK until the GPU is idle.
...
7.  The first call empties the command buffer because calling [**GetData**](idirect3dquery9--getdata.md) with [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md) forces the command buffer to be emptied. When the GPU finishes processing all the command-buffer work, **GetData** returns S\_OK, and the loop is exited because the GPU is idle.
...
1.  The first call empties the command buffer by calling [**GetData**](idirect3dquery9--getdata.md) with [**D3DGETDATA\_FLUSH**](d3dgetdata-flush.md). When the GPU finishes processing all the command-buffer work, **GetData** returns S\_OK, and the loop is exited because the GPU is idle.
2.  The render sequence starts by converting [**SetTexture**](idirect3ddevice9--settexture.md) to a device-independent format and adding it to the command buffer. Assume this takes about 100 cycles.
3.  [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) is converted and added to the command buffer. Assume this takes about 900 cycles.
4.  [**Issue**](idirect3dquery9--issue.md) adds a query marker to the command buffer. Assume this takes about 200 cycles.
5.  [**GetData**](idirect3dquery9--getdata.md) causes the command buffer to be emptied which forces the kernel-mode transition. Assume this takes about 5000 cycles.
6.  The driver then processes the work associated with all four calls. Assume that the driver time to process [**SetTexture**](idirect3ddevice9--settexture.md) is about 2964 cycles, [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) is about 3600 cycles, [**Issue**](idirect3dquery9--issue.md) is about 200 cycles. So the total driver time for all four commands is about 6450 cycles.
    > [!Note]  
    > The driver also takes a little time to see what the status of the GPU is. Because the GPU work is trivial, the GPU should be done already. [**GetData**](idirect3dquery9--getdata.md) will return S\_OK based on the likelihood that the GPU is finished.
...
The numbers for [**GetData**](idirect3dquery9--getdata.md) include:
...
-   When one of the lock methods ([**Lock**](idirect3dvertexbuffer9--lock.md)) is called on a vertex buffer, index buffer, or texture (under certain conditions with certain flags).
...
-   When [**ValidateDevice**](idirect3ddevice9--validatedevice.md) is called.
-   When [**Present**](idirect3ddevice9--present.md) is called.
-   When the command buffer fills up.
-   When [**GetData**](idirect3dquery9--getdata.md) is called with D3DGETDATA\_FLUSH.
...
So far, this paper has shown how to profile a render sequence. Each render sequence has been fairly simple, containing a single [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) call and a [**SetTexture**](idirect3ddevice9--settexture.md) call. This was done to focus on the command buffer and the use of the query mechanism to control it. Here is a brief summary of how to profile an arbitrary render sequence:
...
All of these techniques are used to profile state changes. Assuming that you have read and understood how to control the command buffer, and have successfully completed baseline measurements on [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md), you are ready to add state changes to your render sequences. There are a few additional profiling challenges when adding state changes to a render sequence. If you intend to add state changes to your render sequences, be sure to continue into the next section.
...
1.  Profile [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) first.
...
Starting with a render sequence that contains [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md), here is the code sequence for measuring the cost of adding [**SetTexture**](idirect3ddevice9--settexture.md):
...
Notice that the loop contains two calls, [**SetTexture**](idirect3ddevice9--settexture.md) and [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md). The render sequence loops 1500 times and generates results similar to these:
...
Each iteration of the loop contains a state change and a draw call. Subtracting out the result of the [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) render sequence leaves:
...
This is the average number of cycles to add [**SetTexture**](idirect3ddevice9--settexture.md) to this render sequence. This same technique can be applied to other state changes.

Why is [**SetTexture**](idirect3ddevice9--settexture.md) called a simple state change? Because the state that is being set is constrained so that the pipeline does the same amount of work each time the state is changed. Constraining both textures to the same size and format assures the same amount of work for each **SetTexture** call.
...
2.  Profile a second render sequence that toggles the state change. The render sequence loop contains:
    ...
    -   [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) just like the original sequence.
    -   A state change to set the state into a "true" condition.
    -   A second [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) to force the second state change to be realized.
3.  Find the difference between the two render sequences. This is done by:
    -   Multiply the baseline [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) sequence by 2 because there are two **DrawPrimitive** calls in the new sequence.
...
The loop toggles the state by executing two [**SetRenderState**](idirect3ddevice9--setrenderstate.md) calls. The first **SetRenderState** call disables z-testing and the second **SetRenderState** enables z-testing. Each **SetRenderState** is followed by [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) so that the work associated with the state change is processed by the driver instead of only setting a dirty bit in the driver.
...
Profiling either one of these state changes in a render sequence with [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) generates results similar to these:
...
In general, the runtime will concatenate two or more back-to-back [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) calls when:

1.  The primitive type is a triangle list (D3DPT\_TRIANGLELIST).
2.  Each successive [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) call must reference consecutive vertices within the vertex buffer.

Similarly, the right conditions for concatenating two or more back-to-back [**DrawIndexedPrimitive**](idirect3ddevice9--drawindexedprimitive.md) calls are:

1.  The primitive type is a triangle list (D3DPT\_TRIANGLELIST).
2.  Each successive [**DrawIndexedPrimitive**](idirect3ddevice9--drawindexedprimitive.md) call must sequential reference consecutive indices within the index buffer.
3.  Each successive [**DrawIndexedPrimitive**](idirect3ddevice9--drawindexedprimitive.md) call must use the same value for BaseVertexIndex.
...
-   When the previous call is [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md), if the next draw call:
    -   uses a triangle list, AND
    -   specifies the StartVertex = previous StartVertex + previous PrimitiveCount \* 3
-   When using [**DrawIndexedPrimitive**](idirect3ddevice9--drawindexedprimitive.md), if the next draw call:
    -   uses a triangle list, AND
...
The loop iterates through 1500 triangles, setting a texture and drawing each triangle. This render loop takes approximately 2750 cycles for [**SetTexture**](idirect3ddevice9--settexture.md) and 1100 cycles for [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) as shown in previous sections. You might intuitively expect that moving **SetTexture** outside the render loop should reduce the amount of work done by the driver by 1500 \* 2750 cycles, which is the amount of work associated with calling **SetTexture** 1500 times. The code snippet would look like this:
...
This gives you a baseline performance for [**DrawPrimitive**](idirect3ddevice9--drawprimitive.md) that can be used to build from. To profile one state change, follow these additional tips:
...
3.  For state changes that significantly change the pipeline (like [**SetTexture**](idirect3ddevice9--settexture.md)), subtract the difference between the two sequences to get the time for state change.
4.  For state changes that significantly change the pipeline (and therefore require toggling states like [**SetRenderState**](idirect3ddevice9--setrenderstate.md)), subtract the difference between the render sequences and divide by 2. This will generate the average number of cycles for each state change.
