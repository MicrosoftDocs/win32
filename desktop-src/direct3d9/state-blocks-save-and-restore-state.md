...
To save a custom state sequence, wrap the state that you want to save in a [**IDirect3DDevice9::BeginStateBlock**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-beginstateblock) and [**IDirect3DDevice9::EndStateBlock**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3ddevice9-endstateblock) pair. BeginStateBlock tells the current device to set up a state block and add to it every state change that occurs until EndStateBlock is called. Here's an example:
...
This will save any number of state changes in any sequence to a custom stateblock. Later, when you want to use the stateblock to reset the device state, call [**IDirect3DStateBlock9::Apply**](/windows/desktop/api/d3d9helper/nf-d3d9-idirect3dstateblock9-apply). This will overwrite only the device state that has been captured in the state block. Any other device state that was not captured with the custom stateblock will not be changed. Once again, since the stateblock object is an interface, you will need to release it when you are done with it.
