...
The [**IDirect3DDevice9**](idirect3ddevice9.md) interface supports vertex processing in both software and hardware. In general, the device capabilities for software and hardware vertex processing are not identical. Hardware capabilities are variable, depending on the display adapter and driver, while software capabilities are fixed.
...
The VertexProcessingCaps member of the [**D3DCAPS9**](d3dcaps9.md) structure determines the hardware vertex processing capabilities of the device.
...
In addition, the following table lists the values that are set for members of the [**D3DCAPS9**](d3dcaps9.md) structure for a device in software vertex processing mode.
