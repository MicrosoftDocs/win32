...
Here is an example of creating an effect taken from the [BasicHLSL Sample](nodepage.directx_sdk__august_2009_). The effect creation code for creating a debug shader is from **OnCreateDevice**:
...
> [!Note]  
> Beginning with the December 2006 SDK, the DirectX 10 HLSL compiler is now the default compiler in both DirectX 9 and DirectX 10. See [Effect-Compiler Tool](direct3dtools.fxc) for details.
...
-   Can be associated with [Techniques and Passes (Direct3D 9)](techniques-and-passes.md) and top-level [Effect Parameters](b4fc7cf8-3edd-88e6-d215-455a0b09cbfa).
...
You can compile an effect at run time with [**D3DXCreateEffect**](d3dxcreateeffect.md), or you can compile an effect offline using the command-line compiler tool fxc.exe. The effect in the [CompiledEffect Sample](035e6d80-6520-4252-a9fe-9556f0cf5906) contains a vertex shader, a pixel shader, and one technique:
...
Using [Effect-Compiler Tool](direct3dtools.fxc) to compile the shader for vs\_1\_1 generated the following assembly shader instructions:
...
Using [Effect-Compiler Tool](direct3dtools.fxc) to compile the shader for vs\_1\_1 generates the following assembly instructions:
...
In the [EffectParam Sample](f5f321e1-d364-11b9-8a9f-e12c31a4ec8a), the parameter block is applied in the render sequence:
