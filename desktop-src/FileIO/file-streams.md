...
Each stream also maintains its own state for compression, encryption, and sparseness. The **FILE\_ATTRIBUTE\_SPARSE\_FILE** attribute on the file is set in the **dwFileAttributes** member of the [**WIN32\_FIND\_DATA**](win32-find-data-str.md) structure returned from the [**FindFirstFile**](findfirstfile.md), [**FindFirstFileEx**](findfirstfileex.md), and [**FindNextFile**](findnextfile.md) functions if any of the streams have ever been sparse. [**GetFileAttributes**](getfileattributes.md), [**GetFileAttributesEx**](getfileattributesex.md), [**GetFileAttributesTransacted**](getfileattributestransacted.md), [**GetFileInformationByHandle**](getfileinformationbyhandle.md), and [**GetFileInformationByHandleEx**](getfileinformationbyhandleex.md) return the sparse state of the default data stream if no stream is specified.
...
| ::$DATA                    | Data stream. The default data stream has no name. Data streams can be enumerated using the [**FindFirstStreamW**](findfirststreamw.md) and [**FindNextStreamW**](findnextstreamw.md) functions.                                                                                                                                                                                                                                                |
