...
To dequeue I/O completion packets one at a time, use the [**GetQueuedCompletionStatus**](getqueuedcompletionstatus.md) function.
...
On input, points to a pre-allocated array of [**OVERLAPPED\_ENTRY**](overlapped-entry.md) structures.

On output, receives an array of [**OVERLAPPED\_ENTRY**](overlapped-entry.md) structures that hold the entries. The number of array elements is provided by *ulNumEntriesRemoved*.
...
A completion routine is queued when the [**ReadFileEx**](readfileex.md) or [**WriteFileEx**](writefileex.md) function in which it was specified has completed, and the calling thread is the thread that initiated the operation. An APC is queued when you call [**QueueUserAPC**](https://msdn.microsoft.com/library/windows/desktop/ms684954).
...
If a call to [**GetQueuedCompletionStatusEx**](getqueuedcompletionstatus.md) fails because the handle associated with it is closed, the function returns **FALSE** and [**GetLastError**](https://msdn.microsoft.com/library/windows/desktop/ms679360) will return **ERROR\_ABANDONED\_WAIT\_0**.
