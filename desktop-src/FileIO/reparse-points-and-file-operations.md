...
-   Applications that use the [**CreateFile**](/windows/win32/FileAPI/nf-fileapi-createfilea?branch=master) function should specify the **FILE\_FLAG\_OPEN\_REPARSE\_POINT** flag when opening the file if it is a reparse point. For more information, see [Creating and Opening Files](creating-and-opening-files.md).
...
-   The [**OpenFileById**](/windows/win32/WinBase/nf-winbase-openfilebyid?branch=master) function will either open the file or the reparse point, depending on the use of the **FILE\_FLAG\_OPEN\_REPARSE\_POINT** flag.
...
-   Volume management activities for reading update sequence number (USN) change journal records require special handling for reparse points when using the [**USN\_RECORD**](/windows/win32/WinIoCtl/ns-winioctl-usn_record_v2?branch=master) and [**READ\_USN\_JOURNAL\_DATA**](/windows/win32/WinIoCtl/ns-winioctl-read_usn_journal_data_v0?branch=master) structures.
