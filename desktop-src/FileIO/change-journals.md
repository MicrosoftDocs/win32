...
| [Obtaining a Volume Handle for Change Journal Operations](obtaining-a-volume-handle-for-change-journal-operations.md)<br/> | To obtain a handle to a volume for use with update sequence number (USN) change journal operations, call the [**CreateFile**](createfile.md) function with the *lpFileName* parameter set to a string of the following form: \\\\.\\*X*.<br/> |
