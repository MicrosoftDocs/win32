...
If a file or device is opened for synchronous I/O (that is, **FILE\_FLAG\_OVERLAPPED** is not specified), subsequent calls to functions such as [**WriteFile**](writefile.md) can block execution of the calling thread until one of the following events occurs:
...
A process opens a file for asynchronous I/O in its call to [**CreateFile**](createfile.md) by specifying the **FILE\_FLAG\_OVERLAPPED** flag in the *dwFlagsAndAttributes* parameter. If **FILE\_FLAG\_OVERLAPPED** is not specified, the file is opened for synchronous I/O. When the file has been opened for asynchronous I/O, a pointer to an [**OVERLAPPED**](overlapped-entry.md) structure is passed into the call to [**ReadFile**](readfile.md) and [**WriteFile**](writefile.md). When performing synchronous I/O, this structure is not required in calls to **ReadFile** and **WriteFile**.

> [!Note]  
> If a file or device is opened for asynchronous I/O, subsequent calls to functions such as [**WriteFile**](writefile.md) using that handle generally return immediately but can also behave synchronously with respect to blocked execution. For more information, see [http://support.microsoft.com/kb/156932](Http://go.microsoft.com/fwlink/p/?linkid=116863).

 

Although [**CreateFile**](createfile.md) is the most common function to use for opening files, disk volumes, anonymous pipes, and other similar devices, I/O operations can also be performed using a handle *typecast* from other system objects such as a socket created by the [**socket**](https://msdn.microsoft.com/library/windows/desktop/ms740506) or [**accept**](https://msdn.microsoft.com/library/windows/desktop/ms737526) functions.

Handles to directory objects are obtained by calling the [**CreateFile**](createfile.md) function with the **FILE\_FLAG\_BACKUP\_SEMANTICS** attribute. Directory handles are almost never used—backup applications are one of the few applications that will typically use them.

After opening the file object for asynchronous I/O, an [**OVERLAPPED**](https://msdn.microsoft.com/library/windows/desktop/ms684342) structure must be properly created, initialized, and passed into each call to functions such as [**ReadFile**](readfile.md) and [**WriteFile**](writefile.md). Keep the following in mind when using the [**OVERLAPPED**](overlapped-entry.md) structure in asynchronous read and write operations:
...
-   If you declare your pointer to the [**OVERLAPPED**](https://msdn.microsoft.com/library/windows/desktop/ms684342) structure as a local variable, do not exit the local function until all asynchronous I/O operations to the file object have been completed. If the local function is exited prematurely, the **OVERLAPPED** structure will go out of scope and it will be inaccessible to any [**ReadFile**](readfile.md) or [**WriteFile**](writefile.md) functions it encounters outside of that function.
...
The [**ReadFileEx**](readfileex.md) and [**WriteFileEx**](writefileex.md) functions enable an application to specify a routine to execute (see [**FileIOCompletionRoutine**](fileiocompletionroutine.md)) when the asynchronous I/O request is completed.
