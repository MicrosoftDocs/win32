...
Characters that are not implemented in Tahoma or Microsoft Sans Serif may be implemented in other Windows fonts that are used for text display in user interfaces. Depending on what controls or APIs are used to display text, various mechanisms such as [font linking](https://msdn.microsoft.com/en-us/goglobal/bb688134.aspx) may be used by the system to automatically select such fonts for displaying those characters.
