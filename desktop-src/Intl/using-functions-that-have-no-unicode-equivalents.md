...
Functions that have not been implemented with a [Unicode](unicode.md) version have typically been replaced by more powerful or extended functions that do support Unicode. For example, if you are porting code that calls the [**OpenFile**](fs.openfile) function, your application can support Unicode by using the [**CreateFile**](fs.createfile) function instead.
