...
All supplemental locales use the [LOCALE\_CUSTOM\_UNSPECIFIED](locale-custom-constants.md) constant for the [locale identifier](locale-identifiers.md). As a rule, [**GetLocaleInfo**](getlocaleinfo.md) cannot distinguish among supplemental locales, but [**GetLocaleInfoEx**](getlocaleinfoex.md) can because it uses [locale names](locale-names.md) instead of locale identifiers. Your application can retrieve information about a particular supplemental locale only when that locale is the currently selected user locale. Then, the application can call [**GetLocaleInfo**](getlocaleinfo.md) and pass the constant [LOCALE\_USER\_DEFAULT](locale-user-default.md) as the locale identifier.
...
For replacement locales, your application should generally consult locale-specific information instead of attempting a "shortcut" based on knowledge of a certain locale. For example, when [**GetThreadLocale**](getthreadlocale.md) retrieves the current locale as English (United States), it might actually be a replacement locale that should be allowed to take effect.
