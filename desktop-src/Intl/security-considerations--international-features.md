...
-   [lstrcmpi](https://www.bing.com/search?q=lstrcmpi). Compares two character strings according to the rules of the locale, without case-sensitivity. The function compares the strings by checking the first characters against each other, the second characters against each other, and so on, until it finds an inequality or reaches the ends of the strings.
-   [lstrcmp](https://www.bing.com/search?q=lstrcmp). Compares strings using techniques similar to those of [lstrcmpi](https://www.bing.com/search?q=lstrcmpi). The only difference is that [lstrcmp](https://www.bing.com/search?q=lstrcmp) performs a case-sensitive string comparison.
-   [**CompareString**](/windows/desktop/api/Winnls/nf-stringapiset-comparestringw), [**CompareStringEx**](/windows/desktop/api/Stringapiset/nf-stringapiset-comparestringex) (Windows Vista and later). Perform a string comparison on an application-supplied locale. [**CompareStringEx**](/windows/desktop/api/Stringapiset/nf-stringapiset-comparestringex) is similar to [**CompareString**](/windows/desktop/api/Winnls/nf-stringapiset-comparestringw), but it identifies a locale by [locale name](locale-names.md) instead of [locale identifier](locale-identifiers.md). These functions are similar to [lstrcmpi](https://www.bing.com/search?q=lstrcmpi) and [lstrcmp](https://www.bing.com/search?q=lstrcmp) except that they operate on a specific locale instead of a user-selected locale.
...
Like [lstrcmpi](https://www.bing.com/search?q=lstrcmpi) and [lstrcmp](https://www.bing.com/search?q=lstrcmp), [**CompareString**](/windows/desktop/api/Winnls/nf-stringapiset-comparestringw) evaluates strings character by character. However, many languages have multiple-character elements, for example, the two-character element "CH" in traditional Spanish. Because [**CompareString**](/windows/desktop/api/Winnls/nf-stringapiset-comparestringw) uses the locale furnished by the application to identify multiple-character elements and [lstrcmpi](https://www.bing.com/search?q=lstrcmpi) and [lstrcmp](https://www.bing.com/search?q=lstrcmp) use the thread locale, identical strings might not compare as equal.

[**CompareString**](/windows/desktop/api/Winnls/nf-stringapiset-comparestringw) ignores undefined characters, and thus returns zero (indicating equal strings) for many string pairs that are quite distinct. A string might contain values that do not map to any character or it might contain characters with semantics outside the domain of the application, such as control characters in a URL. Applications using this function should provide error handlers and test strings to make sure that they are valid before using them.

> [!Note]  
> For Windows Vista and later, [**CompareStringEx**](/windows/desktop/api/Stringapiset/nf-stringapiset-comparestringex) is similar to [**CompareString**](/windows/desktop/api/Winnls/nf-stringapiset-comparestringw). The security issues are identical for these functions.
