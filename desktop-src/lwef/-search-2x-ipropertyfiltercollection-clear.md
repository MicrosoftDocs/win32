---
title: IPropertyFilterCollection clear property
description: Clears the collection.
ms.assetid: '99dbedee-bd21-4e82-a2d8-a09c06e9cd5f'
keywords: ["clear property Legacy Windows Environment Features", "clear property Legacy Windows Environment Features , IPropertyFilterCollection interface", "IPropertyFilterCollection interface Legacy Windows Environment Features , clear property"]
topic_type:
- apiref
api_name:
- IPropertyFilterCollection.clear
api_location:
- WdsSharedIDL.h
api_type:
- COM
---

# IPropertyFilterCollection::clear property

\[Windows Search 2.x is available for use in the operating system specified in the Requirements section. It might be altered or unavailable in later versions. Use the [Windows Search API](https://msdn.microsoft.com/library/windows/desktop/ee872061) instead.\]

Clears the collection.

## Syntax

## Property value

## Requirements



|                                     |                                                                                           |
|-------------------------------------|-------------------------------------------------------------------------------------------|
| Minimum supported client<br/> | Windows XP with SP2 \[desktop apps only\]<br/>                                      |
| Minimum supported server<br/> | Windows Server 2003 with SP1 \[desktop apps only\]<br/>                             |
| Redistributable<br/>          | Windows Desktop Search (WDS) 2.6.5<br/>                                             |
| Header<br/>                   | <dl> <dt>WdsSharedIDL.h</dt> </dl> |



 

 





