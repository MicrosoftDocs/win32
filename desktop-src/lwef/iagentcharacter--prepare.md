...
Address of a variable that receives the [**Prepare**](https://msdn.microsoft.com/library/windows/desktop/ms699189) request ID.

</dd> </dl>

If you load a character using the HTTP protocol (an .ACF file), you must use the [**Prepare**](https://msdn.microsoft.com/library/windows/desktop/ms699189) method to retrieve animation data before you can play the animation. You cannot use this method if you loaded the character using the UNC protocol (an .ACS file). You also cannot retrieve HTTP data for a character using **Prepare** if you loaded that character using the UNC protocol (.ACS character file).

Animation or sound data retrieved with the [**Prepare**](https://msdn.microsoft.com/library/windows/desktop/ms699189) method is stored in the browser's cache. Subsequent calls will check the cache, and if the animation data is already there, the control loads the data directly from the cache. Once loaded, the animation or sound data can be played with the [**Play**](https://msdn.microsoft.com/library/windows/desktop/ms697914) or [**Speak**](https://msdn.microsoft.com/library/windows/desktop/ms697891) methods.
