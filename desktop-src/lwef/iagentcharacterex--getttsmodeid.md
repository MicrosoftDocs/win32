...
This setting returns the TTS (text-to-speech) engine mode ID for a character's spoken output. The mode ID for a TTS engine is a string representation of the GUID (formatted with braces and dashes) defined by the speech vendor uniquely identifying the engine. For more information, see the [Microsoft Speech SDK documentation](http://go.microsoft.com/fwlink/p/?linkid=198367). Querying this property will load the associated engine if it is not already loaded.
...
This function does not fail if the [**IAgentAudioObjectProperties::GetEnabled**](lwef-iagentaudioobjectproperties__getenabled) returns **False**.
