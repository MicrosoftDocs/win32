...
-   Verify that Microsoft Agent's use of MIDI is not blocking the audio channel (see [Applications that play MIDI have no audio output when Microsoft Agent is running](output-problems.md#applications-that-play-midi-have-no-audio-output-when-microsoft-agent-is-running)" in the Output Problems section).
