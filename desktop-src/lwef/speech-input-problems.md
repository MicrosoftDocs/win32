...
-   Verify that Microsoft Agent's use of MIDI is not blocking the audio channel (see [Applications that play MIDI have no audio output when Microsoft Agent is running](https://www.bing.com/search?q=Applications that play MIDI have no audio output when Microsoft Agent is running)" in the Output Problems section).
