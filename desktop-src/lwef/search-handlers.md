...
The general procedures for implementing and registering a Shell extension handler are discussed in [Creating Shell Extension Handlers](https://msdn.microsoft.com/library/windows/desktop/cc144067). This document focuses on those aspects of implementation that are specific to search handlers.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Search handlers are implemented as a type of [shortcut menu handler](https://msdn.microsoft.com/library/windows/desktop/cc144171).                                                                                                                     | Search handlers can be implemented as shortcut menu handlers, or as Dynamic HTML (DHTML) documents.                                                                                                                                                                                                               |
...
| Search handlers appear on the **Find** submenu of the **Start** menu and the **Find** submenu of the Windows Explorer **Tools** menu.                                                                                  | Search handlers appear only on the **Search** submenu of the **Start** menu. To make a custom search pane available through the Windows Explorer menu bar, you must implement it as a [band object](https://msdn.microsoft.com/library/windows/desktop/cc144099). It is then listed on the **Explorer Bar** submenu of the Windows Explorer **View** menu. |
...
From this point, the registration procedure depends on whether the handler is to be static or dynamic. For a general discussion of how to register Shell extension handlers, see [Creating Shell Extension Handlers](https://msdn.microsoft.com/library/windows/desktop/cc144067).
...
Unlike static search handlers, you do not specify the menu text in the registry. When the handler is loaded, the Shell will call the handler's [**IContextMenu::QueryContextMenu**](https://msdn.microsoft.com/library/windows/desktop/bb776097) method to add items to the **Find** or **Search** submenu.
...
For a general discussion of how to implement shortcut menu handlers, see [Creating Context Menu Handlers](https://msdn.microsoft.com/library/windows/desktop/cc144171). Search handlers differ from standard shortcut menu handlers in only a few ways.
...
-   When the user launches the handler's menu item, the Shell loads the handler's DLL and calls [**IContextMenu::InvokeCommand**](https://msdn.microsoft.com/library/windows/desktop/bb776096) to notify the handler to launch the search engine. The [**IShellExtInit::Initialize**](https://msdn.microsoft.com/library/windows/desktop/bb775094) and [**IContextMenu::QueryContextMenu**](https://msdn.microsoft.com/library/windows/desktop/bb776097) methods are not called.
-   When [**IContextMenu::InvokeCommand**](https://msdn.microsoft.com/library/windows/desktop/bb776096) is called, the **lpVerb** member of the [**CMINVOKECOMMANDINFO**](https://msdn.microsoft.com/library/windows/desktop/bb773215) structure that is passed in identifies the command. The low-order word of **lpVerb** is set to the numerical equivalent of the command's subkey name. Since this subkey is normally named **0**, **lpVerb** is usually set to zero. The handler should then launch the search engine.

Dynamic search handlers are implemented in much the same way as normal shortcut menu handlers. The primary exception is that when [**IShellExtInit::Initialize**](https://msdn.microsoft.com/library/windows/desktop/bb775094) is called, the *pidlFolder* and *lpdobj* arguments are set to **NULL**.
