...
A window type is used to display a help topic and to provide users with a way to navigate through a compiled help (.chm) file. A help window is based on an [**HH\_WINTYPE**](/previous-versions/windows/desktop/api/HtmlHelp/ns-htmlhelp-taghh_wintype) structure and hosts a Microsoft Internet Explorer DLL (Shdocvw.dll) to display a help topic.
