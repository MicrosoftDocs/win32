...
Time, in seconds, a DNS Server forwarding a query will wait for resolution from the [*forwarder*](https://www.bing.com/search?q=*forwarder*) before attempting to resolve the query itself.
...
Indicates whether the DNS Server parses [*zone files*](https://www.bing.com/search?q=*zone files*) strictly. If undefined or zero, the server will log and ignore bad data in the zone file and continue to load. If nonzero, the server will log and fail on zone file errors.
