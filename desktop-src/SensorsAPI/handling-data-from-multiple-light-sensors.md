...
The following example code shows an implementation for the [**OnDataUpdated**](https://www.bing.com/search?q=**OnDataUpdated**) event. The event handler calls a helper function, named **UpdateUI**, that changes the user interface based on the lux value. Writing the implementation of UpdateUI is up to you.
