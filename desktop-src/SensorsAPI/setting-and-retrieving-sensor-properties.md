...
To set one or more values for properties, call [**ISensor::SetProperties**](https://msdn.microsoft.com/en-us/library/Dd318899(v=VS.85).aspx). You provide this method with an [IPortableDeviceValues](http://go.microsoft.com/fwlink/p/?linkid=134660) pointer that contains the collection of properties to set, and their associated values. The method returns a corresponding [IPortableDeviceValues](http://go.microsoft.com/fwlink/p/?linkid=134660) interface that may contain error codes for properties that could not be set.
