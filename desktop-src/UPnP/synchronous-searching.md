...
The [**IUPnPDeviceFinder::FindByType**](iupnpdevicefinder-findbytype.md) method searches by device or service type. This method takes a type URI as an input parameter and returns a collection of Device objects. A Device object represents an individual device.

The following examples demonstrate how to perform a synchronous search for devices in VBScript. Each script uses [**IUPnPDeviceFinder::FindByType**](iupnpdevicefinder-findbytype.md), called with the type URI (service ID) for the media player device type, *urn:schemas-upnp-org:device:mediaplayer.v1.00.00*. The method returns a collection of Device objects that corresponds to media player devices that were found. For information on extracting individual device objects from a collection, see [Device Collections Returned By Synchronous Searches](device-collections-returned-by-synchronous-searches.md).
...
The following example demonstrates a synchronous search for media player devices using C++. The function takes a pointer to the [**IUPnPDeviceFinder**](iupnpdevicefinder.md) interface as input, and returns the [**IUPnPDevices**](iupnpdevices.md) interface pointer.

The example first allocates a **BSTR** to represent the device type URI and then passes this to the [**IUPnPDeviceFinder::FindByType**](iupnpdevicefinder-findbytype.md) method. It also passes the address of a local [**IUPnPDevices**](iupnpdevices.md) pointer in the buffer in which the method then stores the collection of devices found. If the function call is successful, it returns the pointer to this collection.
