...
A pointer to an endpoint information object, an [**IUPnPRemoteEndpointInfo**](iupnpremoteendpointinfo.md) interface, is passed through this new parameter by the device host. The **IUPnPRemoteEndpointInfo** interface provides three methods for accessing the endpoint information. You must call the appropriate method to get the endpoint information that is required by the service implementation.
...
Device control objects must implement the [**IUPnPDeviceControl**](iupnpdevicecontrol.md) interface. The device host invokes the [**IUPnPDeviceControl::Initialize**](iupnpdevicecontrol-initialize.md) method of the device control object, passing in the UPnP-based device description that the device host previously published for the device and an initialization string that was specified at registration time (see [Registering a Hosted Device with the Device Host](registering-a-hosted-device-with-the-device-host.md)). From this device description, the device control object reads the UDNs assigned to each of the devices in the device tree. You can also use the [**IUPnPRegistrar::GetUniqueDeviceName**](iupnpregistrar-getuniquedevicename.md) method to obtain this information.

When the device host requires a pointer to a service object that implements a particular service, it invokes the [**IUPnPDeviceControl::GetServiceObject**](iupnpdevicecontrol-getserviceobject.md) method on the device control object. The device host passes the UDN and the service ID of the service for which it is requesting a service object and the address of an [**IDispatch**](ebbff4bc-36b2-4861-9efa-ffa45e013eb5) pointer at which the method is expected to return a pointer to the service object. The UDN parameter is necessary because the device control object manages services for the entire device tree, including nested devices. If the device host requests a nested device, **GetServiceObject** uses the UDN to identify it.
