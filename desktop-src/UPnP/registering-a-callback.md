...
> [!Note]  
> Adding a callback in Visual Basic is different from the method used in VBScript. The **GetRef** function used in the VBScript is not available in Visual Basic. Therefore, a developer must create an [**IDispatch**](https://msdn.microsoft.com/en-us/library/ms221608(v=VS.71).aspx) object that has the callback function as the default method. See [Registering a Callback in Visual Basic](registering-a-callback-in-visual-basic.md).
