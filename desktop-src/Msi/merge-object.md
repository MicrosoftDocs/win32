...
Mergemod.dll provides access to the extended functionality at build time through a second version of the existing CLSID. This CLSID supports existing functionality available through the [**IMsmMerge**](/windows/win32/Mergemod/?branch=master) interface, but the default interface on the object (and the associated dual interface) is the [**IMsmMerge2**](/windows/win32/Mergemod/nn-mergemod-imsmmerge2?branch=master) interface instead of the **IMsmMerge** interface.
