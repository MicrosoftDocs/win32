...
> [!Note]  
> To use COM you need to call [**CoInitializeEx**](https://www.bing.com/search?q=**CoInitializeEx**) in the custom action. Do not quit if you find that the thread has already been initialized. For example, the thread is initialized in a per-machine installation but not in a per-user installation.
