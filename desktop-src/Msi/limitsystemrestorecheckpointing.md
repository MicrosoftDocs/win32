...
This policy affects only checkpoints set by Windows Installer. On Windows XP computers, administrators may decide to disable checkpointing from within Windows Installer to improve performance. [**System Restore**](https://msdn.microsoft.com/library/Dd408121(v=VS.85).aspx) also creates additional checkpoints. For more information, see [System Restore Points and the Windows Installer](system-restore-points-and-the-windows-installer.md) and [Setting a Restore Point from a Custom Action](setting-a-restore-point-from-a-custom-action.md).
