...
This property is commonly used by administrators for users who roam between environments that do and do not support advertising. This property can be set by a transform, in the administrative stream, or in the [Property table](property-table.md). If it is set using the command line or by a call to [**MsiSetProperty**](/windows/win32/Msiquery/nf-msiquery-msisetpropertya?branch=master), then it must be reset again during each subsequent installation.
