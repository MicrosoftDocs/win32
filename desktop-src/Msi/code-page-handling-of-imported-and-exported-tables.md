...
The ASCII [Text Archive Files](text-archive-files.md) exported by [**MsiDatabaseExport**](msidatabaseexport.md) are explained in the following format:
...
-   If a table contains streams, [**MsiDatabaseExport**](msidatabaseexport.md) exports each stream in the table to a separate file.
...
-   If a [Text Archive File](text-archive-files.md) with a non-neutral code page is imported into a database that has a different non-neutral code page, the Installer returns an error when [**MsiDatabaseImport**](msidatabaseimport.md) is called.
...
The [Text Archive Files](text-archive-files.md) that are exported by [**MsiDatabaseExport**](msidatabaseexport.md) can be used with version control systems. Use the [Database Functions](database-functions.md) or a database table editor to edit the database.
