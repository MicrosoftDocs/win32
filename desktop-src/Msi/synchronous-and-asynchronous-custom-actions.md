...
In particular, calls to [**MsiGetTargetPath**](msigettargetpath.md) and [**MsiSetTargetPath**](msisettargetpath.md) should be avoided in asynchronous custom actions. Instead use [**MsiGetProperty**](msigetproperty.md) to obtain a target path. Bulk database operations such as import, export, and transform operations should be avoided in any type of custom action.
