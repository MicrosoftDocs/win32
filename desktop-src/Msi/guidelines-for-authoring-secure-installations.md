...
-   When the installation requires the installer to use [*elevated*](e-gly.md#-msi-elevated-gly) privileges, use [Restricted Public Properties](restricted-public-properties.md) to restrict the public properties a user can change. Some restrictions are commonly necessary to maintain a secure environment when the installation requires the installer to use *elevated* privileges.
...
-   The installer runs custom actions with user privileges by default in order to limit the access of custom actions to the system. The installer may run custom actions with [*elevated*](e-gly.md#-msi-elevated-gly) privileges if a managed application is being installed or if the system policy has been specified for elevated privileges. For details, see [Custom Action Security](custom-action-security.md).
