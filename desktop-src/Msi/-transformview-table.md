...
To invoke the transform view mode, obtain a handle and open the reference database. See [Obtaining a Database Handle](obtaining-a-database-handle.md). Call [**MsiDatabaseApplyTransform**](msidatabaseapplytransform.md) with MSITRANSFORM\_ERROR\_VIEWTRANSFORM. This stops the transform from being applied to the database and dumps the transform contents into the \_TransformView table. The data in the table can be accessed using SQL queries. See [Working with Queries](working-with-queries.md).
