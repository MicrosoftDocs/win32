...
Windows Installer 4.5 does not support some services configuration options that can enable a package to customize the [services](base.services) on a computer. For more information, see [Using Services Configuration](using-services-configuration.md).

Windows Installer 4.5 does not support some features that enable the Windows Installer to secure new accounts, Windows [Services](base.services), files, folders, and registry keys. For information, see [Securing Resources](securing-resources-.md).
