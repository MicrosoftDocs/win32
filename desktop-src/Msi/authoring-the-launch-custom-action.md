...
The source code for a sample custom action named Launch, which meets the sample specifications, is provided by the Windows Installer SDK as the file Tutorial.cpp. This custom action makes use of [**MsiFormatRecord**](msiformatrecord.md) to format a string containing properties. The property \[\#FileKey\] resolves to the full path of the HTML file. Use the source file to create the file Tutorial.dll. The entry point to this DLL is LaunchTutorial.
