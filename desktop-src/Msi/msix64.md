...
The **Msix64** property is defined only if running on an x64 processor. The value is set by the installer to the numeric processor level. The values are the same as the **wProcessorLevel** field of the [**SYSTEM\_INFO**](base.system_info_str) structure.
