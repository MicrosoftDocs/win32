...
-   To disable [Registry Reflection](https://msdn.microsoft.com/library/Aa384235(v=VS.85).aspx) for registry keys that are affected by a particular component, set the **msidbComponentAttributesDisableRegistryReflection** bit in the Attributes field of the [Component](component-table.md) table. This may be necessary to have 32-bit and 64-bit copies of the same application coexist. If this bit is set, the Windows Installer calls the [**RegDisableReflectionKey**](https://msdn.microsoft.com/library/ms724858(v=VS.85).aspx) function on each key that is being accessed by the component. This bit is available with Windows Installer version 4.0. This bit is ignored on 32-bit systems. This bit is ignored on the 64-bit versions of Windows XP and Windows 2000.
