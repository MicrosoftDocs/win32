...
Include the MergeModuleSequence tables in the .msm file if the merge module must modify the action [*sequence tables*](https://www.bing.com/search?q=*sequence tables*) of the target .msi file. Merging does not add these tables to the .msi file. These tables only occur in merge modules.
