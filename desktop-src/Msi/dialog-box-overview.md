...
To physically create the dialog box using the Windows user interface API, [**DialogBox**](https://www.bing.com/search?q=**DialogBox**) is called and passes a pointer to the template. In Windows Installer, the dialog box is created during execution of one of the three UI sequence tables.
