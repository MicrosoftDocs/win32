...
To use a datagram context, a client sets the ISC\_REQ\_DATAGRAM flag in its call to the [**InitializeSecurityContext (General)**](/windows/win32/Sspi/?branch=master) function.
...
-   The [*security package*](https://msdn.microsoft.com/library/windows/desktop/ms721625#-security-security-package-gly) does not produce an authentication [*BLOB*](https://msdn.microsoft.com/library/windows/desktop/ms721569#-security-blob-gly) (binary large object) on the first call to [**InitializeSecurityContext (General)**](/windows/win32/Sspi/?branch=master). However, the client can use the returned security context in a call to the [**MakeSignature**](/windows/win32/Sspi/nf-sspi-makesignature?branch=master) function to generate a signature for a message.
-   The security package must allow for the context to be re-established multiple times to allow the server to drop the connection without notice. This implies that any keys used in the [**MakeSignature**](/windows/win32/Sspi/nf-sspi-makesignature?branch=master) and [**VerifySignature**](/windows/win32/Sspi/nf-sspi-verifysignature?branch=master) functions can be reset to a consistent [*state*](https://msdn.microsoft.com/library/windows/desktop/ms721625#-security-state-gly).
