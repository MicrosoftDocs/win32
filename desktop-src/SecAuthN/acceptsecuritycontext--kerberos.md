...
| **SEC_I_COMPLETE_AND_CONTINUE**<br/>`0x00090314L` | The function succeeded. The server must call [CompleteAuthToken](/windows/win32/api/sspi/nf-sspi-completeauthtoken) and pass the output token to the client. The server then waits for a return token from the client and then makes another call to **AcceptSecurityContext (Kerberos)**. |
...
| **SEC_I_CONTINUE_NEEDED**<br/>`0x00090312L` | The function succeeded. The server must send the output token to the client and wait for a returned token. The returned token should be passed in *pInput* for another call to **AcceptSecurityContext (Kerberos)**. |
