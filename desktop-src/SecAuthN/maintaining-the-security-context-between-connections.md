...
To reduce domain controller traffic and improve performance, the client-side of Microsoft Digest caches information received after successful authentication with a server. Client applications need only cache the handle to the [*security context*](https://msdn.microsoft.com/library/ms721625(v=VS.85).aspx) that has been established. The following table describes information cached by the security package.
...
| [*Nonce*](https://msdn.microsoft.com/library/ms721596(v=VS.85).aspx) | A server nonce that is associated with the successful authentication.                                                                               |
