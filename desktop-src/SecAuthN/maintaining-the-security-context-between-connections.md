...
To reduce domain controller traffic and improve performance, the client-side of Microsoft Digest caches information received after successful authentication with a server. Client applications need only cache the handle to the [*security context*](https://www.bing.com/search?q=*security context*) that has been established. The following table describes information cached by the security package.
...
| [*Nonce*](https://www.bing.com/search?q=*Nonce*) | A server nonce that is associated with the successful authentication.                                                                               |
