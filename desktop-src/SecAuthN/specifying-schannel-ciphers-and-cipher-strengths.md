...
For client/server information exchanges, the default behavior of Schannel is to negotiate the best cipher available based on those enabled in the registry. Applications may limit the ciphers and cipher strengths allowed for a connection by using members of the [**SCHANNEL\_CRED**](/windows/win32/Schannel/ns-schannel-_schannel_cred?branch=master) structure as follows:
...
3.  Pass the [**SCHANNEL\_CRED**](/windows/win32/Schannel/ns-schannel-_schannel_cred?branch=master) structure (by means of the *pAuthData* parameter) in a call to the [**AcquireCredentialsHandle**](/windows/win32/Sspi/?branch=master) function. This function returns a credentials handle.
4.  Specify the credentials handle in a call to the client-side [**InitializeSecurityContext (General)**](/windows/win32/Sspi/?branch=master) function or the server-side [**AcceptSecurityContext (General)**](/windows/win32/Sspi/?branch=master) function.
...
To retrieve the cipher strength settings for a credential, call the [**QueryCredentialsAttributes**](/windows/win32/Sspi/nf-sspi-querycredentialsattributesa?branch=master) function and specify SECPKG\_ATTR\_CIPHER\_STRENGTHS as the *ulAttribute* parameter.

To retrieve the list of supported algorithms for a credential, call the [**QueryCredentialsAttributes**](/windows/win32/Sspi/nf-sspi-querycredentialsattributesa?branch=master) with SECPKG\_ATTR\_SUPPORTED\_ALGS as the *ulAttribute* parameter.
