...
When the server receives a connection request message from a client, it creates a local [*security context*](security.s_gly#-security-security-context-gly) for the client using [**AcceptSecurityContext (General)**](/windows/desktop/api/Sspi/). The server uses this local security context to carry out future requests by the same client. For more information on context functions, see [Context Management](authentication-functions.md#context-management).
