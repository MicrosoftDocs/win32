...
In a client/server [*application protocol*](https://msdn.microsoft.com/library/windows/desktop/ms721532#-security-application-protocol-gly), a server binds to a communication port such as a socket or an RPC interface. The server then waits for a client to connect and request service. The role of security at connection setup is two-fold in the case of mutual authentication:

-   Client authentication by the server.
-   Server authentication by the client.

The client and server components of a transport application use a [*security package*](https://msdn.microsoft.com/library/windows/desktop/ms721625#-security-security-package-gly) to establish a secure connection for transmitting messages. The first step in establishing a secure connection is to create a [*security context*](https://msdn.microsoft.com/library/windows/desktop/ms721625#-security-security-context-gly); that is, an opaque data structure that contains the security data relevant to a connection, such as a [*session key*](https://msdn.microsoft.com/library/windows/desktop/ms721625#-security-session-key-gly) and the duration of the session.
