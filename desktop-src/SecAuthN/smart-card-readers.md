...
Readers are standard devices in a [*smart card*](https://www.bing.com/search?q=*smart card*) system. They are controlled through drivers, and are introduced to and removed from the system through Plug and Play or through the Control Panel Devices item.

Each reader must be defined for use by the [*smart card subsystem*](https://www.bing.com/search?q=*smart card subsystem*). The subsystem is not responsible for any reader not specifically given to it.
...
| SCard$DefaultReaders | This default group, one for each [*terminal*](https://www.bing.com/search?q=*terminal*), contains all the readers assigned to the terminal that are not reserved for specific use. |
