...
By default, Schannel validates the [*server certificate*](https://www.bing.com/search?q=*server certificate*) by calling the [**WinVerifyTrust**](https://msdn.microsoft.com/windows/desktop/b7efac6a-ac9f-477a-aada-63fe32208e6f) function; however, if you have disabled this feature using the ISC\_REQ\_MANUAL\_CRED\_VALIDATION flag, you must validate the certificate provided by the server that is attempting to establish its identity.

To manually validate the server certificate, you must first get it. Use the [**QueryContextAttributes (General)**](/windows/desktop/api/Sspi/) function and specify the SECPKG\_ATTR\_REMOTE\_CERT\_CONTEXT attribute value. This attribute returns a [**CERT\_CONTEXT**](https://msdn.microsoft.com/windows/desktop/f0a3200e-6541-423d-a4a3-595a31026eea) structure containing the certificate supplied by the server. This certificate is called the leaf certificate because it is the last certificate in the certificate chain and is farthest away from the [*root certificate*](https://www.bing.com/search?q=*root certificate*).

Using the leaf certificate you must verify the following:

-   The certificate chain is complete and the root is a certificate from a trusted [*certification authority*](https://www.bing.com/search?q=*certification authority*) (CA).
...
-   The usage of the certificate is correct, for example, a [*client certificate*](https://www.bing.com/search?q=*client certificate*) should not be used to authenticate a server.
