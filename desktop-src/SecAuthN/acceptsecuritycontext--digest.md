...
| **SEC_E_SECURITY_QOS_FAILED**<br/>`0x80090332L` | The function failed. A context attribute flag that is not valid was specified in the <em>fContextReq</em> parameter. |
...
| **SEC_I_COMPLETE_NEEDED**<br/>`0x00090313L` | The function succeeded. The server must finish building the message from the client and then call the [<strong>CompleteAuthToken</strong>](/windows/win32/api/sspi/nf-sspi-completeauthtoken) function. |
| **SEC_I_CONTINUE_NEEDED**<br/>`0x00090312L` | The function succeeded. The server must send the output token to the client and wait for a returned token. The returned token should be passed in <em>pInput</em> for another call to [AcceptSecurityContext (Digest)](acceptsecuritycontext--digest.md). |
