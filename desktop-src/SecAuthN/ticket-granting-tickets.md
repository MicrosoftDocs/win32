...
As the [*Kerberos protocol*](https://www.bing.com/search?q=*Kerberos protocol*) was originally designed, a master key for a user was derived from a password provided by the user. When a user logged on, the Kerberos client on the user's workstation accepted the password from the user and converted it into an encryption key by passing the text through a one-way [*hash*](https://www.bing.com/search?q=*hash*) function. The resulting hash was the user's [*master key*](https://www.bing.com/search?q=*master key*). The client used this *master key* to decrypt [*session keys*](https://www.bing.com/search?q=*session keys*) received from KDC.
