...
A [*smart card*](https://www.bing.com/search?q=*smart card*) interface consists of a predefined set of services available within a *smart card*, the protocols necessary to invoke the services, and any assumptions regarding the context of the services.
...
Although an interface has one GUID, it might be implemented differently on different cards. For example, the biorhythm interface mentioned above can have several different implementations, yet all are referenced using the same GUID. The different implementations would not change the interaction between the application and the smart card; however, the interaction between the [*service provider*](https://www.bing.com/search?q=*service provider*) and the smart cards may differ depending on the interface's implementation.
