...
A pointer to a [**SecBufferDesc**](https://docs.microsoft.com/en-us/windows/win32/api/sspi/nf-sspi-deletesecuritypackagea) structure generated by a client call to [**InitializeSecurityContext (NTLM)**](initializesecuritycontext--ntlm.md) that contains the input buffer descriptor.
...
A pointer to a [CtxtHandle](sspi-handles.md) structure. On the first call to **AcceptSecurityContext (NTLM)**, this pointer receives the new context handle. On subsequent calls, *phNewContext* can be the same as the handle specified in the *phContext* parameter.
