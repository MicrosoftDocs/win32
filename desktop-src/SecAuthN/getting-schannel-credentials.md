...
The primary difference between the client-side and server-side request for credentials is that the server must provide a certificate using a [**CERT\_CONTEXT**](https://msdn.microsoft.com/library/windows/desktop/aa377189) structure as follows:
...
> [!Note]  
> When requesting credentials to be used by the server, change the third (*fCredentialUse*) parameter of the [**AcquireCredentialsHandle**](/windows/desktop/api/Sspi/) function from SECPKG\_CRED\_OUTBOUND to SECPKG\_CRED\_INBOUND.
