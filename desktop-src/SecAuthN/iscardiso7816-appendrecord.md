...
The **AppendRecord** method constructs an [*application protocol data unit*](security.a_gly#-security-application-protocol-data-unit-gly) (APDU) command that either appends a record to the end of a linear-structured elementary file (EF) or writes record number 1 in a cyclic-structured elementary file.
...
On return, it is filled with the APDU command constructed by this operation. If *ppCmd* was set to **NULL**, a [*smart card*](security.s_gly#-security-smart-card-gly) [**ISCardCmd**](iscardcmd.md) object is internally created and returned by using the *ppCmd* pointer.
...
The encapsulated command can only be performed if the security status of the [*smart card*](security.s_gly#-security-smart-card-gly) satisfies the security attributes of the elementary file read.
