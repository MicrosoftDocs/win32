...
ms.date: 07/09/2025
# customer intent: As a Windows developer, I want to learn about Microsoft NTLM so that I can understand how it works and how to implement it in my applications.
---

# Microsoft NTLM

Windows Challenge/Response (NTLM) is the authentication protocol used on networks that include systems running the Windows operating system and on stand-alone systems. NTLM is a [security package](../secgloss/s-gly.md) that provides authentication, integrity, and confidentiality to applications. It is used to authenticate users and computers in a Windows domain or on stand-alone systems.
...
The following steps present an outline of NTLM noninteractive authentication. The first step provides the user's NTLM credentials and occurs only as part of the interactive authentication (logon) process. The remaining steps are performed by the client, server, and domain controller to complete the authentication process.
...
1. The client sends the user name to the server (in [plaintext](../secgloss/p-gly.md)).
1. The server generates a 8-byte random number, called a *challenge* or [nonce](../secgloss/n-gly.md), and sends it to the client.
1. The client encrypts this challenge with the hash of the user's password and returns the result to the server. This is called the *response*.
...
1. The domain controller compares the encrypted challenge it computed (in step 6) to the response computed by the client (in step 4). If they are identical, authentication is successful.
