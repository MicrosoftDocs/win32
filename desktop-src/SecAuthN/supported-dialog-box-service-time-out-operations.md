...
[*Winlogon*](https://www.bing.com/search?q=*Winlogon*) implements two time-out operations, one for secure dialog boxes and the other for screen saver activation and termination.

While displaying a secure dialog box, such as logon or unlocking a workstation, Winlogon can time-out the dialog boxes and return an appropriate result code to the dialog box procedure. Winlogon provides a set of dialog box support functions for the [*GINA*](https://www.bing.com/search?q=*GINA*). The GINA must use these functions instead of their Windows counterparts to ensure that the GINA and Winlogon maintain appropriate control over the dialog boxes. Failure to use the Winlogon versions of these functions could result in unauthorized users gaining access to the system.
...
GINA DLLs can also receive WLX\_WM\_SAS messages from Winlogon. These messages are sent to active dialog boxes if an [*secure attention sequence*](https://www.bing.com/search?q=*secure attention sequence*) (SAS) is received. This is useful if the GINA is in the process of prompting for the matching PIN for a [*smart card*](https://www.bing.com/search?q=*smart card*), and the card is removed from the smart card [*reader*](https://www.bing.com/search?q=*reader*). Winlogon uses WLX\_DLG\_SAS as the EndDialog result code when an SAS event occurs during a dialog box operation.
...
GINA dialog boxes can be terminated by Winlogon by using the code WLX\_DLG\_USER\_LOGOFF. This indicates that the user has logged off during the running of the dialog box (for example, by calling the [**ExitWindowsEx**](https://msdn.microsoft.com/windows/desktop/f44ccb66-10bd-4ee6-93e1-16948cf10e50) function from another thread).
...
[Winlogon Support Functions](https://www.bing.com/search?q=Winlogon Support Functions)
</dt> </dl>
