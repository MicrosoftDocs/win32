...
-   Transport applications requests stream semantics by setting the ISC\_REQ\_STREAM and ASC\_REQ\_STREAM flags in the calls to the [**InitializeSecurityContext (General)**](/windows/win32/Sspi/?branch=master) and [**AcceptSecurityContext (General)**](/windows/win32/Sspi/?branch=master) functions.
-   The application calls the [**QueryContextAttributes (General)**](/windows/win32/Sspi/?branch=master) function with a [**SecPkgContext\_StreamSizes**](/windows/win32/Sspi/ns-sspi-_secpkgcontext_streamsizes?branch=master) structure to query the [*security context*](security.s_gly#-security-security-context-gly) for the number of buffers to provide and the sizes to reserve for headers or trailers.
-   The application provides buffer descriptors to spare during the actual processing of the data. By specifying stream semantics, the caller indicates willingness to do extra processing so that the [*security package*](security.s_gly#-security-security-package-gly) can handle the blocking of the messages. In essence, for the [**MakeSignature**](/windows/win32/Sspi/nf-sspi-makesignature?branch=master) and [**VerifySignature**](/windows/win32/Sspi/nf-sspi-verifysignature?branch=master) functions, the caller passes in a list of buffers. When a message is received from a channel that is stream-oriented (such as a TCP port), the caller passes in a buffer list as follows.
    ...
    Similarly, on the sender side of the communication, the caller can call the [**MakeSignature**](/windows/win32/Sspi/nf-sspi-makesignature?branch=master) function. The security package may need to reallocate the buffer or copy things around. The caller can be more efficient by providing a buffer list as follows.
    ...
    This allows the caller to use the buffers more efficiently. By calling the [**QueryContextAttributes**](/windows/win32/Sspi/?branch=master) function to determine the amount of space to reserve before calling [**MakeSignature**](/windows/win32/Sspi/nf-sspi-makesignature?branch=master), the operation is more efficient for the application and the security package.
