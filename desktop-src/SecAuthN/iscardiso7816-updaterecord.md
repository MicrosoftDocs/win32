...
The **UpdateRecord** method constructs an [*application protocol data unit*](security.a_gly#-security-application-protocol-data-unit-gly) (APDU) command that updates a specific record with the bits given in the APDU command.
...
On return, it is filled with the APDU command constructed by this operation. If *ppCmd* was set to **NULL**, a [*smart card*](security.s_gly#-security-smart-card-gly) [**ISCardCmd**](iscardcmd.md) object is internally created and returned via the *ppCmd* pointer.
...
The encapsulated command can only be performed if the security status of the [*smart card*](security.s_gly#-security-smart-card-gly) satisfies the security attributes of the elementary file being processed.
