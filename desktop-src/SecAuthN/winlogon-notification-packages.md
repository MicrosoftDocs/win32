...
[*Winlogon*](security.w_gly#-security-winlogon-gly) notification packages are DLLs that receive and handle events generated by Winlogon. You can implement such a notification package to monitor and respond to Winlogon events. This is useful for applications that need to perform additional processing during logon or logoff, or maintain [*state*](security.s_gly#-security-state-gly) information that must be updated when Winlogon events occur.
