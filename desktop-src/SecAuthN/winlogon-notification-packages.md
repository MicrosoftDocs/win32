...
[*Winlogon*](https://msdn.microsoft.com/en-us/library/ms721635(v=VS.85).aspx) notification packages are DLLs that receive and handle events generated by Winlogon. You can implement such a notification package to monitor and respond to Winlogon events. This is useful for applications that need to perform additional processing during logon or logoff, or maintain [*state*](https://msdn.microsoft.com/en-us/library/ms721625(v=VS.85).aspx) information that must be updated when Winlogon events occur.
