...
If authentication is successful, the user's [*security context*](https://www.bing.com/search?q=*security context*) and a Digest [*session key*](https://www.bing.com/search?q=*session key*) are returned to the originating server. If authentication fails, the originating server must generate an error response. After a successful authentication the originating server returns the requested resource to the client.
