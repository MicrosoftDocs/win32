...
If authentication is successful, the user's [*security context*](security.s_gly#-security-security-context-gly) and a Digest [*session key*](security.s_gly#-security-session-key-gly) are returned to the originating server. If authentication fails, the originating server must generate an error response. After a successful authentication the originating server returns the requested resource to the client.
