...
*X509* digital signatures are also supported by the *Packaging*APIs. Developers can use the digital signature features to sign and validate selected parts of an OPC package or the entire package. Applications can give their documents an added level of security by using digital signatures to detect when the content of an OPC-based file has been altered after the file was signed. (See [Open Packaging Conventions Overview](http://go.microsoft.com/?linkid=9669685).)
