...
1.  Create a message text file that creates the French message strings. For details on creating a message text file, see [Message Text Files](https://msdn.microsoft.com/library/windows/desktop/dd996906). The message identifiers that you specify in the message text file must match the resource identifiers that the message compiler generated for the same strings in the manifest. To determine the resource identifiers used for the strings in the manifest, see the .h file that the message compiler generated when you compiled the manifest.
    ```Text
