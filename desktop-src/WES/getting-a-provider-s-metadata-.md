---
title: Getting a Providers Metadata
...
A provider uses an instrumentation manifest to identify itself, define the events that it writes, and other components such as channels, tasks, and keywords. To access this information at run time, call the [**EvtOpenPublisherMetadata**](/windows/win32/WinEvt/nf-winevt-evtopenpublishermetadata?branch=master) function to get a handle to the publisher's metadata.

The metadata is divided between the publisher metadata and the event metadata. The publisher metadata contains the metadata that identifies the publisher and any metadata that it defines such as channels, levels, tasks, opcodes, and keywords. For a list of the provider metadata that you can retrieve, see the [**EVT\_PUBLISHER\_METADATA\_PROPERTY\_ID**](/windows/win32/WinEvt/ne-winevt-_evt_publisher_metadata_property_id?branch=master) enumeration. To get the metadata, call the [**EvtGetPublisherMetadataProperty**](/windows/win32/WinEvt/nf-winevt-evtgetpublishermetadataproperty?branch=master) function. This function returns the property values for the properties that identify the provider and handles to the channels, levels, tasks, opcodes, and keywords components. To access the metadata for these components, call the [**EvtGetObjectArrayProperty**](/windows/win32/WinEvt/nf-winevt-evtgetobjectarrayproperty?branch=master) function.

To get the event metadata, call the [**EvtOpenEventMetadataEnum**](/windows/win32/WinEvt/nf-winevt-evtopeneventmetadataenum?branch=master) function to get a list of the events that the provider defines. Then call the [**EvtNextEventMetadata**](/windows/win32/WinEvt/nf-winevt-evtnexteventmetadata?branch=master) function in a loop to enumerate the events. For a list of metadata that you can get for an event, see the [**EVT\_EVENT\_METADATA\_PROPERTY\_ID**](/windows/win32/WinEvt/ne-winevt-_evt_event_metadata_property_id?branch=master) enumeration. To get the metadata, call the [**EvtGetEventMetadataProperty**](/windows/win32/WinEvt/nf-winevt-evtgeteventmetadataproperty?branch=master) function. These properties return an integer value for each element of the event definition. For event ID and version, displaying the integer value is fine but for the other elements, such as channel and level, displaying the integer value may not have any meaning to the user. The integer value for these elements is the value of the value attribute. You can use the integer value to map into the provider's metadata and get the message string or name value for the element and display it to the user instead.
