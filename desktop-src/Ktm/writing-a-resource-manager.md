...
To create a durable TM, you must either create a CLFS log and call [**CreateTransactionManager**](/windows/win32/Ktmw32/nf-ktmw32-createtransactionmanager?branch=master) or have KTM create it for you. After a durable TM is created, you must first recover the TM by calling [**RecoverTransactionManager**](/windows/win32/Ktmw32/nf-ktmw32-recovertransactionmanager?branch=master). After the TM is recovered, it is available for use.
...
Next, you create a resource manager by calling [**CreateResourceManager**](/windows/win32/Ktmw32/nf-ktmw32-createresourcemanager?branch=master) with the TM handle. The RM can be volatile or durable. Only durable TMs can be used with durable RMs.

When working transactionally, you enlist in the transaction by calling [**CreateEnlistment**](/windows/win32/KtmW32/nf-ktmw32-createenlistment?branch=master)and specifying which notifications to receive.

**Note**  You can start receiving notifications before the call to [**CreateEnlistment**](/windows/win32/KtmW32/nf-ktmw32-createenlistment?branch=master) is completed.
