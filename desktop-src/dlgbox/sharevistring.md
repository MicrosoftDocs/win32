...
An **Open** or **Save As** dialog box sends the **SHAREVISTRING** registered message to your hook procedure, [*OFNHookProc*](ofnhookproc.md), if a sharing violation occurs for the selected file when the user clicks the **OK** button.
...
A pointer to a [**OPENFILENAME**](/windows/win32/Commdlg/ns-commdlg-tagofna?branch=master) structure. The **lpstrFile** member of this structure contains the file name that caused the sharing violation.
...
The dialog box sends the **SHAREVISTRING** registered message only if you did not specify the **OFN\_SHAREAWARE** flag in the **Flags** member of the [**OPENFILENAME**](/windows/win32/Commdlg/ns-commdlg-tagofna?branch=master) structure when you created the dialog.
