...
2.  Generate the new key. There are two ways to accomplish this—by having CryptoAPI generate all new values for G, P, and X or by using existing values for G and P, and generating a new value for X.
    ...
    **To generate the key by using predefined values for G and P**
    ...
    2.  Initialize a [**CRYPT\_DATA\_BLOB**](https://www.bing.com/search?q=**CRYPT\_DATA\_BLOB**) structure with the **pbData** member set to the P value. The [*BLOB*](security.b_gly#-security-blob-gly) contains no header information and the **pbData** member is in [*little-endian*](security.l_gly#-security-little-endian-gly) format.
...
5.  Create a Diffie-Hellman [*public key BLOB*](security.p_gly#-security-public-key-blob-gly) by calling the [**CryptExportKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptexportkey) function, passing **PUBLICKEYBLOB** in the *dwBlobType* parameter and the handle to the Diffie-Hellman key in the *hKey* parameter. This function call causes the calculation of the public key value, (G^X) mod P.
...
1.  Call the [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) function to get a handle to the Microsoft Diffie-Hellman Cryptographic Provider.
2.  Create a Diffie-Hellman key by calling the [**CryptGenKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgenkey) function to create a new key, or by calling the [**CryptGetUserKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetuserkey) function to retrieve an existing key.
3.  Create a Diffie-Hellman [*private key BLOB*](security.p_gly#-security-private-key-blob-gly) by calling the [**CryptExportKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptexportkey) function, passing **PRIVATEKEYBLOB** in the *dwBlobType* parameter and the handle to the Diffie-Hellman key in the *hKey* parameter.
