...
2.  Generate the new key. There are two ways to accomplish this—by having CryptoAPI generate all new values for G, P, and X or by using existing values for G and P, and generating a new value for X.
    ...
    **To generate the key by generating all new values**
    ...
    2.  The new key is now ready for use. The values of G and P must be sent to the recipient along with the key (or sent by some other method) when doing a [*key exchange*](https://msdn.microsoft.com/library/ms721575(v=VS.85).aspx).
    ...
    **To generate the key by using predefined values for G and P**
    ...
    2.  Initialize a [**CRYPT\_DATA\_BLOB**](https://msdn.microsoft.com/library/Aa381414(v=VS.85).aspx) structure with the **pbData** member set to the P value. The [*BLOB*](https://msdn.microsoft.com/library/ms721569(v=VS.85).aspx) contains no header information and the **pbData** member is in [*little-endian*](https://msdn.microsoft.com/library/ms721592(v=VS.85).aspx) format.
    ...
    4.  Initialize a [**CRYPT\_DATA\_BLOB**](https://msdn.microsoft.com/library/Aa381414(v=VS.85).aspx) structure with the **pbData** member set to the G value. The BLOB contains no header information and the **pbData** member is in little-endian format.
