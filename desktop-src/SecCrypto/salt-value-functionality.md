...
The Base Provider creates 40-bit [*symmetric keys*](https://msdn.microsoft.com/library/ms721625(v=VS.85).aspx) created with eleven bytes of zero-value [*salt*](https://msdn.microsoft.com/library/ms721625(v=VS.85).aspx), eleven bytes of nonzero salt if CRYPT\_CREATE\_SALT is specified, or no salt value. A 40-bit symmetric key with zero-value salt, however, is not equivalent to a 40-bit symmetric key without salt. For interoperability, keys must be created without salt. This problem results from a default condition that occurs only with keys of exactly 40 bits. All other [*key lengths*](https://msdn.microsoft.com/library/ms721590(v=VS.85).aspx) do not have salt allocated by default.
