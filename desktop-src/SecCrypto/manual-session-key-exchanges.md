...
1.  Create a random [*session key*](security.s_gly#-security-session-key-gly) by using the [**CryptGenKey**](/windows/win32/Wincrypt/nf-wincrypt-cryptgenkey?branch=master) function.
...
3.  Export the session key into a [*key BLOB*](security.k_gly#-security-key-blob-gly) with the [**CryptExportKey**](/windows/win32/Wincrypt/nf-wincrypt-cryptexportkey?branch=master) function, specifying that the key be encrypted with the destination user's key exchange public key.
4.  Send both the encrypted message and the encrypted key BLOB to the destination user.
5.  The destination user imports the key BLOB into his or her CSP by using the [**CryptImportKey**](/windows/win32/Wincrypt/nf-wincrypt-cryptimportkey?branch=master) function. This will automatically decrypt the session key, provided the destination user's key exchange public key was specified in step 3.
