...
1.  Create a random [*session key*](security.s_gly#-security-session-key-gly) by using the [**CryptGenKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgenkey) function.
...
3.  Export the session key into a [*key BLOB*](security.k_gly#-security-key-blob-gly) with the [**CryptExportKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptexportkey) function, specifying that the key be encrypted with the destination user's key exchange public key.
