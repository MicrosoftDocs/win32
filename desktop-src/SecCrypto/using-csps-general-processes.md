...
When using [*cryptographic service providers*](https://www.bing.com/search?q=*cryptographic service providers*) CSPs, keep the following conventions in mind.

## Private Key Caching

A CSP can cache some [*private keys*](https://www.bing.com/search?q=*private keys*). It is possible to control this private key caching on a global, but not an application-specific, basis. Caching changes are made by modifying certain registry settings. For more information, see [**Private Key Caching Constants**](private-key-caching-constants.md).
