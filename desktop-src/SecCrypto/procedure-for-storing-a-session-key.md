...
For an example of creating a [*session key*](https://www.bing.com/search?q=*session key*) and exporting that key to a [*simple key BLOB*](https://www.bing.com/search?q=*simple key BLOB*) that can be written to a disk file, see [Example C Program: Exporting a Session Key](example-c-program-exporting-a-session-key.md).
...
The new session key can be encrypted with the original user's [*public key*](https://www.bing.com/search?q=*public key*) [*exchange key*](https://www.bing.com/search?q=*exchange key*), and this new key can be substituted for the original. If the user unknowingly used the substituted session key to encrypt files and messages, the individual who created the substitute key could easily decrypt them.
