...
When the requested certificate is returned from the CA, the application passes the PKCS \#7 message back to the Certificate Enrollment Control where the certificate or chain of certificates is extracted from the PKCS \#7 message. The certificate and any other certificates in the chain of trust are stored in a [*certificate store*](https://msdn.microsoft.com/en-us/library/ms721572(v=VS.85).aspx). The returned certificate is not modified in any way. Any certificate-aware application can now access this certificate from the store.
...
A [*smart card reader*](https://msdn.microsoft.com/en-us/library/ms721604(v=VS.85).aspx) must be installed on the computer running the Smart Card Enrollment Control.
