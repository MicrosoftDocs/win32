...
The following example lists the certificates in a system [*certificate store*](security.c_gly#-security-certificate-store-gly), displaying the name of the subject of each certificate, and it allows the user to choose to delete any certificates from the store. The example gets the name of the certificate store from the user and thus can be used to maintain the contents of any system certificate store.
...
-   Duplicating a pointer to a [*certificate context*](security.c_gly#-security-certificate-context-gly) using [**CertDuplicateCertificateContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatecontext).
