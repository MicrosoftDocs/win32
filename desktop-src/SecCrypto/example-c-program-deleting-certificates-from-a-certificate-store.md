...
The following example lists the certificates in a system [*certificate store*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb), displaying the name of the subject of each certificate, and it allows the user to choose to delete any certificates from the store. The example gets the name of the certificate store from the user and thus can be used to maintain the contents of any system certificate store.
...
-   Duplicating a pointer to a [*certificate context*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb) using [**CertDuplicateCertificateContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatecontext).
-   Comparing the [**CERT\_INFO**](/windows/desktop/api/Wincrypt/ns-wincrypt-_cert_info) members of each certificate using [**CertCompareCertificate**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcomparecertificate).
