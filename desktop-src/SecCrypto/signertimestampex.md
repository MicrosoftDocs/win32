...
The **SignerTimeStampEx** function time stamps the specified subject and optionally returns a pointer to a [**SIGNER\_CONTEXT**](signer-context.md) structure that contains a pointer to a [*BLOB*](https://www.bing.com/search?q=*BLOB*). This function supports Authenticode time stamping. To perform X.509 Public Key Infrastructure (RFC 3161) time stamping, use the [**SignerTimeStampEx2**](signertimestampex2.md) function.
...
Optional. A 32-bit value that is passed as additional data to [*subject interface package*](https://www.bing.com/search?q=*subject interface package*) (SIP) functions. The format and content of this parameter is defined by the SIP provider.
