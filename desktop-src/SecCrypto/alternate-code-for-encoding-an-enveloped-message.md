...
The following example demonstrates an alternate process of encoding a signed message, using that signed message as the [*inner content*](https://www.bing.com/search?q=*inner content*) for an enveloped message. In preparation for decoding, the inner content is tested to determine its inner-content type.
