...
The [MakeCat](makecat.md) tool makes an unsigned catalog file, which contains the [*hashes*](https://www.bing.com/search?q=*hashes*) of a set of files along with associated attributes of each file in the set. A catalog file allows the user to sign one file (the catalog) instead of signing numerous individual files.
