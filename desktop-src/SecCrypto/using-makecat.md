...
The [MakeCat](makecat.md) tool makes an unsigned catalog file, which contains the [*hashes*](security.h_gly#-security-hash-gly) of a set of files along with associated attributes of each file in the set. A catalog file allows the user to sign one file (the catalog) instead of signing numerous individual files.
