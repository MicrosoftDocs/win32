...
[*Authenticode*](security.a_gly#-security-authenticode-gly) applies digital signature technology to guarantee the authorship and integrity of binary data such as installable software. A client web browser, or other system components, can use the Authenticode signatures to verify the integrity of the data when the software is downloaded or installed. Authenticode signatures can be used with many software formats, including .cab, .exe, .ocx, and .dll.

Microsoft maintains a list of public [*certification authorities*](security.c_gly#-security-certification-authority-gly) (CAs). Issuers of Authenticode certificates currently include [VeriSign](http://go.microsoft.com/fwlink/p/?linkid=103355), [Thawte](http://go.microsoft.com/fwlink/p/?linkid=103354), [Comodo](http://go.microsoft.com/fwlink/p/?linkid=131567), [Geotrust](http://go.microsoft.com/fwlink/p/?linkid=131568), and [GlobalSign](http://go.microsoft.com/fwlink/p/?linkid=131569).
...
[SignTool](signtool.md) relies on the Windows Authenticode implementation to create and time stamp signatures. [*Authenticode*](security.a_gly#-security-authenticode-gly) operates on binary files, for example .cab, .exe, .dll, or .ocx. Authenticode first creates the signature, producing a PKCS \#7 [**SignedData**](signeddata.md). It is this **SignedData** that must be countersigned, as described in PKCS \#9.
...
2.  Construct a time stamp request whose content is the original signature. Send this to the time stamp server [*Abstract Syntax Notation One*](security.a_gly#-security-abstract-syntax-notation-one-gly) (ASN.1) encoded as a TimeStampRequest.
...
The countersignatureType is the [*object identifier*](security.o_gly#-security-object-identifier-gly) (OID) that identifies this as a time stamp countersignature and should be the exact OID 1.3.6.1.4.1.311.3.2.1.
