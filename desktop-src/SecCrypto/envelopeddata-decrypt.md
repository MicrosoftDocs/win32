...
The **Decrypt** method decrypts enveloped content. Decryption is done if the recipient of the message has access to the [*private key*](security.p_gly#-security-private-key-gly) paired with one of the [*public keys*](security.p_gly#-security-public-key-gly) used to envelop the message. Calling the **Decrypt** method resets the [*state*](security.s_gly#-security-state-gly) of the object. If the **Decrypt** method succeeds, the [**Content**](envelopeddata-content.md) property of the [**EnvelopedData**](envelopeddata.md) object is set to the plaintext message.
