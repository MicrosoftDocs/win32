...
The **Decrypt** method decrypts enveloped content. Decryption is done if the recipient of the message has access to the [*private key*](https://www.bing.com/search?q=*private key*) paired with one of the [*public keys*](https://www.bing.com/search?q=*public keys*) used to envelop the message. Calling the **Decrypt** method resets the [*state*](https://www.bing.com/search?q=*state*) of the object. If the **Decrypt** method succeeds, the [**Content**](envelopeddata-content.md) property of the [**EnvelopedData**](envelopeddata.md) object is set to the plaintext message.
