...
A **DWORD** value that specifies the type of provider to acquire. The following [*provider types*](security.p_gly#-security-provider-type-gly) are predefined and are discussed in detail in [CSP Interoperability](seccngprov.csp_interoperability):
...
A pointer to an array of context information. The **pbContextInfo** and **cbContextInfo** members together determine the information set used when a [**CPSetProvParam**](seccngprov.cpsetprovparam) function is called with PP\_CONTEXT\_INFO set.
