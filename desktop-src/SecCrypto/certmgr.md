---
description: Learn how to use the CertMgr certificate management tool to manage certificates, CTLs, and CRLs in Windows SDK. Includes syntax, options, and examples.
ms.assetid: c9b68a81-c4f7-4754-9b47-c83f3679f0e3
title: CertMgr Certificate Management Tool Reference
ms.topic: reference
ms.date: 08/21/2025
---
...
CertMgr is a certificate management tool that replaces DumpCert. It provides capabilities for managing [certificates](../secgloss/c-gly.md), [certificate trust lists](../secgloss/c-gly.md) (CTLs), and [certificate revocation lists](../secgloss/c-gly.md) (CRLs). The tool is installed in the \\Bin folder of the Microsoft Windows Software Development Kit (SDK) installation path.

CertMgr is available as part of the Windows SDK, which you can download [here](https://developer.microsoft.com/windows/downloads/windows-sdk/).

## Syntax

CertMgr performs one of four functions depending on the action indicated in the command. Use the following syntax:

**CertMgr** \[**-add**\|**-del**\|**-put**\] \[*Options*\] \[ **-s** \[ **-r** *RegistryLocation* \] \] *SourceName* \[ **-s** \[ **-r** *RegistryLocation* \] \] \[*DestinationName*\]

## CertMgr command actions
...
| **-all**                   | **-add-del**<br/> **-put** | Adds all entries of the chosen type. |
| **-e** *encodingType*      | all                        | Certificate [encoding type](../secgloss/e-gly.md). |
| **-y** *storeProviderType* | all                        | Store provider type. |
| **-7**                     | **-add-del**<br/> **-put** | Saves the destination store as a PKCS \#7 file. |
| **-f** *dwFlags*           | all                        | Store open flag. This is the *dwFlags* parameter passed to [CertOpenStore](/windows/win32/api/Wincrypt/nf-wincrypt-certopenstore). The default value is CERT_SYSTEM_STORE_CURRENT_USER. Meaningful only if **-y** is set. For more information, see **CertOpenStore**.         |
| **-n** *commonNameString*  | **-add-del**<br/> **-put** | Common name of the certificate. Can be used only with certificates. |
| **-sha1** *sha1Hash*       | **-add-del**<br/> **-put** | SHA1 hash of the certificate, CTL, or CRL to be copied, deleted, or saved. |
...
- [CertOpenStore](/windows/win32/api/Wincrypt/nf-wincrypt-certopenstore)
