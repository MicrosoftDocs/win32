...
Additionally, you can edit the Certrqtp.inc file to add, change, or remove [*certificate templates*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb) or types available to the user. These templates and types, as well as related information, are contained in a dimensioned array called rgAvailReqTypes(*m*,5).
...
The array's second dimension, which always has the value five, allocates the six fields that compose each item. These six fields represent the certificate template or type, the display name associated with this template or type, and whether the template requires [*Secure/Multipurpose Internet Mail Extensions*](https://msdn.microsoft.com/3e9d7672-2314-45c8-8178-5a0afcfd0c50) (S/MIME).
...
|---------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **FIELD\_OID**                        | (Applies to stand-alone CAs) Index to the item's first field; represents an [*object identifier*](https://msdn.microsoft.com/e6be8932-015e-4058-b249-1671b3fea521) (OID) for a certificate type.<br/>                                                                                                           |
...
| **FIELD\_CSPLIST**                    | Index to the item's third field. A list of [*Cryptographic Service Provider*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb) names allowed by the template. Each name in the list is separated by a question mark (?).                                                    |
| **FIELD\_CSPLIST2**                   | Index to the item's fourth field. A secondary list of [*Cryptographic Service Provider*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb) names allowed by the template. Each name in the list is separated by a question mark (?). This list provides a different default. |
...
The result of these assignments is that the user will see the text **Web Browser Certificate** when enrolling, and if the user selects **Web Browser Certificate**, the [*certificate request*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb) will contain the OID 1.3.6.1.5.5.7.3.2.
...
Note that merely adding [*certificate templates*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb) and types to the Certrqtp.inc file does not guarantee that the user will be able to get a certificate with those traits; the CA must be authorized to issue a certificate for the specified certificate template or type.

Installations can create their own applications or webpages to request and receive a certificate. The [**ICEnroll4**](/windows/desktop/api/Xenroll/nn-xenroll-icenroll4) and [**ICertRequest2**](/windows/desktop/api/Certcli/nn-certcli-icertrequest2) objects allow programmers or scripters to build [*certificate request*](https://msdn.microsoft.com/db46def4-bfdc-4801-a57d-d568e94a2dbb) applications.

To request a certificate to be issued on a [*smart card*](https://msdn.microsoft.com/3e9d7672-2314-45c8-8178-5a0afcfd0c50), you can use the Smart Card Enrollment Control. For details and example code, see [**ISCrdEnr**](iscrdenr.md).
