...
Before enrolling on behalf of users, you must select or set a signing certificate. The [*private key*](https://msdn.microsoft.com/2fe6cfd3-8a2e-4dbe-9fb8-332633daa97a) associated with this signing certificate is used to sign a PKCS \#7 request. The PKCS \#7, in turn, contains the user's PKCS \#10 request (which is signed with the user's private key).
...
<tbody>
<tr class="odd">
<td><pre><code>SCrdEnr.setSigningCertificate( _
  ByVal dwFlags, _
