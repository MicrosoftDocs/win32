...
<td>[<em>Hash</em>](https://www.bing.com/search?q=<em>Hash</em>) algorithm. Must be set to either <strong>SHA-1</strong> or <strong>MD5</strong> (default). For information about MD5, see [<em>MD5</em>](https://www.bing.com/search?q=<em>MD5</em>).</td>
</tr>
<tr class="even">
...
<td>Certificate type. <em>CertificateTypes</em> can be <strong>end</strong> for end-entity, or <strong>authority</strong> for [<em>certification authority</em>](https://www.bing.com/search?q=<em>certification authority</em>).</td>
</tr>
<tr class="even">
...
<td>Inserts a list of one or more comma-separated, [<em>enhanced key usage</em>](https://www.bing.com/search?q=<em>enhanced key usage</em>) [<em>object identifiers</em>](https://www.bing.com/search?q=<em>object identifiers</em>) (OIDs) into the certificate. For example, <strong>-eku 1.3.6.1.5.5.7.3.2</strong> inserts the client authentication OID. For definitions of allowable OIDs, see the Wincrypt.h file in CryptoAPI 2.0.</td>
</tr>
<tr class="even">
...
<td>Name for the publisher's certificate. This name must conform to the [<em>X.500</em>](https://www.bing.com/search?q=<em>X.500</em>) standard. The simplest method is to use the &quot;CN=<em>MyName</em>&quot; format. For example: <strong>-n &quot;CN=Test&quot;</strong>.</td>
</tr>
<tr class="even">
...
<td>Location of the subject's key container which holds the [<em>private key</em>](https://www.bing.com/search?q=<em>private key</em>). If a key container does not exist, one is created. If neither the <strong>-sk</strong> or <strong>-sv</strong> option is used, a default key container is created and used by default.</td>
</tr>
<tr class="odd">
...
<td>CryptoAPI provider type for subject. The default is [<em>PROV_RSA_FULL</em>](https://www.bing.com/search?q=<em>PROV_RSA_FULL</em>). For information about CryptoAPI provider types, see the CryptoAPI 2.0 documentation.</td>
</tr>
<tr class="odd">
...
> [!Note]  
> If the **-sky** key specification option is used in Internet Explorer version 4.0 or later, the specification must match the key specification indicated by the [*private key*](https://www.bing.com/search?q=*private key*) file or private [*key container*](https://www.bing.com/search?q=*key container*). If the key specification option is not used, the key specification indicated by the private key file or private key container will be used. If there is more than one key specification in the key container, MakeCert will first attempt to use the AT\_SIGNATURE key specification. If that fails, MakeCert will try to use AT\_KEYEXCHANGE. Because most users have either an AT\_SIGNATURE key or an AT\_KEYEXCHANGE key, this option does not need to be used in most cases.

 

The following options are only for [*Software Publisher Certificate*](https://www.bing.com/search?q=*Software Publisher Certificate*) (SPC) files and private key technology.
...
<td>CryptoAPI provider type for issuer. The default is [<em>PROV_RSA_FULL</em>](https://www.bing.com/search?q=<em>PROV_RSA_FULL</em>). For information about CryptoAPI provider types, see the CryptoAPI 2.0 documentation.</td>
</tr>
</tbody>
</table>
...
> [!Note]  
> If the **-iky** key specification option is used in Internet Explorer 4.0 or later, the specification must match the key specification indicated by the [*private key*](https://www.bing.com/search?q=*private key*) file or private [*key container*](https://www.bing.com/search?q=*key container*). If the key specification option is not used, the key specification indicated by the private key file or private key container will be used. If there is more than one key specification in the key container, MakeCert will first attempt to use the AT\_SIGNATURE key specification. If that fails, MakeCert will try to use AT\_KEYEXCHANGE. Because most users have either an AT\_SIGNATURE key or an AT\_KEYEXCHANGE key, this option does not need to be used in most cases.

 

The following options are for [*certificate store*](https://www.bing.com/search?q=*certificate store*) technology only.
