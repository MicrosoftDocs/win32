...
[*Certificates*](https://msdn.microsoft.com/en-us/library/ms721572(v=VS.85).aspx) can be added to or removed from [*certificate stores*](https://msdn.microsoft.com/en-us/library/ms721572(v=VS.85).aspx) if the store is opened with read/write permission. Read/write permission is not granted to Active Directory stores. While certificates can be added to or removed from memory stores, changes in memory stores are not persisted between sessions.
