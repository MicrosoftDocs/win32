...
The **Encrypt** method generates a session key, uses that key to encrypt the contents, envelops the encrypted content for each recipient by encrypting the session key with each recipient's public key, and then returns the [*BLOB*](https://www.bing.com/search?q=*BLOB*) that contains the encrypted contents and the encrypted session keys as an encoded string.
