...
Secure communication over nonsecure networks generally involves three major areas of concern: [privacy](#privacy), [authentication](#authentication), and [integrity](#integrity). The Microsoft cryptography API ([*CryptoAPI*](security.c_gly#-security-cryptoapi-gly)) is a set of functions, interfaces, and tools that applications can use to improve confidence of security in these areas.

In addition to functionality for privacy, authentication, and integrity, [*CryptoAPI*](security.c_gly#-security-cryptoapi-gly) also provides for:

-   Encoding messages to [*Abstract Syntax Notation One*](security.a_gly#-security-abstract-syntax-notation-one-gly) (ASN.1) form.
-   Decoding ASN.1 messages.
-   Managing collections of [*certificates*](security.c_gly#-security-certificate-gly) in [*certificate stores*](security.c_gly#-security-certificate-store-gly).
-   Working with [*certificate trust lists*](security.c_gly#-security-certificate-trust-list-gly) and certificate chains for verification of the validity of certificates.

## Privacy

To achieve privacy, users must prevent anyone except the intended recipient from reading a message. Improving the likelihood of privacy usually involves using some form of [*cryptography*](security.c_gly#-security-cryptography-gly). Cryptographic techniques are used to encrypt (scramble) messages before the messages are stored or transmitted.

Data encryption transforms [*plaintext*](security.p_gly#-security-plaintext-gly) into [*ciphertext*](security.c_gly#-security-ciphertext-gly). The data to be encrypted can be [*ASCII*](security.a_gly#-security-ascii-gly) text, a database file, or any other data. In this documentation, the term [*message*](security.m_gly#-security-message-gly) is used to refer to any piece of data, plaintext refers to data that has not been encrypted, and *ciphertext* refers to data that has been encrypted. A good data encryption system makes it difficult to transform encrypted data back to plaintext without a secret key.
...
When data is encrypted, the message and an [*encryption*](security.e_gly#-security-encryption-gly) key are passed to the encryption algorithm. To decrypt the data, the ciphertext and a [*decryption*](security.d_gly#-security-decryption-gly) key are passed to the decryption algorithm. Encryption and decryption can be done by using a single key in a process called symmetric encryption.
...
To address the stated goals of privacy, developers can use [*CryptoAPI*](security.c_gly#-security-cryptoapi-gly) to encrypt and digitally sign data in a flexible manner, while helping to provide protection for the user's sensitive private key data.

[*CryptoAPI*](security.c_gly#-security-cryptoapi-gly) provides the following areas of functionality to perform the tasks of encryption/decryption, message signing, and key storage:

-   [Base Cryptography Functions](cryptography-functions.md#base-cryptography-functions)
-   [Simplified Message Functions](cryptography-functions.md#simplified-message-functions)
-   [Low-Level Message Functions](cryptography-functions.md#low-level-message-functions)
...
Authentication also involves making sure that the data received is the data that was sent. If party A sends a message to party B, party B needs to be able to prove that the message received was the message that party A sent and not a message that was substituted for that message. To provide this form of authentication, [*CryptoAPI*](security.c_gly#-security-cryptoapi-gly) provides functions for signing data and verifying signatures by using public/private key pairs.
...
[*CryptoAPI*](security.c_gly#-security-cryptoapi-gly) supports authentication through the use of digital certificates, with certificate encode/decode functions, and [*certificate store*](security.c_gly#-security-certificate-store-gly) functions.
...
In the same manner, a receiver of data must be able to verify the identity of the sender of the data and be sure that the data received is exactly the data that was sent; that is, that it has not been tampered with. Establishing the [*integrity*](security.i_gly#-security-integrity-gly) of data received is often done by sending not only the original data but also a verification message, called a [*hash*](security.h_gly#-security-hash-gly), about that data. Both the data and the verification message can be sent with a [*digital signature*](security.d_gly#-security-digital-signature-gly) that proves the origin of both.
...
[*CryptoAPI*](security.c_gly#-security-cryptoapi-gly) supports integrity through the use of message functions to sign data and to verify digital signatures.
