...
A [*certificate context*](security.c_gly#-security-certificate-context-gly), [**CERT\_CONTEXT**](/windows/win32/Wincrypt/ns-wincrypt-_cert_context?branch=master), is a C structure that contains an encoded member, a handle to a [*certificate store*](security.c_gly#-security-certificate-store-gly), a pointer to the original encoded [*certificate BLOB*](security.c_gly#-security-certificate-blob-gly), and a pointer to a [**CERT\_INFO**](/windows/win32/Wincrypt/ns-wincrypt-_cert_info?branch=master) C structure.

The [**CERT\_INFO**](/windows/win32/Wincrypt/ns-wincrypt-_cert_info?branch=master) structure is the heart of the certificate. It contains, in direct form and in encoded form, all the basic information in the certificate. The following illustration shows the **CERT\_INFO** structure with all of its encoded members shown as shaded.
...
If the information contained in the encoded (shaded) members **Issuer** and **Subject** is needed, those members must be decoded. Use [**CryptDecodeObject**](/windows/win32/Wincrypt/nf-wincrypt-cryptdecodeobject?branch=master) to decode these members. The following illustration shows the process of decoding one of these members.

![decoding with cryptdecodeobject](images/infoflow.png)

In the illustrated case, the [**CryptDecodeObject**](/windows/win32/Wincrypt/nf-wincrypt-cryptdecodeobject?branch=master) function creates a [**CERT\_NAME\_INFO**](/windows/win32/Wincrypt/ns-wincrypt-_cert_name_info?branch=master) structure, an array of [**CERT\_RDN**](/windows/win32/Wincrypt/ns-wincrypt-_cert_rdn?branch=master) structures, a corresponding array of [**CERT\_RDN\_ATTR**](/windows/win32/Wincrypt/ns-wincrypt-_cert_rdn_attr?branch=master) structures, and a string that contains the name. Members of the **CERT\_RDN\_ATTR** structure determine the contents of the string. For example, if the **pszObjId** member is 2.5.4.3, the string contains a common name. If it is 2.5.4.10, the string would contain an organization name. For a list of these [*object identifiers*](security.o_gly#-security-object-identifier-gly) (OIDs), see **CERT\_RDN\_ATTR**.
