...
The Filter Mapper exposes the [**IFilterMapper2**](/windows/desktop/api/Strmif/nn-strmif-ifiltermapper2) interface. To search for a filter, call the [**IFilterMapper2::EnumMatchingFilters**](/windows/desktop/api/Strmif/nf-strmif-ifiltermapper2-enummatchingfilters) method. This method takes several parameters that define the search criteria, and returns an enumerator for the matching filters. The enumerator supports the [**IEnumMoniker**](https://msdn.microsoft.com/en-us/library/ms692852(v=VS.85).aspx) interface, and supplies a unique moniker for each matching filter.
