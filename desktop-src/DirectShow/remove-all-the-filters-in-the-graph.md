...
Alternatively, you can remove the filters one at a time, using the [**IFilterGraph::RemoveFilter**](ifiltergraph-removefilter.md) method:
...
This example uses the [**IFilterGraph::EnumFilters**](ifiltergraph-enumfilters.md) method to enumerate the filters in the graph. Removing a filter causes the enumerator object to become out of sync with the graph. Use the [**IEnumFilters::Reset**](ienumfilters-reset.md) method to reset the enumerator. Otherwise, any subsequent call to [**IEnumFilters::Next**](ienumfilters-next.md) will fail.
