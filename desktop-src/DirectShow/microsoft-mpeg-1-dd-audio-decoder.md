...
For AAC, the decoder has certain format constraints on the compressed AAC input. These format constraints are the same as the Media Foundation [**AAC Decoder**](mf.aac_decoder), and are documented in the section "[**Format Constraints**](mf.aac_decoder#format-constraints)".
