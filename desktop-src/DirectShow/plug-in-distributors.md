...
The term *distributor* stems from the fact that a PID can query its outer **IUnknown** pointer for interfaces on the filter graph manager. By calling the [**IFilterGraph::EnumFilters**](ifiltergraph-enumfilters.md) method, the PID can enumerate the filters in the graph and distribute method calls to those filters. In this way, a PID can serve as a single control point for the application to call methods on filters.

When the filter graph manager aggregates a PID, it queries the PID for the [**IDistributorNotify**](idistributornotify.md) interface. If the PID supports this interface, the filter graph manager uses it to notify the PID about changes in the graph:

-   When the filter graph switches between run, paused, and stopped states, it calls [**IDistributorNotify::Run**](idistributornotify-run.md), [**IDistributorNotify::Pause**](idistributornotify-pause.md), or [**IDistributorNotify::Stop**](idistributornotify-stop.md).
-   When a reference clock is set, the filter graph manager calls [**IDistributorNotify::SetSyncSource**](idistributornotify-setsyncsource.md).
-   When filters are added or removed, or pin connections are changed, the filter graph manager calls [**IDistributorNotify::NotifyGraphChange**](idistributornotify-notifygraphchange.md).
