...
To send a Certified Output Protection Protocol (COPP) status request, fill in an [**AMCOPPStatusInput**](/windows/win32/strmif/ns-strmif-_amcoppstatusinput?branch=master) structure with the request data. The structure members are:

-   **rApp**. A 128-bit random number, typed as a GUID. The same number is returned in the driver's response. You should allocate the random number on the heap and then copy it into structure. This guards against attacks where the attacker modifies the contents of the [**AMCOPPStatusInput**](/windows/win32/strmif/ns-strmif-_amcoppstatusinput?branch=master) structure.
...
Pass the [**AMCOPPStatusInput**](/windows/win32/strmif/ns-strmif-_amcoppstatusinput?branch=master) structure to the [**IAMCertifiedOutputProtection::ProtectionStatus**](/windows/win32/Strmif/nf-strmif-iamcertifiedoutputprotection-protectionstatus?branch=master) method. For example, the following code sends a status request that queries which protection mechanisms are available:
...
The response is written into the **COPPStatus** member of the [**AMCOPPStatusOutput**](/windows/win32/strmif/ns-strmif-_amcoppstatusoutput?branch=master) structure. The size of the valid data in the response is given in the **cbSizeData** member. To ensure the integrity of the message, the driver computes a message authentication code (MAC) using the OMAC 1 algorithm, and returns this value in the structure's **macKDI** member. The application should verify this value as follows:

1.  Calculate the OMAC tag for the block of data that appears after the **macKDI** member of the [**AMCOPPStatusOutput**](/windows/win32/strmif/ns-strmif-_amcoppstatusoutput?branch=master) structure (in other words, **cbSizeData** plus **COPPStatus**).
2.  Compare this tag with the value in **macKDI**, using a straight **memcmp**.

The OMAC 1 algorithm is described in detail at [http://www.nuee.nagoya-u.ac.jp/labs/tiwata/omac/omac.html](http://go.microsoft.com/fwlink?linkID=161618). COPP uses the following OMAC-1 parameters:
...
-   A [**COPP\_StatusFlags**](/windows/win32/dxva9typ/ne-dxva9typ-_copp_statusflags?branch=master) value that indicates whether the output protection status has changed.
