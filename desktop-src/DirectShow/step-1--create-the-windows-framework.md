...
Start by creating the basic framework of a Windows application, including WinMain and a window procedure. The WinMain function is not shown here; call [**CoInitialize**](com.coinitialize) before the message loop to initialize the COM library, and [**CoUninitialize**](com.couninitialize) after the message loop exits. Start with the following minimal window procedure:
