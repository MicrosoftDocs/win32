...
-   To determine the type of physical connector to which a pin corresponds, call the [**IAMCrossbar::get\_CrossbarPinInfo**](/windows/desktop/api/Strmif/nf-strmif-iamcrossbar-get_crossbarpininfo) method. The method returns a member of the [**PhysicalConnectorType**](/windows/desktop/api/strmif/ne-strmif-tagphysicalconnectortype) enumeration. For example, an S-Video pin returns the value PhysConn\_Video\_SVideo.
