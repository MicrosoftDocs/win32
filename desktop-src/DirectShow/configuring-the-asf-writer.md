...
3.  Call [**IConfigAsfWriter::ConfigureFilterUsingProfile**](/windows/win32/Dshowasf/nf-dshowasf-iconfigasfwriter-configurefilterusingprofile?branch=master) with the updated profile object.
4.  Find the video input pin, and call its [**IAMWMBufferPass::SetNotify**](/windows/win32/Dshowasf/nf-dshowasf-iamwmbufferpass-setnotify?branch=master) method to register your application-defined [**IAMWMBufferPassCallback**](/windows/win32/Dshowasf/nn-dshowasf-iamwmbufferpasscallback?branch=master) interface.

When the graph runs, your [**IAMWMBufferPassCallback::Notify**](/windows/win32/Dshowasf/nf-dshowasf-iamwmbufferpasscallback-notify?branch=master) method will be called for each frame, and you will be able to get and set properties on the sample using its **INSSBuffer3** interface methods.
