---
Description: The IResize interface must be supported by any custom video resizer filter for DirectShow Editing Services (DES). To set a custom resizer filter, call the IRenderEngine2SetResizerGUID method on the render engine.
...
The **IResize** interface inherits from the [**IUnknown**](com.iunknown) interface. **IResize** also has these types of members:
