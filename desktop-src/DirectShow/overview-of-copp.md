...
2.  Query the VMR for the [**IAMCertifiedOutputProtection**](/windows/win32/Strmif/nn-strmif-iamcertifiedoutputprotection?branch=master) interface.
3.  Call [**IAMCertifiedOutputProtection::KeyExchange**](/windows/win32/Strmif/nf-strmif-iamcertifiedoutputprotection-keyexchange?branch=master). This method returns a 128-bit random number from the driver, along with a certificate chain that contains the driver's 2048-bit RSA public key.
...
3.  Concatenate the 128-bit random number from the graphics driver, the AES session key, the status sequence number, and the command sequence number. Encrypt this byte array using the driver's public key and pass the result to [**IAMCertifiedOutputProtection::SessionSequenceStart**](/windows/win32/Strmif/nf-strmif-iamcertifiedoutputprotection-sessionsequencestart?branch=master).

**Send COPP Commands and Status Requests**

1.  Query for the available protection types and other information by calling [**IAMCertifiedOutputProtection::ProtectionStatus**](/windows/win32/Strmif/nf-strmif-iamcertifiedoutputprotection-protectionstatus?branch=master).
2.  Set the desired protection levels by calling [**IAMCertifiedOutputProtection::ProtectionCommand**](/windows/win32/Strmif/nf-strmif-iamcertifiedoutputprotection-protectioncommand?branch=master).
