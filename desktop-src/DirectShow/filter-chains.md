...
The [**IFilterChain**](ifilterchain.md) interface provides the following methods for controlling filter chains:



|                                                               |                                 |
|---------------------------------------------------------------|---------------------------------|
| [**IFilterChain::StartChain**](ifilterchain-startchain.md)   | Starts a chain.                 |
| [**IFilterChain::StopChain**](ifilterchain-stopchain.md)     | Stops a chain.                  |
| [**IFilterChain::PauseChain**](ifilterchain-pausechain.md)   | Pauses a chain.                 |
| [**IFilterChain::RemoveChain**](ifilterchain-removechain.md) | Removes a chain from the graph. |



 

There is no specific method for adding a chain. To add a chain, insert the new filters using the [**IFilterGraph::AddFilter**](ifiltergraph-addfilter.md) method. Then connect the filters by calling [**IGraphBuilder::Connect**](igraphbuilder-connect.md), [**IGraphBuilder::Render**](igraphbuilder-render.md), or similar methods.
...
-   Before the filter chain's state changes, all data processing calls at the boundary of the filter chain must be completed. This rule applies to the methods [**IMemInputPin::Receive**](imeminputpin-receive.md), [**IPin::NewSegment**](ipin-newsegment.md), and [**IPin::EndOfStream**](ipin-endofstream.md). Filters in the chain must return from calls to these methods made by filters outside the chain; and filters outside the chain must return from calls made by filters within the chain.

For example, in the previous diagram, filter B must complete any data processing calls from filter A, and filter E must finish any calls from filter D. If the pins expose the [**IPinFlowControl**](ipinflowcontrol.md) and [**IPinConnection**](ipinconnection.md) interfaces, you can push the data through the graph by calling the [**IPinFlowControl::Block**](ipinflowcontrol-block.md) and [**IGraphConfig::PushThroughData**](igraphconfig-pushthroughdata.md) methods, as described in [Dynamic Reconnection](dynamic-reconnection.md). Filters might also support private methods for pushing the data.
