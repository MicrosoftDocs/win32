...
Filter merit. Used by the [**IGraphBuilder**](/windows/win32/Strmif/nn-strmif-igraphbuilder?branch=master) interface when constructing a filter graph. For a list of merit values, see [Merit](merit.md).
...
For information about using this structure, see [How to Register DirectShow Filters](how-to-register-directshow-filters.md). Use this structure only for filters that are registered in the default filter category (CLSID\_LegacyAmFilterCategory). To register a filter in a different category, use the [**IFilterMapper2::RegisterFilter**](/windows/win32/Strmif/nf-strmif-ifiltermapper2-registerfilter?branch=master) method, as described in [Implementing DllRegisterServer](implementing-dllregisterserver.md).
