...
When a renderer filter receives a sample, it schedules rendering based on the time stamp. If the sample arrives late, or has no time stamp, the filter renders the sample immediately. Otherwise, the filter waits until the sample's start time before it renders the sample. (It waits for the start time by calling the [**IReferenceClock::AdviseTime**](/windows/win32/Strmif/nf-strmif-ireferenceclock-advisetime?branch=master) method.)
...
To set the time stamp on a sample, call the [**IMediaSample::SetTime**](/windows/win32/Strmif/nf-strmif-imediasample-settime?branch=master) method.
