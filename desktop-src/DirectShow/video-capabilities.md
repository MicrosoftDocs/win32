...
The [**IAMStreamConfig::GetStreamCaps**](iamstreamconfig-getstreamcaps.md) method presents video capabilities in an array of pairs of [**AM\_MEDIA\_TYPE**](am-media-type.md) and [**VIDEO\_STREAM\_CONFIG\_CAPS**](video-stream-config-caps.md) structures. You can use this to expose all the formats and resolutions supported on a pin as discussed below.
...
A filter obtains its media type source and target rectangles from the [**VIDEOINFOHEADER**](videoinfoheader.md) structure's **rcSource** and **rcTarget** members, respectively. Filters do not have to support source and target rectangles.

The cropping rectangle described throughout the [**IAMStreamConfig**](iamstreamconfig.md) documentation is the same as the **VIDEOINFOHEADER** structure's **rcSource** rectangle for the output pin.
...
If a filter's output pin is connected to a media type with nonempty source and target rectangles, then your filter is required to stretch the input format's source subrectangle into the output format's target subrectangle. The source subrectangle is stored in the [**VIDEO\_STREAM\_CONFIG\_CAPS**](video-stream-config-caps.md) structure's **InputSize** member.
