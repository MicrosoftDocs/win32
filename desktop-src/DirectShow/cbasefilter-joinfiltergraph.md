...
The `JoinFilterGraph` method notifies the filter that it has joined or left a filter graph. This method implements the [**IBaseFilter::JoinFilterGraph**](ibasefilter-joinfiltergraph.md) method.
...
Pointer to the filter graph manager's [**IFilterGraph**](ifiltergraph.md) interface, or **NULL** if the filter is leaving the graph.
