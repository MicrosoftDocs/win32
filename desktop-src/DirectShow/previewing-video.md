...
To build a video preview graph, call the [**ICaptureGraphBuilder2::RenderStream**](icapturegraphbuilder2-renderstream.md) method as follows:
...
The first parameter to the [**ICaptureGraphBuilder2::RenderStream**](icapturegraphbuilder2-renderstream.md) method specifies a pin category; for a preview graph, use **PIN\_CATEGORY\_PREVIEW**. The second parameter specifies a media type, as a major type GUID. For video, use **MEDIATYPE\_Video**. DV devices deliver interleaved audio and video, for which the media type is **MEDIATYPE\_Interleaved**. (For more information about DV capture, see [Digital Video in DirectShow](digital-video-in-directshow.md).)

The third parameter is a pointer to the capture filter's [**IBaseFilter**](ibasefilter.md) interface. The next two parameters are not needed in this example. They are used to specify additional filters that might be needed to render the stream. Setting the last parameter to **NULL** causes the Capture Graph Builder to select a default renderer for the stream, based on the media type. For video, the Capture Graph Builder always uses the [Video Renderer](video-renderer-filter.md) filter as the default renderer.

> [!Note]  
> In Windows XP and later, although the Video Mixing Renderer (VMR) is the default video renderer for [**IGraphBuilder**](igraphbuilder.md) methods, it is not the default renderer for the [**RenderStream**](icapturegraphbuilder2-renderstream.md) method. On any platform, the Capture Graph Builder always uses the old Video Renderer filter unless you specify otherwise.
...
In some cases, the video stream must go through the Overlay Mixer filter. If so, the [**RenderStream**](icapturegraphbuilder2-renderstream.md) method adds it to the graph automatically.
