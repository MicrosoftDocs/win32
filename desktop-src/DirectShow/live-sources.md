...
-   The filter returns the AM\_FILTER\_MISC\_FLAGS\_IS\_SOURCE flag from the [**IAMFilterMiscFlags::GetMiscFlags**](/windows/win32/Strmif/nf-strmif-iamfiltermiscflags-getmiscflags?branch=master) method, AND at least one of its output pins exposes the [**IAMPushSource**](/windows/win32/Strmif/nn-strmif-iampushsource?branch=master) interface.
...
Live sources can be synchronized through the [**IAMPushSource**](/windows/win32/Strmif/nn-strmif-iampushsource?branch=master) interface. The Filter Graph Manager does not synchronize live sources unless the application enables synchronization by calling the [**IAMGraphStreams::SyncUsingStreamOffset**](/windows/win32/Strmif/nf-strmif-iamgraphstreams-syncusingstreamoffset?branch=master) method. If synchronization is enabled, the Filter Graph Manager queries each source filter for **IAMPushSource**. If the filter supports **IAMPushSource**, the Filter Graph Manager calls [**IAMLatency::GetLatency**](/windows/win32/Strmif/nf-strmif-iamlatency-getlatency?branch=master) to retrieve the filter's expected latency. (The **IAMPushSource** interface inherits [**IAMLatency**](/windows/win32/Strmif/nn-strmif-iamlatency?branch=master).) From the combined latency values, the Filter Graph Manager determines the maximum expected latency in the graph. It then calls [**IAMPushSource::SetStreamOffset**](/windows/win32/Strmif/nf-strmif-iampushsource-setstreamoffset?branch=master) to give each source filter a stream offset, which that filter adds to the time stamps that it generates.
...
Currently the [**IAMPushSource**](/windows/win32/Strmif/nn-strmif-iampushsource?branch=master) interface is supported by the [VFW Capture](vfw-capture-filter.md) filter and the [Audio Capture](audio-capture-filter.md) filter.
...
-   If there is a live source upstream, and that source exposes the [**IAMPushSource**](/windows/win32/Strmif/nn-strmif-iampushsource?branch=master) interface on its output pin, the audio renderer calls [**IAMPushSource::GetPushSourceFlags**](/windows/win32/Strmif/nf-strmif-iampushsource-getpushsourceflags?branch=master). It looks for one of the following flags:
    ...
    -   AM\_PUSHSOURCECAPS\_NOT\_LIVE. This flag means the source filter is not really a live source, even though it exposes the [**IAMPushSource**](/windows/win32/Strmif/nn-strmif-iampushsource?branch=master) interface. Therefore, the audio renderer does not match rates.
...
-   If [**GetPushSourceFlags**](/windows/win32/Strmif/nf-strmif-iampushsource-getpushsourceflags?branch=master) returns no flags (zero), the audio renderer's behavior depends on the graph clock and whether the samples have time stamps:
