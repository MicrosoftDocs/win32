...
1.  Call [**QueryInterface**](https://docs.microsoft.com/windows/desktop/api/unknwn/nf-unknwn-iunknown-queryinterface(q_)) on the capture filter for the [**IAMVideoProcAmp**](/windows/desktop/api/Strmif/nn-strmif-iamvideoprocamp) interface.
2.  For each property that you want to set, call the [**IAMVideoProcAmp::GetRange**](/windows/desktop/api/Strmif/nf-strmif-iamvideoprocamp-getrange) method. Properties are specified by the [**VideoProcAmpProperty**](/previous-versions/windows/desktop/api/strmif/ne-strmif-videoprocampproperty) enumeration. If the **GetRange** method fails, it means the camera does not support that particular property.
