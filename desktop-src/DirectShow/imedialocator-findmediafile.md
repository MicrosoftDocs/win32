...
A **BSTR** containing pairs of filter strings, formatted as required by the **lpstrFilter** member of the [**OPENFILENAME**](dlgbox.openfilename_str) structure. The media locator uses this filter if it displays a File Open dialog box. The value can be **NULL** if the *Flags* parameter does not include the SFN\_VALIDATEF\_POPUP flag.
