...
The following code shows how to implement the [**IMediaFilter::GetState**](imediafilter-getstate.md) method:
...
A capture filter's output pins should support the [**IAMStreamControl**](iamstreamcontrol.md) interface, so that an application can turn each pin on or off individually. For example, an application can preview without capturing, and then switch to capture mode without rebuilding the filter graph. You can use the [**CBaseStreamControl**](cbasestreamcontrol.md) class to implement this interface.
