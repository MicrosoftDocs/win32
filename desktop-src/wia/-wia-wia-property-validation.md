...
When an application performs an [IPropertyStorage::WriteMultiple](https://msdn.microsoft.com/480a2be3-ccb0-4135-a085-733f6ab48ccd) operation on any writeable Windows Image Acquisition (WIA) property, the WIA driver performs a validation on the new property value. Writing one property may have side affects that change other property values. It is up to the application to read all property values after a write operation to determine that all properties are set to values the application desires.

Multiple properties can be written simultaneously using the [IPropertyStorage::WriteMultiple](https://msdn.microsoft.com/480a2be3-ccb0-4135-a085-733f6ab48ccd) operation. There may be cases where some property assignments conflict. In such cases, the priority used to resolve conflicts is as follows:
