...
After the rowset has been returned, call [**IUnknown::QueryInterface**](https://msdn.microsoft.com/library/ms682521(v=VS.85).aspx) for the capabilities of the rowset and then call [IRowsetInfo::GetProperties](https://msdn.microsoft.com/library/ms719611(VS.85).aspx) for the new property (MSIDXSPROP\_SERVER\_VERSION). The property has a type of **VT\_I4** and can be one of the following values:
