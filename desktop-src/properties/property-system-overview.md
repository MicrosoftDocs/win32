...
-   A Shell data source.
-   An OLE DB provider.
-   A Saved Search file (.search-ms) that is used to initiate a query by navigating to the search file in Windows Explorer or binding to [**IShellFolder**](shell.IShellFolder) programmatically.
...
-   The relationship between Windows Search and indexing, and how to extend the index, are explained in the following topics in Windows Search:
    ...
    -   [Developing Protocol Handlers for Windows Search](search._search_3x_WDS_PHAddins)
    -   [Developing Property Handlers for Windows Search](search._search_3x_WDS_ExtIdx_PropertyHandlers)
