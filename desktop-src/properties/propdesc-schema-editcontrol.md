...
Specifies what control to use when editing the property. There should be only one [editControl](shell.propdesc_schema_editControl) element for each [displayInfo](shell.propdesc_schema_displayInfo) element.

If there are multiple elements, the last one is used. If no [editControl](shell.propdesc_schema_editControl) element is provided, then the default attribute settings are applied to the property description.
