...
Not supported in Windows 7 and later. Specifies what control to use in the query builder. There should be only one [queryControl](shell.propdesc_schema_queryControl) element for each [displayInfo](shell.propdesc_schema_displayInfo) element.

If there are multiple elements, the last one is used. If no [queryControl](shell.propdesc_schema_queryControl) element is provided, then the default attribute settings are applied to the property description.
