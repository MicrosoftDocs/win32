...
Before a property is used by the system, it is initialized by calling an implementation of [**IInitializeWithStream**](shell.IInitializeWithStream). The property handler should be initialized by having the system assign it a stream rather than leaving that assignment to the handler implementation. This method of initialization ensures the following things:
...
-   Use of the [**IInitializeWithStream**](shell.IInitializeWithStream) abstracts your implementation from file system details. This enables the handler to support initialization through alternative storages such as a File Transfer Protocol (FTP) folder or a compressed file with a .zip file name extension.

There are cases where initialization with streams is not possible. In those situations, there are two further interfaces that property handlers can implement: [**IInitializeWithFile**](shell.IInitializeWithFile) and [**IInitializeWithItem**](shell.IInitializeWithItem). If a property handler does not implement the [**IInitializeWithStream**](shell.IInitializeWithStream), it must opt out of running in the isolated process into which the system indexer would place it by default if there were a change to the stream. To opt out of this feature, set the following registry value.
...
However, it is far better to implement [**IInitializeWithStream**](shell.IInitializeWithStream) and do a stream-based initialization. Your property handler will be safer and more reliable as a result. Disabling process isolation is generally intended only for legacy property handlers and should be strenuously avoided by any new code.

To examine the implementation of a property handler in detail, look at the following code example, which is an implementation of [**IInitializeWithStream::Initialize**](shell.IInitializeWithStream_Initialize). The handler is initialized by loading an XML-based recipe document through a pointer to that document's associated [**IStream**](stg.istream) instance. The **\_spDocEle** variable used near the end of the code example is defined earlier in the sample as an MSXML2::IXMLDOMElementPtr.
...
Here is the full implementation of the **\_LoadProperties** method called by [**IInitializeWithStream::Initialize**](shell.IInitializeWithStream_Initialize).
...
In the implementation of **\_LoadProperty**, a property value is provided in the form of a [**PROPVARIANT**](stg.propvariant). A set of APIs in the software development kit (SDK) is provided to convert from primitive types such as **PWSTR** or **int** to or from **PROPVARIANT** types. These APIs are found in Propvarutil.h.

For example, to convert a [**PROPVARIANT**](stg.propvariant) to a string, you can use [**PropVariantToString**](shell.PropVariantToString) as illustrated here.
...
To initialize a PROPVARIANT from a string, you can use [**InitPropVariantFromString**](shell.InitPropVariantFromString).
...
Serialization APIs declared in Propsys.h are used to serialize and deserialize [**PROPVARIANT**](stg.propvariant) types into blobs of data, and then Base64 encoding is used to serialize those blobs into strings that can be saved in the XML. These strings are stored in the **EncodedValue** attribute of the **ExtendedProperties** element. The following utility method, implemented in the sample's Util.cpp file, performs the serialization. It begins with a call to the [**StgSerializePropVariant**](shell.StgSerializePropVariant) function to perform the binary serialization, as illustrated in the following code example.
...
Next, the [**CryptBinaryToString**](security.cryptbinarytostring)Â function, declared in Wincrypt.h, performs the Base64 conversion.
...
The following table summarizes the benefits of each approach using either [**IFilter**](search._search_IFilter) or [**IPropertyStore**](shell.IPropertyStore).
...
In the recipe handler sample, the recipe file format does not have any complex requirements, so only [**IPropertyStore**](shell.IPropertyStore) has been implemented for full-text support. Full-text search is implemented for the XML nodes named in the following array.
...
Because the sample uses in-memory cache, implementing [**IPropertyStore**](shell.IPropertyStore) methods is just a matter of delegating to that cache, as illustrated in the following code example.
...
-   [**IPropertyStore::GetCount**](shell.IPropertyStore_GetCount): If there are no properties, this method returns **S\_OK**.
-   [**IPropertyStore::GetAt**](shell.IPropertyStore_GetAt): If *iProp* is greater than or equal to *cProps*, this method returns E\_INVALIDARG and the structure pointed to by the *pkey* parameter is filled with zeroes.
-   [**IPropertyStore::GetCount**](shell.IPropertyStore_GetCount) and [**IPropertyStore::GetAt**](shell.IPropertyStore_GetAt) reflect the current state of the property handler. If a [**PROPERTYKEY**](shell.PROPERTYKEY) is added to or removed from the file through [**IPropertyStore::SetValue**](shell.IPropertyStore_SetValue), these two methods must reflect that change the next time they are called.
-   [**IPropertyStore::GetValue**](shell.IPropertyStore_GetValue): If this method is asked for a value that does not exist, it returns **S\_OK** with the value reported as VT\_EMPTY.

## Writing Back Values

When the property handler writes the value of a property using [**IPropertyStore::SetValue**](shell.IPropertyStore_SetValue), it does not write the value to the file until [**IPropertyStore::Commit**](shell.IPropertyStore_Commit) is called. The in-memory cache can be useful in implementing this scheme. In the sample code the **IPropertyStore::SetValue** implementation simply sets the new value in the in-memory cache and sets the state of that property to PSC\_DIRTY.
...
In any [**IPropertyStore**](shell.IPropertyStore) implementation, the following behavior is expected from [**IPropertyStore::SetValue**](shell.IPropertyStore_SetValue):
...
When a handler specifies the ManualSafeSave value, the stream with which it is initialized is not a transacted stream (STGM\_TRANSACTED). The handler itself must implement the safe save function to ensure that the file is not corrupted if the save operation is interrupted. If the handler implements in-place writing, it will write to the stream that it is given. If the handler does not support this feature, then it must retrieve a stream with which to write the updated copy of the file using [**IDestinationStreamFactory::GetDestinationStream**](shell.IDestinationStreamFactory_GetDestinationStream). After the handler is done writing, it should call [**IPropertyStore::Commit**](shell.IPropertyStore_Commit) on the original stream to complete the operation, and replace the contents of the original stream with the new copy of the file.

ManualSafeSave is also the default situation if you do not initialize your handler with a stream. Without an original stream to receive the contents of the temporary stream, you must use [**ReplaceFile**](fs.replacefile) to perform an atomic replacement of the source file.
...
As illustrated in the following sample implementation of [**IPropertyStore::Commit**](shell.IPropertyStore_Commit), the handler for ManualSafeSave is registered to illustrate the manual safe save option. The **\_SaveCacheToDom** method writes the property values stored in the in-memory cache to the XMLdocument object.
...
Next, ask whether the pecified supports [**IDestinationStreamFactory**](shell.IDestinationStreamFactory).
...
The [**IPropertyStoreCache::GetState**](shell.IPropertyStoreCache_GetState) method gets the state of the property in the cache. The PSC\_DIRTY flag, which was set in the [**IPropertyStore::SetValue**](shell.IPropertyStore_SetValue) implementation, marks a property as changed.
...
[**IPropertyStoreCapabilities**](shell.IPropertyStoreCapabilities) informs the Shell UI whether a particular property can be edited in the Shell UI. It is important to note that this relates only to the ability to edit the property in the UI, not whether you can successfully call [**IPropertyStore::SetValue**](shell.IPropertyStore_SetValue) on the property. A property that provokes a return value of S\_FALSE from [**IPropertyStoreCapabilities::IsPropertyWritable**](shell.IPropertyStoreCapabilities_IsPropertyWritable) might still be capable of being set through an application.
...
[**IsPropertyWritable**](shell.IPropertyStoreCapabilities_IsPropertyWritable) returns **S\_OK** to indicate that end users should be allowed to edit the property directly; S\_FALSE indicates that they should not. S\_FALSE can mean that applications are responsible for writing the property, not users. The Shell disables editing controls as appropriate based on the results of calls to this method. A handler that does not implement [**IPropertyStoreCapabilities**](shell.IPropertyStoreCapabilities) is assumed to support open metadata through support for the writing of any property.

If you are building a handler that handles only read-only properties, then you should implement your **Initialize** method ([**IInitializeWithStream**](shell.IInitializeWithStream), [**IInitializeWithItem**](shell.IInitializeWithItem), or [**IInitializeWithFile**](shell.IInitializeWithFile)) so that it returns STG\_E\_ACCESSDENIED when called with the STGM\_READWRITE flag.

Some properties have their [isInnate](shell.propdesc_schema_typeInfo) attribute set to **true**. Innate properties have the following characteristics:
...
With the property handler implemented, it must be registered and its file name extension associated with the handler. For more information, see [Registering and Distributing Property Handlers](shell.prophand_reg_dist).
