...
Specifies how the property's labels are displayed. There should be only one [labelInfo](shell.propdesc_schema_labelInfo) element for each [propertyDescription](shell.propdesc_schema_propertyDescription) element.

If there are multiple elements, the last one is used. If no [labelInfo](shell.propdesc_schema_labelInfo) element is provided, then the property's label is not displayed; however, this would typically be a defect.
