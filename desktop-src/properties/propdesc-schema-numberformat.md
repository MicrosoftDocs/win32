...
Specifies how [**IPropertyDescription::FormatForDisplay**](shell.IPropertyDescription_FormatForDisplay) should format the property's value as a string. This is applicable only if &lt;displayInfo displayType="Number"&gt;. There should be only one [numberFormat](shell.propdesc_schema_numberFormat) element for each [displayInfo](shell.propdesc_schema_displayInfo) element.

If there are multiple elements, the last one is used. If no [numberFormat](shell.propdesc_schema_numberFormat) element is provided, then the default attribute settings are applied to the property description.
