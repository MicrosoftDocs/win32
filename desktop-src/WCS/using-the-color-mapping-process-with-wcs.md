...
WCS color mapping is based on [device profiles](https://www.bing.com/search?q=device profiles). These are supplied by vendors of color hardware devices and installed when a device is installed. When color mapping is used by an application program, WCS accesses the device profile of the image to get the information needed to convert the image to the PCS. The conversion is done by the CMM.
...
After the image colors are in the [PCS](https://www.bing.com/search?q=PCS), WCS accesses the device profile of the destination device. It gets the CMM to convert the image colors from PCS to the gamut of the destination device.
