...
An RGB [color space](c.md#-color-wcs-1-0-glossary-color-space-gloss) is created by mapping the colors red, green, and blue onto a 3-D Cartesian coordinate system. This results is a 3-D cube like the ones shown in the following figure. This figure displays the same RGB cube from two different angles. Notice that the origin of the coordinate system is black. This is where the red, green, and blue (RGB) color components are all 0.0. The diagonally opposite corner of the cube is white, where the RGB color components are at their maximum value.

![rgb color space cube at maximum values](images/rgbclrs1.png)

![rgb color space cube at minimum values](images/rgbclrs2.png)

Like most [color spaces](c.md#-color-wcs-1-0-glossary-color-space-gloss), the RGB color space is normalized. That is, all color values are restricted to the range of zero to one inclusive. So black is (0.0, 0.0, 0.0), and white is (1.0, 1.0, 1.0).

In the RGB color space, the [primary colors](p.md#-color-wcs-1-0-glossary-primary-colors-gloss) are red, green, and blue. The secondary colors are cyan, yellow, and magenta.
