...
Data-item values must be rendered in the [**CF\_TEXT**](https://www.bing.com/search?q=**CF\_TEXT**) clipboard format. Individual elements of item values for a system topic must be delimited by tab characters. The following table suggests some items for the system topic.
...
To allow a server to impersonate a client, the client calls the [**DdeSetQualityOfService**](/windows/desktop/api/Dde/nf-dde-ddesetqualityofservice) function. The [**SECURITY\_IMPERSONATION\_LEVEL**](https://www.bing.com/search?q=**SECURITY\_IMPERSONATION\_LEVEL**) structure is used to control the level of impersonation the server may perform.
