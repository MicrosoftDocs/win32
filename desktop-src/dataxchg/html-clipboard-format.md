...
* Thus, the fragment effectively represents the _greater area_ on-screen within-which the user made their text selection (to Copy, for example). This contains the selected text plus the opening tags and attributes of any element that has an end tag within the selected text, and end tags at the end of the fragment for any start tag included. This is all information required for basic pasting of an HTML fragment.
