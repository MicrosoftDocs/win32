...
The low-order word is a handle to a global memory object containing a [**DDEADVISE**](/windows/win32/Dde/ns-dde-ddeadvise?branch=master) structure that specifies how the data is to be sent.
...
The client application allocates the global memory object using the [**GlobalAlloc**](https://msdn.microsoft.com/library/windows/desktop/aa366574) function. It allocates the atom using the [**GlobalAddAtom**](/windows/win32/Winbase/nf-winbase-globaladdatoma?branch=master) function.

The client application must create or reuse the **WM\_DDE\_ADVISE** *lParam* parameter by calling the [**PackDDElParam**](/windows/win32/Dde/nf-dde-packddelparam?branch=master) function or the [**ReuseDDElParam**](/windows/win32/Dde/nf-dde-reuseddelparam?branch=master) function.
