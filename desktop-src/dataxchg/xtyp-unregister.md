...
keywords: ["XTYP_UNREGISTER transaction Data Exchange"]
topic_type:
- apiref
api_name:
- XTYP_UNREGISTER
...
A Dynamic Data Exchange (DDE) callback function, [*DdeCallback*](ddecallback.md), receives the **XTYP\_UNREGISTER** transaction whenever a Dynamic Data Exchange Management Library (DDEML) server application uses the [**DdeNameService**](ddenameservice.md) function to unregister a service name, or whenever a non-DDEML application that supports the System topic is terminated.
