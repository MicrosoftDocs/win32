...
A client uses the **XTYP\_ADVSTART** transaction to establish an advise loop with a server. A Dynamic Data Exchange (DDE) server callback function, [*DdeCallback*](https://www.bing.com/search?q=*DdeCallback*), receives this transaction when a client specifies **XTYP\_ADVSTART** as the *wType* parameter of the [**DdeClientTransaction**](/windows/desktop/api/Ddeml/nf-ddeml-ddeclienttransaction) function.
