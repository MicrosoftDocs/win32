...
keywords: ["WM_CLEAR message Data Exchange"]
topic_type:
- apiref
api_name:
- WM_CLEAR
...
The deletion performed by the **WM\_CLEAR** message can be undone by sending the edit control an [**EM\_UNDO**](_win32_EM_UNDO) message.

To delete the current selection and place the deleted content on the clipboard, use the [**WM\_CUT**](wm-cut.md) message.

When sent to a combo box, the **WM\_CLEAR** message is handled by its edit control. This message has no effect when sent to a combo box with the [**CBS\_DROPDOWNLIST**](_win32_Combo_Box_Styles) style.
