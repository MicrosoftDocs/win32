...
See also [*big-endian*](https://www.bing.com/search?q=*big-endian*).
...
(LRA) An intermediary between a publisher and a [*certification authority*](https://www.bing.com/search?q=*certification authority*) (CA). The LRA can, for example, verify a publisher's credentials before sending them to the CA.
...
See [*virtual store*](https://www.bing.com/search?q=*virtual store*).
...
Information presented to the system by a [*security principal*](https://www.bing.com/search?q=*security principal*) for authentication.
...
A logon session begins whenever a user logs on to a computer. All processes in a logon session have the same primary [*access token*](https://www.bing.com/search?q=*access token*). The access token contains information about the security context of the logon session, including the user's SID, the *logon identifier*, and the *logon SID*.
...
A [*security identifier*](https://www.bing.com/search?q=*security identifier*) (SID) that identifies a *logon session*. You can use the logon SID in a [*DACL*](https://www.bing.com/search?q=*DACL*) to control access during a logon session. A logon SID is valid until the user logs off. A logon SID is unique while the computer is running; no other logon session will have the same logon SID. However, the set of possible logon SIDs is reset when the computer starts up. To retrieve the logon SID from an [*access token*](https://www.bing.com/search?q=*access token*), call the **GetTokenInformation** function for TokenGroups.
