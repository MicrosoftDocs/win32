...
Activating a computer creates a [*machine certificate*](m-gly.md#-rm-machine-certificate-gly) that signs the computer into an Active Directory Rights Management Services (AD RMS) Pre-production (or Production) hierarchy. Machine certificates are saved on a per user basis. That is, each user who has activated the computer has a unique machine certificate saved in the user's certificate store. You can obtain a machine certificate by performing any of the following tasks:

-   Run the Rmactivate\_isv.exe program (client Pre-production).
-   Run the Rmactivate\_ssp\_isv.exe program (server Pre-production).
-   Write your own program that calls the [**DRMActivate**](/windows/previous-versions/Msdrm/nf-msdrm-drmactivate?branch=master) function.
