...
-   Call [**DRMCreateIssuanceLicense**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmcreateissuancelicense) to create an empty license.
-   Call [**DRMCreateUser**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmcreateuser) to create a user object.
-   Call [**DRMCreateRight**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmcreateright) to create a right for the user.
-   Call [**DRMAddRightWithUser**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmaddrightwithuser) to bind the right and user to the license.
-   Create appropriate metadata and call [**DRMSetMetaData**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmsetmetadata) to set it in the license.
-   Call [**DRMSetApplicationSpecificData**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmsetapplicationspecificdata) to specify custom information, if any, that can be understood by the consuming application.
...
To specify an owner for the issuance license, call the [**DRMCreateUser**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmcreateuser) function before calling the [**DRMCreateIssuanceLicense**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmcreateissuancelicense) function as shown in the following example.

> [!Note]  
> When creating an application using the AD RMS SDK, if your intention is that the user creating the protected content has OWNER rights on the content, then you must explicitly add the OWNER right for that user to the issuance license. Do this by calling [**DRMAddRightWithUser**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmaddrightwithuser). Additionally, take note that the owner specified by *hOwner* is the Owner principal, not the OWNER right. Merely specifying someone as the *hOwner* does not grant them the OWNER right; granting yourself (or anyone else) the OWNER right must be done explicitly by using **DRMAddRightWithUser**. Zero, one, or more users can be granted the OWNER right, which never expires. For more information, see the *hOwner* parameter of [**DRMCreateIssuanceLicense**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmcreateissuancelicense).
