...
The **DRM\_MSG\_ACQUIRE\_LICENSE** message is sent to the custom callback function in response to the [**DRMAcquireLicense**](/windows/previous-versions/Msdrm/nf-msdrm-drmacquirelicense?branch=master) function being called with a license storage session handle.
...
<span id="S_DRM_REQUEST_PREPARED"></span><span id="s_drm_request_prepared"></span>****S\_DRM\_REQUEST\_PREPARED**** (A pointer to a [**DRM\_LICENSE\_ACQ\_DATA**](/windows/previous-versions/Msdrmdefs/ns-msdrmdefs-_drm_license_acq_data?branch=master) structure that lists license acquisition information for a nonsilent license request.)


</dt> <dd>

This is only returned when [**DRMAcquireLicense**](/windows/previous-versions/Msdrm/nf-msdrm-drmacquirelicense?branch=master) was called by using the **DRM\_AL\_NONSILENT** flag. Nonsilent license acquisition is supported only in Rights Management Services client 1.0.
