...
> [!Note]  
> To programmatically extract information from the **AUTHENTICATEDDATA** node, use the [**DRMGetBoundLicenseAttribute**](/windows/previous-versions/Msdrm/nf-msdrm-drmgetboundlicenseattribute?branch=master) function with *wszAttribute* set to **g\_wszQUERY\_APPDATANAME**. [**DRMGetBoundLicenseAttributeCount**](/windows/previous-versions/Msdrm/nf-msdrm-drmgetboundlicenseattributecount?branch=master) can be used to determine the count of such elements, and then **DRMGetBoundLicenseAttribute** can be used in a loop, with *wszAttribute* set to **g\_wszQUERY\_APPDATAVALUE**, to get the value of a specific element. See [Querying Licenses](querying-licenses.md) for more information about how to query a license programmatically.
