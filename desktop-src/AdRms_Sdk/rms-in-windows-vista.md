...
-   The [**DRMDeleteLicense**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmdeletelicense) function now allows you to delete an end-user license based on a handle to the client session or a handle to the license storage session.
-   The [**DRMActivate**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmactivate) function returns a new error code, E\_DRM\_PLATFORM\_POLICY\_VIOLATION, if a system DLL fails the module authentication check.
