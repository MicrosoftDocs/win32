...
Active Directory Rights Management Services (AD RMS) requires that any computer used to encrypt or decrypt content be trusted. Trust is established by creating a [*machine certificate*](m-gly.md#-rm-machine-certificate-gly) that identifies the computer and signs it into the Pre-production or Production certificate hierarchy. The hierarchy is a certificate chain that begins with a Microsoft root of trust and ends with the machine certificate. Each certificate in the chain signs the one following.
