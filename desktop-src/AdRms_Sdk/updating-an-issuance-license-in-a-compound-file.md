...
If a compound file is stored for any length of time, it is possible that the users and rights specified in the [*issuance license*](https://www.bing.com/search?q=*issuance license*) are out of date. You may need to add more users, for example, or remove a right from an existing user. To do this, you must rebuild the issuance license and use the latest template or specifications, republish the existing license, and then replace the old issuance license with the new one in the compound file.
