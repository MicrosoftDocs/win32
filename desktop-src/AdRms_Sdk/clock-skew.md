...
Issuance and use licenses can assign validity times to two things: for the license as a whole (specified in [**DRMCreateIssuanceLicense**](/windows/previous-versions/Msdrm/nf-msdrm-drmcreateissuancelicense?branch=master)), and for each individual right (specified in [**DRMCreateRight**](/windows/previous-versions/Msdrm/nf-msdrm-drmcreateright?branch=master)). All validity times in a license are set according to the publisher's clock. Clock skew across these times can cause problems in two places in the publishing and consuming cycle:
...
[**DRMCreateIssuanceLicense**](/windows/previous-versions/Msdrm/nf-msdrm-drmcreateissuancelicense?branch=master)
</dt> <dt>
