...
The AD RMS server licensor certificate chain holds the AD RMS public key used to identify the deployment and encrypt the content key in an issuance license. This certificate chain can be used in [**DRMGetSignedIssuanceLicense**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmgetsignedissuancelicense), when using that function with the **DRM\_SERVER\_ISSUANCELICENSE** flag to get the string unsigned [*issuance license*](https://www.bing.com/search?q=*issuance license*) chain. However, you must first convert it into an acceptable chain by using the [**DRMConstructCertificateChain**](/previous-versions/windows/desktop/api/Msdrm/nf-msdrm-drmconstructcertificatechain) function.
