...
> [!Note]  
> **WritePrinter** only supports GDI printing and must not be used for XPS printing. If your print job uses the XPS or the OpenXPS print path, then use the [XPS Print API](http://msdn.microsoft.com/library/windows/desktop/ff686814.aspx). Sending XPS or OpenXPS print jobs to the spooler using **WritePrinter** is not supported and can result in undetermined results.
