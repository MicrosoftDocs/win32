...
The [XPS Print API](xpsprint-api.md) enables native Windows applications to print XPS documents. An application can create an XPS document by using the [XPS Document API](https://www.bing.com/search?q=XPS Document API). The [Common XPS Document Programming Tasks](https://www.bing.com/search?q=Common XPS Document Programming Tasks) help topic describes how to do this. Once an XPS document has been created, the application can use the XPS Print API to print it.
...
The [XPS Print API](xpsprint-api.md) requires an XPS document to print. In the following example, the XPS document is created as it is sent to the printer by the XPS Print API. It is also possible to create an XPS document without sending it to a printer by using the [XPS Document API](https://www.bing.com/search?q=XPS Document API) and maintaining it as an XPS OM or by saving the XPS OM as an XPS document. For more information about using an XPS OM, see the XPS Document API.
...
Create an [**IXpsOMPackageWriter**](https://www.bing.com/search?q=**IXpsOMPackageWriter**) interface by calling [**IXpsOMObjectFactory::CreatePackageWriterOnStream**](https://www.bing.com/search?q=**IXpsOMObjectFactory::CreatePackageWriterOnStream**) on the stream returned by [**StartXpsPrintJob**](/windows/desktop/api/XpsPrint/nf-xpsprint-startxpsprintjob).
...
Start a new document in the package writer by calling [**IXpsOMPackageWriter::StartNewDocument**](https://www.bing.com/search?q=**IXpsOMPackageWriter::StartNewDocument**). If a document is open when this method is called, it is closed and a new document is opened.
...
Call [**IXpsOMPackageWriter::AddPage**](https://www.bing.com/search?q=**IXpsOMPackageWriter::AddPage**) to write each of the document's pages from the application to the new document in the package writer.

> [!Note]  
> The application is assumed to have created the page prior to this step. For more information about creating document pages and adding content to them, see the [Common XPS Document Programming Tasks](https://www.bing.com/search?q=Common XPS Document Programming Tasks).
...
After all the documents have been written for this print job, call [**IXpsOMPackageWriter::Close**](https://www.bing.com/search?q=**IXpsOMPackageWriter::Close**) to close the package.
