...
1.  Load the document into a signature manager, as described in [Initialize the Signature Manager](initialize-the-signature-manager.md).
2.  Get the collection of signatures from the digital signature manager.
3.  Get the number of signatures in the collection.
4.  For each signature in the collection, call the [**Verify**](/windows/desktop/api/xpsdigitalsignature/nf-xpsdigitalsignature-ixpssignature-verify) method as shown in the code example that follows.
...
1.  Create a temporary certificate store to hold the certificates used by the signature.
2.  Iterate through the signature's certificate set, and load each certificate into the temporary certificate store.
...
1.  Find the signing certificate by iterating through the certificates that are used by the signature.
2.  Test the certificate by verifying the signature against the certificate. The signing certificate is found when the [**Verify**](/windows/desktop/api/xpsdigitalsignature/nf-xpsdigitalsignature-ixpssignature-verify) method returns an [**XPS\_SIGNATURE\_STATUS**](/windows/desktop/api/xpsdigitalsignature/ne-xpsdigitalsignature-__midl___midl_itf_xpsdigitalsignature_0000_0000_0001) of **XPS\_SIGNATURE\_STATUS\_VALID** or **XPS\_SIGNATURE\_STATUS\_QUESTIONABLE**, and does not return a **FACILITY\_SECURITY** error.
...
1.  Save the returned signature status.
2.  Update the local status, if necessary, to perform subsequent certificate tests:
    1.  If the signature status is successful, set the local status to questionable in order to test the certificates.
    2.  If the signature status is incompliant, leave the local status as incompliant.
    3.  If the signature status is broken or incomplete, set the local status to broken.

A signature status of **XPS\_SIGNATURE\_STATUS\_INCOMPLIANT** means that parts of the XPS document that should not have been signed were signed, or parts of the XPS document that should have been signed were not. If [**Verify**](/windows/desktop/api/xpsdigitalsignature/nf-xpsdigitalsignature-ixpssignature-verify) returns this signature status, further checking of the signature will be unnecessary.
...
1.  Get the certificate trust status.
2.  Evaluate the returned certificate trust status.
3.  Return the resulting status.

The next code example does not test for every possible certificate trust status. For additional details on the status values that can be returned, see [**CERT\_TRUST\_STATUS**](https://msdn.microsoft.com/library/windows/desktop/aa377590).
...
[**CERT\_CHAIN\_CONTEXT**](https://msdn.microsoft.com/library/windows/desktop/aa377182)
</dt> <dt>

[**CERT\_CONTEXT**](https://msdn.microsoft.com/library/windows/desktop/aa377189)
</dt> <dt>

[**CERT\_TRUST\_STATUS**](https://msdn.microsoft.com/library/windows/desktop/aa377590)
</dt> <dt>

[**CertAddCertificateContextToStore**](https://msdn.microsoft.com/library/windows/desktop/aa376009)
</dt> <dt>

[**CertOpenStore**](https://msdn.microsoft.com/library/windows/desktop/aa376559)
</dt> <dt>

[**CertGetCertificateChain**](https://msdn.microsoft.com/library/windows/desktop/aa376078)
</dt> <dt>

[**IOpcCertificateEnumerator**](https://msdn.microsoft.com/library/windows/desktop/dd368258)
</dt> <dt>

[**IOpcCertificateEnumerator::GetCurrent**](https://msdn.microsoft.com/library/windows/desktop/dd368262)
</dt> <dt>

[**IOpcCertificateEnumerator::MoveNext**](https://msdn.microsoft.com/library/windows/desktop/dd368265)
</dt> <dt>
...
[**XPS\_SIGNATURE\_STATUS**](/windows/desktop/api/xpsdigitalsignature/ne-xpsdigitalsignature-__midl___midl_itf_xpsdigitalsignature_0000_0000_0001)
</dt> <dt>
