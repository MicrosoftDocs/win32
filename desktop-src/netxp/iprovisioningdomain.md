...
-   Add WPS XML files for pre-provisioning in the provisioning store. Files can only be added if the domain does not currently exist within the provisioning store.
    ...
    For more information about pre-provisioning, see [Wireless Network Provisioning](wireless-network-provisioning.md).
...
WPS manages XML files in the provisioning store based on a WISP domain. The domain and its parameters are defined through the [Master schema](master-schema.md).
...
The **IProvisioningDomain** interface supports the [Wireless Network Provisioning](wireless-network-provisioning.md)method.
