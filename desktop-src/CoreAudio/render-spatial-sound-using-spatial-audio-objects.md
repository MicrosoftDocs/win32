...
When you create a spatial audio stream, you must specify the audio format the stream will use by providing a [WAVEFORMATEX](mf.mf_mt_audio_prefer_waveformatex_attribute) structure. If you are playing back audio from a file, the format is typically determined by the audio file format. This example uses a mono, 32-bit, 48Hz format.
