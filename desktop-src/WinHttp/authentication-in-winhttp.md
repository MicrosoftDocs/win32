...
The WinHTTP application programming interface (API) provides two functions used to access Internet resources in situations where authentication is required: [**WinHttpSetCredentials**](winhttpsetcredentials.md) and [**WinHttpQueryAuthSchemes**](winhttpqueryauthschemes.md).

When a response is received with a 401 or 407 status code, [**WinHttpQueryAuthSchemes**](winhttpqueryauthschemes.md) can be used to parse the authentication headers to determine the supported authentication schemes and the authentication target. The authentication target is the server or proxy that requests authentication. **WinHttpQueryAuthSchemes** also determines the first authentication scheme, from the available schemes, based on the authentication scheme preferences suggested by the server. This method for choosing an authentication scheme is the behavior suggested by [RFC 2616](Http://go.microsoft.com/fwlink/p/?linkid=84048).

[**WinHttpSetCredentials**](winhttpsetcredentials.md) enables an application to specify the authentication scheme that is used along with a valid username and password for use on the target server or proxy. After setting the credentials and resending the request, the necessary headers are generated and added to the request automatically. Because some authentication schemes require multiple transactions [**WinHttpSendRequest**](winhttpsendrequest.md) could return the error, ERROR\_WINHTTP\_RESEND\_REQUEST. When this error is encountered, the application should continue to resend the request until a response is received that does not contain a 401 or 407 status code. A 200 status code indicates that the resource is available and the request is successful. See [**HTTP Status Codes**](http-status-codes.md) for additional status codes that can be returned.

If an acceptable authentication scheme and credentials are known before a request is sent to the server, an application can call [**WinHttpSetCredentials**](winhttpsetcredentials.md) before calling [**WinHttpSendRequest**](winhttpsendrequest.md). In this case, WinHTTP attempts pre-authentication with the server by providing credentials or [*authentication data*](glossary.md#term-authentication-data) in the initial request to the server. Pre-authentication can decrease the number of exchanges in the authentication process and therefore improve application performance.
...
-   Request a resource with [**WinHttpOpenRequest**](winhttpopenrequest.md) and [**WinHttpSendRequest**](winhttpsendrequest.md).
-   Check the response headers with [**WinHttpQueryHeaders**](winhttpqueryheaders.md).
-   If a 401 or 407 status code is returned indicating that authentication is required, call [**WinHttpQueryAuthSchemes**](winhttpqueryauthschemes.md) to find an acceptable scheme.
-   Set the authentication scheme, username, and password with [**WinHttpSetCredentials**](winhttpsetcredentials.md).
-   Resend the request with the same request handle by calling [**WinHttpSendRequest**](winhttpsendrequest.md).

The credentials set by [**WinHttpSetCredentials**](winhttpsetcredentials.md) are only used for one request. WinHTTP does not cache the credentials to use in other requests, which means that applications must be written that can respond to multiple requests. If an authenticated connection is re-used, other requests may not be challenged, but your code should be able to respond to a request at any time.
