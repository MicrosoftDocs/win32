...
A Passport login server handles all requests for [*tickets*](https://www.bing.com/search?q=*tickets*) for any resource in a Passport *domain authority*. Before a request can be authenticated using Passport, the client application must contact the login server to obtain the appropriate *tickets*.

When a client requests [*tickets*](https://www.bing.com/search?q=*tickets*) from a Passport login server, the login server typically responds with a 401 status code to indicate that user credentials must be provided. When these credentials are provided, the login server responds with the *tickets* required to access the specified resource on the server that contains the originally requested resource. The login server can also redirect the client to another server that can provide the requested resource.
...
Once retrieved, [*tickets*](https://www.bing.com/search?q=*tickets*) are managed internally and are automatically sent to applicable servers in future requests.

> [!Note]  
> WinHTTP enables you to disable automatic redirection by calling the [**WinHttpSetOption**](/windows/desktop/api/Winhttp/nf-winhttp-winhttpsetoption) function for the [**WINHTTP\_OPTION\_DISABLE\_FEATURE**](https://www.bing.com/search?q=**WINHTTP\_OPTION\_DISABLE\_FEATURE**) flag and specifying a value of [**WINHTTP\_DISABLE\_REDIRECTS**](https://www.bing.com/search?q=**WINHTTP\_DISABLE\_REDIRECTS**). Disabling redirection does not interfere with the redirection that WinHTTP handles internally for Passport transactions.
