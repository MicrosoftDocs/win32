...
If a server certificate contains errors, details about the error can be obtained in the status callback function. The [**WINHTTP\_CALLBACK\_STATUS\_SECURE\_FAILURE**](/windows/desktop/api/Winhttp/nc-winhttp-winhttp_status_callback) notification indicates an error with a server certificate. The *lpvStatusInformation* parameter contains one or more detailed error flags. See [**WINHTTP\_STATUS\_CALLBACK**](/windows/desktop/api/Winhttp/nc-winhttp-winhttp_status_callback) for more information.
