...
The error values listed below are returned by [**GetLastError**](https://msdn.microsoft.com/library/windows/desktop/ms679360) when one of the Microsoft Windows HTTP Services (WinHTTP) functions fails, and are also returned in the lower 16 bits of [**HRESULT**](_com_structure_of_com_error_codes) error returns from the [**WinHttpRequest**](winhttprequest.md) object.
