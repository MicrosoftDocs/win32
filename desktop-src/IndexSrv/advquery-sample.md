...
This sample is more complex than the Simple (QSample) sample because it creates a command tree and use the low-level [**ICommandTree**](icommandtree.md) interface instead of using the OLE DB Helper functions [CICreateCommand](/windows/desktop/api/Ntquery/nf-ntquery-cicreatecommand) and [CITextToFullTree](/windows/desktop/api/Ntquery/nf-ntquery-citexttofulltree).
