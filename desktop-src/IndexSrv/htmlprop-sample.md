...
The HtmlProp sample is an example [**IFilter**](/windows/desktop/api/Filter/nn-filter-ifilter) implementation that specializes the Indexing Service [HTML filter](html-filter.md) to extract value-type properties. It converts HTML meta properties to data types other than strings as specified by a configuration file.
