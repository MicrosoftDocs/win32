...
The Simple sample (QSample) is an example command-line application written in C++ that executes a query using the OLE DB Helper API functions [CICreateCommand](/windows/desktop/api/Ntquery/nf-ntquery-cicreatecommand) and [CITextToFullTree](/windows/desktop/api/Ntquery/nf-ntquery-citexttofulltree) to simplify the coding needed to create a query to an Indexing Service catalog.
...
This sample is simpler than the AdvQuery sample because it uses the OLE DB Helper functions [**CICreateCommand**](/windows/desktop/api/Ntquery/nf-ntquery-cicreatecommand) and [**CITextToFullTree**](/windows/desktop/api/Ntquery/nf-ntquery-citexttofulltree) instead of creating a command tree and using the low-level **ICommandTree** interface.
