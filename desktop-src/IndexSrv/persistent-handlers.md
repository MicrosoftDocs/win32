...
Indexing Service uses a special binding procedure to access [**IFilter**](/windows/win32/Filter/nn-filter-ifilter?branch=master) interface implementations because the CiDaemon.exe process operates on the persistent image of an object and does not, as in the more common OLE case, bind to an in-memory image of an object. Using a persistent handler allows the CiDaemon.exe process to bind to only the **IFilter** interface for a file instead of being registered as the COM server for all clients that want to access the file.
...
Indexing Service supplies [helper functions](functions.md) that implement a subset of the COM persistent handler functionality. You can use these functions to access the [**IFilter**](/windows/win32/Filter/nn-filter-ifilter?branch=master) interface implementations for embedded or linked objects.
