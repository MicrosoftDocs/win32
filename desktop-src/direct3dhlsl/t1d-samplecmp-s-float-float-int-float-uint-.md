---
title: SampleCmp::SampleCmp(S,float,float,int,float,uint) function for Texture1D
description: The function samples a texture, using a comparison value to reject samples, with an optional value to clamp sample level-of-detail (LOD) values to. For Texture1D.
