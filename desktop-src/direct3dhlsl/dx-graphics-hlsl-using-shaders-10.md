...
-   Shader compiler flags. These flags tell the compiler what information to put into the compiled output and how you want the output code optimized: for speed, for debug, etc. See [Effect Constants (Direct3D 10)](https://msdn.microsoft.com/library/windows/desktop/bb205176) for a listing of the available flags. The sample contains some code you can use to set the compiler flag values for your project - this is mainly a question of whether or not you want to generate debug information.
...
Beginning with the December 2006 SDK, the DirectX 10 HLSL compiler is now the default compiler in both DirectX 9 and DirectX 10. See [Effect-Compiler Tool](https://msdn.microsoft.com/library/windows/desktop/bb232919) for details.

### Get a Pointer to a Compiled Shader

Several API methods require a pointer to a compiled shader. This argument is usually called *pShaderBytecode* because it points to a compiled shader represented as a sequence of byte codes. To get a pointer to a compiled shader, first compile the shader by calling [**D3D10CompileShader**](https://msdn.microsoft.com/library/windows/desktop/bb205084) or a similar function. If compilation is successful, the compiled shader is returned in an [**ID3D10Blob**](https://msdn.microsoft.com/library/windows/desktop/bb173507) interface. Finally, use the [**GetBufferPointer**](https://msdn.microsoft.com/library/windows/desktop/bb173508) method to return the pointer.
