...
Beginning with the December 2006 SDK, the DirectX 10 HLSL compiler is now the default compiler in both DirectX 9 and DirectX 10. See [Effect-Compiler Tool](https://docs.microsoft.com/windows/desktop/direct3dtools/fxc) for details.

### Get a Pointer to a Compiled Shader

Several API methods require a pointer to a compiled shader. This argument is usually called *pShaderBytecode* because it points to a compiled shader represented as a sequence of byte codes. To get a pointer to a compiled shader, first compile the shader by calling [**D3D10CompileShader**](https://docs.microsoft.com/windows/desktop/api/d3d10shader/nf-d3d10shader-d3d10compileshader) or a similar function. If compilation is successful, the compiled shader is returned in an [**ID3D10Blob**](https://docs.microsoft.com/windows/desktop/api/d3dcommon/nn-d3dcommon-id3d10blob) interface. Finally, use the [**GetBufferPointer**](https://docs.microsoft.com/windows/desktop/api/d3dcommon/nf-d3dcommon-id3d10blob-getbufferpointer) method to return the pointer.
