...
| SV\_ViewportArrayIndex                                  | Viewport array index. Applied to geometry shader output and indicates which viewport to use for the primitive currently being written out. Can be read or written by the pixel shader. The primitive will be transformed and clipped against the viewport specified by the index before it is passed to the rasterizer. This semantic applies only to primitives, if a primitive has more than one vertex the value from the leading vertex will be used. <br/> If [VPAndRTArrayIndexFromAnyShaderFeedingRasterizer in D3D11_FEATURE_DATA_D3D11_OPTIONS3](https://docs.microsoft.com/en-us/windows/win32/api/d3d11/ns-d3d11-d3d11_feature_data_d3d11_options3) is true, SV\_ViewportArrayIndex applied to any shader feeding the rasterizer.                                                                                                                                                                                                                                                                                  | uint              |
