...
Any texture data (such as position, normal, and light source direction) can be mapped by a vertex shader into a texture coordinate. This is done by associating a texture with a texture register using [**SetTexture**](https://msdn.microsoft.com/library/windows/desktop/bb205727) and by specifying how the texture sampling is done using [**SetTextureStageState**](https://msdn.microsoft.com/library/windows/desktop/bb174462). If the fixed function pipeline is used, be sure to supply the TSS\_TEXCOORDINDEX flag.
