...
You can also use the set of regular input scopes that are defined as part of the [**SetInputScope**](tsf.setinputscope) function in your regular expressions. For example, the value for Month (numerical) is IS\_DATE\_MONTH. The syntax for specifying an input scope as part of a regular expression is to prepend the input scope enumerated value with an open parenthesis and an exclamation point, (!, and place a closing parenthesis, ), on the end. For example:
...
> [!Note]  
> The following members of [**SetInputScope**](tsf.setinputscope) are not supported in regular expressions: IS\_PHRASELIST, IS\_REGULAREXPRESSION, IS\_SRGS, IS\_XML.
