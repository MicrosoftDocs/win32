...
You can define and assign custom input scopes using the handwriting regular expression syntax that is understood by some of the Microsoft handwriting recognizers. The syntax is a subset of the [Regular Expression Language Implementation](http://go.microsoft.com/fwlink/p/?linkid=20319) in the .NET Framework.
...
You can also use the set of regular input scopes that are defined as part of the [**SetInputScope**](https://msdn.microsoft.com/windows/desktop/4098525c-8d29-419a-9484-9e70420416bc) function in your regular expressions. For example, the value for Month (numerical) is IS\_DATE\_MONTH. The syntax for specifying an input scope as part of a regular expression is to prepend the input scope enumerated value with an open parenthesis and an exclamation point, (!, and place a closing parenthesis, ), on the end. For example:
...
> [!Note]  
> The following members of [**SetInputScope**](https://msdn.microsoft.com/windows/desktop/4098525c-8d29-419a-9484-9e70420416bc) are not supported in regular expressions: IS\_PHRASELIST, IS\_REGULAREXPRESSION, IS\_SRGS, IS\_XML.
