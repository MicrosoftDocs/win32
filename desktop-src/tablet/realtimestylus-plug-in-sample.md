...
Plug-ins, objects that implement the [**IStylusSyncPlugin**](istylussyncplugin.md) or [**IStylusAsyncPlugin**](istylusasyncplugin.md) interface can be added to a [**RealTimeStylus**](realtimestylus-class.md) object. This sample application uses several types of plug-in:
...
-   Dynamic Renderer Plug-in: Modifies dynamic rendering qualities. This sample demonstrates use of the [**DynamicRenderer**](dynamicrenderer-class.md) object as a plug-in to handle dynamic rendering of ink.
...
The `CustomDynamicRenderer` class also implements the [**IStylusSyncPlugin**](istylussyncplugin.md) class to receive pen-input notifications. It then handles the `Packets` notification to draw a small circle around each new packet point.
...
The RealTimeStylusPluginApp project demonstrates the plug-ins previously described, as well as the [**GestureRecognizer**](gesturerecognizer-class.md) and [**DynamicRenderer**](dynamicrenderer-class.md) plug-ins. The project's user interface consists of:
...
The `RealTimeStylusPluginApp` class itself implements the [**IStylusAsyncPlugin**](istylusasyncplugin.md) class. This is necessary so that the `RealTimeStylusPluginApp` class can be notified when the [**GestureRecognizer**](gesturerecognizer-class.md) plug-in adds gesture data to the output queue. The application registers for notification of [CustomStylusDataAdded](frlrfMicrosoftStylusInputIStylusSyncPluginClassCustomStylusDataAddedTopic). When gesture data is received, `RealTimeStylusPluginApp` places a description of it on the status bar at the bottom of the form.
