...
This technique is useful in cases where one [PenInputPanel](https://www.bing.com/search?q=PenInputPanel) object will be associated with a single control throughout its lifetime. In cases where you want to use one PenInputPanel object and associate it with multiple controls, as demonstrated in the [PenInputPanel Sample](peninputpanel-sample.md), use the [AttachedEditControl](https://www.bing.com/search?q=AttachedEditControl) property to change the control to which the PenInputPanel object is associated.

To attach a [PenInputPanel](https://www.bing.com/search?q=PenInputPanel) object to a control without using a constructor, use the [AttachedEditControl](https://www.bing.com/search?q=AttachedEditControl) property. Use this technique for languages that do not support the managed constructors, such as C++.
