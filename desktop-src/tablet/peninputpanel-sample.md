...
The [InkEdit](frlrfMicrosoftInkInkEditClassTopic) control already provides a correction user interface, but in the sample [EnableTsf](frlrfMicrosoftInkPenInputPanelClassEnableTsfTopic) is used to enable the [PenInputPanel](frlrfMicrosoftInkPenInputPanelClassTopic) to use the TSF insertion recognizer context rather than the [**SendInput**](https://msdn.microsoft.com/en-us/library/ms646310(v=VS.85).aspx) function to send the handwriting recognition results into the control. The result is that text can be inserted even if the field no longer has focus.
