...
When the [**CollectionMode**](inkpicture-collectionmode.md) property is set to [**GestureOnly**](inkcollectionmode.md), the timeout between when a user adds a gesture and when the **Gesture** event occurs is a fixed value that you cannot alter programmatically. Gesture recognition is faster in **InkAndGesture** mode.

To prevent the collection of ink while in [**InkAndGesture**](inkcollectionmode.md) mode:

-   Set [**CollectionMode**](inkpicture-collectionmode.md) to [**InkAndGesture**](inkcollectionmode.md).
-   Delete the stroke in the [**Stroke**](inkpicture-stroke.md) event.
-   Process the gesture in the **Gesture** event.

To prevent the flow of ink while gesturing, set [**DynamicRendering**](inkpicture-dynamicrendering.md) property to **FALSE**.
...
For this event to occur, the object or control must have interest in a set of application gestures. To set the objects or controls interest in a set of gestures, call the [**SetGestureStatus**](inkpicture-setgesturestatus.md) method of the object or control.
