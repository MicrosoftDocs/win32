...
The [**RealTimeStylus**](realtimestylus-class.md) object throws an exception if a plug-in calls the **RealTimeStylus** object's [Dispose](frlrfMicrosoftStylusInputRealTimeStylusClassDisposeTopic) method. This happens only when the plug-in does so while it is being called by the **RealTimeStylus** object.
...
> [!Note]  
> For overall stability, reset a plug-in's internal state when either its [**RealTimeStylusEnabled**](istylusplugin-realtimestylusenabled.md) or [**RealTimeStylusDisabled**](istylusplugin-realtimestylusdisabled.md) method is called.
