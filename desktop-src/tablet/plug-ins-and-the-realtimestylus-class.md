...
Both the [**IStylusSyncPlugin**](/windows/desktop/api/RTSCom/) and [**IStylusAsyncPlugin**](/windows/desktop/api/RTSCom/) interfaces define the same methods. These methods allow the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object to pass the pen data to each plug-in, regardless of whether it is an asynchronous or synchronous plug-in. The [Microsoft.StylusInput.IStylusSyncPlugin.DataInterest](https://www.bing.com/search?q=Microsoft.StylusInput.IStylusSyncPlugin.DataInterest) and [Microsoft.StylusInput.IStylusAsyncPlugin.DataInterest](https://www.bing.com/search?q=Microsoft.StylusInput.IStylusAsyncPlugin.DataInterest) properties allow each plug-in to subscribe to specific data in the tablet pen data stream. A plug-in should only subscribe to the data necessary to perform its task, minimizing performance demands. For more information about threading and the **RealTimeStylus** class, see [Threading Considerations for the StylusInput APIs](threading-considerations-for-the-stylusinput-apis.md).

The [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object uses objects in the [Microsoft.StylusInput.PluginData](https://www.bing.com/search?q=Microsoft.StylusInput.PluginData) namespace to pass the pen data to its plug-ins. The **RealTimeStylus** also catches exceptions thrown by plug-ins. When the **RealTimeStylus** catches exceptions, it notifies the plug-ins by calling the [IStylusSyncPlugin.Error](https://www.bing.com/search?q=IStylusSyncPlugin.Error) or [IStylusAsyncPlugin.Error](https://www.bing.com/search?q=IStylusAsyncPlugin.Error) method. For more information about using plug-in data, see [Plug-in Data and the RealTimeStylus](plug-in-data-and-the-realtimestylus-class.md) Class. For more information about error handling, see [Error Handling Considerations for the StylusInput APIs](error-handling-considerations-for-the-stylusinput-apis.md) and the Error Data section of Plug-in Data and the RealTimeStylus Class.

> [!Note]  
> At least one plug-in must be attached to the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object before the **RealTimeStylus** object can be enabled.
...
Because of the complex nature of the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object, you should take note of the following.

The [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object throws an exception if a plug-in modifies the plug-in collection to which it is attached. This happens only when the plug-in does so while it is being called by the **RealTimeStylus** object as a member of that collection.

The following C\# example is code that causes the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object to throw an exception.
...
The [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object throws an exception if a plug-in calls the **RealTimeStylus** object's [Dispose](https://www.bing.com/search?q=Dispose) method. This happens only when the plug-in does so while it is being called by the **RealTimeStylus** object.
...
Plug-in collections can be modified while the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object is enabled; however, this can make the behavior of your application harder to predict. When a plug-in is added while the **RealTimeStylus** object is enabled, the **RealTimeStylus** object calls the plug-in's [IStylusAsyncPlugin.RealTimeStylusEnabled](https://www.bing.com/search?q=IStylusAsyncPlugin.RealTimeStylusEnabled) or [IStylusSyncPlugin.RealTimeStylusEnabled](https://www.bing.com/search?q=IStylusSyncPlugin.RealTimeStylusEnabled) method. When a plug-in is removed while the **RealTimeStylus** object is enabled, the **RealTimeStylus** object calls the plug-in's [IStylusAsyncPlugin.RealTimeStylusDisabled](https://www.bing.com/search?q=IStylusAsyncPlugin.RealTimeStylusDisabled) or [IStylusSyncPlugin.RealTimeStylusDisabled](https://www.bing.com/search?q=IStylusSyncPlugin.RealTimeStylusDisabled) method. This allows the plug-in to maintain its proper state without having to check the [Enabled](https://www.bing.com/search?q=Enabled) property of the **RealTimeStylus** object.

When a plug-in is added while the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object is enabled, the plug-in data that the plug-in receives may not contain enough information to adequately establish the context of the initial data. For example, the newly added plug-in could start receiving packet data from a pen that is mid-stroke. Similarly, when a plug-in removed while the **RealTimeStylus** object is enabled, the plug-in data that the plug-in has received may be insufficient to finish processing the data.
