...
-   Ensure that [**WM\_GETTEXT**](winmsg.wm_gettext) is supported.
-   Ensure that all class-specific messages are supported. It is especially important to support text-retrieval messages such as [**CB\_GETLBTEXT**](controls.CB_GETLBTEXT) and [**LB\_GETTEXT**](controls.LB_GETTEXT). Set the appropriate style bits, such as **CBS\_HASSTRINGS** and **LBS\_HASSTRINGS**, to indicate the control supports those messages.
