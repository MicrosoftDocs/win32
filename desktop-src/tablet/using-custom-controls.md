...
-   Ensure that [**WM\_GETTEXT**](https://msdn.microsoft.com/VS|winui|~\winui\windowsuserinterface\windowing\windows\windowreference\windowmessages\wm_gettext.htm) is supported.
-   Ensure that all class-specific messages are supported. It is especially important to support text-retrieval messages such as [**CB\_GETLBTEXT**](https://msdn.microsoft.com/VS|Controls|~\controls\comboboxes\comboboxreference\comboboxmessages\cb_getlbtext.htm) and [**LB\_GETTEXT**](https://msdn.microsoft.com/VS|Controls|~\controls\listboxes\listboxreference\listboxmessages\lb_gettext.htm). Set the appropriate style bits, such as **CBS\_HASSTRINGS** and **LBS\_HASSTRINGS**, to indicate the control supports those messages.
