...
-   Copy ink, along with text, between two applications that recognize ink, each of which has more than one [**Ink**](/windows/desktop/api/msinkaut/) object. This requires HTML, RTF, or a similar format.
-   Copy ink between two applications, each of which has one [**Ink**](/windows/desktop/api/msinkaut/) object. Ink Serialized Format (ISF) is sufficient for this.
-   Copy ink from an application that has more than one [**Ink**](/windows/desktop/api/msinkaut/) object to an application that has only one **Ink** object. In this case, the application that has more than one **Ink** object must be able to produce Ink Serialized Format (ISF).
-   Copy ink from an application that has one [**Ink**](/windows/desktop/api/msinkaut/) object to an application that has more than one **Ink** object. In this case, the application that has more than one **Ink** object must be able to recognize ISF.
