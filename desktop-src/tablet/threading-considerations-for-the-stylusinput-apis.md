...
The following [**IStylusSyncPlugin**](istylussyncplugin.md) interface methods may call on a thread other than the tablet pen data thread.

-   The [RealTimeStylusEnabled](frlrfMicrosoftStylusInputIStylusSyncPluginClassRealTimeStylusEnabledTopic) and [RealTimeStylusDisabled](frlrfMicrosoftStylusInputIStylusSyncPluginClassRealTimeStylusDisabledTopic) methods are called on the thread that updates the [**RealTimeStylus**](realtimestylus-class.md) object's [Enabled](frlrfMicrosoftStylusInputRealTimeStylusClassEnabledTopic) property or that adds or removes the plug-in while the **RealTimeStylus** object is enabled.
-   The [CustomStylusDataAdded](frlrfMicrosoftStylusInputIStylusSyncPluginClassCustomStylusDataAddedTopic) method is called on the thread that calls the [**RealTimeStylus**](realtimestylus-class.md) object's [AddCustomStylusDataToQueue](frlrfMicrosoftStylusInputRealTimeStylusClassAddCustomStylusDataToQueueTopic) method.
...
To interact with your application from a synchronous plug-in, use the [AddCustomStylusDataToQueue](frlrfMicrosoftStylusInputRealTimeStylusClassAddCustomStylusDataToQueueTopic) method of the [**RealTimeStylus**](realtimestylus-class.md) object and handle the custom stylus data in one of your asynchronous plug-ins. If you make a synchronous call to another thread from a synchronous plug-in, you may block the **RealTimeStylus** object and thus block ink collection.
