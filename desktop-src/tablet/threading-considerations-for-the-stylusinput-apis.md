...
The following [**IStylusSyncPlugin**](/windows/desktop/api/RTSCom/) interface methods may call on a thread other than the tablet pen data thread.

-   The [RealTimeStylusEnabled](https://www.bing.com/search?q=RealTimeStylusEnabled) and [RealTimeStylusDisabled](https://www.bing.com/search?q=RealTimeStylusDisabled) methods are called on the thread that updates the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object's [Enabled](https://www.bing.com/search?q=Enabled) property or that adds or removes the plug-in while the **RealTimeStylus** object is enabled.
-   The [CustomStylusDataAdded](https://www.bing.com/search?q=CustomStylusDataAdded) method is called on the thread that calls the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object's [AddCustomStylusDataToQueue](https://www.bing.com/search?q=AddCustomStylusDataToQueue) method.
...
To interact with your application from a synchronous plug-in, use the [AddCustomStylusDataToQueue](https://www.bing.com/search?q=AddCustomStylusDataToQueue) method of the [**RealTimeStylus**](/windows/desktop/api/RTSCom/) object and handle the custom stylus data in one of your asynchronous plug-ins. If you make a synchronous call to another thread from a synchronous plug-in, you may block the **RealTimeStylus** object and thus block ink collection.
