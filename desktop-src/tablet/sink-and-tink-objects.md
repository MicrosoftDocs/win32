...
-   Use of ambient properties. Ambient properties such as font size, color, and bold formatting need to be transmitted to the tInk object. Application of these properties changes the width of the handwritten ink, so a size update is required by calling the [**GetInkExtent Method**](/windows/desktop/api/msinkaut/nf-msinkaut-iinklineinfo-getinkextent) or [IOleObject::GetExtent](https://msdn.microsoft.com/en-us/library/ms692325(v=VS.85).aspx) method.
-   Override the default [IOleObject::DoVerb](https://msdn.microsoft.com/en-us/library/ms694508(v=VS.85).aspx) method processing. This allows conversion to text to pass a batch of tInk objects to the recognizer, which can then break the words into recognition segments.
