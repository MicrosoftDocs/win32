...
Second, the application's code that is responsible for inserting text into a text field must be altered so that it no longer inserts text into a text field directly, but calls one or the other of [**IHandwrittenTextInsertion**](ihandwrittentextinsertion.md)'s two insertion methods instead. Whether the applications should choose to call [**InsertRecognitionResultsArray**](ihandwrittentextinsertion-insertrecognitionresultsarray.md) or [**InsertRecognitionResults**](ihandwrittentextinsertion-insertinkrecognitionresult.md) depends on whether the application has the recognition alternates for the text stored as an array or as an [**IInkRecognitionResult**](iinkrecognitionresult.md) object.
