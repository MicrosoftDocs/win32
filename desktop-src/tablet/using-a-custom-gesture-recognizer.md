...
You can use a custom gesture recognizer independently, or in addition to the [**GestureRecognizer**](/windows/win32/RTSCom/?branch=master).

Create your custom gesture recognizer as an [**IStylusSyncPlugin**](/windows/win32/RTSCom/?branch=master), have it create [CustomStylusData](frlrfMicrosoftStylusInputPluginDataCustomStylusDataClassTopic), and include the plug-in in the same [StylusSyncPluginCollection](frlrfMicrosoftStylusInputStylusSyncPluginCollectionClassTopic) as the [**GestureRecognizer**](/windows/win32/RTSCom/?branch=master). In the **IStylusSyncPlugin**, combine gesture notifications from both recognizers into notifications to be consumed by an application.
