...
\[[**PenInputPanel**](/windows/win32/msinkaut/?branch=master) has been replaced by [**TextInput**](/windows/win32/peninputpanel/nn-peninputpanel-itextinputpanel?branch=master). For more information, see [Programming the Text Input Panel](programming-the-text-input-panel.md).\]

Starting with the Microsoft Windows XP Tablet PC Edition Software Development Kit (SDK) version 1.0, the system-level Tablet PC Input Panel provides a universal mechanism to accomplish text input across the Windows platform, although it does not provide programmatic access. The Tablet PC SDK version 1.5 [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object integrates text input tools into applications.
...
The [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object is convenient for application developers. There is no need to replace controls on existing forms. You can simply attach **PenInputPanel** objects to existing controls that receive text input, and they can begin receiving input from the **PenInputPanel** object.

The [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object adopts the settings from the Input Panel for the following properties:
...
The [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object does not provide access to the underlying ink. To get the ink, use the [InkPicture](inkpicture-control-reference.md) control.

The [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object provides an in-place user interface (UI) that is easily discoverable by end users of your applications. It is automatically activated when the user taps on a window with a **PenInputPanel** object using the tablet pen. The pen input panel appears automatically when the system detects a CursorButtonUp event for the window to which the **PenInputPanel** object is attached. Automatic activation can be disabled by setting the [**AutoShow**](/windows/win32/msinkaut/?branch=master) property to **FALSE**.

The pen input panel does not appear automatically on mouse events. Pen events are converted to mouse events when using Terminal Services. The [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object does not work over a Terminal Services connection.

## Pen Input Panel Input Modes

The [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object allows either keyboard functionality or handwriting input, with additional keypads to assist input. The UI to the pen input panel includes:
...
Ink is recognized after a specified amount of time has elapsed since the time the last ink stroke was collected. When the timeout occurs, ink is removed from the collection surface and recognition occurs. The recognized text is then inserted into the control to which the [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object is attached.
...
The [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object supports shipping recognizers for Windows XP Tablet PC Edition version 1.0 and the Tablet PC SDK version 1.5.
...
Automatic positioning functions only when the developer has not explicitly set the position using the [**MoveTo**](/windows/win32/peninputpanel/?branch=master) method. To override automatic positioning, change the values of the [**Top**](/windows/win32/peninputpanel/?branch=master) and [**Left**](/windows/win32/msinkaut/?branch=master) properties in a [**PanelMoving**](peninputpanel-panelmoving.md) event handler.
...
By default, the top of the pen input panel appears at the bottom of the control to which it is attached and is separated from the control by the value of the [**VerticalOffset**](/windows/win32/peninputpanel/?branch=master) property. If there is not enough room below the control, the bottom of the pen input panel appears at the top of the control to which it is attached and is separated from the control by the value of the **VerticalOffset** property. If there is still not enough room, as in the case of a full-screen edit control, the pen input panel overlaps the control.

The left edge pen input panel appears at the left edge of the control to which it is attached and is separated from the control by the value of the [**HorizontalOffset**](/windows/win32/peninputpanel/?branch=master) property, except as bounded by the screen. If the desired position places the pen input panel beyond the available screen boundaries, the pen input panel assumes the nearest possible horizontal position.
...
In the case where a [**PenInputPanel**](/windows/win32/msinkaut/?branch=master) object is attached to a windowless control, the pen input panel is positioned relative to the parent of the windowless control. Set the [**Top**](/windows/win32/peninputpanel/?branch=master) and [**Left**](/windows/win32/msinkaut/?branch=master) properties in a [**PanelMoving**](peninputpanel-panelmoving.md) event handler or use the [**MoveTo**](/windows/win32/peninputpanel/?branch=master) method to manually position the pen input panel.
