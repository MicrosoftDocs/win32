...
The text ink object supports the standard set of OLE interfaces required for embedding and clipboard support. The [**IPersistStream**](com.ipersiststream) interface reads from and writes to a stream in ink serialized format (ISF). The text ink object provides the [**IInkLineInfo**](/windows/win32/msinkaut/nn-msinkaut-iinklineinfo?branch=master) interface to access its display properties, and recognition result list.
...
-   When an application copies an [InkStrokes](/windows/win32/msinkaut/?branch=master) or an [InkEdit](inkedit-control-reference.md) object onto the Clipboard and the [**InkClipboardFormats Enumeration**](/windows/win32/msinkaut/ne-msinkaut-inkclipboardformats?branch=master) format is set, the OLE object Clipboard slot contains a text ink OLE object.
...
-   Use of ambient properties. Ambient properties such as font size, color, and bold formatting need to be transmitted to the text ink object. Application of these properties changes the width of the handwritten ink, so a size update is required by calling the [**IInkLineInfo::GetInkExtent**](/windows/win32/msinkaut/nf-msinkaut-iinklineinfo-getinkextent?branch=master) or [**IOleObject::GetExtent**](com.ioleobject_getextent) method.
