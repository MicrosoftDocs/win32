...
-   When an application copies an [InkStrokes](inkstrokes-collection.md) or an [InkEdit](inkedit-control-reference.md) object onto the Clipboard and the [**InkClipboardFormats Enumeration**](inkclipboardformats.md) format is set, the OLE object Clipboard slot contains a text ink OLE object.
...
-   Use of ambient properties. Ambient properties such as font size, color, and bold formatting need to be transmitted to the text ink object. Application of these properties changes the width of the handwritten ink, so a size update is required by calling the [**IInkLineInfo::GetInkExtent**](iinklineinfo-getinkextent.md) or [**IOleObject::GetExtent**](com.ioleobject_getextent) method.
