...
The Auto Claims sample addresses a hypothetical scenario for an insurance assessor. The assessor's work requires him or her to visit with clients at their home or business and to enter their claim information into a form. To increase the assessor's productivity, his IT department develops a tablet application that enables him or her to quickly and accurately enter claim information through two ink controls: [InkEdit](frlrfMicrosoftInkInkEditClassTopic) and [InkPicture](T:Microsoft.Ink.InkPicture) controls.

In this sample, a [InkEdit](frlrfMicrosoftInkInkEditClassTopic) control is used for each text input field. A user enters the relevant information about an insurance policy and vehicle into these fields with a pen. The [InkPicture](T:Microsoft.Ink.InkPicture) control is used to add ink over an automobile image to highlight damaged areas of the automobile. The Auto Claims sample is available for C\# and Microsoft Visual Basic .NET. This topic describes the Visual Basic .NET.

The AutoClaims Class is defined as a subclass of [System.Windows.Forms.Form](T:System.Windows.Forms.Form) , and a nested class is defined for creating and managing layers of ink for different types of damage. Four event handlers are defined to perform the following tasks:

-   Initializing the form and ink layers.
-   Redrawing the [InkPicture](T:Microsoft.Ink.InkPicture) control.
...
You need to import the [Microsoft.Ink](frlrfMicrosoftInk) namespace:
...
Next, in the AutoClaims Class, a nested `InkLayer` class is defined and an array of four `InkLayer` objects is declared. (InkLayer contains an [Microsoft.Ink.Ink](frlrfMicrosoftInkInkClassTopic) object for storing ink, and [System.Drawing.Color](T:System.Drawing.Color) and **Boolean** values for storing the color and hidden state of the layer.) A fifth Ink object is declared to handle ink for the [InkPicture](T:Microsoft.Ink.InkPicture) when all of the ink layers are hidden.
...
Each layer has its own [Ink](frlrfMicrosoftInkInkClassTopic) object. There are four discrete areas of interest in the claim form (body, windows, tires, and headlights), so four InkLayer objects are used. A user can view any combination of layers at once.

## Initializing the Form and Ink Layers

The `Load` event handler initializes the [Ink](frlrfMicrosoftInkInkClassTopic) object and the four `InkLayer` objects.
...
Finally, set the ink color for the [InkPicture](T:Microsoft.Ink.InkPicture) control to the currently selected list box entry.
...
In the [InkPicture](T:Microsoft.Ink.InkPicture) Control's inherited [Paint](frlrfSystemWindowsFormsControlClassPaintTopic) event handler, the ink layers are checked to determine which are hidden. If a layer is not hidden, the event procedure displays it by using the [Renderer](frlrfMicrosoftInkInkPictureClassRendererTopic) property's [Draw](frlrfMicrosoftInkRendererClassDrawTopic) method. If you look in the Object Browser, you'll see that the Microsoft.Ink.InkPicture.Renderer property is defined as a [Microsoft.Ink.Renderer](frlrfMicrosoftInkRendererClassTopic) object:
...
When the user selects an item in the list box, the [SelectedIndexChanged](frlrfSystemWindowsFormsListBoxClassSelectedIndexChangedTopic) event handler first checks that the selection has changed and that the [InkPicture](T:Microsoft.Ink.InkPicture) control is not currently collecting ink. It then sets the ink color of the InkPicture control to the appropriate color for the selected ink layer. Also, it updates the Hide Layer check box to reflect the selected ink layer's hidden status. Finally, the InkPicture control's inherited [Refresh](frlrfSystemWindowsFormsControlClassRefreshTopic) method is used to display only the desired layers within the control.
...
The `CheckedChanged` event handler first checks that the selection has changed and that the [InkPicture](T:Microsoft.Ink.InkPicture) control is not currently collecting ink. It then updates the selected ink layer's hidden status, sets the InkPicture control's InkEnabled to **FALSE**, .

Next, the InkPicture control's InkEnabled property is set to **FALSE** before updating its Ink property.

Finally, the [InkPicture](T:Microsoft.Ink.InkPicture) control is either enabled or disabled for the particular vehicle part based on whether the Hide Layer check box is selected, and the InkPicture control's [Refresh](frlrfSystemWindowsFormsControlClassRefreshTopic) method is used to display only the desired layers within the control.
...
In the Windows Form Designer generated code, the [InkEdit](frlrfMicrosoftInkInkEditClassTopic) and [InkPicture](T:Microsoft.Ink.InkPicture) controls are added to the form's component list when the form is initialized. When the form closes, the InkEdit and InkPicture controls are disposed, as well as the other components of the form, by the form's [Dispose](frlrfmicrosoftinkinkpictureclassdisposetopic) method. The form's Dispose method also disposes the [Ink](frlrfMicrosoftInkInkClassTopic) objects that are created for the form.

## Related topics

<dl> <dt>

[Microsoft.Ink.Ink](frlrfMicrosoftInkInkClassTopic)
</dt> <dt>
