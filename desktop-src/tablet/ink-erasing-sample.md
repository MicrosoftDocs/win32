...
In ink-enabled mode, the [InkCollector](https://www.bing.com/search?q=InkCollector) object collects ink as shown in [Ink Collection Sample](ink-collection-sample.md).
...
The [Paint](https://www.bing.com/search?q=Paint) event occurs whenever a control is redrawn.

> [!Note]  
> The sample forces the form to redraw itself whenever a stroke is erased, or when the application mode changes, using the form's [Refresh](https://www.bing.com/search?q=Refresh) method.
...
In `PaintCusps`, the code iterates through each cusp in each stroke, and draws a red circle around it. The stroke's [PolylineCusps](https://www.bing.com/search?q=PolylineCusps) property returns the indices of the points within a stoke that correspond to cusps. Also, note the [Renderer](https://www.bing.com/search?q=Renderer) object's [InkSpaceToPixel](https://www.bing.com/search?q=InkSpaceToPixel) method, which converts the point to coordinates relevant to the DrawEllipse method.
...
In `PaintIntersections`, the code iterates through each stroke to find its intersections with the entire set of strokes. Note that the stroke's [FindIntersections](https://www.bing.com/search?q=FindIntersections) method is passed a [Strokes](https://www.bing.com/search?q=Strokes) collection and returns an array of floating point index values representing the intersections. The code then calculates an X-Y coordinate for each intersection, and draws a red circle around it.
...
Three event handlers are defined for the [InkCollector](https://www.bing.com/search?q=InkCollector) object for the [CursorDown](https://www.bing.com/search?q=CursorDown), [NewPackets](https://www.bing.com/search?q=NewPackets), and [Stroke](https://www.bing.com/search?q=Stroke) events. Each event handler checks the [Cursor](https://www.bing.com/search?q=Cursor) object's [Inverted](https://www.bing.com/search?q=Inverted) property to see which end of the pen is being used. When the pen is inverted:

-   The `myInkCollector_CursorDown` method makes the stroke transparent.
-   The `myInkCollector_NewPackets` method erases strokes.
-   The `myInkCollector_Stroke` method cancels the event. [NewPackets](https://www.bing.com/search?q=NewPackets) events are generated prior to the [Stroke](https://www.bing.com/search?q=Stroke) event.

## Tracking the Cursor

Whether the user is using a pen or a mouse, [MouseMove](https://www.bing.com/search?q=MouseMove) events are generated. The MouseMove event handler first checks to determine whether the current mode is an erase mode and if any mouse button is pressed, and ignores the event if these states are not present. Then, the event handler converts the pixel coordinates for the cursor into ink space coordinates by using the [Renderer](https://www.bing.com/search?q=Renderer) object's [PixelToInkSpace](https://www.bing.com/search?q=PixelToInkSpace) method, and calls one of the code's erase methods depending on the current erase mode.

## Erasing Strokes

The `EraseStrokes` method takes the cursor's location in ink space and generates a collection of strokes that are within `HitTestRadius` units. The `currentStroke` parameter specifies a [Stroke](https://www.bing.com/search?q=Stroke) object that should not be deleted. Then the strokes collection is deleted from the collector, and the form is redrawn.
...
The [Stroke](https://www.bing.com/search?q=Stroke) object's [Split](https://www.bing.com/search?q=Split) method is used to separate the segment from the rest of the stroke, and then the segment is deleted, leaving the rest of the stroke intact. As in `EraseStrokes`, the form is redrawn before the method returns.
