...
Windows Filtering Platform (WFP) uses the [standard Win32 access rights](https://msdn.microsoft.com/library/windows/desktop/aa379607) plus a set of WFP-specific access rights built into the filtering platform. These access rights are used to secure objects in user mode only. Kernel-mode callers bypass all access checks.
