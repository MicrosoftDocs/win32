...
If a filter at this layer is matched to authorize promiscuous mode receiving, the [**FWP\_CONDITION\_ALE\_PROMISCUOUS\_MODE**](filtering-condition-identifiers-.md) field will be set to SIO\_RCVALL. For a description of SIO\_RCVALL, see [**WSAIoctl**](https://msdn.microsoft.com/library/windows/desktop/ms741621).
...
If the local address is not specified in the [**bind()**](https://msdn.microsoft.com/library/windows/desktop/ms737550) call, the local address field is set to [**FWP\_EMPTY**](/windows/desktop/api/Fwptypes/ne-fwptypes-fwp_data_type_).

## AUTH\_LISTEN

A filter at the [**FWPM\_LAYER\_ALE\_AUTH\_LISTEN\_V{4\|6}**](management-filtering-layer-identifiers-.md) layer is matched for TCP [**listen()**](https://msdn.microsoft.com/library/windows/desktop/ms739168) calls.

## AUTH\_RECV\_ACCEPT

A filter at the [**FWPM\_LAYER\_ALE\_AUTH\_RECV\_ACCEPT\_V{4\|6}**](management-filtering-layer-identifiers-.md) layer is matched for TCP [**accept()**](https://msdn.microsoft.com/library/windows/desktop/ms737526) calls, for first UDP packets (unicast) from a unique remote address/port tuple, and for first inbound non-error ICMP messages (unicast) with a unique ICMP type, code, and ID.
...
TCP packets received by raw sockets are handled similarly to UDP traffic. That is, only the first TCP [**send()**](https://msdn.microsoft.com/library/windows/desktop/ms740149) and the first TCP [**recv()**](https://msdn.microsoft.com/library/windows/desktop/ms740121) over raw sockets will be filtered.

## AUTH\_CONNECT

A filter at the [**FWPM\_LAYER\_ALE\_AUTH\_CONNECT\_V{4\|6}**](management-filtering-layer-identifiers-.md) layer is matched for TCP [**connect()**](https://msdn.microsoft.com/library/windows/desktop/ms737625) calls, for first UDP packets sent to a unique remote address and port tuple, and for first outbound non-error ICMP messages with a unique ICMP type, code, and ID.
...
TCP packets sent by raw sockets are handled similarly to UDP traffic. That is, only the first TCP [**send()**](https://msdn.microsoft.com/library/windows/desktop/ms740149) and the first TCP [**recv()**](https://msdn.microsoft.com/library/windows/desktop/ms740121) over raw sockets will be filtered.
...
This layer is used by callout drivers to track connection state, described in detail in the [Windows Driver Kit](http://go.microsoft.com/fwlink/p/?linkid=95958) documentation.
