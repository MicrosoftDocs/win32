...
*BSTR\_byvalue\_escaping*<dl> Directs the MIDL compiler to honor escape sequences such as '\\n' or '\\t' in BSTRs.  
</dl>
...
-   *zeroout\_alignmentgap*: When IDLs are compiled with --target NT60 or higher, MIDL will create stubs that zero out any alignment gaps between members or a structure in the wire buffer. The command line switch */backward\_compat zeroout\_alignmentgap* directs MIDL to disable this feature.
...
-   *BSTR\_byvalue\_escaping*: By default, the MIDL compiler does not process escape sequences such as '\\n' or '\\t' in string constants for OLE Automation when converting a string constant to types VT\_LPSTR or VT\_LPWSTR. With this backward compatibility switch option, the escape sequences are evaluated.
