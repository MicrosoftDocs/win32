...
1.  Implement the [**IWMWriterPostViewCallback::OnPostViewSample**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriterpostviewcallback-onpostviewsample?branch=master) callback. This method is essentially the same as [**IWMReaderCallback::OnSample**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmreadercallback-onsample?branch=master) except that it specifies stream numbers instead of outputs.
2.  Set up for writing as usual.
3.  Obtain a pointer to the [**IWMWriterPostView**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmwriterpostview?branch=master) interface of the writer object by calling **IWMWriter::QueryInterface**.
4.  Set the callback for the postviewer to use by calling [**IWMWriterPostView::SetPostViewCallback**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriterpostview-setpostviewcallback?branch=master).
5.  For each stream for which you want to receive postview samples, call [**IWMWriterPostView::SetReceivePostViewSamples**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriterpostview-setreceivepostviewsamples?branch=master). You can check to see whether a stream is set to receive postview samples by calling [**IWMWriterPostView::GetReceivePostViewSamples**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriterpostview-getreceivepostviewsamples?branch=master).
