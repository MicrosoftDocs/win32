...
Many other messages are sent to **OnStatus** by the reader object to enable the application to respond to the status of reading operations. The possible status message values are defined in the [**WMT\_STATUS**](/windows/win32/Wmsdkidl/ne-wmsdkidl-wmt_status?branch=master) enumeration type.
