...
keywords: ["Windows Media Format SDK,writing video streams", "Windows Media Format SDK,video streams", "Windows Media Format SDK,non-square pixels", "Windows Media Format SDK,pixels (non-square)", "Advanced Systems Format (ASF),writing video streams", "ASF (Advanced Systems Format),writing video streams", "Advanced Systems Format (ASF),video streams", "ASF (Advanced Systems Format),video streams", "Advanced Systems Format (ASF),non-square pixels", "ASF (Advanced Systems Format),non-square pixels", "Advanced Systems Format (ASF),pixels (non-square)", "ASF (Advanced Systems Format),pixels (non-square)", "writing video streams", "video streams,writing", "video streams,non-square pixels", "non-square pixels", "pixels (non-square)"]
---
...
3.  Call [**IWMWriter::SetProfile**](iwmwriter-setprofile.md). (Always call this method before setting any header attributes.)
4.  Call **QueryInterface** to obtain the [**IWMHeaderInfo3**](iwmheaderinfo3.md) interface and call [**AddAttribute**](iwmheaderinfo3-addattribute.md) twice to add [**AspectRatioX**](aspectratiox.md) and [**AspectRatioY**](aspectratioy.md) as stream-level attributes of the video stream. These attributes are **DWORD** values.
5.  Write the file.
...
1.  Set up the writer object. For more information, see [Writing ASF Files](writing-asf-files.md).
2.  Create or load a profile with one or more video streams. For more information, see [To Use Profiles with the Writer](to-use-profiles-with-the-writer.md).
3.  For each stream (of any media type) in the profile, call [**IWMStreamConfig::SetStreamName**](iwmstreamconfig-setstreamname.md) to specify a unique name of your choice. Do not give two streams the same name.
4.  Use [**IWMStreamConfig2::AddDataUnitExtension**](iwmstreamconfig2-adddataunitextension.md) on the video stream to add a data unit extension for pixel aspect ratio.
5.  Call [**IWMWriter::SetProfile**](iwmwriter-setprofile.md).
6.  Write the file.

**While writing:**

-   For each sample, call [**INSSBuffer3::SetProperty**](inssbuffer3-setproperty.md) and specify the WM\_SampleExtensionGUID\_PixelAspectRatio property along with the correct value. Aspect ratio values are written as two concatenated two-digit values. For example, 16:9 is written as 1609 or 0x0649. This is always a 2-byte value.
