...
keywords: ["Advanced Systems Format (ASF),specifying actions", "ASF (Advanced Systems Format),specifying actions", "digital rights management (DRM),specifying actions", "DRM (digital rights management),specifying actions"]
---

# Specifying the Actions To Be Performed

When you first call [**WMCreateReader**](wmcreatereader.md) to create the reader object, the second parameter is a bitwise **OR** of [**WMT\_RIGHTS**](wmt-rights.md) values. Use this parameter to specify which action(s) the application will take on the first file to be opened. These actions correspond directly to rights that can be specified in the license. On subsequent calls to [**IWMReader::Open**](iwmreader-open.md), you can modify the rights that you are requesting by calling [**IWMDRMReader::SetDRMProperty**](iwmdrmreader-setdrmproperty.md), specifying the defined constant for the [**DRM\_Rights**](drm-rights.md) property, and using string literals (of type **WCHAR**) separated by semicolons to identify the rights. The following code snippet requests four rights: play the file, copy it to a device, and play it as part of a collaborative playlist.
