...
When applying DRM version 1 protection, use the [**IWMHeaderInfo**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmheaderinfo?branch=master) interface, which is obtained as described in [Reading ASF Files in DirectShow](reading-asf-files-in-directshow.md). When applying DRM version 7 protection, obtain the [**IWMDRMWriter**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmdrmwriter?branch=master) interface by calling **QueryService** on the [WM ASF Writer](wm-asf-writer-filter.md) filter, as shown in the code snippet later in this topic.

All other DRM-specific configuration is exactly the same as described in [Enabling DRM Support](enabling-drm-support.md). Use **QueryService** to obtain the [**IWMDRMReader**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmdrmreader?branch=master) interface from the [WM ASF Reader](wm-asf-reader-filter.md) filter.
