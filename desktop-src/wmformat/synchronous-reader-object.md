...
keywords: ["Windows Media Format SDK,synchronous reader objects", "Advanced Systems Format (ASF),synchronous reader objects", "ASF (Advanced Systems Format),synchronous reader objects", "objects,synchronous reader objects", "synchronous readers,objects"]
---
...
The synchronous reader object is created by the function [**WMCreateSyncReader**](wmcreatesyncreader.md), which sets a pointer to an **IWMSyncReader** interface. The other interfaces supported by the synchronous reader interface can be obtained by calling the **QueryInterface** method.
...
|------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**IWMHeaderInfo**](iwmheaderinfo.md)   | Sets and retrieves header information, such as metadata, [*markers*](wmformat-glossary.md#wmformat-marker), and so on.                                            |
| [**IWMHeaderInfo2**](iwmheaderinfo2.md) | Enumerates the available codec information. Inherits all of the methods of **IWMHeaderInfo**.                                                                      |
...
| [**IWMSyncReader**](iwmsyncreader.md)   | Provides synchronous reading capabilities for digital media files.                                                                                                 |
| [**IWMSyncReader2**](iwmsyncreader2.md) | Provides support for seeking to SMPTE time codes and for allocating samples manually. Inherits all of the methods of **IWMSyncReader**.                            |
