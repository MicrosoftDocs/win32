...
keywords: ["Windows Media Format SDK,identifying output numbers", "Advanced Systems Format (ASF),identifying output numbers", "ASF (Advanced Systems Format),identifying output numbers", "Advanced Systems Format (ASF),output numbers and formats", "ASF (Advanced Systems Format),output numbers and formats", "output numbers and formats,identifying"]
---
...
2.  Retrieve the total number of outputs for the file by calling [**IWMReader::GetOutputCount**](iwmreader-getoutputcount.md) (or [**IWMSyncReader::GetOutputCount**](iwmsyncreader-getoutputcount.md)).
3.  Loop through the outputs one at a time, performing the following steps for each:
    -   Retrieve the [**IWMOutputMediaProps**](iwmoutputmediaprops.md) interface for the current output with a call to [**IWMReader::GetOutputProps**](iwmreader-getoutputprops.md) (or [**IWMSyncReader::GetOutputProps**](iwmsyncreader-getoutputprops.md)).
    -   Retrieve the [**WM\_MEDIA\_TYPE**](wm-media-type.md) structure for the output by making two calls to [**IWMMediaProps::GetMediaType**](iwmmediaprops-getmediatype.md). Make the first call to get the size of the structure, then allocate memory for it and pass a pointer to the allocated memory on the second call. Alternatively, you can call [**IWMMediaProps::GetType**](iwmmediaprops-gettype.md), which delivers the major type without requiring you to allocate memory for the **WM\_MEDIA\_TYPE** structure. You can skip outputs of the wrong major type.
