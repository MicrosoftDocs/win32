...
keywords: ["Windows Media Format SDK,assigning output formats", "Advanced Systems Format (ASF),assigning output formats", "ASF (Advanced Systems Format),assigning output formats", "Advanced Systems Format (ASF),output numbers and formats", "ASF (Advanced Systems Format),output numbers and formats", "output numbers and formats,assigning", "codecs,output numbers and formats"]
---
...
3.  Retrieve the total number of available formats for the desired output by calling [**IWMReader::GetOutputFormatCount**](iwmreader-getoutputformatcount.md) (or [**IWMSyncReader::GetOutputFormatCount**](iwmsyncreader-getoutputformatcount.md)).
4.  Loop through the available formats one at a time, performing the following steps for each:
    -   Retrieve the [**IWMOutputMediaProps**](iwmoutputmediaprops.md) interface for the current output format by calling [**IWMReader::GetOutputFormat**](iwmreader-getoutputformat.md) (or [**IWMSyncReader::GetOutputFormat**](iwmsyncreader-getoutputformat.md)).
    -   Retrieve the [**WM\_MEDIA\_TYPE**](wm-media-type.md) structure for the output format by making two calls to [**IWMMediaProps::GetMediaType**](iwmmediaprops-getmediatype.md). Make the first call to get the size of the structure, then allocate memory for it and pass a pointer to the allocated memory on the second call.
...
5.  When you have found the output desired, set it for use with the reader by calling [**IWMReader::SetOutputProps**](iwmreader-setoutputprops.md) (or [**IWMSyncReader::SetOutputProps**](iwmsyncreader-setoutputprops.md)). You must pass a pointer to the **IWMOutputMediaProps** interface obtained in the first step of the loop.
...
[**IWMOutputMediaProps Interface**](iwmoutputmediaprops.md)
</dt> <dt>
