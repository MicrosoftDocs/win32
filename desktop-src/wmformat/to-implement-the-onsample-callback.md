...
Applications that deliver compressed samples from ASF files need to implement the [**IWMReaderCallbackAdvanced::OnStreamSample**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmreadercallbackadvanced-onstreamsample?branch=master) callback method. **OnStreamSample** functions almost identically to **OnSample**, except that it receives compressed samples by stream number instead of uncompressed samples by output number.
