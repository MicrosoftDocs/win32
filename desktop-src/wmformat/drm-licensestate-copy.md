---
title: DRM\_LicenseState\_Copy
description: The DRM\_LicenseState\_Copy property contains a WM\_LICENSE\_STATE\_DATA structure that contains details about how this right has been applied to the content.
ms.assetid: '97a79e35-6533-4781-b368-ffe8d81956ec'
keywords: ["DRM_LicenseState_Copy windows Media Format"]
topic_type:
- apiref
api_name:
- DRM_LicenseState_Copy
api_type:
- NA
---

# DRM\_LicenseState\_Copy

The **DRM\_LicenseState\_Copy** property contains a [**WM\_LICENSE\_STATE\_DATA**](wm-license-state-data.md) structure that contains details about how this right has been applied to the content.

## Global Constant

g\_wszWMDRM\_LicenseState\_Copy

## Data Type

**WMT\_TYPE\_BINARY**

## Remarks

This is a read-only property that is retrieved using [**IWMDRMReader::GetDRMProperty**](iwmdrmreader-getdrmproperty.md).

## See also

<dl> <dt>

[**DRM Properties**](drm-properties.md)
</dt> </dl>

 

 




