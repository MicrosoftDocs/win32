...
Before passing any samples, you must set the writer to accept them by calling [**IWMWriter::BeginWriting**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriter-beginwriting?branch=master).

To pass a sample to the writer, perform the following steps:

1.  Allocate a buffer and retrieve a pointer to the [**INSSBuffer**](/windows/win32/wmsbuffer/nn-wmsbuffer-inssbuffer?branch=master) interface by calling [**IWMWriter::AllocateSample**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriter-allocatesample?branch=master).
2.  Retrieve the address of the buffer created in step 1 by calling [**INSSBuffer::GetBuffer**](/windows/win32/Wmsbuffer/nf-wmsbuffer-inssbuffer-getbuffer?branch=master).
...
5.  Pass the buffer interface to the writer along with the input number and sample time using the [**IWMWriter::WriteSample**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriter-writesample?branch=master) method. All audio samples for an input represent the same duration of content, so you can figure the sample time by adding the sample duration to a running total. For video, you need to calculate the time based on the frame rate.
