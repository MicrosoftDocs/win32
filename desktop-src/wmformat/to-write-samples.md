...
keywords: ["Advanced Systems Format (ASF),writing samples", "ASF (Advanced Systems Format),writing samples", "Advanced Systems Format (ASF),passing samples to the writer", "ASF (Advanced Systems Format),passing samples to the writer"]
---
...
Before passing any samples, you must set the writer to accept them by calling [**IWMWriter::BeginWriting**](iwmwriter-beginwriting.md).

To pass a sample to the writer, perform the following steps:

1.  Allocate a buffer and retrieve a pointer to the [**INSSBuffer**](inssbuffer.md) interface by calling [**IWMWriter::AllocateSample**](iwmwriter-allocatesample.md).
2.  Retrieve the address of the buffer created in step 1 by calling [**INSSBuffer::GetBuffer**](inssbuffer-getbuffer.md).
...
5.  Pass the buffer interface to the writer along with the input number and sample time using the [**IWMWriter::WriteSample**](iwmwriter-writesample.md) method. All audio samples for an input represent the same duration of content, so you can figure the sample time by adding the sample duration to a running total. For video, you need to calculate the time based on the frame rate.
