...
keywords: ["Windows Media Format SDK,writer push sink objects", "Advanced Systems Format (ASF),writer push sink objects", "ASF (Advanced Systems Format),writer push sink objects", "objects,writer push sink objects", "writer push sink objects"]
---
...
A writer push sink object is created by the function [**WMCreateWriterPushSink**](wmcreatewriterpushsink.md), which sets a pointer to an **IWMWriterPushSink** interface. The other interfaces supported by the object, listed in the following table, can be obtained by calling the **QueryInterface** method.
...
| [**IWMWriterPushSink**](iwmwriterpushsink.md)     | Manages a push distribution session.                                                                             |
| [**IWMWriterSink**](iwmwritersink.md)             | Allocates memory, determines whether the sink is operating in real time, and exposes several callback functions. |
