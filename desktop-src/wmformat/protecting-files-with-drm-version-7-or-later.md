...
2.  Call the [**WMCreateWriter**](/windows/win32/Wmsdkidl/nf-wmsdkidl-wmcreatewriter?branch=master) function to create the DRM writer. The first argument is reserved and must be set to **NULL**.
...
4.  Obtain the writer object's [**IWMDRMWriter**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmdrmwriter?branch=master) interface.
5.  Call [**IWMDRMWriter::SetDRMAttribute**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdrmwriter-setdrmattribute?branch=master) and set [**Use\_Advanced\_DRM**](use-advanced-drm.md) to **TRUE**.
6.  If you need to generate a new key seed, call [**IWMDRMWriter::GenerateKeySeed**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdrmwriter-generatekeyseed?branch=master). In most cases, you will be reusing a key seed that was generated previously. This value must remain secret; it is not written into the file.
7.  Call [**IWMDRMWriter::GenerateKeyID**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdrmwriter-generatekeyid?branch=master) to create a key ID, which is the second value used to create the actual key. Unlike the key seed, the key ID is public and is written to the file in the DRM header in the clear. Create a new key ID for each new file you create.
8.  Call **IWMDRMWriter::GenerateSigningKeyPair** if necessary to generate a public and private key that will be used to sign the advanced DRM ASF Header object. For more information about these keys, see [**IWMDRMWriter::GenerateSigningKeyPair**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdrmwriter-generatesigningkeypair?branch=master).
...
11. Write the file using the [**IWMWriter**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmwriter?branch=master) interface methods as described elsewhere in this documentation. To create a live-DRM stream, simply write to a network sink. You can also write to a push sink.
