...
Both the ASF Reader and the ASF Writer forward [**WMT\_STATUS**](/windows/win32/Wmsdkidl/ne-wmsdkidl-wmt_status?branch=master) events to applications. The writer forwards all such events, and the reader forwards only those that pertain to DRM license acquisition. To receive these event notifications in your application, add a case for the **EC\_WMT\_EVENT** in your event-handling function. The *lParam1* parameter associated with the event contains the **WMT\_STATUS** code (which can be **WMT\_ERROR**) and lParam2 contains an [**AM\_WMT\_EVENT\_DATA**](/windows/win32/Evcode/?branch=master) that includes the **HRESULT**.
