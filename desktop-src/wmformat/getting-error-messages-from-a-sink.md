...
keywords: ["Advanced Systems Format (ASF),error messages from sinks", "ASF (Advanced Systems Format),error messages from sinks", "Advanced Systems Format (ASF),sinks", "ASF (Advanced Systems Format),sinks", "sinks,error messages", "error messages"]
---

# Getting Error Messages from a Sink

The writer object does not send messages to the [**IWMStatusCallback::OnStatus**](iwmstatuscallback-onstatus.md) callback method. However, you can set writer sinks to send messages to OnStatus. Each sink must be set to deliver status separately, but all sinks can report to the same callback.

To set a sink to deliver status messages to **OnStatus**, call the [**IWMRegisterCallback::Advise**](iwmregistercallback-advise.md) method.
...
[**IWMRegisterCallback Interface**](iwmregistercallback.md)
</dt> <dt>
