...
| g\_wszFixedFrameRate                   | **WMT\_TYPE\_BOOL**                                                                                                                       | When set to True, instructs the codec not to drop any frames during encoding. This will cause the [*frame rate*](https://www.bing.com/search?q=*frame rate*) of the output video stream to be constant. The frame rate of the input stream does not need to be constant. |
| g\_wszInitialPatternForInverseTelecine | **WMT\_TYPE\_DWORD** set to one of the values in the initial pattern table in the topic [To Deinterlace Video](to-deinterlace-video.md). | When the deinterlace mode is set to WM\_DM\_DEINTERLACE\_INVERSETELECINE, this can be set to specify the pattern of the [*telecine*](https://www.bing.com/search?q=*telecine*) input. For more information, see [To Deinterlace Video](to-deinterlace-video.md).        |
