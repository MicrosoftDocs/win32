...
You should check the return value for the first call you make to [**IWMWriter::WriteSample**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriter-writesample?branch=master) for each stream. If the error code returned is NS\_E\_INVALID\_NUM\_PASSES, the stream requires a preprocessing pass.
