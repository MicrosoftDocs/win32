...
Writer sinks are separate objects from the writer and must be added to the writer to be used. If you are writing to a file, you can simply call [**IWMWriter::SetOutputFilename**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriter-setoutputfilename?branch=master), which will set up the file sink automatically. Otherwise, to add a sink to the writer, call the [**IWMWriterAdvanced::AddSink**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriteradvanced-addsink?branch=master) method. **AddSink** requires a pointer to the [**IWMWriterSink**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmwritersink?branch=master) interface of the sink.

When you are finished using a sink, you should close it by calling the appropriate method, depending on the type of sink, and then remove it from the writer by calling [**IWMWriterAdvanced::RemoveSink**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmwriteradvanced-removesink?branch=master).
