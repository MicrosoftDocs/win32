...
You can read SMPTE time stamps just as you would read any other data unit extension, but the reading objects provide integrated support for searching by SMPTE time code. To be able to search for SMPTE time stamps, you must first index the file by SMPTE time code. You can configure the indexer to index time codes by using the [**IWMIndexer2::Configure**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmindexer2-configure?branch=master) method.

Using the asynchronous reader, you can navigate a file by SMPTE time stamps using the methods of the [**IWMReaderTimecode**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmreadertimecode?branch=master) interface and the [**IWMReaderAdvanced3::StartAtPosition**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmreaderadvanced3-startatposition?branch=master) method. With the synchronous reader, use [**IWMSyncReader2::SetRangeByTimecode**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmsyncreader2-setrangebytimecode?branch=master).
