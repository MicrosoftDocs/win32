...
1.  Parse the message by calling the [**IWMDRMMessageParser::ParseRegistrationReqMsg**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdrmmessageparser-parseregistrationreqmsg?branch=master) method. This method retrieves the device certificate and the device serial number, both of which are needed to identify the device.
2.  Call the [**IWMDeviceRegistration::GetRegisteredDeviceByID**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdeviceregistration-getregistereddevicebyid?branch=master) method, passing in the certificate and device serial number retrieved in step 1. If the device is found, it is already registered and you can skip the next step.
3.  Call the [**IWMDeviceRegistration::RegisterDevice**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdeviceregistration-registerdevice?branch=master) method to add the device to the device registration database.

You can access information about any device in the registration database by retrieving the registered device object associated with it. There are two ways to get a registered device object. If you have the certificate and serial number of the device, you can call the [**IWMDeviceRegistration::GetRegisteredDeviceByID**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdeviceregistration-getregistereddevicebyid?branch=master) method. If you do not have the certificate and serial number of the device, you can enumerate all the devices in the database by calling [**IWMDeviceRegistration::GetFirstRegisteredDevice**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdeviceregistration-getfirstregistereddevice?branch=master) followed by repeated calls to [**IWMDeviceRegistration::GetNextRegisteredDevice**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmdeviceregistration-getnextregistereddevice?branch=master) until a call returns S\_FALSE.

Before your application can send data to a device, you must ensure that the device is approved, validated, and open.

Device approval should involve interaction with the user. When a device sends a registration message, your application can prompt the user to decide whether the device is one that should receive that user's data. Then update the device registration database by calling the [**IWMRegisteredDevice::Approve**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmregistereddevice-approve?branch=master) method, passing **TRUE** or **FALSE** as appropriate.
...
To open a device, call [**IWMRegisteredDevice::Open**](/windows/win32/Wmsdkidl/nf-wmsdkidl-iwmregistereddevice-open?branch=master).
...
[**IWMRegisteredDevice**](/windows/win32/wmsdkidl/nn-wmsdkidl-iwmregistereddevice?branch=master)
</dt> <dt>
