...
2.  Creates an [**IX509CertificateRequestPkcs10**](ix509certificaterequestpkcs10.md) object and initializes it for the end user by specifying the **ContextUser** value of the [**X509CertificateEnrollmentContext**](x509certificateenrollmentcontext-enum.md) enumeration.
3.  Creates an [**IX500DistinguishedName**](ix500distinguishedname.md) object, uses the object to encode the subject name to a byte array, and adds the array to the PKCS \#10 request object.
4.  Creates an [**IObjectId**](iobjectid.md) object, initializes it by using the EKU object identifier (OID) specified on the command line, creates an [**IObjectIds**](iobjectids.md) collection, adds the new **IObjectId** (EKU) object to the collection, uses the collection to initialize an [**IX509ExtensionEnhancedKeyUsage**](ix509extensionenhancedkeyusage.md) object and adds this object to the request.
5.  Creates an [**IAlternativeName**](ialternativename.md) object, initializes it by using the RFC822 name specified on the command line, Creates an [**IAlternativeNames**](ialternativenames.md) collection, adds the new **IAlternativeName** (RFC822 name ) object to the collection, creates an [**IX509ExtensionAlternativeNames**](ix509extensionalternativenames.md) object and adds this object to the request.
6.  Creates an [**IX509Enrollment**](ix509enrollment.md) object, initializes it by using the [**IX509CertificateRequestPkcs10**](ix509certificaterequestpkcs10.md) object, and retrieves a string that contains a base64-encoded request.
