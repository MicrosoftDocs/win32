...
4.  Creates an [**IX509CertificateRequestPkcs7**](ix509certificaterequestpkcs7.md) object and initializes it by using the existing certificate. Because the *inheritOptions* parameter is set to InheritDefault, a new key pair is created for the request but the cryptographic provider in the existing certificate is used. For more information, see the [**InitializeFromCertificate**](ix509certificaterequestpkcs7-initializefromcertificate-method.md) method.
5.  Creates an [**IX509Enrollment**](ix509enrollment.md) object, initializes it by using the PKCS \#7 request object, attempts to enroll it with the CA and monitors the status of the enrollment process. The checkEnrollStatus function is defined in enrollCommon.cpp.
