...
4.  Creates an [**IX509CertificateRequestPkcs7**](/windows/win32/CertEnroll/nn-certenroll-ix509certificaterequestpkcs7?branch=master) object and initializes it by using the existing certificate. The new PKCS \#7 request object inherits the private and public keys and template from the existing certificate.
5.  Creates an [**ISignerCertificate**](/windows/win32/CertEnroll/nn-certenroll-isignercertificate?branch=master) object, initializes it by using the existing certificate, and adds it to the PKCS \#7 request object.
6.  Creates an [**IX509Enrollment**](/windows/win32/CertEnroll/nn-certenroll-ix509enrollment?branch=master) object, initializes it by using the PKCS \#7 request object, attempts to enroll it with the CA and monitors the status of the enrollment process. The checkEnrollStatus function is defined in enrollCommon.cpp.
