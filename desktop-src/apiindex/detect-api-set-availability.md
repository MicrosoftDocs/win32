...
Pass the name of the API set you are testing. API set names are conventionally written without a `.dll` suffix, and the examples on this page use that form. The suffix isn't part of the API set name.

To find the name to pass, see the **Requirements** table on the reference page for the API you want to call. If that table has an **API set** row, it gives the contract name. Otherwise use the **DLL** row, but only when the name there is itself a contract name beginning with `api-` or `ext-`; a physical module name such as *Wtsapi32.dll* isn't a contract, and querying it returns **FALSE**.
