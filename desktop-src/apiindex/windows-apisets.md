...
So why do API sets have `.dll` in their names? The reason is the way the *DLL loader* is implemented. The loader is the part of the OS that loads DLLs and/or resolves references to DLLs, and it identifies what to load by a module name spelled the way file names are spelled in an import table. API set names follow the same convention so that they fit in the same place.

The loader recognizes a name that begins with `api-` or `ext-` and routes it to the *API set runtime*, an extension of the loader that resolves contracts through the schema. From that point the name is parsed by API set naming rules rather than as a file name, so the `.dll` suffix isn't part of the contract name being resolved.

You can pass an API set name to [LoadLibrary](/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibraryw), or use it as a delay-load target. The operation succeeds when the schema on the current device maps that contract to a usable host; there isn't necessarily an actual file with that name anywhere on the PC. If the contract isn't mapped on the current device, a direct **LoadLibrary** fails. A delay-loaded reference behaves differently: the process still loads, and the absence surfaces later, when the API is called.
...
Two different kinds of operation take a contract name.

*Loader operations*&mdash;such as [LoadLibrary](/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibraryw) or [P/Invoke](/dotnet/standard/native-interop/pinvoke)&mdash;take the contract name in the same place where a DLL module name ordinarily appears. An appended `.dll` is conventional in that context, but it isn't required by API set name resolution and isn't part of the contract name. Use a contract name instead of a physical DLL module name to ensure a correct route to the implementation no matter where the API is actually implemented on the current device. There doesn't need to be a file with that contract name on disk.

*Availability-query examples* conventionally omit the **.dll** suffix and use the form that matches how the API is addressed:
