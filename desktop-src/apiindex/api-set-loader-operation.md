...
Because the mappings are kept in the schema rather than on disk, an API set name that ends with **.dll** doesn't refer to a file of that name. The **.dll** part is only a naming convention, carried over from the way module names are spelled in an import table. The API set name is more like an alias, or a virtual name, for a physical DLL file.

When a loader operation receives a name that begins with `api-` or `ext-`, the loader routes it to the *API set runtime*, an extension of the loader that resolves contracts through the schema. The API set runtime parses the name by API set naming rules rather than as a file name, so the **.dll** suffix isn't part of the contract name that gets resolved. Include the suffix when you're working from a name as it appears in an import table; otherwise you can leave it off.

The loader resolves both forms of contract name, a versioned contract name and a contract alias, through the same schema. For the conventions that govern these names, see [API set contract names](windows-apisets.md#api-set-contract-names).
