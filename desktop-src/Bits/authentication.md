...
BITS supports Basic authentication, Passport authentication, and several challenge/response authentication schemes. If the server or proxy requires user authentication, use the [**IBackgroundCopyJob2::SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method to specify the user's credentials. BITS uses the [CryptoAPI](https://msdn.microsoft.com/library/windows/desktop/aa380255) to protect the credentials.

**BITS 1.2 and earlier:** The [**SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method is not available.
...
Instead of embedding the user name and password in the URL, you should use the [**SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method to specify the user name and password for Basic authentication. This prevents others from viewing the credentials. If you specify credentials in both places, BITS uses the user name and password from the URL.
...
For challenge/response authentication, BITS impersonates the user and uses [Snego](_com_snego) to determine which challenge/response authentication to use, such as NTLM or the Kerberos protocol. For a list of challenge/response schemes that BITS supports, see [**BG\_AUTH\_SCHEME**](/windows/win32/Bits1_5/ne-bits1_5-__midl_ibackgroundcopyjob2_0002?branch=master).
...
To use the user's implicit (logon) credentials for NTLM or Kerberos authentication, call the [**IBackgroundCopyJob2::SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method and set the **UserName** and **Password** members of the [**BG\_BASIC\_CREDENTIALS**](/windows/win32/Bits1_5/ns-bits1_5-__midl_ibackgroundcopyjob2_0003?branch=master) structure to **NULL**. If you specify implicit credentials for a proxy, BITS will also use the implicit credentials for server authentication unless you specify explicit server credentials.

For additional information for services, see [Service Accounts and BITS](service-accounts-and-bits.md).

You could also change the **LMCompatibilityLevel** or **UseLMCompat** registry value; however, you should change these values only if you have an existing application that cannot be changed to call the [**SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method.

BITS will use implicit credentials for authentication if the **LMCompatibilityLevel** registry value is two or greater, and you have not called the [**SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method. The full path to the **LMCompatibilityLevel** registry value is **HKEY\_LOCAL\_MACHINE**\\**System**\\**CurrentControlSet**\\**Control**\\**LSA**\\**LmCompatibilityLevel**.
...
| 2     | BITS will send implicit credentials only if the application called the [**SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method. **Prior to BITS 2.0:** Not supported<br/> |
...
-   If [**IBackgroundCopyJob::SetProxySettings**](/windows/win32/Bits/nf-bits-ibackgroundcopyjob-setproxysettings?branch=master) was called with **BG\_JOB\_PROXY\_USAGE\_PRECONFIG**, then detect local IE proxy settings using job owner token context impersonation via [**WinHttpGetIEProxyConfigForCurrentUser**](https://msdn.microsoft.com/library/windows/desktop/aa384096).
-   If [**IBackgroundCopyJob::SetProxySettings**](/windows/win32/Bits/nf-bits-ibackgroundcopyjob-setproxysettings?branch=master) was called with **BG\_PROXY\_USAGE\_AUTODETECT** or if the IE settings from the **BG\_JOB\_PROXY\_USAGE\_PRECONFIG** case specify auto-detect or an auto-config URL, then conduct auto-proxy detection, or Web Proxy Autodiscovery Protocol (WPAD), using helper token impersonation via [**WinHttpGetProxyForUrl**](https://msdn.microsoft.com/library/windows/desktop/aa384097).
...
4.  Call the BITS job's [**SetCredentials**](/windows/win32/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials?branch=master) method with **BG\_AUTH\_SCHEME\_NEGOTIATE**, *UserName* set to **NULL**, *Password* set to **NULL**, and *Target* set to **BG\_AUTH\_TARGET\_PROXY**. This causes the user account's implicit credentials to be used for NTLM and Kerberos authentication with the proxy and server.
5.  If step 2 yielded any user-specific proxy settings (i.e. *lpszProxy* or *lpszProxyBypass* are not **NULL**), set the corresponding job settings manually, using [**SetProxySettings**](/windows/win32/Bits/nf-bits-ibackgroundcopyjob-setproxysettings?branch=master) with the **BG\_JOB\_PROXY\_USAGE\_OVERRIDE** setting.
6.  If step 2 did not yield any user-specific proxy settings, call [**SetProxySettings**](/windows/win32/Bits/nf-bits-ibackgroundcopyjob-setproxysettings?branch=master) with **BG\_JOB\_USAGE\_PRECONFIG**.
7.  QueryInterface for [**IBitsTokenOptions**](/windows/win32/Bits4_0/nn-bits4_0-ibitstokenoptions?branch=master).
8.  Impersonate the user account again.
9.  Call [**SetHelperToken**](/windows/win32/Bits4_0/nf-bits4_0-ibitstokenoptions-sethelpertoken?branch=master).
10. Call [**SetHelperTokenFlags**](/windows/win32/Bits4_0/nf-bits4_0-ibitstokenoptions-sethelpertokenflags?branch=master) with **BG\_TOKEN\_NETWORK**.
11. Revert impersonation.
12. Continue job setup.
13. Call [**Resume**](/windows/win32/Bits/nf-bits-ibackgroundcopyjob-resume?branch=master) on the job.
