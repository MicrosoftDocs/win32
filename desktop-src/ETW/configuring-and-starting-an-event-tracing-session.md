...
To enable the providers that you want to log events to your session, call the [**EnableTrace**](enabletrace.md) function to enable classic providers and the [**EnableTraceEx**](enabletraceex-func.md) function to enable [manifest-based](https://www.bing.com/search?q=manifest-based) providers. To enable providers that you want log events to your session filtering on specific conditions on Windows 8.1,Windows Server 2012 R2, and later, call the [**EnableTraceEx2**](enabletraceex2.md) function.
...
Up to eight trace sessions can enable and receive events from the same [manifest-based](https://www.bing.com/search?q=manifest-based) provider. However, only one trace session can enable a [classic](https://www.bing.com/search?q=classic) provider. If more than one trace session tries to enable a classic provider, the first session would stop receiving events when the second session enables the provider. For example, if Session A enabled Provider 1 and then Session B enabled Provider 1, only Session B would receive events from Provider 1.
