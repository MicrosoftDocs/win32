...
Context data that a consumer can specify when calling [**OpenTrace**](opentrace.md). If the consumer uses [**EventRecordCallback**](eventrecordcallback.md) to consume events, ETW sets the **UserContext** member of the [**EVENT\_RECORD**](/windows/desktop/api/relogger/ns-evntcons-_event_record) structure to this value.
