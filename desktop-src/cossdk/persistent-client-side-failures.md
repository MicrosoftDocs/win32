...
5.  The DLQL queries for [**IPlaybackControl**](/windows/win32/ComSvcs/nn-comsvcs-iplaybackcontrol?branch=master) for the exception class.
6.  The DLQL calls the [**IPlaybackControl::FinalClientRetry**](/windows/win32/ComSvcs/nf-comsvcs-iplaybackcontrol-finalclientretry?branch=master) method in the exception class.
7.  The DLQL plays back all property and method calls from the message to the exception class.
8.  The DLQL deletes the message if the exception handler completes the transaction successfully. The exception handler may issue [**IObjectContext::SetAbort**](/windows/win32/ComSvcs/nf-comsvcs-iobjectcontext-setabort?branch=master), and the message will remain on the dead letter queue.
