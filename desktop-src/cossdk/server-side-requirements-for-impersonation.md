...
The server performs impersonation programmatically. It explicitly assumes the client's security credentials by using [**CoImpersonateClient**](https://msdn.microsoft.com/library/windows/desktop/ms690349). When the client has granted the server sufficient authority, this has the effect of substituting the client's security credentials with the server thread token, in place of the process token.
...
When it has finished using the client's identity, the server can revert to its own process token using [**CoRevertToSelf**](https://msdn.microsoft.com/library/windows/desktop/ms687267).

For details on implementing impersonation and delegation, see [Delegation and Impersonation](https://msdn.microsoft.com/library/windows/desktop/ms680054).
