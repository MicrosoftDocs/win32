...
You should always instantiate the [**SharedPropertyGroupManager**](/windows/desktop/api/ComSvcs/), [**SharedPropertyGroup**](/windows/desktop/api/ComSvcs/), and [**SharedProperty**](/windows/desktop/api/ComSvcs/) objects from COM+ components rather than from a base client. If a base client creates shared property groups and properties, the shared properties are inside the base-client process, not in a server process. This means the COM+ objects cannot share the properties unless the objects are also running in the client process (which is generally not a good idea).
