...
To access the COM+ CRM, the CRM worker first obtains the [**ICrmLogControl**](/windows/win32/ComSvcs/nn-comsvcs-icrmlogcontrol?branch=master) interface, which allows the CRM worker to write records to the durable log. The CRM worker obtains this interface by creating a CRM clerk component.

Next the CRM worker must tell the CRM clerk the name of the CRM Compensator it wants to use. It does this by calling the [**ICrmLogControl::RegisterCompensator**](/windows/win32/ComSvcs/nf-comsvcs-icrmlogcontrol-registercompensator?branch=master) method. After this method is called, the CRM Compensator is created by the CRM infrastructure when the transaction completes.

After the CRM worker has registered its CRM Compensator, it can write records to the CRM log using [**ICrmLogControl**](/windows/win32/ComSvcs/nn-comsvcs-icrmlogcontrol?branch=master). The CRM worker must *write ahead*; that is, it must write a record to the log describing an action before it actually performs the action, in case a crash occurs immediately after completing the action. Without these write-ahead log records, there is no way to correct for the action.
...
The [**ICrmLogControl**](/windows/win32/ComSvcs/nn-comsvcs-icrmlogcontrol?branch=master) interface provides the following two methods for writing log records:

-   [**WriteLogRecordVariants**](/windows/win32/ComSvcs/nf-comsvcs-icrmlogcontrol-writelogrecordvariants?branch=master) is used for writing a structured log record that is built up as a collection of Variants. It is primarily for use when developing CRMs in Microsoft Visual Basic.
-   [**WriteLogRecord**](/windows/win32/ComSvcs/nf-comsvcs-icrmlogcontrol-writelogrecord?branch=master) is used for writing an unstructured log record as BLOBs of bytes. It is primarily for use when developing CRMs in Microsoft Visual C++. Because record structures in C are often made up of a set of headers and fields that may be scattered in memory, the **WriteLogRecord** method implements a gather capability that reduces the copying of data.
...
Both of these write methods write a log record to disk but do not guarantee the record's durability. While allowing lazy writes to accumulate before forcing to disk can improve performance, you can use the [**ICrmLogControl::ForceLog**](/windows/win32/ComSvcs/nf-comsvcs-icrmlogcontrol-forcelog?branch=master) method instead to guarantee that all writes done by the CRM are durable on disk, which is important for failure recovery.

When the CRM worker is done with its actions and has finished writing and forcing records to the log, it must release [**ICrmLogControl**](/windows/win32/ComSvcs/nn-comsvcs-icrmlogcontrol?branch=master). When the transaction completes (typically due to the application component calling [**SetComplete**](/windows/win32/ComSvcs/nf-comsvcs-iobjectcontext-setcomplete?branch=master) or [**SetAbort**](/windows/win32/ComSvcs/nf-comsvcs-iobjectcontext-setabort?branch=master)), the CRM infrastructure creates the CRM Compensator component, which implements either the [**ICrmCompensator**](/windows/win32/ComSvcs/nn-comsvcs-icrmcompensator?branch=master) interface or the [**ICrmCompensatorVariants**](/windows/win32/ComSvcs/nn-comsvcs-icrmcompensatorvariants?branch=master) interface. These interfaces are used to pass the unstructured (Visual C++) or structured (Visual Basic) records to the CRM Compensator along with the transaction outcome notifications.
...
The CRM Compensator can log new records if required by using [**ICrmLogControl**](/windows/win32/ComSvcs/nn-comsvcs-icrmlogcontrol?branch=master), which it receives as it is created. Both the CRM worker and CRM Compensator can also forget the last log record they wrote, which might be required to avoid unnecessary recovery.
