...
The [**ICrmLogControl**](icrmlogcontrol.md) interface provides the following two methods for writing log records:

-   [**WriteLogRecordVariants**](icrmlogcontrol-writelogrecordvariants.md) is used for writing a structured log record that is built up as a collection of Variants. It is primarily for use when developing CRMs in Microsoft Visual Basic.
-   [**WriteLogRecord**](icrmlogcontrol-writelogrecord.md) is used for writing an unstructured log record as BLOBs of bytes. It is primarily for use when developing CRMs in Microsoft Visual C++. Because record structures in C are often made up of a set of headers and fields that may be scattered in memory, the **WriteLogRecord** method implements a gather capability that reduces the copying of data.
...
Both of these write methods write a log record to disk but do not guarantee the record's durability. While allowing lazy writes to accumulate before forcing to disk can improve performance, you can use the [**ICrmLogControl::ForceLog**](icrmlogcontrol-forcelog.md) method instead to guarantee that all writes done by the CRM are durable on disk, which is important for failure recovery.

When the CRM worker is done with its actions and has finished writing and forcing records to the log, it must release [**ICrmLogControl**](icrmlogcontrol.md). When the transaction completes (typically due to the application component calling [**SetComplete**](iobjectcontext-setcomplete.md) or [**SetAbort**](iobjectcontext-setabort.md)), the CRM infrastructure creates the CRM Compensator component, which implements either the [**ICrmCompensator**](icrmcompensator.md) interface or the [**ICrmCompensatorVariants**](icrmcompensatorvariants.md) interface. These interfaces are used to pass the unstructured (Visual C++) or structured (Visual Basic) records to the CRM Compensator along with the transaction outcome notifications.
...
The CRM Compensator can log new records if required by using [**ICrmLogControl**](icrmlogcontrol.md), which it receives as it is created. Both the CRM worker and CRM Compensator can also forget the last log record they wrote, which might be required to avoid unnecessary recovery.
