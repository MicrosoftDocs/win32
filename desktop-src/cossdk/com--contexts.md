...
Each context has an associated [**ObjectContext**](objectcontext.md) object that keeps track of its properties. You can access **ObjectContext** by calling the [**GetObjectContext**](getobjectcontext.md) function. After you have accessed **ObjectContext**, you can call methods on the [**IObjectContext**](iobjectcontext.md) interface it exposes to manipulate context properties.

For example, calling [**IObjectContext::SetComplete**](iobjectcontext-setcomplete.md) has the effect of setting the transaction consistency bit to "consistent" and the [JIT-activation](com--just-in-time-activation.md) done bit to "done" on the context associated with the object. "Consistent" signals to COM+ that you vote to commit the transaction, and "done" indicates that your object is ready to be deactivated when the method returns.

In addition to [**IObjectContext**](iobjectcontext.md), other specialized interfaces providing access to context properties are [**IObjectContextInfo**](iobjectcontextinfo.md), [**IContextState**](icontextstate.md), and [**IObjectContextActivity**](iobjectcontextactivity.md). To a certain extent, [**ISecurityCallContext**](isecuritycallcontext.md) also accesses context properties. You can use [**IGetSecurityCallContext::GetSecurityCallContext**](igetsecuritycallcontext-getsecuritycallcontext.md) to obtain **ISecurityCallContext**.
...
Contexts effectively replace the MTS context wrappers. The purpose they served—providing automatic services by trapping creation requests—is now an integrated feature of COM+. As a result, you no longer need to use the [**SafeRef**](saferef.md) function. In MTS, **SafeRef** was used to obtain a reference to your object which could be passed outside of its context wrapper. In COM+, this is unnecessary; normal object references (**this** pointers) work.
