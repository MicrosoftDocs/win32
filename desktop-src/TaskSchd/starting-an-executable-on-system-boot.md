...
Boot triggers are activated by their start boundary but they do not start the executable until the system is booted. You can also specify a delay time in the boot trigger that specifies the amount of time between when the system is booted and when the task is started. This is defined by the [**Delay**](/windows/win32/taskschd/nf-taskschd-iboottrigger-get_delay?branch=master) property in the [**IBootTrigger**](/windows/win32/taskschd/nn-taskschd-iboottrigger?branch=master) interface ([**BootTrigger**](boottrigger.md) object for scripting).
