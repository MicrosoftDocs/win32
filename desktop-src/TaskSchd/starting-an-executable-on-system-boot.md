...
keywords: ["Task Scheduler examples Task Scheduler , boot trigger"]
---
...
Boot triggers are activated by their start boundary but they do not start the executable until the system is booted. You can also specify a delay time in the boot trigger that specifies the amount of time between when the system is booted and when the task is started. This is defined by the [**Delay**](iboottrigger-delay.md) property in the [**IBootTrigger**](iboottrigger.md) interface ([**BootTrigger**](boottrigger.md) object for scripting).
