...
When creating a task, you will use two Task Scheduler interfaces: [**ITaskScheduler**](/windows/win32/Mstask/nn-mstask-itaskscheduler?branch=master) and [**ITask**](/windows/win32/Mstask/nn-mstask-itask?branch=master). You must provide a unique name for the task, the class identifier of the task object, and the interface identifier of **ITask**. The class identifier and interface identifier are shown in the code example following this topic.

> [!Note]  
> You can also create a task by calling [**ITaskScheduler::AddWorkItem**](/windows/win32/Mstask/nf-mstask-itaskscheduler-addworkitem?branch=master). When you take this route, it is your responsibility to create an instance of the **Task** object (which supports the [**ITask**](/windows/win32/Mstask/nn-mstask-itask?branch=master) interface) and then add the task with the name you supply.
...
The name you supply for the task must be unique within the Scheduled Tasks folder. If a task with the same name already exists, [**ITaskScheduler::NewWorkItem**](/windows/win32/Mstask/nf-mstask-itaskscheduler-newworkitem?branch=master) returns ERROR\_FILE\_EXISTS. If you get this return value, you should specify a different name and attempt to create the task again.
...
2.  Call [**ITaskScheduler::NewWorkItem**](/windows/win32/Mstask/nf-mstask-itaskscheduler-newworkitem?branch=master) to create a new task. (This method returns a pointer to an [**ITask**](/windows/win32/Mstask/nn-mstask-itask?branch=master) interface.)
...
4.  Call **ITask::Release** to release all resources. (Note that [**Release**](_com_iunknown_release) is an [**IUnknown**](_com_iunknown) method inherited by [**ITask**](/windows/win32/Mstask/nn-mstask-itask?branch=master).)
