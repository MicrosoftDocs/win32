...
keywords: ["StartWhenAvailable property Task Scheduler", "StartWhenAvailable property Task Scheduler , TaskSettings object", "TaskSettings object Task Scheduler , StartWhenAvailable property"]
topic_type:
- apiref
api_name:
...
Tasks that are started after the scheduled time has passed (because of the [**StartWhenAvailable**](itasksettings-startwhenavailable.md) property being set to True) are queued in the Task Scheduler service's queue of tasks and they are started after a delay. The default delay is 10 minutes.
