---
title: RunningTaskCollection.Count property
description: For scripting, gets the number of running tasks in the collection.
ms.assetid: '0365b804-6450-494d-8a5b-8fc805766611'
keywords: ["Count property Task Scheduler", "Count property Task Scheduler , RunningTaskCollection object", "RunningTaskCollection object Task Scheduler , Count property"]
topic_type:
- apiref
api_name:
- RunningTaskCollection.Count
api_location:
- taskschd.dll
api_type:
- COM
---

# RunningTaskCollection.Count property

For scripting, gets the number of running tasks in the collection.

## Syntax


```VB
RunningTaskCollection.Count As long
```



## Property value

The number of running tasks in the collection.

## Requirements



|                                     |                                                                                         |
|-------------------------------------|-----------------------------------------------------------------------------------------|
| Minimum supported client<br/> | Windows Vista \[desktop apps only\]<br/>                                          |
| Minimum supported server<br/> | Windows Server 2008 \[desktop apps only\]<br/>                                    |
| Type library<br/>             | <dl> <dt>Taskschd.tlb</dt> </dl> |
| DLL<br/>                      | <dl> <dt>Taskschd.dll</dt> </dl> |



## See also

<dl> <dt>

[Task Scheduler](task-scheduler-start-page.md)
</dt> </dl>

 

 





