...
keywords: ["Task Scheduler examples Task Scheduler , logon trigger"]
---
...
Logon triggers are activated by their start boundary but they do not start the executable until a specified user logs on. You can specify the logon trigger to start when a certain user logs on by specifying the user in the [**UserId**](ilogontrigger-userid.md) property of the [**ILogonTrigger**](ilogontrigger.md) interface ([**LogonTrigger**](logontrigger.md) for scripting).
