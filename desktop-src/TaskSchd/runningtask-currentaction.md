---
title: RunningTask.CurrentAction property
description: For scripting, gets the name of the current action that the running task is performing.
ms.assetid: '52b58477-e968-4c76-835d-02ed63cef641'
keywords: ["CurrentAction property Task Scheduler", "CurrentAction property Task Scheduler , RunningTask object", "RunningTask object Task Scheduler , CurrentAction property"]
topic_type:
- apiref
api_name:
- RunningTask.CurrentAction
api_location:
- taskschd.dll
api_type:
- COM
---

# RunningTask.CurrentAction property

For scripting, gets the name of the current action that the running task is performing.

This property is read-only.

## Syntax


```VB
RunningTask.CurrentAction As String
```



## Property value

The name of the current action that the running task is performing.

## Requirements



|                                     |                                                                                         |
|-------------------------------------|-----------------------------------------------------------------------------------------|
| Minimum supported client<br/> | Windows Vista \[desktop apps only\]<br/>                                          |
| Minimum supported server<br/> | Windows Server 2008 \[desktop apps only\]<br/>                                    |
| Type library<br/>             | <dl> <dt>Taskschd.tlb</dt> </dl> |
| DLL<br/>                      | <dl> <dt>Taskschd.dll</dt> </dl> |



 

 





