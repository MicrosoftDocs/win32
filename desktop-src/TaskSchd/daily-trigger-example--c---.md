...
5.  Create a daily trigger using the [**Triggers property of ITaskDefinition**](itaskdefinition-triggers.md) to access the [**ITriggerCollection**](itriggercollection.md) interface for the task.

    Use the [**ITriggerCollection::Create**](itriggercollection-create.md) method to specify that you want to create a daily trigger. You can set the start boundary and the days interval for the trigger so that the task's actions will be scheduled to execute at a specified time on certain days. The example also shows how to set a repetition pattern for the trigger to repeat the task.
