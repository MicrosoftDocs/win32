---
title: TaskFolderCollection.Count property
description: For scripting, gets the number of folders in the collection.
ms.assetid: '073a812d-209b-4e36-b94b-c24567377a84'
keywords: ["Count property Task Scheduler", "Count property Task Scheduler , TaskFolderCollection object", "TaskFolderCollection object Task Scheduler , Count property"]
topic_type:
- apiref
api_name:
- TaskFolderCollection.Count
api_location:
- taskschd.dll
api_type:
- COM
---

# TaskFolderCollection.Count property

For scripting, gets the number of folders in the collection.

## Syntax


```VB
TaskFolderCollection.Count As long
```



## Property value

The number of triggers in the collection.

## Requirements



|                                     |                                                                                         |
|-------------------------------------|-----------------------------------------------------------------------------------------|
| Minimum supported client<br/> | Windows Vista \[desktop apps only\]<br/>                                          |
| Minimum supported server<br/> | Windows Server 2008 \[desktop apps only\]<br/>                                    |
| Type library<br/>             | <dl> <dt>Taskschd.tlb</dt> </dl> |
| DLL<br/>                      | <dl> <dt>Taskschd.dll</dt> </dl> |



## See also

<dl> <dt>

[Task Scheduler](task-scheduler-start-page.md)
</dt> </dl>

 

 





