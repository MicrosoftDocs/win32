...
To enumerate tasks, you must call [**ITaskScheduler::Enum**](itaskscheduler-enum.md) to create an [*enumeration object*](e.md#-msb-enumeration-objects-gly). Then, use the enumeration object's [**IEnumWorkItems**](ienumworkitems.md) interface to enumerate the tasks in the Scheduled Tasks folder.
...
2.  Call [**ITaskScheduler::Enum**](itaskscheduler-enum.md) to get an enumeration object.
3.  Call [**IEnumWorkItems::Next**](ienumworkitems-next.md) to retrieve the tasks. (This example tries to retrieve five tasks with each call.)
