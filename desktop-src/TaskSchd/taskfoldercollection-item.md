---
title: TaskFolderCollection.Item property
description: For scripting, gets the specified folder from the collection.
ms.assetid: '6caf3128-5e97-4653-88d3-3eb8131409dd'
keywords: ["Item property Task Scheduler", "Item property Task Scheduler , TaskFolderCollection object", "TaskFolderCollection object Task Scheduler , Item property"]
topic_type:
- apiref
api_name:
- TaskFolderCollection.Item
api_location:
- taskschd.dll
api_type:
- COM
---

# TaskFolderCollection.Item property

For scripting, gets the specified folder from the collection.

## Syntax


```VB
TaskFolderCollection.Item( _
  ByVal Index _
) As TaskFolder
```



## Property value

A [**TaskFolder**](taskfolder.md) object that represents the requested folder.

## Requirements



|                                     |                                                                                         |
|-------------------------------------|-----------------------------------------------------------------------------------------|
| Minimum supported client<br/> | Windows Vista \[desktop apps only\]<br/>                                          |
| Minimum supported server<br/> | Windows Server 2008 \[desktop apps only\]<br/>                                    |
| Type library<br/>             | <dl> <dt>Taskschd.tlb</dt> </dl> |
| DLL<br/>                      | <dl> <dt>Taskschd.dll</dt> </dl> |



## See also

<dl> <dt>

[Task Scheduler](task-scheduler-start-page.md)
</dt> </dl>

 

 





