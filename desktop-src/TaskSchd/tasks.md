...
Data that describes a task is kept in a task file that is stored in the Scheduled Tasks folder. For more information, see [*Scheduled Tasks folder*](https://www.bing.com/search?q=*Scheduled Tasks folder*). The name of these task files include the name of the task, followed by a .job file name extension.
