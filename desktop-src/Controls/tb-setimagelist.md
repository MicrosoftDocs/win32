...
3.  For each button, set the **iBitmap** member of the button's [**TBBUTTON**](/windows/win32/Commctrl/ns-commctrl-_tbbutton?branch=master) structure to MAKELONG(*iIndex*, *iImageID*). The *iImageID* value is the ID of the appropriate image list that was defined in step two. The *iIndex* value is the index of the particular image within that list.
