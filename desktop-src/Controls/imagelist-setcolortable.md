...
keywords: ["ImageList_SetColorTable function Windows Controls"]
topic_type:
- apiref
api_name:
...
Only image lists created with the [**ILC\_COLOR4**](imagelist-create.md) or [**ILC\_COLOR8**](imagelist-create.md) flag have color tables. The color table of such an image list is typically set automatically by copying the color table of the first image added to the list (for example, through the [**ImageList\_Add**](imagelist-add.md) function) if that image is a DIB. If the first image added to the image list is not a DIB, then the color table of the halftone palette is used for **ILC\_COLOR8** image lists and the VGA color table is used for **ILC\_COLOR4**.
