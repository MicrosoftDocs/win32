---
title: DTN\_DROPDOWN notification code
...
keywords: ["DTN_DROPDOWN notification code Windows Controls"]
topic_type:
- apiref
api_name:
- DTN_DROPDOWN
...
One task that your notification handler may need to perform is to customize the dropdown month-calendar control. For instance, if you do not want "Go To Today," you need to set the control's [**MCS\_NOTODAY**](month-calendar-control-styles.md#mcs-notoday)  style. To retrieve a handle to the month-calendar control, send the DTP control a [**DTM\_GETMONTHCAL**](dtm-getmonthcal.md) message. You can then use this handle and [**SetWindowLong**](https://msdn.microsoft.com/library/windows/desktop/ms633591) to set the desired month-calendar style.
