...
One task that your notification handler may need to perform is to customize the dropdown month-calendar control. For instance, if you do not want "Go To Today," you need to set the control's [**MCS\_NOTODAY**](https://www.bing.com/search?q=**MCS\_NOTODAY**)  style. To retrieve a handle to the month-calendar control, send the DTP control a [**DTM\_GETMONTHCAL**](dtm-getmonthcal.md) message. You can then use this handle and [**SetWindowLong**](https://msdn.microsoft.com/library/windows/desktop/ms633591) to set the desired month-calendar style.
