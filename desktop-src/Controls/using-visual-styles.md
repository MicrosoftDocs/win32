...
To check whether visual styles are available, call the [**IsAppThemed**](/windows/win32/Uxtheme/nf-uxtheme-isappthemed?branch=master) function. If visual styles are not available, use fallback code to draw the control.

If visual styles are available, you can use visual-styles functions such as [**DrawThemeText**](/windows/win32/Uxtheme/nf-uxtheme-drawthemetext?branch=master) to render your control. Note that [**DrawThemeTextEx**](/windows/win32/Uxtheme/nf-uxtheme-drawthemetextex?branch=master) enables you to customize the appearance of text, retaining some properties of the theme font while modifying others.

**To draw a control in the current visual style**

1.  Call [**OpenThemeData**](/windows/win32/Uxtheme/nf-uxtheme-openthemedata?branch=master), passing the *hwnd* of the control you want to apply visual styles to and a class list that describes the control's type. The classes are defined in Vssym32.h. **OpenThemeData** returns an HTHEME handle, but if the visual styles manager is disabled or the current visual style does not supply specific information for a given control, the function returns **NULL**. If the return value is **NULL**, use non-visual-styles drawing functions.
2.  To draw the control background, call [**DrawThemeBackground**](/windows/win32/Uxtheme/nf-uxtheme-drawthemebackground?branch=master) or [**DrawThemeBackgroundEx**](/windows/win32/Uxtheme/nf-uxtheme-drawthemebackgroundex?branch=master).
3.  To determine the location of the content rectangle, call [**GetThemeBackgroundContentRect**](/windows/win32/Uxtheme/nf-uxtheme-getthemebackgroundcontentrect?branch=master).
4.  To render text, use either [**DrawThemeText**](/windows/win32/Uxtheme/nf-uxtheme-drawthemetext?branch=master) or [**DrawThemeTextEx**](/windows/win32/Uxtheme/nf-uxtheme-drawthemetextex?branch=master), basing the coordinates on the rectangle returned by [**GetThemeBackgroundContentRect**](/windows/win32/Uxtheme/nf-uxtheme-getthemebackgroundcontentrect?branch=master). These functions can render text either in the theme's font for a specified control part and state, or in the font currently selected into the device context (DC).
5.  When your control receives a [**WM\_DESTROY**](https://msdn.microsoft.com/library/windows/desktop/ms632620) message, call [**CloseThemeData**](/windows/win32/Uxtheme/nf-uxtheme-closethemedata?branch=master) to release the theme handle that was returned when you called [**OpenThemeData**](/windows/win32/Uxtheme/nf-uxtheme-openthemedata?branch=master).
...
You can mix and match control parts by calling [**OpenThemeData**](/windows/win32/Uxtheme/nf-uxtheme-openthemedata?branch=master) multiple times for a given control and using the appropriate theme handle to draw different parts. In some visual styles, however, certain parts might not be compatible with other parts.

Another approach to rendering controls in the active visual style is to use the system colors. For example, you can use system colors to set the text color when calling the [**DrawThemeTextEx**](/windows/win32/Uxtheme/nf-uxtheme-drawthemetextex?branch=master) function. Most system colors are set when a visual style file is applied.
...
-   Call [**CloseThemeData**](/windows/win32/Uxtheme/nf-uxtheme-closethemedata?branch=master) to close the existing theme handle.
-   Call [**OpenThemeData**](/windows/win32/Uxtheme/nf-uxtheme-openthemedata?branch=master) to get the theme handle to the newly loaded visual style.
