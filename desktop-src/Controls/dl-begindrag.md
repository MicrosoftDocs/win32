...
Notifies the drag list box's parent window that the user has clicked the left mouse button on an item. A drag list box sends this notification code in the form of a drag list message. For more information, see [Drag List Box Messages](https://www.bing.com/search?q=Drag List Box Messages).
...
A pointer to a [**DRAGLISTINFO**](/windows/desktop/api/Commctrl/ns-commctrl-tagdraglistinfo) structure that contains the DL\_BEGINDRAG notification code, the handle to the drag list box, and the cursor position.
