...
<td style="text-align: left;">Allows you to match an animation's background color to that of the underlying window, creating a &quot;transparent&quot; background. The parent of the animation control must not have the WS_CLIPCHILDREN style (see [Window Styles](https://msdn.microsoft.com/library/windows/desktop/ms632600)). The control sends a [<strong>WM_CTLCOLORSTATIC</strong>](wm-ctlcolorstatic.md) message to its parent. Use [<strong>SetBkColor</strong>](https://msdn.microsoft.com/library/windows/desktop/dd162964) to set the background color for the device context to an appropriate value. The control interprets the upper-left pixel of the first frame as the animation's default background color. It will remap all pixels with that color to the value you supplied in response to WM_CTLCOLORSTATIC. <br/></td>
</tr>
</tbody>
</table>
