...
When you add an item to the list, you assign it to a group by setting the **iGroupId** member of the item's [**LVITEM**](lvitem.md) structure to the value of the **iGroupId** member of the groups's [**LVGROUP**](lvgroup.md) structure. An item that is not assigned to a group does not appear in the list when group view is enabled. To enable or disable group view, use the [**ListView\_EnableGroupView**](listview-enablegroupview.md) macro.
