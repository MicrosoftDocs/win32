...
Gets the bounding rectangle of the overflow button when the [**HDS\_OVERFLOW**](https://www.bing.com/search?q=**HDS\_OVERFLOW**) style is set on the header control and the overflow button is visible. Send this message explicitly or by using the [**Header\_GetOverflowRect**](/windows/desktop/api/Commctrl/nf-commctrl-header_getoverflowrect) macro.
