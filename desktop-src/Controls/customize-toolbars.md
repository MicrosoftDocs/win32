...
The first [TBN\_SAVE](tbn-save.md) notification code is sent at the beginning of the save process. Before any buttons are saved, the members of the [**NMTBSAVE**](nmtbsave.md) structure are set as shown in the following table.
...
After each button is saved, a [TBN\_SAVE](tbn-save.md) notification code is sent and [**NMTBSAVE**](nmtbsave.md) is returned with these members set as follows.
...
| [**TBBUTTON**](tbbutton.md) | A [**TBBUTTON**](tbbutton.md) structure that describes the button that is being saved.                                                                                                                                                                              |



 

When you receive the notification code, you should extract any button-specific information you need from [**TBBUTTON**](tbbutton.md). Remember that when you add a button, you can use the **dwData** member of **TBBUTTON** to hold application-specific data. Load your data into the data stream at **pCurrent**. Advance **pCurrent** to the end of your data, again pointing to the beginning of the unused portion of the stream, and return.

The Shell then goes to the next button, adds its information to **pData**, advances **pCurrent**, loads [**TBBUTTON**](tbbutton.md), and sends another [TBN\_SAVE](tbn-save.md) notification code. This process continues until all buttons are saved.

### Restoring Saved Toolbars

The restore process basically reverses the save process. At the beginning, your application will receive a [TBN\_RESTORE](tbn-restore.md) notification code with the **iItem** member of the [**NMTBRESTORE**](nmtbrestore.md) structure set to –1. The **cbData** member is set to the size of **pData**, and **cButtons** is set to the number of buttons.
