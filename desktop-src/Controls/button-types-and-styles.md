...
A *group box* is a rectangle that surrounds a set of controls, such as check boxes or radio buttons, with an application-defined text label in its upper left corner. The sole purpose of a group box is to organize controls related by a common purpose (usually indicated by the label). The group box has only one style, defined by the constant [**BS\_GROUPBOX**](button-styles.md#bs-groupbox). Because a group box cannot be selected, it has no check state, focus state, or push state.
...
A radio button can be one of two styles: standard or automatic, as defined by the style constants [**BS\_RADIOBUTTON**](button-styles.md#bs-radiobutton) and [**BS\_AUTORADIOBUTTON**](button-styles.md#bs-autoradiobutton). Each style can assume two check states: checked (a dot in the button) or cleared (no dot in the button).
