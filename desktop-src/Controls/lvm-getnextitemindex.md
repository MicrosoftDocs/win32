...
keywords: ["LVM_GETNEXTITEMINDEX message Windows Controls"]
topic_type:
- apiref
api_name:
...
Retrieves the index of an item in a specified list-view control that matches the specified properties and relationship to another item. Send this message explicitly or by using the [**ListView\_GetNextItemIndex**](listview-getnextitemindex.md) macro.
...
A pointer to the [**LVITEMINDEX**](lvitemindex.md) structure for the item to begin the search with, or -1 to find the first item that matches the specified flags. The calling process is responsible for allocating this structure and setting its members.
