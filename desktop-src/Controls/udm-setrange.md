...
The [**LOWORD**](/previous-versions/windows/desktop/legacy/ms632659(v=vs.85)) is a **short** that specifies the maximum position for the up-down control, and the [**HIWORD**]([HIWORD macro](../winmsg/hiword.md)(v=vs.85)) is a **short** that specifies the minimum position. Neither position can be greater than the UD\_MAXVAL value or less than the UD\_MINVAL value. In addition, the difference between the two positions cannot exceed UD\_MAXVAL.
