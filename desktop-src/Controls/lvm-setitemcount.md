...
Causes the list-view control to allocate memory for the specified number of items or sets the virtual number of items in a [virtual list-view control](https://www.bing.com/search?q=virtual list-view control).
...
If the list-view control was created without the [**LVS\_OWNERDATA**](https://www.bing.com/search?q=**LVS\_OWNERDATA**) style, sending this message causes the control to allocate its internal data structures for the specified number of items. This prevents the control from having to allocate the data structures every time an item is added.

If the list-view control was created with the [**LVS\_OWNERDATA**](https://www.bing.com/search?q=**LVS\_OWNERDATA**) style (a virtual list view), sending this message sets the virtual number of items that the control contains.

The *lParam* parameter is intended only for list-view controls that use the [**LVS\_OWNERDATA**](https://www.bing.com/search?q=**LVS\_OWNERDATA**) and [**LVS\_REPORT**](https://www.bing.com/search?q=**LVS\_REPORT**) or [**LVS\_LIST**](https://www.bing.com/search?q=**LVS\_LIST**) styles.

When the common control list-view is a virtualized list-view ([**LVS\_OWNERDATA**](https://www.bing.com/search?q=**LVS\_OWNERDATA**)), there is a 100,000,000 item limit on the list-view. In this scenario, **LVM\_SETITEMCOUNT** will return FALSE when it has a *wParam* of 100,000,001.
