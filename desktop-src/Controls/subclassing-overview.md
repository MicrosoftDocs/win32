...
To make a new control, it is best to start with one of the Windows common controls and extend it to fit a particular need. To extend a control, create a control and replace its existing window procedure with a new one. The new procedure intercepts the control's messages and either acts on them or passes them to the original procedure for default processing. Use the [**SetWindowLong**](https://msdn.microsoft.com/library/windows/desktop/ms633591) or [**SetWindowLongPtr**](https://msdn.microsoft.com/library/windows/desktop/ms644898) function to replace the WNDPROC of the control. The following code sample shows how to replace a WNDPROC.
...
-   To call the next procedure in a subclass chain, you cannot cast the old window procedure and call it, you must call it by using the [**CallWindowProc**](https://msdn.microsoft.com/library/windows/desktop/ms633571) function.
...
-   [**SetWindowSubclass**](https://msdn.microsoft.com/library/windows/desktop/bb762102)
-   [**GetWindowSubclass**](https://msdn.microsoft.com/library/windows/desktop/bb776430)
-   [**RemoveWindowSubclass**](https://msdn.microsoft.com/library/windows/desktop/bb762094)
-   [**DefSubclassProc**](https://msdn.microsoft.com/library/windows/desktop/bb776403)

### SetWindowSubclass

This function is used to initially subclass a window. Each subclass is uniquely identified by the address of the *pfnSubclass* and its *uIdSubclass*. Both of these are parameters of the [**SetWindowSubclass**](https://msdn.microsoft.com/library/windows/desktop/bb762102) function. Several subclasses can share the same subclass procedure and the ID can identify each call. To change reference data you can make subsequent calls to **SetWindowSubclass**. The important advantage is that each subclass instance has its own reference data.
...
The window procedure can be attached to the control in the [**WM\_INITDIALOG**](https://msdn.microsoft.com/library/windows/desktop/ms645428) handler of the dialog procedure, as shown in the following example.
...
This function retrieves information about a subclass. For example, you can use [**GetWindowSubclass**](https://msdn.microsoft.com/library/windows/desktop/bb776430) to access the reference data.

### RemoveWindowSubclass

This function removes subclasses. [**RemoveWindowSubclass**](https://msdn.microsoft.com/library/windows/desktop/bb762094) in combination with [**SetWindowSubclass**](https://msdn.microsoft.com/library/windows/desktop/bb762102) allows you to dynamically add and remove subclasses.

### DefSubclassProc

The [**DefSubclassProc**](https://msdn.microsoft.com/library/windows/desktop/bb776403) function calls the next handler in the subclass chain. The function also retrieves the proper ID and reference data and passes the information to the next window procedure.
