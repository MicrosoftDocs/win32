...
keywords: ["Setting and Retrieving Internet Options"]
---

# Setting and Retrieving Internet Options

This topic describes how to set and retrieve Internet options using the [**InternetSetOption**](internetsetoption.md) and [**InternetQueryOption**](internetqueryoption.md) functions.
...
In Internet Explorer 5 and later, some options can be set or retrieved from a specific Internet connection using the [**INTERNET\_PER\_CONN\_OPTION\_LIST**](internet-per-conn-option-list.md) and [**INTERNET\_PER\_CONN\_OPTION**](internet-per-conn-option.md) structures. For more information and a list of options that can be set or retrieved from a specific Internet connection, see the **dwOptions** member of the [**INTERNET\_PER\_CONN\_OPTION**](internet-per-conn-option.md) structure.
...
For example, client applications that require a proxy with authentication, probably do not require setting the proxy user name and password every time the application attempts to access an Internet resource. If all requests on a given connection are handled by the same proxy, setting the proxy user name and password on a connection type [**HINTERNET**](appendix-a-hinternet-handles.md) handle, that is, a handle created by a call to [**InternetConnect**](internetconnect.md), would allow any calls derived from this **HINTERNET** handle to use the same proxy user name and password. Setting the proxy user name and password every time an **HINTERNET** handle is created by [**HttpOpenRequest**](httpopenrequest.md) would require extra and unnecessary overhead. Be aware that if the application uses a proxy that requires authentication, it should set the proxy credentials on every new connection.

### Setting or Retrieving the Options

When you have determined what Internet options and [**HINTERNET**](appendix-a-hinternet-handles.md) handle to use, retrieve those Internet options. To set or retrieve options, call either [**InternetQueryOption**](internetqueryoption.md) or [**InternetSetOption**](internetsetoption.md).
...
-   The root [**HINTERNET**](appendix-a-hinternet-handles.md) handle (created by a call to [**InternetOpen**](internetopen.md)) would contain all the Internet options that affect this instance of WinINet.
-   [**HINTERNET**](appendix-a-hinternet-handles.md) handles that connect to a server (created by a call to [**InternetConnect**](internetconnect.md))
...
After determining the Internet options you want to set and the scope you want affected by these options, setting Internet options is not complicated. All you would need to do is call the [**InternetSetOption**](internetsetoption.md) function with desired [**HINTERNET**](appendix-a-hinternet-handles.md) handle, Internet option flag, and a buffer that contains the information you want set.
...
Internet options can be retrieved using the [**InternetQueryOption**](internetqueryoption.md) function. To retrieve Internet options:
...
1.  Determine the buffer size necessary to retrieve the Internet option information.
    ...
    The value returned by [**InternetQueryOption**](internetqueryoption.md) is the amount of memory required to retrieve the information, in bytes.
...
1.  Create an [**INTERNET\_PER\_CONN\_OPTION\_LIST**](internet-per-conn-option-list.md) structure.
2.  Allocate the memory for the individual Internet options that you want to set for the connection.
3.  Set the options in [**INTERNET\_PER\_CONN\_OPTION**](internet-per-conn-option.md) structures.
4.  Set the options using [**InternetSetOption**](internetsetoption.md).
...
1.  Create a [**INTERNET\_PER\_CONN\_OPTION\_LIST**](internet-per-conn-option-list.md) structure.
2.  Allocate the memory for the individual Internet options to retrieve from the connection.
3.  Specify the options using [**INTERNET\_PER\_CONN\_OPTION**](internet-per-conn-option.md) structures.
4.  Retrieve the options using [**InternetQueryOption**](internetqueryoption.md).
