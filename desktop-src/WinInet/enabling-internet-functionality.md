...
Before using the WinINet functions, the application should attempt to make a connection to the Internet by using the [**InternetAttemptConnect**](internetattemptconnect.md) function. This function calls the dial-up dialog box to initiate a connection to the Internet or check if a connection already exists. If this function fails, the application can enter offline mode, which allows it to access information that was cached during previous connections to the Internet.

Use the [**InternetCheckConnection**](internetcheckconnection.md) function to check the connection to the Internet. It attempts to ping the server designated by the URL that is passed to the function. If the FLAG\_ICC\_FORCE\_CONNECTION flag is set and the URL is **NULL**, the function checks to see if there is an entry in the server database for the nearest server. If one exists, the function pings that server.

Next, use the [**InternetOpen**](internetopen.md) function to establish the characteristics of the Internet connection the client application is using. [**InternetOpen**](internetopen.md) creates the root [**HINTERNET**](appendix-a-hinternet-handles.md) handle that is used to establish [http](http-sessions.md)[ftp](ftp-sessions.md) sessions. [**InternetOpen**](internetopen.md) does not test the connection to the Internet to verify that the characteristics passed to the function are correct.

Use the [**InternetConnect**](internetconnect.md) function to create a specific session. [**InternetConnect**](internetconnect.md) initializes a session for the specified site using the arguments passed to it and creates an [**HINTERNET**](appendix-a-hinternet-handles.md) handle that is a branch off the root handle. [**InternetConnect**](internetconnect.md) does not attempt to access or establish a connection to the specified site, except in the case of an FTP session. [**FtpFindFirstFile**](ftpfindfirstfile.md), [**FtpOpenFile**](ftpopenfile.md), and [**HttpOpenRequest**](httpopenrequest.md) functions use the handle created by [**InternetConnect**](internetconnect.md) to establish a connection to the specified site.

## Using InternetOpen

To enable a connection to the Internet, a root [**HINTERNET**](appendix-a-hinternet-handles.md) handle must be created by using [**InternetOpen**](internetopen.md). Information about the user agent (the application calling the Internet functions), the type of access to the Internet, the proxy names, the hosts and addresses that bypass the proxy, and the behavior are passed to [**InternetOpen**](internetopen.md).

### Setting the User Agent

The calling application should give a string that contains the name of the application or entity accessing the Internet to the *lpszAgent* parameter of [**InternetOpen**](internetopen.md). This string is used as the user agent in the HTTP protocol. For example, Microsoft Internet Explorer uses "Microsoft Internet Explorer".

### Setting Access Types

[**InternetOpen**](internetopen.md) supports three access types:

-   Use INTERNET\_OPEN\_TYPE\_DIRECT if the system on which the application is running uses a direct connection to the Internet. The *lpszProxyName* and *lpszProxyBypass* parameters of [**InternetOpen**](internetopen.md) are not used and should be set to **NULL**.
-   Use INTERNET\_OPEN\_TYPE\_PROXY if the system on which the application is running uses one or more proxy servers to access the Internet. [**InternetOpen**](internetopen.md) uses the proxy servers indicated by *lpszProxyName* and bypasses the proxy for any host names or IP addresses specified by *lpszProxyBypass*.
...
WinINet recognizes two types of proxies: CERN type proxies (HTTP only) and TIS FTP proxies (FTP only). If Internet Explorer is installed, WinINet also support SOCKS type proxies. [**InternetConnect**](internetconnect.md) assumes, by default, that the specified proxy is a CERN proxy. If the access type is set to INTERNET\_OPEN\_TYPE\_DIRECT or INTERNET\_OPEN\_TYPE\_PRECONFIG, the *lpszProxyName* parameter of [**InternetOpen**](internetopen.md) should be set to **NULL**. Otherwise, the value passed to *lpszProxyName* must contain the proxies in a space-delimited string. The proxy listings can contain the port number that is used to access the proxy.
...
The following example shows sample calls to [**InternetOpen**](internetopen.md) using different proxy bypass strings. The comments above each call describe what effect the bypass string has on the host names that are accessed from the [**HINTERNET**](appendix-a-hinternet-handles.md) handle it creates.
...
To begin a session, the [**InternetConnect**](internetconnect.md) function must create a handle off the root handle returned by the [**InternetOpen**](internetopen.md) function. [**InternetConnect**](internetconnect.md) sets the server address, port number, user name, password, and type of service.

[**InternetConnect**](internetconnect.md) uses the root [**HINTERNET**](appendix-a-hinternet-handles.md) handle created by [**InternetOpen**](internetopen.md) to establish a session handle. If the [INTERNET\_FLAG\_ASYNC](api-flags.md#internet-flag-async) flag was set in the call to [**InternetOpen**](internetopen.md), the call to [**InternetConnect**](internetconnect.md) should include a nonzero context value.
...
The server port is the Transmission Control Protocol/Internet Protocol (TCP/IP) port number to connect to on the server. [**InternetConnect**](internetconnect.md) uses the default port for the selected service type if the INTERNET\_INVALID\_PORT\_NUMBER value is used. The following tables contain the server port defaults for WinINet.
...
This information can be changed by using the [**InternetSetOption**](internetsetoption.md) and [**InternetErrorDlg**](interneterrordlg.md) functions. [**InternetSetOption**](internetsetoption.md) changes the user name and password values, while [**InternetErrorDlg**](interneterrordlg.md) displays a dialog box that requests the proper user name and password.

### Defining the Session

To define the session that is being established, set the service type, flags, and context value for [**InternetConnect**](internetconnect.md).

There are two service types available to [**InternetConnect**](internetconnect.md): INTERNET\_SERVICE\_FTP and INTERNET\_SERVICE\_HTTP. INTERNET\_SERVICE\_HTTP is used for both HTTP and HTTPS sessions.
...
For all synchronous operations, the value of *dwContext* should be set to zero. If asynchronous operations were established by setting the [INTERNET\_FLAG\_ASYNC](api-flags.md#internet-flag-async) flag in the call to [**InternetOpen**](internetopen.md), a nonzero value should be supplied for *dwContext*. For more information on asynchronous operations, see [Setting Up Asynchronous Operations](common-functions.md).

For FTP sessions, [**InternetConnect**](internetconnect.md) tries to establish a connection to the server on the Internet. For HTTP sessions, [**InternetConnect**](internetconnect.md) does not establish a connection until another function attempts to get information from the server.
