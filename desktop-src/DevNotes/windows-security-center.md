...
The Windows Security app is a comprehensive reporting tool that helps users establish and maintain a protective security layer around their computer systems. Once a security layer is established, the Windows Security app is inconspicuous as it monitors the computer’s health state. However, if vulnerabilities exist, the Windows Security app provides alerts and prescriptive guidance to assist the user in achieving a secure state which is surfaced to the end user through the Notification Center.

In order for third-party security solutions that provide real-time protection to be compliant with Windows, they are required to register and report any subsequent status changes using private APIs for communicating with the Windows Security app. In turn, the app communicates these updates to the Notification Center, where they are finally displayed to the end user.
