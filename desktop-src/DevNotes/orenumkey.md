...
A pointer to [FILETIME](http://go.microsoft.com/fwlink/p/?linkid=128776) structure that receives the time at which the enumerated subkey was last written. This parameter can be **NULL**.
...
The application can also set *dwIndex* to the index of the last subkey on the first call to the function and decrement the index until the subkey with the index 0 is enumerated. To retrieve the index of the last subkey, use the [**ORQueryInfoKey**](base.regqueryinfokey) function.
