...
This function is the user-mode equivalent to the [**ZwReadFile**](kernel.zwreadfile) function documented in the Windows Driver Kit (WDK).
...
Pointer to an [**IO\_STATUS\_BLOCK**](kernel.io_status_block) structure that receives the final completion status and information about the requested read operation. The **Information** member receives the number of bytes actually read from the file.
