...
These operations may be sent to the queue in any order, and the enqueuing process need not be contiguous. When the queue is committed, the [**SetupCommitFileQueue**](setupcommitfilequeue.md) function performs file operations in order of the operation type.
