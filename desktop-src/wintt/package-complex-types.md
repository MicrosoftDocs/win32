---
title: Package Complex Types
ms.assetid: '29bb4f93-b3d4-431b-b3ea-f1ca96dd0194'
description: 
---

# Package Complex Types

The Package schema contains the following complex types:

-   [**Choice**](package-choice-complextype.md)
-   [**Choices**](package-choices-complextype.md)
-   [**DiagnosticIdentification**](package-diagnosticidentification-complextype.md)
-   [**DisplayInformation**](package-displayinformation-complextype.md)
-   [**ExtensionPoint**](package-extensionpoint-complextype.md)
-   [**GlobalDiagnosticPackage**](package-globaldiagnosticpackage-complextype.md)
-   [**GlobalRootcause**](package-globalrootcause-complextype.md)
-   [**GlobalRootcauses**](package-globalrootcauses-complextype.md)
-   [**Interactions**](package-interactions-complextype.md)
-   [**LaunchUIInteraction**](package-launchuiinteraction-complextype.md)
-   [**LaunchUIInteractions**](package-launchuiinteractions-complextype.md)
-   [**LocalDiagnosticPackage**](package-localdiagnosticpackage-complextype.md)
-   [**LocalRootcause**](package-localrootcause-complextype.md)
-   [**LocalRootcauses**](package-localrootcauses-complextype.md)
-   [**MultipleResponseInteraction**](package-multipleresponseinteraction-complextype.md)
-   [**MultipleResponseInteractions**](package-multipleresponseinteractions-complextype.md)
-   [**Parameter**](package-parameter-complextype.md)
-   [**Parameters**](package-parameters-complextype.md)
-   [**PauseInteraction**](package-pauseinteraction-complextype.md)
-   [**PauseInteractions**](package-pauseinteractions-complextype.md)
-   [**Resolvers**](package-resolvers-complextype.md)
-   [**Script**](package-script-complextype.md)
-   [**SingleResponseInteraction**](package-singleresponseinteraction-complextype.md)
-   [**SingleResponseInteractions**](package-singleresponseinteractions-complextype.md)
-   [**SupportedOSVersion**](package-supportedosversion-complextype.md)
-   [**TextInteraction**](package-textinteraction-complextype.md)
-   [**TextInteractions**](package-textinteractions-complextype.md)
-   [**Troubleshooter**](package-troubleshooter-complextype.md)
-   [**Verifier**](package-verifier-complextype.md)

 

 




