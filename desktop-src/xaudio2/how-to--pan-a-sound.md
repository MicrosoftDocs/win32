...
1.  Retrieve the speaker configuration using [**IXAudio2MasteringVoice::GetChannelMask**](ixaudio2masteringvoice-interface-getchannelmask.md).
...
4.  Set the output matrix indices corresponding to the left and right speakers with the values calculated in the previous step. The left and right speakers are determined by looking at the channel mask returned by [**IXAudio2MasteringVoice::GetChannelMask**](ixaudio2masteringvoice-interface-getchannelmask.md). Since the channels must always be encoded in the order specified on the [**WAVEFORMATEXTENSIBLE**](audio.waveformatextensible) reference page, it is possible to determine the array index corresponding to an individual speaker.
...
5.  Apply the output matrix to the originating voice by using [**IXAudio2Voice::SetOutputMatrix**](ixaudio2voice-interface-setoutputmatrix.md). The originating voice will be either a source voice or a submix voice sending to either a submix voice or a mastering voice. You can get info about the originating and destination voices, like their number of channels, by using [**IXAudio2Voice::GetVoiceDetails**](ixaudio2voice-interface-getvoicedetails.md).
