...
If a given voice pair in the audio graph has no speaker positions associated with either its source or target voice (one voice has more than eight channels), neither voice is playable until the source voice has a send matrix set explicitly using the [**IXAudio2Voice::SetOutputMatrix**](https://msdn.microsoft.com/library/Ee418598(v=VS.85).aspx) method. Calling the [**IXAudio2SourceVoice::Start**](https://msdn.microsoft.com/library/Ee418471(v=VS.85).aspx) method for either voice will fail until you do this.

If the source voice and target voice have different numbers of speaker positions and [**IXAudio2Voice::SetOutputMatrix**](https://msdn.microsoft.com/library/Ee418598(v=VS.85).aspx) has not been called on the source voice, XAudio2 sends each source channel to the nearest target speaker (or speakers) available, proportionally to how close they are to the intended speaker. There are two special cases where the default behavior is different.
...
[**GetChannelMask**](https://msdn.microsoft.com/library/Hh405046(v=VS.85).aspx)
</dt> </dl>
