...
-   DSP Effects and Per Voice Filtering
    ...
    See [XAudio2 Audio Effects](xaudio2-audio-effects.md) and [**IXAudio2Voice::SetFilterParameters**](https://www.bing.com/search?q=**IXAudio2Voice::SetFilterParameters**) for more information about DSP effects and per voice filtering.
...
-   Nonblocking API Model
    ...
    With few exceptions, an XAudio2 method call will not block the audio processing engine. This means that a client can safely make a set of method calls at any time without blocking on long-running calls causing delays. The exceptions are the [**IXAudio2Voice::DestroyVoice**](https://www.bing.com/search?q=**IXAudio2Voice::DestroyVoice**) method (which may block the engine until the voice being destroyed is finished processing) and the methods that terminate the audio thread: [**IXAudio2::StopEngine**](https://www.bing.com/search?q=**IXAudio2::StopEngine**) and [**IXAudio2::Release**](https://www.bing.com/search?q=**IXAudio2::Release**). Note that while XAudio2 method calls will not block the audio processing engine, the XAudio2 methods contain critical sections and may themselves become blocked in some circumstances.
