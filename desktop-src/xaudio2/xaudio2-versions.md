...
-   Device enumeration functionality has been removed from XAudio2; the GetDeviceDetails and GetDeviceCount functions have been removed from the [**IXAudio2**](/windows/desktop/api/xaudio2/nn-xaudio2-ixaudio2) interface. Apps that want to render to other audio devices on the system must pass a device identifier string to [**CreateMasteringVoice**](https://www.bing.com/search?q=**CreateMasteringVoice**) instead of a device index. The default audio render device can still be created without enumeration.
-   [**IXAudio2MasteringVoice**](/windows/desktop/api/xaudio2/nn-xaudio2-ixaudio2masteringvoice) has an added function [**IXAudio2MasteringVoice::GetChannelMask**](https://www.bing.com/search?q=**IXAudio2MasteringVoice::GetChannelMask**) for that returns the channel mask for the destination output device.
