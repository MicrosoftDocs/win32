---
Description: 'Resubmits the specified job to the queue.'
audience: developer
author: 'REDMOND\\danlep'
manager: 'REDMOND\\timlt'
ms.assetid: '2834F4F3-A0F2-4639-B7E4-C26FAA4AB915'
...
You can specify the Requeue Job request as follows.



| Method                      | Request URI                                                                                     |
|-----------------------------|-------------------------------------------------------------------------------------------------|
| POST (before HPC Pack 2016) | https://*head\_node\_name*:*port*/WindowsHPC/*HPC\_cluster\_name*/Job/*job\_identifier*/Requeue |
| POST (HPC Pack 2016)        | https://*head\_node\_name*:*port*/WindowsHPC/Job/*job\_identifier*/Requeue                      |
...
The request XML should include an empty ArrayOfProperty element. If you include other elements as child elements of the ArrayOfProperty element, such as those you use for the Set Job Properties operation, the values of the corresponding properties do not change.
...
```XML
<string xmlns="http://schemas.microsoft.com/2003/10/Serialization/">Only jobs that are in the Canceled or Failed state can be requeued. To create a new job that is based on a Finished or Running job, save the job as an XML file (render as hpcJobXml) and then POST to http://&amp;lt;headnode&amp;gt;:&amp;lt;port&amp;gt;/WindowsHPC/&amp;lt;cluster&amp;gt;/Jobs/JobFile</string>
```
...
<HpcWebServiceFault xmlns="http://schemas.microsoft.com/HPCS2008R2/common" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">
  <Code>267386880</Code>
  <Message>Only jobs that are in the Canceled or Failed state can be requeued. To create a new job that is based on a Finished or Running job, save the job as an XML file (render as hpcJobXml) and then POST to http://&amp;lt;headnode&amp;gt;:&amp;lt;port&amp;gt;/WindowsHPC/&amp;lt;cluster&amp;gt;/Jobs/JobFile</Message>
