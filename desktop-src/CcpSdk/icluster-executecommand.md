---
Description: 'Executes a task on a specified group of nodes in the cluster.'
audience: developer
author: 'REDMOND\\danlep'
manager: 'REDMOND\\timlt'
ms.assetid: '49afec98-8c48-47b3-90ee-491f649309ff'
ms.prod: 'windows-server-dev'
ms.technology: 'compute-cluster-pack'
ms.tgt_platform: multiple
title: 'ICluster::ExecuteCommand method'
---
...
The command identifier. To get this value, call the [**ICluster::GetNewCommandId**](icluster-getnewcommandid.md) method.
...
An instance of the task runs on the nodes specified in the [**ITask::put\_RequiredNodes**](itask-put-requirednodes.md) method. If no nodes are specified, the command runs on each node in the cluster. The task does not wait in the queue to be scheduled. Standard output and standard error are captured in the result. To get the output, call the [**IExecutionResult::get\_Output**](iexecutionresult-get-output.md) method.

To cancel all instances, call the [**ICluster::EndCommand**](icluster-endcommand.md) method.

If the task generates a lot of output and there are many nodes in the cluster, you should consider using the [**ICluster::ExecuteCommandWithPaging**](icluster-executecommandwithpaging.md) method to limit the amount of memory used for the output (the amount of memory used is the size of the output multiplied by the number of nodes in the cluster).

## Examples

For an example that shows calling this method, see [Executing a Command](executing-a-command.md).
