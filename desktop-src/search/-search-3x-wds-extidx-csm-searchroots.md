...
1.  Create the **CrawlSearchManager** object and obtain its [**ISearchManager**](-search-isearchmanager.md) interface.
2.  Call [**ISearchManager::GetCatalog**](-search-isearchmanager-getcatalog.md) for "SystemIndex" to obtain an instance of an [**ISearchCatalogManager**](-search-isearchcatalogmanager.md) interface.
3.  Call [**ISearchCatalogManager::GetCrawlScopeManager**](-search-isearchcatalogmanager-getcrawlscopemanager.md) to obtain an instance of [**ISearchCrawlScopeManager**](-search-isearchcrawlscopemanager.md) interface.

After making any changes to the Crawl Scope Manager (CSM), you must call [**ISearchCrawlScopeManager::SaveAll**](-search-isearchcrawlscopemanager-saveall.md). This method takes no parameters and returns S\_OK on success.

 

## Windows 7: New Crawl Scope Manager API

In **Windows 7 and later**, [**ISearchCrawlScopeManager2**](-search-isearchcrawlscopemanager2.md) extends the functionality of the [**ISearchCrawlScopeManager**](-search-isearchcrawlscopemanager.md) interface. The [**ISearchCrawlScopeManager2::GetVersion**](-search-isearchcrawlscopemanager2-getversion.md) method gets the Crawl Scope Manager (CSM) version, which informs clients if the state of the CSM has changed. **ISearchCrawlScopeManager2::GetVersion** does not result in a cross-process call. If the function succeeds, then the pointer that is returned remains valid until the client calls **UnmapViewOfFile** on the pointer and **CloseHandle** on the returned handle.

 

## Adding Roots to the Crawl Scope

The [**ISearchCrawlScopeManager**](-search-isearchcrawlscopemanager.md) tells the search engine about containers to crawl and/or watch, and items under those containers to include or exclude. To add a new search root, instantiate an [**ISearchRoot**](-search-isearchroot.md) object, set the root attributes, and then call [**ISearchCrawlScopeManager::AddRoot**](-search-isearchcrawlscopemanager-addroot.md) and pass it a pointer to your **ISearchRoot** object. The search root URL takes the same form as the search root described earlier.

The following table describes the relevant put methods for [**ISearchRoot**](-search-isearchroot.md); the interface's other put methods are not used currently by Windows Search. We recommend including the users' security identifiers (SIDs) in all roots for better security. Per-user roots are more secure as queries are run in a per-user process, ensuring that one user cannot see items indexed from another user's inbox, for example.
...
You can use [**ISearchCrawlScopeManager**](-search-isearchcrawlscopemanager.md) to remove a root from the crawl scope when you no longer want that URL indexed. Removing a root also deletes all scope rules for that URL. For example, suppose you want to uninstall a content store and/or its protocol handler from a system. You can uninstall the application, remove all data, and then remove the search root from the crawl scope, and the Crawl Scope Manager will remove the root and all scope rules associated with the root.

To remove an existing search root, call [**ISearchCrawlScopeManager::RemoveRoot**](-search-isearchcrawlscopemanager-removeroot.md) with the search root you want to remove. The search root URL takes the same form as the search root described earlier. This method returns S\_FALSE if the root is not found and an error code if there was an error removing a root that was found.
