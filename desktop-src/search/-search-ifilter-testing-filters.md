...
The [**IFilter**](-search-ifilter.md) test suite consists of three command-line applications - [ifilttst.exe](#ifilttstexe), [filtdump.exe](#filtdumpexe), and [filtreg.exe](#filtregexe) and an initialization file, [ifilttst.ini](#ifilttstini).
...
<ul>
<li>0 - The test logs only messages concerning specific [<strong>IFilter</strong>](-search-ifilter.md) interface failures. The test dumps the chunk contents.</li>
...
The filtdump.exe program loads a filter handler for a specified document and prints the output produced by the [**IFilter**](-search-ifilter.md) DLL. The following example illustrates how to invoke the filtdump.exe program.
...
The filtreg.exe program inspects [**IFilter**](-search-ifilter.md) installation information in the registry. You invoke the filtreg.exe program from the command line by typing its name, as in the following example.
...
Filtreg.exe enumerates all file name extensions that have filter handlers associated with them by printing the file name extension and the name of the [**IFilter**](-search-ifilter.md) DLL for the extension. This is a simple way to verify the correct installation of an **IFilter**.

### ifilttst.ini

An [**IFilter**](-search-ifilter.md) interface is initialized by calling the [**IFilter::Init**](search._search_IFilter_Init) method. The **IFilter::Init** method takes the following four parameters:
...
| Entry         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|---------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Flags         | The names of the [**IFILTER\_INIT**](search._search_ifilter_init_enum) flags that are to be joined by the OR operator to form the *grfFlags* parameter of the [**IFilter::Init**](search._search_IFilter_Init) method. The flag names must all be uppercase, and on the same line.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
If the Flags and *cAttributes* entries are not specified, they default to 0. If you set *cAttributes* equal to 2, you should specify two *aAttributes* names. In the `[Test5]` section of the sample, *cAttributes* is 1, but no *aAttributes* have been specified. The test then calls the [**IFilter::Init**](search._search_IFilter_Init) method with *cAttributes* equal to 1 and *aAttributes* equal to **NULL**. This is a useful test case because it is likely to cause an access violation in the **IFilter::Init** method.

If ifilttst.exe cannot find a file named ifilttst.ini in the working directory, a default configuration is used to initialize the [**IFilter::Init**](search._search_IFilter_Init) object. The following example illustrates the default configuration.
...
-   If the chunk contains a value, [**IFilter::GetValue**](-search-ifilter-getvalue.md) returns S\_OK or FILTER\_E\_NO\_MORE\_VALUES.

### Consistency Test

The ifilttxt.exe program re-initializes the [**IFilter**](-search-ifilter.md) interface with the same parameters as in the validation test and performs a consistency test. If the **IFilter** implementation has been initialized with the [**IFILTER\_INIT**](search._search_ifilter_init_enum) IFILTER\_INIT\_INDEXING\_ONLY flag, the test releases the **IFilter** interface and re-binds it before making another call to the [**IFilter::Init**](search._search_IFilter_Init) method.
...
The ifilttst.exe program re-initializes the [**IFilter**](-search-ifilter.md) interface with the same parameters,and performs an invalid input test. This test steps through the document one chunk at a time making function calls incorrectly, such as calling the [**IFilter::GetValue**](-search-ifilter-getvalue.md) method when the current chuck contains text. The test checks all return codes for compliance with the **IFilter** specification.

The invalid input test verifies the following conditions:

-   If the current chunk contains text, [**IFilter::GetValue**](-search-ifilter-getvalue.md) returns FILTER\_E\_NO\_VALUES, and a call to [**IFilter::GetText**](-search-ifilter-gettext.md) succeeds.
-   If the current chunk contains a value, [**IFilter::GetText**](-search-ifilter-gettext.md) returns FILTER\_E\_NO\_TEXT, and a call to [**IFilter::GetValue**](-search-ifilter-getvalue.md) succeeds.
-   If the previous call to [**IFilter::GetText**](-search-ifilter-gettext.md) returned FILTER\_E\_NO\_MORE\_TEXT, successive calls to **IFilter::GetText** return FILTER\_E\_NO\_MORE\_TEXT.
-   If the previous call to [**IFilter::GetValue**](-search-ifilter-getvalue.md) returned FILTER\_E\_NO\_MORE\_VALUES, successive calls to **IFilter::GetValue** return FILTER\_E\_NO\_MORE\_VALUES.
-   If the previous call to [**IFilter::GetChunk**](-search-ifilter-getchunk.md) returned FILTER\_E\_END\_OF\_CHUNKS, successive calls to **IFilter::GetChunk** return FILTER\_E\_END\_OF\_CHUNKS.
...
The following example error message indicates that the [**IFilter**](-search-ifilter.md) emitted a chunk that was not requested.
