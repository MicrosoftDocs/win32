...
1.  [**IRowsetPrioritization**](/windows/desktop/api/Searchapi/nn-searchapi-irowsetprioritization) is acquired with [IUnknown::QueryInterface Method](http://msdn.microsoft.com/en-us/library/ms682521(VS.85).aspx) on an indexer rowset. **DBPROP\_ENABLEROWSETEVENTS** must be set to **TRUE** with the OLE DB [ICommandProperties::SetProperties](http://msdn.microsoft.com/en-us/library/ms711497(VS.85).aspx) method prior to executing the query in order to use rowset prioritization.
