...
This topic supplements the [Property System](_shell_Building_Property_Handlers) topic with information specific to Windows Search and contains the following sections:
...
After you have made these decisions, you can write formal descriptions of your custom properties so that the Windows Search engine can begin indexing your files and properties. These formal descriptions are XML files, described in [Property Description Schema](shell_propdesc_schema_entry2).
...
Your file type can support any custom properties you define, as well as a set of system-defined properties. Before you create a custom property, please review [System Properties](http://msdn.microsoft.com/en-us/library/bb763010(VS.85).aspx) to see if the property you want to support is already defined by a system property. Always be sure you support the most important system-defined properties.
...
In Windows 7 and later, there is new behavior when registering a property handler, [**IFilter**](-search-ifilter.md), or new extension. When a new property handler and/or **IFilter** is installed, files with the corresponding extensions are automatically reindexed.

In Windows 7 it is recommended that an [**IFilter**](-search-ifilter.md) is installed in conjunction with its corresponding property handlers, and that the **IFilter** is registered before the property handler. The registration of the property handler initiates immediate re-indexing of previously indexed files without first requiring a reboot, and takes advantage of any previously registered IFilter(s) for the purpose of content indexing.

If only an [**IFilter**](-search-ifilter.md) is installed, without a corresponding property handler, then automatic reindexing occurs either after a restart of the indexing service, or a reboot of the system.

For property description flags specific to Windows 7, see the following reference topics:

-   [GETPROPERTYSTOREFLAGS](http://msdn.microsoft.com/en-us/library/bb762582(VS.85).aspx)
-   [PROPDESC\_COLUMNINDEX\_TYPE](http://msdn.microsoft.com/en-us/library/bb762587(VS.85).aspx)
-   [PROPDESC\_SEARCHINFO\_FLAGS](http://msdn.microsoft.com/en-us/library/bb762588(VS.85).aspx)

### Implementation Information for Windows Vista and Earlier

Prior to Windows Vista, filters provided support for parsing and enumerating file content and properties. With the introduction of the property system, property handlers handle file properties while filters handle file content. For Windows Vista, you need develop only a partial implementation of the [**IFilter**](-search-ifilter.md)interface in coordination with a property handler, as described in [Best Practices for Creating Filter Handlers in Windows Search](-search-3x-wds-extidx-filters.md).
...
Important considerations for implementing this interface are included in the [**IPropertyStore**](properties.IPropertyStore) documentation.
...
Before creating a custom property, you should be sure there isn't a system-defined property you can use instead. You can enumerate the system-defined properties by calling [**PSEnumeratePropertyDescriptions**](properties.PSEnumeratePropertyDescriptions) or using the prop.exe command line tool.
...
[Property System](_shell_Building_Property_Handlers)
</dt> <dt>

[System Properties](http://msdn.microsoft.com/en-us/library/bb763010(VS.85).aspx)
</dt> <dt>

[Windows Search SDK Samples](http://www.microsoft.com/downloads/details.aspx?FamilyID=645300AE-5E7A-4CE7-95F0-49793F8F76E8)
</dt> </dl>
