---
Description: In Windows Vista, users can save searches as a Search Folder which is generated by an XML file that stores the query in a form that can be used by the Windows search subsystem.
...
In Windows Vista, users can save searches as a Search Folder which is generated by an XML file that stores the query in a form that can be used by the Windows search subsystem. This topic describes the file format (\*.search-ms) and includes the following sections:

-   [Overview of Saved Searches](#overview-of-saved-searches)
-   [<viewInfo&gt; Element](https://docs.microsoft.com/windows)
    -   [<viewInfo&gt; Attributes](https://docs.microsoft.com/windows)
    -   [<viewInfo&gt; Child Elements](https://docs.microsoft.com/windows)
-   [<query&gt; Element](https://docs.microsoft.com/windows)
    -   [<query&gt; Child Elements](https://docs.microsoft.com/windows)
-   [<properties&gt; Element](https://docs.microsoft.com/windows)
-   [Full Specification of the search-ms File Format](#full-specification-of-the-search-ms-file-format)
-   [Examples of Vista Saved Searches](#examples-of-vista-saved-searches)

 

## Overview of Saved Searches

Windows Vista users can save a search query as a Search Folder, a virtual folder displayed in Windows Explorer under the Searches folder. Opening a Search Folder runs the saved search and displays up-to-date results. The saved search file stores the query in a format Windows Search can act on, specifying what to search for, where to search, and how to present the results.
...
| Scope      | Identifies locations to include or exclude in the search.                                                                 | Either a path or a [known folder ID](http://msdn.microsoft.com/en-us/library/Bb761758(VS.85).aspx) of the location to be included or excluded. This element can also specify whether the search should include/exclude child paths (a shallow or deep search). |
...
## Examples of Vista Saved Searches

Windows Vista ships with a number of standard search folders. The following are examples of these Windows Vista \*.search-ms files.
