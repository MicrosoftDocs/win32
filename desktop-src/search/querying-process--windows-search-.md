...
AQS is the default query syntax used by Windows Search to query the index and to refine and narrow search parameters. AQS is primarily user facing, and can be used by users to build AQS queries, but can also be used by developers to build queries programmatically. In Windows 7, canonical AQS was introduced and must be used to programmatically generate AQS queries. In Windows 7 and later, a shortcut menu option can be available based on whether an AQS condition is met. For more information, see "Getting Dynamic Behavior for Static Verbs by Using Advanced Query Syntax" in [Creating Context Menu Handlers](https://msdn.microsoft.com/library/Cc144171(VS.85).aspx). AQS queries can be limited to specific types of files, which are known as file kinds. For more information, see [File Types and Associations](https://msdn.microsoft.com/library/cc144104(VS.85).aspx). For reference documentation on the relevant properties, see [System.Kind](https://msdn.microsoft.com/library/Bb787521(v=VS.85).aspx), and [System.KindText](https://msdn.microsoft.com/library/Bb787523(v=VS.85).aspx).
