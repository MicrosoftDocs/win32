...
The stemmer uses the [IWordFormSink](iwordformsink.md) object to gather the list of alternative words. [**IWordFormSink::PutWord**](iwordformsink-putword.md) generates the final word from the stemmer. In all cases, this final word is the same as the input word from [**IStemmer::GenerateWordForms**](-search-istemmer-generatewordforms.md). For example, given the word "swim," the stemmer generates the following word forms: "swimming," "swimmer," "swims," "swam," and "swum," through calls to [**IWordFormSink::PutAltWord**](iwordformsink-putphrase.md). The stemmer generates "swim" through **IWordFormSink::PutWord**.
