...
|-----------------|--------|------------------------|-----------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Input Assembler | In     | Vertex Buffer          | Buffer                                  | [**IASetVertexBuffers**](/windows/win32/D3D10/nf-d3d10-id3d10device-iasetvertexbuffers?branch=master)                                                                                                                                           |
| Input Assembler | In     | Index Buffer           | Buffer                                  | [**IASetIndexBuffer**](/windows/win32/D3D10/nf-d3d10-id3d10device-iasetindexbuffer?branch=master)                                                                                                                                               |
| Shader Stages   | In     | Shader-ResourceView    | Buffer, Texture1D, Texture2D, Texture3D | [**VSSetShaderResources**](/windows/win32/D3D10/nf-d3d10-id3d10device-vssetshaderresources?branch=master), [**GSSetShaderResources**](/windows/win32/D3D10/nf-d3d10-id3d10device-gssetshaderresources?branch=master), [**PSSetShaderResources**](/windows/win32/D3D10/nf-d3d10-id3d10device-pssetshaderresources?branch=master) |
| Shader Stages   | In     | Shader-Constant Buffer | Buffer                                  | [**VSSetConstantBuffers**](/windows/win32/D3D10/nf-d3d10-id3d10device-vssetconstantbuffers?branch=master), [**GSSetConstantBuffers**](/windows/win32/D3D10/nf-d3d10-id3d10device-gssetconstantbuffers?branch=master), [**PSSetConstantBuffers**](/windows/win32/D3D10/nf-d3d10-id3d10device-pssetconstantbuffers?branch=master) |
| Stream Output   | Out    | Buffer                 | Buffer                                  | [**SOSetTargets**](/windows/win32/D3D10/nf-d3d10-id3d10device-sosettargets?branch=master)                                                                                                                                                       |
| Output Merger   | Out    | Render-target View     | Buffer, Texture1D, Texture2D, Texture3D | [**OMSetRenderTargets**](/windows/win32/D3D10/nf-d3d10-id3d10device-omsetrendertargets?branch=master)                                                                                                                                           |
| Output Merger   | Out    | Depth/Stencil View     | Texture1D, Texture2D                    | [**OMSetRenderTargets**](/windows/win32/D3D10/nf-d3d10-id3d10device-omsetrendertargets?branch=master)                                                                                                                                           |
...
The hardware that your application is running on will have a minimum of at least one CPU and one GPU. To pick a usage value, consider which type of processor needs to read or write to the resource from the following options (see [**D3D10\_USAGE**](/windows/win32/D3D10/ne-d3d10-d3d10_usage?branch=master)).
...
A resource can be bound to more than one pipeline stage at the same time as long as the restrictions specified when the resource was created ([**usage flags**](/windows/win32/D3D10/ne-d3d10-d3d10_usage?branch=master), [**bind flags**](/windows/win32/D3D10/ne-d3d10-d3d10_bind_flag?branch=master), [**cpu access flags**](/windows/win32/D3D10/ne-d3d10-d3d10_cpu_access_flag?branch=master)) are met. More specifically, a resource can be bound as an input and an output simultaneously as long as reading and writing part of a resource cannot happen at the same time.
...
For example, consider the case of a render target used as a texture multiple times. It may be faster to have two resources: a render target and a texture used as a shader resource. Each resource would use only one bind flag ([**D3D10\_BIND\_RENDER\_TARGET**](/windows/win32/D3D10/ne-d3d10-d3d10_bind_flag?branch=master) or **D3D10\_BIND\_SHADER\_RESOURCE**). The data would be copied from the render-target texture to the shader texture using [**CopyResource**](/windows/win32/D3D10/nf-d3d10-id3d10device-copyresource?branch=master) or [**CopySubresourceRegion**](/windows/win32/D3D10/nf-d3d10-id3d10device-copysubresourceregion?branch=master). This may improve performance by isolating the render-target write from the shader-texture read. Of course, the only way to be sure is to implement both approaches and measure the performance difference in your particular application.
