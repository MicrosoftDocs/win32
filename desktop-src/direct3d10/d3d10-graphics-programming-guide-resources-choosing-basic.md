...
|-----------------|--------|------------------------|-----------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Input Assembler | In     | Vertex Buffer          | Buffer                                  | [**IASetVertexBuffers**](id3d10device-iasetvertexbuffers.md)                                                                                                                                           |
| Input Assembler | In     | Index Buffer           | Buffer                                  | [**IASetIndexBuffer**](id3d10device-iasetindexbuffer.md)                                                                                                                                               |
| Shader Stages   | In     | Shader-ResourceView    | Buffer, Texture1D, Texture2D, Texture3D | [**VSSetShaderResources**](id3d10device-vssetshaderresources.md), [**GSSetShaderResources**](id3d10device-gssetshaderresources.md), [**PSSetShaderResources**](id3d10device-pssetshaderresources.md) |
| Shader Stages   | In     | Shader-Constant Buffer | Buffer                                  | [**VSSetConstantBuffers**](id3d10device-vssetconstantbuffers.md), [**GSSetConstantBuffers**](id3d10device-gssetconstantbuffers.md), [**PSSetConstantBuffers**](id3d10device-pssetconstantbuffers.md) |
| Stream Output   | Out    | Buffer                 | Buffer                                  | [**SOSetTargets**](id3d10device-sosettargets.md)                                                                                                                                                       |
| Output Merger   | Out    | Render-target View     | Buffer, Texture1D, Texture2D, Texture3D | [**OMSetRenderTargets**](id3d10device-omsetrendertargets.md)                                                                                                                                           |
| Output Merger   | Out    | Depth/Stencil View     | Texture1D, Texture2D                    | [**OMSetRenderTargets**](id3d10device-omsetrendertargets.md)                                                                                                                                           |
...
The hardware that your application is running on will have a minimum of at least one CPU and one GPU. To pick a usage value, consider which type of processor needs to read or write to the resource from the following options (see [**D3D10\_USAGE**](d3d10-usage.md)).
...
A resource can be bound to more than one pipeline stage at the same time as long as the restrictions specified when the resource was created ([**usage flags**](d3d10-usage.md), [**bind flags**](d3d10-bind-flag.md), [**cpu access flags**](d3d10-cpu-access-flag.md)) are met. More specifically, a resource can be bound as an input and an output simultaneously as long as reading and writing part of a resource cannot happen at the same time.
...
For example, consider the case of a render target used as a texture multiple times. It may be faster to have two resources: a render target and a texture used as a shader resource. Each resource would use only one bind flag ([**D3D10\_BIND\_RENDER\_TARGET**](d3d10-bind-flag.md) or **D3D10\_BIND\_SHADER\_RESOURCE**). The data would be copied from the render-target texture to the shader texture using [**CopyResource**](id3d10device-copyresource.md) or [**CopySubresourceRegion**](id3d10device-copysubresourceregion.md). This may improve performance by isolating the render-target write from the shader-texture read. Of course, the only way to be sure is to implement both approaches and measure the performance difference in your particular application.
