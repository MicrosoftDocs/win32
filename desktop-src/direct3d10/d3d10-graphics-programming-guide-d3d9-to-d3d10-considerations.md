...
Although the info in this topic compares Direct3D 9 with Direct3D 10, because Direct3D 11 builds on the improvements made in Direct3D 10 and 10.1, you also need this info to migrate from Direct3D 9 to Direct3D 11. For info about moving beyond Direct3D 10 to Direct3D 11, see [Migrating to Direct3D 11](direct3d11.d3d11_programming_guide_migrating).
...
This documentation contains a white paper containing replacement shader sources for the most common FF behaviors (see [Fixed Function EMU Sample](af769594-3ecd-52b1-337e-8a411e29b8ab)). Some fixed-function pixel behavior including alpha test has been moved into shaders.
...
[Tutorial 1: Direct3D 10 Basics](27d7b68d-1c99-041e-6aa3-b7b6da82da19)

> [!Note]  
> Direct3D 10 and later versions of Direct3D extend the DDS file format to support new DXGI formats, texture arrays, and cube-map arrays. For more information about the DDS file format extension, see the [Programming Guide for DDS](direct3ddds.dx_graphics_dds_pguide).
...
[CubeMap Sample](7bb33415-9ed7-1a13-637b-df3669cae2fa)
...
[HLSL without Effects sample](61f892cf-4760-177b-5048-0ba3906d8bfc)
...
For details, compare the Direct3D 9 [**D3DXIMAGE\_FILEFORMAT**](direct3d9.d3dximage_fileformat) enumerations with the [**D3DX10\_IMAGE\_FILE\_FORMAT**](d3dx10-image-file-format.md) enumerations for Direct3D 10.
...
Starting with Windows 8, you can use the **clipplanes** function attribute in an HLSL [function declaration](direct3dhlsl.dx_graphics_hlsl_function_syntax) rather than [SV\_ClipDistance](direct3dhlsl.dx_graphics_hlsl_semantics#system-value) to make your shader work on [feature level](direct3d11.overviews_direct3d_11_devices_downlevel_intro#overview) 9\_x as well as feature level 10 and higher. For more info, see [User clip planes on feature level 9 hardware](direct3dhlsl.user_clip_planes_on_10level9).

## Additional Direct3D 10 Differences to Watch For

### Integers as Input

In Direct3D 9, there was no real hardware support for integer data types, however Direct3D 10 hardware supports explicit integer types. If you have floating-point data in your vertex buffer, then you must have a float input. Otherwise an integer type will be the bit pattern representation of the floating-point value. An integer type is not allowed for a pixel shader input unless the value is marked for no interpolation (see [Interpolation Modifiers](direct3dhlsl.dx_graphics_hlsl_struct)).

### Mouse Cursors

On previous versions of Windows, the standard GDI mouse cursor routines did not operate correctly on all full-screen exclusive devices. The [**SetCursorProperties**](direct3d9.idirect3ddevice9__setcursorproperties), [**ShowCursor**](direct3d9.idirect3ddevice9__showcursor), and [**SetCursorPosition**](direct3d9.idirect3ddevice9__setcursorposition) APIs were added to handle these cases. Since Windows Vista's version of GDI fully understands [DXGI](direct3ddxgi.d3d10_graphics_programming_guide_dxgi) surfaces, there is no need for this specialized mouse cursor API so there is no Direct3D 10 equivalent. Direct3D 10 applications should instead use the standard [GDI mouse cursor routines](http://msdn2.microsoft.com/en-us/library/ms646970.aspx) for mouse cursors.

### Mapping Texels to Pixels in Direct3D 10

In Direct3D 9, texel centers and pixel centers were a half unit apart (see [Directly Mapping Texels to Pixels (Direct3D 9)](direct3d9.directly_mapping_texels_to_pixels)). In Direct3D 10, texel centers are already at half-units, so there is no need to shift vertex coordinates at all.
...
The functionality of the Direct3D 9 API [**TestCooperativeLevel**](direct3d9.idirect3ddevice9__testcooperativelevel) is analogous to setting the [DXGI\_PRESENT\_TEST](direct3ddxgi.dxgi_present) when calling [**Present**](direct3ddxgi.idxgiswapchain_present).
