...
The format the texture will be saved as (see [**D3DX10\_IMAGE\_FILE\_FORMAT**](d3dx10-image-file-format.md)). D3DX10\_IFF\_DDS is the preferred format since it is the only option that supports all the formats in [**DXGI\_FORMAT**](https://msdn.microsoft.com/en-us/library/Bb173059(v=VS.85).aspx).
...
**D3DX10SaveTextureToFile** writes out the extra [**DDS\_HEADER\_DXT10**](https://msdn.microsoft.com/en-us/library/Bb943983(v=VS.85).aspx) structure for the input texture only if necessary (for example, because the input texture is in standard RGB (sRGB) format). If **D3DX10SaveTextureToFile** writes out the **DDS\_HEADER\_DXT10** structure, it sets the **dwFourCC** member of the [**DDS\_PIXELFORMAT**](https://msdn.microsoft.com/en-us/library/Bb943984(v=VS.85).aspx) structure for the texture to **DX10** to indicate the prescense of the **DDS\_HEADER\_DXT10** extended header.
