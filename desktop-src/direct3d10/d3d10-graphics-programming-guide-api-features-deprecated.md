...
-   **Clip planes**. Instead of using clip planes, Direct3D 10 implements clip distances and cull distances, with up to 8 components each in up to 2 elements of vertex attributes. See [Semantics (DirectX HLSL)](direct3dhlsl.dx_graphics_hlsl_semantics) for additional information. [FixedFuncEMU sample](af769594-3ecd-52b1-337e-8a411e29b8ab) provides an example of emulating clip planes in Direct3D 10.
...
-   **Pixel and vertex shader models**: 1\_x, 2\_x, and 3\_0. Direct3D 10 supports shader model 4. See [Shader Model 4](direct3dhlsl.dx_graphics_hlsl_sm4) for additional information.
-   **Point sprites**. Use a geometry shader instead. See [Shader Stages (Direct3D 10)](direct3d10.d3d10_graphics_programming_guide_shader_stages) for additional information.
-   **Rasterization rules**. Legacy GDI line rasterization rules are replaced with cleaner, simpler rules. The last-pixel control for lines is no longer supported. See [Rasterization Rules (Direct3D 10)](direct3d10.d3d10_graphics_programming_guide_rasterizer_stage_rules) for additional information.
-   **Shade modes**. D3DSHADEMODE (which support flat/gouraud/phong shading) has been removed. Direct3D 10 implements two interpolations modifiers for vertex shader outputs instead. See [FixedFuncEMU sample](af769594-3ecd-52b1-337e-8a411e29b8ab) for an example of emulating Direct3D 9 gouraud and flat shade modes in Direct3D 10.
-   **texldp** instruction. An application must implement a projected-texture load with extra HLSL statements. See [Reference for HLSL](direct3dhlsl.dx_graphics_hlsl_reference) for additional information. [FixedFuncEMU sample](af769594-3ecd-52b1-337e-8a411e29b8ab) provides an example of emulating texldp in Direct3D 10.
