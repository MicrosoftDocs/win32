...
-   [Input-Assembler Stage](https://msdn.microsoft.com/library/Bb205116(v=VS.85).aspx) - The input-assembler stage is responsible for supplying data (triangles, lines and points) to the pipeline.
...
-   [Stream-Output Stage](https://msdn.microsoft.com/library/Bb205121(v=VS.85).aspx) - The stream-output stage is designed for streaming primitive data from the pipeline to memory on its way to the rasterizer. Data can be streamed out and/or passed into the rasterizer. Data streamed out to memory can be recirculated back into the pipeline as input data or read-back from the CPU.
-   [Rasterizer Stage](https://msdn.microsoft.com/library/Bb205125(v=VS.85).aspx) - The rasterizer is responsible for clipping primitives, preparing primitives for the pixel shader and determining how to invoke pixel shaders.
