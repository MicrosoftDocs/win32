...
-   [Input-Assembler Stage](direct3d11.d3d10_graphics_programming_guide_input_assembler_stage) - The input-assembler stage is responsible for supplying data (triangles, lines and points) to the pipeline.
...
-   [Stream-Output Stage](direct3d11.d3d10_graphics_programming_guide_output_stream_stage) - The stream-output stage is designed for streaming primitive data from the pipeline to memory on its way to the rasterizer. Data can be streamed out and/or passed into the rasterizer. Data streamed out to memory can be recirculated back into the pipeline as input data or read-back from the CPU.
-   [Rasterizer Stage](direct3d11.d3d10_graphics_programming_guide_rasterizer_stage) - The rasterizer is responsible for clipping primitives, preparing primitives for the pixel shader and determining how to invoke pixel shaders.
