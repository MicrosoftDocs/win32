...
> [!Note]  
> When the application calls the Map method, it receives a pointer to the resource data to access. The runtime ensures that the pointer has a specific alignment, depending on [feature level](https://msdn.microsoft.com/library/Ff476876(v=VS.85).aspx). For [**D3D\_FEATURE\_LEVEL\_10\_0**](https://msdn.microsoft.com/library/Ff476329(v=VS.85).aspx) and higher, the pointer is aligned to 16 bytes. For lower than [**D3D\_FEATURE\_LEVEL\_10\_0**](https://msdn.microsoft.com/library/Ff476329(v=VS.85).aspx), the pointer is aligned to 4 bytes. The 16-byte alignment allows the application to perform [SSE](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2010/t467de55(v=vs.100))-optimized operations on the data natively, without realignment or copy.
...
1.  [**Present**](https://msdn.microsoft.com/library/Bb174576(v=VS.85).aspx) is called.
