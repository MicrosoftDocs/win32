...
Applications often define keyboard shortcuts, such as CTRL+O for the File Open command. You could implement keyboard shortcuts by handling individual [**WM\_KEYDOWN**](https://msdn.microsoft.com/library/windows/desktop/ms646280) messages, but accelerator tables provide a better solution that:
...
The resource for the accelerator table must be loaded before the program can use it. To load an accelerator table, call the [**LoadAccelerators**](https://msdn.microsoft.com/library/windows/desktop/ms646370) function.
...
Call this function before you enter the message loop. The first parameter is the handle to the module. (This parameter is passed to your [**WinMain**](https://msdn.microsoft.com/library/windows/desktop/ms633559) function. For details, see [WinMain: The Application Entry Point](winmain--the-application-entry-point.md).) The second parameter is the resource identifier. The function returns a handle to the resource. Recall that a handle is an opaque type that refers to an object managed by the system. If the function fails, it returns **NULL**.

You can release an accelerator table by calling [**DestroyAcceleratorTable**](https://msdn.microsoft.com/library/windows/desktop/ms646368). However, the system automatically releases the table when the program exits, so you only need to call this function if you are replacing one table with another. There is an interesting example of this in the topic [Creating User Editable Accelerators](https://msdn.microsoft.com/library/windows/desktop/ms646337#editable-acc).

## Translating Key Strokes into Commands

An accelerator table works by translating key strokes into [**WM\_COMMAND**](https://msdn.microsoft.com/library/windows/desktop/ms647591) messages. The *wParam* parameter of **WM\_COMMAND** contains the numeric identifier of the command. For example, using the table shown previously, the key stroke CTRL+M is translated into a **WM\_COMMAND** message with the value `ID_TOGGLE_MODE`. To make this happen, change your message loop to the following:
...
This code adds a call to the [**TranslateAccelerator**](https://msdn.microsoft.com/library/windows/desktop/ms646373) function inside the message loop. The **TranslateAccelerator** function examines each window message, looking for key-down messages. If the user presses one of the key combinations listed in the accelerator table, **TranslateAccelerator** sends a [**WM\_COMMAND**](https://msdn.microsoft.com/library/windows/desktop/ms647591) message to the window. The function sends **WM\_COMMAND** by directly invoking the window procedure. When **TranslateAccelerator** successfully translates a key stroke, the function returns a non-zero value, which means you should skip the normal processing for the message. Otherwise, **TranslateAccelerator** returns zero. In that case, pass the window message to [**TranslateMessage**](https://msdn.microsoft.com/library/windows/desktop/ms644955) and [**DispatchMessage**](https://msdn.microsoft.com/library/windows/desktop/ms644934), as normal.

Here is how the drawing program might handle the [**WM\_COMMAND**](https://msdn.microsoft.com/library/windows/desktop/ms647591) message:
