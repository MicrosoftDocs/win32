...
[**ISyncMgrSyncItem**](/windows/win32/Syncmgr/nn-syncmgr-isyncmgrsyncitem?branch=master) and [**ISyncMgrSyncItemInfo**](/windows/win32/Syncmgr/nn-syncmgr-isyncmgrsynciteminfo?branch=master) are used to describe a single sync item involved in the synchronization to the Sync Center. A sync item generally represents either a particular data type (such as images) or a particular location of data.
...
If a handler supports more than one data type or multiple data locations, then it needs to support more than one sync item object. An example might be a personal data assistant (PDA) that allows the user to sync contacts, calendar items and documents. These three data types would need to be represented by three unique objects that each expose the [**ISyncMgrSyncItem**](/windows/win32/Syncmgr/nn-syncmgr-isyncmgrsyncitem?branch=master) and [**ISyncMgrSyncItemInfo**](/windows/win32/Syncmgr/nn-syncmgr-isyncmgrsynciteminfo?branch=master) interfaces.

The [**IEnumSyncMgrSyncItems**](/windows/win32/Syncmgr/nn-syncmgr-ienumsyncmgrsyncitems?branch=master) interface provides a mechanism to enumerate through a handler's sync items. To retrieve this enumerator, Sync Center calls the [**ISyncMgrSyncItemContainer::GetSyncItemEnumerator**](/windows/win32/Syncmgr/nf-syncmgr-isyncmgrsyncitemcontainer-getsyncitemenumerator?branch=master) method exposed by the handler. [**ISyncMgrSyncItemContainer**](/windows/win32/Syncmgr/nn-syncmgr-isyncmgrsyncitemcontainer?branch=master) also contains two other methods that the Sync Center can use to retrieve information about the handler's sync items:

-   [**GetSyncItem**](/windows/win32/Syncmgr/nf-syncmgr-isyncmgrsyncitemcontainer-getsyncitem?branch=master) returns a specific sync item.
-   [**GetSyncItemCount**](/windows/win32/Syncmgr/nf-syncmgr-isyncmgrsyncitemcontainer-getsyncitemcount?branch=master) returns the number of sync items supported by the handler.

[**ISyncMgrHandler**](/windows/win32/Syncmgr/nn-syncmgr-isyncmgrhandler?branch=master) and [**ISyncMgrHandlerInfo**](/windows/win32/Syncmgr/nn-syncmgr-isyncmgrhandlerinfo?branch=master) are used to describe the properties of the hander and launch the actual synchronization. [**ISyncMgrHandler::Synchronize**](/windows/win32/Syncmgr/nf-syncmgr-isyncmgrhandler-synchronize?branch=master) is where the handler code carries out the synchronization and provides feedback on progress and any issues that occur.
