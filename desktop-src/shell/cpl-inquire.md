...
The address of a [**CPLINFO**](/windows/win32/Cpl/ns-cpl-tagcplinfo?branch=master) structure. The application must fill this structure with resource identifiers for the icon, short name, description, and any user-defined value associated with the dialog box.
...
The [**CPL\_NEWINQUIRE**](cpl-newinquire.md) message returns information in a form that the system cannot cache. For this reason, most [**CPlApplet**](/windows/win32/Cpl/nc-cpl-applet_proc?branch=master) functions should process **CPL\_INQUIRE** and ignore **CPL\_NEWINQUIRE**.
