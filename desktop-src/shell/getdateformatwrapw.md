...
\[**GetDateFormatWrapW** is available for use in Windows XP. It will not be available in subsequent versions. You should use [**GetDateFormatW**](intl.getdateformat) in its place.\]
...
> [!Note]  
> **GetDateFormatWrapW** is a wrapper for the **GetDateFormatW** function. See the [**GetDateFormat**](intl.getdateformat) page for further usage notes.
...
This parameter can be a locale identifier created by the [**MAKELCID**](intl.makelcid) macro, or one of the following predefined values.
...
Type: **const [**SYSTEMTIME**](base.systemtime_str)\***

A pointer to a [**SYSTEMTIME**](base.systemtime_str) structure that contains the date information to be formatted. If this pointer is **NULL**, the function uses the current local system date.
...
A pointer to a format picture to use to form the date string. If *pwzFormat* is **NULL**, the function uses the date format of the specified locale. See [**GetDateFormat**](intl.getdateformat) for more details.
...
**GetDateFormatWrapW** provides the ability to use Unicode strings in operating systems earlier than Windows XP. The preferred method is to use [**GetDateFormatW**](intl.getdateformat) in conjunction with the Microsoft Layer for Unicode (MSLU).
...
[**GetDateFormat**](intl.getdateformat)
</dt> </dl>
