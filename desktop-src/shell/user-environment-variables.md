...
To retrieve a copy of the environment block for a given user, use the [**CreateEnvironmentBlock**](/windows/win32/Userenv/nf-userenv-createenvironmentblock?branch=master) function. To free an environment block created by **CreateEnvironmentBlock**, use the [**DestroyEnvironmentBlock**](/windows/win32/Userenv/nf-userenv-destroyenvironmentblock?branch=master) function. These functions reference a pointer to an environment block. The environment block is an array of null-terminated Unicode strings. The list ends with two nulls (\\0\\0).

To expand a string that contains environment variables by using the environment block for a specified user, use the [**ExpandEnvironmentStringsForUser**](/windows/win32/Userenv/nf-userenv-expandenvironmentstringsforusera?branch=master) function.
