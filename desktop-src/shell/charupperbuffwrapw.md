...
\[**CharUpperBuffWrapW** is available for use in Windows XP. It may not be available in subsequent versions. You should use [**CharUpperBuffW**](menurc.charupperbuff) in its place.\]

Converts lowercase characters in a buffer to uppercase characters. The function converts the characters in place.

> [!Note]  
> **CharUpperBuffWrapW** is a wrapper for the **CharUpperBuffW** function. See the [**CharUpperBuff**](menurc.charupperbuff) page for further usage notes.
...
The preferred method is to use [**CharUpperBuffW**](menurc.charupperbuff) in conjunction with the Microsoft Layer for Unicode (MSLU).
...
[**CharUpperBuff**](menurc.charupperbuff)
</dt> </dl>
