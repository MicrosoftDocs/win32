...
\[**CharLowerWrapW** is available for use in Windows XP. It may not be available in subsequent versions. You should use [**CharLowerW**](menurc.charlower) in its place.\]
...
> [!Note]  
> **CharLowerWrapW** is a wrapper for the **CharLowerW** function. See the [**CharLower**](menurc.charlower) page for further usage notes.
...
There is no indication of success or failure. Failure is rare. There is no extended error information for this function; do not call [**GetLastError**](base.getlasterror).

## Remarks

The preferred method is to use [**CharLowerW**](menurc.charlower) in conjunction with the Microsoft Layer for Unicode (MSLU).
