...
Set to **true** to have the service started by the service control manager when [**ServiceStart**](shell.Shell_ServiceStart) is called. To leave the service configuration unchanged, set *vPersistent* to **false**.
...
The method returns **false** if the service has already been stopped. Before calling this method, you can call [**Shell.IsServiceRunning**](shell.Shell_IsServiceRunning) to ascertain the status of the service.
