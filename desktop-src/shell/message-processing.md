...
The [**CPlApplet**](/windows/desktop/api/Cpl/nc-cpl-applet_proc) callback function processes all messages sent to a Control Panel item by Windows. Messages sent to the function are in a specific order. By the same token, the .cpl item requires the messages to be processed in a specific way.

First, the [**CPlApplet**](/windows/desktop/api/Cpl/nc-cpl-applet_proc) function receives the CPL\_INIT message when Windows first loads the Control Panel item. The function should carry out any initialization, such as allocating memory, and return nonzero. If **CPlApplet** cannot complete the initialization, it must return zero, directing Windows to terminate communication and release the DLL.
