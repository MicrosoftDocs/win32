...
\[**CompareStringWrapW** is available for use in Windows XP. It will not be available in subsequent versions. You should use [**CompareStringW**](intl.comparestring) in its place.\]

Compares two Unicode character strings, using a specified locale.

> [!Note]  
> **CompareStringWrapW** is a wrapper for the **CompareStringW** function. See the [**CompareString**](intl.comparestring) page for further usage notes.
...
A locale identifier used for the comparison. This parameter can be one of the following predefined locale identifiers or a locale identifier created by the [**MAKELCID**](intl.makelcid) macro.
...
If the function fails, the return value is zero. To get extended error information, call [**GetLastError**](base.getlasterror). **GetLastError** may return one of the following error codes.
...
**Security Warning:** Using this function incorrectly can compromise the security of your application. Strings that are not compared correctly can produce invalid input. Test strings to make sure they are valid before using them and provide error handlers. For more information, see [Security Considerations: International Features](intl.security_considerations__international_features)

The preferred method is to use [**CompareStringW**](intl.comparestring) in conjunction with the Microsoft Layer for Unicode (MSLU).
...
[**CompareString**](intl.comparestring)
</dt> </dl>
