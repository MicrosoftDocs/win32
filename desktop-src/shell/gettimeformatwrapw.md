...
\[**GetTimeFormatWrapW** is available for use in Windows XP. It may not be available in subsequent versions. You should use [**GetTimeFormatW**](intl.gettimeformat) in its place.\]
...
> [!Note]  
> **GetTimeFormatWrapW** is a wrapper for the **GetTimeFormatW** function. See the [**GetTimeFormat**](intl.gettimeformat) page for further usage notes.
...
A pointer to a [**SYSTEMTIME**](base.systemtime_str) structure that contains the time information to be formatted. If this pointer is **NULL**, the function uses the current local system time.
...
A pointer to a format to use to form the time string. If *pwzFormat* is **NULL**, the function uses the time format of the specified locale. See [**GetTimeFormat**](intl.gettimeformat) for more details.
...
**GetTimeFormatWrapW** provides the ability to use Unicode strings in operating systems earlier than Windows XP. The preferred method is to use [**GetTimeFormatW**](intl.gettimeformat) in conjunction with the Microsoft Layer for Unicode (MSLU).
...
[**GetTimeFormat**](intl.gettimeformat)
</dt> </dl>
