...
-   QueryForOverlay. Set this value to indicate that the root folder's icon can have an icon overlay. The folder object must support the [**IShellIconOverlay**](/windows/desktop/api/Shlobj_core/) interface. Before Windows Explorer displays the root folder's icon, it will request an overlay icon by calling one of the two **IShellIconOverlay** methods with *pidlItem* set to an empty PIDL.
...
The [**IShellFolder::CreateViewObject**](/windows/desktop/api/shobjidl_core/nf-shobjidl_core-ishellfolder-createviewobject) method is also called to request one of the optional interfaces, such as [**IContextMenu**](/windows/desktop/api/Shobjidl/nn-shobjidl_core-icontextmenu), for the folder itself. Your implementation of this method should create an object that exposes the requested interface and returns the interface pointer. If Windows Explorer needs an optional interface for one of the objects contained by the folder, it will call [**IShellFolder::GetUIObjectOf**](/windows/desktop/api/shobjidl_core/nf-shobjidl_core-ishellfolder-getuiobjectof).
...
> [!Note]  
> At present, Windows Explorer calls this method only to request an [**IShellFolder**](/windows/desktop/api/Shobjidl/nn-shobjidl_core-ishellfolder) interface. However, do not assume that this will always be the case. You should always check the value of *riid* before proceeding.
...
When a user right-clicks an object, Windows Explorer requests an [**IContextMenu**](/windows/desktop/api/Shobjidl/nn-shobjidl_core-icontextmenu) interface. To provide shortcut menus for your objects, create a menu handler object and return its **IContextMenu** interface.

The procedures for creating a menu handler object are very similar to those used to create a menu handler Shell extension. For details, see [Creating Context Menu Handlers](context-menu-handlers.md) or the [**IContextMenu**](/windows/desktop/api/Shobjidl/nn-shobjidl_core-icontextmenu), [**IContextMenu2**](/windows/desktop/api/shobjidl_core/nn-shobjidl_core-icontextmenu2), or [**IContextMenu3**](/windows/desktop/api/shobjidl_core/nn-shobjidl_core-icontextmenu3) reference.

### IQueryInfo

Windows Explorer calls the [**IQueryInfo**](/windows/desktop/api/Shlobj_core/) interface to retrieve an infotip text string.
