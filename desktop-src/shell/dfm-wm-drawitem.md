...
A pointer to a [**DRAWITEMSTRUCT**](controls.DRAWITEMSTRUCT) structure containing information about the item to be drawn and the type of drawing required.
...
The **itemAction** member of the [**DRAWITEMSTRUCT**](controls.DRAWITEMSTRUCT) structure specifies the drawing operation that an application should perform.

Before returning from processing this message, an application should ensure that the device context identified by the **hDC** member of the [**DRAWITEMSTRUCT**](controls.DRAWITEMSTRUCT) structure is in the default state.
