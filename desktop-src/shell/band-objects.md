...
A *tool band* is a band object that was introduced with Microsoft Internet Explorer 5 to support the Windows radio toolbar feature. The Internet Explorer toolbar is actually a [rebar control](_win32_Rebar_Controls) that contains several [toolbar controls](controls.Toolbar_Control_Reference). By creating a tool band, you can add a band to that rebar control. However, like Explorer Bars, a tool band is a general purpose window.
...
-   DBID\_SHOWONLY
    ...
    | Value | Description                                                                                                 |
    |-------|-------------------------------------------------------------------------------------------------------------|
    | pUnk  | A pointer to the band object's [**IUnknown**](com.iunknown) interface. All other desk bands will be hidden. |
...
When the user selects an Explorer Bar, the container calls the corresponding band object's [**IObjectWithSite::SetSite**](com.iobjectwithsite_setsite) method. The *punkSite* parameter will be set to the site's [**IUnknown**](com.iunknown) pointer.

In general, a [**SetSite**](com.iobjectwithsite_setsite) implementation should perform the following steps:

1.  Release any site pointer that is currently being held.
2.  If the pointer passed to [**SetSite**](com.iobjectwithsite_setsite) is set to **NULL**, the band is being removed. **SetSite** can return S\_OK.
3.  If the pointer passed to [**SetSite**](com.iobjectwithsite_setsite) is non-**NULL**, a new site is being set. **SetSite** should do the following:
...
The sample's [**GetSite**](com.iobjectwithsite_getsite) implementation simply wraps a call to the site's [**QueryInterface**](com.iunknown_queryinterface) method, using the site pointer saved by [**SetSite**](com.iobjectwithsite_setsite).
...
Internet Explorer will call the Explorer Bar's [**IPersistStream**](com.ipersiststream) interface to allow the Explorer Bar to load or save persistent data. If there is no persistent data, the methods must still return a success code. The **IPersistStream** interface inherits from [**IPersist**](com.ipersist), so five methods must be implemented.

-   [**IPersist::GetClassID**](com.ipersist_getclassid)
-   [**IPersistStream::IsDirty**](com.ipersiststream_isdirty)
-   [**IPersistStream::Load**](com.ipersiststream_load)
-   [**IPersistStream::Save**](com.ipersiststream_save)
-   [**IPersistStream::GetSizeMax**](com.ipersiststream_getsizemax)

The Explorer Bar sample does not use any persistent data and has only a minimal implementation of [**IPersistStream**](com.ipersiststream). [**IPersist::GetClassID**](com.ipersist_getclassid) returns the object's CLSID (CLSID\_SampleExplorerBar), and the remainder return either S\_OK, S\_FALSE, or E\_NOTIMPL.
...
-   [**WM\_NCCREATE**](winmsg.wm_nccreate)
-   [**WM\_PAINT**](gdi.wm_paint)
-   [**WM\_COMMAND**](menurc.wm_command)
-   [**WM\_SETFOCUS**](inputdev.wm_setfocus)
-   [**WM\_KILLFOCUS**](inputdev.wm_killfocus)
