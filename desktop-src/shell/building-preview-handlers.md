...
-   [IInitializeWithStream::Initialize](#iinitializewithstreaminitialize) (strongly preferred), [**IInitializeWithFile**](/windows/desktop/api/Propsys/nn-propsys-iinitializewithfile), or [**IInitializeWithItem**](https://msdn.microsoft.com/en-us/library/Bb761814(v=VS.85).aspx)
...
Store the [**IUnknown**](https://msdn.microsoft.com/en-us/library/ms680509(v=VS.85).aspx) pointer for later access.

If you currently have a reference to an [**IPreviewHandlerFrame**](https://msdn.microsoft.com/en-us/library/Bb775309(v=VS.85).aspx) object, release it. Use the stored [**IUnknown**](https://msdn.microsoft.com/en-us/library/ms680509(v=VS.85).aspx) pointer to call [**QueryInterface**](https://msdn.microsoft.com/en-us/library/ms682521(v=VS.85).aspx) on the site for a new **IPreviewHandlerFrame** reference.
...
Once you have a window, load the data from the [**IStream**](https://msdn.microsoft.com/en-us/library/Aa380034(v=VS.85).aspx) that the preview handler was initialized with, and render that data to your preview handler's window.
...
This method should call the [**GetFocus**](https://msdn.microsoft.com/en-us/library/ms646294(v=VS.85).aspx) function and return the result of that call in the *phwnd* parameter.
...
When this method is called, stop any rendering, release any resources allocated by reading data from the stream, and release the [**IStream**](https://msdn.microsoft.com/en-us/library/Aa380034(v=VS.85).aspx) itself.
...
These methods should be implemented when directing the preview handler to respond to the host's color and font schemes. The host queries the handler for [**IPreviewHandlerVisuals**](https://msdn.microsoft.com/en-us/library/Bb775299(v=VS.85).aspx). If found to be supported, the host provides it with color, font, and text color.
