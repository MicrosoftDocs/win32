...
1. To back up the entry, right-click the **PackageList** entry, select **Export**, and then save the registry file.
1. For each of the applications that are listed in the Event ID 5960 log entries, follow these steps:  
   1. Locate the **PackageStatus** subkey. In the case of the example event in this article, the subkey looks like the following:  
   **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\AppModel\StateChange\PackageList\Microsoft.BingNews_8wekyb3d8bbwe!AppexNews\PackageStatus**  
   1. Set the value of the **PackageStatus** value to zero (**0**).
   > [!NOTE]  
