...
Learn how to sign an app package by using the [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155) function.

If you want to programmatically create Windows Store app packages by using the [Packaging API](interfaces.md), you also need to sign the app packages before they can be deployed. The Packaging API doesn't provide a specialized method for signing app packages. Instead, use the standard [cryptography functions](https://msdn.microsoft.com/library/windows/desktop/aa380252) to sign your app packages.
...
-   You need to have a packaged Windows Store app. For info about creating an app package, see [How to create an app package](how-to-create-a-package.md).
-   You need to have a code signing certificate that is appropriate for signing the app package. For info about creating a test code signing certificate, see [How to create an app package signing certificate](how-to-create-a-package-signing-certificate.md). Load this signing certificate into a [**CERT\_CONTEXT**](https://msdn.microsoft.com/library/windows/desktop/aa377189) structure. For example, you can use [**PFXImportCertStore**](https://msdn.microsoft.com/library/windows/desktop/aa387314) and [**CertFindCertificateInStore**](https://msdn.microsoft.com/library/windows/desktop/aa376064) to load a signing certificate.
-   Windows 8 introduces the [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155) function. Use **SignerSignEx2** when you sign Windows Store app packages.

## Instructions

### Step 1: Define the required structures for SignerSignEx2

In addition to the Wincrypt.h header, the [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155) function depends on many structures that aren't defined in any SDK header file. To use **SignerSignEx2**, you must define these structures yourself:
...
After you define the required structures that are specified in the previous step, you can use any of the options available on the [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155) function to sign an app package. When you use **SignerSignEx2** with Windows Store app packages, these restrictions apply:

-   You must provide a pointer to an **APPX\_SIP\_CLIENT\_DATA** structure as the *pSipData* parameter when you sign an app package. You must populate the **pSignerParams** member of **APPX\_SIP\_CLIENT\_DATA** with the same parameters that you use to sign the app package. To do this, define your desired parameters on the **SIGNER\_SIGN\_EX2\_PARAMS** structure, assign the address of this structure to **pSignerParams**, and then directly reference the structure's members as well when you call [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155).
-   After you call [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155), you must free the **pAppxSipState** on the *pSipData* by calling [**IUnknown::Release**](https://msdn.microsoft.com/library/windows/desktop/ms682317) on **pAppxSipState** if it's not **NULL**.
...
-   If you want to time stamp the signature on the app package as well, you must do so during the call to [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155) by providing **SignerSignEx2**'s optional time stamping parameters (*dwTimestampFlags*, *pszTimestampAlgorithmOid*, *pwszHttpTimeStamp*, *psRequest*). Calling [**SignerTimeStampEx3**](https://msdn.microsoft.com/library/windows/desktop/hh968156) or its variants on an app package that is already signed is unsupported.

Here is some example code that shows how to call [**SignerSignEx2**](https://msdn.microsoft.com/library/windows/desktop/hh968155):
...
After you sign the app package, you can also attempt to validate the signature programmatically by using the [**WinVerifyTrust**](https://msdn.microsoft.com/library/windows/desktop/aa388208) function with **WINTRUST\_ACTION\_GENERIC\_VERIFY\_V2**. There are no special considerations in this case for using **WinVerifyTrust** with Windows Store app packages.
