...
-   [Certificate Trust Verification](https://msdn.microsoft.com/library/windows/desktop/aa376546)

### Prerequisites

-   [Windows Event Log](https://msdn.microsoft.com/library/windows/desktop/aa385780) to diagnose installation failures.
-   [Certutil tasks for managing certificates](5e0f52f2-f7c8-4c74-9497-be52366df52e) for certificate store manipulation during troubleshooting
...
|------------|---------------------------------------|----------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0x80073CF0 | ERROR\_INSTALL\_OPEN\_PACKAGE\_FAILED | The app package could not be opened.                                                                                 | This error typically indicates a problem with the package. You need to build and sign the package again. For more info, see [Using App Packager](make-appx-package--makeappx-exe-.md#using-app-packager). |
| 0x80080205 | APPX\_E\_INVALID\_BLOCKMAP            | The app package has been tampered with or has an invalid block map.                                                  | The package is corrupted. You need to build and sign the package again. For more info, see [Using App Packager](make-appx-package--makeappx-exe-.md#using-app-packager).                                  |
...
If you don't specify the certificate serial number, [Certutil](c264ccf0-ba1e-412b-9dd3-d77dd9345ad9) lists all certificates that are trusted by the local computer for that store.

The package may fail to install due to certificate chaining errors, even if the signing certificate is not self-signed and the root certificate is in the root store of the local computer. In this case, there might be an issue with trust for the intermediate certificate authorities. For more info about this issue, see [Working with Certificates](6ffb8682-8f07-4a45-afbb-8d2487e9dbc3).
...
[Certutil tasks for managing certificates](5e0f52f2-f7c8-4c74-9497-be52366df52e)
</dt> </dl>
