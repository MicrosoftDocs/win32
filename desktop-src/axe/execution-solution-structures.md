---
title: Execution Solution Structures
description: AXE Execution Solution API provides the following structures. The interfaces of the API are implemented with the struct keyword and are documented in this section.
ms.assetid: B2781ED5-B3F1-414E-81C1-B6826371E7DE
ms.technology: desktop
ms.prod: windows
ms.author: windowssdkdev
ms.topic: article
ms.date: 05/31/2018
---

# Execution Solution Structures

AXE Execution Solution API provides the following structures. The interfaces of the API are implemented with the **struct** keyword and are documented in this section.

<dl>

[**AnalyzeResultsInfo**](analyzeresultsinfo.md)  
[**Assessment**](assessment-inf.md)  
[**AssessmentBeginEventArgs**](assessmentbegineventargs.md)  
[**AssessmentEndEventArgs**](assessmentendeventargs.md)  
[**AssessmentResults**](assessmentresults-struct.md)  
[**AssessmentResultsCollection**](assessmentresultscollection.md)  
[**AxeErrorInfo**](axeerrorinfo.md)  
[**Engine**](engine-if.md)  
[**ErrorInfo**](errorinfo.md)  
[**ErrorList**](errorlist.md)  
[**IAssessmentBeginEventHandler**](iassessmentbegineventhandler.md)  
[**IAssessmentEndEventHandler**](iassessmentendeventhandler.md)  
[**IJobBeginEventHandler**](ijobbegineventhandler.md)  
[**IJobEndEventHandler**](ijobendeventhandler.md)  
[**IJobPendingEventHandler**](ijobpendingeventhandler.md)  
[**IJobRestartEventHandler**](ijobrestarteventhandler.md)  
[**ILogReaderCallback**](ilogreadercallback.md)  
[**IProgressUpdateEventHandler**](iprogressupdateeventhandler.md)  
[**Job**](job-if.md)  
[**JobBeginEventArgs**](jobbegineventargs.md)  
[**JobEndEventArgs**](jobendeventargs.md)  
[**JobExecutionInfo**](jobexecutioninfo.md)  
[**JobPendingEventArgs**](jobpendingeventargs.md)  
[**JobRestartEventArgs**](jobrestarteventargs.md)  
[**JobResults**](jobresults.md)  
[**Logging**](logging.md)  
[**LogReaderCallbackData**](logreadercallbackdata.md)  
[**PrerequisiteCollection**](prerequisitecollection.md)  
[**PrerequisiteIssue**](prerequisiteissue.md)  
[**ProgressUpdateEventArgs**](progressupdateeventargs.md)  
[**Solution**](solution-inf.md)  
[**SolutionParameters**](solutionparameters.md)  
</dl>

 

 




