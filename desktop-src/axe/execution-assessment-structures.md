---
title: Execution Assessment Structures
description: AXE Execution Assessment API provides the following structures. The interfaces of the API are implemented with the struct keyword and are documented in this section.
ms.assetid: C7204B7A-359C-4BC1-BD34-A549C33B67BD
ms.date: 05/31/2018
ms.topic: article
ms.author: windowssdkdev
ms.prod: windows
ms.technology: desktop
---

# Execution Assessment Structures

AXE Execution Assessment API provides the following structures. The interfaces of the API are implemented with the **struct** keyword and are documented in this section.

## AXE Execution Assessment Structures

<dl>

[**Activity**](activity-struct.md)  
[**ActivityCollection**](activitycollection.md)  
[**ActivityReference**](activityreference-struct.md)  
[**AffectedMetricCollection**](affectedmetriccollection.md)  
[**AxeErrorInfo**](axeerrorinfo.md)  
[**CategoryCollection**](categorycollection.md)  
[**ErrorList**](errorlist.md)  
[**ErrorWarning**](errorwarning.md)  
[**ErrorWarningCollection**](errorwarningcollection.md)  
[**Issue**](issue-struct.md)  
[**IssueCollection**](issuecollection.md)  
[**IssueReference**](issuereference-struct.md)  
[**Iteration**](iteration-struct.md)  
[**IterationCollection**](iterationcollection.md)  
[**Link**](link-struct.md)  
[**LinkCollection**](linkcollection.md)  
[**LogFileCollection**](logfilecollection.md)  
[**Logging**](logging.md)  
[**MetricThreshold**](metricthreshold-struct.md)  
[**MetricThresholdCollection**](metricthresholdcollection.md)  
[**MetricThresholdValue**](metricthresholdvalue-struct.md)  
[**MetricThresholdValueCollection**](metricthresholdvaluecollection.md)  
[**MetricValue**](metricvalue-struct.md)  
[**MetricValueCollection**](metricvaluecollection.md)  
[**Parameter**](parameter.md)  
[**ParameterSet**](parameterset.md)  
[**ParentCollection**](parentcollection.md)  
[**ReferenceCollection**](referencecollection.md)  
[**RelatedActivityCollection**](relatedactivitycollection.md)  
[**ResultSnippet**](resultsnippet.md)  
[**Subject**](subject.md)  
[**Support**](support.md)  
[**SystemState**](systemstate.md)  
[**TestCase**](testcase-struct.md)  
[**TestCaseCollection**](testcasecollection.md)  
[**Trace**](trace-struct.md)  
</dl>

## Related topics

<dl> <dt>

[AXE Execution Assessment API](axe-execution-assessment-api.md)
</dt> </dl>

 

 




