...
The command string to execute in the [**RestartDirectory**](https://www.bing.com/search?q=**RestartDirectory**) in order to run the solution if the job causes the system to reboot. If the Solution API is configured in Workload mode, this value must be **NULL**. Otherwise, this member must not be **NULL**.
