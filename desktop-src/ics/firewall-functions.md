---
title: Windows Firewall with Advanced Security functions
description: Windows Firewall with Advanced Security contains the following functions.
keywords:
- Windows Firewall with Advanced Security, functions
ms.topic: reference
ms.date: 11/14/2025
---

# Windows Firewall with Advanced Security functions

Windows Firewall with Advanced Security contains the following functions.

* [FWAddFirewallRule](./firewall/nf-firewall-fwaddfirewallrule.md)
* [FWClosePolicyStore](./firewall/nf-firewall-fwclosepolicystore.md)
* [FWDeleteFirewallRule](./firewall/nf-firewall-fwdeletefirewallrule.md)
* [FWEnumFirewallRules](./firewall/nf-firewall-fwenumfirewallrules.md)
* [FWOpenPolicyStore](./firewall/nf-firewall-fwopenpolicystore.md)
