...
To open a handle to the [**Policy**](policy-object.md) object of a trusted domain, call [**LsaCreateTrustedDomainEx**](lsacreatetrusteddomainex.md) (to create a new trust relationship with a domain) or call [**LsaOpenTrustedDomainByName**](lsaopentrusteddomainbyname.md) (to access an existing trusted domain). Both of these functions set a pointer to an [**LSA\_HANDLE**](lsa-handle.md), which you can then specify in subsequent LSA Policy function calls. As with [**LsaOpenPolicy**](https://msdn.microsoft.com/library/windows/desktop/aa378299), your application should call [**LsaClose**](lsaclose.md) when it no longer needs the handle to the trusted domain's **Policy** object.
