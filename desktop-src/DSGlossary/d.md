...
Active Directory *data model* is derived from the [*X.500*](https://www.bing.com/search?q=*X.500*) data model. The directory holds objects that represent various elements described by attributes. The types of objects stored in the directory are defined in the [*schema*](https://www.bing.com/search?q=*schema*). For each [*object class*](https://www.bing.com/search?q=*object class*), the schema defines what attributes an instance of the class must have, what additional attributes it may have, and which object classes can be a parent of the current object class.
...
*Delegation* is one of the most important security features of Active Directory. Delegation allows a higher administrative authority to grant specific administrative rights for [*containers*](https://www.bing.com/search?q=*containers*) and subtrees to individuals and groups. This eliminates the need for domain administrators with broad authority over large segments of users. An [*access-control entry*](https://www.bing.com/search?q=*access-control entry*) can grant specific administrative rights on the objects in a container to a user or group. Rights are granted for specific operations on specific [*object classes*](https://www.bing.com/search?q=*object classes*) using an ACE in the container [*access-control list*](https://www.bing.com/search?q=*access-control list*).
...
A directory partition, or naming context, is a contiguous Active Directory subtree replicated on one, or more, Windows 2000 domain controllers in a forest. By default, each domain controller has a replica of three partitions: the [*schema partition*](https://www.bing.com/search?q=*schema partition*), the [*configuration partition*](https://www.bing.com/search?q=*configuration partition*), and a *domain partition*.
...
Also called a [*forest*](https://www.bing.com/search?q=*forest*). A logical structure formed by combining two or more Windows 2000 or Windows Server 2003 *domain trees*.
...
A [*group*](https://www.bing.com/search?q=*group*) that can contain members from any domain, but can be granted permissions only to resources in its own domain.
