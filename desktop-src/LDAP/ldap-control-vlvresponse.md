...
This control cannot be sent; it can only be received. The [**LDAPControl**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-ldapcontrola) structure will have its members set to the following:
...
Returns a **controlValue** that is an OCTET STRING whose value is the BER-encoded sequence of parameters that the application uses to specify the LDAP\_CONTROL\_VLVRESPONSE control. The [**berval**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-berval) structure returns a pointer in **bv\_val** to the sequence that contains the virtual list view data, and sets **bv\_len** to the length of the sequence. For more information, see the Remarks section.
