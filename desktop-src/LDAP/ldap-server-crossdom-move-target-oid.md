...
Specifies the DNS name of the destination DC. In the [**berval**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-berval) structure, set **bv\_val** to a pointer to an UTF-8 string that contains the DNS name, and set **bv\_len** to the length of the string.
...
The following code example shows how to use the cross-domain control with the [**ldap\_rename\_ext\_s**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_rename_ext_s) function.
