...
Specifies a BER-encoded sequence of parameters that enables the application to specify a server to verify the existence of an object. In the [**berval**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-berval) structure, set **bv\_val** to a pointer to the sequence that contains the flag and server name data and set **bv\_len** to the length of the sequence. For more information, see the Remarks section.
...
The Verify Name control is used with [**ldap\_modify\_ext**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_modify_ext) or [**ldap\_add\_ext**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_add_ext) to specify the specific server used to verify the existence of an object. The **ldctl\_value** field is set to the following BER-encoded sequence:
...
The [**ber\_printf**](/previous-versions/windows/desktop/api/Winber/nf-winber-ber_printf) function is used to create the sequence data. The flags portion is set to 0, and the *ServerName* is a Unicode string that contains the fully qualified DNS name of the server to contact for verification.
