...
To use this control, format the control contents using the [**ldap\_create\_sort\_control**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_create_sort_control) function, or set the members of the [**LDAPControl**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-ldapcontrola) structure as follows.
...
Specifies a BER-encoded sequence of parameters that enables the application to specify the sort keys and matching rules, if any. In the [**berval**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-berval) structure, set **bv\_val** to a pointer to the sequence that contains the sort key data and set **bv\_len** to the length of the sequence. For more information, see Remarks.
...
The [**ber\_printf**](/previous-versions/windows/desktop/api/Winber/nf-winber-ber_printf) function is used to create the sequence data. The sort key list sequence is in order of highest to lowest sort key precedence.
...
Client applications should use the various API calls such as [**ldap\_create\_sort\_control**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_create_sort_control), [**ldap\_search\_init\_page**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_search_init_page), [**ldap\_parse\_result**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_parse_result), [**ldap\_parse\_sort\_control**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_parse_sort_control), and so on, instead of manually creating and specifying the sort controls.
