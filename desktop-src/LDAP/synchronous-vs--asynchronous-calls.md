...
At any time a client application can check on the status of an asynchronous call by calling [**ldap\_result**](/windows/previous-versions/Winldap/nf-winldap-ldap_result?branch=master) with the message ID. To abandon an asynchronous call in progress, call [**ldap\_abandon**](/windows/previous-versions/Winldap/nf-winldap-ldap_abandon?branch=master).
