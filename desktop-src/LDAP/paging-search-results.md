...
Paged results are indicated as a control on the [**ldap\_search\_ext**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_search_ext) function call. Use [**ldap\_create\_page\_control**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_create_page_control) to construct this control, and then call **ldap\_search\_ext** to add the control. This control structure must then be added to the list of server controls in the **ldap\_search\_ext** call. When the server returns the first page of results, it includes the resume cookie in the controls field of the SearchResultDone message. The client must then extract the cookie from the search result by retrieving the server controls by using [**ldap\_parse\_result**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_parse_result) and parsing the control with [**ldap\_parse\_page\_control**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_parse_page_control). The client then uses the cookie in the next call to **LDAP\_create\_page\_control** to retrieve the next page of results.
