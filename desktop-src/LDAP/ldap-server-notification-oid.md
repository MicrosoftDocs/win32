...
This control is used with an extended LDAP search function such as [**ldap\_search\_ext**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_search_ext) to register a change notification request of the directory objects that match the search filter specified. The control is part of the SearchRequest and SearchResultDone messages in the control fields of [**LDAPMessage**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-ldapmsg). A client can register up to five (5) notification requests with this control.

Notifications are asynchronous operations. The server sends SearchEntry responses, that contain the modified objects, to the client using the [**LDAPMessage**](/previous-versions/windows/desktop/api/Winldap/ns-winldap-ldapmsg) ID of the original notification request. Notifications from this control may be canceled using the [**ldap\_abandon**](/previous-versions/windows/desktop/api/Winldap/nf-winldap-ldap_abandon) function. For more information about Notifications in Active Directory, see [Change Notifications in Active Directory](https://msdn.microsoft.com/library/aa772153).
