...
For a pipe client, a pipe handle returned by [**CreateFile**](https://msdn.microsoft.com/library/windows/desktop/aa363858) is always in byte-read mode initially. Both pipe clients and pipe servers can use the [**SetNamedPipeHandleState**](/windows/desktop/api/Winbase/) function to change the read mode of a pipe handle. The pipe handle must have the FILE\_WRITE\_ATTRIBUTES access right.

## Wait Mode

The wait mode of a pipe handle determines how the [**ReadFile**](https://msdn.microsoft.com/library/windows/desktop/aa365467), [**WriteFile**](https://msdn.microsoft.com/library/windows/desktop/aa365747), and [**ConnectNamedPipe**](/windows/desktop/api/Winbase/) functions handle lengthy operations. In blocking-wait mode, the functions wait indefinitely for a process on the other end of the pipe to complete an operation. In nonblocking-wait mode, the functions return immediately in situations that would otherwise require an indefinite wait.
...
Both pipe clients and pipe servers can change a pipe handle's wait mode by specifying either PIPE\_WAIT or PIPE\_NOWAIT in a call to the [**SetNamedPipeHandleState**](/windows/desktop/api/Winbase/) function.
