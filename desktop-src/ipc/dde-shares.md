...
The client starts the DDE conversation by connecting to the DDE share. The client must call the [**DdeInitialize**](https://www.bing.com/search?q=**DdeInitialize**) function to initialize DDEML and call the [**DdeConnect**](https://www.bing.com/search?q=**DdeConnect**) function to connect to the DDE share. In the **DdeConnect** call, the client specifies the service name as follows:

\\\\*ComputerName*\\NDDE$

where *ComputerName* is the name of the computer running the server application. The NDDE$ indicates that the topic provided to [**DdeConnect**](https://www.bing.com/search?q=**DdeConnect**) is the DDE share name on the remote computer named *ComputerName*.
