...
Each named pipe has a unique name that distinguishes it from other named pipes in the system's list of named objects. A pipe server specifies a name for the pipe when it calls the [**CreateNamedPipe**](/windows/win32/Winbase/nf-winbase-createnamedpipea?branch=master) function to create one or more instances of a named pipe. Pipe clients specify the pipe name when they call the [**CreateFile**](https://msdn.microsoft.com/library/windows/desktop/aa363858) or [**CallNamedPipe**](/windows/win32/Winbase/nf-winbase-callnamedpipea?branch=master) function to connect to an instance of the named pipe.

Use the following form when specifying the name of a pipe in the [**CreateFile**](https://msdn.microsoft.com/library/windows/desktop/aa363858), [**WaitNamedPipe**](/windows/win32/Winbase/nf-winbase-waitnamedpipea?branch=master), or [**CallNamedPipe**](/windows/win32/Winbase/nf-winbase-callnamedpipea?branch=master) function:
...
The pipe server cannot create a pipe on another computer, so [**CreateNamedPipe**](/windows/win32/Winbase/nf-winbase-createnamedpipea?branch=master) must use a period for the server name, as shown in the following example.
