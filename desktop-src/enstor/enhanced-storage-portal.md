---
title: Enhanced Storage
description: Enhanced Storage provides an extensible platform for accessing additional functions made available by enhanced storage devices; above and beyond the primary function of data storage.
ms.assetid: 'f36a98b2-51d3-4341-9ec8-85ce8d375880'
---

# Enhanced Storage

## Purpose

Enhanced Storage provides an extensible platform for accessing additional functions made available by enhanced storage devices; above and beyond the primary function of data storage.

## Developer audience

The Enhanced Storage API is designed for use with C/C++.

## Run-time requirements

Enhanced Storage is supported natively in Windows Vista with Service Pack 2 (SP2) and Windows 7.

## In this section



| Topic                                                                   | Description                                                                                     |
|-------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------|
| [About Enhanced Storage](about-enhanced-storage.md)<br/>         | General information about the Enhanced Storage API.<br/>                                  |
| [Using Enhanced Storage](using-enhanced-storage.md)<br/>         | Task-related information that demonstrates how to use the Enhanced Storage API.<br/>      |
| [Enhanced Storage Reference](enhanced-storage-reference.md)<br/> | Detailed descriptions of Enhanced Storage interfaces and other programming elements.<br/> |



 

 

 





