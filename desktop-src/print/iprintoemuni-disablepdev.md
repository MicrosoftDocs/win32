...
The `IPrintOemUni::DisablePDEV` method performs the same types of operations as the [**DrvDisablePDEV**](display.drvdisablepdev) function that is exported by a printer graphics DLL. Its purpose is to allow a rendering plug-in to delete the private PDEV structure that is pointed to by the DEVOBJ structure's **pdevOEM** member. This PDEV structure is one that was allocated by the plug-in's [**IPrintOemUni::EnablePDEV**](iprintoemuni-enablepdev.md) method.
