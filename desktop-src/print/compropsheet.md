...
The *ComPropSheet* function is supplied by [CPSUI](print.common_property_sheet_user_interface) and can be called by CPSUI applications (including printer interface DLLs) to build property sheet pages.
...
Caller-supplied handle to a property sheet [group parent](print.group_parent). For more information, see the following Remarks section.
...
The return value depends on the [ComPropSheet function code](print.compropsheet_function_codes) supplied for *Function*.
...
A [printer interface DLL](print.printer_interface_dll) can call *ComPropSheet* from within its [**DrvDocumentPropertySheets**](drvdocumentpropertysheets.md) function or its [**DrvDevicePropertySheets**](drvdevicepropertysheets.md) function.

[User interface plug-ins](print.user_interface_plug_ins) for Microsoft's [*Unidrv*](wdkgloss.u#wdkgloss-unidrv) and [*Pscript*](wdkgloss.p#wdkgloss-pscript) drivers can call *ComPropSheet* from within their [**IPrintOemUI::DocumentPropertySheets**](iprintoemui-documentpropertysheets.md) and [**IPrintOemUI::DevicePropertySheets**](iprintoemui-devicepropertysheets.md) methods.

The [group parent](print.group_parent) handle specified for the *hComPropSheet* parameter can be either of the following:

-   The handle received in the *hComPropSheet* member of a [**PROPSHEETUI\_INFO**](propsheetui-info.md) structure.

-   The handle received as a result of previously calling *ComPropSheet* with a [**CPSFUNC\_INSERT\_PSUIPAGE**](print.cpsfunc_insert_psuipage) function code, and specifying PSUIPAGEINSERT\_GROUP\_PARENT as the **Type** member for an [**INSERTPSUIPAGE\_INFO**](insertpsuipage-info.md) structure.
