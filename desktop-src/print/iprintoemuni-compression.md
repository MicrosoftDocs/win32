...
If the `IPrintOemUni::Compression` method is defined, and if the printer's [*GPD*](https://www.bing.com/search?q=*GPD*) file contains a CmdEnableOEMComp command entry, Unidrv calls the method each time a scan line is ready to be sent to the print spooler. (For information about the CmdEnableOEMComp command, see [Raster Data Compression Commands](https://www.bing.com/search?q=Raster Data Compression Commands).)
