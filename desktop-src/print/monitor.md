...
Pointer to the print monitor's [**OpenPort**](print.openport) function.
...
Pointer to the print monitor's [**WritePort**](print.writeport) function.
...
Pointer to the print monitor's [**ReadPort**](print.readport) function.
...
Pointer to the print monitor's [**ClosePort**](print.closeport) function.
...
The [**AddPort**](print.addport) function is obsolete and is for use only with Windows NT 4.0 and previous versions.

[**AddPort**](print.addport) creates a port and adds it to the list of ports currently supported by the specified monitor in the spooler environment.
...
(Obsolete. Must be **NULL**.) Pointer to the print monitor's [**AddPortEx**](print.addportex) function. (Port monitors only.)
...
The [**ConfigurePort**](print.configureport) function is obsolete and is for use only with Windows NT 4.0 and previous versions. For later versions of Windows, use [**ConfigurePortUI**](print.configureportui).

[**ConfigurePort**](print.configureport) is a port management function that configures the specified port.
...
Pointer to the print monitor's [**XcvOpenPort**](print.xcvopenport) function. (Port monitors only.)
...
Pointer to the print monitor's [**XcvDataPort**](print.xcvdataport) function. (Port monitors only.)
...
Pointer to the print monitor's [**XcvClosePort**](print.xcvcloseport) function. (Port monitors only.)
...
The spooler calls [**AddPort**](print.addport) when it receives an application request to add a port to its environment. The spooler forwards the call to the named port monitor on the named server.

[**AddPort**](print.addport) allows a port to be added interactively. A monitor should prompt a user to enter the port name in a dialog box on the window associated with *hWnd*. **AddPort** should validate the entered port name by calling the Win32 EnumPorts function to ensure that no duplicate port names are added to the spooler environment. A monitor should also verify that the port is one that it supports.

The spooler does not support remote [**AddPort**](print.addport) calls. Consequently, **AddPort** implementations can ignore the *pName* and *pMonitorName* parameters.

The spooler calls [**ConfigurePort**](print.configureport) so a port monitor can perform port configuration. **ConfigurePort** can offer a dialog box to obtain some or all of the necessary port configuration information from the user.

The spooler does not support remote [**ConfigurePort**](print.configureport) calls; consequently, monitors can ignore the *pName* parameter.
...
[Language monitors](print.language_monitors) and port monitor server DLLs are required to define an **EndDocPort** function and include the function's address in a [**MONITOR2**](monitor2.md) structure.

The handle received as the function's hPort argument is the port handle that the monitor's [**OpenPort**](print.openport) or [**OpenPortEx**](print.openportex) function supplied.
...
[Language monitors](print.language_monitors) and port monitor server DLLs can optionally define a **GetPrinterDataFromPort** function and include the function's address in a [**MONITOR2**](monitor2.md) structure.
...
The language monitor's responsibility is to send a command to the printer hardware by calling the port monitor's [**WritePort**](print.writeport) function, and reading the response by calling [**ReadPort**](print.readport), to obtain the needed value. For example, pjlmon.dll, the [sample language monitor](print.sample_language_monitor), can return values for a port's "Installed Memory" and "Available Memory" registry value names.
...
The `OpenPortEx` function's primary purpose is to return a port handle that the caller can use as an input argument for subsequent calls to the language monitor's [**StartDocPort**](print.startdocport), [**WritePort**](print.writeport), [**ReadPort**](print.readport), [**EndDocPort**](print.enddocport), and [**GetPrinterDataFromPort**](print.getprinterdatafromport) functions. Because a language monitor typically implements these functions by calling the equivalent functions in its associated port monitor, a language monitor typically obtains a port handle by calling the port monitor's [**OpenPort**](print.openport) function. For more information see [Language and Port Monitor Interaction](print.language_and_port_monitor_interaction).
...
The function is called by pjlmon.dll, the [sample language monitor](print.sample_language_monitor), and you can write a customized language monitor that calls it. The print spooler does not call `SetPortTimeOuts`.

The port monitor should initialize the port's time-out values from within its [**OpenPort**](print.openport) function.

[Language monitors](print.language_monitors) and port monitor server DLLs are required to define a `StartDocPort` function and include the function's address in a [**MONITOR2**](monitor2.md) structure.

The handle received as the function's *hPort* argument is the port handle that the monitor's [**OpenPort**](print.openport) or [**OpenPortEx**](print.openportex) function supplied.

A language monitor's `StartDocPort` function typically calls the associated port monitor's `StartDocPort` function.

A port monitor server DLL's `StartDocPort` function typically calls the [**CreateFile**](fs.createfile) function, described in the Windows SDK documentation, to create a connection to the kernel-mode port driver.

If necessary, the port monitor should prevent other processes from using the specified port until [**EndDocPort**](print.enddocport) is called. For example, a port monitor for a COM port must ensure that, while a spooler is sending printer data to the port, another application does not assume the port is connected to a particular communications device and then attempt to communicate with that device. This cautionary note does not apply to the local print provider, which guarantees that it never calls `StartDocPort` twice in succession without an intervening call to **EndDocPort**, but it does apply to print providers that do not make this guarantee.
