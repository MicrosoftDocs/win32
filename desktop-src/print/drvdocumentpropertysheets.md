...
If *pPSUIInfo* is **NULL**, and if either *lParam* -&gt; **fMode** is zero or *lParam -*&gt; **pdmOut** is **NULL**, this function should return the size, in bytes, of the printer's [**DEVMODEW**](display.devmodew) structure. Otherwise, the function's return value is the same as that described in the ReturnValue section of the [**PFNPROPSHEETUI**](pfnpropsheetui.md) function type. For more information, see the Remarks section.

## Remarks

All [printer interface DLLs](print.printer_interface_dll) must provide a **DrvDocumentPropertySheets** function, which is defined using the [**PFNPROPSHEETUI**](pfnpropsheetui.md) function type. The function's purpose is to call the [**ComPropSheet**](compropsheet.md) function, provided by [CPSUI](print.common_property_sheet_user_interface), to specify property sheet pages containing user-modifiable properties for print documents.
...
-   If the **fMode** member of the [**DOCUMENTPROPERTYHEADER**](documentpropertyheader.md) structure is zero, or if the **pdmOut** member of the same structure is **NULL**, the function should return just the total size of the printer's [**DEVMODEW**](display.devmodew) structure, including public and private structure members, in the DOCUMENTPROPERTYHEADER structure's **cbOut** member.
