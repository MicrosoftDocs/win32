...
A rendering plug-in's `IPrintOemPS::ResetPDEV` method performs the same types of operations as the [**DrvResetPDEV**](https://msdn.microsoft.com/8e530874-7774-4f8f-852c-001b2ce4a707) function that is exported by a printer graphics DLL. During the processing of an application's call to the Microsoft Windows SDK **ResetDC** function, the `IPrintOemPS::ResetPDEV` method is called by the **DrvResetPDEV** function in Pscript5's printer graphics DLL. For more information about when **DrvResetPDEV** is called, see its description.
