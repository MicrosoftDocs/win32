...
A rendering plug-in's `IPrintOemPS::ResetPDEV` method performs the same types of operations as the [**DrvResetPDEV**](display.drvresetpdev) function that is exported by a printer graphics DLL. During the processing of an application's call to the Microsoft Windows SDK **ResetDC** function, the `IPrintOemPS::ResetPDEV` method is called by the **DrvResetPDEV** function in Pscript5's printer graphics DLL. For more information about when **DrvResetPDEV** is called, see its description.
