...
The received [**DEVQUERYPRINT\_INFO**](devqueryprint-info.md) structure points to a [**DEVMODEW**](https://msdn.microsoft.com/b2369876-9a79-40c8-8d27-c8b9d8e68e6b) structure describing the printer characteristics required by the print job. The **DevQueryPrintEx** function should first verify that the size and version members of the received DEVMODEW structure are compatible with the driver. Then it should determine if the supplied DEVMODEW contents are compatible with the current printer configuration.
