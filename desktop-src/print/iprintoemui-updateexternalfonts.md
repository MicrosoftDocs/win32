...
A user interface plug-in must implement the `IPrintOemUI::UpdateExternalFonts` method if the plug-in is replacing Unidrv's default font installer. For more information, see [Customized Font Installers for Unidrv](print.customized_font_installers_for_unidrv).
