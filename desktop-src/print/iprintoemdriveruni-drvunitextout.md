---
Description: The IPrintOemDriverUniDrvUniTextOut method is provided by the Unidrv driver so that a rendering plug-in using a device-managed drawing surface can easily output text strings.
ms.assetid: f8c21813-9bfd-46a5-abb2-78ac2f2301af
title: IPrintOemDriverUniDrvUniTextOut method
ms.date: 05/31/2018
ms.topic: article
...
Pointer to a [**SURFOBJ**](display.surfobj) structure that describes the surface on which to write.
...
Pointer to a [**STROBJ**](display.strobj) structure that defines the glyphs to be rendered and the positions in which to place them.
...
Pointer to a [**FONTOBJ**](display.fontobj) structure from which to retrieve information about the font and its glyphs.
...
Pointer to a [**CLIPOBJ**](display.clipobj) structure that defines the clip region through which all rendering must be done. The driver cannot affect any pixels outside the clip region.
...
Pointer to a [**BRUSHOBJ**](display.brushobj) structure that represents the brush object to be used for the foreground pixels. This brush will always be a solid color brush.
...
Pointer to a [**POINTL**](display.pointl) structure that defines the brush origin for both brushes.
...
For more information, see [Handling Device-Managed Surfaces](print.handling_device_managed_surfaces).
