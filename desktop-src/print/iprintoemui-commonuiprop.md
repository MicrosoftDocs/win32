...
The `IPrintOemUI::CommonUIProp` method is called by the printer driver's [printer interface DLL](print.printer_interface_dll). The method should supply an array of [**OPTITEM**](optitem.md) structures describing property sheet items, along with a callback function for processing user modifications to option values.
...
For more information, see [Modifying a Driver-Supplied Property Sheet Page](print.modifying_a_driver_supplied_property_sheet_page).
