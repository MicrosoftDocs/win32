...
The following code demonstrates how to create an alpha blended cursor. `DrawRGBCircles` writes three overlapping R/G/B semi-transparent circles directly into a pixel buffer: it composites the circles analytically using the AC\_SRC\_OVER formula and stores premultiplied RGB channels with alpha in `rgbReserved`. The cursor size is read from [**GetSystemMetrics**](/windows/win32/api/winuser/nf-winuser-getsystemmetrics) to match the system cursor size setting. Change `fIcon` to `TRUE` to create an icon instead.
