...
Defines one or more string resources for an application. String resources are simply null-terminated Unicode or ASCII strings that can be loaded when needed from the executable file, using the [**LoadString**](_win32_loadstring_cpp) function.
...
RC allocates 16 strings per section and uses the identifier value to determine which section is to contain the string. Strings whose identifiers differ only in the bottom 4 bits are placed in the same section. For more information, see [Q196774](Http://go.microsoft.com/fwlink/p/?linkid=83996).
