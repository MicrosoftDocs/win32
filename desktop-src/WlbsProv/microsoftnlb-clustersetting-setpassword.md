...
> [!Note]  
> Before calling this method, it is highly recommended that the authentication level for the call to the [**CoSetProxyBlanket**](https://www.bing.com/search?q=**CoSetProxyBlanket**) function or the [**CoInitializeSecurity**](https://www.bing.com/search?q=**CoInitializeSecurity**) function be set to **RPC\_C\_AUTHN\_LEVEL\_PKT\_PRIVACY**. This setting reduces the threat of disclosure by ensuring that the password string is encrypted during its transfer from the client computer to the server computer.
