...
The completion of system fix-up is a system health change event. That means you must call **NotifySoHChange** when a [**INapSystemHealthAgentCallback::GetFixupInfo**](inapsystemhealthagentcallback-getfixupinfo-method.md) notification indicates that the client is fixed. When a client is fixed, the **state** member of the [**FixupInfo**](/windows/win32/NapTypes/ns-naptypes-tagfixupinfo?branch=master) structure returned by **GetFixupInfo** has a value of **fixupStateSuccess**.
