...
keywords: ["Validate method NAP", "Validate method NAP , INapSystemHealthValidator interface", "INapSystemHealthValidator interface NAP , Validate method"]
topic_type:
- apiref
api_name:
...
The **INapSystemHealthValidator::Validate** method is defined by the SHV developer and called by the NAP system to validate the [**SoHRequest**](soh-struct.md) received from a client.
...
In order to support intrusion detection, SHVs will be asked to validate the client machine regardless of whether the client sent an [**SoHRequest**](soh-struct.md) intended for the SHV.

The SHV must do the following:

-   Retrieve the [**SoHRequest**](soh-struct.md) from *request* by calling [**request.GetSoHRequest()**](inapsystemhealthvalidationrequest-getsohrequest-method.md).
-   If the [**SoHRequest**](soh-struct.md) packet is null:
    -   If the SHV is an intrusion detection system, populate an [**SoHRequest**](soh-struct.md) packet with the appropriate [**NAP error code**](nap-error-constants.md) as to why the client machine is malicious.
    -   All other SHVs should populate an [**SoHRequest**](soh-struct.md) packet with an error code of [**NAP\_E\_MISSING\_SOH**](nap-error-constants.md).
-   If *napSystemGenerated* is **TRUE** from the call to [**request.GetSoHRequest()**](inapsystemhealthvalidationrequest-getsohrequest-method.md), the SHV should expect an [**SoH**](soh-struct.md) packet with the following 3 TLVs: [**sohAttributeTypeSystemHealthId**](sohattributetype-enum.md), **sohAttributeTypeFailureCategory**, **sohAttributeTypeErrorCodes**. This **SoHRequest** is generated by the NapAgent on behalf of the System Health Agent (SHA) since there was an error in retrieving a request packet from the SHA.
-   Validate the [**SoHRequest**](soh-struct.md) packet.
    -   If the [**SoHRequest**](soh-struct.md) is malformed, then construct a **SoHResponse** packet with error code [**NAP\_E\_INVALID\_PACKET**](nap-error-constants.md).
    -   If the SHV is only using cached information to validate the [**SoHRequest**](soh-struct.md) packet (i.e. no I/O is performed), then it can construct the **SoHResponse**, set it on the object in *request* and return **S\_OK**.
...
An SHV must return either an **sohAttributeTypeComplianceResultCodes** or **sohAttributeTypeFailureCategory** TLV in its [**SoHRequest**](soh-struct.md).
