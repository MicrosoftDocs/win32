...
The **INapSystemHealthValidator::Validate** method is defined by the SHV developer and called by the NAP system to validate the [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) received from a client.
...
In order to support intrusion detection, SHVs will be asked to validate the client machine regardless of whether the client sent an [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) intended for the SHV.

The SHV must do the following:

-   Retrieve the [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) from *request* by calling [**request.GetSoHRequest()**](inapsystemhealthvalidationrequest-getsohrequest-method.md).
-   If the [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) packet is null:
    -   If the SHV is an intrusion detection system, populate an [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) packet with the appropriate [**NAP error code**](nap-error-constants.md) as to why the client machine is malicious.
    -   All other SHVs should populate an [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) packet with an error code of [**NAP\_E\_MISSING\_SOH**](nap-error-constants.md).
...
-   Validate the [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) packet.
    -   If the [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) is malformed, then construct a **SoHResponse** packet with error code [**NAP\_E\_INVALID\_PACKET**](nap-error-constants.md).
    -   If the SHV is only using cached information to validate the [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh) packet (i.e. no I/O is performed), then it can construct the **SoHResponse**, set it on the object in *request* and return **S\_OK**.
...
An SHV must return either an **sohAttributeTypeComplianceResultCodes** or **sohAttributeTypeFailureCategory** TLV in its [**SoHRequest**](/windows/desktop/api/NapTypes/ns-naptypes-tagsoh).
