...
-   The interface stubs implement [**IRpcStubBuffer**](/windows/win32/objidlbase/nn-objidl-irpcstubbuffer?branch=master), not [**IRpcProxyBuffer**](/windows/win32/objidlbase/nn-objidl-irpcproxybuffer?branch=master).
...
The following diagram shows the structure of the stub. Each interface stub is connected to an interface on the object. The channel dispatches incoming messages to the appropriate interface stub. All the components talk to the channel through [**IRpcChannelBuffer**](/windows/win32/objidlbase/nn-objidl-irpcchannelbuffer?branch=master), the interface that provides access to the RPC run-time library.
