...
-   The interface stubs implement [**IRpcStubBuffer**](irpcstubbuffer.md), not [**IRpcProxyBuffer**](irpcproxybuffer.md).
...
The following diagram shows the structure of the stub. Each interface stub is connected to an interface on the object. The channel dispatches incoming messages to the appropriate interface stub. All the components talk to the channel through [**IRpcChannelBuffer**](irpcchannelbuffer.md), the interface that provides access to the RPC run-time library.
