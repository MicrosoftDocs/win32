...
-   The aggregable (or inner) object's implementation of [**QueryInterface**](iunknown-queryinterface.md), [**AddRef**](iunknown-addref.md), and [**Release**](iunknown-release.md) for its [**IUnknown**](iunknown.md) interface controls the inner object's reference count, and this implementation must not delegate to the outer object's unknown (the controlling **IUnknown**).
-   The aggregable (or inner) object's implementation of [**QueryInterface**](iunknown-queryinterface.md), [**AddRef**](iunknown-addref.md), and [**Release**](iunknown-release.md) for its other interfaces must delegate to the controlling [**IUnknown**](iunknown.md) and must not directly affect the inner object's reference count.
-   The inner [**IUnknown**](iunknown.md) must implement [**QueryInterface**](iunknown-queryinterface.md) only for the inner object.
-   The aggregable object must not call [**AddRef**](iunknown-addref.md) when holding a reference to the controlling [**IUnknown**](iunknown.md) pointer.
-   When the object is created, if any interface other than [**IUnknown**](iunknown.md) is requested, the creation must fail with E\_NOINTERFACE.
...
-   When creating the inner object, the outer object must explicitly ask for its [**IUnknown**](iunknown.md).
-   The outer object must protect its implementation of [**Release**](iunknown-release.md) from reentrancy with an artificial reference count around its destruction code.
-   The outer object must call its controlling [**IUnknown**](iunknown.md)Â [**Release**](iunknown-release.md) method if it queries for a pointer to any of the inner object's interfaces. To free this pointer, the outer object calls its controlling **IUnknown**Â [**AddRef**](iunknown-addref.md) method, followed by **Release** on the inner object's pointer.
    ```C++
