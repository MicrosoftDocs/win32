...
The server cannot impersonate the client after the server's call to [**ISynchronize::Signal**](isynchronize-signal.md) completes, even if the Begin\_ method has not yet completed. For example, suppose a client calls the Begin\_ method, the server processes the call immediately, and the server calls **Signal** to indicate it is finished processing. Even if work remains to be done in the Begin\_ method, the server cannot impersonate the client after the call to **Signal** completes.

If the server impersonates the client before it calls [**Signal**](isynchronize-signal.md), the impersonation token will not be removed from the thread until the server calls [**IServerSecurity::RevertToSelf**](iserversecurity-reverttoself.md) or until the server's call to Begin\_ returns, whichever comes first.
