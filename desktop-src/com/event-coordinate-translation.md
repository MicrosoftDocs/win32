...
-   When a 96 container hosts a pre-96 control the control will present the container with coordinates and expect the container to any translation necessary. However the 96 container will be expecting a control to conform to the 96 controls specification and present its coordinates as points. The control uses the [**TransformCoords**](iolecontrolsite-transformcoords.md) method on the [**IOleControlSite**](iolecontrolsite.md) interface provided by the container in the same way as it does for properties and methods to achieve this.
