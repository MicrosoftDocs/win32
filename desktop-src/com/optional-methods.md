...
| [**EnumObjects**](/windows/desktop/api/OleIdl/nf-oleidl-iolecontainer-enumobjects)<br/>               | Returns all ActiveX controls through an enumerator with [**IEnumUnknown**](/windows/desktop/api/objidlbase/nn-objidl-ienumunknown), but not necessarily all objects (because there's no guarantee that all objects are ActiveX controls; some may be regular Windows controls).<br/> |
