...
When a call is marshaled, the proxy creates a cancel object, which implements the [**ICancelMethodCalls**](/windows/desktop/api/objidlbase/nn-objidl-icancelmethodcalls) interface. The cancel object is associated with both the call and the thread on which the call is pending.
