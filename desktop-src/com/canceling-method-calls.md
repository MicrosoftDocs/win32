...
When a call is marshaled, the proxy creates a cancel object, which implements the [**ICancelMethodCalls**](https://msdn.microsoft.com/en-us/library/ms683860(v=VS.85).aspx) interface. The cancel object is associated with both the call and the thread on which the call is pending.
