...
To solve this problem, the outer object aggregating the free-threaded marshaler should call [**IGlobalInterfaceTable::RegisterInterfaceInGlobal**](https://msdn.microsoft.com/en-us/library/ms682465(v=VS.85).aspx) on the inner interface and store the resulting cookie in its member variable, instead of storing the actual interface pointer. When the outer object wants to call on an inner object's interface pointer, it should call [**IGlobalInterfaceTable::GetInterfaceFromGlobal**](https://msdn.microsoft.com/en-us/library/ms680695(v=VS.85).aspx), use the returned interface pointer, and then release it. When the outer object goes away, it should call [**IGlobalInterfaceTable::RevokeInterfaceFromGlobal**](https://msdn.microsoft.com/en-us/library/ms679756(v=VS.85).aspx) to remove the interface from the global interface table.
