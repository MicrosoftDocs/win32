...
Clients can also specify Snego when they call [**CoInitializeSecurity**](coinitializesecurity.md). The *dwAuthnSvc* and *pAuthInfo* parameters of [**CoSetProxyBlanket**](cosetproxyblanket.md) become members of a [**SOLE\_AUTHENTICATION\_INFO**](sole-authentication-info.md) structure that is passed to **CoInitializeSecurity** through its *pAuthList* parameter. The details of the values of those members are the same as described in the preceding paragraph.

If Snego is used, calls to [**CoQueryProxyBlanket**](coqueryproxyblanket.md) or [**CoQueryClientBlanket**](coqueryclientblanket.md) will return Snego as the authentication service, rather than the actual authentication service that Snego picked for establishing the connection.
