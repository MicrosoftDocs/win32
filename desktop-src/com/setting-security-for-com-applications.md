...
There are several ways you can help control security for your applications. You can change a computer's default security settings, which are used by all applications on the computer that do not supply their own security values. You can set security for a particular process, either by using Dcomcnfg.exe or by calling [**CoInitializeSecurity**](coinitializesecurity.md). You can also programmatically control security settings at the interface proxy level.
