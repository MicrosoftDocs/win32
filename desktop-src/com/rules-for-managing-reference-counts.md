...
The default case is that [**AddRef**](/windows/desktop/api) must be called for every new copy of an interface pointer and [**Release**](/windows/desktop/api) must be called for every destruction of an interface pointer, except where the following rules permit otherwise:

-   **In-out parameters to functions.** The caller must call [**AddRef**](/windows/desktop/api) on the parameter because it will be released (with a call to [**Release**](/windows/desktop/api)) in the implementing code when the out value is stored on top of it.
-   **Fetching a global variable.** When creating a local copy of an interface pointer from an existing copy of the pointer in a global variable, you must call [**AddRef**](/windows/desktop/api) on the local copy because another function might destroy the copy in the global variable while the local copy is still valid.
-   **New pointers synthesized out of "thin air."** A function that synthesizes an interface pointer using special internal knowledge rather than obtaining it from some other source must call [**AddRef**](/windows/desktop/api) initially on the newly synthesized pointer. Important examples of such routines include instance creation routines, implementations of [**QueryInterface**](/windows/desktop/api/Unknwn/nf-unknwn-iunknown-queryinterface(q_)), and so on.
-   **Retrieving a copy of an internally stored pointer.** When a function retrieves a copy of a pointer that is stored internally by the object called, that object's code must call [**AddRef**](/windows/desktop/api) on the pointer before the function returns. Once the pointer has been retrieved, the originating object has no other way of determining how its lifetime relates to that of the internally stored copy of the pointer.
...
-   When one copy of a pointer already exists and a second is created subsequently and then is destroyed while the first copy still exists, calls to [**AddRef**](/windows/desktop/api) and [**Release**](/windows/desktop/api)for the second copy can be omitted.
-   When one copy of a pointer exists and a second is created and then the first is destroyed before the second, the calls to [**AddRef**](/windows/desktop/api)for the second copy and to [**Release**](/windows/desktop/api) for the first copy can be omitted.
...
-   **Local variables.** A method implementation has control of the lifetimes of each of the pointer variables allocated on the stack frame and can use this to determine how to omit redundant [**AddRef**](/windows/desktop/api)/[**Release**](/windows/desktop/api) pairs.
...
When implementing or using reference-counted objects, it may be useful to apply *artificial reference counts*, which guarantee object stability during processing of a function. In implementing a method of an interface, you might call functions that have a chance of decrementing your reference count to an object, causing a premature release of the object and failure of the implementation. A robust way to avoid this is to insert a call to [**AddRef**](/windows/desktop/api) at the beginning of the method implementation and pair it with a call to [**Release**](/windows/desktop/api) just before the method returns.

In some situations, the return values of [**AddRef**](/windows/desktop/api) and [**Release**](/windows/desktop/api) may be unstable and should not be relied upon; they should be used only for debugging or diagnostic purposes.
