...
1.  Query the server object for the [**ICallFactory**](/windows/win32/objidlbase/nn-objidl-icallfactory?branch=master) interface. If [**QueryInterface**](/windows/win32/Unknwn/nf-unknwn-iunknown-queryinterface(q,)?branch=master) returns E\_NOINTERFACE, the server object does not support asynchronous calling.

2.  Call [**ICallFactory::CreateCall**](/windows/win32/objidlbase/nf-objidl-icallfactory-createcall?branch=master) to create a call object corresponding to the interface you want, and then release the pointer to [**ICallFactory**](/windows/win32/objidlbase/nn-objidl-icallfactory?branch=master).

3.  If you did not request a pointer to the asynchronous interface from the call to [**CreateCall**](/windows/win32/objidlbase/nf-objidl-icallfactory-createcall?branch=master), query the call object for the asynchronous interface.
...
The mechanism for this signaling and checking between client and server is the [**ISynchronize**](/windows/win32/objidlbase/nn-objidl-isynchronize?branch=master) interface on the call object. The call object normally implements this interface by aggregating a system-supplied synchronization object. The synchronization object wraps an event handle, which the server signals just before returning from the Begin\_ method by calling [**ISynchronize::Signal**](/windows/win32/objidlbase/nf-objidl-isynchronize-signal?branch=master).

**To complete an asynchronous call**

1.  Query the call object for the [**ISynchronize**](/windows/win32/objidlbase/nn-objidl-isynchronize?branch=master) interface.

2.  Call [**ISynchronize::Wait**](/windows/win32/objidlbase/nf-objidl-isynchronize-wait?branch=master).

3.  If [**Wait**](/windows/win32/objidlbase/nf-objidl-isynchronize-wait?branch=master) returns RPC\_E\_TIMEOUT, the Begin\_ method is not finished processing. The client can continue with other work and call **Wait** again later. It cannot call the Finish\_ method until **Wait** returns S\_OK.

    If [**Wait**](/windows/win32/objidlbase/nf-objidl-isynchronize-wait?branch=master) returns S\_OK, the Begin\_ method has returned. Call the appropriate Finish\_ method.
...
> [!Note]  
> If the client and server objects are in the same apartment, calls to [**ICallFactory::CreateCall**](/windows/win32/objidlbase/nf-objidl-icallfactory-createcall?branch=master) are not guaranteed to succeed. If the server object does not support asynchronous calling on a particular interface, the attempt to create a call object will fail and the client must use the synchronous interface.
