...
1.  Query the server object for the [**ICallFactory**](icallfactory.md) interface. If [**QueryInterface**](iunknown-queryinterface.md) returns E\_NOINTERFACE, the server object does not support asynchronous calling.

2.  Call [**ICallFactory::CreateCall**](icallfactory-createcall.md) to create a call object corresponding to the interface you want, and then release the pointer to [**ICallFactory**](icallfactory.md).

3.  If you did not request a pointer to the asynchronous interface from the call to [**CreateCall**](icallfactory-createcall.md), query the call object for the asynchronous interface.
...
The mechanism for this signaling and checking between client and server is the [**ISynchronize**](isynchronize.md) interface on the call object. The call object normally implements this interface by aggregating a system-supplied synchronization object. The synchronization object wraps an event handle, which the server signals just before returning from the Begin\_ method by calling [**ISynchronize::Signal**](isynchronize-signal.md).

**To complete an asynchronous call**

1.  Query the call object for the [**ISynchronize**](isynchronize.md) interface.

2.  Call [**ISynchronize::Wait**](isynchronize-wait.md).

3.  If [**Wait**](isynchronize-wait.md) returns RPC\_E\_TIMEOUT, the Begin\_ method is not finished processing. The client can continue with other work and call **Wait** again later. It cannot call the Finish\_ method until **Wait** returns S\_OK.

    If [**Wait**](isynchronize-wait.md) returns S\_OK, the Begin\_ method has returned. Call the appropriate Finish\_ method.
...
> [!Note]  
> If the client and server objects are in the same apartment, calls to [**ICallFactory::CreateCall**](icallfactory-createcall.md) are not guaranteed to succeed. If the server object does not support asynchronous calling on a particular interface, the attempt to create a call object will fail and the client must use the synchronous interface.
