...
If a control has any persistent state whatsoever, it must, as a minimum, implement either [**IPersistStream**](ipersiststream.md) or [**IPersistStreamInit**](ipersiststreaminit.md) (the two are mutually exclusive and shouldn't be implemented together for the most part). The latter is used when a control wishes to know when it is created new as opposed to reloaded from an existing persistent state (**IPersistStream** does not have the created new capability). The existence of either interface indicates that the control can save and load its persistent state into a stream, that is, an instance of [**IStream**](https://msdn.microsoft.com/library/windows/desktop/aa380034).
...
| [**IPersistMoniker**](_inet_IPersistMoniker_Interface_cpp)<br/>  | The object can save and load its state to a location named by a moniker. The control calls [**IMoniker::BindToStorage**](imoniker-bindtostorage.md) to retrieve the storage interface it requires, such as [**IStorage**](https://msdn.microsoft.com/library/windows/desktop/aa380015), [**IStream**](https://msdn.microsoft.com/library/windows/desktop/aa380034), [**ILockBytes**](https://msdn.microsoft.com/library/windows/desktop/aa379238), [**IDataObject**](idataobject.md), etc.<br/> |
...
With the exception of [**IPersistStream::GetSizeMax**](ipersiststream-getsizemax.md), [**IPersistStreamInit::GetSizeMax**](ipersiststreaminit-getsizemax.md), and [**IPersistMemory::GetSizeMax**](_inet_IPersistMemory_GetSizeMax_Method), all methods of each interface must be fully implemented.
