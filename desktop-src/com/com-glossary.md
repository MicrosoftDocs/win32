...
A COM object that implements, at a minimum, the [**IHlink**](_inet_IHlink_Interface_cpp) interface and acts as a link to an object at another location (the target). A hyperlink is made up of four parts: a moniker that identifies the target's location; a string for the location within the target; a friendly, or displayable, name for the target; and a string that can contain additional parameters.
...
A COM object that implements the [**IHlinkBrowseContext**](_inet_IHlinkBrowseContext_Interface_cpp) interface and maintains the hyperlink navigation stack. The browse context object manages the hyperlink frame window and hyperlink target object's window.
...
A container application that supports hyperlinks by implementing the [**IHlinkSite**](_inet_IHlinkSite_Interface_cpp) interface and, if the container's objects can be targets of other hyperlinks, the [**IHlinkTarget**](_inet_IHlinkTarget_Interface_cpp) interface.
...
A COM object that implements the [**IHlinkFrame**](_inet_IHlinkFrame_Interface_cpp) interface and controls the top-level navigation and display of hyperlinks for the frame's container and the hyperlink target's server.
...
A COM object that implements the [**IHlinkSite**](_inet_IHlinkSite_Interface_cpp) interface and supplies either the moniker or interface identifier of its hyperlink container. One hyperlink site can serve multiple hyperlinks.
...
A COM object that implements the [**IHlinkTarget**](_inet_IHlinkTarget_Interface_cpp) interface and supplies its moniker, friendly name, and other information that other hyperlink objects will use to navigate to it.
...
An in-process COM object that implements [**IShellFolder**](_win32_IShellFolder_cpp), [**IPersistFolder**](_win32_IPersistFolder_cpp), and [**IShellView**](_win32_IShellView_cpp), which are sometimes referred to as the namespace extension interfaces. A namespace extension is used either to extend the shell's namespace or to create a separate namespace. Primary users are the Windows Explorer and common file dialog boxes.
