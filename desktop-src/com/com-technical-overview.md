...
All COM interfaces inherit from the [**IUnknown**](/windows/desktop/api/Unknwn/nn-unknwn-iunknown) interface. The **IUnknown** interface contains the fundamental COM operations for polymorphism and instance lifetime management. The **IUnknown** interface has three member functions, named [**QueryInterface**](/windows/desktop/api/Unknwn/nf-unknwn-iunknown-queryinterface(q_)), [**AddRef**](https://msdn.microsoft.com/library/ms691379(v=VS.85).aspx), and [**Release**](https://msdn.microsoft.com/library/ms682317(v=VS.85).aspx). All COM objects are required to implement the **IUnknown** interface.
