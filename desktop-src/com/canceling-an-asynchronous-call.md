...
A client can cancel an asynchronous call that is in progress if the call object implements the [**ICancelMethodCalls**](icancelmethodcalls.md) interface. For objects that use standard marshaling, **ICancelMethodCalls** is always available for marshaled calls. For objects that use custom marshaling or for calls to server objects within the same apartment, this functionality is available only if the call object implements **ICancelMethodCalls**.
...
1.  Query the call object for [**ICancelMethodCalls**](icancelmethodcalls.md).

2.  Call [**ICancelMethodCalls::Cancel**](icancelmethodcalls-cancel.md), and then call [**Release**](iunknown-release.md) to release the pointer obtained by the [**QueryInterface**](iunknown-queryinterface.md) call in step 1.
