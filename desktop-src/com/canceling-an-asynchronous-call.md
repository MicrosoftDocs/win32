...
A client can cancel an asynchronous call that is in progress if the call object implements the [**ICancelMethodCalls**](/windows/win32/objidlbase/nn-objidl-icancelmethodcalls?branch=master) interface. For objects that use standard marshaling, **ICancelMethodCalls** is always available for marshaled calls. For objects that use custom marshaling or for calls to server objects within the same apartment, this functionality is available only if the call object implements **ICancelMethodCalls**.
...
1.  Query the call object for [**ICancelMethodCalls**](/windows/win32/objidlbase/nn-objidl-icancelmethodcalls?branch=master).

2.  Call [**ICancelMethodCalls::Cancel**](/windows/win32/objidlbase/nf-objidl-icancelmethodcalls-cancel?branch=master), and then call [**Release**](/windows/win32/unknwnbase/nf-unknwn-iunknown-release?branch=master) to release the pointer obtained by the [**QueryInterface**](/windows/win32/Unknwn/nf-unknwn-iunknown-queryinterface(q,)?branch=master) call in step 1.
