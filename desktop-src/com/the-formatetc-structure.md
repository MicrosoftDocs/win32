...
The FORMATETC structure is a generalized clipboard format, enhanced to encompass a target device, an *aspect* or view of the data, and a storage medium. A data consumer, such as an OLE container application, passes the [**FORMATETC**](/windows/win32/ObjIdl/ns-objidl-tagformatetc?branch=master) structure as an argument in calls to [**IDataObject**](/windows/win32/ObjIdl/nn-objidl-idataobject?branch=master) to indicate the type of data it wants from a data source, such as a compound document object. The source uses the **FORMATETC** structure to describe what formats it can provide.

[**FORMATETC**](/windows/win32/ObjIdl/ns-objidl-tagformatetc?branch=master) can describe virtually any data, including other objects such as monikers. A container can ask one of its embedded objects to list its data formats by calling [**IDataObject::EnumFormatEtc**](/windows/win32/ObjIdl/nf-objidl-idataobject-enumformatetc?branch=master), which returns an enumerator object that implements the [**IEnumFORMATETC**](/windows/win32/ObjIdl/nn-objidl-ienumformatetc?branch=master) interface. Instead of replying merely that it has "text and a bitmap," the object can provide a detailed description of the data, including the device (normally screen or printer) for which it is rendered, the aspect to be presented to the user (full contents, thumbnail, icon, or formatted for printing), and the storage medium containing the data (global memory, disk file, storage object, or stream). This ability to tightly describe data will, in time, result in higher quality printer and screen output as well as more efficiency in data browsing, where a thumbnail sketch is much faster to retrieve and display than a fully detailed rendering.

The following table lists fields of the [**FORMATETC**](/windows/win32/ObjIdl/ns-objidl-tagformatetc?branch=master) data structure and the information that they specify.
...
| **ptd**<br/>      | A [**DVTARGETDEVICE**](/windows/win32/ObjIdl/ns-objidl-tagdvtargetdevice?branch=master) structure, which contains enough information about a Windows target device, such as a screen or printer, so that a handle to its device context (hDC) can be created using the [**CreateDC**](https://msdn.microsoft.com/library/windows/desktop/dd183490) function. <br/> |
