...
COM provides a way for any apartment in a process to get access to an interface implemented on an object in any other apartment in the process. This is done through the [**IGlobalInterfaceTable**](iglobalinterfacetable.md) interface. This interface has three methods, which allow you to do the following:
...
The [**IGlobalInterfaceTable**](iglobalinterfacetable.md) interface is an efficient way for a process to store an interface pointer in a memory location that can be accessed from multiple apartments within the process, such as process-wide variables and *agile* objects (free-threaded, marshaled objects) containing interface pointers to other objects.
