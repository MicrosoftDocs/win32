...
The methods of a COM interface always provide memory management of pointers to the interface by calling the [**AddRef**](iunknown-addref.md) and [**Release**](iunknown-release.md) functions found in the [**IUnknown**](iunknown.md) interface, from which all other COM interfaces derive. (See [Rules for Managing Reference Counts](rules-for-managing-reference-counts.md) for more information.)
