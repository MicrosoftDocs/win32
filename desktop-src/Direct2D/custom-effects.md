...
Conceptually, a [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) effect performs an imaging task, like changing brightness, de-saturating an image, or as shown above, creating a drop shadow. To the app, they are simple. They can accept zero or more input images, expose multiple properties that control their operation, and generate a single output image.
...
The [**ID2D1EffectImpl**](https://msdn.microsoft.com/en-us/library/Hh404568(v=VS.85).aspx) interface contains three methods that you must implement:

### Initialize(ID2D1EffectContext \*pContextInternal, ID2D1TransformGraph \*pTransformGraph)

[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) calls the [**Initialize**](https://msdn.microsoft.com/en-us/library/Hh404570(v=VS.85).aspx) method after the [**ID2D1DeviceContext::CreateEffect**](https://msdn.microsoft.com/en-us/library/Hh404500(v=VS.85).aspx) method has been called by the app. You can use this method to perform internal initialization or any other operations needed for the effect. Additionally, you can use it to create the effect's initial transform graph.

### SetGraph(ID2D1TransformGraph \*pTransformGraph)

[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) calls the [**SetGraph**](https://msdn.microsoft.com/en-us/library/Hh871459(v=VS.85).aspx) method when the number of inputs to the effect is changed. While most effects have a constant number of inputs, others like the [Composite effect](composite.md) support a variable number of inputs. This method allows these effects to update their transform graph in response to a changing input count. If an effect does not support a variable input count, this method can simply return E\_NOTIMPL.

### PrepareForRender (D2D1\_CHANGE\_TYPE changeType)

The [**PrepareForRender**](https://msdn.microsoft.com/en-us/library/Hh404572(v=VS.85).aspx) method provides an opportunity for effects to perform any operations in response to external changes. [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) calls this method just before it renders an effect if at least one of these is true:

-   The effect has been previously initialized but not yet drawn.
-   An effect property has changed since the last draw call.
-   The state of the calling [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) context (like DPI) has changed since the last draw call.

### Implement the effect registration and callback methods

Apps must register effects with [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) before instantiating them. This registration is scoped to an instance of a Direct2D factory, and must be repeated each time the app is run. To enable this registration, a custom effect defines a unique GUID, a public method that registers the effect, and a private callback method that returns an instance of the effect.

### Define a GUID

You must define a GUID that uniquely identifies the effect for registration with [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx). The app uses the same to identify the effect when it calls [**ID2D1DeviceContext::CreateEffect**](https://msdn.microsoft.com/en-us/library/Hh404500(v=VS.85).aspx).
...
Next, define a public method for the app to call to register the effect with [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx). Because effect registration is specific to an instance of a Direct2D factory, the method accepts an [**ID2D1Factory1**](https://msdn.microsoft.com/en-us/library/Hh404596(v=VS.85).aspx) interface as a parameter. To register the effect, the method then calls the [**ID2D1Factory1::RegisterEffectFromString**](https://msdn.microsoft.com/en-us/library/Hh404614(v=VS.85).aspx) API on the **ID2D1Factory1** parameter.

This API accepts an XML string that describes the metadata, inputs, and properties of the effect. The metadata for an effect is for informational purposes only, and can be queried by the app through the [**ID2D1Properties**](https://msdn.microsoft.com/en-us/library/Hh446854(v=VS.85).aspx) interface. The input and property data, on the other hand, is used by [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) and represents the effect's functionality.
...
The effect must also provide a private callback method that returns an instance of the effect through a single IUnknown\*\* parameter. A pointer to this method is provided to [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) when the effect is registered via the [**ID2D1Factory1::RegisterEffectFromString**](https://msdn.microsoft.com/en-us/library/Hh404614(v=VS.85).aspx) API through the [**PD2D1\_EFFECT\_FACTORY**](/windows/desktop/api/D2D1_1/nc-d2d1_1-pd2d1_effect_factory)\\ parameter.
...
An effect can use several different transforms (individual image operations) to create its desired imaging effect. To control the order in which these transforms are applied to the input image, the effect arranges them into a transform graph. A transform graph can make use of the effects and transforms included in [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) as well as custom transforms created by the effect author.

### Using transforms included with Direct2D

These are the most commonly used transforms provided with [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx).

-   [**ID2D1BlendTransform**](https://msdn.microsoft.com/en-us/library/Hh404361(v=VS.85).aspx): This transform blends an arbitrary number of inputs together based on a blend description, see the [**D2D1\_BLEND\_DESCRIPTION**](/windows/desktop/api/D2d1effectauthor/ns-d2d1effectauthor-d2d1_blend_description) topic and the [Direct2D Composite Modes sample](http://go.microsoft.com/fwlink/p/?linkid=231580) for blending examples. This transform is returned by the [**ID2D1EffectContext::CreateBlendTransform**](https://msdn.microsoft.com/en-us/library/Hh404461(v=VS.85).aspx) method.
-   [**ID2D1BorderTransform**](https://msdn.microsoft.com/en-us/library/Hh404367(v=VS.85).aspx): This transform expands an image's output size according to the [**D2D1\_EXTEND\_MODE**](/windows/desktop/api/d2d1/ne-d2d1-d2d1_extend_mode) specified. This transform is returned by the [**ID2D1EffectContext::CreateBorderTransform**](https://msdn.microsoft.com/en-us/library/Hh404463(v=VS.85).aspx) method.
-   [**ID2D1OffsetTransform**](https://msdn.microsoft.com/en-us/library/Hh446820(v=VS.85).aspx): This transform offsets (translates) its input by any whole number of pixels. This transform is returned by the [**ID2D1EffectContext::CreateOffsetTransform**](https://msdn.microsoft.com/en-us/library/Hh404468(v=VS.85).aspx) method.
-   Any existing effect can be treated as a transform for the purposes of transform graph creation by using the [**ID2D1EffectContext::CreateTransformNodeFromEffect**](https://msdn.microsoft.com/en-us/library/Hh404470(v=VS.85).aspx) method.

### Creating a single-node transform graph

Once you create a transform, the effect's input needs to be connected to the transform's input, and the transform's output needs to be connected to the effect's output. When an effect only contains a single transform, you can use the [**ID2D1TransformGraph::SetSingleTransformNode**](https://msdn.microsoft.com/en-us/library/Hh446935(v=VS.85).aspx) method to easily accomplish this.

You can create or modify a transform in the effect's [**Initialize**](https://msdn.microsoft.com/en-us/library/Hh404570(v=VS.85).aspx) or [**SetGraph**](https://msdn.microsoft.com/en-us/library/Hh871459(v=VS.85).aspx) methods using the provided ID2D1TransformGraph parameter. If an effect needs to make changes to the transform graph in another method where this parameter is not available, the effect can save the [**ID2D1TransformGraph**](https://msdn.microsoft.com/en-us/library/Hh446920(v=VS.85).aspx) parameter as a member variable of the class and access it elsewhere, such as [**PrepareForRender**](https://msdn.microsoft.com/en-us/library/Hh404572(v=VS.85).aspx) or a custom property callback method.

A sample [**Initialize**](https://msdn.microsoft.com/en-us/library/Hh404570(v=VS.85).aspx) method is shown here. This method creates a single-node transform graph that offsets the image by one hundred pixels in each axis.
...
As noted above, the effect's transform graph may be edited in any effect method using the [**ID2D1TransformGraph**](https://msdn.microsoft.com/en-us/library/Hh446920(v=VS.85).aspx) parameter received in the effect's [**Initialize**](https://msdn.microsoft.com/en-us/library/Hh404570(v=VS.85).aspx) method. The following APIs on that interface can be used to create or modify an effect's transform graph:

### AddNode(ID2D1TransformNode \*pNode)

The [**AddNode**](https://msdn.microsoft.com/en-us/library/Hh446922(v=VS.85).aspx) method, in effect, 'registers' the transform with the effect, and must be called before the transform can be used with any of the other transform graph methods.

### ConnectToEffectInput(UINT32 toEffectInputIndex, ID2D1TransformNode \*pNode, UINT32 toNodeInputIndex)

The [**ConnectToEffectInput**](https://msdn.microsoft.com/en-us/library/Hh446928(v=VS.85).aspx) method connects the effect's image input to a transform's input. The same effect input can be connected to multiple transforms.

### ConnectNode(ID2D1TransformNode \*pFromNode, ID2D1TransformNode \*pToNode, UINT32 toNodeInputIndex)

The [**ConnectNode**](https://msdn.microsoft.com/en-us/library/Hh446926(v=VS.85).aspx) method connects a transform's output to another transform's input. A transform output can be connected to multiple transforms.

### SetOutputNode(ID2D1TransformNode \*pNode)

The [**SetOutputNode**](https://msdn.microsoft.com/en-us/library/Hh446932(v=VS.85).aspx) method connects a transform's output to the effect's output. Because an effect only has one output, only a single transform can be designated as the 'output node'.
...
You must define a custom effect's properties during the effect's initial registration with [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx). First, you must update the effect's registration XML in its public registration method with the new property:
...
For each property, an effect can optionally specify default, min, and max values. These values are for informational use only. They are not enforced by [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx). It is up to you to implement any specified default/min/max logic in the effect class yourself.
...
| [**D2D\_MATRIX\_3X2\_F**](https://msdn.microsoft.com/en-us/library/Dd316742(v=VS.85).aspx) | matrix3x2               |
...
Once you create the XML and bindings array, pass them into the [**RegisterEffectFromString**](https://msdn.microsoft.com/en-us/library/Hh404614(v=VS.85).aspx) method:
...
The D2D1\_VALUE\_TYPE\_BINDING macro requires the effect class to inherit from [**ID2D1EffectImpl**](https://msdn.microsoft.com/en-us/library/Hh404568(v=VS.85).aspx) before any other interface.
...
To actually update an effect's image output in response to a property change, the effect needs to change its underlying transforms. This is typically done in the effect's [**PrepareForRender**](https://msdn.microsoft.com/en-us/library/Hh404572(v=VS.85).aspx) method which [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) automatically calls when one of an effect's properties has been changed. However, transforms can be updated in any of the effect's methods: such as Initialize or the effect's property setter methods.

For example, if an effect contained an [**ID2D1OffsetTransform**](https://msdn.microsoft.com/en-us/library/Hh446820(v=VS.85).aspx) and wanted to modify its offset value in response to the effect's Offset property being changed, it would add the following code in [**PrepareForRender**](https://msdn.microsoft.com/en-us/library/Hh404572(v=VS.85).aspx):
...
To implement image operations beyond what is provided in [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx), you must implement custom transforms. Custom transforms can arbitrarily change an input image through the use of custom HLSL shaders.

Transforms implement one of two different interfaces depending on the types of shaders they use. Transforms using pixel and/or vertex shaders must implement [**ID2D1DrawTransform**](https://msdn.microsoft.com/en-us/library/Hh847992(v=VS.85).aspx), while transforms using compute shaders must implement [**ID2D1ComputeTransform**](https://msdn.microsoft.com/en-us/library/Hh404434(v=VS.85).aspx). These interfaces both inherit from [**ID2D1Transform**](https://msdn.microsoft.com/en-us/library/Hh446919(v=VS.85).aspx). This section focuses on implementing the functionality that is common to both.

The [**ID2D1Transform**](https://msdn.microsoft.com/en-us/library/Hh446919(v=VS.85).aspx) interface has four methods to implement:
...
[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) calls the [**MapInputRectsToOutputRect**](https://msdn.microsoft.com/en-us/library/Hh446943(v=VS.85).aspx) method each time the transform is rendered. Direct2D passes a rectangle representing the bounds of each of the inputs to the transform. The transform is then responsible for calculating the bounds of the output image. The size of the rectangles for all the methods on this interface ([**ID2D1Transform**](https://msdn.microsoft.com/en-us/library/Hh446919(v=VS.85).aspx)) are defined in pixels, not DIPs.

This method is also responsible for calculating the region of the output that is opaque based on the logic of its shader and the opaque regions of each input. An opaque region of an image is defined as that where the alpha channel is '1' for the entirety of the rectangle. If it is unclear whether a transform's output is opaque, the output opaque rectangle should be set to (0, 0, 0, 0) as a safe value. [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) uses this info to perform rendering optimizations with 'guaranteed opaque' content. If this value is inaccurate, it can result in incorrect rendering.
...
For more info about this method, see the [**MapInputRectsToOutputRect**](https://msdn.microsoft.com/en-us/library/Hh446943(v=VS.85).aspx) reference page.

### MapOutputRectToInputRects

[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) calls the [**MapOutputRectToInputRects**](https://msdn.microsoft.com/en-us/library/Hh446945(v=VS.85).aspx) method after [**MapInputRectsToOutputRect**](https://msdn.microsoft.com/en-us/library/Hh446943(v=VS.85).aspx). The transform must calculate what portion of the image it needs to read from in order to correctly render the requested output region.
...
This figure visualizes the calculation. [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) automatically samples transparent black pixels where the input image doesn't exist, allowing the blur to be blended gradually with the existing content on the screen.
...
For more info about this method, see the [**MapOutputRectToInputRects**](https://msdn.microsoft.com/en-us/library/Hh446945(v=VS.85).aspx) topic.

### MapInvalidRect

[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) also calls the [**MapInvalidRect**](https://msdn.microsoft.com/en-us/library/Hh446946(v=VS.85).aspx) method. However, unlike the [**MapInputRectsToOutputRect**](https://msdn.microsoft.com/en-us/library/Hh446943(v=VS.85).aspx) and [**MapOutputRectToInputRects**](https://msdn.microsoft.com/en-us/library/Hh446945(v=VS.85).aspx) methods Direct2D is not guaranteed to call it at any particular time. This method conceptually decides what part of a transform's output needs to be re-rendered in response to part or all of its input changing. There are three different scenarios for which to calculate a transform's invalid rect.
...
For more info about this method, see the [**MapInvalidRect**](https://msdn.microsoft.com/en-us/library/Hh446946(v=VS.85).aspx) topic.
...
To use a pixel shader, the transform must implement the [**ID2D1DrawTransform**](https://msdn.microsoft.com/en-us/library/Hh847992(v=VS.85).aspx) interface, which inherits from the [**ID2D1Transform**](https://msdn.microsoft.com/en-us/library/Hh446919(v=VS.85).aspx) interface described in section 5. This interface contains one new method to implement:

### SetDrawInfo(ID2D1DrawInfo \*pDrawInfo)

[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) calls the [**SetDrawInfo**](https://msdn.microsoft.com/en-us/library/Hh847993(v=VS.85).aspx) method when the transform is first added to an effect's transform graph. This method provides an [**ID2D1DrawInfo**](https://msdn.microsoft.com/en-us/library/Hh847986(v=VS.85).aspx) parameter that controls how the transform is rendered. See the **ID2D1DrawInfo** topic for the methods available here.

If the transform chooses to store this parameter as a class member variable, the *drawInfo* object can be accessed and changed from other methods such as property setters or [**MapInputRectsToOutputRect**](https://msdn.microsoft.com/en-us/library/Hh446943(v=VS.85).aspx). Notably, it cannot be called from the [**MapOutputRectToInputRects**](https://msdn.microsoft.com/en-us/library/Hh446945(v=VS.85).aspx) or [**MapInvalidRect**](https://msdn.microsoft.com/en-us/library/Hh446946(v=VS.85).aspx) methods on [**ID2D1Transform**](https://msdn.microsoft.com/en-us/library/Hh446919(v=VS.85).aspx).

### Creating a GUID for the pixel shader

Next, the transform must define a unique GUID for the pixel shader itself. This is used when [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) loads the shader into memory, as well as when the transform chooses which pixel shader to use for execution. Tools such as guidgen.exe, which is included with Visual Studio, can be used to generate a random GUID.
...
After a pixel shader has been loaded into memory, the transform needs to select it for execution by passing its GUID to the [**SetPixelShader**](https://msdn.microsoft.com/en-us/library/Hh847988(v=VS.85).aspx) method on the [**ID2D1DrawInfo**](https://msdn.microsoft.com/en-us/library/Hh847986(v=VS.85).aspx) parameter provided during the [**SetDrawInfo**](https://msdn.microsoft.com/en-us/library/Hh847993(v=VS.85).aspx) method. The pixel shader must be already loaded into memory before being selected for execution.
...
[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) transforms use shaders authored using standard [HLSL](https://msdn.microsoft.com/library/windows/desktop/bb509561). However, there are a few key concepts to writing a pixel shader that executes from the context of a transform. For a completed example of a fully functionally pixel shader, see the [D2DCustomEffects SDK sample](http://go.microsoft.com/fwlink/p/?LinkId=620531).
...
After creating a vertex buffer in memory, the transform uses the [**CreateVertexBuffer**](https://msdn.microsoft.com/en-us/library/Hh404471(v=VS.85).aspx) method on the containing effect's [**ID2D1EffectContext**](https://msdn.microsoft.com/en-us/library/Hh404459(v=VS.85).aspx) object to pass that data to the GPU. Again, see the [D2DCustomEffects SDK sample](http://go.microsoft.com/fwlink/p/?LinkId=620531) for a sample implementation.

If no vertex buffer is specified by the transform, [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) passes a default vertex buffer representing the rectangular image location.

### Changing SetDrawInfo to utilize a vertex shader

Like with pixel shaders, the transform must load and select a vertex shader for execution. To load the vertex shader, it calls the [**LoadVertexShader**](https://msdn.microsoft.com/en-us/library/Hh404477(v=VS.85).aspx) method on the [**ID2D1EffectContext**](https://msdn.microsoft.com/en-us/library/Hh404459(v=VS.85).aspx) method received in the effect's Initialize method. To select the vertex shader for execution, it calls [**SetVertexProcessing**](https://msdn.microsoft.com/en-us/library/Hh847990(v=VS.85).aspx) on the [**ID2D1DrawInfo**](https://msdn.microsoft.com/en-us/library/Hh847986(v=VS.85).aspx) parameter received in the transform's [**SetDrawInfo**](https://msdn.microsoft.com/en-us/library/Hh847993(v=VS.85).aspx) method. This method accepts a GUID for a previously-loaded vertex shader as well as (optionally) a previously-created vertex buffer for the shader to execute on.
...
On the other hand, if a transform only contains a vertex shader, and relies on [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx)'s default pass-through pixel shader, it must return the following default output:
...
A vertex shader stores the result of its vertex transformations in the shader's Scene-space output variable. To compute the Clip-space output and the Texel-space input variables, [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) automatically provides conversion matrices in a constant buffer:
...
Sample vertex shader code can be found below that uses the conversion matrices to calculate the correct clip and texel spaces expected by [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx):
...
If no vertex buffer is specified by the transform, [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) substitutes in a default vertex buffer representing the rectangular image location. The parameters to the vertex shader are changed to those of the default shader output:
...
This interface contains two new methods to implement in addition to the ones in [**ID2D1Transform**](https://msdn.microsoft.com/en-us/library/Dd371304(v=VS.85).aspx):

### SetComputeInfo(ID2D1ComputeInfo \*pComputeInfo)

Like with pixel and vertex shaders, [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) calls the [**SetComputeInfo**](https://msdn.microsoft.com/en-us/library/Hh404450(v=VS.85).aspx) method when the transform is first added to an effect's transform graph. This method provides an [**ID2D1ComputeInfo**](https://msdn.microsoft.com/en-us/library/Hh847966(v=VS.85).aspx) parameter that controls how the transform is rendered. This includes choosing the compute shader to execute through the [**ID2D1ComputeInfo::SetComputeShader**](https://msdn.microsoft.com/en-us/library/Hh847968(v=VS.85).aspx) method. If the transform chooses to store this parameter as a class member variable, it can be accessed and changed from any transform or effect method with the exception of the [**MapOutputRectToInputRects**](https://msdn.microsoft.com/en-us/library/Hh446945(v=VS.85).aspx) and [**MapInvalidRect**](https://msdn.microsoft.com/en-us/library/Hh446946(v=VS.85).aspx) methods. See the **ID2D1ComputeInfo** topic for other methods available here.
...
However, like pixel shaders, the image's data is not guaranteed to begin at (0, 0) on the texture. Instead, [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) provides system constants that allow shaders to compensate for any offset:
...
[Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) expects a shader to define an output buffer for the resulting image to be placed. In Shader Model 4 (DirectX 10), this must be a single-dimensional buffer due to feature constraints:
...
With Shader Model 5 shaders, [Direct2D](https://msdn.microsoft.com/en-us/library/Dd370990(v=VS.85).aspx) provides an additional 'outputOffset' parameter in the constant buffer. The shader's output should be offsetted by this amount:
...
[D2DCustomEffects SDK sample](http://go.microsoft.com/fwlink/p/?LinkId=620531)
</dt> </dl>
