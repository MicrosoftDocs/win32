...
You must define a custom effect's properties during the effect's initial registration with [Direct2D](direct2d.direct2d_portal.xml). First, you must update the effect's registration XML in its public registration method with the new property:
...
For each property, an effect can optionally specify default, min, and max values. These values are for informational use only. They are not enforced by [Direct2D](direct2d.direct2d_portal.xml). It is up to you to implement any specified default/min/max logic in the effect class yourself.
...
This figure visualizes the calculation. [Direct2D](direct2d.direct2d_portal.xml) automatically samples transparent black pixels where the input image doesn't exist, allowing the blur to be blended gradually with the existing content on the screen.
...
Next, the transform must define a unique GUID for the pixel shader itself. This is used when [Direct2D](direct2d.direct2d_portal.xml) loads the shader into memory, as well as when the transform chooses which pixel shader to use for execution. Tools such as guidgen.exe, which is included with Visual Studio, can be used to generate a random GUID.
...
[Direct2D](direct2d.direct2d_portal.xml) transforms use shaders authored using standard [HLSL](https://msdn.microsoft.com/library/windows/desktop/bb509561). However, there are a few key concepts to writing a pixel shader that executes from the context of a transform. For a completed example of a fully functionally pixel shader, see the [D2DCustomEffects SDK sample](http://go.microsoft.com/fwlink/p/?LinkId=620531).

[Direct2D](direct2d.direct2d_portal.xml) automatically maps a transform's inputs to [**Texture2D**](https://msdn.microsoft.com/library/windows/desktop/ff471525) and [**SamplerState**](https://msdn.microsoft.com/library/windows/desktop/ff476588) objects in the HLSL. The first **Texture2D** is located at register t0, and the first **SamplerState** is located at register s0. Each additional input is located at the next corresponding registers (t1 and s1 for example). Pixel data for a particular input can be sampled by calling Sample on the **Texture2D** object and passing in the corresponding **SamplerState** object and the texel coordinates.
...
On the other hand, if a transform only contains a vertex shader, and relies on [Direct2D](direct2d.direct2d_portal.xml)'s default pass-through pixel shader, it must return the following default output:
...
A vertex shader stores the result of its vertex transformations in the shader's Scene-space output variable. To compute the Clip-space output and the Texel-space input variables, [Direct2D](direct2d.direct2d_portal.xml) automatically provides conversion matrices in a constant buffer:
...
Sample vertex shader code can be found below that uses the conversion matrices to calculate the correct clip and texel spaces expected by [Direct2D](direct2d.direct2d_portal.xml):
...
If no vertex buffer is specified by the transform, [Direct2D](direct2d.direct2d_portal.xml) substitutes in a default vertex buffer representing the rectangular image location. The parameters to the vertex shader are changed to those of the default shader output:
...
However, like pixel shaders, the image's data is not guaranteed to begin at (0, 0) on the texture. Instead, [Direct2D](direct2d.direct2d_portal.xml) provides system constants that allow shaders to compensate for any offset:
...
[Direct2D](direct2d.direct2d_portal.xml) expects a shader to define an output buffer for the resulting image to be placed. In Shader Model 4 (DirectX 10), this must be a single-dimensional buffer due to feature constraints:
...
With Shader Model 5 shaders, [Direct2D](direct2d.direct2d_portal.xml) provides an additional 'outputOffset' parameter in the constant buffer. The shader's output should be offsetted by this amount:
