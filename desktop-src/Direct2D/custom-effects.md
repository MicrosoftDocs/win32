...
The [**ID2D1EffectImpl**](/windows/desktop/api/d2d1effectauthor/) interface contains three methods that you must implement:
...
-   [**ID2D1OffsetTransform**](/windows/desktop/api/d2d1effectauthor/): This transform offsets (translates) its input by any whole number of pixels. This transform is returned by the [**ID2D1EffectContext::CreateOffsetTransform**](/windows/desktop/api/D2D1EffectAuthor/) method.
-   Any existing effect can be treated as a transform for the purposes of transform graph creation by using the [**ID2D1EffectContext::CreateTransformNodeFromEffect**](/windows/desktop/api/D2D1EffectAuthor/) method.

### Creating a single-node transform graph

Once you create a transform, the effect's input needs to be connected to the transform's input, and the transform's output needs to be connected to the effect's output. When an effect only contains a single transform, you can use the [**ID2D1TransformGraph::SetSingleTransformNode**](/windows/desktop/api/d2d1effectauthor/) method to easily accomplish this.

You can create or modify a transform in the effect's [**Initialize**](/windows/desktop/api/D2D1EffectAuthor/) or [**SetGraph**](/windows/desktop/api/D2D1EffectAuthor/) methods using the provided ID2D1TransformGraph parameter. If an effect needs to make changes to the transform graph in another method where this parameter is not available, the effect can save the [**ID2D1TransformGraph**](/windows/desktop/api/d2d1effectauthor/) parameter as a member variable of the class and access it elsewhere, such as [**PrepareForRender**](/windows/desktop/api/D2D1EffectAuthor/) or a custom property callback method.

A sample [**Initialize**](/windows/desktop/api/D2D1EffectAuthor/) method is shown here. This method creates a single-node transform graph that offsets the image by one hundred pixels in each axis.
...
As noted above, the effect's transform graph may be edited in any effect method using the [**ID2D1TransformGraph**](/windows/desktop/api/d2d1effectauthor/) parameter received in the effect's [**Initialize**](/windows/desktop/api/D2D1EffectAuthor/) method. The following APIs on that interface can be used to create or modify an effect's transform graph:

### AddNode(ID2D1TransformNode \*pNode)

The [**AddNode**](/windows/desktop/api/d2d1effectauthor/) method, in effect, 'registers' the transform with the effect, and must be called before the transform can be used with any of the other transform graph methods.

### ConnectToEffectInput(UINT32 toEffectInputIndex, ID2D1TransformNode \*pNode, UINT32 toNodeInputIndex)

The [**ConnectToEffectInput**](/windows/desktop/api/d2d1effectauthor/) method connects the effect's image input to a transform's input. The same effect input can be connected to multiple transforms.

### ConnectNode(ID2D1TransformNode \*pFromNode, ID2D1TransformNode \*pToNode, UINT32 toNodeInputIndex)

The [**ConnectNode**](/windows/desktop/api/d2d1effectauthor/) method connects a transform's output to another transform's input. A transform output can be connected to multiple transforms.

### SetOutputNode(ID2D1TransformNode \*pNode)

The [**SetOutputNode**](/windows/desktop/api/d2d1effectauthor/) method connects a transform's output to the effect's output. Because an effect only has one output, only a single transform can be designated as the 'output node'.
...
| [**D2D\_MATRIX\_3X2\_F**](/windows/desktop/api/d2d1/) | matrix3x2               |
...
| BYTE\[\]                                                                                                                               | blob                    |
| IUnknown\*                                                                                                                             | iunknown                |
| [**ID2D1ColorContext**](/windows/desktop/api/D2d1_1/)\*                                                                                       | colorcontext            |
| CLSID                                                                                                                                  | clsid                   |
...
Once you create the XML and bindings array, pass them into the [**RegisterEffectFromString**](/windows/desktop/api/D2d1_1/) method:
...
The D2D1\_VALUE\_TYPE\_BINDING macro requires the effect class to inherit from [**ID2D1EffectImpl**](/windows/desktop/api/d2d1effectauthor/) before any other interface.
...
The [**ID2D1Transform**](/windows/desktop/api/d2d1effectauthor/) interface has four methods to implement:
...
For more info about this method, see the [**MapInputRectsToOutputRect**](/windows/desktop/api/d2d1effectauthor/) reference page.
...
For more info about this method, see the [**MapOutputRectToInputRects**](/windows/desktop/api/d2d1effectauthor/) topic.
...
For more info about this method, see the [**MapInvalidRect**](/windows/desktop/api/d2d1effectauthor/) topic.
...
To use a pixel shader, the transform must implement the [**ID2D1DrawTransform**](/windows/desktop/api/d2d1effectauthor/) interface, which inherits from the [**ID2D1Transform**](/windows/desktop/api/d2d1effectauthor/) interface described in section 5. This interface contains one new method to implement:
...
The transform then calls the [**ID2D1DrawInfo::SetPixelShaderConstantBuffer**](/windows/desktop/api/d2d1effectauthor/) method on the [**ID2D1DrawInfo**](/windows/desktop/api/d2d1effectauthor/) parameter provided in the [**SetDrawInfo**](/windows/desktop/api/d2d1effectauthor/) method to pass this buffer to the shader.
...
Like with pixel shaders, the transform must load and select a vertex shader for execution. To load the vertex shader, it calls the [**LoadVertexShader**](/windows/desktop/api/D2D1EffectAuthor/) method on the [**ID2D1EffectContext**](/windows/desktop/api/D2D1EffectAuthor/) method received in the effect's Initialize method. To select the vertex shader for execution, it calls [**SetVertexProcessing**](/windows/desktop/api/d2d1effectauthor/) on the [**ID2D1DrawInfo**](/windows/desktop/api/d2d1effectauthor/) parameter received in the transform's [**SetDrawInfo**](/windows/desktop/api/d2d1effectauthor/) method. This method accepts a GUID for a previously-loaded vertex shader as well as (optionally) a previously-created vertex buffer for the shader to execute on.
...
This interface contains two new methods to implement in addition to the ones in [**ID2D1Transform**](/windows/desktop/api/d2d1/):
