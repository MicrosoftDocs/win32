...
> [!Note]  
> Starting with Windows 8, you can use the [**CreateBitmapBrush**](/windows/win32/D2d1_1/?branch=master) method on the [**ID2D1DeviceContext**](/windows/win32/D2d1_1/?branch=master) interface to create a [**ID2D1BitmapBrush**](/windows/win32/d2d1/?branch=master) instead of a **ID2D1BitmapBrush**. **ID2D1BitmapBrush** adds high-quality scaling modes to the bitmap brush.

 

![screen shot of a square filled with a plant bitmap](images/brushes-ovw-bitmap.png)

1.  Declare a variable of type [**ID2D1BitmapBrush**](/windows/win32/d2d1/?branch=master).
    ```C++
...
3.  Choose the extend modes ([**D2D1\_EXTEND\_MODE**](/windows/win32/d2d1/ne-d2d1-d2d1_extend_mode?branch=master)) and interpolation mode ([**D2D1\_BITMAP\_INTERPOLATION\_MODE**](/windows/win32/d2d1/ne-d2d1-d2d1_bitmap_interpolation_mode?branch=master)) of the bitmap brush and then call the [**CreateBitmapBrush**](id2d1rendertarget-createbitmapbrush.md) method to create a brush, as shown in the following code.
    ```C++
