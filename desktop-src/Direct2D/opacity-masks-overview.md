...
-   Use the [**ID2D1RenderTarget::FillGeometry**](/windows/desktop/api/d2d1/) method. The **FillGeometry** method paints the interior of geometry with the specified [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/), then applies an opacity mask, defined by a brush. Use this method when you want to apply an opacity mask to a geometry or you want to use a brush as an opacity mask.
-   Use an [**ID2D1Layer**](/windows/desktop/api/d2d1/) to apply an opacity mask. Use this approach when you want to apply an opacity mask to a group of drawing content, not just a single shape or image. For details, see the [Layers Overview](direct2d-layers-overview.md).

## Use a Bitmap as an Opacity Mask with the FillOpacityMask Method

The [**FillOpacityMask**](id2d1rendertarget-fillopacitymask.md) method paints a rectangular region of a render target and then applies an opacity mask, defined by an [**ID2D1Bitmap**](/windows/desktop/api/d2d1/). Use this method when you have a bitmap that you want to use as an opacity mask for a rectangular region.

The following diagram shows an effect of applying the opacity mask (an [**ID2D1Bitmap**](/windows/desktop/api/d2d1/) with an image of a flower) to an [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/) with an image of a fern plant. The resulting image is a bitmap of a plant clipped to the flower shape.
...
In this example, the [**ID2D1Bitmap**](/windows/desktop/api/d2d1/) is loaded by a helper method, LoadResourceBitmap, defined elsewhere in the sample.
...
The next example defines the brush, *m\_pFernBitmapBrush*, to which the opacity mask is applied. This example uses an [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/) that contains an image of a fern, but you could use an [**ID2D1SolidColorBrush**](/windows/desktop/api/d2d1/), [**ID2D1LinearGradientBrush**](/windows/desktop/api/d2d1/), or [**ID2D1RadialGradientBrush**](/windows/desktop/api/d2d1/) instead. The following illustration shows the output that is produced.
...
> [!Note]  
> Starting with Windows 8, the [**D2D1\_OPACITY\_MASK\_CONTENT**](/windows/desktop/api/d2d1/ne-d2d1-d2d1_opacity_mask_content) is not required. See the [**ID2D1DeviceContext::FillOpacityMask**](/windows/desktop/api/D2d1_1/) method, which has no **D2D1\_OPACITY\_MASK\_CONTENT** parameter.
...
The preceding section described how to use an [**ID2D1Bitmap**](/windows/desktop/api/d2d1/) as an opacity mask. Direct2D also provides the [**ID2D1RenderTarget::FillGeometry**](/windows/desktop/api/d2d1/) method, which enables you to optionally specify brush as an opacity mask when you fill an [**ID2D1Geometry**](/windows/desktop/api/d2d1/). This enables you to create opacity masks from gradients (using [**ID2D1LinearGradientBrush**](/windows/desktop/api/d2d1/) or [**ID2D1RadialGradientBrush**](/windows/desktop/api/d2d1/)) and bitmaps (using **ID2D1Bitmap**).

The [**FillGeometry**](/windows/desktop/api/d2d1/) method takes three parameters:

-   The first parameter, an [**ID2D1Geometry**](/windows/desktop/api/d2d1/), defines the shape to paint.
-   The second parameter, an [**ID2D1Brush**](/windows/desktop/api/d2d1/), specifies the brush used to paint the geometry. This parameter must be an [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/) that has its x- and y-extend modes set to [**D2D1\_EXTEND\_MODE\_CLAMP**](/windows/desktop/api/d2d1/ne-d2d1-d2d1_extend_mode).
-   The third parameter, an [**ID2D1Brush**](/windows/desktop/api/d2d1/), specifies a brush to use as the opacity mask. This brush may be an [**ID2D1LinearGradientBrush**](/windows/desktop/api/d2d1/), [**ID2D1RadialGradientBrush**](/windows/desktop/api/d2d1/), or an [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/). (Technically, you may use an [**ID2D1SolidColorBrush**](/windows/desktop/api/d2d1/), but using a solid color brush as an opacity mask doesn't produce interesting results.)

The following sections describe how to use [**ID2D1LinearGradientBrush**](/windows/desktop/api/d2d1/) and [**ID2D1RadialGradientBrush**](/windows/desktop/api/d2d1/) objects as opacity masks.
...
1.  Define the content to be masked. The following example creates an [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/), *m\_pLinearFadeFlowersBitmap*. The extend mode x- and y- for *m\_pLinearFadeFlowersBitmap* are set to [**D2D1\_EXTEND\_MODE\_CLAMP**](/windows/desktop/api/d2d1/ne-d2d1-d2d1_extend_mode) so that it can be used with an opacity mask by the [**FillGeometry**](/windows/desktop/api/d2d1/) method.
    ```C++
                if (SUCCEEDED(hr))
                {
...
3.  Use the [**FillGeometry**](/windows/desktop/api/d2d1/) method. The final example uses the **FillGeometry** method to the content brush to fill a [**ID2D1RectangleGeometry**](/windows/desktop/api/d2d1/) (*m\_pRectGeo*) with an [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/) (*m\_pLinearFadeFlowersBitmap*) and applies an opacity mask (*m\_pLinearGradientBrush*).
    ```C++
...
The first example creates an [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/), *m\_pRadialFadeFlowersBitmapBrush*. So that it can be used with an opacity mask by the [**FillGeometry**](/windows/desktop/api/d2d1/) method, the extend mode x- and y- for *m\_pRadialFadeFlowersBitmapBrush* are set to [**D2D1\_EXTEND\_MODE\_CLAMP**](/windows/desktop/api/d2d1/ne-d2d1-d2d1_extend_mode).
...
The final code example uses the [**ID2D1BitmapBrush**](/windows/desktop/api/d2d1/) (*m\_pRadialFadeFlowersBitmapBrush*) and the opacity mask (*m\_pRadialGradientBrush*) to fill an [**ID2D1RectangleGeometry**](/windows/desktop/api/d2d1/) (*m\_pRectGeo*).
...
When you call [**PushLayer**](/windows/desktop/api/d2d1_1/nf-d2d1-pushlayer) to push an [**ID2D1Layer**](/windows/desktop/api/d2d1/) onto an render target, you can use the [**D2D1\_LAYER\_PARAMETERS**](/windows/desktop/api/d2d1/ns-d2d1-d2d1_layer_parameters) structure to apply a brush as an opacity mask. The following code example uses an [**ID2D1RadialGradientBrush**](/windows/desktop/api/d2d1/) as an opacity mask.
