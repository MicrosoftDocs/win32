...
You need a Direct2D device and a Direct3D device to create a Direct2D device context. A [**Direct2D device**](/windows/win32/D2d1_1/?branch=master) (exposes an **ID2D1Device** interface pointer) represents a display adapter. A Direct3D device (exposes an [**ID3D11Device**](https://msdn.microsoft.com/library/windows/desktop/ff476379) interface pointer) is associated with a Direct2D device. Each app must have one **Direct2D device**, but can have more than one **device**.

## What is a Direct2D device context?

A [Direct2D](direct2d.direct2d_portal.xml) [**device context**](/windows/win32/D2d1_1/?branch=master) (exposes an **ID2D1DeviceContext** interface pointer) represents a set of state and command buffers that you use to render to a target. You can call methods on the device context to set pipeline state and generate rendering commands by using the resources owned by a device.

## Rendering with Direct2D on Windows 8

On Windows 7 and earlier, you use a [**ID2D1HwndRenderTarget**](/windows/win32/d2d1/?branch=master) or another render target interface to render to a window or surface. Starting with Windows 8, we do not recommend rendering by using methods that rely on interfaces like **ID2D1HwndRenderTarget** because they won't work with Windows Store apps. You can use a [**device context**](/windows/win32/D2d1_1/?branch=master) to render to an Hwnd if you want to make a desktop app and still take advantage of the **device context's** additional features. However, the **device context** is required to render content in a Windows Store apps with [Direct2D](direct2d.direct2d_portal.xml).
...
-   You can use more than one type of window with a device context. You can use a [**device context**](/windows/win32/D2d1_1/?branch=master) and a [**DXGI swap chain**](https://msdn.microsoft.com/library/windows/desktop/hh404631) to render directly to a [**Windows::UI::Core::CoreWindow**](https://msdn.microsoft.com/library/windows/apps/br208225) or a [**Windows::UI::XAML::SwapChainBackgroundPanel**](https://msdn.microsoft.com/library/windows/apps/hh702626).
-   You can use the [Direct2D](direct2d.direct2d_portal.xml) [**device context**](/windows/win32/D2d1_1/?branch=master) to create [Direct2D effects](effects-overview.md) and to render the output of an image effect or effect graph to a render target.
-   You can have multiple [**device contexts**](/windows/win32/D2d1_1/?branch=master), which can be helpful for improving performance in a threaded app. See [Multithreaded Direct2D apps](multi-threaded-direct2d-apps.md) for more information.
-   The [**device context**](/windows/win32/D2d1_1/?branch=master) interoperates closely with Direct3D, giving you more access to Direct3D options.

## How to create a Direct2D device context for rendering

The code here shows you how to create a Direct3D11 device, get the associated DXGI device, create a [**Direct2D device**](/windows/win32/D2d1_1/?branch=master), and then finally create the Direct2D [**device context**](/windows/win32/D2d1_1/?branch=master) for rendering.
...
> [!Note]  
> This code assumes you already have an [**ID2D1Factory1**](/windows/win32/D2d1_1/?branch=master) object, for more information see the [**ID2D1Factory reference page**](/windows/win32/d2d1/?branch=master).
...
3.  Create an [**ID2D1Device**](/windows/win32/D2d1_1/?branch=master) object by calling the [**ID2D1Factory::CreateDevice**](/windows/win32/d2d1_1/nf-d2d1_1-d2d1createdevice?branch=master) method and passing in the [**IDXGIDevice**](https://msdn.microsoft.com/library/windows/desktop/bb174527) object.
4.  Create an [**ID2D1DeviceContext**](/windows/win32/D2d1_1/?branch=master) pointer using the [**ID2D1Device::CreateDeviceContext**](/windows/win32/D2d1_1/?branch=master) method.

## Selecting a target

The code here shows you how to get the [**2 dimensional Direct3D texture**](https://msdn.microsoft.com/library/windows/desktop/ff476635) for the window back buffer and create a bitmap target that links to this texture to which the [**Direct2D device context**](/windows/win32/D2d1_1/?branch=master) renders.
...
5.  Declare a [**D2D1\_BITMAP\_PROPERTIES1**](/windows/win32/D2D1_1/ns-d2d1_1-d2d1_bitmap_properties1?branch=master) struct and set the property values. Set the pixel format to BGRA because this is the format the [**Direct3D device**](https://msdn.microsoft.com/library/windows/desktop/ff476379) and the [**DXGI device**](https://msdn.microsoft.com/library/windows/desktop/bb174527) use.
...
6.  Get the back buffer as an [**IDXGISurface**](https://msdn.microsoft.com/library/windows/desktop/hh404628) to pass to Direct2D. Direct2D doesn't accept an [**ID3D11Texture2D**](https://msdn.microsoft.com/library/windows/desktop/ff476635) directly.
    ...
    Create a [**ID2D1Bitmap**](/windows/win32/d2d1/?branch=master) object from the back buffer using the [**ID2D1DeviceContext::CreateBitmapFromDxgiSurface**](/windows/win32/D2d1_1/?branch=master) method.

7.  Now the [**Direct2D bitmap**](/windows/win32/d2d1/?branch=master) is linked to the back buffer. Set the target on the [**Direct2D device context**](/windows/win32/D2d1_1/?branch=master) to the **bitmap**.

## How to render and display

Now that you have a target bitmap, you can draw primitives, images, image effects, and text to it using the [**Direct2D device context**](/windows/win32/D2d1_1/?branch=master). The code here shows you how to draw a rectangle.
...
1.  Call the [**CreateSolidColorBrush**](/windows/win32/d2d1/nf-d2d1-createsolidcolorbrush?branch=master) to create a brush to paint the rectangle.
2.  Call the [**BeginDraw**](/windows/win32/d2d1/?branch=master) method before issuing any drawing commands.
3.  Call the [**DrawRectangle**](/windows/win32/d2d1/?branch=master) method the rectangle to be drawn and the brush.
4.  Call the [**EndDraw**](/windows/win32/d2d1/?branch=master) method after you've finished issuing drawing commands.
...
Now you can use the [**Direct2D device context**](/windows/win32/D2d1_1/?branch=master) draw primitives, images, image effects, and text to the screen.
