...
Geometry realizations, introduced in Windows 8.1, are a new type of drawing primitive that make it easy for [Direct2D](direct2d-portal.md) apps to improve geometry rendering performance in certain cases. Geometry realizations are represented by the [**ID2D1GeometryRealization**](/windows/desktop/api/D2d1_2/) interface.
...
A common way that developers solve this problem is to cache the full rasterization of the geometry. In particular, it is common to create a new bitmap, rasterize the geometry to that bitmap, and then draw that bitmap to the scene as needed. (This approach is described in the [Geometry rendering](https://www.bing.com/search?q=Geometry rendering) section of Improving the performance of Direct2D apps.) While this approach is very computationally efficient, it has some drawbacks:
...
On the other hand, consider a kaleidoscope app with animated geometry that continually changes. This app would probably not benefit from using geometry realizations. Since the shapes themselves change from frame to frame, it is not useful to cache their tessellations. The best approach for this app is to draw [**ID2D1Geometry**](/windows/desktop/api/d2d1/) objects directly.

## Creating geometry realizations

An [**ID2D1GeometryRealization**](/windows/desktop/api/D2d1_2/) object must be created from an existing [**ID2D1Geometry**](/windows/desktop/api/d2d1/) object. To create a geometry realization, call the [**CreateFilledGeometryRealization**](/windows/desktop/api/D2d1_2/) method or the [**CreateStrokedGeometryRealization**](/windows/desktop/api/D2d1_2/) method and pass in the **ID2D1Geometry** to be realized.

-   [**CreateFilledGeometryRealization**](/windows/desktop/api/D2d1_2/) creates a realization of the interior of the shape: the region that would be drawn by calling [**FillGeometry**](/windows/desktop/api/d2d1/).
-   [**CreateStrokedGeometryRealization**](/windows/desktop/api/D2d1_2/) creates a realization of the stroke of the shape: the region that would be drawn by calling [**DrawGeometry**](/windows/desktop/api/d2d1/).

Both kinds of geometry realization are represented by the [**ID2D1GeometryRealization**](/windows/desktop/api/D2d1_2/) interface.
...
Drawing geometry realizations is similar to drawing other [Direct2D](direct2d-portal.md) primitives, like bitmaps. To do so, call the [**DrawGeometryRealization**](/windows/desktop/api/D2d1_2/) method and pass it the geometry realization object to be drawn and the brush to use. As with other Direct2D drawing methods, you must call **DrawGeometryRealization** between calls to [**BeginDraw**](/windows/desktop/api/d2d1/) and [**EndDraw**](/windows/desktop/api/d2d1/).
...
[General guidelines for rendering complex static content](https://www.bing.com/search?q=General guidelines for rendering complex static content)
</dt> <dt>
...
[**ComputeFlatteningTolerance function**](/windows/desktop/api/D2d1_1helper/)
</dt> </dl>
