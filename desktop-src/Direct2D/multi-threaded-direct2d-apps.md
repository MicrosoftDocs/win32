...
The image here shows how [Direct2D](direct2d.direct2d_portal.xml) serializes two threads that make calls using only the Direct2D API.
...
It is more than often that a [Direct2D](direct2d.direct2d_portal.xml) app also makes some [Direct3D](https://msdn.microsoft.com/library/windows/desktop/ff476080) or DXGI calls. For example, a display thread will draw in Direct2D then present using a [**DXGI swap chain**](https://msdn.microsoft.com/library/windows/desktop/bb174569).

In this case, assuring thread-safety is more complicated: some [Direct2D](direct2d.direct2d_portal.xml) calls indirectly access the underlying [Direct3D](https://msdn.microsoft.com/library/windows/desktop/ff476080) resources, which might be simultaneously accessed by another thread which calls Direct3D or DXGI. Since those Direct3D or DXGI calls are out of Direct2D’s awareness and control, you need to create a multithreaded Direct2D factory, but you must do mor to avoid access conflicts.

The diagram here shows a [Direct3D](https://msdn.microsoft.com/library/windows/desktop/ff476080) resource access conflict due to thread T0 accessing a resource indirectly via a [Direct2D](direct2d.direct2d_portal.xml) call and T2 accessing the same resource directly via a Direct3D or DXGI call.

> [!Note]  
> The thread protection that [Direct2D](direct2d.direct2d_portal.xml) provides (the blue lock in this image) doesn't help in this case.
...
While thread-safety features of [DirectX](nodepage-windows_directx_graphics_documentation__august_2009_) can help ensure that no two individual API calls are made concurrently, you must also ensure that threads that make stateful API calls don't interfere with each other. Here is an example.
