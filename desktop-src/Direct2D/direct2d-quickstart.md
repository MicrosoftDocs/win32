...
1.  Implement the DemoApp::WndProc method to handle window messages. For the [**WM\_SIZE**](_win32_WM_SIZE) message, call the DemoApp::OnResize method and pass it the new width and height. For the [**WM\_PAINT**](https://msdn.microsoft.com/library/windows/desktop/dd145213) and [**WM\_DISPLAYCHANGE**](https://msdn.microsoft.com/library/windows/desktop/dd145210) messages, call the DemoApp::OnRender method to paint the window. You implement the OnRender and OnResize methods in the steps that follow.
    ```C++
