...
To draw Direct2D content to a GDI DC, you use an [**ID2D1DCRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371213(v=VS.85).aspx). To create a DC render target, you use the [**ID2D1Factory::CreateDCRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371248(v=VS.85).aspx) method. This method takes two parameters.

The first parameter, a [**D2D1\_RENDER\_TARGET\_PROPERTIES**](/windows/desktop/api/d2d1/ns-d2d1-d2d1_render_target_properties) structure, specifies rendering, remoting, DPI, pixel format, and usage information. To enable the DC render target to work with GDI, set the DXGI format to [DXGI\_FORMAT\_B8G8R8A8\_UNORM](http://msdn.microsoft.com/library/bb173059(VS.85).aspx) and the alpha mode to [**D2D1\_ALPHA\_MODE\_PREMULTIPLIED**](/windows/desktop/api/dcommon/ne-dcommon-d2d1_alpha_mode) or **D2D1\_ALPHA\_MODE\_IGNORE**.
...
In the preceding code, *m\_pD2DFactory* is a pointer to an [**ID2D1Factory**](https://msdn.microsoft.com/en-us/library/Dd371246(v=VS.85).aspx), and *m\_pDCRT* is a pointer to an [**ID2D1DCRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371213(v=VS.85).aspx).

Before you can render with the DC render target, you must use its [**BindDC**](https://msdn.microsoft.com/en-us/library/Dd371214(v=VS.85).aspx) method to associate it with a GDI DC. You do this each time you use a different DC, or the size of the area you want to draw to changes.

The [**BindDC**](https://msdn.microsoft.com/en-us/library/Dd371214(v=VS.85).aspx) method takes two parameters, *hDC* and *pSubRect*. The *hDC* parameter provides a handle to the device context that receives the output of the render target. The *pSubRect* parameter is a rectangle that describes the portion of the device context to which content is rendered. The DC render target updates its size to match the device context area described by *pSubRect*, should it change size.
...
In some cases, it might not be obvious that GDI is performing additional processing that might degrade the quality of the Direct2D content. For example, on a right-to-left (RTL) build of Windows, content rendered by an [**ID2D1DCRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371213(v=VS.85).aspx) might be horizontally inverted when GDI copies it to its destination. Whether the content is actually inverted depends on the current settings of the DC.
...
To render GDI content to a Direct2D GDI-compatible render target, use an [**ID2D1GdiInteropRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371321(v=VS.85).aspx), which provides access to a device context that can accept GDI draw calls. Unlike other interfaces, an **ID2D1GdiInteropRenderTarget** object is not created directly. Instead, use the [**QueryInterface**](https://msdn.microsoft.com/en-us/library/ms682521(v=VS.85).aspx) method of an existing render target instance. The following code shows how to do this:
...
In the preceding code, *m\_pD2DFactory* is a pointer to an [**ID2D1Factory**](https://msdn.microsoft.com/en-us/library/Dd371246(v=VS.85).aspx), and *m\_pGDIRT* is a pointer to an [**ID2D1GdiInteropRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371321(v=VS.85).aspx).

Notice that the[**D2D1\_RENDER\_TARGET\_USAGE\_GDI\_COMPATIBLE**](/windows/desktop/api/d2d1/ne-d2d1-d2d1_render_target_usage) flag is specified when creating the Hwnd GDI-compatible render target. If a pixel format is required, use [DXGI\_FORMAT\_B8G8R8A8\_UNORM](http://msdn.microsoft.com/library/bb173059(VS.85).aspx). If an alpha mode is required, use [**D2D1\_ALPHA\_MODE\_PREMULTIPLIED**](/windows/desktop/api/dcommon/ne-dcommon-d2d1_alpha_mode) or **D2D1\_ALPHA\_MODE\_IGNORE**.

Note that the [**QueryInterface**](https://msdn.microsoft.com/en-us/library/ms682521(v=VS.85).aspx) method always succeeds. To test whether the [**ID2D1GdiInteropRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371321(v=VS.85).aspx) interface's methods will work for a given render target, create a [**D2D1\_RENDER\_TARGET\_PROPERTIES**](/windows/desktop/api/d2d1/ns-d2d1-d2d1_render_target_properties) that specifies GDI compatibility and the appropriate pixel format, and then call the render target's [**IsSupported**](https://msdn.microsoft.com/en-us/library/Dd742854(v=VS.85).aspx) method to see whether the render target is GDI-compatible.
...
[**ID2D1Factory::CreateDCRenderTarget**](https://msdn.microsoft.com/en-us/library/Dd371248(v=VS.85).aspx)
</dt> <dt>
