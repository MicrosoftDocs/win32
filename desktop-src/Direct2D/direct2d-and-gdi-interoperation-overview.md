...
To draw Direct2D content to a GDI DC, you use an [**ID2D1DCRenderTarget**](/windows/desktop/api/d2d1/). To create a DC render target, you use the [**ID2D1Factory::CreateDCRenderTarget**](/windows/desktop/api/d2d1/) method. This method takes two parameters.
...
In the preceding code, *m\_pD2DFactory* is a pointer to an [**ID2D1Factory**](/windows/desktop/api/d2d1/), and *m\_pDCRT* is a pointer to an [**ID2D1DCRenderTarget**](/windows/desktop/api/d2d1/).

Before you can render with the DC render target, you must use its [**BindDC**](/windows/desktop/api/d2d1/) method to associate it with a GDI DC. You do this each time you use a different DC, or the size of the area you want to draw to changes.

The [**BindDC**](/windows/desktop/api/d2d1/) method takes two parameters, *hDC* and *pSubRect*. The *hDC* parameter provides a handle to the device context that receives the output of the render target. The *pSubRect* parameter is a rectangle that describes the portion of the device context to which content is rendered. The DC render target updates its size to match the device context area described by *pSubRect*, should it change size.
...
When you use an [**ID2D1DCRenderTarget**](/windows/desktop/api/d2d1/), it renders Direct2D content to an internal bitmap, and then renders the bitmap to the DC with GDI.
...
In some cases, it might not be obvious that GDI is performing additional processing that might degrade the quality of the Direct2D content. For example, on a right-to-left (RTL) build of Windows, content rendered by an [**ID2D1DCRenderTarget**](/windows/desktop/api/d2d1/) might be horizontally inverted when GDI copies it to its destination. Whether the content is actually inverted depends on the current settings of the DC.
...
To render GDI content to a Direct2D GDI-compatible render target, use an [**ID2D1GdiInteropRenderTarget**](/windows/desktop/api/d2d1/), which provides access to a device context that can accept GDI draw calls. Unlike other interfaces, an **ID2D1GdiInteropRenderTarget** object is not created directly. Instead, use the [**QueryInterface**](https://msdn.microsoft.com/windows/desktop/54d5ff80-18db-43f2-b636-f93ac053146d) method of an existing render target instance. The following code shows how to do this:
...
In the preceding code, *m\_pD2DFactory* is a pointer to an [**ID2D1Factory**](/windows/desktop/api/d2d1/), and *m\_pGDIRT* is a pointer to an [**ID2D1GdiInteropRenderTarget**](/windows/desktop/api/d2d1/).
...
Note that the [**QueryInterface**](https://msdn.microsoft.com/windows/desktop/54d5ff80-18db-43f2-b636-f93ac053146d) method always succeeds. To test whether the [**ID2D1GdiInteropRenderTarget**](/windows/desktop/api/d2d1/) interface's methods will work for a given render target, create a [**D2D1\_RENDER\_TARGET\_PROPERTIES**](/windows/desktop/api/d2d1/ns-d2d1-d2d1_render_target_properties) that specifies GDI compatibility and the appropriate pixel format, and then call the render target's [**IsSupported**](/windows/desktop/api/d2d1/) method to see whether the render target is GDI-compatible.
...
[**ID2D1Factory::CreateDCRenderTarget**](/windows/desktop/api/d2d1/)
</dt> <dt>
