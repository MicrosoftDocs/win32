...
4.  Call [**ID2D1Factory::SetAntialiasMode**](/windows/win32/d2d1/?branch=master) to set antialias mode on the render target to D2D1\_ANTIALIAS\_MODE\_ALIASED for [**ID2D1Factory::FillOpacityMask**](id2d1rendertarget-fillopacitymask.md) to work.
5.  Call [**FillOpacityMask**](/windows/win32/d2d1/?branch=master) with the bitmap mask and the bitmap brush on the render target to fill the clip.
...
It then calls [**SetAntialiasMode**](/windows/win32/d2d1/?branch=master) to set the antialias mode. Call [**FillOpacityMask**](/windows/win32/d2d1/?branch=master) to use a bitmap mask to clip the original bitmap.
