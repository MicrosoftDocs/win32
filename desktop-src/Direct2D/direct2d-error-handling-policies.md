...
Batched functions in Direct2D are the functions that are processed as a single unit when [**EndDraw**](/windows/desktop/api/d2d1/) or [**Close**](/windows/desktop/api/d2d1/) is called. They are the drawing commands between [**BeginDraw**](/windows/desktop/api/d2d1/) and **EndDraw** or commands on [**GeometrySink**](/windows/desktop/api/d2d1/). For these functions, errors are reported at the time the batch is completed. The error is returned after **EndDraw** for drawing commands, and after **Close** for **GeometrySink**.

RenderTargets stop drawing if an error state is set, but an application can call [**Flush**](/windows/desktop/api/d2d1/) to reset the error state and resume drawing.
...
Direct2D dereferences an output pointer and assigns it to **NULL** immediately upon entering the function. This causes an access violation if a caller passes in **NULL** as the pointer to the return value. This policy also applies to arrays of pointers. For other output parameters, such as a struct, the dereference happens later and also results in an access violation. However, there are some methods that have optional output pointers (that is, [**EndDraw**](/windows/desktop/api/d2d1/), [**Flush**](/windows/desktop/api/d2d1/)) that will not cause an access violation.
