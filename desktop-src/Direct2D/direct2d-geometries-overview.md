...
keywords: ["Direct2D,geometries overview"]
---

# Geometries Overview

This overview describes how to create and use [**ID2D1Geometry**](id2d1geometry.md) objects to define and manipulate 2D figures. It contains the following sections.
...
A Direct2D geometry is an [**ID2D1Geometry**](id2d1geometry.md) object. This object can be a simple geometry ([**ID2D1RectangleGeometry**](id2d1rectanglegeometry.md), [**ID2D1RoundedRectangleGeometry**](id2d1roundedrectanglegeometry.md), or [**ID2D1EllipseGeometry**](id2d1ellipsegeometry.md)), a path geometry ([**ID2D1PathGeometry**](id2d1pathgeometry.md)), or a composite geometry ([**ID2D1GeometryGroup**](id2d1geometrygroup.md) and [**ID2D1TransformedGeometry**](id2d1transformedgeometry.md)).
...
Direct2D geometries are immutable and device-independent resources created by [**ID2D1Factory**](id2d1factory.md). Generally, you should create geometries one time and keep them for the life of the application, or until they have to be changed. For more information about device-independent and device-dependent resources, see the [Resources Overview](resources-and-resource-domains.md).

The following sections describe the different kinds of geometries.

## Simple Geometries

Simple geometries include [**ID2D1RectangleGeometry**](id2d1rectanglegeometry.md), [**ID2D1RoundedRectangleGeometry**](id2d1roundedrectanglegeometry.md), and [**ID2D1EllipseGeometry**](id2d1ellipsegeometry.md) objects, and can be used to create basic geometric figures, such as rectangles, rounded rectangles, circles, and ellipses.

To create a simple geometry, use one of the [**ID2D1Factory::Create&lt;*geometryType*&gt;Geometry**](id2d1factory.md) methods. These methods create an object of the specified type. For example, to create a rectangle, call [**ID2D1Factory::CreateRectangleGeometry**](id2d1factory-createrectanglegeometry.md), which returns an [**ID2D1RectangleGeometry**](id2d1rectanglegeometry.md) object; to create a rounded rectangle, call [**ID2D1Factory::CreateRoundedRectangleGeometry**](id2d1factory-createroundedrectanglegeometry.md), which returns an [**ID2D1RoundedRectangleGeometry**](id2d1roundedrectanglegeometry.md) object, and so on.

The following code example calls the [**CreateEllipseGeometry**](id2d1factory-createellipsegeometry.md) method, passing in an ellipse structure with the *center* set to (100, 100), *x-radius* to 100, and *y-radius* to 50. Then, it calls [**DrawGeometry**](id2d1rendertarget-drawgeometry.md), passing in the returned ellipse geometry, a pointer to a black [**ID2D1SolidColorBrush**](id2d1solidcolorbrush.md), and a stroke width of 5. The following illustration shows the output from the code example.
...
To draw the outline of any geometry, use the [**DrawGeometry**](id2d1rendertarget-drawgeometry.md) method. To paint its interior, use the [**FillGeometry**](id2d1rendertarget-fillgeometry.md) method.

## Path Geometries

Path geometries are represented by the [**ID2D1PathGeometry**](id2d1pathgeometry.md) interface. These objects can be used to describe complex geometric figures composed of segments such as arcs, curves, and lines. The following illustration shows a drawing created by using path geometry.
...
A composite geometry is a geometry grouped or combined with another geometry object, or with a transform. Composite geometries include [**ID2D1TransformedGeometry**](id2d1transformedgeometry.md) and [**ID2D1GeometryGroup**](id2d1geometrygroup.md) objects.

### Geometry Groups

Geometry groups are a convenient way to group several geometries at the same time so all figures of several distinct geometries are concatenated into one. To create a [**ID2D1GeometryGroup**](id2d1geometrygroup.md) object, call the [**CreateGeometryGroup**](id2d1factory-creategeometrygroup.md) method on the [**ID2D1Factory**](id2d1factory.md) object, passing in the *fillMode* with possible values of [**D2D1\_FILL\_MODE\_ALTERNATE**](d2d1-fill-mode.md) (alternate) and **D2D1\_FILL\_MODE\_WINDING**, an array of geometry objects to add to the geometry group, and the number of elements in this array.

The following code example first declares an array of geometry objects. These objects are four concentric circles that have the following radii: 25, 50, 75, and 100. Then call the [**CreateGeometryGroup**](id2d1factory-creategeometrygroup.md) on the [**ID2D1Factory**](id2d1factory.md) object, passing in [**D2D1\_FILL\_MODE\_ALTERNATE**](d2d1-fill-mode.md), an array of geometry objects to add to the geometry group, and the number of elements in this array.
...
There are multiple ways to transform a geometry. You can use the [**SetTransform**](id2d1brush-settransform.md) method of a render target to transform everything that the render target draws, or you can associate a transform directly with a geometry by using the [**CreateTransformedGeometry**](id2d1factory-createtransformedgeometry.md) method to create an [**ID2D1TransformedGeometry**](id2d1factory-createtransformedgeometry-ptr-id2d1geometry-ptr-d2d-matrix-3x2-f-ptr-ptr-id2d1transformedgeometry.md).

The method that you should use depends on the effect that you want. When you use the render target to transform and then render a geometry, the transform affects everything about the geometry, including the width of any stroke that you have applied. On the other hand, when you use an [**ID2D1TransformedGeometry**](id2d1transformedgeometry.md), the transform affects only the coordinates that describe the shape. The transformation will not affect the stroke thickness when the geometry is drawn.

> [!Note]  
> Starting with Windows 8 the world transform will not affect the stroke thickness of strokes with [**D2D1\_STROKE\_TRANSFORM\_TYPE\_FIXED**](--d2d1-stroke-transform-type.md)or [**D2D1\_STROKE\_TRANSFORM\_TYPE\_HAIRLINE**](--d2d1-stroke-transform-type.md). You should use these transform types to achieve transform independent strokes

 

The following example creates an [**ID2D1RectangleGeometry**](id2d1rectanglegeometry.md), then draws it without transforming it. It produces the output shown in the following illustration.
...
The next example uses the [**CreateTransformedGeometry**](id2d1factory-createtransformedgeometry.md) method to scale the geometry by a factor of 3, then draws it. It produces the output shown in the following illustration. Notice that, although the rectangle is larger, its stroke hasn't increased.
...
You can use an [**ID2D1Geometry**](id2d1geometry.md) object as a geometric mask when you call the [**PushLayer**](id2d1rendertarget-push-layer.md) method. The geometric mask specifies the area of the layer that is composited into the render target. For more information, see the Geometric Masks section of the [Layers Overview](direct2d-layers-overview.md).

## Geometric Operations

The [**ID2D1Geometry**](id2d1geometry.md) interface provides several geometric operations that you can use to manipulate and measure geometric figures. For example, you can use them to calculate and return their bounds, compare to see how one geometry is spatially related to another (useful for hit testing), calculate the areas and lengths, and more. The following table describes the common geometric operations.
...
| Bounds/ Widened Bounds/Retrieve Bounds, Dirty Region update | [**Widen**](id2d1geometry-widen.md), [**GetBounds**](id2d1geometry-getbounds.md), [**GetWidenedBounds**](id2d1geometry-getwidenedbounds.md)                             |
...
> [!Note]  
> Starting in Windows 8, you can use the [**ComputePointAndSegmentAtLength**](id2d1pathgeometry1-computepointandsegmentatlength.md) method on the [**ID2D1PathGeometry1**](id2d1pathgeometry1.md) to calculate the area or length of a geometry.

 

### Combining Geometries

To combine one geometry with another, call the [**ID2D1Geometry::CombineWithGeometry**](id2d1geometry-combinewithgeometry.md) method. When you combine the geometries, you specify one of the four ways to perform the combine operation: [**D2D1\_COMBINE\_MODE\_UNION**](d2d1-combine-mode.md) (union), [**D2D1\_COMBINE\_MODE\_INTERSECT**](d2d1-combine-mode.md) (intersect), [**D2D1\_COMBINE\_MODE\_XOR**](d2d1-combine-mode.md) (xor), and [**D2D1\_COMBINE\_MODE\_EXCLUDE**](d2d1-combine-mode.md) (exclude). The following code example shows two circles that are combined by using the union combine mode, where the first circle has the center point of (75, 75) and the radius of 50, and the second circle has the center point of (125, 75) and the radius of 50.
...
For illustrations of all the combine modes, see the [**D2D1\_COMBINE\_MODE enumeration**](d2d1-combine-mode.md).

### Widen

The [**Widen**](id2d1geometry-widen.md) method generates a new geometry whose fill is equivalent to stroking the existing geometry, and then writes the result to the specified [**ID2D1SimplifiedGeometrySink**](id2d1simplifiedgeometrysink.md) object. The following code example calls [**Open**](id2d1pathgeometry-open.md) on the [**ID2D1PathGeometry**](id2d1pathgeometry.md) object. If **Open** succeeds, it calls **Widen** on the geometry object.
...
The [**Outline**](id2d1geometry-outline.md) method computes the outline of the geometry (a version of the geometry in which no figure crosses itself or any other figure) and writes the result to an [**ID2D1SimplifiedGeometrySink**](id2d1simplifiedgeometrysink.md). The following code example uses **Outline** to construct an equivalent geometry without any self-intersections. It uses the default flattening tolerance.
...
The [**GetBounds**](id2d1geometry-getbounds.md) method retrieves the bounds of the geometry. The following code example uses **GetBounds** to retrieve the bounds of a specified circle (**m\_pCircleGeometry1**).
