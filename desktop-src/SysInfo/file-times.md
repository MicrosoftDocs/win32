...
To retrieve the file times for a specified file, use the [**GetFileTime**](getfiletime.md) function. **GetFileTime** copies the creation, last access, and last write times to individual [**FILETIME**](filetime-str.md) structures. You can also retrieve file times using the [**FindFirstFile**](https://msdn.microsoft.com/library/windows/desktop/aa364418) and [**FindNextFile**](https://msdn.microsoft.com/library/windows/desktop/aa364428) functions. These functions copy the file times to **FILETIME** structures in a [**WIN32\_FIND\_DATA**](https://msdn.microsoft.com/library/windows/desktop/aa365740) structure. When writing to a file, the last write time is not fully updated until all handles that are used for writing are closed.

To set the file times for a file, use the [**SetFileTime**](setfiletime.md) function. This function lets you modify creation, last access, and last write times without changing the content of the file. You can compare the times of different files by using the [**CompareFileTime**](comparefiletime.md) function. The function compares two file times and returns a value that indicates which time is later or returns 0 (zero) if the times are equal.

If you plan to modify file times for specified files, you can convert a date and time of day to a file time by using the [**SystemTimeToFileTime**](systemtimetofiletime.md) function. You can also obtain the system time in a [**FILETIME**](filetime-str.md) structure by calling the [**GetSystemTimeAsFileTime**](getsystemtimeasfiletime.md) function.

To make a file time easy to display to a user, use the [**FileTimeToSystemTime**](filetimetosystemtime.md) function. **FileTimeToSystemTime** converts the file time and copies the month, day, year, and time of day from the file time to a [**SYSTEMTIME**](systemtime-str.md) structure.
...
To convert a file time to local time, use the [**FileTimeToLocalFileTime**](filetimetolocalfiletime.md) function. However, **FileTimeToLocalFileTime** uses the current settings for the time zone and daylight saving time. Therefore, if it is daylight saving time, it takes daylight saving time into account, even if the file time you are converting is in standard time.

The FAT file system records times on disk in local time. [**GetFileTime**](getfiletime.md) retrieves cached UTC times from the FAT file system. When it becomes daylight saving time, the time retrieved by **GetFileTime** is off an hour, because the cache is not updated. When you restart the computer, the cached time that **GetFileTime** retrieves is correct. [**FindFirstFile**](https://msdn.microsoft.com/library/windows/desktop/aa364418) retrieves the local time from the FAT file system and converts it to UTC by using the current settings for the time zone and daylight saving time. Therefore, if it is daylight saving time, **FindFirstFile** takes daylight saving time into account, even if the file time you are converting is in standard time.

The NTFS file system records times on disk in UTC. To account for daylight saving time when converting a file time to a local time, use the following sequence of functions instead of using [**FileTimeToLocalFileTime**](filetimetolocalfiletime.md):

-   [**FileTimeToSystemTime**](filetimetosystemtime.md)
-   [**SystemTimeToTzSpecificLocalTime**](systemtimetotzspecificlocaltime.md)
-   [**SystemTimeToFileTime**](systemtimetofiletime.md)
