...
To set the system time, use the [**SetSystemTime**](/windows/win32/Winbase/?branch=master) function. **SetSystemTime** assumes you have specified a UTC-based time.

The [**GetSystemTimeAdjustment**](/windows/win32/Winbase/?branch=master) and [**SetSystemTimeAdjustment**](/windows/win32/Winbase/?branch=master) functions synchronize the time-of-day clock with another time source using a periodic time adjustment applied at each clock interrupt.
