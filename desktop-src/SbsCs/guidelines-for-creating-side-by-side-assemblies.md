...
-   Developers of applications that use [private assemblies](https://msdn.microsoft.com/library/windows/desktop/aa370850) should secure the application directory. If the application is installed using the [Windows Installer](https://msdn.microsoft.com/en-us/library/Cc185688(v=VS.85).aspx), then the application directory can be secured by using the LockPermissions table. Typically, the system is given read, write, and execute access to private assemblies; all other processes are given only execute and read access.
