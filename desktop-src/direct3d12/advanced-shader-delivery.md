...
As graphics continue to improve, the number of shaders a game compiles grows with them. Compiling those shaders on the player's device at launch, or "just-in-time" during gameplay, leads to longer load times, in-game shader compilation stutter, and higher power consumption. ASD addresses these problems by handling shader compilation before the game reaches the player. Using ASD requires a supported Windows version, an ASD-capable GPU and driver, and storefront integration. See the [adoption guide](https://aka.ms/xgd-asd-selfservice) for current requirements.

## How Advanced Shader Delivery works

ASD reduces on-device shader compilation through a pipeline that prepares shaders ahead of time and ships the compiled results to players. The pipeline has three stages:
...
- **Less in-game stutter.** Precompiling shaders can reduce the just-in-time compilation that causes shader compilation stutter during gameplay.
- **Lower power consumption.** The player's device spends less power compiling shaders locally.
