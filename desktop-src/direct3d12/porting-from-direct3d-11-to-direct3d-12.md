...
-   [**ID3D11Buffer**](https://msdn.microsoft.com/library/windows/desktop/ff476351) and [**ID3D11Device::CreateBuffer**](https://msdn.microsoft.com/library/windows/desktop/ff476501)
-   [**ID3D11Texture1D**](https://msdn.microsoft.com/library/windows/desktop/ff476633) and [**ID3D11Device:CreateTexture1D**](https://msdn.microsoft.com/library/windows/desktop/ff476520)
-   [**ID3D11Texture2D**](https://msdn.microsoft.com/library/windows/desktop/ff476635) and [**ID3D11Device::CreateTexture2D**](https://msdn.microsoft.com/library/windows/desktop/ff476521)
-   [**ID3D11Texture3D**](https://msdn.microsoft.com/library/windows/desktop/ff476637) and [**ID3D11Device::CreateTexture3D**](https://msdn.microsoft.com/library/windows/desktop/ff476522)

In Direct3D 12 these are all represented by [**ID3D12Resource**](/windows/win32/D3D12/nn-d3d12-id3d12resource?branch=master) and [**ID3D12Device::CreateCommittedResource**](/windows/win32/D3D12/nf-d3d12-id3d12device-createcommittedresource?branch=master).

## Reserved resources

Reserved resources are resources where only virtual address space has been allocated, physical memory is not allocated until there is a call to [**ID3D12Device::CreateHeap**](/windows/win32/D3D12/nf-d3d12-id3d12device-createheap?branch=master). This is essentially the same concept as tiled resources in Direct3D 11.

The flags ([**D3D11\_RESOURCE\_MISC\_FLAG**](https://msdn.microsoft.com/library/windows/desktop/ff476203)) used in Direct3D 11 to set up tiled resources, then map them to physical memory.
...
In Direct3D 11 there is the appearance of a single timeline (calls following a sequence, such as data initialized with [**D3D11\_SUBRESOURCE\_DATA**](https://msdn.microsoft.com/library/windows/desktop/ff476220), then a call is made to [**ID3D11DeviceContext::UpdateSubresource**](https://msdn.microsoft.com/library/windows/desktop/ff476486), and then a call to [**ID3D11DeviceContext::Map**](https://msdn.microsoft.com/library/windows/desktop/ff476457)). The number of copies created of the data is not obvious to a Direct3D 11 developer.

In Direct3D 12 there are two timelines, the GPU timeline (set up by calls to [**CopyTextureRegion**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-copytextureregion?branch=master), and [**CopyBufferRegion**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-copybufferregion?branch=master) from mappable memory) and the CPU timeline (determined by calls to [**Map**](/windows/win32/D3D12/nf-d3d12-id3d12resource-map?branch=master)). Helper functions are provided (in the d3dx12.h file) called [**Updatesubresources**](updatesubresources1.md) that use a shared timeline. There are several variations of this helper function, one that uses [**ID3D12Device::GetCopyableFootprints**](/windows/win32/d3d12/nf-d3d12-id3d12device-getcopyablefootprints?branch=master), another that uses a heap-allocating mechanism, and another that uses a stack-allocating mechanism. These helper functions copy resources to both the GPU and CPU, via an intermediate staging area of memory.
...
In Direct3D 11 there is a lot of creation of shader and state objects, and setting the state of those objects, using the [**ID3D11Device**](https://msdn.microsoft.com/library/windows/desktop/ff476379) creation methods and the [**ID3D11DeviceContext**](https://msdn.microsoft.com/library/windows/desktop/ff476385) set methods. Typically a large number of calls are made to these methods, which are then combined at draw time by the driver to set the correct pipeline state.

In Direct3D 12 this setting of pipeline state has been combined into a single object ([**CreateComputePipelineState**](/windows/win32/D3D12/nf-d3d12-id3d12device-createcomputepipelinestate?branch=master) for a compute engine, and [**CreateGraphicsPipelineState**](/windows/win32/D3D12/nf-d3d12-id3d12device-creategraphicspipelinestate?branch=master) for a graphics engine), which is then attached to a command list before the draw call with a call to [**SetPipelineState**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-setpipelinestate?branch=master).
...
in Direct3D 11 there is little control over actually how work is submitted, it is largely handled by the driver, though some control is enabled through the [**ID3D11DeviceContext::Flush**](https://msdn.microsoft.com/library/windows/desktop/ff476425) and [**IDXGISwapChain1::Present1**](https://msdn.microsoft.com/library/windows/desktop/hh446797) calls.

In Direct3D 12 work submission is very explicit and controlled by the app. The primary construct for submitting work is the [**ID3D12GraphicsCommandList**](/windows/win32/d3d12/nn-d3d12-id3d12graphicscommandlist?branch=master), which is used to record all the apps commands (and is quite similar in concept to the ID3D11 deferred context). Backing store for a command list is provided by the [**ID3D12CommandAllocator**](/windows/win32/D3D12/nn-d3d12-id3d12commandallocator?branch=master), which enables the app to manage the memory utilization of the command list by actually exposing the memory that the Direct3D 12 driver is going to use to store the command list.

Finally the [**ID3D12CommandQueue**](/windows/win32/D3D12/nn-d3d12-id3d12commandqueue?branch=master) is a first-in first-out queue, that stores the correct order of the command lists for submission to the GPU. Only when one command list has completed execution on the GPU, will the next command list from the queue be submitted by the driver.
...
The main object for synchronizing the timelines is the [**ID3D12Fence**](/windows/win32/D3D12/nn-d3d12-id3d12fence?branch=master) object. The operation of fences is failry simple, they enable the GPU to signal when it has completed a task. The GPU and CPU can both signal, and can both wait on fences.
...
In Direct3D 11 the [**ID3D11DeviceContext::Map**](https://msdn.microsoft.com/library/windows/desktop/ff476457) flag D3D11\_MAP\_WRITE\_DISCARD essentially treated each resource as an endless supply of memory the app could write to (a process known as "renaming"). In Direct3D 12 again the process is explicit: additional memory needs to be allocated, and fences should be used to sync the operations. Ring buffers (consisting of large buffers) might be a good technique for this, refer to the ring buffer scenario in [Fence-Based Resource Management](fence-based-resource-management.md).

![using a ring buffer](images/ring-buffer-1.png)

## Resource Binding

Views in Direct3D 11 (shader resource views, render target views, and so on), have largely been replaced in Direct3D 12 with the concept of a descriptor. The creation methods still exist in Direct3D 12 (such as [**CreateShaderResourceView**](/windows/win32/D3D12/nf-d3d12-id3d12device-createshaderresourceview?branch=master) and [**CreateRenderTargetView**](/windows/win32/D3D12/nf-d3d12-id3d12device-createrendertargetview?branch=master)), which are called after the descriptor heap has been created, to write the data into the heap. Binding in Direct3D 12 is now handled by descriptor handles described in a root signature, and submitted using the [**SetGraphicsRootDescriptorTable**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-setgraphicsrootdescriptortable?branch=master) or [**SetComputeRootDescriptorTable**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-setcomputerootdescriptortable?branch=master) methods.
...
In Direct3D 11 there is automatic backbuffer rotation: only one render target view is needed for back buffer 0. In Direct3D 12 buffer rotation is explicit, there needs to be a render target view for each back buffer. Use the [**IDXGISwapChain3::GetCurrentBackBufferIndex**](https://msdn.microsoft.com/library/windows/desktop/dn903675) method to select which one to render to. Again this additional flexibility enables greater parallelization.

## Fixed function rendering

In Direct3D 11 there were a few methods that simplified various higher level operations, such as [**GenerateMips**](https://msdn.microsoft.com/library/windows/desktop/ff476426) (creating full mip chains) and [**DrawAuto**](https://msdn.microsoft.com/library/windows/desktop/ff476408) (using stream output as shader input without further input from the app). These methods are not available in Direct3D 12, the app needs to handle these operations by creating shaders to perform them.
...
|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**ID3D11Query**](https://msdn.microsoft.com/library/windows/desktop/ff476578)                                              | [**ID3D12QueryHeap**](/windows/win32/d3d12/?branch=master) allows queries to be grouped together, reducing the cost.                                                                                                                                                                                                                                                                                                                                     |
| [**ID3D11Predicate**](https://msdn.microsoft.com/library/windows/desktop/ff476577)                                      | Predication is now enabled by having data in a fully transparent buffer. The Direct3D 11 [**ID3D11Predicate**](https://msdn.microsoft.com/library/windows/desktop/ff476577) object is replaced by [**ID3D12Resource::Map**](/windows/win32/D3D12/nf-d3d12-id3d12resource-map?branch=master), which must follow a call to [**ResolveQueryData**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-resolvequerydata?branch=master) and a GPU sync operation using a fence to wait for the data to be ready. Refer to [Predication](predication.md). |
...
| Draw\*Indirect/[**DispatchIndirect**](https://msdn.microsoft.com/library/windows/desktop/ff476406) | Drawing indirect methods are all merged into the one [**ExecuteIndirect**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-executeindirect?branch=master) method.                                                                                                                                                                                                                                                                                                        |
...
| [**ResizeTilePool**](https://msdn.microsoft.com/library/windows/desktop/dn280505)                   | Reserved resources can be mapped to multiple heaps. When a tile pool would have been grown in D3D11, an additional heap can be allocated in D3D12 instead.                                                                                                                                                                                                                                                                               |
