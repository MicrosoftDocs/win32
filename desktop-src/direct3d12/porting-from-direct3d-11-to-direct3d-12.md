...
In Direct3D 12 these are all represented by [**ID3D12Resource**](id3d12resource.md) and [**ID3D12Device::CreateCommittedResource**](id3d12device-createcommittedresource.md).

## Reserved resources

Reserved resources are resources where only virtual address space has been allocated, physical memory is not allocated until there is a call to [**ID3D12Device::CreateHeap**](id3d12device-createheap.md). This is essentially the same concept as tiled resources in Direct3D 11.
...
In Direct3D 12 there are two timelines, the GPU timeline (set up by calls to [**CopyTextureRegion**](id3d12graphicscommandlist-copytextureregion.md), and [**CopyBufferRegion**](id3d12graphicscommandlist-copybufferregion.md) from mappable memory) and the CPU timeline (determined by calls to [**Map**](id3d12resource-map.md)). Helper functions are provided (in the d3dx12.h file) called [**Updatesubresources**](updatesubresources1.md) that use a shared timeline. There are several variations of this helper function, one that uses [**ID3D12Device::GetCopyableFootprints**](id3d12device-getcopyablefootprints.md), another that uses a heap-allocating mechanism, and another that uses a stack-allocating mechanism. These helper functions copy resources to both the GPU and CPU, via an intermediate staging area of memory.
...
In Direct3D 12 this setting of pipeline state has been combined into a single object ([**CreateComputePipelineState**](id3d12device-createcomputepipelinestate.md) for a compute engine, and [**CreateGraphicsPipelineState**](id3d12device-creategraphicspipelinestate.md) for a graphics engine), which is then attached to a command list before the draw call with a call to [**SetPipelineState**](id3d12graphicscommandlist-setpipelinestate.md).
...
In Direct3D 12 work submission is very explicit and controlled by the app. The primary construct for submitting work is the [**ID3D12GraphicsCommandList**](id3d12graphicscommandlist.md), which is used to record all the apps commands (and is quite similar in concept to the ID3D11 deferred context). Backing store for a command list is provided by the [**ID3D12CommandAllocator**](id3d12commandallocator.md), which enables the app to manage the memory utilization of the command list by actually exposing the memory that the Direct3D 12 driver is going to use to store the command list.

Finally the [**ID3D12CommandQueue**](id3d12commandqueue.md) is a first-in first-out queue, that stores the correct order of the command lists for submission to the GPU. Only when one command list has completed execution on the GPU, will the next command list from the queue be submitted by the driver.
...
The main object for synchronizing the timelines is the [**ID3D12Fence**](id3d12fence.md) object. The operation of fences is failry simple, they enable the GPU to signal when it has completed a task. The GPU and CPU can both signal, and can both wait on fences.
...
Views in Direct3D 11 (shader resource views, render target views, and so on), have largely been replaced in Direct3D 12 with the concept of a descriptor. The creation methods still exist in Direct3D 12 (such as [**CreateShaderResourceView**](id3d12device-createshaderresourceview.md) and [**CreateRenderTargetView**](id3d12device-createrendertargetview.md)), which are called after the descriptor heap has been created, to write the data into the heap. Binding in Direct3D 12 is now handled by descriptor handles described in a root signature, and submitted using the [**SetGraphicsRootDescriptorTable**](id3d12graphicscommandlist-setgraphicsrootdescriptortable.md) or [**SetComputeRootDescriptorTable**](id3d12graphicscommandlist-setcomputerootdescriptortable.md) methods.
...
|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**ID3D11Query**](https://msdn.microsoft.com/library/windows/desktop/ff476578)                                              | [**ID3D12QueryHeap**](id3d12queryheap.md) allows queries to be grouped together, reducing the cost.                                                                                                                                                                                                                                                                                                                                     |
| [**ID3D11Predicate**](https://msdn.microsoft.com/library/windows/desktop/ff476577)                                      | Predication is now enabled by having data in a fully transparent buffer. The Direct3D 11 [**ID3D11Predicate**](https://msdn.microsoft.com/library/windows/desktop/ff476577) object is replaced by [**ID3D12Resource::Map**](id3d12resource-map.md), which must follow a call to [**ResolveQueryData**](id3d12graphicscommandlist-resolvequerydata.md) and a GPU sync operation using a fence to wait for the data to be ready. Refer to [Predication](predication.md). |
...
| Draw\*Indirect/[**DispatchIndirect**](https://msdn.microsoft.com/library/windows/desktop/ff476406) | Drawing indirect methods are all merged into the one [**ExecuteIndirect**](id3d12graphicscommandlist-executeindirect.md) method.                                                                                                                                                                                                                                                                                                        |
