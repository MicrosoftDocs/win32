...
The command signature object ([**ID3D12CommandSignature**](id3d12commandsignature.md)) enables apps to specify indirect drawing, in particular setting the following:

-   The indirect argument buffer format.
-   The command type that will be used (from the [**ID3D12GraphicsCommandList**](id3d12graphicscommandlist.md) methods [**DrawInstanced**](id3d12graphicscommandlist-drawinstanced.md), [**DrawIndexedInstanced**](id3d12graphicscommandlist-drawindexedinstanced.md), or [**Dispatch**](id3d12graphicscommandlist-dispatch.md)).
-   The set of resource bindings which will change per-command call versus the set which will be inherited.

At startup, an app creates a small set of **command signatures**. At runtime, the application fills a buffer with commands (via whatever means the app developer chooses). The commands optionally containing state to set for vertex buffer views, index buffer views, root constants and root descriptors (raw or structured SRV/UAV/CBVs). These argument layouts are not hardware specific so apps can generate the buffers directly. The command signature inherits the remaining state from the command list. Then the app calls [**ExecuteIndirect**](id3d12graphicscommandlist-executeindirect.md) to instruct the GPU to interpret the contents of the indirect argument buffer according to the format defined by a particular command signature.
...
-   [**D3D12\_DRAW\_INDEXED\_ARGUMENTS**](d3d12-draw-indexed-arguments.md)
-   [**D3D12\_DISPATCH\_ARGUMENTS**](d3d12-dispatch-arguments.md)
-   [**D3D12\_VERTEX\_BUFFER\_VIEW**](d3d12-vertex-buffer-view.md)
-   [**D3D12\_INDEX\_BUFFER\_VIEW**](d3d12-index-buffer-view.md)
-   D3D12\_GPU\_VIRTUAL\_ADDRESS (a typedef'd synonym of UINT64).
-   [**D3D12\_CONSTANT\_BUFFER\_VIEW**](d3d12-constant-buffer-view-desc.md)

## Command Signature Creation

To create a command signature, use the following API items:

-   [**ID3D12Device::CreateCommandSignature**](id3d12device-createcommandsignature.md) (outputs an [**ID3D12CommandSignature**](id3d12commandsignature.md))
-   [**D3D12\_INDIRECT\_ARGUMENT\_TYPE**](d3d12-indirect-argument-type.md)
-   [**D3D12\_INDIRECT\_ARGUMENT\_DESC**](d3d12-indirect-argument-desc.md)
-   [**D3D12\_COMMAND\_SIGNATURE\_DESC**](d3d12-command-signature-desc.md)

The ordering of arguments within an indirect argument buffer is defined to exactly match the order of arguments specified in the *pArguments* parameter of [**D3D12\_COMMAND\_SIGNATURE\_DESC**](d3d12-command-signature-desc.md). All of the arguments for one draw (graphics)/dispatch (compute) call within an indirect argument buffer are tightly packed. However, applications are allowed to specify an arbitrary byte stride between draw/dispatch commands in an indirect argument buffer.
...
In this example, the indirect argument buffer generated by the application holds an array of 36-byte structures. Each structure only contains the five parameters passed to [**DrawIndexedInstanced**](id3d12graphicscommandlist-drawindexedinstanced.md) (plus padding).
