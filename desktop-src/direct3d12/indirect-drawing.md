...
-   [**D3D12\_DRAW\_INDEXED\_ARGUMENTS**](/windows/win32/D3D12/ns-d3d12-d3d12_draw_indexed_arguments?branch=master)
-   [**D3D12\_DISPATCH\_ARGUMENTS**](/windows/win32/D3D12/ns-d3d12-d3d12_dispatch_arguments?branch=master)
-   [**D3D12\_VERTEX\_BUFFER\_VIEW**](/windows/win32/d3d12/ns-d3d12-d3d12_vertex_buffer_view?branch=master)
-   [**D3D12\_INDEX\_BUFFER\_VIEW**](/windows/win32/D3D12/ns-d3d12-d3d12_index_buffer_view?branch=master)
-   D3D12\_GPU\_VIRTUAL\_ADDRESS (a typedef'd synonym of UINT64).
-   [**D3D12\_CONSTANT\_BUFFER\_VIEW**](/windows/win32/D3D12/ns-d3d12-d3d12_constant_buffer_view_desc?branch=master)

## Command Signature Creation

To create a command signature, use the following API items:

-   [**ID3D12Device::CreateCommandSignature**](/windows/win32/D3D12/nf-d3d12-id3d12device-createcommandsignature?branch=master) (outputs an [**ID3D12CommandSignature**](/windows/win32/d3d12/?branch=master))
-   [**D3D12\_INDIRECT\_ARGUMENT\_TYPE**](/windows/win32/D3D12/ne-d3d12-d3d12_indirect_argument_type?branch=master)
-   [**D3D12\_INDIRECT\_ARGUMENT\_DESC**](/windows/win32/D3D12/ns-d3d12-d3d12_indirect_argument_desc?branch=master)
-   [**D3D12\_COMMAND\_SIGNATURE\_DESC**](/windows/win32/D3D12/ns-d3d12-d3d12_command_signature_desc?branch=master)

The ordering of arguments within an indirect argument buffer is defined to exactly match the order of arguments specified in the *pArguments* parameter of [**D3D12\_COMMAND\_SIGNATURE\_DESC**](/windows/win32/D3D12/ns-d3d12-d3d12_command_signature_desc?branch=master). All of the arguments for one draw (graphics)/dispatch (compute) call within an indirect argument buffer are tightly packed. However, applications are allowed to specify an arbitrary byte stride between draw/dispatch commands in an indirect argument buffer.
...
In this example, the indirect argument buffer generated by the application holds an array of 36-byte structures. Each structure only contains the five parameters passed to [**DrawIndexedInstanced**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-drawindexedinstanced?branch=master) (plus padding).
