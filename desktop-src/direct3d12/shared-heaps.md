...
-   [**ID3D12Device::CreateSharedHandle**](/windows/win32/D3D12/nf-d3d12-id3d12device-createsharedhandle?branch=master)
-   [**ID3D12Device::OpenSharedHandle**](/windows/win32/D3D12/nf-d3d12-id3d12device-opensharedhandle?branch=master)
-   [**ID3D12Device::OpenSharedHandleByName**](/windows/win32/D3D12/nf-d3d12-id3d12device-opensharedhandlebyname?branch=master)

## Sharing heaps across processes

Shared heaps are specified with the D3D12\_HEAP\_FLAG\_SHARED member of the [**D3D12\_HEAP\_FLAGS**](/windows/win32/D3D12/ne-d3d12-d3d12_heap_flags?branch=master) enum.
...
Shared heaps across adapters is specified with the D3D12\_HEAP\_FLAG\_SHARED\_CROSS\_ADAPTER member of the [**D3D12\_HEAP\_FLAGS**](/windows/win32/D3D12/ne-d3d12-d3d12_heap_flags?branch=master) enum.
...
Cross-adapter sharing works with heaps created by calling [**ID3D12Device::CreateHeap**](/windows/win32/D3D12/nf-d3d12-id3d12device-createheap?branch=master). Applications can then create resources via [**CreatePlacedResource**](/windows/win32/D3D12/nf-d3d12-id3d12device-createplacedresource?branch=master). It is also allowed by resources/heaps created by [**CreateCommittedResource**](/windows/win32/D3D12/nf-d3d12-id3d12device-createcommittedresource?branch=master) but only for row-major D3D12\_RESOURCE\_DIMENSION\_TEXTURE2D resources (refer to [**D3D12\_RESOURCE\_DIMENSION**](/windows/win32/D3D12/ne-d3d12-d3d12_resource_dimension?branch=master)). Cross-adapter sharing does not work with [**CreateReservedResource**](/windows/win32/D3D12/nf-d3d12-id3d12device-createreservedresource?branch=master).
...
Applications should confine the usage of cross adapter heaps to only those scenarios which require the functionality they provide. Cross-adapter heaps are located in D3D12\_MEMORY\_POOL\_L0, which is not always what [**GetCustomHeapProperties**](/windows/win32/D3D12/nf-d3d12-id3d12device-getcustomheapproperties?branch=master) suggests. That memory pool is not efficient for discrete/ NUMA adapter architectures. And, the most efficient texture layouts are not always available.
