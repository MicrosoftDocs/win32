...
Descriptors are of varying size, that can be queried for via [**ID3D12Device::GetDescriptorHandleIncrementSize**](/windows/win32/api/d3d12/nf-d3d12-id3d12device-getdescriptorhandleincrementsize) for an SRV, UAV or CBV (depending on the GPU hardware), and are shown in this documentation as indivisible units, for example:
...
CPU handles are for immediate use, such as copying where both the source and destination need to be identified. Immediately after use (e.g a call to [**ID3D12GraphicsCommandList::OMSetRenderTargets**](/windows/win32/api/d3d12/nf-d3d12-id3d12graphicscommandlist-omsetrendertargets)), they can be reused or their underlying heap disposed
...
On Tier1 hardware (see [**Hardware Tiers**](/windows/win32/direct3d12/hardware-support), all bound descriptors (via descriptor tables) must be initialized, either as real descriptors or null descriptors, even if not accessed by the hardware, else behaviour is undefined.

On Tier2 hardware, this applies to bound CBV and UAV descriptors, but not SRV descriptors.

On Tier3 hardware there is no restriction on this, provided uninitialized descriptors are never accessed.
...
Default descriptors cannot be used with a raytracing acceleration structure view, as the provided *pResource* parameter must be **NULL** and the location must be passed via a [**D3D12_RAYTRACING_ACCELERATION_STRUCTURE_SRV**]/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_acceleration_structure_srv).
