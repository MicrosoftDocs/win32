...
Some APIs access an entire resource (such as the [**ID3D12GraphicsCommandList::CopyResource**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-copyresource?branch=master) method), others access a portion of a resource (for example the [**ID3D12Resource::ReadFromSubresource**](/windows/win32/d3d12/nf-d3d12-id3d12resource-readfromsubresource?branch=master) method). The methods that access a portion of a resource generally use a view description (such as the [**D3D12\_TEX2D\_ARRAY\_SRV**](/windows/win32/D3D12/ns-d3d12-d3d12_tex2d_array_srv?branch=master) structure) to specify the subresources to access. Refer to the [Subresource APIs](#subresource-apis) section for a complete list.
...
A shader-resource view can select any rectangular region of subresources, using one of the slices described above and judicious use of fields in the view structures (such as [**D3D12\_TEX2D\_ARRAY\_SRV**](/windows/win32/D3D12/ns-d3d12-d3d12_tex2d_array_srv?branch=master)), as shown in the image.
...
-   [**D3D12\_DISCARD\_REGION**](/windows/win32/d3d12/ns-d3d12-d3d12_discard_region?branch=master) : a structure used in preparation for discarding a resource.
-   [**D3D12\_PLACED\_SUBRESOURCE\_FOOTPRINT**](/windows/win32/d3d12/ns-d3d12-d3d12_placed_subresource_footprint?branch=master) : adds an offset into a resource to the basic footprint.
-   [**D3D12\_RESOURCE\_TRANSITION\_BARRIER**](/windows/win32/d3d12/ns-d3d12-d3d12_resource_transition_barrier?branch=master) : describes the transition of subresources between different usages (shader resource, render target, etc.).
-   [**D3D12\_SUBRESOURCE\_DATA**](/windows/win32/D3D12/ns-d3d12-d3d12_subresource_data?branch=master) : subresource data includes the data itself, and the row and slice pitch.
-   [**D3D12\_SUBRESOURCE\_FOOTPRINT**](/windows/win32/d3d12/ns-d3d12-d3d12_subresource_footprint?branch=master) : a footprint includes the format, width, height, depth and row-pitch of the subresource.
-   [**D3D12\_SUBRESOURCE\_INFO**](/windows/win32/D3D12/ns-d3d12-d3d12_subresource_info?branch=master) : contains the offset, row pitch and depth pitch of the subresource.
-   [**D3D12\_SUBRESOURCE\_TILING**](/windows/win32/D3D12/ns-d3d12-d3d12_subresource_tiling?branch=master) : describes a tiled subresource volume (refer to [Volume Tiled Resources](volume-tiled-resources.md)).
-   [**D3D12\_TEXTURE\_COPY\_LOCATION**](/windows/win32/D3D12/ns-d3d12-d3d12_texture_copy_location?branch=master) : describes a portion of a texture for the purpose of copying.
-   [**D3D12\_TILED\_RESOURCE\_COORDINATE**](/windows/win32/D3D12/ns-d3d12-d3d12_tiled_resource_coordinate?branch=master) : describes the coordinates of a tiled resource.

Methods:

-   [**ID3D12Device::GetCopyableFootprints**](/windows/win32/d3d12/nf-d3d12-id3d12device-getcopyablefootprints?branch=master) : gets information on a resource, to enable a copy to be made.
-   [**ID3D12Device::GetResourceTiling**](/windows/win32/d3d12/nf-d3d12-id3d12device-getresourcetiling?branch=master) : gets info about how a tiled resource is broken into tiles.
-   [**ID3D12GraphicsCommandList::ResolveSubresource**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-resolvesubresource?branch=master) : copies a multi-sampled subresource into a non-multi-sampled subresource.
-   [**ID3D12Resource::Map**](/windows/win32/D3D12/nf-d3d12-id3d12resource-map?branch=master) : returns a pointer to the specified data in the resource, and denies the GPU access to the subresource.
-   [**ID3D12Resource::ReadFromSubresource**](/windows/win32/d3d12/nf-d3d12-id3d12resource-readfromsubresource?branch=master) : copies data from a subresource, or a rectangular region of a subresource.
-   [**ID3D12Resource::Unmap**](/windows/win32/D3D12/nf-d3d12-id3d12resource-unmap?branch=master) : unmaps the specified range of memory and invalidates the pointer to the resource. Reinstates GPU access to the subresource.
-   [**ID3D12Resource::WriteToSubresource**](/windows/win32/d3d12/nf-d3d12-id3d12resource-writetosubresource?branch=master) : copies data to a subresource, or a rectangular region of a subresource.

Textures must be in the [**D3D12\_RESOURCE\_STATE\_COMMON**](/windows/win32/D3D12/ne-d3d12-d3d12_resource_states?branch=master) state for CPU access through [**WriteToSubresource**](/windows/win32/d3d12/nf-d3d12-id3d12resource-writetosubresource?branch=master) and [**ReadFromSubresource**](/windows/win32/d3d12/nf-d3d12-id3d12resource-readfromsubresource?branch=master) to be legal; but buffers do not. CPU access to a resource is typically done through [**Map**](/windows/win32/D3D12/nf-d3d12-id3d12resource-map?branch=master)/[**Unmap**](/windows/win32/D3D12/nf-d3d12-id3d12resource-unmap?branch=master).
