...
GPU-Based Validation (GBV) Preview enables new validation scenarios on the GPU timeline that is not possible to do during API calls on the CPU. GBV is available starting with the Graphics Tools for Windows 10 Anniversary Update.

-   [Purpose of GPU-Based Validation](#purpose-of-gpu-based-validation)
-   [Turning on GPU-Based Validation](#turning-on-gpu-based-validation)
-   [Recommended Usage](#recommended-usage)
-   [Debug Output](#debug-output)
-   [Example Debug Message](#example-debug-message)
-   [Known Issues](#known-issues)
-   [Debug Layer APIs](#debug-layer-apis)
...
GBV can be forced on using the DirectX Control Panel (DXCPL) by forcing on the D3D12 Debug Layer and additionally forcing on GPU-Based Validation (new tab in the control panel). Once enabled, GBV will remain enabled until the D3D12 device is released. Alternatively, GBV can be enabled programmatically prior to creating the D3D12 Device:
...
The Preview release has some known bugs:

-   False error when a resource is used in the DEPTH\_WRITE state for read-only access.
-   Resources with a very large number (hundreds) of subresources can cause device removal.
