...
Command queues can be created by calling [**ID3D12Device::CreateCommandQueue**](/windows/win32/D3D12/nf-d3d12-id3d12device-createcommandqueue?branch=master). This method takes a [**D3D12\_COMMAND\_LIST\_TYPE**](/windows/win32/D3D12/ne-d3d12-d3d12_command_list_type?branch=master) indicating what type of queue should be created, and therefore, what type of commands can be submitted to that queue. Remember that bundles can only be called from direct command lists and can't be submitted directly to a queue. The supported queue types are:

-   [**D3D12\_COMMAND\_LIST\_TYPE\_DIRECT**](/windows/win32/D3D12/ne-d3d12-d3d12_command_list_type?branch=master)
-   [**D3D12\_COMMAND\_LIST\_TYPE\_COMPUTE**](/windows/win32/D3D12/ne-d3d12-d3d12_command_list_type?branch=master)
-   [**D3D12\_COMMAND\_LIST\_TYPE\_COPY**](/windows/win32/D3D12/ne-d3d12-d3d12_command_list_type?branch=master)
...
After you have recorded a command list and either retrieved the default command queue or created a new one, you execute command lists by calling [**ID3D12CommandQueue::ExecuteCommandLists**](/windows/win32/d3d12/nf-d3d12-id3d12commandqueue-executecommandlists?branch=master).
...
The runtime will validate the submitted command list and will drop the call to [**ExecuteCommandLists**](/windows/win32/d3d12/nf-d3d12-id3d12commandqueue-executecommandlists?branch=master) if any of the restrictions are violated. Calls will be dropped for the following reasons:

-   The supplied command list is a bundle, not a direct command list.
-   [**ID3D12GraphicsCommandList::Close**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-close?branch=master) has not been called on the supplied command list to complete recording.
-   [**ID3D12CommandAllocator::Reset**](/windows/win32/D3D12/nf-d3d12-id3d12commandallocator-reset?branch=master) has been called on the command allocator associated with the command list since it was recorded. For more information about command allocators, see [Creating and recording command lists and bundles](recording-command-lists-and-bundles.md).
...
-   The before and after states of queries, set with calls to [**ID3D12GraphicsCommandList::BeginQuery**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-beginquery?branch=master) and [**ID3D12GraphicsCommandList::EndQuery**](/windows/win32/d3d12/nf-d3d12-id3d12graphicscommandlist-endquery?branch=master), are not matched properly.
...
A fence is an integer that represents the current unit of work being processed. When the app advances the fence, by calling [**ID3D12CommandQueue::Signal**](/windows/win32/D3D12/nf-d3d12-id3d12commandqueue-signal?branch=master), the integer is updated. Apps can check the value of a fence and determine if a unit of work has been completed in order to decide whether a subsequent operation can be started.
...
Methods for managing tiled resources, which were exposed through the [**ID3D11DeviceContext2**](https://msdn.microsoft.com/library/windows/desktop/dn280498) interface in Direct3D 11, are provided by the [**ID3D12CommandQueue**](/windows/win32/D3D12/nn-d3d12-id3d12commandqueue?branch=master) interface in Direct3D 12. These methods include:



| Method                                                              | Description                                                                                              |
|---------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| [**CopyTileMappings**](/windows/win32/d3d12/nf-d3d12-id3d12commandqueue-copytilemappings?branch=master)     | Copies mappings from a source tiled resource to a destination tiled resource.<br/>                 |
| [**UpdateTileMappings**](/windows/win32/d3d12/nf-d3d12-id3d12commandqueue-updatetilemappings?branch=master) | Updates mappings of tile locations in tiled resources to memory locations in a resource heap.<br/> |
