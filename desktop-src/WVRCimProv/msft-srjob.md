---
title: MSFT\_SrJob class
description: A generic and instantiable unit of work, such as a batch or a print job, used in storage replica to track the progress of asynchronous operations.
audience: developer
author: REDMOND\\markl
manager: REDMOND\\markl
ms.assetid: '49a05f71-cdb4-4098-8c25-4a7c9b1ab6bd'
ms.prod: 'windows-server-dev'
ms.technology:
- 'storage-replica'
- 'windows-management-instrumentation'
ms.tgt_platform: multiple
keywords: ["MSFT_SrJob class", "MSFT_SrJob class, described"]
topic_type:
- apiref
api_name:
- MSFT_SrJob
- MSFT_SrJob.Caption
- MSFT_SrJob.Description
- MSFT_SrJob.ElementName
- MSFT_SrJob.InstallDate
- MSFT_SrJob.OperationalStatus
- MSFT_SrJob.StatusDescriptions
- MSFT_SrJob.Status
- MSFT_SrJob.HealthState
- MSFT_SrJob.CommunicationStatus
- MSFT_SrJob.DetailedStatus
- MSFT_SrJob.OperatingStatus
- MSFT_SrJob.PrimaryStatus
- MSFT_SrJob.JobStatus
- MSFT_SrJob.TimeSubmitted
- MSFT_SrJob.ScheduledStartTime
- MSFT_SrJob.StartTime
- MSFT_SrJob.ElapsedTime
- MSFT_SrJob.JobRunTimes
- MSFT_SrJob.RunMonth
- MSFT_SrJob.RunDay
- MSFT_SrJob.RunDayOfWeek
- MSFT_SrJob.RunStartInterval
- MSFT_SrJob.LocalOrUtcTime
- MSFT_SrJob.UntilTime
- MSFT_SrJob.Notify
- MSFT_SrJob.Owner
- MSFT_SrJob.Priority
- MSFT_SrJob.PercentComplete
- MSFT_SrJob.DeleteOnCompletion
- MSFT_SrJob.ErrorCode
- MSFT_SrJob.ErrorDescription
- MSFT_SrJob.RecoveryAction
- MSFT_SrJob.OtherRecoveryAction
- MSFT_SrJob.InstanceID
- MSFT_SrJob.Name
- MSFT_SrJob.JobState
- MSFT_SrJob.TimeOfLastStateChange
- MSFT_SrJob.TimeBeforeRemoval
api_location:
- WvrCimProv.dll
api_type:
- DllExport
...
A generic and instantiable unit of work, such as a batch or a print job, used in storage replica to track the progress of asynchronous operations.
...
``` syntax
[UMLPackagePath("CIM::Core::CoreElements"), ClassVersion("1.0.0"), dynamic, provider("wvrcimprov"), AMENDMENT]
...
The **MSFT\_SrJob** class has these types of members:
...
The **MSFT\_SrJob** class has these methods.



| Method                                                      | Description                                                                                                                                                                                                                                                                                                                                              |
|:------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**GetError**](msft-srjob-geterror.md)                     | Returns a [**CIM\_Error**](cim-error.md) instance if the job fails or is terminated by a client.<br/> This method is inherited from the [**CIM\_ConcreteJob**](cim-concretejob.md) class.<br/>                                                                                                                                             |
| [**KillJob**](msft-srjob-killjob.md)                       | Kills this job and any underlying processes, and removes any associations containing this job. This method is deprecated, instead use the **RequestStateChange** method to distinguish between an orderly shutdown and an immediate shutdown without cleanup.<br/> This method is inherited from the [**CIM\_Job**](cim-job.md) class.<br/> |
| [**RequestStateChange**](msft-srjob-requeststatechange.md) | Requests that the state of the job be changed to the specified value. The allowed transitions are job and vendor specific.<br/> This method is inherited from the [**CIM\_ConcreteJob**](cim-concretejob.md) class.<br/>                                                                                                                    |
...
The **MSFT\_SrJob** class has these properties.
...
Specifies whether or not the job should be automatically deleted upon completion. Note that the completion of a recurring job is defined by its **JobRunTimes** or **UntilTime** properties, or by manual termination. If this property is set to **FALSE** before the job completes, setting it to **TRUE** after the job completes will not delete the job. Instead, to manually delete the job you must use the [CimSession](https://msdn.microsoft.com/library/microsoft.management.infrastructure.cimsession.aspx).[**DeleteInstance**](Overload:Microsoft.Management.Infrastructure.CimSession.DeleteInstance) method.
...
Qualifiers: [**ModelCorrespondence**](https://msdn.microsoft.com/library/aa393650) ("[**CIM\_EnabledLogicalElement**](https://msdn.microsoft.com/library/mt446063).**PrimaryStatus**", "[**CIM\_ManagedSystemElement**](cim-managedsystemelement.md).**HealthState**")
</dt> </dl>
...
The job is moving from the **New**, **Suspended**, or **Service** state to the **Running** state.
...
The job is in an abnormal state, which might indicate an error condition. Actual status might be displayed though job-specific objects.
...
Qualifiers: [**MaxLen**](https://msdn.microsoft.com/library/aa393650) (1024), [**Required**](https://msdn.microsoft.com/library/aa393650)
</dt> </dl>
