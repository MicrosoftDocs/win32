...
Peer groups require that each member have a specific certificate, which is called a Group Member Certificate (GMC). The GMC certificate ensures that all records exchanged between peers are digitally signed. The public key of a peer is contained in the structures that are passed as part of the communication between peers—including [**PEER\_CREDENTIAL\_INFO**](peer-credential-info.md).

A GMC can be issued in a chain. For example, a creator can issue a GMC to administrator A, who can issue a GMC to administrator B, who can issue a GMC to member M. The resulting GMC chain is: creator-&gt;A-&gt;B-&gt;M, which has a length of 4. The chain length is important, because it cannot be longer than 24. If the 24th administrator in a chain attempts to issue a GMC to a member, [**PeerGroupIssueCredentials**](peergroupissuecredentials.md) or [**PeerGroupCreateInvitation**](peergroupcreateinvitation.md) returns PEER\_E\_CHAIN\_TOO\_LONG.

A GMC can also be issued by calling [**PeerGroupIssueCredentials**](peergroupissuecredentials.md). A GMC implies user membership in the group that the GMC is issued for, and can have a finite or infinite lifetime. Group member activity cannot be longer than the lifetime specified in the GMC.
...
-   Call [**PeerGroupIssueCredentials**](peergroupissuecredentials.md) and pass the identity of the member whose lifetime is ready to expire. If the credentials are specified as **NULL** and the peer's membership information is available to the group, the renewal time is set to the same duration that is specified in the previously published member credentials. If a different duration period must be specified, a new [**PEER\_CREDENTIAL\_INFO**](peer-credential-info.md) structure must be provided that contains the new lifetime duration, and then a new GMC is published for the member.

    The [**PeerGroupIssueCredentials**](peergroupissuecredentials.md) function optionally takes the PEER\_GROUP\_STORE\_CREDENTIALS flag, which automatically publishes the new credentials of the member in the group database. When the member connects to the group the next time, for either the first time or after going offline, the member obtains the updated GMC from the database.

-   Call [**PeerGroupCreateInvitation**](peergroupcreateinvitation.md) to pass the identity of the member whose GMC lifetime is ready to expire. If the expiration specified in the invitation is **NULL**, the lifetime duration of the GMC is the same as the administrator GMC who issues membership. If the creator specifies the expiration as **NULL**, a GMC with an infinite lifetime is issued.

    If a peer is issued a GMC that expires before the presence lifetime value, the addresses of the peer are not available in the [**PEER\_MEMBER**](peer-member.md) structures returned in the enumeration initiated by a call to [**PeerGroupEnumMembers**](peergroupenummembers.md). This happens because presence information is not published in this case, even if the PEER\_DISABLE\_PRESENCE flag is not set.
