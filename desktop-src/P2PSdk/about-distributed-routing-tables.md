...
| DRT\_FAULTED     | A serious error has occurred within the local node. For example, if the local machine runs out of physical memory.<br/> While a node enters this state, the application must call the [**DrtClose**](/windows/win32/drt/nf-drt-drtclose?branch=master) API, correct the problem, and re-initialize the DRT with [**DrtOpen**](/windows/win32/drt/nf-drt-drtopen?branch=master).<br/>                                                                                                   |
