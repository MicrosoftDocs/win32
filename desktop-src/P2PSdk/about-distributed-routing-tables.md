...
| DRT\_FAULTED     | A serious error has occurred within the local node. For example, if the local machine runs out of physical memory.<br/> While a node enters this state, the application must call the [**DrtClose**](drtclose.md) API, correct the problem, and re-initialize the DRT with [**DrtOpen**](drtopen.md).<br/>                                                                                                   |
