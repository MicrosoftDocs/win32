...
1.  Obtain a handle to the enumeration. Call a **PeerEnum** function, for example, call the [**PeerGraphEnumRecords**](peergraphenumrecords.md) Peer Graphing function. The **PeerEnum** functions create the enumeration, and return a handle to that enumeration to the calling application. This handle must be used to retrieve the results.
2.  Optionally, obtain the number of entities in the enumeration. Call a **GetItemCount** function, for example, call [**PeerGraphGetItemCount**](peergraphgetitemcount.md) or [**PeerGetItemCount**](peergetitemcount.md).
    > [!Note]  
    > You can use the value returned by the **GetItemCount** function to determine the number of items available to retrieve.

     

3.  Retrieve a group of results. Call a **GetNextItem** function, for example, call [**PeerGraphGetNextItem**](peergraphgetnextitem.md).
    > [!Note]  
...
4.  After the data is not needed, free the pointer to the data. Call a **FreeData** function, for example, call [**PeerGraphFreeData**](peergraphfreedata.md) or [**PeerFreeData**](peerfreedata.md).
    > [!Note]  
    > For more information, see [Freeing Peer Data](freeing-peer-data.md).

     

5.  After the application does not need the handle to the enumeration, release it. Call an **EndEnumeration** function, for example, call [**PeerEndEnumeration**](peerendenumeration.md) or [**PeerGraphEndEnumeration**](peergraphendenumeration.md).
