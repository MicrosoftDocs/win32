...
To draw a line with GDI, you need two objects: a device context and a pen. You get a handle to a device context by calling [**BeginPaint**](https://msdn.microsoft.com/en-us/library/Dd183362(v=VS.85).aspx), and a handle to a pen by calling [CreatePen](https://msdn.microsoft.com/library/en-us/gdi/pens_9wha.asp). Next, you call [SelectObject](https://msdn.microsoft.com/library/en-us/gdi/devcons_9v3o.asp) to select the pen into the device context. You set the pen position to (20, 10) by calling [MoveToEx](https://msdn.microsoft.com/library/en-us/gdi/linecurv_4660.asp) and then draw a line from that pen position to (200, 100) by calling **LineTo**. Note that MoveToEx and [LineTo](https://msdn.microsoft.com/library/en-us/gdi/linecurv_7567.asp) both receive **hdc** as an argument.
...
GDI+ is more flexible than GDI when it comes to drawing the outlines and filling the interiors of shapes like rectangles. GDI has a [Rectangle](https://msdn.microsoft.com/library/en-us/gdi/fillshap_6kth.asp) function that draws the outline and fills the interior of a rectangle all in one step. The outline is drawn with the currently selected pen, and the interior is filled with the currently selected brush.
