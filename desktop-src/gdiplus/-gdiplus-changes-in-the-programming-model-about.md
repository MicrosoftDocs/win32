...
The preceding examples show that [**Pen**](-gdiplus-class-pen-class.md) objects can be created and maintained separately from the [**Graphics**](-gdiplus-class-graphics-class.md) object, which supplies the drawing methods. [**Brush**](-gdiplus-class-brush-class.md), [**GraphicsPath**](-gdiplus-class-graphicspath-class.md), [**Image**](-gdiplus-class-image-class.md), and [**Font**](-gdiplus-class-font-class.md) objects can also be created and maintained separately from the **Graphics** object. Many of the drawing methods provided by the **Graphics** class receive a **Brush**, **GraphicsPath**, **Image**, or **Font** object as an argument. For example, the address of a **Brush** object is passed as an argument to the [FillRectangle](-gdiplus-class-graphics-fillrectangle-methods.md) method, and the address of a **GraphicsPath** object is passed as an argument to the [**Graphics::DrawPath**](-gdiplus-class-graphics-drawpath-pen-path-.md) method. Similarly, addresses of **Image** and **Font** objects are passed to the [DrawImage](-gdiplus-class-graphics-drawimage-methods.md) and [DrawString](-gdiplus-class-graphics-drawstring-methods.md) methods. This is in contrast to GDI where you select a brush, path, image, or font into the device context and then pass a handle to the device context as an argument to a drawing function.

## Method Overloading

Many of the GDI+ methods are overloaded; that is, several methods share the same name but have different parameter lists. For example, the [DrawLine](-gdiplus-class-graphics-drawline-methods.md) method of the [**Graphics**](-gdiplus-class-graphics-class.md) class comes in the following forms:
...
All four of the [DrawLine](-gdiplus-class-graphics-drawline-methods.md) variations above receive a pointer to a [**Pen**](-gdiplus-class-pen-class.md) object, the coordinates of the starting point, and the coordinates of the ending point. The first two variations receive the coordinates as floating point numbers, and the last two variations receive the coordinates as integers. The first and third variations receive the coordinates as a list of four separate numbers, while the second and fourth variations receive the coordinates as a pair of [**Point**](-gdiplus-class-point-class.md) (or [**PointF**](-gdiplus-class-pointf-class.md)) objects.

## No More Current Position

Note that in the [DrawLine](-gdiplus-class-graphics-drawline-methods.md) methods shown previously both the starting point and the ending point of the line are received as arguments. This is a departure from the GDI scheme where you call to set the current pen position followed by to draw a line starting at (**x1**, **y1**) and ending at (**x2**, **y2**). GDI+ as a whole has abandoned the notion of current position.
...
GDI+ has separate methods for drawing the outline and filling the interior of a rectangle. The [DrawRectangle](-gdiplus-class-graphics-drawrectangle-methods.md) method of the [**Graphics**](-gdiplus-class-graphics-class.md) class has the address of a [**Pen**](-gdiplus-class-pen-class.md) object as one of its parameters, and the [FillRectangle](-gdiplus-class-graphics-fillrectangle-methods.md) method has the address of a [**Brush**](-gdiplus-class-brush-class.md) object as one of its parameters.
...
Note that the [FillRectangle](-gdiplus-class-graphics-fillrectangle-methods.md) and [DrawRectangle](-gdiplus-class-graphics-drawrectangle-methods.md) methods in GDI+ receive arguments that specify the rectangle's left edge, top, width, and height. This is in contrast to the GDI[Rectangle](http://msdn.microsoft.com/library/en-us/gdi/fillshap_6kth.asp) function, which takes arguments that specify the rectangle's left edge, right edge, top, and bottom. Also note that the constructor for the [**Color**](-gdiplus-class-color-class.md) class in GDI+ has four parameters. The last three parameters are the usual red, green, and blue values; the first parameter is the alpha value, which specifies the extent to which the color being drawn is blended with the background color.

## Constructing Regions

GDI provides several functions for creating regions: CreateRectRgn, CreateEllpticRgn, CreateRoundRectRgn, CreatePolygonRgn, and CreatePolyPolygonRgn. You might expect the [**Region**](-gdiplus-class-region-class.md) class in GDI+ to have analogous constructors that take rectangles, ellipses, rounded rectangles, and polygons as arguments, but that is not the case. The **Region** class in GDI+ provides a constructor that receives a [**Rect**](-gdiplus-class-rect-class.md) object reference and another constructor that receives the address of a [**GraphicsPath**](-gdiplus-class-graphicspath-class.md) object. If you want to construct a region based on an ellipse, rounded rectangle, or polygon, you can easily do so by creating a **GraphicsPath** object (that contains an ellipse, for example) and then passing the address of that **GraphicsPath** object to a **Region** constructor.

GDI+ makes it easy to form complex regions by combining shapes and paths. The [**Region**](-gdiplus-class-region-class.md) class has [Union](-gdiplus-class-region-union-methods.md) and [Intersect](-gdiplus-class-region-intersect-methods.md) methods that you can use to augment an existing region with a path or another region. One nice feature of the GDI+ scheme is that a [**GraphicsPath**](-gdiplus-class-graphicspath-class.md) object is not destroyed when it is passed as an argument to a **Region** constructor. In GDI, you can convert a path to a region with the [PathToRegion](http://msdn.microsoft.com/library/en-us/gdi/paths_56lq.asp) function, but the path is destroyed in the process. Also, a **GraphicsPath** object is not destroyed when its address is passed as an argument to a Union or Intersect method, so you can use a given path as a building block for several separate regions. This is shown in the following example. Assume that **onePath** is a pointer to a **GraphicsPath** object (simple or complex) that has already been initialized.
