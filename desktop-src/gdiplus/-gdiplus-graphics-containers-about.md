...
Note that all three transformations apply to the [**Graphics::DrawEllipse**](/windows/desktop/api/Gdiplusgraphics/) call made in the second (innermost) container. Also note the order of the transformations: first scale, then rotate, then translate. The innermost transformation is applied first, and the outermost transformation is applied last.
