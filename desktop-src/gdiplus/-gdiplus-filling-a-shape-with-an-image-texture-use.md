...
You can fill a closed shape with a texture by using the [**Image**](-gdiplus-class-image-class.md) class and the [**TextureBrush**](-gdiplus-class-texturebrush-class.md) class.

The following example fills an ellipse with an image. The code constructs an [**Image**](-gdiplus-class-image-class.md) object, and then passes the address of that **Image** object as an argument to a [**TextureBrush**](-gdiplus-class-texturebrush-class.md) constructor. The third code statement scales the image, and the fourth statement fills the ellipse with repeated copies of the scaled image:
