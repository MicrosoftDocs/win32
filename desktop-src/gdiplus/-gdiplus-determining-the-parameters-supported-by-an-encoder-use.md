...
Each of the [**EncoderParameter**](/windows/desktop/api/Gdiplusimaging/) objects in the array has the following four public data members:

The following code is a continuation of the console application shown in the preceding example. The code looks at the second [**EncoderParameter**](/windows/desktop/api/Gdiplusimaging/) object in the array returned by [**Image::GetEncoderParameterList**](/windows/desktop/api/Gdiplusheaders/nf-gdiplusheaders-image-getencoderparameterlist). The code calls [StringFromGUID2](http://msdn.microsoft.com/library/en-us/com/htm/cmf_m2z_2jzm.asp), which is a system function declared in Objbase.h, to convert the **Guid** member of the **EncoderParameter** object to a string.
...
You can look up the GUID in Gdiplusimaging.h and find out that the category of this [**EncoderParameter**](/windows/desktop/api/Gdiplusimaging/) object is EncoderQuality. You can use this category (EncoderQuality) of parameter to set the compression level of a JPEG image.
...
The number of values is one, so we know that the **Value** member of the [**EncoderParameter**](/windows/desktop/api/Gdiplusimaging/) object is a pointer to an array that has one element. That one element is a pair of **ULONG** values.
