...
A single [**Matrix**](-gdiplus-class-matrix-class.md) object can store a single transformation or a sequence of transformations. The latter is called a *composite*  *transformation*. The matrix of a composite transformation is obtained by multiplying the matrices of the individual transformations.
...
The following example combines scaling, rotation and translation (in that order) to form a composite transformation. The argument [****MatrixOrderAppend****](-gdiplus-enum-matrixorder.md) passed to the [**Graphics::RotateTransform**](-gdiplus-class-graphics-rotatetransform-angle-order-.md) method specifies that the rotation will follow the scaling. Likewise, the argument [****MatrixOrderAppend****](-gdiplus-enum-matrixorder.md) passed to the [**Graphics::TranslateTransform**](-gdiplus-class-graphics-translatetransform-dx-dy-order-.md) method specifies that the translation will follow the rotation.
