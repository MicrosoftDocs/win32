...
-   A server calls [**NotifyWinEvent**](notifywinevent.md) to generate a WinEvent notification for each change to its user interface elements.
-   The WinEvent management code in USER determines if any client applications have registered a WinEvent [*hook function*](wineventproc-callback-function.md) using [**SetWinEventHook**](setwineventhook.md) and calls the registered callback function.
-   In its callback function, the client requests access to the object that generated the event by calling [**AccessibleObjectFromEvent**](accessibleobjectfromevent.md) or other accessible object retrieval functions. For more information, see [Retrieving an IAccessible Object](retrieving-an-iaccessible-object.md).
...
-   If the server returns zero, Microsoft Active Accessibility creates a proxy object and gives its address to the client. Otherwise, Microsoft Active Accessibility uses this reference to retrieve the address of an object interface such as [**IAccessible**](iaccessible.md) or [**IDispatch**](idispatch-interface.md), and gives that address to the client.

Once the client has an interface address, it can call the [**IAccessible**](iaccessible.md) properties and methods of the accessible object to retrieve information.
