...
-   If the window or control that receives [**WM\_GETOBJECT**](wm-getobject.md) implements [**IAccessible**](iaccessible.md), the window returns a reference to the **IAccessible** interface using [**LresultFromObject**](lresultfromobject.md). Microsoft Active Accessibility, in conjunction with the Component Object Model (COM) library, performs the appropriate marshaling and passes the interface pointer from the server back to the client.
-   If the window that receives the message does not implement [**IAccessible**](iaccessible.md), it should return zero.
