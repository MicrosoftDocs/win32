...
keywords: ["UI Automation,Active Accessibility servers", "UI Automation,Microsoft Active Accessibility servers", "UI Automation,IAccessible", "UI Automation,IAccessibleEx", "UI Automation,exposing IAccessibleEx", "UI Automation,implementing IAccessibleEx", "IAccessible", "IAccessibleEx", "Microsoft Active Accessibility", "Active Accessibility", "exposing IAccessibleEx", "implementing IAccessibleEx"]
---

# Adding UI Automation Functionality to Active Accessibility Servers

Controls that do not have a Microsoft UI Automation provider but that implement [**IAccessible**](iaccessible.md) can easily be upgraded to provide some UI Automation functionality, by implementing the [**IAccessibleEx**](uiauto-iaccessibleex.md) interface. This interface enables the control to expose UI Automation properties and control patterns, without the need for a full implementation of UI Automation provider interfaces such as [**IRawElementProviderFragment**](uiauto-irawelementproviderfragment.md). To implement **IAccessibleEx**, the baseline Microsoft Active Accessibility object hierarchy must contain no errors or inconsistencies (such as a child object whose parent object does not list it as a child), and must not conflict with UI Automation specifications. If the Microsoft Active Accessibility object hierarchy meets these requirements, it is a good candidate for adding functionality by using **IAccessibleEx**; otherwise, you must implement UI Automation alone or alongside the Microsoft Active Accessibility implementation.
...
-   Implement [IServiceProvider](http://go.microsoft.com/fwlink/p/?linkid=178086) on the accessible object so that the [**IAccessibleEx**](uiauto-iaccessibleex.md) interface can be found on this or a separate object.
-   Implement [**IAccessibleEx**](uiauto-iaccessibleex.md) on the accessible object.
-   Create distinct accessible objects for any Microsoft Active Accessibility child items, which in Microsoft Active Accessibility might have been represented by the [**IAccessible**](iaccessible.md) interface on the parent object (for example, list items). Implement [**IAccessibleEx**](uiauto-iaccessibleex.md) on these objects.
...
The method of [**IAccessibleEx**](uiauto-iaccessibleex.md) that is of most interest is [**GetObjectForChild**](uiauto-iaccessibleex-getobjectforchild.md). This method gives the Microsoft Active Accessibility server an opportunity to create an accessible object (one that exposes, at a minimum, **IAccessibleEx**) for a child item. In Microsoft Active Accessibility, child items typically are not represented as accessible objects but as children of an accessible object. However, because UI Automation requires that each element be represented by a separate accessible object, **GetObjectForChild** must create a separate object for each child on demand.
