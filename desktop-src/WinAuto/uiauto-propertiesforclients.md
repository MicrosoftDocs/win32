...
If a UI Automation provider does not implement a property, UI Automation can supply a default value. For example, if the provider for a control does not support the property identified by [**UIA\_HelpTextPropertyId**](https://www.bing.com/search?q=**UIA\_HelpTextPropertyId**), UI Automation returns an empty string. Similarly, if the provider does not support the property identified by [**UIA\_IsDockPatternAvailablePropertyId**](https://www.bing.com/search?q=**UIA\_IsDockPatternAvailablePropertyId**), UI Automation returns **FALSE**.
