...
To notify clients of an event occurrence, servers call [**NotifyWinEvent**](notifywinevent.md). The system checks whether any client applications have set hook functions for the event and calls the appropriate hook functions as necessary.

When the client's hook function is called, it receives a number of parameters that describe the event and the object that generated the event. To gain access to the object that generated the event, the client hook function calls [**AccessibleObjectFromEvent**](accessibleobjectfromevent.md).

> [!Note] If no clients have registered to receive WinEvents, the performance impact on a server for calling [**NotifyWinEvent**](notifywinevent.md) is negligible.
>
> Servers call [**NotifyWinEvent**](notifywinevent.md) for changes only in their own accessible objects; they do not call **NotifyWinEvent** for changes in system-provided user interface elements.
...
Inside the hook procedure, the client can call [**AccessibleObjectFromEvent**](accessibleobjectfromevent.md) to retrieve an [**IAccessible**](iaccessible.md) object for the element to which the event applies. With this object, the client can begin calling **IAccessible** methods to retrieve information or interact with the UI element.
