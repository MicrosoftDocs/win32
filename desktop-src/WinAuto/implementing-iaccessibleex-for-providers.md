...
An [**IAccessibleEx**](/windows/desktop/api/UIAutomationCore/nn-uiautomationcore-iaccessibleex) server exposes control patterns by implementing [**IRawElementProviderSimple::GetPatternProvider**](/windows/desktop/api/UIAutomationCore/nf-uiautomationcore-irawelementprovidersimple-getpatternprovider). This method takes an integer parameter that specifies the control pattern. The server returns **NULL** if the pattern is not supported. If the control pattern interface is supported, servers return an [**IUnknown**](https://msdn.microsoft.com/library/windows/desktop/ms680509) and the client then calls [**QueryInterface**](https://msdn.microsoft.com/library/windows/desktop/ms682521) to get the appropriate control pattern.
