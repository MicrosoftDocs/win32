...
Microsoft Active Accessibility provides application programming interfaces (APIs) for both clients and servers. Most are implemented in the Microsoft Active Accessibility dynamic-link library, Oleacc.dll, but [**NotifyWinEvent**](notifywinevent.md), [**SetWinEventHook**](setwineventhook.md), and [**UnhookWinEvent**](unhookwinevent.md) are implemented in user32.dll, which is a core component of the Microsoft Windows operating system.
...
For example, if an application uses [**NotifyWinEvent**](notifywinevent.md), it calls [**GetProcAddress**](https://msdn.microsoft.com/library/windows/desktop/ms683212) using the module handle of user32.dll to get the address of the function. If the call is successful (meaning that this version of Windows supports Microsoft Active Accessibility), then the application sets a flag that indicates that it is safe to call **NotifyWinEvent**. The address received from **GetProcAddress** is stored in a function pointer variable and used throughout the code.
