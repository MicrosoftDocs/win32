...
Describes guidelines and conventions for implementing [**IItemContainerProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-iitemcontainerprovider?branch=master), including information about methods. The **ItemContainer** control pattern is used to support item virtualization.
...
To provide information about virtualized items, providers implement the **ItemContainer** control pattern, which exposes the [**IItemContainerProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-iitemcontainerprovider?branch=master) interface. The [**FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) method finds child items based on the value of a particular property, such as **Name**, **AutomationId**, or **IsSelected**. If an item is virtualized, **FindItemByProperty** retrieves a UI Automation placeholder element for the item. A placeholder element is an implementation of the [**IRawElementProviderSimple**](/windows/win32/UIAutomationCore/nn-uiautomationcore-irawelementprovidersimple?branch=master) interface that supports only the [VirtualizedItem](uiauto-implementingvirtualizeditem.md) control pattern.

The [**IVirtualizedItemProvider::Realize**](/windows/win32/UIAutomationCore/nf-uiautomationcore-ivirtualizeditemprovider-realize?branch=master) method enables a client to request that a virtualized item be realized, thereby exposing a full UI Automation element for the item so that all required properties and patterns are available.
...
-   When a container is virtualized, other controls patterns such as [Selection](uiauto-implementingselection.md), [Table](uiauto-implementingtable.md), and [Grid](uiauto-implementinggrid.md) can be affected. For example, the [**ISelectionProvider::GetSelection**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iselectionprovider-getselection?branch=master) method may support only elements that are in the viewport, or only selected elements that are currently not virtualized.
...
The following guidelines and requirements apply to the [**IItemContainerProvider::FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) method.

-   While not required, Microsoft highly recommends that [**FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) support the **Name**, **AutomationId**, and **IsSelected** properties.
-   [**FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) can be slow if it needs to traverse multiple objects to find a matching one.
-   [**FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) can be called repeatedly to find items in sequence. The items can be in any order as long as each item is returned only once.
-   [**FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) may be implemented to find only those elements that appear in the control or content view of the UI Automation tree. Elements that appear only in raw view can be skipped to avoid retrieving multiple elements that represent only a portion of an "item" to the user.
...
-   When no item is found, [**IItemContainerProvider::FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) should set the *pFound* parameter to **NULL** and return **S\_OK**.
...
The following method is required for implementing the [**IItemContainerProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-iitemcontainerprovider?branch=master) interface.



| Required members                                                               | Member type | Notes |
|--------------------------------------------------------------------------------|-------------|-------|
| [**FindItemByProperty**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iitemcontainerprovider-finditembyproperty?branch=master) | Method      | None  |
