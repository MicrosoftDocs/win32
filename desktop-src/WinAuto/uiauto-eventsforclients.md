...
When a client adds an event handler for all descendants ([**TreeScope\_Descendants**](https://www.bing.com/search?q=**TreeScope\_Descendants**)), UI Automation adds only one handler for the root of sub-tree, and the handler listens across all descendents. UI Automation does not add event handlers recursively.
