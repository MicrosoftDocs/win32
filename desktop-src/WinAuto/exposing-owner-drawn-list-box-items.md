...
-   When adding items to the owner-drawn list box, use the [LB\_ADDSTRING](https://go.microsoft.com/fwlink/p/?linkid=178091) message to add the text that you want Microsoft Active Accessibility to expose. This text is not displayed, so it is not part of the owner-drawn data. Add the owner-drawn item data using the [LB\_SETITEMDATA](https://go.microsoft.com/fwlink/p/?linkid=178092) message.

When using the above method, note the following:

-   If you use the **LBS\_SORT** style, the list box is sorted using the supplied strings and not the [WM\_COMPAREITEM](https://go.microsoft.com/fwlink/p/?linkid=178093) callback procedure.
-   With owner-drawn variable list boxes created with the style **LBS\_OWNERDRAWVARIABLE**, use a global variable or some other mechanism to keep track of when the **itemData member** of the [MEASUREITEMSTRUCT](https://go.microsoft.com/fwlink/p/?linkid=178094) is valid. The global variable is needed because the system sends the [WM\_MEASUREITEM](https://go.microsoft.com/fwlink/p/?linkid=178096) message as soon as the string is added but before the item data is attached, and at this point the **itemData** member is not valid.
-   To change the string for an item in a list box with the **LBS\_HASSTRINGS** style, delete the item with the [LB\_DELETESTRING](https://go.microsoft.com/fwlink/p/?linkid=178097) message and add the new string with the LB\_ADDSTRING message.
