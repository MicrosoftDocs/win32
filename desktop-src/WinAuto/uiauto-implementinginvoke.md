...
Describes guidelines and conventions for implementing [**IInvokeProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-iinvokeprovider?branch=master), including information about methods. The **Invoke** control pattern is used to support controls that do not maintain state when activated but rather initiate or perform a single, unambiguous action.

Controls that do maintain state, such as check boxes and radio buttons, must instead implement [**IToggleProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-itoggleprovider?branch=master) and [**ISelectionProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-iselectionprovider?branch=master) respectively. For examples of controls that implement this control pattern, see [Control Types and Their Supported Control Patterns](uiauto-controlpatternmapping.md).
...
-   Controls implement [**IInvokeProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-iinvokeprovider?branch=master) if the same behavior is not exposed through another control pattern provider. For example, if the [**IUIAutomationInvokePattern::Invoke**](/windows/win32/UIAutomationClient/nf-uiautomationclient-iuiautomationinvokepattern-invoke?branch=master) method on a control performs the same action as the [**IUIAutomationExpandCollapsePattern::Expand**](/windows/win32/UIAutomationClient/nf-uiautomationclient-iuiautomationexpandcollapsepattern-expand?branch=master) or [**Collapse**](/windows/win32/UIAutomationClient/nf-uiautomationclient-iuiautomationexpandcollapsepattern-collapse?branch=master) method, the control should not implement **IInvokeProvider**.
...
The following method is required for implementing the [**IInvokeProvider**](/windows/win32/UIAutomationCore/nn-uiautomationcore-iinvokeprovider?branch=master) interface.



| Required members                                | Member type | Notes                                                                                                                                                                                                                                                                                                                                  |
|-------------------------------------------------|-------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**Invoke**](/windows/win32/UIAutomationCore/nf-uiautomationcore-iinvokeprovider-invoke?branch=master) | Method      | **Invoke** is an asynchronous call and must return immediately without blocking.<br/> This behavior is particularly critical for controls that, directly or indirectly, launch a modal dialog when invoked. Any UI Automation client that instigated the event will remain blocked until the modal dialog is closed. <br/> |
