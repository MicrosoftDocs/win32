...
keywords: ["UI Automation,implementing Invoke control pattern", "UI Automation,Invoke control pattern", "UI Automation,IInvokeProvider", "IInvokeProvider", "implementing UI Automation Invoke control patterns", "Invoke control patterns", "control patterns,IInvokeProvider", "control patterns,implementing UI Automation Invoke", "control patterns,Invoke", "interfaces,IInvokeProvider"]
---

# Invoke Control Pattern

Describes guidelines and conventions for implementing [**IInvokeProvider**](uiauto-iinvokeprovider.md), including information about methods. The **Invoke** control pattern is used to support controls that do not maintain state when activated but rather initiate or perform a single, unambiguous action.

Controls that do maintain state, such as check boxes and radio buttons, must instead implement [**IToggleProvider**](uiauto-itoggleprovider.md) and [**ISelectionProvider**](uiauto-iselectionprovider.md) respectively. For examples of controls that implement this control pattern, see [Control Types and Their Supported Control Patterns](uiauto-controlpatternmapping.md).
...
-   Controls implement [**IInvokeProvider**](uiauto-iinvokeprovider.md) if the same behavior is not exposed through another control pattern provider. For example, if the [**IUIAutomationInvokePattern::Invoke**](uiauto-iuiautomationinvokepattern-invoke.md) method on a control performs the same action as the [**IUIAutomationExpandCollapsePattern::Expand**](uiauto-iuiautomationexpandcollapsepattern-expand.md) or [**Collapse**](uiauto-iuiautomationexpandcollapsepattern-collapse.md) method, the control should not implement **IInvokeProvider**.
...
The following method is required for implementing the [**IInvokeProvider**](uiauto-iinvokeprovider.md) interface.



| Required members                                | Member type | Notes                                                                                                                                                                                                                                                                                                                                  |
|-------------------------------------------------|-------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**Invoke**](uiauto-iinvokeprovider-invoke.md) | Method      | **Invoke** is an asynchronous call and must return immediately without blocking.<br/> This behavior is particularly critical for controls that, directly or indirectly, launch a modal dialog when invoked. Any UI Automation client that instigated the event will remain blocked until the modal dialog is closed. <br/> |
