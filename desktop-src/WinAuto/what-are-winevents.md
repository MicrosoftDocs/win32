...
Event generation for **HWND**-based controls is especially important for server developers. The Microsoft Active Accessibility run-time provides [**IAccessible**](iaccessible.md) proxies for all standard UI elements. Similarly, the system automatically generates the appropriate WinEvents whenever it creates, destroys, moves, resizes, or performs any other action on an **HWND**-based control.
...
When an event occurs that affects the UI, servers can broadcast an event notification to all interested clients by calling the [**NotifyWinEvent**](notifywinevent.md) function. The function call includes information that identifies the type of event that occurred, and the UI element to which the event applies. Clients can use this information to retrieve an [**IAccessible**](iaccessible.md) object for the UI element and collect more information.

For example, to notify clients that a control's name has changed, a server calls [**NotifyWinEvent**](notifywinevent.md) and passes [**EVENT\_OBJECT\_NAMECHANGE**](event-constants.md#event-object-namechange) in the event parameter. The system responds by determining which clients have registered to receive that particular event and calls their registered callback function. If no clients have registered for the event, the server's call to **NotifyWinEvent** is comparable to a "no operation" and the performance impact is negligible.

Servers call [**NotifyWinEvent**](notifywinevent.md) to announce the event to the system after the event has occurred. They must never notify the system of an event before the event occurs.

To be notified of events, clients register callback hook functions by using [**SetWinEventHook**](setwineventhook.md). Clients set a single hook function for all possible events or multiple hook functions for discrete ranges of events. For more information, see [Registering a Hook Function](registering-a-hook-function.md).

When Microsoft Active Accessibility is notified of an event, it calls any hook functions that were registered for that event, passing along the parameters from [**NotifyWinEvent**](notifywinevent.md).
