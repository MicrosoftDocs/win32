...
-   If you use [**AnnotationType\_Unknown**](https://www.bing.com/search?q=**AnnotationType\_Unknown**), you must implement the [**IAnnotationProvider::AnnotationTypeName**](https://msdn.microsoft.com/library/windows/desktop/hh448759) property to enable clients to discover the name of the annotation type. You do not need to implement **AnnotationTypeName** for a standard annotation type because UI Automation provides a default name, but you can implement it if you need to override the default name.
...
-   Annotations in a text-based control can be complex if the control contains overlapping comments. Use the following guidelines to handle complex annotations:
    -   A text range with no annotations should return an empty array for the [**AnnotationTypes**](https://www.bing.com/search?q=**AnnotationTypes**) text attribute and an empty array for the [**AnnotationObjects**](https://www.bing.com/search?q=**AnnotationObjects**) text attribute.
    -   A text range with one annotation should return an array of one integer value for the [**AnnotationTypes**](https://www.bing.com/search?q=**AnnotationTypes**) text attribute and an array of one [**IRawElementProviderSimple**](/windows/desktop/api/UIAutomationCore/nn-uiautomationcore-irawelementprovidersimple) interface for the [**AnnotationObjects**](https://www.bing.com/search?q=**AnnotationObjects**) text attribute.
    -   A text range with multiple annotations should return an array of multiple integer values for the [**AnnotationTypes**](https://www.bing.com/search?q=**AnnotationTypes**) text attribute and an array of a matching number of [**IRawElementProviderSimple**](/windows/desktop/api/UIAutomationCore/nn-uiautomationcore-irawelementprovidersimple) interfaces for the [**AnnotationObjects**](https://www.bing.com/search?q=**AnnotationObjects**) text attribute.
    -   A text range with varying annotations, such as a range with both annotated and non-annotated text, should return the [**ReservedMixedAttributeValue**](/windows/desktop/api/UIAutomationCoreApi/nf-uiautomationcoreapi-uiagetreservedmixedattributevalue) property for both [**AnnotationTypes**](https://www.bing.com/search?q=**AnnotationTypes**) and [**AnnotationObjects**](https://www.bing.com/search?q=**AnnotationObjects**). A client receiving this response can subdivide the text range to find where the annotations begin and end.
