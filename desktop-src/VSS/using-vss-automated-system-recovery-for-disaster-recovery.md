...
-   Boot Configuration Data (BCD) store. This component specifies the path of the directory that contains the BCD store. The requester must specify this component and back up all of the files in the BCD store directory. For more information about the BCD store, see [About BCD](https://msdn.microsoft.com/windows/desktop/f0c9f54b-b178-44cf-aa70-607a167972be).
    > [!Note]  
...
-   For disk components, the format is
    ...
    where *n* is the disk number. Only the disk number is recorded. To get the disk number, use the [**IOCTL\_STORAGE\_GET\_DEVICE\_NUMBER**](https://msdn.microsoft.com/windows/desktop/2cd9610b-aa83-4d0a-a7a9-1d4dab8be331) control code.
...
-   A dynamic disk is not re-created if its disk layout is intact or only additive changes have been made to it. For a dynamic disk to be intact, all of the conditions for a basic disk must be met. In addition, the entire disk pack's volume structure must be intact. The disk pack's volume structure is intact if it meets the following conditions, which apply to both MBR and GPT disks:
    ...
    -   The number of [*plexes*](https://www.bing.com/search?q=*plexes*) per volume must be unchanged.
    -   The number of [*members*](https://www.bing.com/search?q=*members*) must be unchanged.
...
5.  Set the **HKLM**\\**SOFTWARE**\\**Microsoft**\\**Windows NT**\\**CurrentVersion**\\**ASR**\\**RestoreSession** registry key to indicate the set of volumes that have been restored or reformatted.
    ...
    You should also create a value named **LastInstance** with the data type REG\_SZ. This key should contain a random cookie that uniquely identifies the current restore operation. Such a cookie can be created by using the [**UuidCreate**](https://msdn.microsoft.com/windows/desktop/4008fb54-7770-4f1a-8e1c-4b20bef884f9) and [**UuidToString**](https://msdn.microsoft.com/windows/desktop/49235b28-a0c5-4f69-9932-85350d7bcbb8) functions. Each time a bare-metal recovery is performed, ASR resets this registry value to notify requesters and non-VSS backup applications that the recovery has occurred.
...
Under this key, specify **LastInstance** for the value name, REG\_SZ for the value type, and a random cookie (such as a GUID created by the [**UuidCreate**](https://msdn.microsoft.com/windows/desktop/4008fb54-7770-4f1a-8e1c-4b20bef884f9) function) for the value data.
