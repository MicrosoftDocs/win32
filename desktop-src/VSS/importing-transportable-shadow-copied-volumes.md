...
4.  With a shadow copy context of **VSS\_CTX\_APP\_BACKUP** \| **VSS\_VOLSNAP\_ATTR\_TRANSPORTABLE**, the requester that is on *systemOne* creates a shadow copy by calling [**IVssBackupComponents::DoSnapshotSet**](ivssbackupcomponents-dosnapshotset.md).
5.  *SystemOne* uses [**IVssBackupComponents::SaveAsXML**](ivssbackupcomponents-saveasxml.md) to save the current state of the Backup Components Document and [**IVssExamineWriterMetadata::SaveAsXML**](ivssexaminewritermetadata-saveasxml.md) to save each writer's Writer Metadata Documents. The XML strings that contain these documents are then made available to a requester that is running on *systemTwo*.

    The requester transfers the Backup Components Document to *systemTwo*.

    Note that the requester on *systemOne* does not release its instance of [**IVssBackupComponents**](ivssbackupcomponents.md) at this point if the purpose of the shadow copy is for backup. The interface should remain open until *systemTwo* successfully finishes its backup operations. Only then should the requester issue a [**BackupComplete**](ivssbackupcomponents-backupcomplete.md) event since some writers will truncate logs and do other work after a successful backup. If the goal of the shadow copy is data mining or other purposes, then the interface can be closed at this step.

6.  The requester on *systemTwo* then calls [**IVssBackupComponents::ImportSnapshots**](ivssbackupcomponents-importsnapshots.md) to get access to the shadow copy that was created by the requester on *systemOne*.
    > [!Note]  
    > The requester is responsible for serializing the import shadow copy operation. Also, if the call to [**IVssBackupComponents::ImportSnapshots**](ivssbackupcomponents-importsnapshots.md) fails, the VSS won't clean up LUNs on it's own. The requester has to initiate the cleanup of LUNs.
