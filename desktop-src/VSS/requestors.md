...
-   During a backup operation, only a subset of all the components involved in the backup—[*nonselectable for backup*](vssgloss-s.md#base-vssgloss-selectability-for-backup) components without selectable for backup ancestors and selectable for backup components that have been [*included explicitly*](vssgloss-e.md#base-vssgloss-explicit-component-inclusion) in the backup—have had their information added to the requester's Backup Components Document.
...
-   During restore operations, a component [*implicitly included*](vssgloss-i.md#base-vssgloss-implicit-component-inclusion) in the backup may be [*selectable for restore*](vssgloss-s.md#base-vssgloss-selectability-for-restore) and therefore can be explicitly included in the restore. This will require the updating of the requester's Backup Components Document with information from stored copies of a writer's Writer Metadata Document.
