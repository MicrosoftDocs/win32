...
A requester must make use of the files contained on the shadow-copied volumes after the [**IVssBackupComponents::DoSnapshotSet**](ivssbackupcomponents-dosnapshotset.md) returns successfully to generate the full list of files to be backed up.
...
2.  Using the [*file set*](vssgloss-f.md#base-vssgloss-file-set) information (contained in [**IVssWMFiledesc**](ivsswmfiledesc.md) objects returned by [**IVssExamineWriterMetadata::GetExcludeFile**](ivssexaminewritermetadata-getexcludefile.md)) to create a list of the explicitly excluded files, if necessary using **FindFileFirst**, **FindFileFirstEx**, and **FindNextFile**.
3.  Iterating over all of a writer's components, using [**IVssExamineWriterMetadata::GetComponent**](ivssexaminewritermetadata-getcomponent.md). If a selectable component is selected, use [*logical path*](vssgloss-l.md#base-vssgloss-logical-path) to obtain those nonselectable components associated with it in a component set. (See [Working with Selectability and Logical Paths](working-with-selectability-and-logical-paths.md).)
4.  Obtaining the [*file sets*](vssgloss-f.md#base-vssgloss-file-set) contained in each selected component by using the [**IVssWMComponent**](ivsswmcomponent.md) interface corresponding to each component it contains.
5.  Generating a list of files from the specifications—if necessary using **FindFileFirst**, **FindFileFirstEx**, and **FindNextFile**.
6.  Checking each file in the list generated from component information against the list of excluded files generated above. This should be done using the default path for the file (returned by [**IVssWMFiledesc::GetPath**](ivsswmfiledesc-getpath.md)), not by the alternate path returned by [**IVssWMFiledesc::GetAlternateLocation**](ivsswmfiledesc-getalternatelocation.md)). If the file matches the excluded list, it will not be backed up.
