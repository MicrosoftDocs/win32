...
In addition, a requester must deal with the possibility that some files have [*alternate paths*](https://www.bing.com/search?q=*alternate paths*) and that some files have been excluded.

An algorithm for selecting files to back up should go on a [*writer instance*](https://www.bing.com/search?q=*writer instance*) by writer instance, component-by-component basis (as will be the case during restore; see [Generating a Restore Set](generating-a-restore-set.md)) and might proceed by doing the following:

1.  Determining the volumes that contain the writer's files and the corresponding device objects
2.  Using the [*file set*](https://www.bing.com/search?q=*file set*) information (contained in [**IVssWMFiledesc**](/windows/desktop/api/VsWriter/nl-vswriter-ivsswmfiledesc) objects returned by [**IVssExamineWriterMetadata::GetExcludeFile**](/windows/desktop/api/VsBackup/nf-vsbackup-ivssexaminewritermetadata-getexcludefile)) to create a list of the explicitly excluded files, if necessary using **FindFileFirst**, **FindFileFirstEx**, and **FindNextFile**.
3.  Iterating over all of a writer's components, using [**IVssExamineWriterMetadata::GetComponent**](/windows/desktop/api/VsBackup/nf-vsbackup-ivssexaminewritermetadata-getcomponent). If a selectable component is selected, use [*logical path*](https://www.bing.com/search?q=*logical path*) to obtain those nonselectable components associated with it in a component set. (See [Working with Selectability and Logical Paths](working-with-selectability-and-logical-paths.md).)
4.  Obtaining the [*file sets*](https://www.bing.com/search?q=*file sets*) contained in each selected component by using the [**IVssWMComponent**](/windows/desktop/api/VsBackup/nl-vsbackup-ivsswmcomponent) interface corresponding to each component it contains.
