...
A writer's participation in a backup or restore operation, and which of its data are saved, depends on which of its components are [*explicitly included*](https://www.bing.com/search?q=*explicitly included*) as part of a requester's Backup Components Document and the relationship between those components and the logical path of other components within the Writer Metadata Document.

Writers with no components added to a requester's Backup Component Document prior to the generation of a [*PrepareForBackup*](https://www.bing.com/search?q=*PrepareForBackup*) event (in the case of backup operations) or of a [**PreRestore**](/windows/desktop/api/VsBackup/nf-vsbackup-ivssbackupcomponents-prerestore) event (in the case of restore operations) receive no events after this point and will not participate in the backup or restore operation.
...
-   Any hierarchy that exists between the components managed by a writer and expressed by [*logical paths*](https://www.bing.com/search?q=*logical paths*)
-   Whether the component is designated as being [*selectable for backup*](https://www.bing.com/search?q=*selectable for backup*)
-   Whether it is designated as being [*selectable for restore*](https://www.bing.com/search?q=*selectable for restore*)
