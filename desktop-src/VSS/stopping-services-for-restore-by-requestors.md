...
Typically, stopping and starting a service to support a restore would be performed by a writer when handling the [*PreRestore*](vssgloss-p.md#base-vssgloss-prerestore-event) event (with [**CVssWriter::OnPreRestore**](cvsswriter-onprerestore.md)) and the [*PostRestore*](vssgloss-p.md#base-vssgloss-postrestore-event) event (with [**CVssWriter::OnPostRestore**](cvsswriter-onpostrestore.md)).

However, there may be cases when it is necessary for a requester to explicitly stop a running service. Writers indicate if this is the case by setting the VSS\_RME\_STOP\_RESTORE\_START or VSS\_RME\_RESTORE\_STOP\_START value of the [**VSS\_RESTOREMETHOD\_ENUM**](vss-restoremethod-enum.md) enumeration as the restore method argument of a call to the [**IVssCreateWriterMetadata::SetRestoreMethod**](ivsscreatewritermetadata-setrestoremethod.md) method and specifying the name of the service to be stopped.

A requester obtains information about the restore method and the name of the service to be stopped when working with writer metadata by using the [**IVssExamineWriterMetadata::GetRestoreMethod**](ivssexaminewritermetadata-getrestoremethod.md) method.
