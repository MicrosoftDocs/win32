...
A writer can fail for numerous programmatic reasons. When this happens, it should veto the ongoing backup, restore, or shadow copy operation by calling the [**CVssWriter::SetWriterFailure**](cvsswriter-setwriterfailure.md) method in one of its handler methods (for example, [**CVssWriter::OnFreeze**](cvsswriter-onfreeze.md) or [**CVssWriter::OnPreRestore**](cvsswriter-onprerestore.md)) and returning **TRUE**. It can also optionally set an error message string in response to a failure condition in certain handler methods with the [**IVssComponentEx::SetPrepareForBackupFailureMsg**](ivsscomponentex-setprepareforbackupfailuremsg.md), [**IVssComponentEx::SetPostSnapshotFailureMsg**](ivsscomponentex-setpostsnapshotfailuremsg.md), [**IVssComponent::SetPreRestoreFailureMsg**](ivsscomponent-setprerestorefailuremsg.md), and [**IVssComponent::SetPostRestoreFailureMsg**](ivsscomponent-setpostrestorefailuremsg.md) methods. The requester can accept the veto or continue with the backup, ignoring the veto.

A requester should check the writer status (using [**IVssBackupComponents::GatherWriterStatus**](ivssbackupcomponents-gatherwriterstatus.md) and [**IVssBackupComponents::GetWriterStatus**](ivssbackupcomponents-getwriterstatus.md)) following each event it generates.

In some cases, error messages can be retrieved from these failures (using the [**IVssComponentEx::GetPrepareForBackupFailureMsg**](ivsscomponentex-getprepareforbackupfailuremsg.md), [**IVssComponent::GetPreRestoreFailureMsg**](ivsscomponent-getprerestorefailuremsg.md), [**IVssComponentEx::GetPostSnapshotFailureMsg**](ivsscomponentex-getpostsnapshotfailuremsg.md), and [**IVssComponent::GetPostRestoreFailureMsg**](ivsscomponent-getpostrestorefailuremsg.md) methods), or a writer may choose to set metadata (using [**IVssComponent::SetRestoreMetadata**](ivsscomponent-setrestoremetadata.md) and [**IVssComponent::SetBackupMetadata**](ivsscomponent-setbackupmetadata.md) with error state information). For example code that shows how to view such error messages, see [**IVssComponentEx::GetPrepareForBackupFailureMsg**](ivsscomponentex-getprepareforbackupfailuremsg.md).
...
For example, suppose [**GetWriterStatus**](ivssbackupcomponents-getwriterstatus.md) returned the following:
