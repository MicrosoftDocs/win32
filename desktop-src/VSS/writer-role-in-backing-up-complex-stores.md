...
The writers indicate what type of backups are supported by calling [**IVssCreateWriterMetadata::SetBackupSchema**](/windows/desktop/api/VsWriter/nf-vswriter-ivsscreatewritermetadata-setbackupschema) while processing the [*Identify*](https://www.bing.com/search?q=*Identify*) event. The *dsSchemaMask* parameter to the **IVssCreateWriterMetadata::SetBackupSchema** method is a bit mask indicating what types of backup are supported. All writers must support full backups.
...
A writer can store a timestamp with each component that is being backed up. The writer stores the timestamp by calling [**IVssComponent::SetBackupStamp**](/windows/desktop/api/VsWriter/nf-vswriter-ivsscomponent-setbackupstamp), and passing in a string representation of the stamp for the *wszBackupStamp* parameter. Generally, a writer will call this method while processing the [*PostSnapshot*](https://www.bing.com/search?q=*PostSnapshot*) event. However, for backups that do not involve a shadow copy, the PostSnapshot event will not be sent. In this case, **IVssComponent::SetBackupStamp** must be called while processing the [*PrepareForBackup*](https://www.bing.com/search?q=*PrepareForBackup*) event.
...
If the writer needs to perform log truncation or other post-backup cleanup, the proper place to do this is while processing the [*BackupComplete*](https://www.bing.com/search?q=*BackupComplete*) event. The [*BackupShutdown*](https://www.bing.com/search?q=*BackupShutdown*) event will be sent some time after BackupComplete, so some cleanup may also be done in the BackupShutdown event handler.
