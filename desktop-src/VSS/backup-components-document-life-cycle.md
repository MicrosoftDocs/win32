...
This control is exercised by an instance of the [**IVssBackupComponents**](ivssbackupcomponents.md) interface object returned by [**CreateVssBackupComponents**](createvssbackupcomponents.md).

A requester must initialize a Backup Components Document prior to a backup or restore by calling [**IVssBackupComponents::InitializeForBackup**](ivssbackupcomponents-initializeforbackup.md) or [**IVssBackupComponents::InitializeForRestore**](ivssbackupcomponents-initializeforrestore.md). The requester can initialize the document as empty, or it can load a previously stored copy of the document.
...
Information about the stored components is available both to requesters and writers through instances of the [**IVssComponent**](ivsscomponent.md) interface.

It is through [**IVssComponent**](ivsscomponent.md) interfaces that writers can query and (until the end of [**PostSnapshot**](cvsswriter-onpostsnapshot.md) and [**PostRestore**](cvsswriter-onpostrestore.md) events) modify information in the Backup Components Document.

When the [**CVssWriter::OnPrepareBackup**](cvsswriter-onpreparebackup.md), [**CVssWriter::OnPreRestore**](cvsswriter-onprerestore.md), [**CVssWriter::OnPostSnapshot**](cvsswriter-onpostsnapshot.md), [**CVssWriter::OnBackupComplete**](cvsswriter-onbackupcomplete.md), or [**CVssWriter::OnPostRestore**](cvsswriter-onpostrestore.md) event handler is called, a writer receives an instance of an [**IVssWriterComponents**](ivsswritercomponents.md) interface.

Note that upon generation of the [**BackupComplete**](ivssbackupcomponents-backupcomplete.md) event, the Backup Components Document is made read-only, and therefore [**CVssWriter::OnBackupComplete**](cvsswriter-onbackupcomplete.md) cannot use the [**IVssComponent**](ivsscomponent.md) interface to modify it.

From the [**IVSSWriterComponents**](ivsswritercomponents.md) interface, the writer can retrieve instances of the [**IVssComponent**](ivsscomponent.md) interface that will allow it to access all of its components explicitly added to the Backup Components Document and to alter their state. For more information, see [Overview of Processing a Backup Under VSS](overview-of-processing-a-backup-under-vss.md) and [Overview of Processing a Restore Under VSS](overview-of-processing-a-restore-under-vss.md).

Backup Components Documents are removed from memory when the [**IVssBackupComponents**](ivssbackupcomponents.md) interface is released, and must be stored using [**IVssBackupComponents::SaveAsXML**](ivssbackupcomponents-saveasxml.md), or all their information will be lost.

In addition, when an [**IVssBackupComponents**](ivssbackupcomponents.md) document is properly released, a [**BackupShutdown**](cvsswriter-onbackupshutdown.md) event is generated and[*auto-release shadow copies*](vssgloss-a.md#base-vssgloss-auto-release-shadow-copy) are deleted.
