...
[*Logical pathing*](https://www.bing.com/search?q=*Logical pathing*) is used to organize components managed by a writer into well-defined groups.
...
The most important use of logical pathing is in defining [*component sets*](https://www.bing.com/search?q=*component sets*), where the [*explicit component inclusion*](https://www.bing.com/search?q=*explicit component inclusion*) in a backup or restore operation of one selectable component requires the inclusion of a number of other components ([*subcomponent*](https://www.bing.com/search?q=*subcomponent*)). The logical path of the defining component of a component set is a parent to the logical paths of its subcomponents and:
...
-   The combination of logical path and component name must be unique across all instances of a [*writer class*](https://www.bing.com/search?q=*writer class*).
...
Note that the components "Executables" and "ConfigFile" have a parent-child relationship, but both are nonselectable. Therefore, they do not form a component set. Whenever the writer *MyWriter* is backed up or restored, these two components will have to be [*explicitly included*](https://www.bing.com/search?q=*explicitly included*) in the operation.
...
If the component "writerData" is explicitly included in a backup or restore, then all of its subcomponents—even those in the nested component set defined by "Usage"—will be [*implicitly included*](https://www.bing.com/search?q=*implicitly included*)[*implicitly included*](https://www.bing.com/search?q=*implicitly included*) in the operation.
...
-   Components that define component sets may exist simply for organizational reasons. For instance, either the "writerData" or the "Usage" component, or both, might be empty; that is, no [*file sets*](https://www.bing.com/search?q=*file sets*) were added to them using the [**IVssCreateWriterMetadata::AddFilesToFileGroup**](/windows/desktop/api/VsWriter/nf-vswriter-ivsscreatewritermetadata-addfilestofilegroup), [**IVssCreateWriterMetadata::AddDatabaseFiles**](/windows/desktop/api/VsWriter/nf-vswriter-ivsscreatewritermetadata-adddatabasefiles) or [**IVssCreateWriterMetadata::AddDatabaseLogFiles**](/windows/desktop/api/VsWriter/nf-vswriter-ivsscreatewritermetadata-adddatabaselogfiles) method. The components still define a component set.
