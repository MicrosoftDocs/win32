...
| Use the list of components and their [*file sets*](https://www.bing.com/search?q=*file sets*), as well as excluded files, to obtain a list of volumes and files involved in the backup (see [**IVssWMComponent**](/windows/desktop/api/VsBackup/nl-vsbackup-ivsswmcomponent), [**IVssWMFiledesc**](/windows/desktop/api/VsWriter/nl-vswriter-ivsswmfiledesc)).                                                                                                                                                                                                                                                                      | None  | None                                                                              |
...
Because the [**IVssWMFiledesc**](/windows/desktop/api/VsWriter/nl-vswriter-ivsswmfiledesc) object can use wildcard characters to hold file location information, it may be necessary to use functions such as [**FindFirstFile**](https://msdn.microsoft.com/windows/desktop/02fc92c4-582d-4c9f-a811-b5c839e9fffa), [**FindFirstFileEx**](https://msdn.microsoft.com/windows/desktop/9f40e98f-153f-4b65-afd9-06742684c100), and [**FindNextFile**](https://msdn.microsoft.com/windows/desktop/db7acb83-2da6-40bf-9962-5cfe54e257a5).
...
1.  Examining all selectable for backup and nonselectable components in each writer's Writer Metadata Document (using [**IVssExamineWriterMetadata**](/windows/desktop/api/VsBackup/nl-vsbackup-ivssexaminewritermetadata)) and organizing them into [*component sets*](https://www.bing.com/search?q=*component sets*) use [*logical path*](https://www.bing.com/search?q=*logical path*) (see [Working with Selectability and Logical Paths](working-with-selectability-and-logical-paths.md))
2.  [*Including explicitly*](https://www.bing.com/search?q=*Including explicitly*) all required components (nonselectable for backup components without selectable for backup ancestors) in the Backup Components Document using [**IVssBackupComponents::AddComponent**](/windows/desktop/api/VsBackup/nf-vsbackup-ivssbackupcomponents-addcomponent)
...
4.  Selecting [*component sets*](https://www.bing.com/search?q=*component sets*) for participation in a backup by explicitly adding their defining selectable for backup component (using [**IVssBackupComponents::AddComponent**](/windows/desktop/api/VsBackup/nf-vsbackup-ivssbackupcomponents-addcomponent)), which [*implicitly includes*](https://www.bing.com/search?q=*implicitly includes*) the component set's [*subcomponents*](https://www.bing.com/search?q=*subcomponents*).
5.  Using [*file set*](https://www.bing.com/search?q=*file set*) information in the selected writers' Writer Metadata Document and volume management functions, a requester determines the paths of files to be backed up and the volumes that will need to be shadow copied
...
Although it is not generally useful, a requester can sometimes also choose which [*provider*](https://www.bing.com/search?q=*provider*) will maintain the shadow copy for a given volume (see [Selecting Providers](selecting-providers.md) for details).
...
Writers are given access to the [**IVssComponent**](/windows/desktop/api/VsWriter/nl-vswriter-ivsscomponent) interface when handling the [*PrepareForBackup*](https://www.bing.com/search?q=*PrepareForBackup*), [*PostSnapshot*](https://www.bing.com/search?q=*PostSnapshot*), and [*BackupComplete*](https://www.bing.com/search?q=*BackupComplete*) events.
