...
The Cluster Service VSS writer is documented in the [Cluster Service](mscs.backing_up_and_restoring_the_failover_cluster_configuration_using_vss) API documentation.
...
To restore the COM+ Registration Database, a backup application (requester) must call the [**ICOMAdminCatalog::RestoreREGDB**](cos.icomadmincatalog_restoreregdb) method.
...
The Data Deduplication VSS writer is documented in the [Data Deduplication](dedup.backup_and_restore_of_data_deduplication-enabled_volumes) API documentation. This writer is an in-box writer for Windows Server operating system versions; it does not ship in Windows Client.

**Windows Server 2008 R2, Windows Server 2008 and Windows Server 2003:** This writer is not supported.

## Distributed File System Replication (DFSR)

The following component includes a VSS writer: [Distributed File System Replication (DFSR)](dfsr.dfsr_replicated_folders)
...
The Hyper-V VSS writer is documented in the [Hyper-V](virtual.backing_up_and_restoring_virtual_machines) API documentation. This writer is an in-box writer for Windows Server operating system versions; it does not ship in Windows Client.
