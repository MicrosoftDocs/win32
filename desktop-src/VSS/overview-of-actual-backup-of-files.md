...
<tbody>
<tr class="odd">
<td>Access files on the shadow-copied volume (see [<strong>IVssBackupComponents::GetSnapshotProperties</strong>](ivssbackupcomponents-getsnapshotproperties.md), [<strong>VSS_SNAPSHOT_PROP</strong>](vss-snapshot-prop.md))</td>
<td>None</td>
<td>None</td>
</tr>
<tr class="even">
...
<tr class="odd">
<td>Indicate the success or failure of the backup with [<strong>IVssBackupComponents::SetBackupSucceeded</strong>](ivssbackupcomponents-setbackupsucceeded.md).</td>
<td>None</td>
<td>None</td>
</tr>
<tr class="even">
<td>The requester indicates that the backup has completed by calling [<strong>IVssBackupComponents::BackupComplete</strong>](ivssbackupcomponents-backupcomplete.md).</td>
<td>[<em>BackupComplete</em>](vssgloss-b.md#base-vssgloss-backupcomplete-event)</td>
<td>Perform any post-backup cleanup (see [<strong>CVssWriter::OnBackupComplete</strong>](cvsswriter-onbackupcomplete.md), [<strong>IVssWriterComponents</strong>](ivsswritercomponents.md), [<strong>IVssComponent</strong>](ivsscomponent.md)).</td>
</tr>
<tr class="odd">
<td>The requester waits for all writers to acknowledge receipt of the [<strong>IVssBackupComponents::BackupComplete</strong>](ivssbackupcomponents-backupcomplete.md) event using [<strong>IVssAsync</strong>](ivssasync.md). It should also verify writer status (see [<strong>IVssBackupComponents::GatherWriterStatus</strong>](ivssbackupcomponents-gatherwriterstatus.md), [<strong>IVssBackupComponents::GetWriterStatus</strong>](ivssbackupcomponents-getwriterstatus.md)). The requester must call <strong>GatherWriterStatus</strong> at this time to cause the writer session to be set to a completed state.
<blockquote>
[!Note]<br />
...
<tr class="even">
<td>Save the Backup Components Document and each Writer Metadata Document to XML documents, which can be written to the backup media (see [<strong>IVssBackupComponents::SaveAsXML</strong>](ivssbackupcomponents-saveasxml.md) and [<strong>IVssExamineWriterMetadata::SaveAsXML</strong>](ivssexaminewritermetadata-saveasxml.md)).</td>
<td>None</td>
<td>None</td>
</tr>
</tbody>
...
After a backup has completed, and a requester has generated a [*BackupComplete*](vssgloss-b.md#base-vssgloss-backupcomplete-event) event, VSS will call each writer's [**CVssWriter::OnBackupComplete**](cvsswriter-onbackupcomplete.md) method, a virtual method that by default simply returns **TRUE**. However, writers can override the default implementation and take such actions as removing remaining temporary files, or use the [**IVssWriterComponents**](ivsswritercomponents.md) interface it is called with to check the state of the backup of each of its [*included explicitly*](vssgloss-e.md#base-vssgloss-explicit-component-inclusion) components (and any [*component set*](vssgloss-c.md#base-vssgloss-component-set) they might define) by retrieving the corresponding [**IVssComponent**](ivsscomponent.md) object. The writer can then determine, and act on, the success or failure of the backup by calling [**IVssComponent:GetBackupSucceeded**](ivsscomponent-getbackupsucceeded.md). The value returned by **IVssComponent:GetBackupSucceeded** will be **TRUE** only if all explicitly included files in the component and all [*implicitly included*](vssgloss-i.md#base-vssgloss-implicit-component-inclusion) of any of its [*subcomponents*](vssgloss-s.md#base-vssgloss-subcomponent) have been successfully backed up.

When the call to [**CVssWriter::OnBackupComplete**](cvsswriter-onbackupcomplete.md) has completed, the requester should call [**IVssBackupComponents::GatherWriterStatus**](ivssbackupcomponents-gatherwriterstatus.md) and [**IVssBackupComponents::GetWriterStatus**](ivssbackupcomponents-getwriterstatus.md) (for each writer) one last time. The writer session state memory is a limited resource and writers must eventually reuse session states. This step marks the writer’s backup session state as completed and notifies VSS that this backup session slot can be reused by a subsequent backup operation.
...
The success or failure of backing up any of the files managed by a given component and (if it defines a [*component set*](vssgloss-c.md#base-vssgloss-component-set)) its [*subcomponents*](vssgloss-s.md#base-vssgloss-subcomponent) for a given writer instance should be preserved in the Backup Components Document by calling [**IVssBackupComponents::SetBackupSucceeded**](ivssbackupcomponents-setbackupsucceeded.md). If any file managed by a component or component set fails to back up, the entire component is said to fail. Exact information about which file failed to back up should always be logged.
...
Because writers can modify the Backup Components Document while handling the [**PostSnapshot**](cvsswriter-onpostsnapshot.md) event generated by the requester's call to [**IVssBackupComponents::DoSnapshotSet**](ivssbackupcomponents-dosnapshotset.md), the Backup Components Document should not be saved until after that asynchronous call has completed.

Although it may take place earlier, this is also a convenient time to save the Writer Metadata Document.

It is very important that both the Backup Components Document and the Writer Metadata Documents be preserved using [**IVssBackupComponents::SaveAsXML**](ivssbackupcomponents-saveasxml.md) and [**IVssExamineWriterMetadata::SaveAsXML**](ivssexaminewritermetadata-saveasxml.md). If they are not, it will not be possible to make use of VSS during file restoration.
