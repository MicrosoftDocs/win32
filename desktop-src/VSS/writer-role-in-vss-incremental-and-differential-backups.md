...
A writer's participation in incremental and differential backups typically takes place while handling [*Identify*](vssgloss-i.md#base-vssgloss-identify-event) ([**CVssWriter::OnIdentify**](cvsswriter-onidentify.md)), [*PrepareForBackup*](vssgloss-p.md#-win32-vssgloss-prepareforbackup-event) ([**CVssWriter:OnPrepareBackup**](cvsswriter-onpreparebackup.md)), and *PostSnapshot* ([**CVssWriter:OnPostSnapshot**](cvsswriter-onpostsnapshot.md)) events. How a writer participates is shaped by whether it supports backup stamps and last modification times, and whether the requester running the backup supports [*partial file operations*](vssgloss-p.md#base-vssgloss-partial-file-support).

## Handling Identify Events during Incremental and Differential Backups

While handling the Identify event, writers establish their basic architecture for incremental and differential backup operation through the backup schema ([**VSS\_BACKUP\_SCHEMA**](vss-backup-schema.md)) and file specification backup type ([**VSS\_FILE\_SPEC\_BACKUP\_TYPE**](vss-file-spec-backup-type.md)) masks.

A writer indicates which operations it supports in its Writer Metadata Document by creating a bit mask of [**VSS\_BACKUP\_SCHEMA**](vss-backup-schema.md) values and passing it to the [**IVssCreateWriterMetadata::SetBackupSchema**](ivsscreatewritermetadata-setbackupschema.md) method. With this, a writer can indicate whether it supports the following:
...
A writer can set a file set's file specification backup type mask when it adds the file set to a component by creating a bit mask of [**VSS\_FILE\_SPEC\_BACKUP\_TYPE**](vss-file-spec-backup-type.md) values and passing it to [**IVssCreateWriterMetadata::AddDatabaseFiles**](ivsscreatewritermetadata-adddatabasefiles.md), [**IVssCreateWriterMetadata::AddDatabaseLogFiles**](ivsscreatewritermetadata-adddatabaselogfiles.md), or [**IVssCreateWriterMetadata::AddFilesToFileGroup**](ivsscreatewritermetadata-addfilestofilegroup.md).
...
Because the writers use the [**IVssComponent**](ivsscomponent.md) interface, they typically perform these preparations while handling the [**PrepareForBackup**](ivssbackupcomponents-prepareforbackup.md) event.

In [**CVssWriter:OnPrepareBackup**](cvsswriter-onpreparebackup.md), writers can more precisely specify how some files are to be evaluated for backup, specify what mechanisms should be used in backing them up, and possibly set backup stamps.
...
If a requester supports them, a writer can choose to have an incremental or differential backup implemented by using partial file operations. (Writers can determine if a requester supports partial file operations by calling [**CVssWriter::IsPartialFileSupportEnabled**](cvsswriter-ispartialfilesupportenabled.md).)

Writers use [**IVssComponent::AddPartialFile**](ivsscomponent-addpartialfile.md) to indicate those portions of the selected files to be backed up during the incremental or differential operation. Requesters must respect this specification, and must always back up the specified sections of the files. (See [Working with Partial Files](working-with-partial-files.md) for more information on partial file operations.)

Using [**IVssComponent::AddPartialFile**](ivsscomponent-addpartialfile.md), a writer can add a file to the backup that was not previously added to one of its component sets (by [**IVssCreateWriterMetadata::AddDatabaseFiles**](ivsscreatewritermetadata-adddatabasefiles.md), [**IVssCreateWriterMetadata::AddDatabaseLogFiles**](ivsscreatewritermetadata-adddatabaselogfiles.md), or [**IVssCreateWriterMetadata::AddFilesToFileGroup**](ivsscreatewritermetadata-addfilestofilegroup.md)) as a partial file. Any new files added to the backup in this manner must be on a volume that already is being shadow copied for this backup.
...
In specifying a differenced file, a writer uses [**IVssComponent::AddDifferencedFileByLastModifyTime**](ivsscomponent-adddifferencedfilesbylastmodifytime.md) and specifies a path, a file name, and a recursive flag—however, these do not have to match a file set included in any component.

In fact, a writer can add a file not previously added to one of its component sets (by [**IVssCreateWriterMetadata::AddDatabaseFiles**](ivsscreatewritermetadata-adddatabasefiles.md), [**IVssCreateWriterMetadata::AddDatabaseLogFiles**](ivsscreatewritermetadata-adddatabaselogfiles.md), or [**IVssCreateWriterMetadata::AddFilesToFileGroup**](ivsscreatewritermetadata-addfilestofilegroup.md)) to the backup as a differenced file. Any new files added to the backup in this manner must be on a volume that already is being shadow copied for this backup.
...
The backup stamp is stored in the Backup Components Document as a string by the [**IVssComponent::SetBackupStamp**](ivsscomponent-setbackupstamp.md) method. The backup stamp applies to all file sets in the component (or component set) corresponding to the instance of the [**IVssComponent**](ivsscomponent.md).

If a requester has access to the backup stamp of a previous backup, it will have made it available to the writer by calling [**IVssBackupComponents::SetPreviousBackupStamp**](ivssbackupcomponents-setpreviousbackupstamp.md).

A writer then can examine this time stamp using [**IVssComponent::GetPreviousBackupStamp**](ivsscomponent-getpreviousbackupstamp.md).
...
A writer may choose to update the current backup stamp using [**IVssComponent::SetBackupStamp**](ivsscomponent-setbackupstamp.md) after it has called [**IVssComponent::GetPreviousBackupStamp**](ivsscomponent-getpreviousbackupstamp.md), thus recording in its own format the date of the current incremental or differential backup operation.
