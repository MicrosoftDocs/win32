...
-   [**NdfCreateConnectivityIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreateconnectivityincident?branch=master): General Internet connectivity problems which do not need additional information.
-   [**NdfCreateWebIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreatewebincident?branch=master)/[**NdfCreateWebIncidentEx**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreatewebincidentex?branch=master): Connecting to an HTTP or HTTPS URL.
-   [**NdfCreateSharingIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreatesharingincident?branch=master): Accessing a UNC path or file share.
-   [**NdfCreateDNSIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreatednsincident?branch=master): Resolving a DNS host name.
-   [**NdfCreatePnrpIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreatepnrpincident?branch=master): Resolving a PNRP peer name.
-   [**NdfCreateGroupingIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreategroupingincident?branch=master): Joining a peer-to-peer group.
-   [**NdfCreateWinSockIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreatewinsockincident?branch=master): Connecting to a destination by using a socket (when none of the other functions specifically apply).
-   [**NdfCreateIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcreateincident?branch=master): Used when no other scenario is appropriate and the specific NDF helper class to be invoked is known (along with the arguments it requires). Primarily used for testing purposes by application developers who have written their own helper class.
...
-   Using a Custom User Interface (Windows 7 and later only)
    ...
    To begin diagnosis, call the [**NdfDiagnoseIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfdiagnoseincident?branch=master) function. Any problems found will be returned to the application as a collection of [**RootCauseInfo**](/windows/win32/ndattrib/ns-ndattrib-tagrootcauseinfo?branch=master) structures describing identified root causes and possible repairs.

    After selecting a repair (or asking the user to select a repair), [**NdfRepairIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfrepairincident?branch=master) should be called to attempt the repair and determine whether the problem was resolved.
    ...
    [**NdfDiagnoseIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfdiagnoseincident?branch=master) and [**NdfRepairIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfrepairincident?branch=master) are synchronous APIs. If you wish to cancel activity initiated by these functions, call [**NdfCancelIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcancelincident?branch=master) from another thread. The function will return at the next available stopping point in the diagnosis or repair process.

    At any point, you may optionally call [**NdfGetTraceFile**](/windows/win32/Ndfapi/nf-ndfapi-ndfgettracefile?branch=master) to retrieve a copy of the NDF log for the current diagnosis session and include it with your application logs. The log is flushed once retrieved, and subsequent calls will only retrieve events which occurred after the last call to this function.

## Closing an Incident

When you are finished diagnosing an incident, call [**NdfCloseIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfcloseincident?branch=master) to free system resources associated with performing diagnostics on that incident. (Note that this does not free objects created by [**NdfDiagnoseIncident**](/windows/win32/Ndfapi/nf-ndfapi-ndfdiagnoseincident?branch=master). For an example of how to do this, see the [Microsoft Windows Network Diagnostics Samples](http://go.microsoft.com/fwlink/p/?linkid=158047).)
