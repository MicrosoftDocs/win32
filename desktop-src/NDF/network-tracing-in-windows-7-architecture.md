...
Providers using the new [Event Logging APIs](https://msdn.microsoft.com/library/aa363752.aspx) are expected to supply an XML file called the [event manifest](https://msdn.microsoft.com/library/aa385201.aspx). This file provides metadata to define all of the custom data and layout information for the events written by the provider. A general purpose consumer application then uses [Trace Data Helper (TDH)](/windows/win32/api/tdh/) APIs to retrieve the event metadata, decode the events, and display them.
