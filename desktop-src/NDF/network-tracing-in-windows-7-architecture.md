...
Each ETW event has a common header where ETW stores information such as the event properties, time stamps, and activity ID. (For more information about activity IDs, see [Writing Related Events in an End-to-End Scenario](https://go.microsoft.com/fwlink/p/?linkid=144751)). The activity ID is used to correlate events. In user mode, activity IDs are stored in threads, and all events logged in one thread will automatically be tagged with the same activity ID. In kernel mode, the activity ID must be explicitly passed when an event is logged. By default, ETL files are correlated to group events together under specific activity IDs.

For more information about Windows Events and ETW, see [Windows Events](https://go.microsoft.com/fwlink/p/?linkid=141515).
...
Providers using the new [Event Logging APIs](https://go.microsoft.com/fwlink/p/?linkid=144753) are expected to supply an XML file called the [event manifest](https://go.microsoft.com/fwlink/p/?linkid=90000). This file provides metadata to define all of the custom data and layout information for the events written by the provider. A general purpose consumer application then uses [Trace Data Helper (TDH)](https://go.microsoft.com/fwlink/p/?linkid=144754) APIs to retrieve the event metadata, decode the events, and display them.
