...
keywords: ["DLNASourceURI Attribute Windows Media Player"]
topic_type:
- apiref
api_name:
- DLNASourceURI
...
If the item is in the current user's local library, this attribute, the [**SourceURL**](sourceurl-attribute.md) attribute, and the value returned by [**IWMPMedia::get\_sourceURL**](iwmpmedia-get-sourceurl.md) are all the same.
...
You can pass a dlna-playsingle URI to the [**IWMPCore3::newMedia**](iwmpcore3-newmedia.md) method to obtain an [**IWMPMedia**](iwmpmedia.md) interface that enables you to view the media item's metadata and potentially edit the media item's star rating. Note, however, that the dlna-playsingle URI must be for a content directory service (CDS) that Windows Media Player has already discovered. The **newMedia** method will not initiate UPnP discovery and search for the CDS.
