...
2.  The Player calls the plug-in's [GetTemplate](/windows/win32/contentpartner/nf-contentpartner-iwmpcontentpartner-gettemplate?branch=master) method to determine which discovery page to display next. The Player stores the URL of the new discovery page but does not display the new discovery page at this time.
