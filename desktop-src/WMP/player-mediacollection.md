---
title: Player.mediaCollection
description: The mediaCollection property retrieves the MediaCollection object.
ms.assetid: 'fbcf23db-2a36-4c3f-9d5d-628b604d1803'
keywords: ["Player.mediaCollection Windows Media Player"]
topic_type:
- apiref
api_name:
- Player.mediaCollection
api_location:
- wmp.dll
api_type:
- DllExport
---

# Player.mediaCollection

The **mediaCollection** property retrieves the **MediaCollection** object.

## Syntax

*player* .**mediaCollection**

## Possible Values

This property is a read-only **MediaCollection** object.

## Remarks

To retrieve the value of this property, read access to the library is required. For more information, see [Library Access](library-access.md).

## Requirements



|                    |                                                                                    |
|--------------------|------------------------------------------------------------------------------------|
| Version<br/> | Windows Media Player version 7.0 or later.<br/>                              |
| DLL<br/>     | <dl> <dt>Wmp.dll</dt> </dl> |



## See also

<dl> <dt>

[**MediaCollection Object**](mediacollection-object.md)
</dt> <dt>

[**Player Object**](player-object.md)
</dt> <dt>

[**Settings.mediaAccessRights**](settings-mediaaccessrights.md)
</dt> <dt>

[**Settings.requestMediaAccessRights**](settings-requestmediaaccessrights.md)
</dt> </dl>

 

 





