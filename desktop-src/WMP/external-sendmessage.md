...
keywords: ["sendMessage method Windows Media Player", "sendMessage method Windows Media Player , External class", "External class Windows Media Player , sendMessage method"]
topic_type:
- apiref
api_name:
...
The message is sent asynchronously. That is, this method returns immediately rather than waiting for the message to be processed. When the plug-in finishes processing the message, it calls the [IWMPContentPartnerCallback::SendMessageComplete](iwmpcontentpartnercallback-sendmessagecomplete.md) method, which in turn calls the script's [OnSendMessageComplete](external-onsendmessagecomplete-event.md) event handler.
