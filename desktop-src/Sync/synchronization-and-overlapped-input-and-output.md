...
You can perform either synchronous or asynchronous (also called overlapped) I/O operations on files, named pipes, and serial communications devices. The [**WriteFile**](https://www.bing.com/search?q=**WriteFile**), [**ReadFile**](https://www.bing.com/search?q=**ReadFile**), [**DeviceIoControl**](https://msdn.microsoft.com/en-us/library/Aa363216(v=VS.85).aspx), [**WaitCommEvent**](https://msdn.microsoft.com/en-us/library/Aa363479(v=VS.85).aspx), [**ConnectNamedPipe**](https://msdn.microsoft.com/en-us/library/Aa365146(v=VS.85).aspx), and [**TransactNamedPipe**](https://msdn.microsoft.com/en-us/library/Aa365790(v=VS.85).aspx) functions can be performed either synchronously or asynchronously. The [**ReadFileEx**](https://www.bing.com/search?q=**ReadFileEx**) and [**WriteFileEx**](https://www.bing.com/search?q=**WriteFileEx**) functions can be performed only asynchronously.
...
For examples that illustrate the use of overlapped operations, completion routines, and the [**GetOverlappedResult**](/windows/desktop/api) function, see [Using Pipes](https://msdn.microsoft.com/en-us/library/Aa365799(v=VS.85).aspx).

**Windows Vista, Windows Server 2003 and Windows XP:  **

Be careful when reusing [**OVERLAPPED**](/windows/desktop/api) structures. If **OVERLAPPED** structures are reused on multiple threads and [**GetOverlappedResult**](/windows/desktop/api) is called with the *bWait* parameter set to **TRUE**, the calling thread must ensure that the associated event is signaled before reusing the structure. This can be accomplished by using the [**WaitForSingleObject**](/windows/desktop/api) function after calling **GetOverlappedResult** to force the thread to wait until the operation completes. Note that the event object must be a manual-reset event object. If an autoreset event object is used, calling **GetOverlappedResult** with the *bWait* parameter set to **TRUE** causes the function to be blocked indefinitely. This behavior changed starting with Windows 7 and Windows Server 2008 R2 for applications that specify Windows 7 as the supported operating system in the application manifest. For more information see [Application Manifests](https://msdn.microsoft.com/en-us/library/Cc530406(v=VS.85).aspx).
