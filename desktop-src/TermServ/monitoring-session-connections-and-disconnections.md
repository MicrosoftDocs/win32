...
For each registered service application, Remote Desktop Services will signal a set of event objects when a client connects or disconnects from the session. Each virtual channel plug-in must register itself and create the notification events by calling [**CreateEvent**](https://msdn.microsoft.com/library/windows/desktop/ms682396). The names of these event objects adhere to the following format.
