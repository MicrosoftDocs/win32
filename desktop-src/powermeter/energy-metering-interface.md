...
1.  Call [**IOCTL\_EMI\_GET\_VERSION**](ioctl-emi-get-version.md) and verify the EMI interface version supported by the device in the returned [**EMI\_VERSION**](emi-version.md) value. In Windows 10, devices can support **EMI\_VERSION\_V1**. Future operating system releases may introduce later versions.
2.  Call [**IOCTL\_EMI\_GET\_METADATA\_SIZE**](ioctl-emi-get-metadata-size.md) to get size of the EMI metadata.
3.  Allocate a buffer of the required EMI metadata size and call [**IOCTL\_EMI\_GET\_METADATA**](ioctl-emi-get-metadata.md). Verify that the returned [**EMI\_MEASUREMENT\_UNIT**](emi-measurement-unit.md) is **EmiMeasurementUnitPicowattHours**. Releases after Windows 10 may define additional unit types.
4.  To measure total energy consumption, call [**IOCTL\_EMI\_GET\_MEASUREMENT**](ioctl-emi-get-measurement.md). The **AbsoluteEnergy** value in the returned [**EMI\_MEASUREMENT\_DATA**](emi-measurement-data.md) is the total accumulated energy in picowatt-hours with some arbitrary zero-point. In general, you need to compare samples at two different times and subtract the energy values for energy consumption over that interval.
5.  To measure average power consumption, call [**IOCTL\_EMI\_GET\_MEASUREMENT**](ioctl-emi-get-measurement.md) at the beginning and end of the desired interval. Subtract the **AbsoluteEnergy** and **AbsoluteTime** values of the [**EMI\_MEASUREMENT\_DATA**](emi-measurement-data.md) returned by the latter sample from those of the earlier sample.
