...
When an application fails to specify an interface upon calling the [**connect**](/windows/win32/Winsock2/nf-winsock2-connect?branch=master) function, the first available interface is used. If no interface is available, **connect** fails.

When an application specifies an interface using the [RM\_SET\_SEND\_IF](socket-options.md) socket option, a [**bind**](/windows/win32/winsock/nf-winsock-bind?branch=master) attempt is made implicitly to that interface using TCP/IP, and fails if TCP/IP fails the bind request. If the interface is set using RM\_SET\_SEND\_IF multiple times, only the last interface set successfully is applicable.
...
When an application fails to specify an interface upon calling the [**listen**](/windows/win32/Winsock2/nf-winsock2-listen?branch=master) function, the default interface is used. If no interface is available, [**bind**](/windows/win32/winsock/nf-winsock-bind?branch=master) fails.
