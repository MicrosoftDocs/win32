...
When an application fails to specify an interface upon calling the [**connect**](connect-2.md) function, the first available interface is used. If no interface is available, **connect** fails.

When an application specifies an interface using the [RM\_SET\_SEND\_IF](socket-options.md) socket option, a [**bind**](bind-2.md) attempt is made implicitly to that interface using TCP/IP, and fails if TCP/IP fails the bind request. If the interface is set using RM\_SET\_SEND\_IF multiple times, only the last interface set successfully is applicable.
...
When an application fails to specify an interface upon calling the [**listen**](listen-2.md) function, the default interface is used. If no interface is available, [**bind**](bind-2.md) fails.
