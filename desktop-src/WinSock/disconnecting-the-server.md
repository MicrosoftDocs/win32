...
1.  When the server is done sending data to the client, the [**shutdown**](/windows/win32/winsock/nf-winsock-shutdown?branch=master) function can be called specifying SD\_SEND to shutdown the sending side of the socket. This allows the client to release some of the resources for this socket. The server application can still receive data on the socket.
    ```C++
