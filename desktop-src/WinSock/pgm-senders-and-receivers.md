...
2.  [**bind**](bind-2.md) the socket to INADDR\_ANY.
3.  [**connect**](connect-2.md) to the multicast group transmission address.

No formal session handshaking is performed with any clients. The connection process is similar to a UDP [**connect**](connect-2.md), in that it associates an endpoint address (the multicast group) with the socket. Once completed, data may be sent on the socket.
...
1.  Create a PGM socket.
2.  [**bind**](bind-2.md) the socket to the multicast group address on which the sender is transmitting.
3.  Call the [**listen**](listen-2.md) function on the socket to put the socket in listening mode. The listen function returns when a PGM session is detected on the specified multicast group address and port.
4.  Call the [**accept**](accept-2.md) function to obtain a new socket handle corresponding to the session.

Only original PGM data (ODATA) triggers the acceptance of a new session. As such, other PGM traffic (such as SPM or RDATA packets) may be received by the transport, but do not result in the [**listen**](listen-2.md) function returning.
