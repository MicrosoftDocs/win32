...
The best approach to ensuring that your structures are properly sized is to use the [**SOCKADDR\_STORAGE**](/windows/desktop/api/Ws2def/) structure. The **SOCKADDR\_STORAGE** structure is agnostic to IP address version. When the **SOCKADDR\_STORAGE** structure is used to store IP addresses, IPv4 and IPv6 addresses can be properly handled with one code base.
...
> [!Note]  
> The [**SOCKADDR\_STORAGE**](/windows/desktop/api/Ws2def/) structure is new for Windows XP.
...
3.  The utility alerts you to usage of **sockaddr** or **sockaddr\_in** structures, and provides recommendations on how to replace either with the IPv6 compatible structure [**SOCKADDR\_STORAGE**](/windows/desktop/api/Ws2def/).
...
> [!Note]  
> The **addrinfo** and **SOCKADDR\_STORAGE** structures include protocol and address family members (**ai\_family** and **ss\_family**), respectively. RFC 2553 specifies the **ai\_family** member of [**addrinfo**](/windows/desktop/api/Ws2tcpip/nc-ws2tcpip-lpfn_freeaddrinfo) as an int, while **ss\_family** is specified as a short; as such, a direct copy between those members results in a compiler error.
