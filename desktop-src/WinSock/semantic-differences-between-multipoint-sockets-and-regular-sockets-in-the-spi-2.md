...
-   The c\_root socket can be used in [**WSPJoinLeaf**](/windows/win32/Ws2spi/nc-ws2spi-lpwspjoinleaf?branch=master) to join a new leaf.
-   Placing a c\_root socket into the listening mode (by calling [**WSPListen**](/windows/win32/Ws2spi/?branch=master)) does not preclude the c\_root socket from being used in a call to [**WSPJoinLeaf**](/windows/win32/Ws2spi/nc-ws2spi-lpwspjoinleaf?branch=master) to add a new leaf, or for sending and receiving multipoint data.
-   The closing of a c\_root socket will cause all the associated c\_leaf sockets to get FD\_CLOSE notification.

There are no semantic differences between a c\_leaf socket and a regular socket in the control plane, except that the c\_leaf socket can be used in [**WSPJoinLeaf**](/windows/win32/Ws2spi/nc-ws2spi-lpwspjoinleaf?branch=master), and the use of c\_leaf socket in [**WSPListen**](/windows/win32/Ws2spi/?branch=master) indicates that only multipoint connection requests should be accepted.
