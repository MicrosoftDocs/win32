...
-   The c\_root socket can be used in [**WSPJoinLeaf**](wspjoinleaf-2.md) to join a new leaf.
-   Placing a c\_root socket into the listening mode (by calling [**WSPListen**](wsplisten-2.md)) does not preclude the c\_root socket from being used in a call to [**WSPJoinLeaf**](wspjoinleaf-2.md) to add a new leaf, or for sending and receiving multipoint data.
-   The closing of a c\_root socket will cause all the associated c\_leaf sockets to get FD\_CLOSE notification.

There are no semantic differences between a c\_leaf socket and a regular socket in the control plane, except that the c\_leaf socket can be used in [**WSPJoinLeaf**](wspjoinleaf-2.md), and the use of c\_leaf socket in [**WSPListen**](wsplisten-2.md) indicates that only multipoint connection requests should be accepted.
