...
In support of the taxonomy described above, three attribute parameters in the [**WSAPROTOCOL\_INFO**](wsaprotocol-info-2.md) structure are used to distinguish the schemes used in the control and data planes respectively:
...
Note that two [**WSAPROTOCOL\_INFO**](wsaprotocol-info-2.md) entries would be present if a multipoint protocol supported both rooted and nonrooted data planes, one entry for each.

The application can use [**WSAEnumProtocols**](wsaenumprotocols-2.md) to discover whether multipoint communications is supported for a given protocol and, if so, how it is supported with respect to the control and data planes, respectively.
