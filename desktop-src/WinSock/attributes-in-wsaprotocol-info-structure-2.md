...
In support of the taxonomy described above, three attribute parameters in the [**WSAPROTOCOL\_INFO**](/windows/desktop/api/Winsock2/) structure are used to distinguish the schemes used in the control and data planes respectively:
...
Note that two [**WSAPROTOCOL\_INFO**](/windows/desktop/api/Winsock2/) entries would be present if a multipoint protocol supported both rooted and nonrooted data planes, one entry for each.
