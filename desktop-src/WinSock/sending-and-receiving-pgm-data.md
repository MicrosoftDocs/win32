...
Once a PGM sender session is created, data is sent using the various Windows Sockets send functions: [**send**](/windows/win32/Winsock2/nf-winsock2-send?branch=master), [**sendto**](/windows/win32/winsock/nf-winsock-sendto?branch=master), [**WSASend**](/windows/win32/Winsock2/nf-winsock2-wsasend?branch=master), and [**WSASendTo**](/windows/win32/Winsock2/nf-winsock2-wsasendto?branch=master). Since Windows Sockets handles are file system handles, other functions such as [**WriteFile**](base.writefile) and CRT functions can also transmit data. The following code snippet illustrates a PGM sender operation:
...
Once a PGM receiver session is created, data is received using the various Windows Sockets receive functions: [**recv**](/windows/win32/winsock/nf-winsock-recv?branch=master), [**recvfrom**](/windows/win32/winsock/nf-winsock-recvfrom?branch=master), [**WSARecv**](/windows/win32/Winsock2/nf-winsock2-wsarecv?branch=master), and [**WSARecvFrom**](/windows/win32/Winsock2/nf-winsock2-wsarecvfrom?branch=master). Since Windows Sockets handles are also file handles, the [**ReadFile**](base.readfile) and CRT functions can also be used to receive PGM session data. The transport forwards data up to the receiver as it arrives as long as the data is in sequence. The transport guarantees that the data returned is contiguous and free of duplicates. The following code snippet illustrates a PGM receive operation:
...
When using message mode (SOCK\_RDM), the transport indicates when a partial message is received, either with the WSAEMSGSIZE error, or by setting the MSG\_PARTIAL flag upon return from the [**WSARecv**](/windows/win32/Winsock2/nf-winsock2-wsarecv?branch=master) and [**WSARecvFrom**](/windows/win32/Winsock2/nf-winsock2-wsarecvfrom?branch=master) functions. When the last fragment of the full message is returned to the client, the error or flag is not indicated.
...
The PGM sender or receiver can stop sending or receiving data by calling [**closesocket**](/windows/win32/winsock/nf-winsock-closesocket?branch=master). The receiver must call **closesocket** on both the listening and receiving sockets to prevent handle leaks. Calling [**shutdown**](/windows/win32/winsock/nf-winsock-shutdown?branch=master) on the sender before calling **closesocket** ensures all data gets sent, and ensures repair data is maintained until the send window advances past the last data sequence, even if the application itself terminates.
