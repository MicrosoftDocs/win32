...
The Winsock service provider calls the blocking hook only if all of the following are true: the routine is one which is defined as being able to block, the specified socket is a blocking socket, and the request cannot be completed immediately. If only nonblocking sockets and [**WSPAsyncSelect**](/windows/desktop/api/Ws2spi/)/[**WSPEventSelect**](/windows/desktop/api/Ws2spi/) instead of [**WSPSelect**](/windows/desktop/api/Ws2spi/) are used, then the blocking hook will never be called.
