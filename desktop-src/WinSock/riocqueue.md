...
If multiple threads attempt to access the same **RIO\_CQ** using [**RIODequeueCompletion**](/windows/win32/Mswsock/nc-mswsock-lpfn_riodequeuecompletion?branch=master), access must be coordinated by a critical section, slim reader writer lock , or similar mutual exclusion mechanism. If the completion queues are not shared, mutual exclusion is not required.

When a completion queue is no longer needed, an application can close it using the [**RIOCloseCompletionQueue**](/windows/win32/Mswsock/nc-mswsock-lpfn_rioclosecompletionqueue?branch=master) function.
