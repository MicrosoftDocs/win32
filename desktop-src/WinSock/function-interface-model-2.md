...
Windows Sockets transport and namespace–service providers are DLLs with a single exported procedure entry point for the service provider initialization function [**WSPStartup**](/windows/win32/Ws2spi/nf-ws2spi-wspstartup?branch=master) or [**NSPStartup**](/windows/win32/Ws2spi/nf-ws2spi-nspstartup?branch=master), respectively. All other service provider functions are made accessible to the Ws2\_32.dll through the service provider's dispatch table. Service provider DLL's are loaded into memory by the Ws2\_32.dll only when needed, and are unloaded when their services are no longer required.

The SPI also defines several circumstances in which a transport service provider calls up into the Ws2\_32.dll (upcalls) to obtain DLL support services. The transport service provider DLL is given the Ws2\_32.dll's upcall dispatch table through the *UpcallTable* parameter to [**WSPStartup**](/windows/win32/Ws2spi/nf-ws2spi-wspstartup?branch=master).
...
As described above, these entry points are not exported (with the exception of [**WSPStartup**](/windows/win32/Ws2spi/nf-ws2spi-wspstartup?branch=master) and [**NSPStartup**](/windows/win32/Ws2spi/nf-ws2spi-nspstartup?branch=master)), but are accessed through an exchange of dispatch tables.
