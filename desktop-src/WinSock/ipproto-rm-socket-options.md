...
The following table describes **IPPROTO\_RM** socket options that apply to sockets created for the IPv4 address family (AF\_INET) with the *protocol* parameter to the [**socket**](socket-2.md) function specified as reliable multicast (IPPROTO\_RM). See the [**getsockopt**](getsockopt-2.md) and [**setsockopt**](setsockopt-2.md) function reference pages for more information on getting and setting socket options.
...
| RM\_RATE\_WINDOW\_SIZE              | yes | yes | [**RM\_SEND\_WINDOW**](rm-send-window.md)       | Sender only. Sets the transmission rate limit, window advance time, and window size.                                                                                                                                                                                                                                                                                                                                   |
| RM\_RECEIVER\_STATISTICS            | yes |     | [**RM\_RECEIVER\_STATS**](rm-receiver-stats.md) | Receiver only. Retrieves statistics for the receiving session.                                                                                                                                                                                                                                                                                                                                                         |
...
| RM\_SENDER\_STATISTICS              | yes |     | [**RM\_SENDER\_STATS**](rm-sender-stats.md)     | Sender only. Retrieves statistics for the sending session.                                                                                                                                                                                                                                                                                                                                                             |
...
| RM\_USE\_FEC                        | yes | yes | [**RM\_FEC\_INFO**](rm-fec-info.md)             | Sender only. Notifies sender to apply forward error correction techniques to send repair data. FEC has three modes: pro-active parity packets only, OnDemand parity packets only, or both. See [**RM\_FEC\_INFO**](rm-fec-info.md) structure for more information.                                                                                                                                                    |
