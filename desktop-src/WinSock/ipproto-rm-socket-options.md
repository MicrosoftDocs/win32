...
title: IPPROTO\_RM Socket Options
ms.date: 05/31/2018
ms.topic: article
...
The following table describes **IPPROTO\_RM** socket options that apply to sockets created for the IPv4 address family (AF\_INET) with the *protocol* parameter to the [**socket**](/windows/win32/Winsock2/nf-winsock2-socket?branch=master) function specified as reliable multicast (IPPROTO\_RM). See the [**getsockopt**](/windows/win32/winsock/nf-winsock-getsockopt?branch=master) and [**setsockopt**](/windows/win32/winsock/nf-winsock-setsockopt?branch=master) function reference pages for more information on getting and setting socket options.
...
| RM\_RATE\_WINDOW\_SIZE              | yes | yes | [**RM\_SEND\_WINDOW**](/windows/win32/Wsrm/ns-wsrm-_rm_send_window?branch=master)       | Sender only. Sets the transmission rate limit, window advance time, and window size.                                                                                                                                                                                                                                                                                                                                   |
| RM\_RECEIVER\_STATISTICS            | yes |     | [**RM\_RECEIVER\_STATS**](/windows/win32/Wsrm/ns-wsrm-_rm_receiver_stats?branch=master) | Receiver only. Retrieves statistics for the receiving session.                                                                                                                                                                                                                                                                                                                                                         |
...
| RM\_SENDER\_STATISTICS              | yes |     | [**RM\_SENDER\_STATS**](/windows/win32/Wsrm/ns-wsrm-_rm_sender_stats?branch=master)     | Sender only. Retrieves statistics for the sending session.                                                                                                                                                                                                                                                                                                                                                             |
...
| RM\_USE\_FEC                        | yes | yes | [**RM\_FEC\_INFO**](/windows/win32/Wsrm/ns-wsrm-_rm_fec_info?branch=master)             | Sender only. Notifies sender to apply forward error correction techniques to send repair data. FEC has three modes: pro-active parity packets only, OnDemand parity packets only, or both. See [**RM\_FEC\_INFO**](/windows/win32/Wsrm/ns-wsrm-_rm_fec_info?branch=master) structure for more information.                                                                                                                                                    |
