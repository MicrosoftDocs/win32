...
| IP\_MULTICAST\_LOOP | yes | yes | DWORD (boolean) | For a socket that is joined to one or more multicast groups, this controls whether it will receive *outgoing* packets sent to those multicast groups via the selected multicast interface.  By default, **IP\_MULTICAST\_LOOPBACK** is enabled, so sockets *will* receive matching multicast packets sent by the current machine.  Disabling this option means that this socket will not receive multicasts sent from the local machine, even if the socket is open on the loopback interface. <br> Note: This is not compatible with the POSIX version of IP\_MULTICAST\_LOOP -- this option must be set on the receiving socket while the POSIX option must be sent on the sending socket. |
