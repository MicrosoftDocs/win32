...
On Windows Vista and later, new Winsock system extensions are exported directly from the Winsock DLL, so the [**WSAIoctl**](/windows/win32/Winsock2/nf-winsock2-wsaioctl?branch=master) function is not needed to load these extensions. The new extension functions available on Windows Vista and later include the [**WSAPoll**](/windows/win32/Mswsock/nf-winsock2-wsapoll?branch=master) and [**WSASendMsg**](/windows/win32/winsock2/nf-winsock2-wsasendmsg?branch=master) functions that are exported from *Ws2\_32.dll*.
