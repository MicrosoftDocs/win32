...
> [!Note]  
> Some SMI APIs can return system error codes. The HRESULT values that begin with "WCM\_S\_" are not returned from an SMI API. They may only appear embedded in an object that implements [**ISettingsResult**](/previous-versions/windows/desktop/api/WcmConfig/nn-wcmconfig-isettingsresult) interface.
