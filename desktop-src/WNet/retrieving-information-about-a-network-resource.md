...
To identify the network provider that owns a resource, an application can call the [**WNetGetResourceInformation**](wnetgetresourceinformation.md) function, as illustrated in the following code sample.

The following sample is a function (CheckServer) that takes a server name as a parameter and returns information about that server. First the function calls the [**ZeroMemory**](https://msdn.microsoft.com/library/windows/desktop/aa366920) function to initialize the contents of a block of memory to zero. Then the sample calls the [**WNetGetResourceInformation**](wnetgetresourceinformation.md) function, specifying a buffer large enough to hold only a [**NETRESOURCE**](netresource-str.md) structure. The routine includes error processing to handle the case when a buffer of this size is insufficient to hold the variable-length strings to which members of the **NETRESOURCE** structure point. If this error occurs, the sample allocates sufficient memory and calls **WNetGetResourceInformation** again. Finally, the sample frees the allocated memory.
