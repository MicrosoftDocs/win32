...
A pointer to an output buffer for the [JET\_RECSIZE](gg294072\(v=exchg.10\).md) structure.
...
The size of the key accumulated in the **cbOverhead** field of [JET\_RECSIZE](gg294072\(v=exchg.10\).md), is affected by JET\_bitRecordSizeInCopyBuffer. If this bit is specified, the key size accumulated in the **cbOverhead** field is the full key size. If this bit is not used, then the key size accumulated will not include any size saved due to key prefix compression.
