...
If the window being destroyed is part of the clipboard viewer chain (set by calling the [**SetClipboardViewer**](dataxchg.setclipboardviewer) function), the window must remove itself from the chain by processing the [**ChangeClipboardChain**](dataxchg.changeclipboardchain) function before returning from the **WM\_DESTROY** message.
