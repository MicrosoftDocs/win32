...
The [**WM\_NCCREATE**](wm-nccreate.md) message is sent just after your window is created, but if an application responds to this message by returning **FALSE**, [**CreateWindowEx**](/windows/win32/Winuser/nf-winuser-createwindowexa?branch=master) function fails. The [**WM\_CREATE**](wm-create.md) message is sent after your window is already created.

The [**WM\_DESTROY**](wm-destroy.md) message is sent when your window is about to be destroyed. The [**DestroyWindow**](/windows/win32/Winuser/nf-winuser-destroywindow?branch=master) function takes care of destroying any child windows of the window being destroyed. The [**WM\_NCDESTROY**](wm-ncdestroy.md) message is sent just before a window is destroyed.

At the very least, a window procedure should process the [**WM\_PAINT**](gdi.wm_paint) message to draw itself. Typically, it should handle mouse and keyboard messages as well. Consult the descriptions of individual messages to determine whether your window procedure should handle them.

Your application can call the [**DefWindowProc**](/windows/win32/Winuser/nf-winuser-defwindowproca?branch=master) function as part of the processing of a message. In such a case, the application can modify the message parameters before passing the message to **DefWindowProc**, or it can continue with the default processing after performing its own operations.

A dialog box procedure receives a [**WM\_INITDIALOG**](dlgbox.wm_initdialog) message instead of a [**WM\_CREATE**](wm-create.md) message and does not pass unprocessed messages to the [**DefDlgProc**](dlgbox.defdlgproc) function. Otherwise, a dialog box procedure is exactly the same as a window procedure.

## Associating a Window Procedure with a Window Class

You associate a window procedure with a window class when registering the class. You must fill a [**WNDCLASS**](/windows/win32/Winuser/ns-winuser-tagwndclassa?branch=master) structure with information about the class, and the **lpfnWndProc** member must specify the address of the window procedure. To register the class, pass the address of **WNDCLASS** structure to the [**RegisterClass**](/windows/win32/Winuser/nf-kusbfnclasslib-usbfnkmclasslibregisterclassdevice?branch=master) function. After the window class has been registered, the window procedure is automatically associated with each new window created with that class.
...
To subclass an instance of a window, call the [**SetWindowLong**](/windows/win32/Winuser/nf-winuser-setwindowlonga?branch=master) function and specify the handle to the window to subclass the GWL\_WNDPROC flag and a pointer to the subclass procedure. **SetWindowLong** returns a pointer to the original window procedure; use this pointer to pass messages to the original procedure. The subclass window procedure must use the [**CallWindowProc**](/windows/win32/Winuser/nf-winuser-callwindowproca?branch=master) function to call the original window procedure.

> [!Note]  
> To write code that is compatible with both 32-bit and 64-bit versions of Windows, use the [**SetWindowLongPtr**](/windows/win32/Winuser/nf-winuser-setwindowlongptra?branch=master) function.
