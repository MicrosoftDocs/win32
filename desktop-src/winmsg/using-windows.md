...
The first window an application creates is typically the main window. You create the main window by using the [**CreateWindowEx**](createwindowex.md) function, specifying the window class, window name, window styles, size, position, menu handle, instance handle, and creation data. A main window belongs to an application-defined window class, so you must register the window class and provide a window procedure for the class before creating the main window.

Most applications typically use the [**WS\_OVERLAPPEDWINDOW**](window-styles.md) style to create the main window. This style gives the window a title bar, a window menu, a sizing border, and minimize and maximize buttons. The [**CreateWindowEx**](createwindowex.md) function returns a handle that uniquely identifies the window.
...
Notice that the preceding example calls the [**ShowWindow**](showwindow.md) function after creating the main window. This is done because the system does not automatically display the main window after creating it. By passing the **SW\_SHOWDEFAULT** flag to **ShowWindow**, the application allows the program that started the application to set the initial show state of the main window. The [**UpdateWindow**](gdi.updatewindow) function sends the window its first [**WM\_PAINT**](gdi.wm_paint) message.

## Creating, Enumerating, and Sizing Child Windows

You can divide a window's client area into different functional areas by using child windows. Creating a child window is like creating a main window—you use the [**CreateWindowEx**](createwindowex.md) function. To create a window of an application-defined window class, you must register the window class and provide a window procedure before creating the child window. You must give the child window the [**WS\_CHILD**](window-styles.md) style and specify a parent window for the child window when you create it.
...
The main window sizes and positions the child windows in response to the [**WM\_SIZE**](wm-size.md) message, which the main window receives when its size changes. In response to **WM\_SIZE**, the main window retrieves the dimensions of its client area by using the [**GetClientRect**](getclientrect.md) function and then passes the dimensions to the [**EnumChildWindows**](enumchildwindows.md) function. **EnumChildWindows** passes the handle to each child window, in turn, to the application-defined [**EnumChildProc**](enumchildproc.md) callback function. This function sizes and positions each child window by calling the [**MoveWindow**](movewindow.md) function; the size and position are based on the dimensions of the main window's client area and the identifier of the child window. Afterward, **EnumChildProc** calls the [**ShowWindow**](showwindow.md) function to make the window visible.
...
Note that the third parameter of [**SetLayeredWindowAttributes**](setlayeredwindowattributes.md) is a value that ranges from 0 to 255, with 0 making the window completely transparent and 255 making it completely opaque. This parameter mimics the more versatile [**BLENDFUNCTION**](gdi.blendfunction) of the [**AlphaBlend**](gdi.alphablend) function.

To make this window completely opaque again, remove the **WS\_EX\_LAYERED** bit by calling [**SetWindowLong**](setwindowlong.md) and then ask the window to repaint. Removing the bit is desired to let the system know that it can free up some memory associated with layering and redirection. The code might look like this:
