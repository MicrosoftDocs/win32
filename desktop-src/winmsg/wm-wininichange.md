...
title: 'WM\_WININICHANGE message'
---

# WM\_WININICHANGE message

An application sends the **WM\_WININICHANGE** message to all top-level windows after making a change to the WIN.INI file. The [**SystemParametersInfo**](systemparametersinfo.md) function sends this message after an application uses the function to change a setting in WIN.INI.
...
To send the **WM\_WININICHANGE** message to all top-level windows, use the [**SendMessage**](_win32_sendmessage_cpp) function with the *hWnd* parameter set to **HWND\_BROADCAST**.
