...
-   Configure the request queue using the [**HttpSetRequestQueueProperty**](/windows/win32/Http/nf-http-httpsetrequestqueueproperty?branch=master) function.
-   Query the request queue configuration parameters using the [**HttpQueryRequestQueueProperty**](/windows/win32/Http/nf-http-httpqueryrequestqueueproperty?branch=master) function.
...
-   Call [**HttpWaitForDemandStart**](/windows/win32/Http/nf-http-httpwaitfordemandstart?branch=master) to delay the instantiation of worker processes until the first request arrives on the request queue.

In addition to these tasks, the creator process can also perform I/O operations on the request queue.

## Worker Processes

A worker process can open an existing request queue only if they have been granted access to it in the ACL. Worker processes operating under least privileges can open a request queue and perform I/O on it. Applications open an existing request queue by calling [**HttpCreateRequestQueue**](/windows/win32/Http/nf-http-httpcreaterequestqueue?branch=master) with the **HTTP\_CREATE\_REQUEST\_QUEUE\_FLAG\_OPEN\_EXISTING** in the *Flags* parameter, and the name of the request queue in the *pName* parameter.
