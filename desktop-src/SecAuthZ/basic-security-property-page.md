...
The basic security property page is the starting page of the property sheet displayed by the [**EditSecurity**](/windows/win32/Aclui/nf-aclui-editsecurity?branch=master) function. You can also use the [**CreateSecurityPage**](/windows/win32/Aclui/nf-aclui-createsecuritypage?branch=master) function to create a basic security property page to insert in your own property sheet.

The property page displays a list of the [trustees](trustees.md) named in the [*access control entries*](https://msdn.microsoft.com/library/windows/desktop/ms721532#-security-access-control-entry-gly) (ACEs) of the object's [*discretionary access control list*](https://msdn.microsoft.com/library/windows/desktop/ms721573#-security-discretionary-access-control-list-gly) (DACL). The page also contains a list of the access rights supported by the object. When the user selects a name from the list of trustees, the check boxes next to each access right indicate the rights that are allowed or denied for that trustee. The user can then select or clear the check boxes to modify the trustee's access rights. The user can also add or remove trustees from the list.
...
You can use the **pszPageTitle** member of the [**SI\_OBJECT\_INFO**](/windows/win32/Aclui/ns-aclui-_si_object_info?branch=master) structure to specify the title of the basic security property page. The default title is **Security**.
