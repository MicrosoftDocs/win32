...
To determine the privileges held in an access token, call the [**GetTokenInformation**](https://msdn.microsoft.com/en-us/library/Aa446671(v=VS.85).aspx) function, which also indicates which privileges are enabled. Most privileges are disabled by default.
