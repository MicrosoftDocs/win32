...
Use the [**InitializeClientContextFromToken**](iazapplication-initializeclientcontextfromtoken.md), [**InitializeClientContextFromName**](iazapplication-initializeclientcontextfromname.md), and [**InitializeClientContextFromStringSid**](iazapplication-initializeclientcontextfromstringsid.md) methods of the [**IAzApplication**](iazapplication.md) interface to create a client context.

The following example shows how to create an [**IAzClientContext**](iazclientcontext.md) object from a client token. The example assumes that there is an existing XML policy store named MyStore.xml in the root directory of drive C, that this store contains an application named Expense, and that the variable hToken contains a valid client token.
