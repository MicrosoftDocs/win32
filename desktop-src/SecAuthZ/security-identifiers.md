---
description: A security identifier (SID) is a unique value of variable length used to identify a trustee. Learn how SIDs work in the Windows security model, including their role in access tokens, security descriptors, and access control entries.
ms.assetid: 7cb07bcd-70f4-43dd-8382-320fcff151c7
title: Security Identifiers
ms.topic: concept-article
ms.date: 07/08/2025
# customer intent: As a Windows app developer, I want to understand how security identifiers (SIDs) work in the Windows security model, so that I can effectively manage user and group access in my applications.
---

# Security identifiers

A [security identifier](/windows/win32/SecGloss/s-gly) (SID) is a unique value of variable length used to identify a [trustee](trustees.md). Each account has a unique SID issued by an authority, such as a Windows domain controller, and stored in a security database. Each time a user logs on, the system retrieves the SID for that user from the database and places it in the [access token](/windows/win32/SecGloss/a-gly) for that user. The system uses the SID in the access token to identify the user in all subsequent interactions with Windows security. When a SID has been used as the unique identifier for a user or group, it cannot ever be used again to identify another user or group.
...
| [ConvertSidToStringSid](/windows/win32/api/Sddl/nf-sddl-convertsidtostringsida) | Converts a SID to a string format suitable for display, storage, or transport. |
| [ConvertStringSidToSid](/windows/win32/api/Sddl/nf-sddl-convertstringsidtosida) | Converts a string-format SID to a valid, functional SID. |
...
| [InitializeSid](/windows/win32/api/securitybaseapi/nf-securitybaseapi-initializesid) | Initializes a [SID](/windows/win32/api/Winnt/ns-winnt-sid) structure. |
...
| [LookupAccountName](/windows/win32/api/Winbase/nf-winbase-lookupaccountnamea) | Retrieves the SID that corresponds to a specified account name. |
| [LookupAccountSid](/windows/win32/api/Winbase/nf-winbase-lookupaccountsida) | Retrieves the account name that corresponds to a specified SID. |
