...
The first step to using using System Restore is setting up the COM calls to [**CoInitializeEx**](https://www.bing.com/search?q=**CoInitializeEx**) and [**CoInitializeSecurity**](https://www.bing.com/search?q=**CoInitializeSecurity**). This is required for any process that uses the [**SRSetRestorePoint**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) function. The NetworkService, LocalService, and System must be allowed make calls to the process. The following **InitializeCOMSecurity** function is an example of how to initialize COM security. You may need to modify the parameters to the **CoInitializeSecurity** function for your application.
