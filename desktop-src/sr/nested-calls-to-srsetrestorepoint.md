...
This topic describes support for nested calls to [**SRSetRestorePoint**](/windows/win32/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa?branch=master) through the BEGIN\_NESTED\_SYSTEM\_CHANGE and END\_NESTED\_SYSTEM\_CHANGE event types.

Applications can safely call [**SRSetRestorePoint**](/windows/win32/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa?branch=master) when using these event types. The first call to the function creates a restore point. Subsequent nested calls to the function do not create restore points. For example, suppose an application makes the following calls to **SRSetRestorePoint**:<dl> For restore point A with dwEventType = BEGIN\_NESTED\_SYSTEM\_CHANGE  
