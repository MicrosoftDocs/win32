...
The [**IDWriteFactory**](/windows/desktop/api/dwrite/) interface is the starting point for using DirectWrite functionality. The factory is the root object that creates a set of objects that can be used together.
...
In addition to specifying a font family name for the text string by using the [**IDWriteTextFormat**](/windows/desktop/api/dwrite/) interface in the example above, [DirectWrite](direct-write-portal.md) provides applications more control over font selection through font enumeration and the ability to create custom font collection based on embedded document fonts.

The [**IDWriteFontCollection**](/windows/desktop/api/dwrite/) object is a collection of font families. DirectWrite provides access to the set of fonts installed on the system through a special font collection called the system font collection. This is obtained by calling the [**GetSystemFontCollection**](/windows/desktop/api/dwrite/) method of the [**IDWriteFactory**](/windows/desktop/api/dwrite/) object. An application can also create a custom font collection from a set of fonts enumerated by an application-defined callback, that is, private fonts installed by an application, or fonts embedded in a document.

The application can then call [**GetFontFamily**](/windows/desktop/api/dwrite/) to get to a specific FontFamily object within the collection, and then call [**IDWriteFontFamily::GetFirstMatchingFont**](/windows/desktop/api/dwrite/) to get to a specific [**IDWriteFont**](/windows/desktop/api/dwrite/) object. The **IDWriteFont** object represents a font in a font collection and exposes properties and a few basic font metrics.

The [**IDWriteFontFace**](/windows/desktop/api/dwrite/) is another object that represents a font and exposes a full set of metrics on a font. The **IDWriteFontFace** can be created directly from a font name; an application does not have to get a font collection to access it. It is useful for a text layout application such as Microsoft Word that needs to query the details for a specific font.
...
The [**IDWriteFontFace**](/windows/desktop/api/dwrite/) object represents a font and provides more detailed information about the font than the [**IDWriteFont**](/windows/desktop/api/dwrite/) object does. The font and glyph metrics from the **IDWriteFontFace** are useful for applications that implement text layout.

Most mainstream applications will not use these APIs directly, and instead will use [**IDWriteFont**](/windows/desktop/api/dwrite/) or specify the font family name directly.
...
2.  [**IDWriteBitmapRenderTarget::DrawGlyphRun**](/windows/desktop/api/dwrite/)
    ...
    Applications can use the [**IDWriteBitmapRenderTarget::DrawGlyphRun**](/windows/desktop/api/dwrite/) method to perform a software-rendering of a run of glyphs to a 32-bpp bitmap. The [**IDWriteBitmapRenderTarget**](/windows/desktop/api/dwrite/) object encapsulates a bitmap and a memory device context that can be used for rendering glyphs. This API is useful if you want to stay with GDI because you have an existing code base that renders in GDI.

If you have an application that has existing text layout code that uses GDI, and you want to preserve its existing layout code but use [DirectWrite](direct-write-portal.md) just for the final step of rendering glyphs, [**IDWriteGdiInterop::CreateFontFaceFromHdc**](/windows/desktop/api/dwrite/) provides the bridge between the two APIs. Before calling this function, the application will use the **IDWriteGdiInterop::CreateFontFaceFromHdc** function to get a font-face reference from a device context.

> [!Note]  
> For most scenarios, applications may not need to use these glyph-rendering APIs. After an application has created an [**IDWriteTextLayout**](/windows/desktop/api/dwrite/) object, it can use the [**ID2D1RenderTarget::DrawTextLayout**](https://msdn.microsoft.com/library/windows/desktop/dd371913) method to render the text.

 

## Custom Rendering Modes

A number of parameters affect text rendering, such as gamma, ClearType level, pixel geometry, and enhanced contrast. Rendering parameters are encapsulated by an object, which implements the public [**IDWriteRenderingParams**](/windows/desktop/api/dwrite/) interface. The rendering parameters object is automatically initialized based on hardware properties and/or user preferences specified through the ClearType control panel applet in Windows 7. Generally, if a client uses the [DirectWrite](direct-write-portal.md) layout API, DirectWrite will automatically select a rendering mode that corresponds to the specified measuring mode.

Applications that want more control can use [**IDWriteFactory::CreateCustomRenderingParams**](/windows/desktop/api/dwrite/) to implement the different rendering options. This function can also be used to set the gamma, pixel geometry, and enhanced contrast.
...
The [**IDWriteGdiInterop**](/windows/desktop/api/dwrite/) interface provides interoperability with GDI. This enables applications to continue their existing investment in GDI code bases and selectively use [DirectWrite](direct-write-portal.md) for either rendering or layout.
...
-   [**CreateFontFromLOGFONT**](/windows/desktop/api/dwrite/)
    ...
    Creates an [**IDWriteFont**](/windows/desktop/api/dwrite/) object that matches the properties specified by the [**LOGFONT**](https://msdn.microsoft.com/library/windows/desktop/dd145037) structure.

-   [**ConvertFontToLOGFONT**](/windows/desktop/api/dwrite/)

    Initializes a [**LOGFONT**](https://msdn.microsoft.com/library/windows/desktop/dd145037) structure based on the GDI-compatible properties of the specified [**IDWriteFont**](/windows/desktop/api/dwrite/).

-   [**ConvertFontFaceToLOGFONT**](/windows/desktop/api/dwrite/)

    Initializes a [**LOGFONT**](https://msdn.microsoft.com/library/windows/desktop/dd145037) structure based on the GDI-compatible properties of the specified [**IDWriteFontFace**](/windows/desktop/api/dwrite/).

-   [**CreateFontFaceFromHdc**](/windows/desktop/api/dwrite/)

    Creates an [**IDWriteFontFace**](/windows/desktop/api/dwrite/) object that corresponds to the currently selected **HFONT**.
