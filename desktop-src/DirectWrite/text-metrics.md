...
In order to access the PANOSE info for a font, use the [**GetPanose**](/windows/desktop/api/dwrite_1/) method on the [**IDWriteFont1**](/windows/desktop/api/dwrite_1/) and [**IDWriteFontFace1**](/windows/desktop/api/dwrite_1/) interfaces. This method returns a [**DWRITE\_PANOSE**](/windows/desktop/api/Dwrite_1/ns-dwrite_1-dwrite_panose) enumeration that contains all of the PANOSE info for that font.
...
This info is all available through the new [**GetMetrics**](/windows/desktop/api/dwrite_1/) method on the [**IDWriteFontFace1**](/windows/desktop/api/dwrite_1/) and [**IDWriteFont1**](/windows/desktop/api/dwrite_1/) interfaces. This method returns a [**DWRITE\_FONT\_METRICS1**](/windows/desktop/api/Dwrite_1/ns-dwrite_1-dwrite_font_metrics1) structure that contains all this info.

## Caret Metrics

To create text editing apps you need access to info about how to draw the caret that navigates through the text. Starting in Windows 8, [DirectWrite](direct-write-portal.md) provides the [**GetCaretMetrics**](/windows/desktop/api/dwrite_1/) method on the [**IDWriteFontFace1**](/windows/desktop/api/dwrite_1/) and [**IDWriteFont1**](/windows/desktop/api/dwrite_1/) interfaces for this scenario. **GetCaretMetrics** returns a [**DWRITE\_CARET\_METRICS**](/windows/desktop/api/Dwrite_1/ns-dwrite_1-dwrite_caret_metrics) enumeration which contains info about the slope and offset for the caret along the baseline.
...
you use the [**DWRITE\_INFORMATIONAL\_STRING\_ID**](/windows/desktop/api/dwrite/ne-dwrite-dwrite_informational_string_id) enumeration to get the full name, PostScript name, and PostScript CID name of any font on the system. This info is valuable when you need to match fonts in your app to appropriate fonts on the local system.

## Glyph Advances

The **GetGlyphAdvances** method on the [**IDWriteFontFace1**](/windows/desktop/api/dwrite_1/) and [**IDWriteFont1**](/windows/desktop/api/dwrite_1/) interfaces takes in the glyph count and indices that you need advances info about and then returns the advances for the glyphs in question.
...
The [**GetUnicodeRanges**](/windows/desktop/api/dwrite_1/) method on the the [**IDWriteFontFace1**](/windows/desktop/api/dwrite_1/) and [**IDWriteFont1**](/windows/desktop/api/dwrite_1/) interfaces takes in the maximum number of ranges passed in from the client, and returns the actual ranges supported by the font.

## EUDC Font Collection

Use the [**GetEudcFontCollection**](/windows/desktop/api/dwrite_1/) method on the [**IDWriteFactory1**](/windows/desktop/api/dwrite_1/) interface access the EUDC font collection. This method works in the same way as [**GetSystemFontCollection**](/windows/desktop/api/dwrite/), but instead returns a pointer to an EUDC font collection.
