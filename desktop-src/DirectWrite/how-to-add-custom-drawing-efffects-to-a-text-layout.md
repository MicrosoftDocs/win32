...
Provides a short tutorial on adding client drawing effects to a [DirectWrite](direct-write-portal.md) application that displays text using the [**IDWriteTextLayout**](/windows/win32/dwrite/?branch=master) interface and a custom text renderer.
...
> [!Note]  
> This tutorial is meant to be a simplified example of how to create custom client drawing effects, not an example of a simple method for drawing color text. See the [**IDWriteTextLayout::SetDrawingEffect**](/windows/win32/dwrite/?branch=master) reference page for more information.
...
To begin, you will need an application with an [**IDWriteTextLayout**](/windows/win32/dwrite/?branch=master) object. If you already have an application that displays text with a text layout, or you are using the Custom DrawingEffect Example Code, skip to Step 2.
...
After a client drawing effect is applied to a text range in a [**IDWriteTextLayout**](/windows/win32/dwrite/?branch=master) object, the drawing effect is passed to the [**IDWriteTextRenderer::DrawGlyphRun**](/windows/win32/dwrite/?branch=master) method of any glyph run that is to be rendered. The methods of the drawing effect are then available to the text renderer.
...
In order to take advantage of a client drawing effect, you must implement a custom text renderer. This text renderer will apply the drawing effect passed to it by the [**IDWriteTextLayout::Draw**](/windows/win32/dwrite/?branch=master) method to the glyph run being drawn.

### The Constructor

The constructor for the custom text renderer stores the [**ID2D1Factory**](https://msdn.microsoft.com/library/windows/desktop/dd371246) object that will be used to create [Direct2D](direct2d.direct2d_portal.xml) objects, and the Direct2D render target that the text will be drawn on to.
...
A glyph run is a set of glyphs that share the same format, including the client drawing effect. The [**DrawGlyphRun**](/windows/win32/dwrite/?branch=master) method takes care of the text rendering for a specified glyph run.

First, create an [**ID2D1PathGeometry**](https://msdn.microsoft.com/library/windows/desktop/dd371512) and an [**ID2D1GeometrySink**](https://msdn.microsoft.com/library/windows/desktop/dd316592), and then retrieve the glyph run outline by using [**IDWriteFontFace::GetGlyphRunOutline**](/windows/win32/dwrite/?branch=master). Then transform the origin of the geometry by using the [Direct2D](direct2d.direct2d_portal.xml) [**ID2D1Factory::CreateTransformedGeometry**](https://msdn.microsoft.com/library/windows/desktop/dd371304) method, as shown in the following code.
...
Next, declare a [Direct2D](direct2d.direct2d_portal.xml) solid brush object.
...
Don't forget to release the [Direct2D](direct2d.direct2d_portal.xml) factory and render target in the destructor.
...
Set the drawing effect for specific ranges of text by using the [**IDWriteTextLayou::SetDrawingEffect**](/windows/win32/dwrite/?branch=master) method and a [**DWRITE\_TEXT\_RANGE**](/windows/win32/dwrite/ns-dwrite-dwrite_text_range?branch=master) struct.
...
You must call the [**IDWriteTextLayout::Draw**](/windows/win32/dwrite/?branch=master) method rather than the [**ID2D1RenderTarget::DrawText**](https://msdn.microsoft.com/library/windows/desktop/dd371919) or [**ID2D1RenderTarget::DrawTextLayout**](https://msdn.microsoft.com/library/windows/desktop/dd371913) methods.
