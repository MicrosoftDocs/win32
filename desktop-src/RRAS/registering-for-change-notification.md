...
To register for change notification, a client must call [**RtmRegisterForChangeNotification**](/windows/win32/Rtmv2/nf-rtmv2-rtmregisterforchangenotification?branch=master), specifying the types of changes for which the client must receive notification. If the client must be notified of change to specific destinations, the client calls [**RtmMarkDestForChangeNotification**](/windows/win32/Rtmv2/nf-rtmv2-rtmmarkdestforchangenotification?branch=master) once for each destination.

The client can stop receiving change notifications by calling [**RtmDeregisterFromChangeNotification**](/windows/win32/Rtmv2/nf-rtmv2-rtmderegisterfromchangenotification?branch=master).
