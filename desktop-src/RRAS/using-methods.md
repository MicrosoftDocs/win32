...
To invoke a second client's method, a client calls the [**RtmInvokeMethod**](/windows/win32/Rtmv2/nf-rtmv2-rtminvokemethod?branch=master) function. The routing table manager arbitrates all requests to invoke a client's methods. The routing table manager performs two functions when it arbitrates between clients:
...
If a client must prevent other clients from executing its methods, the client can call [**RtmBlockMethods**](/windows/win32/Rtmv2/nf-rtmv2-rtmblockmethods?branch=master). The routing table manager will not allow a call to [**RtmInvokeMethod**](/windows/win32/Rtmv2/nf-rtmv2-rtminvokemethod?branch=master) to be processed until the client unblocks its methods again.
