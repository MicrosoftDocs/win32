...
After the kernel forwarder receives multicast data from a specific source on the wrong interface, it notifies the multicast group manager. The multicast group manager then invokes the [**PMGM\_WRONG\_IF\_CALLBACK**](pmgm-wrong-if-callback.md) callback to the routing protocol that owns the interface on which the data incorrectly arrived.
