...
A protocol that implements hold-down states indicates that a destination is in a hold-down state by using the [**RtmHoldDestination**](/windows/win32/Rtmv2/nf-rtmv2-rtmholddestination?branch=master) function. The client calls this function when it advertises the best route to this destination. If all routes to this destination are later deleted, the last route that is deleted is kept in a hold-down state for the period of time specified in the earlier call to **RtmHoldDestination**.
