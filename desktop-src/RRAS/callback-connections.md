...
A user assigned the Set By Caller privilege can choose to connect either with or without a callback. The [**RasDial**](/windows/desktop/api/Ras/nf-ras-rasdiala) call uses the **szCallbackNumber** member of the [**RASDIALPARAMS**](https://msdn.microsoft.com/en-us/library/Aa377238(v=VS.85).aspx) structure to indicate the choice.
