...
keywords: ["Routing, Management Information Base", "Routing, Management Information Base, described", "Management Information Base RRAS", "MIB", "MIB, See Management Information Base", "Management Information Base RRAS", "Management Information Base RRAS , described"]
---
...
The MprAdminMib functions can be called on a computer other than the one being administered. The MprAdminMIB functions that query or write values, take as a parameter a handle to the computer to administer. Use the [**MprAdminMIBServerConnect**](mpradminmibserverconnect.md) function to establish the connection to the remote computer and obtain this handle. After calling the necessary MprAdminMIB functions to accomplish a particular administrative task, call the [**MprAdminMIBServerDisconnect**](mpradminmibserverdisconnect.md) function to terminate the connection to the remote computer.

The [**MprAdminMIBEntryCreate**](mpradminmibentrycreate.md) and [**MprAdminMIBEntrySet**](mpradminmibentryset.md) functions take as parameters an OID and a buffer which contains the new value for the object.

The [**MprAdminMIBEntryGet**](mpradminmibentryget.md), [**MprAdminMIBEntryGetFirst**](mpradminmibentrygetfirst.md) and [**MprAdminMIBEntryGetNext**](mpradminmibentrygetnext.md) functions take as parameters an OID and the address of a pointer variable. On successful return, the pointer variable points to a buffer that contains the value for the object. The caller should free the memory for this buffer by calling the [**MprAdminMIBBufferFree**](mpradminmibbufferfree.md) function.

The [**MprAdminMIBEntryGetFirst**](mpradminmibentrygetfirst.md) and [**MprAdminMIBEntryGetNext**](mpradminmibentrygetnext.md) functions enable a developer to perform an *SNMP walk*. Because the OIDs are ordered, each OID and therefore each manageable object has a *next* OID. An SNMP walk refers to traversing a portion of the MIB by reading or writing a sequence of OIDs.
...
-   [**MibCreate**](mibcreate.md)
-   [**MibDelete**](mibdelete.md)
-   [**MibSet**](mibset.md)
-   [**MibGet**](mibget.md)
-   [**MibGetFirst**](mibgetfirst.md)
-   [**MibGetNext**](mibgetnext.md)
-   [**MibGetTrapInfo**](mibgettrapinfo.md)
-   [**MibSetTrapInfo**](mibsettrapinfo.md)
