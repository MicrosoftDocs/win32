---
title: To Open a Project File
description: To Open a Project File
ms.assetid: ef955844-14cb-4c19-8265-c15f753b0496
keywords:
- Windows Movie Maker,opening project files
- Movie Maker,opening project files
- programming reference,opening project files
- reference for Windows Movie Maker,opening project files
- command-line options for Windows Movie Maker,opening project files
- opening project files
ms.technology: desktop
ms.prod: windows
ms.author: windowssdkdev
ms.topic: article
ms.date: 05/31/2018
---

# To Open a Project File

This option specifies a Windows Movie Maker project file to open.

**Syntax**


```C++
moviemk 
projectfile.mswmm
```



*Parameters*

*projectfile.mswmm*

Specifies the path of a Windows Movie Maker project file to open.

**Examples**


```C++
moviemk C:\Users\johnevans\dadsbirthday.mswmm
```



## Related topics

<dl> <dt>

[**Command-line Options for Windows Movie Maker**](command-line-options-for-windows-movie-maker.md)
</dt> </dl>

 

 




