...
The stand-alone implementation of [**IPropertyStorage**](/windows/desktop/api/Propidl/nn-propidl-ipropertystorage) does not support the variant types VT\_UNKNOWN or VT\_DISPATCH in the **vt** member of the [**PROPVARIANT**](/windows/desktop/api/propidlbase/ns-propidl-tagpropvariant) structure.

The following variant types are supported within a SafeArray; that is, these values can be combined with VT\_ARRAY in the **vt** member of the [**PROPVARIANT**](/windows/desktop/api/propidlbase/ns-propidl-tagpropvariant) structure.
...
Reads the properties specified in the *rgpspec* array and supplies the values of all valid properties in the *rgvar* array of [**PROPVARIANT**](/windows/desktop/api/propidlbase/ns-propidl-tagpropvariant) elements.
...
Writes the properties specified in the *rgpspec*\[\] array, assigning them the [**PROPVARIANT**](/windows/desktop/api/propidlbase/ns-propidl-tagpropvariant) tags and values specified in *rgvar*\[\]. Properties that already exist are assigned the specified **PROPVARIANT** values, and properties that do not currently exist are created.
...
Creates an enumerator object that implements [**IEnumSTATPROPSTG**](/windows/desktop/api/propidlbase/nn-propidl-ienumstatpropstg), the methods of which can be called to enumerate the [**STATPROPSTG**](/windows/desktop/api/propidlbase/ns-propidl-tagstatpropstg) structures that provide information about each of the properties in the set.

This implementation creates an array into which the entire property set is read and which can be shared when [**IEnumSTATPROPSTG::Clone**](/windows/desktop/api/propidlbase/nn-propidl-ienumstatpropstg) is called.
...
Fills in the members of a [**STATPROPSETSTG**](/windows/desktop/api/propidlbase/ns-propidl-tagstatpropsetstg) structure, which contains information about the property set as a whole. On return, supplies a pointer to the structure.
