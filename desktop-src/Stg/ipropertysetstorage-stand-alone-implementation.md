...
Call the methods of [**IPropertySetStorage**](/windows/desktop/api/Propidl/nn-propidl-ipropertysetstorage) to create, open, and delete property sets in any structured storage. There is also a method that supplies a pointer to the [**IEnumSTATPROPSETSTG**](/windows/desktop/api/propidlbase/nn-propidl-ienumstatpropsetstg) enumerator that can be used to enumerate the property sets in the storage.
...
Creates an object that can be used to enumerate [**STATPROPSETSTG**](/windows/desktop/api/propidlbase/ns-propidl-tagstatpropsetstg) structures. Each **STATPROPSETSTG** structure provides data about a single property set.
