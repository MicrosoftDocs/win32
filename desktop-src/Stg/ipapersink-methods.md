...
COPaper exposes the [**IConnectionPointContainer**](https://www.bing.com/search?q=**IConnectionPointContainer**) interface so clients can connect to COPaper in order to receive notifications of specified events that occur in COPaper. By exposing this interface, COPaper becomes a connectable object. A client can call [**QueryInterface**](https://www.bing.com/search?q=**QueryInterface**) for this interface and use it to obtain the object's connection points. The client participation in this scheme is covered in the associated **StoClien** sample.
