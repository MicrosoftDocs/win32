...
**StoServe** is a DLL that is intended primarily as a COM server. Although it can be implicitly loaded by linking to its associated .LIB file, it is normally used after an explicit LoadLibrary call, usually from within the COM function [**CoGetClassObject**](https://msdn.microsoft.com/en-us/library/ms684007(v=VS.85).aspx). **StoServe** is a self-registering in-process server.
