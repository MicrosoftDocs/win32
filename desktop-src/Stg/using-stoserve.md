...
**StoServe** is a DLL that is intended primarily as a COM server. Although it can be implicitly loaded by linking to its associated .LIB file, it is normally used after an explicit LoadLibrary call, usually from within the COM function [**CoGetClassObject**](https://www.bing.com/search?q=**CoGetClassObject**). **StoServe** is a self-registering in-process server.
