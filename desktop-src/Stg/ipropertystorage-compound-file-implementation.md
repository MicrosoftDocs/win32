...
The compound file implementation of [**IPropertyStorage**](/windows/desktop/api/Propidl/nn-propidl-ipropertystorage) does not support the variant types VT\_UNKNOWN or VT\_DISPATCH in the **vt** member of the [**PROPVARIANT**](/windows/desktop/api) structure.

The following table lists variant types that are supported within a SafeArray; that is, these values can be combined with VT\_ARRAY in the **vt** member of the [**PROPVARIANT**](/windows/desktop/api) structure.
...
When VT\_VARIANT is combined with VT\_ARRAY, the SafeArray itself holds [**PROPVARIANT**](/windows/desktop/api) structures. However, the types of these elements must be taken from the preceding list, cannot be VT\_VARIANT, and cannot include the VT\_VECTOR, VT\_ARRAY, or VT\_BYREF indicators.
...
Writes the properties specified in the *rgpspec*\[\] array, assigning them the [**PROPVARIANT**](/windows/desktop/api) tags and values specified in *rgvar*\[\]. Properties that already exist are assigned the specified **PROPVARIANT** values. Properties that do not currently exist are created.
...
Constructs an instance of [**IEnumSTATPROPSTG**](/windows/desktop/api), the methods of which can be called to enumerate the [**STATPROPSTG**](/windows/desktop/api) structures that provide information about each of the properties in the set. This implementation creates an array into which the entire property set is read and which can be shared when **IEnumSTATPROPSTG::Clone** is called. Changes to the property set are not reflected in an open **IEnumSTATPROPSTG** instance. To see such changes, a new instance of this enumerator must be constructed.
