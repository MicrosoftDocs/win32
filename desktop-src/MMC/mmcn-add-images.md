...
The **MMCN\_ADD\_IMAGES** notification message is sent to the snap-in's [**IComponent**](/windows/desktop/api/Mmc/nn-mmc-icomponent) implementation to add images for the result pane. MMC provides a pointer to the [**IImageList**](/windows/desktop/api/Mmc/nn-mmc-iimagelist) interface when this notification message is sent to the snap-in.
...
A pointer to the result pane image list ([**IImageList**](/windows/desktop/api/Mmc/nn-mmc-iimagelist)). This pointer is valid only while the specific **MMCN\_ADD\_IMAGES** notification is being processed and should not be stored for later use. Additionally, the snap-in must not call the Release method of IImageList because MMC must release it.
