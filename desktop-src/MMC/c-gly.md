...
Last page of a Wizard97-style wizard. The Completion page typically has a bitmap image, called a [*watermark*](w-gly.md#mmc-watermark-gly), to convey a specific message to the user. This is the same watermark image that is used in the Wizard97-style wizard [*Welcome page*](w-gly.md#mmc-welcome-page-gly).
...
Data object created by MMC that consists of an array of [*multiselection data objects*](m-gly.md#mmc-multiselection-data-object-gly) provided by the snap-ins involved in a multiple selection operation.
...
[*Wizard*](w-gly.md#mmc-wizard-gly) that a snap-in can optionally display when the user adds the snap-in to a [*console*](#mmc-console-gly) using the **Add/Remove Snap-in** dialog box. The snap-in can use the wizard to gather settings for initializing the snap-in state.
...
File with an .msc (management saved console) extension that preserves the list of loaded snap-ins for a [*console*](#mmc-console-gly), the arrangement and contents of open [*multiple-document interface (MDI)*](m-gly.md#mmc-multiple-document-interface-mdi--gly) child windows ([*views*](v-gly.md#mmc-view-gly)) in the main MMC window, the default mode, and information about permissions. Configuration settings for the tools and controls are saved with the console and restored when the console file is opened.
...
Hierarchical structure in the scope pane of a [*view*](v-gly.md#mmc-view-gly). Displays the [*static nodes*](s-gly.md#mmc-static-node-gly) and [*enumerated nodes*](e-gly.md#mmc-enumerated-node-gly) associated with stand-alone and extension snap-ins.
...
[*Scope item*](s-gly.md#mmc-scope-item-gly) with child scope items.
...
[*Extension snap-in*](e-gly.md#mmc-extension-snap-in-gly) that extends the functionality of a [*primary snap-in*](p-gly.md#mmc-primary-snap-in-gly). The context menu extension adds menu items to the context menus of scope and result items of a particular [*node type*](n-gly.md#mmc-node-type-gly).
...
Child window that an application uses with another window to perform simple input and output (I/O) tasks. Examples of controls in the main MMC window are the [*toolbar control*](t-gly.md#mmc-toolbar-control-gly) and the [*menu button control*](m-gly.md#mmc-menu-button-control-gly).
...
See [*OCX view*](o-gly.md#mmc-ocx-view-gly).
...
[*Taskpad*](t-gly.md#mmc-taskpad-gly) view style in which the snap-in displays a custom DHTML page that contains tasks as well as other elements.
...
See [*HTML view*](h-gly.md#mmc-html-view-gly).
