...
The standard method for providing version and provider information within most applications that run on Windows platforms is the **About** menu selection item. MMC supports snap-in applications in providing this type of information. The information is displayed on a property page and is made available by using the [**ISnapinAbout**](/windows/win32/Mmc/nn-mmc-isnapinabout?branch=master) interface.
...
MMC also calls the snap-in [**ISnapinAbout**](/windows/win32/Mmc/nn-mmc-isnapinabout?branch=master) implementation to obtain custom folder images for the snap-in static node. If a snap-in does not implement **ISnapinAbout**, MMC uses default folder images instead. Be aware that snap-ins can also programmatically specify the static node's folder images when the static node is selected and the snap-in receives an [**MMCN\_EXPAND**](mmcn-expand.md) notification message. However, implementing **ISnapinAbout** is the recommended method for specifying the static folder images.

Methods belonging to [**ISnapinAbout**](/windows/win32/Mmc/nn-mmc-isnapinabout?branch=master) include [**GetSnapinDescription**](/windows/win32/Mmc/nf-mmc-isnapinabout-getsnapindescription?branch=master), which provides text for the snap-in description box. [**GetProvider**](/windows/win32/Mmc/nf-mmc-isnapinabout-getprovider?branch=master) handles the name of the provider, and [**GetSnapinVersion**](/windows/win32/Mmc/nf-mmc-isnapinabout-getsnapinversion?branch=master) obtains the snap-in version number. [**GetSnapinImage**](/windows/win32/Mmc/nf-mmc-isnapinabout-getsnapinimage?branch=master) gets the main icon for the About property page, and [**GetStaticFolderImage**](/windows/win32/Mmc/nf-mmc-isnapinabout-getstaticfolderimage?branch=master) gets the static folder images for the scope and result panes.
...
1.  Implement the [**ISnapinAbout**](/windows/win32/Mmc/nn-mmc-isnapinabout?branch=master) interface in the snap-in.
2.  Use a COM object other than the snap-in's primary object (the one that exposes the snap-in's [**IComponentData**](/windows/win32/Mmc/nn-mmc-icomponentdata?branch=master) interface) to expose the [**ISnapinAbout**](/windows/win32/Mmc/nn-mmc-isnapinabout?branch=master) interface. This is so that, for performance reasons, MMC is not required to create an instance of the entire snap-in to access its About information.
3.  Register the CLSID of this object under the HKEY\_CLASSES\_ROOT\\CLSID key in the {CLSID} subkey as an in-process server DLL. Also register an apartment threading model for the object. This allows the snap-in's [**ISnapinAbout**](/windows/win32/Mmc/nn-mmc-isnapinabout?branch=master) interface to be marshaled by COM to MMC.
