...
[**SMMCObjectTypes**](/windows/desktop/api/Mmc/ns-mmc-_smmcobjecttypes) structure. The structure contains the array of node type GUIDs that constitute the multiselection data object. A snap-in's multiselection data object has a list that contains each node type represented in the set of selected items for that particular snap-in in the result pane.
