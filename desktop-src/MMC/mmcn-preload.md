...
The **MMCN\_PRELOAD** notification message is sent to the snap-in's [**IComponentData**](/windows/desktop/api/Mmc/nn-mmc-icomponentdata) implementation if the snap-in returned **TRUE** when MMC called [**IDataObject::GetDataHere**](https://www.bing.com/search?q=**IDataObject::GetDataHere**) with the [**CCF\_SNAPIN\_PRELOADS**](ccf-snapin-preloads.md) format.
...
When a console file is saved, MMC calls [**IDataObject::GetDataHere**](https://www.bing.com/search?q=**IDataObject::GetDataHere**) on each loaded snap-in using the CCF\_SNAPIN\_PRELOADS clipboard format. If the snap-in handles the [**CCF\_SNAPIN\_PRELOADS**](ccf-snapin-preloads.md) clipboard format and returns **TRUE**, MMC calls the [**IComponentData::Notify**](/windows/desktop/api/Mmc/nf-mmc-icomponentdata-notify) method of the snap-in with the **MMCN\_PRELOAD** notification when the snap-in's parent is first selected. If a preload is not requested, MMC does not load a snap-in until the snap-in's static node is selected. Until then MMC displays a cached icon and name for the snap-in's static node.
