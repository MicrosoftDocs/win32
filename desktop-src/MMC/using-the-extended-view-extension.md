...
-   The second way for a snap-in to provide values for the **CCF\_DESCRIPTION** and **CCF\_HTML\_DETAILS** properties is to implement the [**INodeProperties**](/windows/desktop/api/Mmc/nn-mmc-inodeproperties) interface. The **INodeProperties** interface contains a single method, [**GetProperty**](/windows/desktop/api/Mmc/nf-mmc-inodeproperties-getproperty), which is used to return text-only property values. The **INodeProperties** interface is queried (using IUnknown::QueryInterface) from the [**IComponentData**](/windows/desktop/api/Mmc/nn-mmc-icomponentdata) interface for scope nodes and from the [**IComponent**](/windows/desktop/api/Mmc/nn-mmc-icomponent) interface for result items.
