...
The [**IHeaderCtrl2**](/windows/desktop/api/Mmc/nn-mmc-iheaderctrl2) interface contains three methods that can be used to modify the column configuration data of a scope item's column set:

-   [**IHeaderCtrl2::InsertColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::InsertColumn**)
-   [**IHeaderCtrl2::DeleteColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::DeleteColumn**)
-   [**IHeaderCtrl2::SetColumnWidth**](https://www.bing.com/search?q=**IHeaderCtrl2::SetColumnWidth**)

MMC does not persist in memory any changes made to a column set due to the action of [**IHeaderCtrl2::InsertColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::InsertColumn**), [**IHeaderCtrl2::DeleteColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::DeleteColumn**), or [**IHeaderCtrl2::SetColumnWidth**](https://www.bing.com/search?q=**IHeaderCtrl2::SetColumnWidth**), so snap-ins must update persisted column configuration data after using these methods.
...
-   **Scenario 2:** Suppose the user selects a scope item for which MMC persists column configuration data. The user does not customize its list view. The snap-in does not make any changes either. The user then selects a different item. The user then reselects the original item. In the meantime, the data source for the original item has changed in some way, and as a result, the snap-in inserts an extra column into the list view of the original item using [**IHeaderCtrl2::InsertColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::InsertColumn**). When [applying the persisted data to the list view](https://www.bing.com/search?q=applying the persisted data to the list view), MMC notices that the number of new columns does not match the number of columns persisted in memory. As a result, MMC throws away the persisted data and does not apply it to the column set.

In Scenario 2, the snap-in does not persist the new column configuration data after calling [**IHeaderCtrl2::InsertColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::InsertColumn**), and consequently MMC ignores the outdated data persisted in memory when the list view is displayed.
...
-   Immediately after making changes to a column set using the [**IHeaderCtrl2**](/windows/desktop/api/Mmc/nn-mmc-iheaderctrl2) interface, snap-ins can call [**IColumnData::SetColumnConfigData**](/windows/desktop/api/Mmc/nf-mmc-icolumndata-setcolumnconfigdata) to update the column set's persisted column configuration data.
...
-   When handling an [**MMCN\_EXPAND**](mmcn-expand.md) notification, a snap-in can use the methods of [**IColumnData**](/windows/desktop/api/Mmc/nn-mmc-icolumndata) to examine persisted column configuration data for the selected scope item's list view and modify it if required.
