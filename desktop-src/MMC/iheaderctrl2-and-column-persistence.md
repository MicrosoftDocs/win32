...
The [**IHeaderCtrl2**](/windows/win32/Mmc/nn-mmc-iheaderctrl2?branch=master) interface contains three methods that can be used to modify the column configuration data of a scope item's column set:
...
-   Immediately after making changes to a column set using the [**IHeaderCtrl2**](/windows/win32/Mmc/nn-mmc-iheaderctrl2?branch=master) interface, snap-ins can call [**IColumnData::SetColumnConfigData**](/windows/win32/Mmc/nf-mmc-icolumndata-setcolumnconfigdata?branch=master) to update the column set's persisted column configuration data.
...
-   When handling an [**MMCN\_EXPAND**](mmcn-expand.md) notification, a snap-in can use the methods of [**IColumnData**](/windows/win32/Mmc/nn-mmc-icolumndata?branch=master) to examine persisted column configuration data for the selected scope item's list view and modify it if required.
