...
The **MMCN\_FILTER\_CHANGE** notification message is sent to the snap-in's [**IComponent**](/windows/desktop/api/Mmc/nn-mmc-icomponent) implementation when the filter value for a filtered result view column has been changed.

MMC calls [**IComponent::Notify**](/windows/desktop/api/Mmc/nf-mmc-icomponent-notify) with the **MMCN\_FILTER\_CHANGE** notification as the *event* parameter when filtering has been enabled or disabled, or when the filter value of a particular column has changed.
...
Filter change code (see [**MMC\_FILTER\_CHANGE\_CODE**](/windows/desktop/api/Mmc/ne-mmc-_mmc_filter_change_code) enumeration).
