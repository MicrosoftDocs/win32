...
A primary snap-in has the opportunity to enable MMC standard verbs for scope or result items when its [**IComponent::Notify**](/windows/desktop/api/Mmc/nf-mmc-icomponent-notify) method is called with the [**MMCN\_SELECT**](mmcn-select.md) notification message. Be aware that MMC standard verbs are not to be confused with context menu items that snap-ins can add to the context menus of their scope and result items. For more information about context menus, see [Working with Context Menus](working-with-context-menus.md).

The [**MMC\_CONSOLE\_VERB**](/windows/desktop/api/Mmc/ne-mmc-_mmc_console_verb) enumeration defines the command identifiers available for standard verbs. The following verbs are allowed:
...
When a standard verb is enabled for an item and the user selects it, MMC calls the snap-in's [**IComponent::Notify**](/windows/desktop/api/Mmc/nf-mmc-icomponent-notify) method with the appropriate notification message. See the [**MMC\_CONSOLE\_VERB**](/windows/desktop/api/Mmc/ne-mmc-_mmc_console_verb) documentation in the MMC SDK for details.

Be aware that MMC does not send a notification message in the case of the MMC\_VERB\_PROPERTIES, MMC\_VERB\_COPY and MMC\_VERB\_CUT verbs. When the MMC\_VERB\_PROPERTIES verb is enabled and the user selects it, MMC calls the [**IExtendPropertySheet2::CreatePropertyPages**](https://www.bing.com/search?q=**IExtendPropertySheet2::CreatePropertyPages**) method of all snap-ins (primary and extension) that add property pages for the selected item. Be aware that primary snap-ins are responsible for enabling the MMC\_VERB\_PROPERTIES verb. Extension snap-ins cannot do this, because they do not own the item for which the verb is enabled.
...
As mentioned above, snap-ins enable standard verbs in their [**MMCN\_SELECT**](mmcn-select.md) notification handler. In the notification handler, snap-ins should call [**IConsole2::QueryConsoleVerb**](https://www.bing.com/search?q=**IConsole2::QueryConsoleVerb**) to request a pointer to MMC's [**IConsoleVerb**](/windows/desktop/api/Mmc/nn-mmc-iconsoleverb) interface implementation. Using the returned interface pointer, snap-ins can then call the [**IConsoleVerb::SetVerbState**](/windows/desktop/api/Mmc/nf-mmc-iconsoleverb-setverbstate) method to enable (or disable) the standard verbs, one call per standard verb. Snap-ins can also call the [**SetDefaultVerb**](/windows/desktop/api/Mmc/nf-mmc-iconsoleverb-setdefaultverb) method to set the default verb for an item. The default verb is indicated in bold type in the item's context menu.
