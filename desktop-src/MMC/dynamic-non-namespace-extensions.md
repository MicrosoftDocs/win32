...
For these types of extensions, add the clipboard format CCF\_MMC\_DYNAMIC\_EXTENSIONS to the snap-in's [**IDataObject**](https://www.bing.com/search?q=**IDataObject**) implementation for the items you want to extend. The CCF\_MMC\_DYNAMIC\_EXTENSIONS format uses the [**SMMCDynamicExtensions**](https://www.bing.com/search?q=**SMMCDynamicExtensions**) structure. This structure specifies the extension snap-ins you want to extend the item represented by the **IDataObject** object.
...
1.  Add the CCF\_MMC\_DYNAMIC\_EXTENSIONS clipboard format to the snap-in's [**IDataObject**](https://www.bing.com/search?q=**IDataObject**) implementation.

    Usually, [**IDataObject**](https://www.bing.com/search?q=**IDataObject**) has its supported clipboard formats as static members. If so, add the CCF\_MMC\_DYNAMIC\_EXTENSIONS format as a member:
...
2.  Handle the CCF\_MMC\_DYNAMIC\_EXTENSIONS format in the [**IDataObject::GetData**](https://www.bing.com/search?q=**IDataObject::GetData**) method.

    The following example handles the CCF\_MMC\_DYNAMIC\_EXTENSIONS format and returns an [**SMMCDynamicExtensions**](https://www.bing.com/search?q=**SMMCDynamicExtensions**) structure that contains an array of two GUIDs that are the CLSIDs of the two extension snap-ins (CLSID\_Snapin1 and CLSID\_Snapin2) that will extend the item:
