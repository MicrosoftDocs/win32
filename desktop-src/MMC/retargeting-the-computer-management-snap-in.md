...
4.  Your extension's [**IComponentData::Notify**](/windows/desktop/api/Mmc/nf-mmc-icomponentdata-notify) method receives an [**MMCN\_EXPAND**](mmcn-expand.md) notification.
...
6.  Your extension's IComponentData::Notify method receives an [**MMCN\_REMOVE\_CHILDREN**](mmcn-remove-children.md) notification. Your extension should destroy all resources for the subtree that your extension added under the System Tools scope item.
    ...
    MMC removes the items from the scope pane, so your snap-in is not required to call [**IConsoleNameSpace2::DeleteItem**](https://www.bing.com/search?q=**IConsoleNameSpace2::DeleteItem**). Be aware that your [**IComponentData**](/windows/desktop/api/Mmc/nn-mmc-icomponentdata) instance is not released — MMC uses this instance for the lifetime of the Computer Management console root node.
