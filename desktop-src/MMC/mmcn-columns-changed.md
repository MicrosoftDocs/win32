...
The **MMCN\_COLUMNS\_CHANGED** notification is sent to the snap-in's [**IComponent::Notify**](/windows/desktop/api/Mmc/nf-mmc-icomponent-notify) method when the user hides columns or makes columns visible in the list view using the **Modify Columns** dialog box.

The notification tells the snap-in which columns in the list view are visible. The *param* parameter passed to the snap-in in the [**IComponent::Notify**](/windows/desktop/api/Mmc/nf-mmc-icomponent-notify) method is a pointer to the [**MMC\_VISIBLE\_COLUMNS**](/windows/desktop/api/Mmc/ns-mmc-_mmc_visible_columns) structure that contains the visible columns data.
...
A pointer to an [**MMC\_VISIBLE\_COLUMNS**](/windows/desktop/api/Mmc/ns-mmc-_mmc_visible_columns) structure that contains the list of visible columns.
...
Be aware that the **MMCN\_COLUMNS\_CHANGED** notification is not generated for column width changes or sorted column and sort direction changes made by the user or by the snap-in (using the [**IColumnData::SetColumnConfigData**](/windows/desktop/api/Mmc/nf-mmc-icolumndata-setcolumnconfigdata) and [**IColumnData::SetColumnSortData**](/windows/desktop/api/Mmc/nf-mmc-icolumndata-setcolumnsortdata) methods).

Also, the **MMCN\_COLUMNS\_CHANGED** notification is not generated for any changes made by the snap-in using the [**IHeaderCtrl2::SetColumnWidth**](https://www.bing.com/search?q=**IHeaderCtrl2::SetColumnWidth**), [**IHeaderCtrl2::InsertColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::InsertColumn**), and [**IHeaderCtrl2::DeleteColumn**](https://www.bing.com/search?q=**IHeaderCtrl2::DeleteColumn**) methods.
