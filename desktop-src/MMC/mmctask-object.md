...
An MMCTask object is returned by the GetFirstTask and GetNextTask methods of the MMCCtrl control. The MMCTask object's properties represent the members of the MMC\_TASK structure passed in the [**IEnumTASK::Next**](/windows/desktop/api/Mmc/nf-mmc-ienumtask-next) method that MMC called when it retrieved the information for that task during the setup of the taskpad. The MMCTask object also has one additional property that represents the snap-in's CLSID.
