...
4.  In the source snap-in, handle the [**MMCN\_CUTORMOVE**](mmcn-cutormove.md) notification. MMC sends this notification after a successful paste in a cut/copy operation. The arg parameter passed through the notification contains the data object for the cut source item.
    ...
    Be aware that MMC does not delete a source item that has been cut by the user. If the user cuts a result item from a result pane, and the result pane is still visible after the cut, the snap-in should call [**IResultData::DeleteItem**](/windows/desktop/api/Mmc/nf-mmc-iresultdata-deleteitem) to remove the cut item. For a cut scope item, the snap-in should call the [**IConsoleNameSpace2::DeleteItem**](https://www.bing.com/search?q=**IConsoleNameSpace2::DeleteItem**) method to delete the scope item from the scope pane.
