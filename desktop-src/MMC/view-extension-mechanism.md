...
A view extension snap-in implements a single interface, [**IExtendView**](/windows/desktop/api/Mmc/nn-mmc-iextendview), which contains a single method, [**IExtendView::GetViews**](/windows/desktop/api/Mmc/nf-mmc-iextendview-getviews). When MMC calls the view extension's IExtendView::GetViews method, MMC provides a pointer to an [**IViewExtensionCallback**](/windows/desktop/api/Mmc/nn-mmc-iviewextensioncallback) interface. The view extension calls the [**IViewExtensionCallback::AddView**](/windows/desktop/api/Mmc/nf-mmc-iviewextensioncallback-addview) method to add a view to the node.

The view extension can call the IViewExtensionCallback::AddView method multiple times to add multiple views. The IViewExtensionCallback::AddView method uses a pointer to an [**MMC\_EXT\_VIEW\_DATA**](/windows/desktop/api/Mmc/ns-mmc-_mmc_ext_view_data) structure, which has the following definition.
