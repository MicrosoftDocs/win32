...
The CCF\_NODEID2 clipboard format replaces the [**CCF\_NODEID**](ccf-nodeid.md) format in MMC 1.2. The CCF\_NODEID2 format uses an [**SNodeID2**](/windows/desktop/api/Mmc/ns-mmc-_snodeid2) structure.
...
[**SNodeID2**](/windows/desktop/api/Mmc/ns-mmc-_snodeid2) structure. Be aware that returning cBytes=0 is illegal for CCF\_NODEID2. The [**CCF\_NODEID**](ccf-nodeid.md) format permitted SNodeID.cBytes to be zero to designate that the item should not be persisted. In the **SNodeID2** structure, this is indicated by setting SNodeID2.dwFlags to **MMC\_NODEID\_SLOW\_RETRIEVAL**.
