...
3.  If [**SetContact**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationviewport-setcontact?branch=master) is called from the IHT thread, messages might get sent to the UI thread by the delegate thread, depending on the value of [**DIRECTMANIPULATION\_HITTEST\_TYPE**](/windows/previous-versions/directmanipulation/ne-directmanipulation-directmanipulation_hittest_type?branch=master) when [**RegisterHitTestTarget**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationmanager-registerhittesttarget?branch=master) was called.
4.  If [**SetContact**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationviewport-setcontact?branch=master) is not called from the IHT thread, messages are always sent by the delegate thread to the UI thread.
5.  The client can call [**SetContact**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationviewport-setcontact?branch=master) from the UI thread to let [Direct Manipulation](direct-manipulation-portal.md) detect a manipulation.
...
In some cases, an application UI thread may be too slow to respond to hit testing. A hit test thread may be used ([**RegisterHitTestTarget**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationmanager-registerhittesttarget?branch=master)) to allow the client to move [**WM\_POINTERDOWN**](inputmsg.wm_pointerdown) and [**DM\_POINTERHITTEST**](inputmsg.dm_pointerhittest) messages to a specific thread to allow for hit testing.

## Remarks

Typically, [Direct Manipulation](direct-manipulation-portal.md) sends only [**WM\_POINTERDOWN**](inputmsg.wm_pointerdown) and [**DM\_POINTERHITTEST**](inputmsg.dm_pointerhittest) messages to the UI thread, withholding later messages while waiting for a response from the client. If the client calls [**SetContact**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationviewport-setcontact?branch=master), the only messages the UI thread receives when a manipulation is detected are **WM\_POINTERDOWN**,**DM\_POINTERHITTEST**, and [**WM\_POINTERCAPTURECHANGED**](inputmsg.wm_pointercapturechanged).

The client might not call [**SetContact**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationviewport-setcontact?branch=master) when processing [**WM\_POINTERDOWN**](inputmsg.wm_pointerdown) and [**DM\_POINTERHITTEST**](inputmsg.dm_pointerhittest) messages. In this case, [Direct Manipulation](direct-manipulation-portal.md) sends all messages to the UI thread without analyzing the messages to see if there is a manipulation. The client can then choose any point to call **SetContact** and have Direct Manipulation start detecting manipulations, and send [**WM\_POINTERCAPTURECHANGED**](inputmsg.wm_pointercapturechanged) messages when one is detected.

**Windows 10 and later:** You can decide which manipulations you want to handle by calling [**DeferContact**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationdefercontactservice-defercontact?branch=master) before calling [**SetContact**](/windows/previous-versions/DirectManipulation/nf-directmanipulation-idirectmanipulationviewport-setcontact?branch=master) on a [**WM\_POINTERDOWN**](inputmsg.wm_pointerdown) or [**DM\_POINTERHITTEST**](inputmsg.dm_pointerhittest) message. **DeferContact** ensures all subsequent messages are sent to the UI thread for the specified period of time.
