...
| [**IPortableDeviceService**](iportabledeviceservice.md)                                | Used to retrieve the **IPortableDeviceServiceMethods** interface to invoke methods on a given service.                                                                  |
| [**IPortableDeviceServiceMethods**](iportabledeviceservicemethods.md)                  | Used to invoke a service method.                                                                                                                                        |
...
Service methods encapsulate functionality that each service defines and implements. They are unique to each type of service and are represented by a GUID. For example, the Contacts service defines a **BeginSync** method that applications call to prepare the device for synchronizing Contact objects, and an **EndSync** method to notify the device that synchronization has completed. Applications execute a portable device service method by calling [**IPortableDeviceServiceMethods::Invoke**](iportabledeviceservicemethods-invoke.md).

Service methods should not be confused with WPD commands. WPD commands are part of the standard WPD Device Driver Interface (DDI), and are the mechanism for communication between a WPD application and the driver. Commands are predefined, grouped by categories (for example, **WPD\_CATEGORY\_COMMON**), and are represented by a **PROPERTYKEY** structure. An application sends commands to the device driver by calling [**IPortableDeviceService::SendCommand**](iportabledevice-sendcommand.md). For more information, see the Commands topic.

The **InvokeMethodsAsync** method invokes [**IPortableDeviceService::Methods**](iportabledeviceservice-capabilities.md) to retrieve an [**IPortableDeviceServiceMethods**](iportabledeviceservicecapabilities.md) interface. Using this interface, it invokes the **InvokeMethodAsync** helper function twice; once for the **BeginSync** method and once for the **EndSync** method. In this example, , **InvokeMethodAsync** waits indefinitely for a global event to be signaled when [**IPortableDeviceServiceMethodCallback::OnComplete**](iportabledeviceservicemethodcallback-oncomplete.md) is called.
...
-   Creates a **CMethodCallback** object that is supplied as an argument to [**IPortableDeviceServiceMethods:InvokeAsync**](iportabledeviceservicemethods-invokeasync.md).
...
[**IPortableDeviceServiceMethodCallback**](iportabledeviceservicemethodcallback.md)
</dt> <dt>

[**IPortableDeviceServiceMethods**](iportabledeviceservicemethods.md)
</dt> <dt>
