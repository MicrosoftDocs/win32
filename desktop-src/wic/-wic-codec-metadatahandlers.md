...
When getting a metadata item by ID, you should use [PropVariantChangeType Function](https://msdn.microsoft.com/en-us/library/Bb776514(VS.85).aspx) to coerce the ID into the expected type. For example, the IFD reader will coerce an ID to type `VT_UI2` to coincide with the data type of an IFD tag ID USHORT. The input type and expected type must both be [PROPVARIANT](https://msdn.microsoft.com/en-us/library/Aa380072(VS.85).aspx) to do this. This is not required, but doing this coercion simplifies code that calls the reader to query for metadata items.
...
It is the metadata writer's responsibility to convert the [PROPVARIANT](https://msdn.microsoft.com/en-us/library/Aa380072(VS.85).aspx) items to the underlying structure required by your format. However, unlike the metadata reader, VARIANT types should not normally be coerced to different types as the caller is specifically indicating what data type to use.
...
All metadata handlers must be digitally signed to participate in the WIC discovery process. WIC will not load any handler that is not signed by a trusted certificate authority. For more information on digital signing, see [Introduction to Code Signing](https://msdn.microsoft.com/en-us/library/ms537361(VS.85).aspx).
...
To avoid redundancy in representing array items, do not use safe arrays; use only simple arrays. This reduces the work an application needs to perform when interpreting [PROPVARIANT](https://msdn.microsoft.com/en-us/library/Aa380072(VS.85).aspx) types.
...
Before using a [PROPVARIANT](https://msdn.microsoft.com/en-us/library/Aa380072(VS.85).aspx), always call **PropVariantInit** to initialize the value. When you are finished with the PROPVARIANT, always call **PropVariantClear** to release any memory allocated for the variable.
