...
The workstation can be unlocked by the user or an administrator. To unlock the system, press Ctrl+Alt+Del and log in. To receive notification when the user logs in, use the [**WTSRegisterSessionNotification**](https://msdn.microsoft.com/en-us/library/Aa383841(v=VS.85).aspx) function to register to receive [**WM\_WTSSESSION\_CHANGE**](https://msdn.microsoft.com/en-us/library/Aa383828(v=VS.85).aspx) messages. When this message is received, check whether the *wParam* parameter is equal to WTS\_SESSION\_LOCK.
