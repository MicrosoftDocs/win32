...
An [IUnknown](https://www.bing.com/search?q=IUnknown) pointer passed in by the MCSVC. To obtain a supported monitor control interface, the monitor must call [IUnknown::QueryInterface](https://www.bing.com/search?q=IUnknown::QueryInterface) on the pointer.
...
If the method is unsuccessful, the return value is an error code. On error, the MCSVC will not create the monitor or call [IUnknown::Release](https://www.bing.com/search?q=IUnknown::Release) on the interface pointer.
