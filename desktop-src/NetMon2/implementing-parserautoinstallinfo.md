...
> [!Note]  
> Network Monitor keeps a list of existing parsers in the [*Parser.ini*](p.md#-netmon-parser-ini-gly) file, and creates a separate INI file for each installed parser.
...
> [!Note]  
> Network Monitor uses the preceding and following parser protocol information to update the [*handoff sets*](h.md#-netmon-handoff-set-gly) and [*follow sets*](f.md#-netmon-follow-set-gly) of parsers that your parser DLL identifies.
...
6.  Specify the protocols that precede each DLL protocol. One of the following conditions applies to an incoming handoff set.
    -   If the preceding protocols can determine that your protocol follows from data in the preceding protocols, set the **pWhoHandsOffToMe** member of [**PF\_PARSERINFO**](pf-parserinfo.md). In this case, your protocol is then added to the [*handoff sets*](h.md#-netmon-handoff-set-gly) of the preceding protocols.
    -   If the preceding protocols cannot determine that your protocol follows from data in the preceding protocols, set **pWhoCanPrecedeMe** member of [**PF\_PARSERINFO**](pf-parserinfo.md). In this case, the your protocol is then added to the [*follow sets*](f.md#-netmon-follow-set-gly) of the protocols.
7.  Specify the protocols that follow each DLL protocol. One of the following conditions applies to an outgoing follow-set.
    -   If your protocol can determine which protocols follow based on data in your protocol, set the **pWhoDoIHandOffTo** member of [**PF\_PARSERINFO**](pf-parserinfo.md). In this case, the these protocols are added to the [*handoff set*](h.md#-netmon-handoff-set-gly) of your protocols.
    -   If your protocol cannot determine which protocols follow based on data in your protocol, set the **pWhoCanFollowMe** member of [**PF\_PARSERINFO**](pf-parserinfo.md). In this case, these protocols are added to the [*follow set*](f.md#-netmon-follow-set-gly) of your protocol.
