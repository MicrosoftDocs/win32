...
The **AttachPropertyInstanceEx** and **AttachPropertyInstance** functions are called only for the properties that exist in the recognized data. Note that Network Monitor has a [*property database*](https://www.bing.com/search?q=*property database*) for the parser that contains a description of all the properties that the parser supports.
