...
You can use the same DLL in both load-time and run-time dynamic linking. The following example uses the [**LoadLibrary**](loadlibrary.md) function to get a handle to the Myputs DLL (see [Creating a Simple Dynamic-Link Library](creating-a-simple-dynamic-link-library.md)). If **LoadLibrary** succeeds, the program uses the returned handle in the [**GetProcAddress**](getprocaddress.md) function to get the address of the DLL's myPuts function. After calling the DLL function, the program calls the [**FreeLibrary**](freelibrary.md) function to unload the DLL.
