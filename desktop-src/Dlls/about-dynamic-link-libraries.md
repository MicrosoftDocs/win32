...
-   In *run-time dynamic linking*, a module uses the [**LoadLibrary**](https://www.bing.com/search?q=**LoadLibrary**) or [**LoadLibraryEx**](/windows/desktop/api/LibLoaderAPI/nf-libloaderapi-loadlibraryexa) function to load the DLL at run time. After the DLL is loaded, the module calls the [**GetProcAddress**](https://www.bing.com/search?q=**GetProcAddress**) function to get the addresses of the exported DLL functions. The module calls the exported DLL functions using the function pointers returned by **GetProcAddress**. This eliminates the need for an import library.
