...
1.  Declare some variables that are needed.
    ...
    Declare a **DWORD** variable `dwRetval` that will be using for error checking function calls. Declare a pointer to an [**MIB\_IPSTATS**](rras.mib_ipstats) variable called *pStats*, and allocate memory for the structure. Check that memory could be allocated.
...
4.  Free the memory allocated for the [**MIB\_IPSTATS**](rras.mib_ipstats) structure pointed to by the *pStats* parameter. This should be done once the application no longer needs the data returned by the *pStats* parameter.
    ```C++
    if (pStats)
        free(pStats);
    ```
