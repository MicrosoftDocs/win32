...
Function Discovery providers are used internally by Function Discovery to perform the discovery process. Clients of Function Discovery normally use the [**IFunctionDiscovery**](ifunctiondiscovery.md) interface, which delegates queries to the installed provider objects. You can write a provider for new base-level resources similar to Plug and Play (PnP) instances or Simple Service Discovery Protocol (SSDP) network instances. Service providers can expose new functionality for function instances through the Function Discovery API.
