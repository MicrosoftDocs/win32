...
Each PnP-X device has an associated [function instance](function-objects.md). A list of PnP-X function instances is stored in the *PnP-X association database*. If a device has an associated entry marked in the association database and the device is currently online (on the network), the PnP [*devnode*](function-discovery-glossary.md#devnode-ncd-gly) for the device is marked *present* on the system, and the associated device is available for use. If a device does not have an associated entry marked in the association database, no PnP *devnode* will exist on the system and the device is not available for use.
