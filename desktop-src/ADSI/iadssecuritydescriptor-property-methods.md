...
The property methods of the [**IADsSecurityDescriptor**](/windows/desktop/api/Iads/nn-iads-iadssecuritydescriptor) interface get or set the properties described in the following table. For more information, see [Interface Property Methods](interface-property-methods.md).
...
Flags that qualify the meaning of the security descriptor. Values are taken from the Win32 [**SECURITY\_DESCRIPTOR\_CONTROL**](https://msdn.microsoft.com/library/windows/desktop/aa379566) structure.
...
Discretionary access-control list (DACL) that specifies the types of access granted to the object for specified users and groups. For more information about DACLs, see [Null DACLs and Empty DACLs](https://msdn.microsoft.com/library/ms677610).
...
Revision level of the security descriptor. This value is taken from the Win32 [**ACL\_REVISION\_INFORMATION**](https://msdn.microsoft.com/library/windows/desktop/aa374942) structure. All ACEs in an ACL must be at the same revision level.
...
A flag of the BOOL type that indicates that the SACL is derived from a default mechanism, rather than being explicitly provided by the original provider of the security descriptor. This flag can affect how the system handles the SACL, with respect to ACE inheritance. The system ignores this flag if the SE\_SACL\_PRESENT flag is not set.
