...
1.  First, open the [SQL Query Analyzer](http://go.microsoft.com/fwlink/p/?linkid=83913) on the SQL Server that is linked to the directory service (see Creating a Linked Server).
...
To set up a distributed join on a Windows 2000 Server directory service, create a linked server. To do this, set up ADSI mapping by using the [sp\_addlinkedserver](http://go.microsoft.com/fwlink/p/?linkid=83926) System Stored Procedure. This procedure links a name to an OLE DB Provider name.

In the following example, note that there are several arguments used with the [sp\_addlinkedserver](http://go.microsoft.com/fwlink/p/?linkid=83926) System Stored Procedure:
...
The [EXEC](http://go.microsoft.com/fwlink/p/?linkid=83921) command is used to execute System Stored Procedures.
...
For Windows-authenticated logins, the self-mapping is sufficient to access the directory with SQL Server Security Delegation. Because the self-mapping is created by default for linked servers created through [sp\_addlinkedserver](http://go.microsoft.com/fwlink/p/?linkid=83926), no other login mapping is necessary.

For SQL Server–authenticated logins, you can configure suitable logins and passwords for connecting to the directory service by using the [sp\_addlinkedsrvlogin](http://go.microsoft.com/fwlink/p/?linkid=83925) System Stored Procedure.
...
If you prefer to use a SQL Server–authenticated login rather than Windows Authentication, add a login to the linked server (see the previous section). To do this, use the [sp\_addlinkedsrvlogin](http://go.microsoft.com/fwlink/p/?linkid=83925) System Stored Procedure.

In the following example, there are several arguments that are used with the [sp\_addlinkedsrvlogin](http://go.microsoft.com/fwlink/p/?linkid=83925) System Stored Procedure:
...
After you have created a linked server, use an [OPENQUERY](http://go.microsoft.com/fwlink/p/?linkid=83923) statement to send a query to the Directory Service. The following SQL query creates a virtual table to hold your query results by using the [CREATE VIEW](http://go.microsoft.com/fwlink/p/?linkid=83920) statement. The view that is created is named "viewADContacts".

The first [SELECT](http://go.microsoft.com/fwlink/p/?linkid=83924) statement defines the information that is being queried from the directory service and maps it to a column in the table. Information surrounded by brackets indicates the data that is put into the virtual table. The information that is not in brackets indicates the data that is retrieved from the directory service. Notice that the information that is being retrieved from the directory service must be referenced by its LDAP display name.

The next statement is the [OPENQUERY](http://go.microsoft.com/fwlink/p/?linkid=83923) statement. This statement has two arguments: ADSI, which is the name of the linked server that you created, and a query statement. The query statement contains the following items:
...
-   The [WHERE](http://go.microsoft.com/fwlink/p/?linkid=83927) statement provides the search conditions. In this example, it is searching for contacts.

The final [SELECT](http://go.microsoft.com/fwlink/p/?linkid=83924) statement is used to pick up results from the view to display.
