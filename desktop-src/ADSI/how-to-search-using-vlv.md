...
keywords: ["How to Search Using VLV"]
---
...
To perform a VLV search, instruct ADSI to use the VLV control. To do this, call the [**IDirectorySearch::SetSearchPreference**](idirectorysearch-setsearchpreference.md) method with an **ADS\_SEARCHPREF\_VLV** search option with an **ADSTYPE\_PROV\_SPECIFIC** value. The **ADSTYPE\_PROV\_SPECIFIC** value is a pointer to an [**ADS\_VLV**](ads-vlv.md) structure that contains data about the search. The [**GetVLVItemCount**](example-code-for-using-a-vlv-search.md#getvlvitemcount-example-function) example function shows how to set both of these search preferences.
...
1.  Fill an [**ADS\_VLV**](ads-vlv.md) structure with all zero or **NULL** values.
2.  Fill an [**ADS\_SEARCHPREF\_INFO**](ads-searchpref-info.md) with the following values.

    -   Set the **dwSearchPref** member to **ADS\_SEARCHPREF\_VLV**.
    -   Set the **vValue.dwType** member to **ADSTYPE\_PROV\_SPECIFIC**.
    -   Set the **vValue.ProviderSpecific.dwLength** member to the size of the [**ADS\_VLV**](ads-vlv.md) structure.
    -   Set the **vValue.ProviderSpecific.lpValue** member to the address of the [**ADS\_VLV**](ads-vlv.md) structure from Step 1.

3.  Fill an [**ADS\_SORTKEY**](ads-sortkey.md) structure as shown in [Sorting the Search Results with IDirectorySearch](sorting-the-search-results-with-idirectorysearch.md) to sort on the desired attribute.
4.  Fill another [**ADS\_SEARCHPREF\_INFO**](ads-searchpref-info.md) to add the [**ADS\_SORTKEY**](ads-sortkey.md) structure to the search preferences as shown in [Sorting the Search Results with IDirectorySearch](sorting-the-search-results-with-idirectorysearch.md).
5.  Add any other desired search preferences and call [**IDirectorySearch::SetSearchPreference**](idirectorysearch-setsearchpreference.md) to set the search preferences.
6.  Perform the search with [**IDirectorySearch::ExecuteSearch**](idirectorysearch-executesearch.md).
7.  Get the first row of results by calling [**IDirectorySearch::GetFirstRow**](idirectorysearch-getfirstrow.md).
8.  Call [**IDirectorySearch::GetColumn**](idirectorysearch-getcolumn.md) with **ADS\_VLV\_RESPONSE** to obtain the VLV search metadata.
9.  Cast the **pADsValues-&gt;ProviderSpecific.lpValue** of the [**ADS\_SEARCH\_COLUMN**](ads-search-column.md) structure to an [**ADS\_VLV**](ads-vlv.md) structure pointer. The **dwContentCount** member of this **ADS\_VLV** structure contains the approximate number of items that would be returned by a search of that type.
...
>
> 1.  Fill an [**ADS\_VLV**](ads-vlv.md) structure with the following values. Additional members of the structure should be set to zero or **NULL**.
>
...
> 3.  Perform the search with [**IDirectorySearch::ExecuteSearch**](idirectorysearch-executesearch.md).
> 4.  Get the first row of results by calling [**IDirectorySearch::GetFirstRow**](idirectorysearch-getfirstrow.md).
> 5.  Call [**IDirectorySearch::GetColumn**](idirectorysearch-getcolumn.md) with the name of the attribute to retrieve to get the actual data for the requested item.
> 6.  Call [**IDirectorySearch::GetColumn**](idirectorysearch-getcolumn.md) with **ADS\_VLV\_RESPONSE** to obtain the VLV search metadata.
> 7.  Cast the **pADsValues-&gt;ProviderSpecific.lpValue** of the [**ADS\_SEARCH\_COLUMN**](ads-search-column.md) structure to an [**ADS\_VLV**](ads-vlv.md) structure pointer.
...
Similarly to searching by index, it is also possible to retrieve more than one row of data with a single search call. This is accomplished in the same manner by setting the **dwBeforeCount** and **dwAfterCount** members of the [**ADS\_VLV**](ads-vlv.md) structure appropriately.
