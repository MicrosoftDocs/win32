...
For an application to receive Teredo traffic, the application must be permitted to receive IPv6 traffic in the host firewall, and the application is required to set the socket option [IPV6\_PROTECTION\_LEVEL](https://msdn.microsoft.com/library/windows/desktop/aa832668) to 'PROTECTION\_LEVEL\_UNRESTRICTED'. To enable this type of scenario, the firewall exceptions detailed in this document must be implemented.
...
-   The firewall must retrieve dynamic UDP ports used by Teredo service on the local machine by calling the [**FwpmSystemPortsGet0**](https://msdn.microsoft.com/library/windows/desktop/dd744944) function; relevant ports are of type FWPM\_SYSTEM\_PORT\_TEREDO. The **FwpmSystemPortsGet0** function should be implemented in place of the now deprecated [**GetTeredoPort**](https://msdn.microsoft.com/library/windows/desktop/aa814426) or [**NotifyTeredoPortChange**](https://msdn.microsoft.com/library/windows/desktop/aa814453) functions.
