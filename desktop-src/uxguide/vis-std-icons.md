...
The standard icons are notable because they are built into many Windows application programming interfaces (APIs), such as [task dialogs](win-dialog-box.md), [message boxes](https://www.bing.com/search?q=message boxes), [balloons](ctrl-balloons.md), and [notifications](mess-notif.md). They are also commonly used on [in-place messages](https://www.bing.com/search?q=in-place messages) and [status bars](ctrl-status-bars.md).
...
-   Make sure that the issue warrants heightened user attention. [Routine confirmations](https://www.bing.com/search?q=Routine confirmations) and questions shouldn't have warning icons.
...
| Notifications\[newline\]     | use for all warnings. (for [non-critical system events](https://www.bing.com/search?q=non-critical system events).) \[newline\] ![screen shot of low-battery warning notification ](images/vis-std-icons-image14.png)<br/> |
...
-   **Use the question mark icon only for Help entry points.** For more information, see the [Help entry point](https://www.bing.com/search?q=Help entry point) guidelines.
