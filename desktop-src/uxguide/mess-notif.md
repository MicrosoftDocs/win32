...
-   **Is the information the immediate, direct result of users' interaction with your application?** If so, display this synchronous information directly within your application instead using a [dialog box](win-dialog-box.md), [message box](https://www.bing.com/search?q=message box), [balloon](ctrl-balloons.md), or [in place](https://www.bing.com/search?q=in place) UI. Notifications are for asynchronous information only.
...
-   **Are the primary target users IT professionals?** If so, use an alternative feedback mechanism such as [log file](https://www.bing.com/search?q=log file) entries or e-mail messages. IT professionals strongly prefer log files for non-critical information. Furthermore, servers are often managed remotely and typically run without any users logged on, making notifications ineffective.
...
Don't assume that if notifications are [modeless](https://www.bing.com/search?q=modeless) a they can't be an annoying interruption. Notifications don't demand users' attention, but they certainly request it. You can break users flow by:
...
-   **Put double quotation marks around object names.**
    -   **Exception:** Don't use quotation marks when:
        -   The object name always uses [title-style capitalization](https://www.bing.com/search?q=title-style capitalization), such as with user names.
