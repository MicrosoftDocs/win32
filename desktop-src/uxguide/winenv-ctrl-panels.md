...
Control panel items are implemented using [task flows](https://www.bing.com/search?q=task flows) or property sheets. For Windows Vista and later, task flows are the preferred user interface (UI).
...
-   **To navigate between pages:**
    -   Use [task links](https://www.bing.com/search?q=task links) to start a task.
...
-   You can put task links and buttons in the following places (in order of discoverability):
    -   The [command area](https://www.bing.com/search?q=command area) (for command buttons on spoke pages only).
    -   The [content area](https://www.bing.com/search?q=content area):
        -   Command buttons
        -   Task links
        -   Other links
    -   Links in the [task pane](https://www.bing.com/search?q=task pane) (hub pages only).
...
-   To prompt users for an administrator user name and password. Always use the credential manager dialog box for this purpose. (Should be [modal](https://www.bing.com/search?q=modal).)
...
-   To perform secondary, stand-alone tasks. Using a [modeless](https://www.bing.com/search?q=modeless), secondary window allows such tasks to be performed independently and outside the main task flow.
...
-   **Don't put commit buttons on hub pages.** Hub pages are fundamentally launch points. Users never "commit" hub pages they are never done with them. And commit buttons on hub pages make any tasks initiated from a hub confusing (users will wonder if those tasks need to be committed).
    -   **Exception:** If changing a setting requires [elevation](https://www.bing.com/search?q=elevation), provide an Apply button with a [security shield icon](https://www.bing.com/search?q=security shield icon). Disable the commit button once changes have been applied.
...
-   **Confirm in-place commands whose effects cannot be easily undone.** Use a [task dialog](win-dialog-box.md) or [message box](https://www.bing.com/search?q=message box).
...
-   **Use commit buttons to complete a task.** Use a [delayed commit model](https://www.bing.com/search?q=delayed commit model) for spoke pages, so that changes aren't made until explicitly committed (if users navigate away using Back, Close, or the Address bar, changes are abandoned). The commit buttons are a visual clue that the user is about to complete a task. Don't use links for this purpose.
...
-   **Make sure the commit buttons appear on the page without scrolling.** If the page is long, you can make commit buttons always visible by placing them in a [command area](https://www.bing.com/search?q=command area), instead of in the content area.
...
-   **You can use Preview buttons on any spoke page.** Hub pages don't need Preview buttons because they use an [immediate commit model](https://www.bing.com/search?q=immediate commit model).
...
-   **Use Apply buttons only on final spoke pages.** Apply buttons should not be used on intermediate spoke pages to maintain an immediate commit model.
    -   **Exception:** You can use Apply buttons on a hybrid hub page if changing a setting requires [elevation](https://www.bing.com/search?q=elevation). For more details, see [hub page interaction](#hub-pages).
...
**Many settings require administrator privileges to change.** If a process requires administrator privileges, Windows Vista and later requires [Standard users](https://www.bing.com/search?q=Standard users) and [Protected administrators](https://www.bing.com/search?q=Protected administrators) to elevate their privileges explicitly. Doing so helps prevent malicious code from running with administrator privileges.
...
**Note:** As with all Explorer windows, control panel page titles are displayed on the [address bar](https://www.bing.com/search?q=address bar), but not the title bar.
...
-   Use complete sentences and [sentence-style capitalization](https://www.bing.com/search?q=sentence-style capitalization).
