...
The Search button is visually connected to a Search box. To minimize space, an optional [prompt](https://www.bing.com/search?q=prompt) is used inside a Search box instead of a label. The prompt may be an instruction (for example, Type to search) or indicate the scope of the search (for example, Search for pictures).
