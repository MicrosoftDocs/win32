...
-   **Is the purpose to present a collection of commands or tools?** If so, use a toolbar or [palette window](https://www.bing.com/search?q=palette window).
...
-   **Is the purpose to perform a multi-step task that requires more than a single window?** If so, use a [task flow](https://www.bing.com/search?q=task flow) or [wizard](win-wizards.md) instead.
...
-   Use a [delayed commit model](https://www.bing.com/search?q=delayed commit model) so that changes don't take effect until explicitly committed.
...
-   Use an [immediate commit model](https://www.bing.com/search?q=immediate commit model) so that changes take effect immediately.
...
**Developers:** You can create multi-page task dialogs using the [TDM\_NAVIGATE\_PAGE](http://msdn2.microsoft.com/library/aa969766.aspx) message.
...
-   **Dialog boxes don't have title bar icons.** Title bar icons are used as a visual distinction between [primary windows](https://www.bing.com/search?q=primary windows) and [secondary windows](https://www.bing.com/search?q=secondary windows).
...
**Don't use a notification for completion feedback.** Use either a progress dialog or an [action success notification](https://www.bing.com/search?q=action success notification), but not both.
...
-   **Prefer specific responses to Yes and No buttons.** While there's nothing wrong with using Yes and No, specific responses can be understood more quickly, resulting in efficient decision making. However, [confirmations](mess-confirm.md) usually have Yes and No buttons to make users give the confirmation [some thought](https://www.bing.com/search?q=some thought) before responding.
...
-   When providing user assistance, consider the following options (listed in their order of preference):
    ...
    -   Provide in-context explanations using [static text labels](https://www.bing.com/search?q=static text labels).
...
-   Use sentence-style capitalization.
-   Don't use ending punctuation.
-   Assign a unique [access key](https://www.bing.com/search?q=access key).
