...
-   First, assign access keys to the [commit buttons](https://www.bing.com/search?q=commit buttons) and command links. Use the standard access key assignments table when it applies, otherwise use the first letter of the first word.
...
-   **Don't distinguish between key locations.** For example, Windows can distinguish between left and right Shift, Alt, Ctrl, [Windows logo](https://www.bing.com/search?q=Windows logo), and [Application keys](https://www.bing.com/search?q=Application keys), as well as keys on the numeric keypad. Assigning behavior to only one key location is confusing and unexpected.
