...
| [**SNMPAPI_CALLBACK**](/windows/desktop/api/Winsnmp/nc-winsnmp-snmpapi_callback) | Notifies a WinSNMP session that an SNMP message or asynchronous event is available. **Note:** This callback function applies only to sessions opened as a result of a call to the [**SnmpCreateSession**](/windows/desktop/api/Winsnmp/nf-winsnmp-snmpcreatesession) function.<br> | 
