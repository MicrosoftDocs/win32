...
The implementation identifies the default retransmission mode in a return value from the [**SnmpStartup**](snmpstartup.md) function during initialization. The mode can be one of the following values.
...
A WinSNMP application can retrieve at any time the current retransmission mode in effect for the implementation by calling the [**SnmpGetRetransmitMode**](snmpgetretransmitmode.md) function. The WinSNMP API provides other [database functions](winsnmp-functions.md#winsnmp-database-functions) that simplify management of the retransmission policy.
...
-   Request that the implementation start or stop executing the retransmission policy by calling the [**SnmpSetRetransmitMode**](snmpsetretransmitmode.md) function. For more information, see [Turning Retransmission On and Off](turning-retransmission-on-and-off.md).
...
-   Call the [**SnmpCancelMsg**](snmpcancelmsg.md) function to cancel the retransmission cycle and release internal data structures associated with a single SNMP message request. For more information, see [Canceling Retransmission](canceling-retransmission.md).
