...
A WinSNMP application can call the [**SnmpCreatePdu**](snmpcreatepdu.md) function or the [**SnmpSetPduData**](snmpsetpdudata.md) function to assign an application-generated request identifier to a PDU. The application must pass the value in the *request\_id* parameter.

An application can request that the Microsoft WinSNMP implementation generate and assign a request identifier to a PDU by passing zero in the *request\_id* parameter of the [**SnmpCreatePdu**](snmpcreatepdu.md) function. The application can retrieve the assigned request identifier with a call to the [**SnmpGetPduData**](snmpgetpdudata.md) function.

To assign a request identifier equal to a specific value, including zero, the application must pass that value in the *request\_id* parameter of the [**SnmpSetPduData**](snmpsetpdudata.md) function.
