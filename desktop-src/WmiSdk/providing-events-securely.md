...
-   Using access control by implementing [**IWbemEventProviderSecurity::AccessCheck**](iwbemeventprovidersecurity.md) is the most efficient way.
...
-   Implementing [**IWbemEventSink::SetSinkSecurity**](iwbemeventsink-setsinksecurity.md) on the sink used to raise events allows the setting of a security descriptor (SD) on a sink for all the events passing through.

    WMI performs access checks based on the SD. Use this mode when the provider cannot make the decision regarding who is allowed to consume its events, but can decide on an SD for a specific sink. For example, use [**IWbemEventSink::SetSinkSecurity**](iwbemeventsink-setsinksecurity.md) if your event provider obtained several sinks by calls to [**IWbemEventSink::GetRestrictedSink**](iwbemeventsink-getrestrictedsink.md) and you want a security descriptor for each sink.
...
Decoupled event providers differ from nondecoupled event providers in the way that they register with WMI. The call to [**IWbemEventProviderSecurity::AccessCheck**](iwbemeventprovidersecurity-accesscheck.md) for events from a decoupled provider never propagates the client access token. WMI handles the access control in the same manner as for nondecoupled event providers. For more information about writing a decoupled provider, see [Incorporating a Provider in an Application](incorporating-a-provider-in-an-application.md).
