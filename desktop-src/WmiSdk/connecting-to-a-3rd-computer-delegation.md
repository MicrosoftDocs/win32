...
<span id="C__"></span><span id="c__"></span>C++
    ...
    Set the impersonation level parameter to **RPC\_C\_IMP\_LEVEL\_DELEGATE** in the call to [**CoInitializeSecurity**](_com_coinitializesecurity) or [**CoSetProxyBlanket**](_com_cosetproxyblanket). For more information about when to make these calls, see [Initializing COM for a WMI Application](initializing-com-for-a-wmi-application.md).

    To pass the client identity to remote COM servers in C++, set cloaking in the call to [**CoSetProxyBlanket**](_com_cosetproxyblanket). For more information, see [Cloaking](_com_cloaking).
