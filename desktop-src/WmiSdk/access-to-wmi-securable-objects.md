...
WMI relies on standard Windows [*security descriptors*](https://docs.microsoft.com/windows/desktop/SecGloss/s-gly) to control and protect access to securable objects like WMI namespaces, printers, services, and DCOM applications. For more information, see [Access to WMI Namespaces](access-to-wmi-namespaces.md).
...
WMI maintains access security by comparing the [*access token*](https://docs.microsoft.com/windows/desktop/SecGloss/a-gly) of the user that attempts to access a securable object with the security descriptor of the object.

When a user or group is created on a system, the account is given a [*security identifier (SID)*](https://docs.microsoft.com/windows/desktop/SecGloss/s-gly) The SID ensures that an account created with the same name as a previously deleted account does not inherit the previous security settings. An access token is created by combining the SID, the list of groups of which the user is a member, and the list of enabled or disabled privileges. These tokens are assigned to all processes and threads owned by the user.

## Access Control

When the user wants to use a secured object, the access token is compared with the [*discretionary access control list (DACL)*](https://docs.microsoft.com/windows/desktop/SecGloss/d-gly) in the security descriptor on the object. The DACL contains permissions called [*access control entries (ACE)*](https://docs.microsoft.com/windows/desktop/SecGloss/a-gly). [*System access control lists (SACL)*](https://docs.microsoft.com/windows/desktop/SecGloss/s-gly) do the same thing as DACLs, but can generate security audit events. Starting with Windows Vista, WMI can make auditing entries in the Windows Security Log. For more information about auditing in WMI, see [Access to WMI Namespaces](access-to-wmi-namespaces.md).
