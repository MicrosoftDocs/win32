...
After completion of the **ExecMethodAsync\_** method, the [Err](https://msdn.microsoft.com/library/sbf5ze0e.aspx) object may contain one of the error codes in the following list.
...
The following example shows the [**ExecMethodAsync**](swbemservices-execmethodasync.md) method. The script creates a [**Win32\_Process**](https://msdn.microsoft.com/library/aa394372) object that represents a process that is running Notepad. It shows setting up of [**InParameters**](swbemmethod-inparameters.md) object, and how to obtain results from an [**OutParameters**](swbemmethod-outparameters.md) object.

For a script that shows the same operations performed synchronously, see [**SWbemObject.ExecMethod**](swbemobject-execmethod-.md). For an example using direct access, see [**Create Method in Class Win32\_Process**](https://msdn.microsoft.com/library/aa389388). For an example of the same operation using an [**SWbemServices**](swbemservices.md) object, see [**SWbemServices.ExecMethodAsync**](swbemservices-execmethodasync.md).
