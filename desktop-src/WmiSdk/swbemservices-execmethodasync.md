...
After the completion of the **ExecMethodAsync** method, the [Err](https://msdn.microsoft.com/library/sbf5ze0e.aspx) object may contain one of the error codes listed in the following list.
...
The following code example shows the **ExecMethodAsync** method. The script creates a [**Win32\_Process**](https://msdn.microsoft.com/library/aa394372) object that represents a process that is running Notepad. It shows the setting up of an [**InParameters**](swbemmethod-inparameters.md) object and how to obtain results from an [**OutParameters**](swbemmethod-outparameters.md) object. For a script that shows the same operations performed synchronously, see [**SWbemServices.ExecMethod**](swbemservices-execmethod.md). For an example of using direct access, see [Create Method in Class Win32\_Process](https://msdn.microsoft.com/library/aa389388). For an example of the same operation using [**SWbemObject**](swbemobject.md), see [**SWbemObject.ExecMethodAsync**](swbemobject-execmethodasync-.md).
