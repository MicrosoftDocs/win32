...
You can create an instance in C++ through the [**IWbemServices**](iwbemservices.md) interface.
...
1.  Retrieve the definition of the existing class by calling the [**IWbemServices::GetObject**](iwbemservices-getobject.md) or [**IWbemServices::GetObjectAsync**](iwbemservices-getobjectasync.md) methods.

    The following code example shows how to use the [**GetObject**](iwbemservices-getobject.md) and [**GetObjectAsync**](iwbemservices-getobjectasync.md) methods to get a pointer to the [**IWbemClassObject**](iwbemclassobject.md) interface that provides access to the class definition.
...
2.  Create the new instance by calling the [**IWbemClassObject::SpawnInstance**](iwbemclassobject-spawninstance.md) method.
...
3.  Set values for any properties that do not inherit the values defined for the class by calling the [**IWbemClassObject::Put**](iwbemclassobject-put.md) method.
    ...
    If the existing class has a key property, you should set the property either to **NULL** or a guaranteed unique value. If you set the key to **NULL** and the key is a string, then [**PutInstanceAsync**](iwbemservices-putinstanceasync.md) or [**PutInstance**](iwbemservices-putinstance.md) internally generates and assigns a GUID to the key. This way, specifying **NULL** for a key property lets you create a unique instance that will not overwrite any previous instance.
...
4.  Set the values for any relevant qualifiers through a call to [**IWbemClassObject::GetQualifierSet**](iwbemclassobject-getqualifierset.md).

    The [**GetQualifierSet**](iwbemclassobject-getqualifierset.md) method returns a pointer to an [**IWbemQualifierSet**](iwbemqualifierset.md) interface, which use to access the qualifiers for a class or instance. You can specify different values for a qualifier defined for the class if the class qualifier flavor is **EnableOverride**. You cannot modify or delete a class qualifier with the flavor set to **DisableOverride**. For more information, see [Qualifier Flavors](qualifier-flavors.md).
...
5.  Save the instance by calling the [**IWbemServices::PutInstance**](iwbemservices-putinstance.md) or [**IWbemServices::PutInstanceAsync**](iwbemservices-putinstanceasync.md) method.
