...
1.  Set up your application with calls to the [**CoInitializeEx**](_com_coinitializeex) and [**CoInitializeSecurity**](_com_coinitializesecurity) functions.

    Calling [**CoInitializeEx**](_com_coinitializeex) initializes COM, while [**CoInitializeSecurity**](_com_coinitializesecurity) grants WMI the permission to call into the process of the consumer. The **CoInitializeEx** function also grants you the ability to program a multithreaded application, which is necessary for asynchronous notification. For more information, see [Maintaining WMI Security](maintaining-wmi-security.md).
    ...
    The following code example describes how to set up the temporary event consumer with calls to [**CoInitializeEx**](_com_coinitializeex) and [**CoInitializeSecurity**](_com_coinitializesecurity).
