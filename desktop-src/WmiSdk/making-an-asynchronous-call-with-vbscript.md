...
Asynchronous methods with names that end in "Async\_" always return immediately after being called so that a program can continue executing. For example, [**SWbemServices.ExecQuery**](swbemservices-execquery.md) is synchronous and blocks execution until all of the objects are returned. The [**SWbemServices.ExecQueryAsync**](swbemservices-execqueryasync.md) method is the nonblocking asynchronous version. A more secure way to make the call to **SWbemServices.ExecQuery** nonblocking is by making the call [*semisynchronously*](https://www.bing.com/search?q=*semisynchronously*). For more information, see [Setting Security on an Asynchronous Call](setting-security-on-an-asynchronous-call.md) and [Making a Semisynchronous Call with VBScript](making-a-semisynchronous-call-with-vbscript.md).
...
2.  Create the object sink using either [CreateObject](https://msdn.microsoft.com/library/xzysf6hc.aspx) or (for Windows Script Host 2.0 only) the OBJECT tag with an events attribute set to **TRUE**.
...
The following code example shows an asynchronous query for all the instances of [**Win32\_Process**](https://msdn.microsoft.com/library/aa394372) on the local machine. For a semisynchronous version of the same method, see [Calling a Method](calling-a-method.md).
