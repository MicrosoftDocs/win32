...
When writing a high-performance provider that derives classes from [**Win32\_PerfRawData**](https://msdn.microsoft.com/library/aa394299), you must follow specific conventions so that WMI can supply data to the property values.
...
The following procedure describes how to support the [**Win32\_PerfRawData**](https://msdn.microsoft.com/library/aa394299) class with your high-performance provider.

**To support the Win32\_PerfRawData class**

1.  Create your class in the Root\\CIMv2 namespace.

    The class must be derived from [**Win32\_PerfRawData**](https://msdn.microsoft.com/library/aa394299) and have the **Hiperf** qualifier set to **TRUE**. You can also add WDM (driver) performance data classes to the root\\wmi namespace. For more information about creating your own class for WMI, see [Designing Managed Object Format (MOF) Classes](designing-managed-object-format--mof--classes.md).
...
4.  Populate the appropriate timestamp and frequency properties used to compute counter-type formulas.
    ...
    These properties are inherited from [**Win32\_PerfRawData**](https://msdn.microsoft.com/library/aa394299) and, if you are writing a high-performance provider, you must fill these out for the class to be displayed in System Monitor.
