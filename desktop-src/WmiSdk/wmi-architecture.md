...
To obtain data from WMI, write a client script or application that accesses [WMI Classes](wmi-classes.md) or provide data to WMI by writing a [*WMI provider*](https://www.bing.com/search?q=*WMI provider*). For more information, see [Using WMI](using-wmi.md).
...
-   Managed objects and WMI providers
    ...
    A WMI provider is a COM object that monitors one or more [*managed objects*](https://www.bing.com/search?q=*managed objects*) for WMI. A managed object is a logical or physical enterprise component, such as a hard disk drive, network adapter, database system, operating system, process, or service.

    Similar to a driver, a provider supplies WMI with data from a managed object and handles messages from WMI to the managed object. WMI providers consist of a DLL file and a [*Managed Object Format (MOF)*](https://www.bing.com/search?q=*Managed Object Format (MOF)*) file that defines the classes for which the provider returns data and performs operations. Providers, like WMI C++ applications, use the [COM API for WMI](com-api-for-wmi.md). For more information, see [Providing Data to WMI](providing-data-to-wmi.md).

    An example of a provider is the preinstalled [Registry provider](https://msdn.microsoft.com/library/aa393886), which accesses data in the system registry. The Registry provider has one [*WMI class*](https://www.bing.com/search?q=*WMI class*), [**StdRegProv**](https://msdn.microsoft.com/library/aa393664), with many methods but no properties. Other preinstalled providers, such as the [Win32 provider](https://msdn.microsoft.com/library/aa394388), usually have classes with many properties but few methods, such as [**Win32\_Process**](https://msdn.microsoft.com/library/aa394372) or [**Win32\_LogicalDisk**](https://msdn.microsoft.com/library/aa394173). The Registry provider DLL file, Stdprov.dll, contains the code that dynamically returns data when requested by client scripts or applications.

    WMI MOF and DLL files are located in %WINDIR%\\System32\\Wbem, along with the [WMI Command-Line Tools](wmi-command-line-tools.md), such as [**Winmgmt.exe**](winmgmt.md) and [**Mofcomp.exe**](mofcomp.md). Provider classes, such as [**Win32\_LogicalDisk**](https://msdn.microsoft.com/library/aa394173), are defined in MOF files, and then compiled into the WMI repository at system startup.

-   [WMI infrastructure](wmi-infrastructure.md)

    The WMI infrastructure is a Microsoft Windows operating system component know as the WMI service(winmgmt). The WMI infrastructure has two components: the WMI Core, and the [*WMI repository*](https://www.bing.com/search?q=*WMI repository*).

    The WMI repository is organized by WMI [*namespaces*](https://www.bing.com/search?q=*namespaces*). The WMI service creates some namespaces such as root\\default, root\\cimv2, and root\\subscription at system startup and preinstalls a default set of class definitions, including the [Win32 Classes](https://msdn.microsoft.com/library/aa394388), the [WMI System Classes](wmi-system-classes.md), and others. The remaining namespaces found on your system are created by providers for other parts of the operating system or products. For more information and a list of WMI providers found in most operating system versions, see [WMI Providers](wmi-providers.md).
