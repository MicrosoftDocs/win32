...
To obtain data from WMI, write a client script or application that accesses [WMI Classes](wmi-classes.md) or provide data to WMI by writing a [*WMI provider*](gloss-p.md#wmi-gloss-provider). For more information, see [Using WMI](using-wmi.md).
...
-   Managed objects and WMI providers
    ...
    A WMI provider is a COM object that monitors one or more [*managed objects*](gloss-m.md#wmi-gloss-managed-object) for WMI. A managed object is a logical or physical enterprise component, such as a hard disk drive, network adapter, database system, operating system, process, or service.

    Similar to a driver, a provider supplies WMI with data from a managed object and handles messages from WMI to the managed object. WMI providers consist of a DLL file and a [*Managed Object Format (MOF)*](gloss-m.md#wmi-gloss-managed-object-format) file that defines the classes for which the provider returns data and performs operations. Providers, like WMI C++ applications, use the [COM API for WMI](com-api-for-wmi.md). For more information, see [Providing Data to WMI](providing-data-to-wmi.md).

    An example of a provider is the preinstalled [Registry provider](https://msdn.microsoft.com/library/aa393886), which accesses data in the system registry. The Registry provider has one [*WMI class*](gloss-w.md#wmi-gloss-wmi-class), [**StdRegProv**](https://msdn.microsoft.com/library/aa393664), with many methods but no properties. Other preinstalled providers, such as the [Win32 provider](https://msdn.microsoft.com/library/aa394388), usually have classes with many properties but few methods, such as [**Win32\_Process**](https://msdn.microsoft.com/library/aa394372) or [**Win32\_LogicalDisk**](https://msdn.microsoft.com/library/aa394173). The Registry provider DLL file, Stdprov.dll, contains the code that dynamically returns data when requested by client scripts or applications.
