...
The following VBScript code example requests notification when an instance of [**Win32\_ProcessTrace**](https://msdn.microsoft.com/library/aa394377) is created. An instance of this class is generated when a process is started or stopped.
...
The following script code example is an asynchronous version of the [**Win32\_ProcessTrace**](https://msdn.microsoft.com/library/aa394377) script. Because asynchronous operations return immediately, a dialog box keeps the script active while it is waiting for events.
...
1.  [Register the event provider](registering-a-provider.md) with the namespace that you are using.
    ...
    Some event providers can only use a specific namespace. For example, [**\_\_InstanceCreationEvent**](--instancecreationevent.md) is an intrinsic event that is supported by the [Win32 provider](https://msdn.microsoft.com/library/aa394388) and is registered by default with the \\root\\cimv2 namespace.
