...
2.  Log on to the target WMI namespace.
    ...
    Applications always have to call the COM function [**CoInitializeSecurity**](_com_coinitializesecurity) during the initialization phase. If they do not do so before making an asynchronous call, WMI releases the application sink without completing the asynchronous call. For more information, see [Initializing COM for a WMI Application](initializing-com-for-a-wmi-application.md).
