...
Name of the event property that contains data to be passed to the [**ReportEvent**](https://msdn.microsoft.com/library/windows/desktop/aa363679) function *lpRawData* parameter.
...
Name of the event property that contains a security identifier (SID) to be passed to the [**ReportEvent**](https://msdn.microsoft.com/library/windows/desktop/aa363679) function *lpUserSid* parameter. The property must be either an array of bytes (**uint8**) or a string. If it is an array of bytes, it is assumed to be a SID. If it is a string, it is a string SID that is converted into a SID.
