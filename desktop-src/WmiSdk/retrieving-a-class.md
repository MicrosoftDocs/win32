...
1.  You can use the [ManagementObjectSerarcher](https://msdn.microsoft.com/library/system.management.managementobjectsearcher.aspx) with a query to **meta\_class**, with the WHERE clause containing the name of the class you with to retrieve.
    ...
    [ManagementObjectSerarcher](https://msdn.microsoft.com/library/system.management.managementobjectsearcher.aspx) is the standard class .NET uses to retrieve class and instance information from WMI. [ManagementObjectSerarcher.Get](https://msdn.microsoft.com/library/67a77ef1.aspx) returns a [ManagementObjectCollection](https://msdn.microsoft.com/library/system.management.managementobjectcollection.aspx) that contains the schema definition class. The **meta\_class** class defines the query as a schema query. Without the **meta\_class** class, this query would return all instances of [**Win32\_LogicalDisk**](https://msdn.microsoft.com/library/aa394173). For more information about querying WMI, see [SELECT Statement for Schema Queries](select-statement-for-schema-queries.md).

2.  Alternately, create a new [ManagementClass](https://msdn.microsoft.com/library/system.management.managementclass.aspx) object, with the name as the path, to retrieve the class.
...
2.  The following code example retrieves the class definition for the class that describes logical drives on your computer.
    ...
    On Active Server Pages (ASP) use [**GetObject**](https://msdn.microsoft.com/windows/desktop/47dd01cb-9468-481e-be7e-55f69a744635) or [CreateObject](https://msdn.microsoft.com/library/xzysf6hc.aspx) in the server-side script. For more information, see [Creating Active Server Pages for WMI](creating-active-server-pages-for-wmi.md).

3.  A class or instance can also be specified, in which case the returned object is a WMI object, for example, an instance of [**Win32\_LogicalDisk**](https://msdn.microsoft.com/library/aa394173), rather than a services object. Note that you cannot use the VBScript [**GetObject**](https://msdn.microsoft.com/windows/desktop/47dd01cb-9468-481e-be7e-55f69a744635) functions to create an instance of the generic object [**SWbemObject**](swbemobject.md).
4.  In HTML pages running in Microsoft Internet Explorer (IE), [**GetObject**](https://msdn.microsoft.com/windows/desktop/47dd01cb-9468-481e-be7e-55f69a744635) and [CreateObject](https://msdn.microsoft.com/library/xzysf6hc.aspx) can fail because WMI scripting objects, like ActiveX controls, are not marked as safe for scripting. The one exception is the [**SWbemDateTime**](swbemdatetime.md) object. The only way that these calls can succeed is when you lower the IE security settings, which is not recommended.
