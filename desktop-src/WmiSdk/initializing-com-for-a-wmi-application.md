...
The first step in connecting to WMI is setting up the COM calls to [**CoInitializeEx**](_com_coinitializeex) and [**CoInitializeSecurity**](_com_coinitializesecurity).
...
1.  Initialize COM with a call to [**CoInitializeEx**](_com_coinitializeex).

    Calling [**CoInitializeEx**](_com_coinitializeex) is a standard procedure for setting up a COM interface. Therefore, WMI does not require that you observe any additional procedures when calling **CoInitializeEx**. For more information, see [COM](_com_com_start_page).

    The following code example describes how to call [**CoInitializeEx**](_com_coinitializeex).
...
2.  Set the general COM security levels with a call to the [**CoInitializeSecurity**](_com_coinitializesecurity) interface.
    ...
    The following code example describes how to call [**CoInitializeSecurity**](_com_coinitializesecurity) to set COM security on the process.
