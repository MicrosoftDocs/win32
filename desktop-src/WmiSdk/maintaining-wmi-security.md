...
WMI security focuses on protecting access to namespace data. WMI first grants access to groups of users as specified by the [*WMI Control*](https://www.bing.com/search?q=*WMI Control*) and DCOM settings and then providers determine if the user should have access to namespace data.
...
Namespace security depends on standard Windows user [*security identifiers (SID)*](https://www.bing.com/search?q=*security identifiers (SID)*) and the [*security descriptor*](https://www.bing.com/search?q=*security descriptor*) for the WMI namespace.
