...
-   Applications have control over content scaling that happens during window resize, using the [DXGI\_SCALING](/windows/win32/DXGI1_2/ne-dxgi1_2-dxgi_scaling?branch=master) property set during swapchain creation.
...
2.  After [Present](/windows/win32/DXGI/nf-dxgi-idxgiswapchain-present?branch=master) calls, the back buffer needs to explicitly be re-bound to the D3D11 immediate context before it can be used again.
3.  After calling [SetFullscreenState](/windows/win32/DXGI/nf-dxgi-idxgiswapchain-setfullscreenstate?branch=master), the application must call [ResizeBuffers](/windows/win32/DXGI/nf-dxgi-idxgiswapchain-resizebuffers?branch=master) before **Present**.
