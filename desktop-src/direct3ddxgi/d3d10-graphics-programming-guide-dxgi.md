...
> [!IMPORTANT]
> **Modern swap chain best practice:** For new applications targeting Windows 10 and later, always use the **flip model** swap effect (`DXGI_SWAP_EFFECT_FLIP_DISCARD` or `DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL`). The legacy blt-model swap effects (`DXGI_SWAP_EFFECT_DISCARD` and `DXGI_SWAP_EFFECT_SEQUENTIAL`) have higher memory usage, do not support modern features like variable refresh rate (VRR) and HDR, and are not recommended for new applications. Create swap chains using `CreateSwapChainForHwnd` (DXGI 1.2+) rather than the legacy `CreateSwapChain`. See [DXGI flip model](/windows/win32/direct3ddxgi/dxgi-flip-model) for details.

> [!NOTE]
> DXGI is shared infrastructure used by **Direct3D 11, Direct3D 12**, and Direct2D. Regardless of which Direct3D version you use, swap chain management and adapter enumeration go through DXGI.
