...
You can use scroll only with DXGI swap chains that run in flip presentation model. You can use dirty rectangles with DXGI swap chains that run in both flip model and bitblt model (set with [**DXGI\_SWAP\_EFFECT\_SEQUENTIAL**](https://www.bing.com/search?q=**DXGI\_SWAP\_EFFECT\_SEQUENTIAL**)).
...
In the simplest case, when you update a single dirty rectangle per frame, the dirty rectangles across two frames might intersect. To find out whether the dirty rectangle of the previous frame and the dirty rectangle of the current frame overlap, you need to verify whether the dirty rectangle of the previous frame intersects with the dirty rectangle of the current frame. You can call the GDI [**IntersectRect**](https://msdn.microsoft.com/windows/desktop/da686f78-e557-4ff2-9f24-b229f0c01563) function to determine whether two [**RECT**](https://msdn.microsoft.com/windows/desktop/9439cb6c-f2f7-4c27-b1d7-8ddf16d81fe8) structures that represent the two dirty rectangles intersect.

In this code snippet, a call to [**IntersectRect**](https://msdn.microsoft.com/windows/desktop/da686f78-e557-4ff2-9f24-b229f0c01563) returns the intersection of two dirty rectangles in another [**RECT**](https://msdn.microsoft.com/windows/desktop/9439cb6c-f2f7-4c27-b1d7-8ddf16d81fe8) called dirtyRectCopy. After the code snippet determines that the two dirty rectangles intersect, it calls the [**ID3D11DeviceContext1::CopySubresourceRegion1**](https://msdn.microsoft.com/windows/desktop/1963011F-C3E2-428D-B667-195A4976510B) method to copy the region of intersection into the current frame.
...
In this code snippet, we call the GDI [**SetRectRgn**](https://msdn.microsoft.com/windows/desktop/9a024d61-f397-43d8-a48e-edb8102a6f55) function to convert each dirty rectangle into a rectangular region and then we call the GDI [**CombineRgn**](https://msdn.microsoft.com/windows/desktop/ef9fc4f3-737e-4c10-a80b-8ae2097c17d1) function to combine all the dirty rectangular regions into a group.
...
You can now use the GDI [**CombineRgn**](https://msdn.microsoft.com/windows/desktop/ef9fc4f3-737e-4c10-a80b-8ae2097c17d1) function to determine the intersection between the dirty region of the previous frame and the dirty region of the current frame. After you obtain the intersecting region, call the GDI [**GetRegionData**](https://msdn.microsoft.com/windows/desktop/e0d4862d-a405-4c00-b7b0-af4dd60407c0) function to obtain each individual rectangle from the intersecting region and then call the [**ID3D11DeviceContext1::CopySubresourceRegion1**](https://msdn.microsoft.com/windows/desktop/1963011F-C3E2-428D-B667-195A4976510B) method to copy each intersecting rectangle into the current back buffer. The next code snippet shows how to use these GDI and Direct3D functions.
