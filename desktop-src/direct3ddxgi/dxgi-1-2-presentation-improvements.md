...
In this code snippet, we call the GDI [**SetRectRgn**](gdi.setrectrgn) function to convert each dirty rectangle into a rectangular region and then we call the GDI [**CombineRgn**](gdi.combinergn) function to combine all the dirty rectangular regions into a group.
...
You can now use the GDI [**CombineRgn**](gdi.combinergn) function to determine the intersection between the dirty region of the previous frame and the dirty region of the current frame. After you obtain the intersecting region, call the GDI [**GetRegionData**](gdi.getregiondata) function to obtain each individual rectangle from the intersecting region and then call the [**ID3D11DeviceContext1::CopySubresourceRegion1**](direct3d11.id3d11devicecontext1_copysubresourceregion1) method to copy each intersecting rectangle into the current back buffer. The next code snippet shows how to use these GDI and Direct3D functions.
