...
-   **Count** member of the [**DXGI\_SAMPLE\_DESC**](dxgi-sample-desc.md) structure that the **SampleDesc** member specifies to one and the **Quality** member of **DXGI\_SAMPLE\_DESC** to zero because multiple sample antialiasing (MSAA) is not supported

If you use [**DXGI\_SWAP\_EFFECT\_FLIP\_SEQUENTIAL**](dxgi-swap-effect.md#dxgi-swap-effect-flip-sequential) on Windows 7 or earlier operating system, device creation fails. When you use flip model, you can use full-screen present statistics in windowed mode. Full-screen behavior is not affected. If you pass **NULL** to the *pFullscreenDesc* parameter of [**IDXGIFactory2::CreateSwapChainForHwnd**](idxgifactory2-createswapchain1.md) for a windowed swap chain and set **SwapEffect** to **DXGI\_SWAP\_EFFECT\_FLIP\_SEQUENTIAL**, the runtime creates one extra back buffer and rotates whichever handle belongs to the buffer that becomes the front buffer at presentation time.
...
Present statistics are frame-timing information that media apps use to synchronize video and audio streams and recover from video playback glitches. Apps can use the frame-timing information in present statistics to adjust the presentation rate of their video frames for smoother presentation. To obtain present statistics information, call the [**IDXGISwapChain::GetFrameStatistics**](idxgiswapchain-getframestatistics.md) method to obtain the [**DXGI\_FRAME\_STATISTICS**](dxgi-frame-statistics.md) structure. **DXGI\_FRAME\_STATISTICS** contains statistics about [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md) calls. A flip model swap chain provides present statistics information in both windowed and full-screen modes. For bitblt model swap chains in windowed mode, all **DXGI\_FRAME\_STATISTICS** values are zeroes.

For flip model present statistics, [**IDXGISwapChain::GetFrameStatistics**](idxgiswapchain-getframestatistics.md) returns **DXGI\_ERROR\_FRAME\_STATISTICS\_DISJOINT** in these situations:

-   First call to [**GetFrameStatistics**](idxgiswapchain-getframestatistics.md), which indicates the beginning of a sequence
-   Mode change: either windowed mode to or from full screen or full screen to full screen transitions

The values in **PresentRefreshCount**, **SyncRefreshCount**, and **SyncQPCTime** members of [**DXGI\_FRAME\_STATISTICS**](dxgi-frame-statistics.md) have the following characteristics:
...
The [**IDXGISwapChain::GetLastPresentCount**](idxgiswapchain-getlastpresentcount.md) method returns the last present count, that is, the present ID of the last successful [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md) call made by a display device that is associated with the swap chain. This present ID is the value of the **PresentCount** member of the [**DXGI\_FRAME\_STATISTICS**](dxgi-frame-statistics.md) structure. For bitblt model swap chains, while in windowed mode, all **DXGI\_FRAME\_STATISTICS** values are zeroes.
...
1.  Queue [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md) calls (that is, call **IDXGISwapChain1::Present1** multiple times, which causes them to collect in a queue).
2.  Create a present-queue structure to store all successfully submitted [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md)'s present ID (returned by [**IDXGISwapChain::GetLastPresentCount**](idxgiswapchain-getlastpresentcount.md)) and associated, calculated/expected **PresentRefreshCount** values.
3.  To detect a glitch:

    -   Call [**IDXGISwapChain::GetFrameStatistics**](idxgiswapchain-getframestatistics.md).
...
4.  To recover from the glitch:
    ...
    -   Pass 0 to the *SyncInterval* parameter in this number of calls to [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md) to discard and skip this number of frames.

        > [!Note]  
        > If the glitch consists of a large number of frames, call [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md) with the *Flags* parameter set to [**DXGI\_PRESENT\_RESTART**](dxgi-present.md#dxgi-present-restart) to discard and skip all outstanding queued presents.
...
In the example scenario, you expect frame A to go on screen on a vsync count of 1. But you actually detect the vsync count that frame A appears on screen as 4. Therefore, you determine that a glitch occurred. You can then discard 3 frames, that is, you can pass 0 to the *SyncInterval* parameter in 3 calls to [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md). In the preceding example scenario, to recover from the glitch, you need a total of 8 **IDXGISwapChain1::Present1** calls. The 9th frame is then visible as per the vsync count that you expect.
...
1.  The app wakes up on vsync, renders blue, calls [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md), and then goes back to sleep.
2.  The graphics processing unit (GPU) wakes up from idle, performs the render to blue, and then goes back to sleep.
3.  The DWM wakes up at the next vsync, composes blue into its back buffer, calls [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md), and then goes back to sleep.
4.  The app wakes up, renders green, calls [**IDXGISwapChain1::Present1**](idxgiswapchain1-present1.md), and then goes back to sleep.
    > [!Note]  
