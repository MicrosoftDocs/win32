...
Check for code that incorrectly special cases around Internet Explorer 6, Windows Internet Explorer 7, or Internet Explorer 8 through [user-agent string sniffing, versions vectors, or conditional comments](inet_user_agent).
...
-   Applies to [specified document modes](inet_doc_compat_ovw).
...
-   Use the [EmulateIE7 X-UA-Compatible](csssupport) HTTP header or **meta** element to enable Compatibility View on websites or specific webpages.
...
-   [XML prologs](expie20060201) in the first line of the source code no longer cause DOCTYPE declarations to fail.
-   [Box model](expie20060201) overflow content intersects box and no longer automatically -grows the box div to fit the content.
-   [Certain CSS filters](expie20060201) (for example, \*HTML, \_underscore, and /\*\*/ comment) are not supported.
-   [Only the outermost OBJECT element in nested objects](_inet_whatsnew_70_sdk) is instantiated .
-   [Applications that rely on the SELECT element](_inet_whatsnew_70_sdk) to get an HWND to use with Microsoft Win32 APIs might break because the [SELECT element](http://go.microsoft.com/fwlink/p/?linkid=204996) is now a windowless control.
...
-   [Unclosed P elements](csssupport) are automatically closed when they are followed by [**TABLE**](_inet_TABLE_Element_TABLE_Object), [**FORM**](_inet_FORM_Element_FORM_Object), [**NOFRAMES**](_inet_NOFRAMES_Element_NOFRAMES_Object), or [**NOSCRIPT**](_inet_NOSCRIPT_Element_NOSCRIPT_Object) elements.
...
-   Check the [complete list of CSS updates from Internet Explorer 6 to Internet Explorer 8](csssupport).
...
-   [Object caching protection](inet_whatsnew_70_sec) is enabled by default, which blocks access to references of objects when users browse to a new domain (applies to Internet Explorer 6 and later versions on Windows XP with Service Pack 2 (SP2) and later versions).
-   [DHTML scriptlets](inet_whatsnew_70_sec) are disabled by default.
-   [Scripts that write to the status bar](inet_whatsnew_70_sec) are blocked.
-   [URL creation might fail](ie7_compat_log) if URLs do not meet RFC guidelines.
-   [HTTPS pages display an error page](ie7_https_imps) if the site is configured to SSLv2 only, or if the site security certificate is outdated or invalid, has errors, or has weak ciphers.
-   Only ["Punycode" encoded internationalized domain names](ie7_idn_support) are supported. Other formats like ANSI and UTF-8 are blocked.
-   [Cross-domain script URLs](ie7_compat_log), redirected navigation in DOM objects, and frame navigations are blocked.
...
-   AJAX-enabled sites that [manually manipulate the hash of the URL](inet_ajax_navigations) might be broken by the new window.location.hash navigation property.
-   [New AJAX features](_inet_whatsnew_80_sdk) like [XDM](http://go.microsoft.com/fwlink/p/?linkid=205003) have [**native properties**](inet_IHTMLEventObj5) that might conflict with existing custom properties.
...
-   [Navigating a top-level frame](_inet_appcompat_event1062_dhtml) to a site in a different security context opens a new window or tab instead of navigating within the existing frame.
-   [UTF-7 encoded script](_inet_appcompat_event1058_dhtml) is forced into Windows-1252 encoding, which might cause plain text rendering.
...
-   [Content that is returned by a web proxy](_inet_appcompat_event1065_dhtml) is blocked if an SSL tunnel is not established in response to a CONNECT request to the original server.
...
-   [Protected Mode](http://go.microsoft.com/fwlink/p/?linkid=205010) is enabled by default for [Internet, Intranet, and Restricted Sites zones](_inet_URL_Security_Zones_ovw_entry). This mode [blocks browser extensions that could pose a security risk](_inet_appcompat_event1041_dhtml) from running and [lower privilege applications from accessing higher privilege processes](_inet_appcompat_event1042_dhtml), like the Start menu, Control Panel, and the Microsoft Windows registry (applies to Internet Explorer 7 and later versions on Windows Vista and later versions).

<!-- -->

-   [Protected Mode Update](_inet_appcompat_event1047_dhtml): Intranet runs in medium (instead of low) integrity level by default.
