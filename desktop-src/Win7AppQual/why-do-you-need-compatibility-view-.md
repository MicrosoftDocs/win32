...
Users do not always see this icon. It is a secondary solution instead of the primary application compatibility mechanism. Windows Internet Explorer displays this icon only when a change into Compatibility View makes sense, such as when a user views standards mode pages. In all other cases, such as when a user views quirks mode pages or views Intranet Zone sites, the icon does not appear. For more information about Compatibility View and when the icon appears, see [Introducing Compatibility View](http://go.microsoft.com/fwlink/p/?linkid=204995).
