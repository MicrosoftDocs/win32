...
Note that the touch input handle in this parameter is no longer valid after the message has been passed to [DefWindowProc](http://go.microsoft.com/fwlink/p/?linkid=136637). [DefWindowProc](http://go.microsoft.com/fwlink/p/?linkid=136637) will close and invalidate this handle.
...
If the application does not process the message, it must call [DefWindowProc](http://go.microsoft.com/fwlink/p/?linkid=136637). Not doing so causes the application to leak memory because the touch input handle is not closed and associated process memory is not freed.
