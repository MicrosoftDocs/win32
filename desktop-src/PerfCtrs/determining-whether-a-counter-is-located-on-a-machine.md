...
To determine if a counter is installed on a particular computer, call [**PdhValidatePath**](/windows/win32/Pdh/nf-pdh-pdhvalidatepatha?branch=master) with the fully qualified counter path. The function returns ERROR\_SUCCESS if the counter is located on the specified computer.

[**PdhValidatePath**](/windows/win32/Pdh/nf-pdh-pdhvalidatepatha?branch=master) validates the entire path; therefore, if any object, instance, or counter in the path does not exist, the return value indicates so. **PdhValidatePath** verifies the specified counter path in this order: computer, performance object, instance, and counter.
