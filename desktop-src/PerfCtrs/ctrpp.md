...
The **PerfAutoInitialize** function calls the [**PerfStartProvider**](perfstartprovider.md) function to register the provider and the [**PerfSetCounterSetInfo**](perfsetcountersetinfo.md) function to initialize each counter set. If you specify the **-MemoryRoutines** or **-NotificationCallback** arguments, the **PerfAutoInitialize** function will call the [**PerfStartProviderEx**](perfstartproviderex.md) function instead of calling **PerfStartProvider**.

The **PerfAutoCleanup** function calls [**PerfStopProvider**](perfstopprovider.md) to remove the provider's registration from the list of registered providers and frees all resources associated with the provider.

Your provider calls the **PerfAutoInitialize** and **PerfAutoCleanup** functions.

The .c file includes the [**notification**](controlcallback-perflibv2.md) callback function only if the **callback** attribute of [**provider**](https://msdn.microsoft.com/library/windows/desktop/ee781352) is set to "custom" or you use the **-NotificationCallback** argument. The generated callback includes all the callback cases. You can provide implementation for each case or remove those cases that you do not want to support.
...
Changes the default signature of the [**CounterInitialize**](counterinitialize.md) function to include parameters for specifying the name of your [*AllocateMemory*](allocatememory.md) and [*FreeMemory*](freememory.md) callback functions.
...
Changes the default signature of the [**CounterInitialize**](counterinitialize.md) function to include the parameter for specifying the name of your [*ControlCallback*](controlcallback-perflibv2.md) callback function.
...
The .h file also includes a counter set GUID and [**PERF\_COUNTERSET\_INFO**](perf-counterset-info.md) block for each counter set defined in the manifest and the symbolic names for the provider, counter sets, and counters (if specified). You specify the symbolic names in the manifest. You can use the symbolic names when you reference the provider, counter set, or counter. For example, when calling the [**PerfCreateInstance**](perfcreateinstance.md) function, set the *CounterSetGuid* parameter to the symbolic name.
