...
The **PerfAutoInitialize** function calls the [**PerfStartProvider**](/windows/win32/Perflib/nf-perflib-perfstartprovider?branch=master) function to register the provider and the [**PerfSetCounterSetInfo**](/windows/win32/Perflib/nf-perflib-perfsetcountersetinfo?branch=master) function to initialize each counter set. If you specify the **-MemoryRoutines** or **-NotificationCallback** arguments, the **PerfAutoInitialize** function will call the [**PerfStartProviderEx**](/windows/win32/Perflib/nf-perflib-perfstartproviderex?branch=master) function instead of calling **PerfStartProvider**.

The **PerfAutoCleanup** function calls [**PerfStopProvider**](/windows/win32/Perflib/nf-perflib-perfstopprovider?branch=master) to remove the provider's registration from the list of registered providers and frees all resources associated with the provider.

Your provider calls the **PerfAutoInitialize** and **PerfAutoCleanup** functions.

The .c file includes the [**notification**](/windows/win32/Perflib/nc-perflib-perflibrequest?branch=master) callback function only if the **callback** attribute of [**provider**](https://msdn.microsoft.com/library/windows/desktop/ee781352) is set to "custom" or you use the **-NotificationCallback** argument. The generated callback includes all the callback cases. You can provide implementation for each case or remove those cases that you do not want to support.
...
Changes the default signature of the [**CounterInitialize**](counterinitialize.md) function to include parameters for specifying the name of your [*AllocateMemory*](/windows/win32/Perflib/nc-perflib-perf_mem_alloc?branch=master) and [*FreeMemory*](/windows/win32/Perflib/nc-perflib-perf_mem_free?branch=master) callback functions.
...
Changes the default signature of the [**CounterInitialize**](counterinitialize.md) function to include the parameter for specifying the name of your [*ControlCallback*](/windows/win32/Perflib/nc-perflib-perflibrequest?branch=master) callback function.
...
The .h file also includes a counter set GUID and [**PERF\_COUNTERSET\_INFO**](/windows/win32/Perflib/ns-perflib-_perf_counterset_info?branch=master) block for each counter set defined in the manifest and the symbolic names for the provider, counter sets, and counters (if specified). You specify the symbolic names in the manifest. You can use the symbolic names when you reference the provider, counter set, or counter. For example, when calling the [**PerfCreateInstance**](/windows/win32/Perflib/nf-perflib-perfcreateinstance?branch=master) function, set the *CounterSetGuid* parameter to the symbolic name.
