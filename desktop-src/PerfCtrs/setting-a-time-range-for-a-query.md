...
If the data source is a log file, you can specify a time range for the query. The query retrieves counter data from the log file that was collected during the specified time range. To set the time range, call the [**PdhSetQueryTimeRange**](pdhsetquerytimerange.md) function. **PdhSetQueryTimeRange** is not used to query performance data from real time data sources.
...
4.  Copy the LONGLONG value to the appropriate field in the [**PDH\_TIME\_INFO**](pdh-time-info-str.md) structure.

To retrieve the time range of all of the performance data contained in a log file, call the [**PdhGetDataSourceTimeRange**](pdhgetdatasourcetimerange.md) function.
