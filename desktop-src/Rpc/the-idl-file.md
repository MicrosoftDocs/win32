...
The interface body contains the function prototype for **HelloProc**. In this prototype, the function parameter pszString has the attributes **\[**[**in**](https://msdn.microsoft.com/library/windows/desktop/aa367051)**\]** and **\[**[**string**](https://msdn.microsoft.com/library/windows/desktop/aa367270)**\]**. The **\[in\]** attribute tells the run-time library that the parameter is passed only from the client to the server. The **\[string\]** attribute specifies that the stub should treat the parameter as a C-style character string.
