...
The **\[**[**transmit\_as**](https://msdn.microsoft.com/library/windows/desktop/aa367286)**\]** attribute offers a way to control data marshaling without worrying about marshaling data at a low level—that is, without worrying about data sizes or byte swapping in a heterogeneous environment. By letting you reduce the amount of data transmitted over the network, the **\[transmit\_as\]** attribute can make your application more efficient.
...
These programmer-supplied routines are provided by either the client or the server application based on the directional attributes. If the parameter is **\[**[**in**](https://msdn.microsoft.com/library/windows/desktop/aa367051)**\]** only, the client transmits to the server. The client needs the **type\_to\_xmit** and **type\_free\_xmit** functions. The server needs the **type\_from\_xmit** and **type\_free\_inst** functions. For an **\[**[**out**](https://msdn.microsoft.com/library/windows/desktop/aa367136)**\]**-only parameter, the server transmits to the client. The server application must implement the **type\_to\_xmit** and **type\_free\_xmit** functions, while the client program must supply the **type\_from\_xmit** function. For the temporary **xmit\_type** objects, the stub will call **type***\_***free\_xmit** to free any memory allocated by a call to **type\_to\_xmit**.
...
Rather than shipping the complex structure, the **\[**[**transmit\_as**](https://msdn.microsoft.com/library/windows/desktop/aa367286)**\]** attribute can be used to send it over the network as an array. The sequence of items in the array retains the ordering of the elements in the list at a lower cost:
