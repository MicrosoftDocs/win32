...
The [**RpcTryFinally**](rpctryfinally.md) / [**RpcFinally**](/windows/win32/Rpc/?branch=master) / [**RpcEndFinally**](/windows/win32/Rpc/?branch=master) structure is equivalent to the Windows **try-finally** statement. The RPC exception construct [**RpcTryExcept**](rpctryexcept.md) / [**RpcExcept**](/windows/win32/Rpc/nf-rpc-rpcexcept?branch=master) / [**RpcEndExcept**](/windows/win32/Rpc/?branch=master) is equivalent to the Windows **try-except** statement.

When you use the RPC exception handlers, your client-side source code is portable. The different RPC header files provided for each platform resolve the **RpcTry** and [**RpcExcept**](/windows/win32/Rpc/nf-rpc-rpcexcept?branch=master) macros for each platform. In the Windows environment, these macros map directly to the Windows **try-finally** and **try-except** statements. In other environments, these macros map to other platform-specific implementations of exception handlers.
...
While the **RpcTry** and [**RpcExcept**](/windows/win32/Rpc/nf-rpc-rpcexcept?branch=master) macros provide a customizable platform-agnostic way to handle exceptions, in Windows Vista and later versions of Windows, [**RpcExceptionFilter**](/windows/win32/Rpcdce/nf-rpcdce-rpcexceptionfilter?branch=master) is the recommended way of handling exceptions. It does not require custom filters to be written to capture many of the most common structured exceptions; however, custom exception filters still require **RpcExcept**.
