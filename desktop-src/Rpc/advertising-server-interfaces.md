...
The calls to the first two functions in this code fragment are similar to the [Hello, World example](tutorial.md). These functions make information about the binding available to the client. The calls to [**RpcServerInqBindings**](rpcserverinqbindings.md) and [**RpcNsBindingExport**](rpcnsbindingexport.md) put the information in the name service database. The call to [**RpcServerInqBindings**](rpcserverinqbindings.md) fills the binding vector with valid binding handles before the handles are exported to the name service. After the server program exports the handles to the database, the client (or client stubs) can call [**RpcNsBindingImportBegin**](rpcnsbindingimportbegin.md) and [**RpcNsBindingImportNext**](rpcnsbindingimportnext.md) to obtain this information. For details, see [Finding Server Host Systems](finding-server-host-systems.md).

The calls to [**RpcServerInqBindings**](rpcserverinqbindings.md) and [**RpcNsBindingExport**](rpcnsbindingexport.md) and their associated data structures look similar to the following:
...
Note that the [**RpcServerInqBindings**](rpcserverinqbindings.md) parameter *pBindingVector* is a pointer to a pointer to [**RPC\_BINDING\_VECTOR**](rpc-binding-vector.md). Also remember that each call to [**RpcNsBindingExport**](rpcnsbindingexport.md) must be followed by a call to [**RpcBindingVectorFree**](rpcbindingvectorfree.md).

To remove the exported interface from the name service database, the server calls [**RpcNsBindingUnexport**](rpcnsbindingunexport.md) as shown:
