...
To be safe, the RPC run time treats all calls as non-idempotent. The \[idempotent\] attribute is not supported for [**ncacn\_ip\_tcp**](https://msdn.microsoft.com/library/windows/desktop/aa367104), and is ignored. As such, the first condition in the preceding list is reduced to *the server that cannot possibly execute the call*.
