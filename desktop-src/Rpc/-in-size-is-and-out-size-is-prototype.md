...
Note the parameters that describe the array length are transmitted in the same direction as the arrays: *cbIn* and *achIn* are \[[**in**](https://msdn.microsoft.com/library/windows/desktop/aa367051)\] parameters while *pcbOut* and *achOut* are \[[**out**](https://msdn.microsoft.com/library/windows/desktop/aa367136)\] parameters. As an **\[out\]** parameter, the parameter *pcbOut* must follow C convention and be declared as a pointer.
...
The \[**string**\] attribute can be utilized when a parameter is known to be a string. This attribute directs the stub to calculate the string size, thus eliminating the overhead associated with the \[[**length is**](https://msdn.microsoft.com/library/windows/desktop/aa367164)\] parameters. Additionally, it will direct the stub to verify the string is **NULL** terminated before passing the parameter to the application.
