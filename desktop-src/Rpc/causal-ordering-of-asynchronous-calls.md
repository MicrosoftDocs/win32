...
-   If a client application selects noncausal ordering for a particular binding handle, it disables serialization for that handle. Applications enable noncausal ordering by calling [**RpcBindingSetOption**](/windows/win32/Rpcdce/nf-rpcdce-rpcbindingsetoption?branch=master) with the *Option* parameter set to RPC\_C\_OPT\_BINDING\_NONCAUSAL and the *OptionValue* parameter set to **TRUE**. For details, see [Binding Option Constants](binding-option-constants.md).
