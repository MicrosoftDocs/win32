...
The problem with this approach is that RPC return values are simply long integers. They have no special meaning as errors (note that [**error\_status\_t**](https://msdn.microsoft.com/library/windows/desktop/aa366823) has no special semantics on the server side), which carries important implications.
