...
4.  Map new parameters or new data types through a wrapper.
    ...
    A similar situation may happen when the user calls a remote API directly, but a wrapper could be introduced to do a new type mapping or some other additional actions that have become necessary. Interface Definition Language (IDL) has several ways of facilitating such remapping, namely \[[**call\_as**](https://msdn.microsoft.com/library/windows/desktop/aa366748)\], \[[**transmit\_as**](https://msdn.microsoft.com/library/windows/desktop/aa367286)\], and \[[**wire\_marshal**](https://msdn.microsoft.com/library/windows/desktop/aa367309)\]. The \[**call\_as**\] attribute introduces a function wrapper on the client and server. Both are placed between the user code and the marshaler. The other attributes deal with direct type mapping. For extension problems, \[**call\_as**\] is the most frequently used, and is easiest to understand and manipulate without pitfalls.
