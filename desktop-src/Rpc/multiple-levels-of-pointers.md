...
In this example, the stub passes the server a unique pointer, which the server initializes to **NULL**. The server then allocates a block of BARs, sets the pointer, sets the size argument and returns. Note that in order for the server to have an effect on the caller you must pass a \[ref\] pointer to a \[[**unique**](https://msdn.microsoft.com/library/windows/desktop/aa367294)\] pointer to your data. Also note the comma in \[[**size\_is**](https://msdn.microsoft.com/library/windows/desktop/aa367164)( , \*pSize )\], which indicates that the top-level pointer is not a sized pointer, but that the lower-level pointer is.
...
[**size\_is**](https://msdn.microsoft.com/library/windows/desktop/aa367164)
</dt> </dl>
