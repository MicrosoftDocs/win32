...
RPC provides you with the ability to override the default method of generating encryption keys. Your application can call the [**RpcServerRegisterAuthInfo**](rpcserverregisterauthinfo.md) function and pass it a pointer to a key acquisition function. You can write the key acquisition function so that it generates keys using any method you choose. However, the key it passes to the server program must match the format that the SSP requires.
