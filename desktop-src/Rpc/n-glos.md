...
Service that maps names to objects and stores the name/object pairs in a database. For example, the RPC name service maps a logical name to a [*binding handle*](https://www.bing.com/search?q=*binding handle*) so client applications can refer to that logical name, rather than a protocol sequence and network address. See also name service–interface daemon ([nsid)](#-rpc-nsid-glos), Client Directory Service ([*CDS)*](https://www.bing.com/search?q=*CDS)*), [*Locator*](https://www.bing.com/search?q=*Locator*).
...
Service that provides an interface between Microsoft [*Locator*](https://www.bing.com/search?q=*Locator*) and the OSF-DCE Cell Directory Service name service databases for RPC name-service functions.
