...
The server application can call [**RpcServerTestCancel**](rpcservertestcancel.md) with the binding handle of the call in question to find out if the client has requested cancellation of the asynchronous call. It will return RPC\_S\_OK if the client canceled the call.
