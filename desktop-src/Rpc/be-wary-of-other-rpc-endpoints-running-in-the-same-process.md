...
When an application resides in a process with other RPC servers, all applications listen on all protocols. As such, if a component calls [**RpcServerUseProtseq**](/windows/win32/Rpcdce/nf-rpcdce-rpcserveruseprotseq?branch=master)\* for LRPC only, it is not necessarily accessible over LRPC only. It may be accessible over other protocols, since other RPC servers in the process may be listening on pipes or sockets (for example).
