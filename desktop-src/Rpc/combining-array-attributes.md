...
-   Do not specify a \[[**first\_is**](https://docs.microsoft.com/windows/desktop/Midl/first-is)\] index value smaller than zero or a \[[**last\_is**](https://docs.microsoft.com/windows/desktop/Midl/last-is)\] value greater than \[[**max\_is**](https://docs.microsoft.com/windows/desktop/Midl/max-is)\].
-   Do not specify a negative size for an array. Define the first and last elements so that they result in a length value of zero or greater. Define the \[[**max\_is**](https://docs.microsoft.com/windows/desktop/Midl/max-is)\] value so that the size is zero or greater. If MIDL was invoked with the [**/error**](https://docs.microsoft.com/windows/desktop/Midl/-error) bounds\_check option, then the stub raises an exception when the size is less than zero, or the transmitted length is less than zero.
-   Do not use the \[[**length\_is**](https://docs.microsoft.com/windows/desktop/Midl/length-is)\] and \[[**last\_is**](https://docs.microsoft.com/windows/desktop/Midl/last-is)\] attributes at the same time, nor the \[**size\_is**\] and \[**last\_is**\] attributes at the same time.
