...
This tutorial demonstrates another use of the ACF file—specifying the type of binding handle that represents the connection between client and server. The **\[**[**implicit\_handle**](https://docs.microsoft.com/windows/desktop/Midl/implicit-handle)**\]** attribute in the ACF header allows the client application to select a server for its remote procedure call. The ACF defines the handle to be of the type [**handle\_t**](https://docs.microsoft.com/windows/desktop/Midl/handle-t) (a MIDL primitive data type). The MIDL compiler will put the binding handle name that the ACF specified, hello\_IfHandle into the header file it generates. Notice that this particular ACF file has an empty body.
