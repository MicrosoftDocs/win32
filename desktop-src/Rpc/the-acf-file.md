...
This tutorial demonstrates another use of the ACF file—specifying the type of binding handle that represents the connection between client and server. The **\[**[**implicit\_handle**](https://msdn.microsoft.com/library/windows/desktop/aa367046)**\]** attribute in the ACF header allows the client application to select a server for its remote procedure call. The ACF defines the handle to be of the type [**handle\_t**](https://msdn.microsoft.com/library/windows/desktop/aa366849) (a MIDL primitive data type). The MIDL compiler will put the binding handle name that the ACF specified, hello\_IfHandle into the header file it generates. Notice that this particular ACF file has an empty body.
