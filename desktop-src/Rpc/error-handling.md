...
| Client receives a call complete notification, but when it calls [**RpcAsyncCompleteCall**](rpcasynccompletecall.md), it receives an error code. | No RPC API calls are necessary. All RPC state has been cleaned up.                                                              |
| Client issues non-abortive or abortive cancel.                                                                                                   | Client must wait for notification, and call [**RpcAsyncCompleteCall**](rpcasynccompletecall.md) when the notification arrives. |
...
|------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Before hand-off point. | Throw exception. No call to [**RpcAsyncCompleteCall**](rpcasynccompletecall.md) is necessary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| After hand-off point.  | Call [**RpcAsyncAbortCall**](rpcasyncabortcall.md) or, if the error is not fatal and results can still be returned to the client, [**RpcAsyncCompleteCall**](rpcasynccompletecall.md). If the **RpcAsyncCompleteCall** function call fails, the RPC runtime frees the parameters. The user must not access those parameters. The dispatcher thread must not perform any substantial processing that may fail after the hand off point, because it no longer can safely abort the call. Specifically, it must not throw an exception after the hand off point, or the server may crash. |
...
| Client calls [**RpcAsyncCompleteCall**](rpcasynccompletecall.md) before the [**in**](https://msdn.microsoft.com/library/windows/desktop/aa367051) pipes are drained. | Call fails with the appropriate pipe-filling error code.                                                   |
...
| Server calls push or pull and the call fails.                                                                     | Push returns a failure code. No call to [**RpcAsyncCompleteCall**](rpcasynccompletecall.md) is necessary. |
