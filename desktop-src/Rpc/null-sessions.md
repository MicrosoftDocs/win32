...
Sometimes a call arriving on the null session can appear like an authenticated call. Specifically, calling the [**RpcBindingInqAuthClient**](rpcbindinginqauthclient.md) function returns the authentication level and security provider used for the call. This operation does not mean the call was not on a null session. The two issues are orthogonal. On Microsoft Windows 2000, a remote procedure call can attempt to impersonate a caller and check the permissions after impersonation. On Microsoft Windows XP, it is faster to call the [**RpcServerInqCallAttributes**](rpcserverinqcallattributes.md) function and check for the **NullSession** flag.
