...
All handles declared with the [handle\_t](https://msdn.microsoft.com/library/windows/desktop/aa366849) or [**RPC\_BINDING\_HANDLE**](rpc-binding-handle.md) types are primitive binding handles. You can extend the [handle\_t](https://msdn.microsoft.com/library/windows/desktop/aa366849) or [**RPC\_BINDING\_HANDLE**](rpc-binding-handle.md) types to include more or different information than the primitive handle type contains. When you do, you create a custom binding handle.

To make a custom binding handle for your distributed application, you will need to create your own data type and specify the \[ [handle](https://msdn.microsoft.com/library/windows/desktop/aa366846)\] attribute on a type definition in your IDL file. Ultimately, the stub files map custom binding handles to primitive handles.
