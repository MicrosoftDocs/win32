---
title: Enumerated Types
description: The enum declaration is not translated into \ define statements as it is by some DCE compilers, but is reproduced as a C-language enum declaration in the generated header file.
ms.assetid: dc38d3a2-cc6a-467f-a497-2a7abec07573
ms.author: windowssdkdev
ms.topic: article
ms.date: 05/31/2018
---

# Enumerated Types

The [**enum**](https://msdn.microsoft.com/library/windows/desktop/aa366818) declaration is not translated into **\#define** statements as it is by some DCE compilers, but is reproduced as a C-language **enum** declaration in the generated header file.

 

 




