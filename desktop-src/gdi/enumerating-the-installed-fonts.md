...
In some instances, an application must be able to enumerate the available fonts and select the one most appropriate for a particular operation. An application can enumerate the available fonts by calling the [EnumFonts](enumfonts.md) or [EnumFontFamilies](enumfontfamilies.md) function. These functions send information about the available fonts to a callback function that the application supplies. The callback function receives information in [LOGFONT](logfont.md) and [NEWTEXTMETRIC](newtextmetric.md) structures. (The [**NEWTEXTMETRIC**](gdi.NEWTEXTMETRIC) structure contains information about a TrueType font. When the callback function receives information about a non-TrueType font, the information is contained in a [TEXTMETRIC](textmetric.md) structure.) By using this information, an application can limit the user's choices to only those fonts that are available.
...
If an application does not supply a typeface name, the [EnumFonts](enumfonts.md) and [EnumFontFamilies](enumfontfamilies.md) functions supply information about one font in each available family. To enumerate all the fonts in a device context, the application can specify **NULL** for the typeface name, compile a list of the available typefaces, and then enumerate each font in each typeface.
...
An application can also check bits 1 and 2 in the **tmPitchAndFamily** member of the [NEWTEXTMETRIC](newtextmetric.md) structure to identify a TrueType font. If bit 1 is 0 and bit 2 is 1, the font is a TrueType font.
