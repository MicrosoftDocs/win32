...
An application can use six functions to set the text-formatting attributes for a device context: [SetBkColor](setbkcolor.md), [SetBkMode](setbkmode.md), [SetTextAlign](settextalign.md), [SetTextCharacterExtra](settextcharacterextra.md), [SetTextColor](settextcolor.md), and [SetTextJustification](settextjustification.md). These functions affect the text alignment, the intercharacter spacing, the text justification, and text and background colors. In addition, six other functions can be used to retrieve the current text formatting attributes for any device context: [GetBkColor](getbkcolor.md), [GetBkMode](getbkmode.md), [GetTextAlign](gettextalign.md), [GetTextCharacterExtra](gettextcharacterextra.md), [GetTextColor](gettextcolor.md), and [GetTextExtentPoint32](gettextextentpoint32.md).

## Text Alignment

Applications can use the [SetTextAlign](settextalign.md) function to specify how the system should position the characters in a string of text when they call one of the drawing functions. This function can be used to position headings, page numbers, callouts, and so on. The system positions a string of text by aligning a reference point on an imaginary rectangle that surrounds the string, with the current cursor position or with a point passed as an argument to one of the text drawing functions. The [**SetTextAlign**](gdi.SetTextAlign) function lets the application specify the location of this reference point. The following is a list of the possible reference point locations.
...
The following illustration shows a string of text drawn by calling the [TextOut](textout.md) function. Before drawing the text, the [SetTextAlign](settextalign.md) function was called to relocate the reference point at each one of the nine possible locations.

![illustration showing the same text nine times, one for each possible reference point location](images/csftx-04.png)

The default text alignment for a device context is the upper-left corner of the imaginary rectangle that surrounds the text. An application can retrieve the current text-alignment setting for any device context by calling the [GetTextAlign](gettextalign.md) function.

## Intercharacter Spacing

Applications can use the [SetTextCharacterExtra](settextcharacterextra.md) function to alter the intercharacter spacing for all text output operations in a specified device context. The following illustration shows a string of text drawn twice by calling the [TextOut](textout.md) function. Before drawing the text the second time, the [**SetTextCharacterExtra**](gdi.SetTextCharacterExtra) function was called to increment the intercharacter spacing.
...
Applications can use the [SetTextColor](settextcolor.md) function to set the color of text drawn in the client-area of their windows, as well as the color of text drawn on a color printer. An application can use the [SetBkColor](setbkcolor.md) function to set the color that appears behind each character and the [SetBkMode](setbkmode.md) function to specify how the system should blend the selected background color with the current color or colors on the video display.

The default text color for a display device context is black; the default background color is white; and the default background mode is OPAQUE. An application can retrieve the current text color for a device context by calling the [GetTextColor](gettextcolor.md) function. An application can retrieve the current background color for a device context by calling the [GetBkColor](getbkcolor.md) function and the current background mode by calling the [GetBkMode](getbkmode.md) function.
