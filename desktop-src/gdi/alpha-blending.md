...
Alpha blending mechanisms are invoked by calling [**AlphaBlend**](/windows/win32/WinGdi/nf-wingdi-alphablend?branch=master), which references the [**BLENDFUNCTION**](/windows/win32/Wingdi/ns-wingdi-_blendfunction?branch=master) structure.
...
Bitmaps may also be displayed with a transparency factor applied to the entire bitmap. Any bitmap format can be displayed with a global constant alpha value by setting **SourceConstantAlpha** in the [**BLENDFUNCTION**](/windows/win32/Wingdi/ns-wingdi-_blendfunction?branch=master) structure. The global constant alpha value has 256 levels of transparency, from 0 (entire bitmap is completely transparent) to 255 (entire bitmap is completely opaque). The global constant alpha value is combined with the per-pixel alpha value.
