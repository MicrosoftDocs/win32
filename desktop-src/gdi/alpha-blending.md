...
Alpha blending mechanisms are invoked by calling [**AlphaBlend**](alphablend.md), which references the [**BLENDFUNCTION**](blendfunction.md) structure.
...
Bitmaps may also be displayed with a transparency factor applied to the entire bitmap. Any bitmap format can be displayed with a global constant alpha value by setting **SourceConstantAlpha** in the [**BLENDFUNCTION**](blendfunction.md) structure. The global constant alpha value has 256 levels of transparency, from 0 (entire bitmap is completely transparent) to 255 (entire bitmap is completely opaque). The global constant alpha value is combined with the per-pixel alpha value.
