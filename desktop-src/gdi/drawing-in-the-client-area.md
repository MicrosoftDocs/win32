...
In the following example, the window procedure writes the message "Hello, Windows!" in the client area. To make sure the string is visible when the window is first created, the [**WinMain**](https://www.bing.com/search?q=**WinMain**) function calls [**UpdateWindow**](/windows/desktop/api/Winuser/nf-winuser-updatewindow) immediately after creating and showing the window. This causes a [**WM\_PAINT**](wm-paint.md) message to be sent immediately to the window procedure.
