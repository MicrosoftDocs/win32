...
You can draw at timed intervals by creating a timer with the [**SetTimer**](https://www.bing.com/search?q=**SetTimer**) function. By using a timer to send [**WM\_TIMER**](https://www.bing.com/search?q=**WM\_TIMER**) messages to the window procedure at regular intervals, an application can carry out simple animation in the client area while other applications continue running.

In the following example, the application bounces a star from side to side in the client area. Each time the window procedure receives a [**WM\_TIMER**](https://www.bing.com/search?q=**WM\_TIMER**) message, the procedure erases the star at the current position, calculates a new position, and draws the star within the new position. The procedure starts the timer by calling [**SetTimer**](https://www.bing.com/search?q=**SetTimer**) while processing the [**WM\_CREATE**](https://www.bing.com/search?q=**WM\_CREATE**) message.
