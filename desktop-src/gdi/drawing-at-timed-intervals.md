...
You can draw at timed intervals by creating a timer with the [**SetTimer**](_win32_settimer_cpp) function. By using a timer to send [**WM\_TIMER**](_win32_wm_timer_cpp) messages to the window procedure at regular intervals, an application can carry out simple animation in the client area while other applications continue running.

In the following example, the application bounces a star from side to side in the client area. Each time the window procedure receives a [**WM\_TIMER**](_win32_wm_timer_cpp) message, the procedure erases the star at the current position, calculates a new position, and draws the star within the new position. The procedure starts the timer by calling [**SetTimer**](_win32_settimer_cpp) while processing the [**WM\_CREATE**](_win32_wm_create_cpp) message.
