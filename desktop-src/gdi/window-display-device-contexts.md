...
An application can retrieve a window device context by using the [**GetWindowDC**](getwindowdc.md) or [**GetDCEx**](getdcex.md) function with the DCX\_WINDOW option specified. The function retrieves a window device context from the display device context cache. A window that uses a window device context must release it after drawing by using the [**ReleaseDC**](releasedc.md) function as soon as possible. Window device contexts are always from the cache; the CS\_OWNDC and CS\_CLASSDC class styles do not affect the device context.
