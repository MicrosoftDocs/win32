...
A *window device context* enables an application to draw anywhere in a window, including the nonclient area. Window device contexts are typically used by applications that process the [**WM\_NCPAINT**](wm-ncpaint.md) and [**WM\_NCACTIVATE**](_win32_wm_ncactivate_cpp) messages for windows with custom nonclient areas. Using a window device context is not recommended for any other purpose.

An application can retrieve a window device context by using the [**GetWindowDC**](/windows/win32/Winuser/nf-winuser-getwindowdc?branch=master) or [**GetDCEx**](/windows/win32/Winuser/nf-winuser-getdcex?branch=master) function with the DCX\_WINDOW option specified. The function retrieves a window device context from the display device context cache. A window that uses a window device context must release it after drawing by using the [**ReleaseDC**](/windows/win32/Winuser/nf-winuser-releasedc?branch=master) function as soon as possible. Window device contexts are always from the cache; the CS\_OWNDC and CS\_CLASSDC class styles do not affect the device context.
