...
In the following example, the window procedure draws a star in the minimized window. The procedure uses the [**IsIconic**](_win32_isiconic_cpp) function to determine when the window is minimized. This ensures that the star is drawn only when the window is minimized.
...
You set the class icon to **NULL** by setting the **hIcon** member of the [**WNDCLASS**](_win32_wndclass_str_cpp) structure to **NULL** before calling the [**RegisterClass**](_win32_registerclass_cpp) function for the window class.
