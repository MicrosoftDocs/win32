...
The system maintains a cache of display device contexts that it uses for common, parent, and window device contexts. The system retrieves a device context from the cache whenever an application calls the [**GetDC**](/windows/win32/Winuser/nf-winuser-getdc?branch=master) or [**BeginPaint**](/windows/win32/Winuser/nf-winuser-beginpaint?branch=master) function; the system returns the DC to the cache when the application subsequently calls the [**ReleaseDC**](/windows/win32/Winuser/nf-winuser-releasedc?branch=master) or [**EndPaint**](/windows/win32/Winuser/nf-winuser-endpaint?branch=master) function.
