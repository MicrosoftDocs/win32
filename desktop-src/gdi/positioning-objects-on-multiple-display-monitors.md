...
To center dialog boxes in a window's work area, use the DS\_CENTER style. To center the dialog box to an application window, use [**GetWindowRect**](https://www.bing.com/search?q=**GetWindowRect**). Windows automatically restricts menus and dialog boxes to a monitor. However, there can be a problem for custom menus, custom drop-down boxes, custom tool palettes, and the saved application position.
...
To save, and later restore, the position of a window when an application exits, use the [**GetWindowPlacement**](https://www.bing.com/search?q=**GetWindowPlacement**) and [**SetWindowPlacement**](https://www.bing.com/search?q=**SetWindowPlacement**) functions. However, check that the position is still valid before using it because the monitor could have been moved or removed from the system. The application displays the window on the primary monitor if the **HMONITOR** of a window is invalid.
...
If you use [**ShellExecute**](https://www.bing.com/search?q=**ShellExecute**) or [**ShellExecuteEx**](https://www.bing.com/search?q=**ShellExecuteEx**) , supply an *hWnd* so the system will open any new windows on the same monitor as the calling application.

Note that the values for the [**MINMAXINFO**](https://www.bing.com/search?q=**MINMAXINFO**) structure are slightly altered for a system with multiple monitors.
