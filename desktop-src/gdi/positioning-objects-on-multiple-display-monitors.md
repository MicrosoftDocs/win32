...
Typically, you will position an object either on the primary monitor or on a monitor that has an object already on it. To identify the monitor for a given point, rectangle, or window, use [**MonitorFromPoint**](monitorfrompoint.md), [**MonitorFromRect**](monitorfromrect.md), and [**MonitorFromWindow**](monitorfromwindow.md).

To get the coordinates for the monitor, use [**GetMonitorInfo**](getmonitorinfo.md), which provides both the work area and the entire monitor rectangle. Note that SM\_CXSCREEN and SM\_CYSCREEN always refer to the primary monitor, not necessarily the monitor that displays your application. Also, avoid SM\_xxVIRTUALSCREEN because this centers your window on the virtual screen, not a monitor.
