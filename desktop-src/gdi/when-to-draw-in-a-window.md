...
In all cases, an application can draw in a window as soon as it is created. To draw in the window, the application must first retrieve a handle to a display device context for the window. Ideally, an application carries out most of its drawing operations during the processing of [**WM\_PAINT**](wm-paint.md) messages. In this case, the application retrieves a display device context by calling the [**BeginPaint**](beginpaint.md) function. If an application draws at any other time, such as from within [**WinMain**](_win32_winmain_cpp) or during the processing of keyboard or mouse messages, it calls the [**GetDC**](getdc.md) or [**GetDCEx**](getdcex.md) function to retrieve the display DC.
