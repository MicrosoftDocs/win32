...
When the user clicks Define Clip Region , the system issues a [**WM\_COMMAND**](_win32_wm_command_cpp) message. The *wParam* parameter of this message contains an application-defined constant, IDM\_DEFINE, that indicates that the user selected this option from the menu. The application processes this input by setting a Boolean flag, fDefineRegion, as shown in the following code sample.
...
When the user presses the left button, the system issues a [**WM\_LBUTTONDOWN**](_win32_wm_lbuttondown_cpp) message. The *lParam* parameter of this message contains the cursor coordinates, which correspond to the upper left corner of a rectangle used to define the clipping region. The application processes the **WM\_LBUTTONDOWN** message, as follows.
