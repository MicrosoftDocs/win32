...
To add a font from multiple files to the system, use the [**AddFontResource**](gdi.AddFontResource) or [**AddFontResourceEx**](gdi.AddFontResourceEx) functions. The *lpszFilename* parameter in these functions must point to a string that contains the file names separated by the vertical bar or pipe ( \| ). For example, to specify abcxxxxx.pfm and abcxxxxx.pfb for a Type 1 font, use the string "abcxxxxx.pfm \| abcxxxxx.pfb."

[**AddFontResourceEx**](gdi.AddFontResourceEx) differs from [**AddFontResource**](gdi.AddFontResource) in that the application calling **AddFontResourceEx** can specify the font as private to itself or non-enumerable.
