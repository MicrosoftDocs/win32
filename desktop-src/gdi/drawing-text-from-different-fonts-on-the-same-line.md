...
In this example, the [GetTextExtentPoint32](gettextextentpoint32.md) function initializes the members of a [SIZE](size.md) structure with the length and height of the specified string. The [GetTextMetrics](gettextmetrics.md) function retrieves the overhang for the current font. Because the overhang is zero if the font is a TrueType font, the overhang value does not change the string placement. For raster fonts, however, it is important to use the overhang value.
...
The [SetTextJustification](settextjustification.md) function adds extra space to the break characters in a line of text. You can use the [GetTextExtentPoint](gettextextentpoint.md) function to determine the extent of a string, then subtract that extent from the total amount of space the line should occupy, and use the [**SetTextJustification**](gdi.SetTextJustification) function to distribute the extra space among the break characters in the string. The [SetTextCharacterExtra](settextcharacterextra.md) function adds extra space to every character cell in the selected font, including the break character. (You can use the [GetTextCharacterExtra](gettextcharacterextra.md) function to determine the current amount of extra space being added to the character cells; the default setting is zero.)

You can place characters with greater precision by using the [GetCharWidth32](getcharwidth32.md) or [GetCharABCWidths](getcharabcwidths.md) function to retrieve the widths of individual characters in a font. The [**GetCharABCWidths**](gdi.GetCharABCWidths) function is more accurate than the [**GetCharWidth32**](gdi.GetCharWidth32) function, but it can only be used with TrueType fonts.
