...
An application creates a private device context by first specifying the CS\_OWNDC window-class style when it initializes the **style** member of the [**WNDCLASS**](https://www.bing.com/search?q=**WNDCLASS**) structure and calls the [**RegisterClass**](https://www.bing.com/search?q=**RegisterClass**) function. (For more information about window classes, see [Window Classes](https://www.bing.com/search?q=Window+Classes).)
...
A *window device context* enables an application to draw anywhere in a window, including the nonclient area. Window device contexts are typically used by applications that process the [**WM\_NCPAINT**](wm-ncpaint.md) and [**WM\_NCACTIVATE**](https://www.bing.com/search?q=**WM\_NCACTIVATE**) messages for windows with custom nonclient areas. Using a window device context is not recommended for any other purpose. For more information; see [**GetWindowDC**](/windows/desktop/api/Winuser/nf-winuser-getwindowdc).
