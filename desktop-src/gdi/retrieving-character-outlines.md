...
You can use the [GetGlyphOutline](/windows/win32/Wingdi/nf-wingdi-getglyphoutlinea?branch=master) function to retrieve the outline of a glyph from a TrueType font. The glyph outline returned by the [**GetGlyphOutline**](gdi.GetGlyphOutline) function is for a grid-fitted glyph. (A grid-fitted glyph has been modified so that its bitmap image conforms as closely as possible to the original design of the glyph.) If your application requires an unmodified glyph outline, request the glyph outline for a character in a font whose size is equal to the font's em units. (To create a font with this size, set the **lfHeight** member of the [LOGFONT](/windows/win32/Wingdi/ns-wingdi-taglogfonta?branch=master) structure to the negative of the value of the **ntmSizeEM** member of the [NEWTEXTMETRIC](/windows/win32/Wingdi/ns-wingdi-tagnewtextmetrica?branch=master) structure.)

[**GetGlyphOutline**](gdi.GetGlyphOutline) returns the outline as a bitmap or as a series of polylines and splines. When an application retrieves a glyph outline as a series of polylines and splines, the information is returned in a [TTPOLYGONHEADER](/windows/win32/Wingdi/ns-wingdi-tagttpolygonheader?branch=master) structure followed by as many [TTPOLYCURVE](/windows/win32/Wingdi/ns-wingdi-tagttpolycurve?branch=master) structures as required to describe the glyph. All points are returned as [POINTFX](/windows/win32/Wingdi/ns-wingdi-tagpointfx?branch=master) structures and represent absolute positions, not relative moves. The starting point specified by the **pfxStart** member of the [**TTPOLYGONHEADER**](gdi.TTPOLYGONHEADER) structure is the point where the outline for a contour begins. The [**TTPOLYCURVE**](gdi.TTPOLYCURVE) structures that follow can be either polyline records or spline records.
