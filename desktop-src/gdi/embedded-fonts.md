...
To embed a TrueType font, an application can use the [**GetFontData**](/windows/desktop/api/Wingdi/nf-wingdi-getfontdata) function to read the font file. Setting the *dwTable* and *dwOffset* parameters of [**GetFontData**](https://msdn.microsoft.com/library/Dd144885(v=VS.85).aspx) to 0L and the *cbData* parameter to 1L ensures that the application reads the entire font file from the beginning.
