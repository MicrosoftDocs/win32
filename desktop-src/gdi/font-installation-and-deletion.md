...
An application can use a font to draw text only if that font is either resident on a specified device or installed in the system font table. The font table is an internal array that identifies all nondevice fonts that are available to an application. An application can retrieve the names of fonts currently installed on a device or stored in the internal font table by calling the [**EnumFontFamilies**](/windows/win32/Wingdi/nf-wingdi-enumfontfamiliesa?branch=master) or [**ChooseFont**](_win32_choosefont_cpp) functions.

To temporarily install a font, call [**AddFontResource**](/windows/win32/Wingdi/nf-wingdi-addfontresourcea?branch=master) or [**AddFontResourceEx**](/windows/win32/Wingdi/nf-wingdi-addfontresourceexa?branch=master). These functions load a font that is stored in a font-resource file. However, this is a temporary installation because after a reboot the font will not be present.
...
When an application finishes using an installed font, it must remove that font by calling the [**RemoveFontResource**](/windows/win32/Wingdi/nf-wingdi-removefontresourcea?branch=master) function.
