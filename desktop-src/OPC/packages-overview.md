...
keywords: ["Packaging APIs,packages", "packaging,packages", "packages,about", "packages,objects"]
---
...
Windows 7 provides the package object implementation of the [**IOpcPackage**](iopcpackage.md) interface.

A package object is instantiated to represent a package when the [**IOpcFactory::CreatePackage**](iopcfactory-createpackage.md) or [**IOpcFactory::ReadPackageFromStream**](iopcfactory-readpackagefromstream.md) method is called.

A pointer to the [**IOpcPackage**](iopcpackage.md) interface of a package object provides access to the [**IOpcRelationshipSet**](iopcrelationshipset.md) and [**IOpcPartSet**](iopcpartset.md) interface pointers of a relationship set object and a part set object, respectively.
...
A relationship set object that is retrieved by calling [**IOpcPackage::GetRelationshipSet**](iopcpackage-getrelationshipset.md) represents the Relationships part that stores package relationships. This object is an unordered set of [**IOpcRelationship**](iopcrelationship.md) interface pointers to relationship objects. Each relationship object represents one package relationship. For more information about relationship objects and relationship set objects, see the [Relationships Overview](relationships-overview.md) topic.
...
A part set object, which can only be retrieved by calling the [**IOpcPackage::GetPartSet**](iopcpackage-getpartset.md) method, represents all parts contained in the package that are not Relationships parts. The set is an unordered set of [**IOpcPart**](iopcpart.md) interface pointers to part objects. Each part object represents one part (which is not a Relationships part) in the package. For more information about part objects, see the [Parts Overview](parts-overview.md).

For information about how to create a package object, see the [Loading a Package to be Read](loading-a-package.md) how-to topic and the [**IOpcFactory::CreatePackage**](iopcfactory-createpackage.md) method.
