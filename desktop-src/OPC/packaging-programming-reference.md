---
title: Packaging API Reference
description: The topics in this section provide information about the Packaging APIs.
ms.assetid: 7ab1cc09-ce81-4f56-8adf-d8c95bf2c4cd
ms.date: 05/31/2018
ms.topic: article
ms.author: windowssdkdev
ms.prod: windows
ms.technology: desktop
---

# Packaging API Reference

The topics in this section provide information about the Packaging APIs.

## Contents



|                                                      |                                                     |
|------------------------------------------------------|-----------------------------------------------------|
| [Packaging Interfaces](packaging-interfaces.md)     | A listing of Packaging API interfaces.<br/>   |
| [Packaging Enumerations](packaging-enumerations.md) | A listing of Packaging API enumerations.<br/> |
| [Packaging Errors](packaging-errors.md)             | A listing of Packaging API errors.<br/>       |



 

## Related topics

<dl> <dt>

[Packaging](packaging.md)
</dt> <dt>

**Overviews**
</dt> <dt>

[Getting Started with the Packaging API](packaging-api-overview.md)
</dt> <dt>

[Packaging API Programming Guide](packaging-programming-guide.md)
</dt> <dt>

**Reference**
</dt> <dt>

[Packaging API Glossary](packaging-glossary.md)
</dt> <dt>

[Packaging API Samples](packaging-programming-samples.md)
</dt> </dl>

 

 





