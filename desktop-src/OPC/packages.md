---
title: Packages Fundamentals
description: The topics in this section describe using the Packaging APIs to represent and interact with packages.
ms.assetid: e3b21b21-ecd0-470c-a333-958c9944e218
ms.date: 05/31/2018
ms.topic: article
ms.author: windowssdkdev
ms.prod: windows
ms.technology: desktop
---

# Packages Fundamentals

The topics in this section describe using the Packaging APIs to represent and interact with packages.



| Topic                                                | Description                                                                        |
|------------------------------------------------------|------------------------------------------------------------------------------------|
| [Packages Overview](packages-overview.md)           | This topic describes the basics of using Packaging APIs to interact with packages. |
| [Packages How-To Topics](packages-how-to-topics.md) | Provides examples of tasks related to the package as a whole.                      |



 

## Related topics

<dl> <dt>

[Packaging API Programming Guide](packaging-programming-guide.md)
</dt> <dt>

**Overviews**
</dt> <dt>

[Getting Started with the Packaging API](packaging-api-overview.md)
</dt> <dt>

[Open Packaging Conventions Fundamentals](open-packaging-conventions-overview.md)
</dt> <dt>

**Reference**
</dt> <dt>

[Packaging API Reference](packaging-programming-reference.md)
</dt> <dt>

[Packaging API Samples](packaging-programming-samples.md)
</dt> </dl>

 

 




