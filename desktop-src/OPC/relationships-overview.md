...
Windows 7 provides the relationship object implementation of the [**IOpcRelationship**](/previous-versions/windows/desktop/api/msopc/nn-msopc-iopcrelationship) interface.

A relationship object is instantiated to represent a relationship. The object is created by calling the [**IOpcRelationshipSet::CreateRelationship**](/previous-versions/windows/desktop/api/msopc/nf-msopc-iopcrelationshipset-createrelationship) method and accessed by calling the [**IOpcRelationshipSet::GetRelationship**](/previous-versions/windows/desktop/api/msopc/nf-msopc-iopcrelationshipset-getrelationship) or [**IOpcRelationshipEnumerator::GetCurrent**](/previous-versions/windows/desktop/api/msopc/nf-msopc-iopcrelationshipenumerator-getcurrent) method.

Relationship objects are contained in a relationship set object, which represents the Relationships part where all relationships that have the same source are stored. For more information about relationship set objects, see the [Relationship Set Objects](#relationship-set-objects) section and the [**IOpcRelationshipSet**](/previous-versions/windows/desktop/api/msopc/nn-msopc-iopcrelationshipset) topic.

A pointer to the [**IOpcRelationship**](/previous-versions/windows/desktop/api/msopc/nn-msopc-iopcrelationship) interface of a relationship object provides to relationship information through methods that enable access to the following properties: relationship identifier, relationship type, source URI, target resource URI, and target mode. These properties are listed in the following table.
...
The URI of the relationship source is represented by an OPC URI object, which implements the [**IOpcUri**](/previous-versions/windows/desktop/api/msopc/nn-msopc-iopcuri) interface. The relative URI of the relationship target is represented by an implementation of the [IUri](http://go.microsoft.com/fwlink/p/?linkid=116163) interface. The following diagram shows a relationship object, the OPC URI object of the source, and the URI of the target URI that are retrieved from the relationship object.

![illustration showing a relationship object, the opc uri object, and the uri that can be retrieved from it](https://www.bing.com/search?q=illustration showing a relationship object, the opc uri object, and the uri that can be retrieved from it)

## Relationship Set Objects

Windows 7 provides the relationship set object implementation of the [**IOpcRelationshipSet**](/previous-versions/windows/desktop/api/msopc/nn-msopc-iopcrelationshipset) interface.

A relationship set object represents a Relationships part that stores all relationships that have the same source. These set objects can be accessed by calling either the [**IOpcPackage::GetRelationshipSet**](/previous-versions/windows/desktop/api/msopc/nf-msopc-iopcpackage-getrelationshipset) or [**IOpcPart::GetRelationshipSet**](/previous-versions/windows/desktop/api/msopc/nf-msopc-iopcpart-getrelationshipset) method. If the set is retrieved using **IOpcPackage::GetRelationshipSet**, it represents the Relationships part that stores package relationships. If it is retrieved using **IOpcPart::GetRelationshipSet**, the set represents the Relationships part that stores part relationships that have the same source.

A pointer to the [**IOpcRelationshipSet**](/previous-versions/windows/desktop/api/msopc/nn-msopc-iopcrelationshipset) interface of a relationship set object provides access to relationship objects that represent the relationships stored in the corresponding Relationships part, and a part content stream of the relationship markup that defines the relationships in the Relationships part.

The [**IOpcRelationshipSet**](/previous-versions/windows/desktop/api/msopc/nn-msopc-iopcrelationshipset) interface provides methods to create, delete, and get enumerators for the relationship objects contained in the set.
