...
keywords: ["Text Services Framework (TSF),embedded objects", "TSF (Text Services Framework),embedded objects", "TSF-enabled applications,embedded objects", "embedded objects"]
---
...
The TSF manager inserts an embedded object into the text stream by calling [ITextStoreACP::InsertEmbedded](https://msdn.microsoft.com/library/windows/desktop/ms538439) for an ACP-based application, or [ITextStoreAnchor::InsertEmbedded](itextstoreanchor-insertembedded.md) for an anchor-based application. When an embedded object is inserted, the application should place the **TS\_CHAR\_EMBEDDED** value at the character position (or anchor location) where the object is embedded and store the IDataObject associated with the embedded object. When [ITextStoreACP::GetText](https://msdn.microsoft.com/library/windows/desktop/ms538433) or [ITextStoreAnchor::GetText](itextstoreanchor-gettext.md) is called and an embedded object is contained within the text obtained, the **TS\_CHAR\_EMBEDDED** value indicates the presence and location of the embedded object. To obtain the embedded object, call [ITextStoreACP::GetEmbedded](https://msdn.microsoft.com/library/windows/desktop/ms538422) with the character position of the embedded object, or [ITextStoreAnchor::GetEmbedded](itextstoreanchor-getembedded.md) with the anchor location of the embedded object.
...
A text service inserts an embedded object into a context by calling [ITfRange::InsertEmbedded](https://msdn.microsoft.com/library/windows/desktop/ms628924) or [ITfInsertAtSelection::InsertEmbeddedAtSelection](https://msdn.microsoft.com/library/windows/desktop/ms628598). The text service must supply the embedded IDataObject.
