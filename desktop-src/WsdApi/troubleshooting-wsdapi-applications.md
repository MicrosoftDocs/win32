...
Applications call [**WSDCreateDeviceProxy**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxy?branch=master) to create and initialize an instance of the [**IWSDDeviceProxy**](/windows/win32/WsdClient/nn-wsdclient-iwsddeviceproxy?branch=master) interface. This device proxy object can be used to advertise services on a device and also to exchange metadata.

An application calling [**WSDCreateDeviceProxy**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxy?branch=master) always uses the following messages.

-   [Get](get--metadata-exchange--http-request-and-message.md)
-   [GetResponse](getresponse--metadata-exchange--message.md)

An application calling [**WSDCreateDeviceProxy**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxy?branch=master) sometimes uses the following messages.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxy**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxy?branch=master) with a physical device address.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxy**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxy?branch=master) with a logical device address.
...
Applications call [**WSDCreateDeviceProxyAdvanced**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxyadvanced?branch=master) to create and initialize an instance of the [**IWSDDeviceProxy**](/windows/win32/WsdClient/nn-wsdclient-iwsddeviceproxy?branch=master) interface. Unlike [**WSDCreateDeviceProxy**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxy?branch=master), **WSDCreateDeviceProxyAdvanced** has a *pDeviceAddress* parameter that is used to define the device transport address. If this transport address is specified, then logical address resolution is not required and [Resolve](resolve-message.md) and [ResolveMatches](resolvematches-message.md) messages are not generated.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxyAdvanced**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxyadvanced?branch=master) with a non-**NULL***pDeviceAddress* parameter. These procedures can also be used when *pDeviceAddress* is **NULL** and *pszDeviceId* is a physical address.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxyAdvanced**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxyadvanced?branch=master) with *pDeviceAddress* set to **NULL** and with *pszDeviceId* set to a logical address.
...
Applications calling into the [**IWSDiscoveryProvider**](/windows/win32/WsdDisco/nn-wsddisco-iwsdiscoveryprovider?branch=master) interface do not perform metadata exchange. This interface is only used for discovery. The message patterns and troubleshooting procedures are different for each method called on the **IWSDiscoveryProvider** interface.

When an application calls [**IWSDiscoveryProvider::SearchByType**](/windows/win32/WsdDisco/nf-wsddisco-iwsdiscoveryprovider-searchbytype?branch=master), a [Probe](probe-message.md) message is generated. The Probe message is sent by UDP multicast to port 3702. A [ProbeMatches](probematches-message.md) message is generated in response. The ProbeMatches message is sent by UDP unicast and it originates from port 3702.

When an application calls [**IWSDiscoveryProvider::SearchById**](/windows/win32/WsdDisco/nf-wsddisco-iwsdiscoveryprovider-searchbyid?branch=master), a [Resolve](resolve-message.md) message is generated. A Resolve message is sent by UDP multicast to port 3702. A [ResolveMatches](resolvematches-message.md) message is generated in response. The ResolveMatches is sent by UDP unicast and it originates from port 3702.

The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**IWSDiscoveryProvider::SearchByType**](/windows/win32/WsdDisco/nf-wsddisco-iwsdiscoveryprovider-searchbytype?branch=master) or [**IWSDiscoveryProvider::SearchById**](/windows/win32/WsdDisco/nf-wsddisco-iwsdiscoveryprovider-searchbyid?branch=master). Verify that the messages generated by the called API satisfy the traffic requirements.
...
If an application calls [**IWSDiscoveryProvider::SearchByAddress**](/windows/win32/WsdDisco/nf-wsddisco-iwsdiscoveryprovider-searchbyaddress?branch=master), then it is a directed discovery application. For more troubleshooting information, see [Troubleshooting Applications Using Directed Discovery](troubleshooting-applications-using-directed-discovery.md).
