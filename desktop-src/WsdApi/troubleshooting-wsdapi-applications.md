...
Applications call [**WSDCreateDeviceProxy**](wsdcreatedeviceproxy.md) to create and initialize an instance of the [**IWSDDeviceProxy**](iwsddeviceproxy.md) interface. This device proxy object can be used to advertise services on a device and also to exchange metadata.

An application calling [**WSDCreateDeviceProxy**](wsdcreatedeviceproxy.md) always uses the following messages.

-   [Get](get--metadata-exchange--http-request-and-message.md)
-   [GetResponse](getresponse--metadata-exchange--message.md)

An application calling [**WSDCreateDeviceProxy**](wsdcreatedeviceproxy.md) sometimes uses the following messages.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxy**](wsdcreatedeviceproxy.md) with a physical device address.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxy**](wsdcreatedeviceproxy.md) with a logical device address.
...
Applications call [**WSDCreateDeviceProxyAdvanced**](wsdcreatedeviceproxyadvanced.md) to create and initialize an instance of the [**IWSDDeviceProxy**](iwsddeviceproxy.md) interface. Unlike [**WSDCreateDeviceProxy**](wsdcreatedeviceproxy.md), **WSDCreateDeviceProxyAdvanced** has a *pDeviceAddress* parameter that is used to define the device transport address. If this transport address is specified, then logical address resolution is not required and [Resolve](resolve-message.md) and [ResolveMatches](resolvematches-message.md) messages are not generated.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxyAdvanced**](wsdcreatedeviceproxyadvanced.md) with a non-**NULL***pDeviceAddress* parameter. These procedures can also be used when *pDeviceAddress* is **NULL** and *pszDeviceId* is a physical address.
...
The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**WSDCreateDeviceProxyAdvanced**](wsdcreatedeviceproxyadvanced.md) with *pDeviceAddress* set to **NULL** and with *pszDeviceId* set to a logical address.
...
Applications calling into the [**IWSDiscoveryProvider**](iwsdiscoveryprovider.md) interface do not perform metadata exchange. This interface is only used for discovery. The message patterns and troubleshooting procedures are different for each method called on the **IWSDiscoveryProvider** interface.

When an application calls [**IWSDiscoveryProvider::SearchByType**](iwsdiscoveryprovider-searchbytype-method.md), a [Probe](probe-message.md) message is generated. The Probe message is sent by UDP multicast to port 3702. A [ProbeMatches](probematches-message.md) message is generated in response. The ProbeMatches message is sent by UDP unicast and it originates from port 3702.

When an application calls [**IWSDiscoveryProvider::SearchById**](iwsdiscoveryprovider-searchbyid.md), a [Resolve](resolve-message.md) message is generated. A Resolve message is sent by UDP multicast to port 3702. A [ResolveMatches](resolvematches-message.md) message is generated in response. The ResolveMatches is sent by UDP unicast and it originates from port 3702.

The following diagnostic procedures should be used (in order) to help identify problems with an application calling [**IWSDiscoveryProvider::SearchByType**](iwsdiscoveryprovider-searchbytype-method.md) or [**IWSDiscoveryProvider::SearchById**](iwsdiscoveryprovider-searchbyid.md). Verify that the messages generated by the called API satisfy the traffic requirements.
...
If an application calls [**IWSDiscoveryProvider::SearchByAddress**](iwsdiscoveryprovider-searchbyaddress.md), then it is a directed discovery application. For more troubleshooting information, see [Troubleshooting Applications Using Directed Discovery](troubleshooting-applications-using-directed-discovery.md).
