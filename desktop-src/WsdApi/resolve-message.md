...
-   A client calling the [**IWSDiscoveryProvider::SearchById**](/windows/win32/WsdDisco/nf-wsddisco-iwsdiscoveryprovider-searchbyid?branch=master) methods will send a Resolve message.
-   A client calling [**WSDCreateDeviceProxy**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxy?branch=master) may send a Resolve message if a logical device address is passed to *pszDeviceId*.
-   A client calling [**WSDCreateDeviceProxyAdvanced**](/windows/win32/WsdClient/nf-wsdclient-wsdcreatedeviceproxyadvanced?branch=master) will send a Resolve message if the function is called with the pDeviceAddress parameter set to **NULL**.
