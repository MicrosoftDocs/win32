...
You can obtain the current thread affinity for all threads of the process by calling the [**GetProcessAffinityMask**](/windows/win32/WinBase/nf-winbase-getprocessaffinitymask?branch=master) function. Use the [**SetProcessAffinityMask**](/windows/win32/WinBase/nf-winbase-setprocessaffinitymask?branch=master) function to specify thread affinity for all threads of the process. To set the thread affinity for a single thread, use the [**SetThreadAffinityMask**](/windows/win32/WinBase/nf-winbase-setthreadaffinitymask?branch=master) function. The thread affinity must be a subset of the process affinity.
...
When you specify a *thread ideal processor*, the scheduler runs the thread on the specified processor when possible. Use the [**SetThreadIdealProcessor**](setthreadidealprocessor.md) function to specify a preferred processor for a thread. This does not guarantee that the ideal processor will be chosen but provides a useful hint to the scheduler. On systems with more than 64 processors, you can use the [**SetThreadIdealProcessorEx**](/windows/win32/WinBase/nf-processthreadsapi-setthreadidealprocessorex?branch=master) function to specify a preferred processor in a specific processor group.
