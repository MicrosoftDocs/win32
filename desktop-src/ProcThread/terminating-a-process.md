...
The [**GetExitCodeProcess**](/windows/desktop/api/WinBase/nf-processthreadsapi-getexitcodeprocess) function returns the termination status of a process. While a process is executing, its termination status is STILL\_ACTIVE. When a process terminates, its termination status changes from STILL\_ACTIVE to the exit code of the process.
...
Use the [**SetProcessShutdownParameters**](/windows/desktop/api/WinBase/nf-processthreadsapi-setprocessshutdownparameters) function to specify certain aspects of the process termination at system shutdown, such as when a process should terminate relative to the other processes in the system.
...
The primary thread can avoid terminating other threads by directing them to call [**ExitThread**](/windows/desktop/api/WinBase/nf-processthreadsapi-exitthread) before causing the process to terminate (for more information, see [Terminating a Thread](terminating-a-thread.md)). The primary thread can still call [**ExitProcess**](/windows/desktop/api/WinBase/nf-processthreadsapi-exitprocess) afterwards to ensure that all threads are terminated.
...
-   The process receiving the private message calls [**ExitProcess**](/windows/desktop/api/WinBase/nf-processthreadsapi-exitprocess) to terminate its execution.

The execution of the [**ExitProcess**](/windows/desktop/api/WinBase/nf-processthreadsapi-exitprocess), [**ExitThread**](/windows/desktop/api/WinBase/nf-processthreadsapi-exitthread), [**CreateThread**](/windows/desktop/api/WinBase/nf-processthreadsapi-createthread), [**CreateRemoteThread**](/windows/desktop/api/WinBase/nf-processthreadsapi-createremotethread), and [**CreateProcess**](/windows/desktop/api/WinBase/nf-processthreadsapi-createprocessa) functions is serialized within an address space. The following restrictions apply:
...
-   The [**ExitProcess**](/windows/desktop/api/WinBase/nf-processthreadsapi-exitprocess) function does not return until there are no threads are in their DLL initialization or detach routines.
