...
A fiber can retrieve the fiber data by calling the [**GetFiberData**](https://www.bing.com/search?q=**GetFiberData**) macro. A fiber can retrieve the fiber address at any time by calling the [**GetCurrentFiber**](https://www.bing.com/search?q=**GetCurrentFiber**) macro.

## Fiber Local Storage

A fiber can use *fiber local storage* (FLS) to create a unique copy of a variable for each fiber. If no fiber switching occurs, FLS acts exactly the same as [thread local storage](thread-local-storage.md). The FLS functions ([**FlsAlloc**](/windows/desktop/api/WinBase/), [**FlsFree**](/windows/desktop/api/WinBase/), [**FlsGetValue**](/windows/desktop/api/WinBase/), and [**FlsSetValue**](/windows/desktop/api/WinBase/)) manipulate the FLS associated with the current thread. If the thread is executing a fiber and the fiber is switched, the FLS is also switched.
