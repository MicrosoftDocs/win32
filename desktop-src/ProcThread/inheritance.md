...
-   Open handles to process, thread, mutex, event, semaphore, named-pipe, anonymous-pipe, and file-mapping objects. These are returned by the [**CreateProcess**](createprocess.md), [**CreateThread**](createthread.md), [**CreateMutex**](https://msdn.microsoft.com/library/windows/desktop/ms682411), [**CreateEvent**](https://msdn.microsoft.com/library/windows/desktop/ms682396), [**CreateSemaphore**](https://msdn.microsoft.com/library/windows/desktop/ms682438), [**CreateNamedPipe**](https://msdn.microsoft.com/library/windows/desktop/aa365150), [**CreatePipe**](https://msdn.microsoft.com/library/windows/desktop/aa365152), and [**CreateFileMapping**](https://msdn.microsoft.com/library/windows/desktop/aa366537) functions, respectively.
...
-   Pseudo handles, as in the handles returned by the [**GetCurrentProcess**](getcurrentprocess.md) or [**GetCurrentThread**](getcurrentthread.md) function. These handles are valid only for the calling process.
...
-   Specify that inheritable handles are to be inherited by setting the *bInheritHandles* parameter to TRUE when calling the [**CreateProcess**](createprocess.md) function. Additionally, to inherit the standard input, standard output, and standard error handles, the **dwFlags** member of the [**STARTUPINFO**](startupinfo-str.md) structure must include STARTF\_USESTDHANDLES.
...
A child process inherits the environment variables of its parent process by default. However, [**CreateProcess**](createprocess.md) enables the parent process to specify a different block of environment variables. For more information, see [Environment Variables](environment-variables.md).

## Inheriting the Current Directory

The [**GetCurrentDirectory**](https://msdn.microsoft.com/library/windows/desktop/aa364934) function retrieves the current directory of the calling process. A child process inherits the current directory of its parent process by default. However, [**CreateProcess**](createprocess.md) enables the parent process to specify a different current directory for the child process. To change the current directory of the calling process, use the [**SetCurrentDirectory**](https://msdn.microsoft.com/library/windows/desktop/aa365530) function.
