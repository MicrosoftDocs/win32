...
The [**GetEnvironmentStrings**](https://msdn.microsoft.com/library/ms683187(v=VS.85).aspx) function returns a pointer to the environment block of the calling process. This should be treated as a read-only block; do not modify it directly. Instead, use the [**SetEnvironmentVariable**](/windows/desktop/api/WinBase/nf-winbase-setenvironmentvariable) function to change an environment variable. When you are finished with the environment block obtained from **GetEnvironmentStrings**, call the [**FreeEnvironmentStrings**](https://msdn.microsoft.com/library/ms683151(v=VS.85).aspx) function to free the block.
...
[User Environment Variables](https://msdn.microsoft.com/library/Bb776899(v=VS.85).aspx)
</dt> </dl>
