...
Applications that use the FD\_QOS event suite to monitor QOS events have access to status and error codes associated with the event, as well as updated QOS parameters (in the [**QOS**](qos.md) structure associated with the event). The following is a list of common QOS-related Windows Sockets 2 status and error codes.
...
| WSA\_QOS\_BAD\_OBJECT          | Problem with some part of the [**FLOWSPEC**](flowspec.md). |
