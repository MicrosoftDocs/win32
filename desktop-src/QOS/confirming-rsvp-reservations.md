...
The [**RSVP\_RESERVE\_INFO**](/previous-versions/windows/desktop/api/Qossp/ns-qossp-_rsvp_reserve_info) object enables a receiving application to be notified of the outcome of an RSVP reservation request. RSVP reservation confirmation is achieved by setting the **ConfirmRequest** member of the [**RSVP\_RESERVE\_INFO**](/previous-versions/windows/desktop/api/Qossp/ns-qossp-_rsvp_reserve_info) object to a nonzero value. This setting is necessary because RSVP network nodes are not required to automatically generate RSVP CONFIRMATION messages, per the RSVP specification.
