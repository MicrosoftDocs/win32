...
The [**QOS**](qos.md) structure is the all-encompassing structure for QOS-specific parameters, and is comprised of three parameters, two of which are [**FLOWSPEC**](flowspec.md) structures (one for sending, one for receiving).

The[**FLOWSPEC**](flowspec.md) structure includes flow members, which are **TokenRate**, **TokenBucketSize**, **PeakBandwidth**, **Latency**, **DelayVariation**, **ServiceType**, **MaxSduSize** (maximum packet size permitted in the traffic flow), and **MinimumPolicedSize**. When the application provides values for these parameters, the RSVP SP can characterize the service quality being requested; this enables the RSVP SP to make appropriate requests to corresponding components of quality of service.

The third parameter of the [**QOS**](qos.md) structure is the **ProviderSpecific** buffer, which is used for additional provider-specific QOS parameters that cannot be specified in the [**FLOWSPEC**](flowspec.md) structures. For more information about the **ProviderSpecific** buffer, see [Using the ProviderSpecific Buffer](using-the-providerspecific-buffer.md).

QOS parameters for the [**FLOWSPEC**](flowspec.md) structure may be common among certain application types. This commonality lends itself to the establishment of standardized QOS parameters for a given type of transmission. The RSVP SP provides such standardization, and the ease of implementation that comes with prescribed service types, through the use of QOS templates. These templates are described in detail in the [QOS Templates](qos-templates.md) section.

Further details of the [**QOS**](qos.md) structure and the [**FLOWSPEC**](flowspec.md) structure can be found in their respective API entries.
