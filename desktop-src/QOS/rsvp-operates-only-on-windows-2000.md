...
-   Windows Sockets QOS semantics are fully operational and unchanged, but RSVP message exchange is not generated from any QOS function calls. As soon QOS settings and parameters are passed, application state is equivalent to RSVP signaling completing successfully. Outgoing packets are marked according to the service level specified in the requesting [**QOS**](/windows/win32/Winsock2/ns-winsock2-_qualityofservice?branch=master) structure.
