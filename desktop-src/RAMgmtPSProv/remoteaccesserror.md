...
api_name:
- RemoteAccessError
- RemoteAccessError.ErrorType
- RemoteAccessError.OtherErrorType
- RemoteAccessError.OwningEntity
- RemoteAccessError.MessageID
- RemoteAccessError.Message
- RemoteAccessError.MessageArguments
- RemoteAccessError.PerceivedSeverity
- RemoteAccessError.ProbableCause
- RemoteAccessError.ProbableCauseDescription
- RemoteAccessError.RecommendedActions
- RemoteAccessError.ErrorSource
- RemoteAccessError.ErrorSourceFormat
- RemoteAccessError.OtherErrorSourceFormat
- RemoteAccessError.CIMStatusCode
- RemoteAccessError.CIMStatusCodeDescription
- RemoteAccessError.error_Code
- RemoteAccessError.error_Category
- RemoteAccessError.error_Type
- RemoteAccessError.error_WindowsErrorMessage
- RemoteAccessError.RemoteAccessErrorCode
api_location:
...
An extended error object that contains the error details of the RemoteAccess cmdlets. It inherits from the **MSFT\_WmiError** and extends it by adding the following fields to the error object
...
``` syntax
[Indication, Exception, UMLPackagePath("CIM::Interop"), ClassVersion("1.0.0"), dynamic, provider("RAMgmtPSProvider"), AMENDMENT]
class RemoteAccessError : MSFT_WmiError
{
  uint16 ErrorType;
  ...
  uint32 RemoteAccessErrorCode;
};
```

## Members

The **RemoteAccessError** class has these types of members:

-   [Properties](#properties)

### Properties

The **RemoteAccessError** class has these properties.
...
Qualifiers: [**ModelCorrespondence**](https://msdn.microsoft.com/library/aa393650) ("[**CIM\_Error**](cim-error.md).CIMStatusCodeDescription")
</dt> </dl>
...
This property is inherited from [**CIM\_Error**](cim-error.md).

The defined values for CIM status code are:
...
The operation cannot be carried out on this class because it has instances.
...
The operation cannot be carried out because the specified super class does not exist.
...
The operation cannot be carried out because an object already exists.
...
The WBEM Server limits have been exceeded, for example memory or connection limits.
...
Qualifiers: [**ModelCorrespondence**](https://msdn.microsoft.com/library/aa393650) ("[**CIM\_Error**](cim-error.md).CIMStatusCode")
</dt> </dl>

A human-readable description of **CIMStatusCode**. This description may extend, but must be consistent with, the definition of **CIMStatusCode.**
...
This property is inherited from [**MSFT\_WmiError**](msft-wmierror.md).
...
The identifying information of the entity or instance generating the error. If this entity is modeled in the CIM Schema, this property contains the path of the instance encoded as a string parameter. If not modeled, the property contains some identifying string that names the entity that generated the error. The path or identifying string is formatted per the **ErrorSourceFormat** property.
...
Qualifiers: [**ModelCorrespondence**](https://msdn.microsoft.com/library/aa393650) ("[**CIM\_Error**](cim-error.md).OtherErrorType")
</dt> </dl>
...
The format of the **ErrorSource** property when the value of the **ErrorSourceFormat** property is **Other** (1).

This value must be a non NULL value when the value of the **ErrorSourceFormat** is **Other** (1). For all other values of **ErrorSourceFormat**, the value of this property must be NULL.
...
Describes the error when the **ErrorType** is set to 1 ("Other").
...
Uniquely identifies the entity that owns the definition of the format of the Message described in this instance. **OwningEntity** MUST include a copyrighted, trademarked or otherwise unique name that is owned by the business entity or standards body defining the format.
...
Describes the severity of the Indication from the notifier's point of view:
...
The indication contains an informative response.
...
The user may decide if action is needed.
...
Action is needed immediately and the scope is broad. Perhaps an imminent outage to a critical resource will result.
...
Qualifiers: [**ModelCorrespondence**](https://msdn.microsoft.com/library/aa393650) ("[**CIM\_Error**](cim-error.md).ProbableCauseDescription")
</dt> </dl>

Describes the probable cause of the error.
...
Describes the recommended actions to take to resolve the error.
...
The remote access specific error code
