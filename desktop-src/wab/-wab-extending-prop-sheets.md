...
2.  Implementing [IShellPropSheetExt](https://www.bing.com/search?q=IShellPropSheetExt) and [**IWABExtInit**](/previous-versions/windows/desktop/api/Wabapi/) to expose its property sheets.
...
To expose your property sheet, you must implement the [IShellPropSheetExt](https://www.bing.com/search?q=IShellPropSheetExt) interface. You must also implement the [**IWABExtInt**](/previous-versions/windows/desktop/api/Wabapi/) interface defined in Wabapi.h. This interface has the standard [IUnknown](https://msdn.microsoft.com/windows/desktop/33f1d79a-33fc-4ce5-a372-e08bda378332) methods, as well as [**IWABExtInit::Initialize**](/previous-versions/windows/desktop/api/Wabapi/).

When the WAB is ready to create your property sheet, it retrieves your module's CLSID and calls [CoCreateInstance](https://msdn.microsoft.com/windows/desktop/7295a55b-12c7-4ed0-a7a4-9ecee16afdec) on the CLSID specifying IID\_IShellPropSheetExt to request a [IShellPropSheetExt Interface](https://www.bing.com/search?q=IShellPropSheetExt Interface) object.

The WAB calls [IUnknown::QueryInterface](https://msdn.microsoft.com/windows/desktop/54d5ff80-18db-43f2-b636-f93ac053146d) on the returned object, requesting an [**IWABExtInit**](/previous-versions/windows/desktop/api/Wabapi/) object. The WAB then calls [**Initialize**](/previous-versions/windows/desktop/api/Wabapi/) to provide your module with information about the object that is being displayed.

To add your page, the WAB calls [IShellPropSheetExt::AddPages Method](https://www.bing.com/search?q=IShellPropSheetExt::AddPages Method). In this call, you can create as many property sheets as you want by calling [CreatePropertySheetPage](https://www.bing.com/search?q=CreatePropertySheetPage) and passing the resulting **HPROPSHEETPAGE** handle and *lParam* to the WAB through the *lpfnAddPage* function.

The [IShellPropSheetExt::ReplacePage Method](https://www.bing.com/search?q=IShellPropSheetExt::ReplacePage Method) method is not supported at present and will not be invoked by the WAB.
...
When the WAB invokes [**Initialize**](/previous-versions/windows/desktop/api/Wabapi/), it will pass a pointer to a [**WABEXTDISPLAY**](/previous-versions/windows/desktop/api/Wabapi/ns-wabapi-_wabextdisplay) structure. The object being displayed is available to user's property sheet through the **lpPropObj** member of the **WABEXTDISPLAY** structure.

When your property sheet has been activated by the user, it receives a [PSN\_ACTIVE](https://www.bing.com/search?q=PSN\_ACTIVE) notification. The property sheet must then retrieve its relevant properties from the object by calling [IMAPIProp::GetProps](https://msdn.microsoft.com/windows/desktop/68b1621a-ba95-44a6-9433-56e59fafbcfe) and use these properties to populate the UI.

When your page receives a [PSN\_KILLACTIVE](https://www.bing.com/search?q=PSN\_KILLACTIVE) notification and loses focus, you must update the relevant properties on the object. This is done by calling [IMAPIProp::DeleteProps](https://msdn.microsoft.com/windows/desktop/7825c1aa-c5a8-4211-b782-a82846cbb7c6) to clear any properties that have been deleted by you and calling [IMAPIProp::SetProps](https://msdn.microsoft.com/windows/desktop/2d255c31-b855-4bf0-9e64-5a1401b7d992) to set any new data added. You thereby ensure that the data is accessible to all other property sheets at all times. At no point should your property page call [IMAPIProp::SaveChanges](https://msdn.microsoft.com/windows/desktop/CD0D7010-EA53-4053-87C7-92E4EA6454C3) on the object, as this might cause unintended data loss. The WAB determines if and when to persist any changes made to the object.
...
Register your property sheet extension module with the system just as you would register a [Shell Property Sheet Extension module](https://www.bing.com/search?q=Shell Property Sheet Extension module).

With the WAB, you can create property sheet extensions for Contacts ([**IMailUser**](/previous-versions/windows/desktop/api/wabdefs/) objects), Distribution Lists ([**IDistList**](/previous-versions/windows/desktop/api/wabdefs/) objects), or both.

### Registering for Contacts

To register property sheet extension under the WAB for extending the UI on Contacts ([**IMailUser**](/previous-versions/windows/desktop/api/wabdefs/) objects), place this information in the registry:
...
If you want to add a property sheet that will only be shown within your own process, identify your process to the WAB. To do this, pass the GUID used for registering the property sheet extension into the **guidPSExt** member of the [**WAB\_PARAM**](/previous-versions/windows/desktop/api/Wabapi/ns-wabapi-_tagwab_param) structure passed into [**WABOpen**](/previous-versions/windows/desktop/api/Wabapi/nc-wabapi-wabopen). When the WAB needs to load property sheets, it will load your extension only if the GUID matches and the Value-data is "1". The WAB will automatically load all extensions with value data "0".

### Registering for Distribution Lists

To register your property sheet extension for extending the UI on Groups ([**IDistList**](/previous-versions/windows/desktop/api/wabdefs/) objects), register the same information as for the property sheet extension under the following key.
