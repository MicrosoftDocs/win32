...
Pointer to a variable of type [FLAGLIST](fdc53a19-38be-4ee9-a092-a085196bdde5) that receives an array of input-output flags. Each flag corresponds to an entry in *lpAdrList* and provides the name resolution status for that particular entry. The flags in the *lpFlagList* parameter are in the same order as the entries in *lpAdrList*. The following flags can be set:
