...
keywords: ["DeleteProps method Windows Address Book", "DeleteProps method Windows Address Book , IMailUser interface", "IMailUser interface Windows Address Book , DeleteProps method"]
topic_type:
- apiref
api_name:
...
Pointer to a variable of type [**SPropTagArray**](-wab-sproptagarray.md) that specifies the properties to be deleted. The cValues member of the **SPropTagArray** structure to which *lpPropTagArray* points must not be zero, and the *lpPropTagArray* parameter itself must not be **NULL**.
...
Address of a pointer to a variable of type [**SPropProblemArray**](-wab-spropproblemarray.md) that receives any problems that occur. Must be freed by the client if a non-**null** value is returned.
