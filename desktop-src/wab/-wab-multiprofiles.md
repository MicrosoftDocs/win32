...
- Windows Address Book (WAB),what's new
- WAB (Windows Address Book),what's new
...
When a user creates a folder, the WAB creates an [IABContainer : IMAPIContainer](https://msdn.microsoft.com/windows/desktop/d495fd99-2d3a-44ef-9415-34c7c0272f12) object. Thus, a user's profile consists of one or more containers. Applications can treat each container individually or can access multiple container contents simultaneously.
...
To take advantage of multiple-user support, an application calls [**WABOpen**](/previous-versions/windows/desktop/api/Wabapi/nc-wabapi-wabopen) and sets the **WAB\_ENABLE\_PROFILES** flag in the **ulFlags** member of the [**WAB\_PARAM**](/previous-versions/windows/desktop/api/Wabapi/ns-wabapi-_tagwab_param) structure that is passed into **WABOpen**.
...
| [**GetPAB**](/previous-versions/windows/desktop/api/Wabiab/)                                      | Returns the EntryID of the default WAB container. A single container represents the entire WAB.                    | Returns the EntryID of the default folder corresponding to the current identity. If an application manipulates data in this container, the data go directly into the current identity's default folder location.                                           | Returns the EntryID of the top level "Shared Contacts" folder. The WAB does not know who is using it, so the data go to a common place where all identities will be able to see it.          |
...
| [**ResolveName**](/previous-versions/windows/desktop/api/Wabiab/)                            | Resolves names first against the complete WAB and then the LDAP contents.                                          | Resolves names against the current user's set of folders, the shared folders, and finally the LDAP contents.                                                                                                                                               | Resolves names against the full WAB, followed by LDAP.                                                                                                                                       |
...
| [**GetMe**](/previous-versions/windows/desktop/api/Wabapi/)[**SetMe**](/previous-versions/windows/desktop/api/Wabapi/) | Acts upon one ME entry per WAB                                                                                     | Manipulates a per-identity ME entry. Each identity can specify its own ME.                                                                                                                                                                                 | Returns/Sets ME entry for the default identity.                                                                                                                                              |
...
A client can enumerate all the containers within the WAB by obtaining a Root Table. The Root Table is a MAPITABLE that contains a list of all the containers within the WAB. The Root object can be obtained by calling [**OpenEntry**](/previous-versions/windows/desktop/api/Wabiab/) with a **NULL** EntryID. The client then calls [**GetContentsTable**](-wab-iabcontainer-getcontentstable.md) on the Root Object to obtain the Root Table.
...
Note that the default WAB container is the same as the container that is directly accessed through the [**GetPAB**](/previous-versions/windows/desktop/api/Wabiab/) method.
...
-   The [**IABContainer**](/previous-versions/windows/desktop/api/Wabdefs/) interface works with all folder containers, instead of working only with the default WAB container.
-   [**GetPAB**](/previous-versions/windows/desktop/api/Wabiab/) returns the EntryID of the default WAB container. For a WAB that is identity-aware, this container corresponds to the current user's default folder. For identity-aware applications running in non-identity mode, this container corresponds to the "Shared Contacts" folder. For the non-identity WAB, this container corresponds to the entire WAB.
-   When calling [**NewEntry**](/previous-versions/windows/desktop/api/Wabiab/), the caller can specify a container EntryID. The new entry will be created in that particular folder or container.
-   If you call [**ResolveName**](/previous-versions/windows/desktop/api/Wabiab/) with profile support turned off, the name resolution process works as before. Calling **ResolveName** with profiles turned on causes the name resolution to search containers linked to the current profile first. If no matches are found in the profile's container list, the name resolution process searches the entire address book, followed by appropriate LDAP servers.
-   [**CreateEntry**](/previous-versions/windows/desktop/api/Wabdefs/) creates the new entry in the folder represented by the current Container.
