...
The [**ExitProcess**](https://msdn.microsoft.com/windows/desktop/c26dbf15-62e8-4892-b7c5-2e6c085e4cd5) function in the **\_\_except** block automatically releases virtual memory allocations, so it is not necessary to explicitly free the pages when the program terminates through this execution path. The [**VirtualFree**](https://www.bing.com/search?q=**VirtualFree**) function frees the reserved and committed pages if the program is built with exception handling disabled. This function uses **MEM\_RELEASE** to decommit and release the entire region of reserved and committed pages.
