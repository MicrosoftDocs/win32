---
Description: The ActivityLogging property retrieves the FaxActivityLogging configuration object.
ms.assetid: 979bcd48-fccc-4b47-9a6b-7d6b697a61a0
title: FaxLoggingOptions.ActivityLogging property
ms.technology: desktop
ms.prod: windows
ms.author: windowssdkdev
ms.topic: article
ms.date: 05/31/2018
---

# FaxLoggingOptions.ActivityLogging property

The **ActivityLogging** property retrieves the [**FaxActivityLogging**](-mfax-faxactivitylogging.md) configuration object.

This property is read-only.

## Syntax


```VB
Property ActivityLogging As IFaxActivityLogging
```



## Property value

A variable of type [**IFaxActivityLogging**](/previous-versions/windows/desktop/api/FaxComex/nn-faxcomex-ifaxactivitylogging) that receives a [**FaxActivityLogging**](-mfax-faxactivitylogging.md) object.

## Requirements



|                                     |                                                                                         |
|-------------------------------------|-----------------------------------------------------------------------------------------|
| Minimum supported client<br/> | Windows XP \[desktop apps only\]<br/>                                             |
| Minimum supported server<br/> | Windows Server 2003 \[desktop apps only\]<br/>                                    |
| Header<br/>                   | <dl> <dt>FaxComex.h</dt> </dl>   |
| DLL<br/>                      | <dl> <dt>Fxscomex.dll</dt> </dl> |



## See also

<dl> <dt>

[Visual Basic Example](-mfax-setting-logging-options.md)
</dt> <dt>

[**FaxLoggingOptions**](-mfax-faxloggingoptions.md)
</dt> <dt>

[**IFaxLoggingOptions**](/previous-versions/windows/desktop/api/FaxComex/nn-faxcomex-ifaxloggingoptions)
</dt> </dl>

 

 




