...
The **Descriptor** property represents the security descriptor, which contains the rights explicitly granted to a user by the fax administrator. The [**GrantedRights**](-mfax-faxsecurity-grantedrights-vb.md) property reflects the user rights that the fax server grants based on the descriptor. Specifically, if a user has the access right [****farSUBMIT\_HIGH****](/previous-versions/windows/desktop/api/FaxComex/ne-faxcomex-fax_access_rights_enum), the user can send high-priority, normal-priority and low-priority faxes. If a user has the access right [****farSUBMIT\_NORMAL****](/previous-versions/windows/desktop/api/FaxComex/ne-faxcomex-fax_access_rights_enum), the user can send normal-priority and low-priority faxes.
...
[**IFaxSecurity::Descriptor**](/previous-versions/windows/desktop/api/FaxComex/nf-faxcomex-ifaxsecurity-get_descriptor)
</dt> </dl>
