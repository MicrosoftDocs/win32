...
If the setting 'All incoming faxes are viewable by everyone' is true (see [**IncomingFaxesArePublic**](/previous-versions/windows/desktop/api/Faxcomex/nf-faxcomex-ifaxconfiguration-get_incomingfaxesarepublic)) or if [****far2MANAGE\_RECEIVE\_FOLDER****](/previous-versions/windows/desktop/api/FaxComex/ne-faxcomex-fax_access_rights_enum_2) access rights, then the fax service enumerates all faxes in the Incoming archive of the Fax Server Receive Folder.
