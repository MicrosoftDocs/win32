...
For example, change the [**RingsBeforeAnswer**](/previous-versions/windows/desktop/api/FaxComex/) property of the third device from a collection of devices. Use the **Item()** property to access the third device. This can be expressed in Visual Basic in the following four ways (the last two forms are unique to Visual Basic; other languages do not have equivalents):
...
-or- (the alternative syntax for the [**RingsBeforeAnswer**](/previous-versions/windows/desktop/api/FaxComex/) property is also shown)
