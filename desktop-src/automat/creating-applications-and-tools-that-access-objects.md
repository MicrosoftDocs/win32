...
You can use the [**IDispatch**](idispatch.md) interface to access ActiveX objects, or you can access objects directly through the VTBL. Because VTBL references can be bound at compile time, VTBL access is generally faster than through **IDispatch**. Whenever possible, use the **ITypeInfo** or [**ITypeInfo2**](itypeinfo2.md) interfaces to get information about an object, including the VTBL addresses of the object's members. Then, use this information to access ActiveX objects through the VTBL.

To create compilers and other programming tools that use information from type libraries, use the **ITypeComp** interface. This interface binds to exposed objects at compile time. For details on [**ITypeComp**](itypecomp.md) see [Type Description Interfaces](387D44B7-407B-44A9-9239-A4CB20E88CAC).
