...
As in the previous example, the code calls [**GetIDsOfNames**](/previous-versions/windows/desktop/api/oaidl/nf-oaidl-idispatch-getidsofnames) for the DISPID of the "On" property, and then passes the ID to [**Invoke**](/previous-versions/windows/desktop/api/oaidl/nf-oaidl-idispatch-invoke). Then, **Invoke** returns the property's value in pVarResult. In general, the return value does not set VT\_BYREF. However, this bit may be set and a pointer returned to the return value, if the lifetime of the return value is the same as that of the object.
