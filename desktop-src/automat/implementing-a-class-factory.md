...
-   [**CreateInstance**](itypeinfo-createinstance.md) — Creates an instance of the object's class.
...
The object implements two interfaces: **IUnknown** and IMyInterface. The interface IMyInterface is a [dual](dual.md) interface, which supports both late binding through [**IDispatch**](idispatch.md), and early binding through the VTBL. The *dual* interface provides two ways to invoke the object's methods and properties. **IDispatch** includes the member functions [**GetIDsOfNames**](idispatch-getidsofnames.md), [**GetTypeInfo**](idispatch-gettypeinfo.md), [**GetTypeInfoCount**](idispatch-gettypeinfocount.md), and [**Invoke**](idispatch-invoke.md).
...
You must decide how to handle errors that occur in the exposed objects. If an object supports a *dual* interface and needs to return detailed, contextual error information, you also need to implement the Automation error interface, [**IErrorInfo**](ierrorinfo.md).
