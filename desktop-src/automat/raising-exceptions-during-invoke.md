...
When you implement [**Invoke**](idispatch-invoke.md), errors can be communicated either through the normal return value or by raising an exception. An exception is a special situation that is normally handled by jumping to the nearest routine enclosing the exception handler.
...
If you consider [**IDispatch**](idispatch.md) another way to call C++ methods in an interface, EXCEPINFO models the raising of an exception or **longjmp()** call by such a method.
