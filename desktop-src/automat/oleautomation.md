...
| **IDispatch \***                            | Pointer to the[**IDispatch**](/windows/previous-versions/oaidl/nn-oaidl-idispatch?branch=master) interface (VT\_DISPATCH).                                                                                           |
| **IUnknown \***                             | Pointer to an interface that does not derive from [**IDispatch**](/windows/previous-versions/oaidl/nn-oaidl-idispatch?branch=master) (VT\_UNKNOWN). (Any OLE interface can be represented by its IUnknowninterface.) |
| **dispinterface** Typename \*               | Pointer to an interface derived from[**IDispatch**](/windows/previous-versions/oaidl/nn-oaidl-idispatch?branch=master)(VT\_DISPATCH).                                                                                |
...
An interface is compatible with Automation if it derives from [**IDispatch**](/windows/previous-versions/oaidl/nn-oaidl-idispatch?branch=master) or IUnknown, if it has the oleautomation attribute, and if all of its VTBL entries are compatible with Automation. For 32-bit systems, the calling convention for all methods in the interface must be STDCALL. For 16-bit systems, all methods must have the CDECL calling convention. Every dispinterface is compatible with Automation.
