...
This statement defines a set of properties and methods on which [IDispatch::Invoke](https://msdn.microsoft.com/windows/desktop/964ADE8E-9D8A-4D32-BD47-AA678912A54D) can be called. A dispinterface can be defined by explicitly listing the set of supported methods and properties (Syntax 1) or by listing a single interface (Syntax 2).
...
(Syntax 2) The name of the interface to declare as an [**IDispatch**](/previous-versions/windows/desktop/api/oaidl/nn-oaidl-idispatch) interface.
...
The properties and methods of a dispinterface are not part of the VTBL of the dispinterface. Consequently, [**CreateStdDispatch**](/previous-versions/windows/desktop/api/OleAuto/nf-oleauto-createstddispatch) and [**DispInvoke**](/previous-versions/windows/desktop/api/OleAuto/nf-oleauto-dispinvoke) cannot be used to implement [**IDispatch::Invoke**](/previous-versions/windows/desktop/api/oaidl/nf-oaidl-idispatch-invoke). The dispinterface is used when an application needs to expose existing non-VTBL functions through Automation. These applications can implement **IDispatch::Invoke** by examining the *dispidMember* parameter and directly calling the corresponding function.
