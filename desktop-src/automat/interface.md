...
Because the functions described by the interface statement are in the VTBL, [**DispInvoke**](/previous-versions/windows/desktop/api/OleAuto/nf-oleauto-dispinvoke) and [**CreateStdDispatch**](/previous-versions/windows/desktop/api/OleAuto/nf-oleauto-createstddispatch) can be used to provide an implementation of [**IDispatch::Invoke**](/previous-versions/windows/desktop/api/oaidl/nf-oaidl-idispatch-invoke). For this reason, interface is more commonly used than dispinterface to describe the properties and methods of an object.
...
A dual interface must derive from [**IDispatch**](/previous-versions/windows/desktop/api/oaidl/nn-oaidl-idispatch).
