...
When a service is started in response to a trigger event, the service receives **SERVICE\_TRIGGER\_STARTED\_ARGUMENT** as *argv*\[1\] in its [*ServiceMain*](https://msdn.microsoft.com/library/ms685138(v=VS.85).aspx) callback function. *Argv*\[0\] is always the short name of the service.
