...
**Windows Server 2003:** On a computer that has multiple network interfaces (that is, a multihomed computer), delays up to 60 seconds may occur when using UNC paths to access files that are stored on a remote server message block (SMB) server. For more information, see [article 890553](http://go.microsoft.com/fwlink/p/?linkid=100321) in the Help and Support Knowledge Base.
