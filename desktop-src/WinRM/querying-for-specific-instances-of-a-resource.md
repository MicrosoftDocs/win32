...
This procedure executes a query for network adapters that have TCP/IP bound and enabled. The query requests all the instances of [**Win32\_NetworkAdapterConfiguration**](https://msdn.microsoft.com/library/aa394217) that have the **IpEnabled** property set to **True**. Except for the addition of the *filter* and *dialect*, the query is handled like a simple enumeration.

In this example, the resource name for the Resource constant is represented by an asterisk "\*" because the class name, [**Win32\_NetworkAdapterConfiguration**](https://msdn.microsoft.com/library/aa394217), is already mentioned in the *strFilter* string.
...
3.  Construct the filter string. Windows Remote Management supports [WQL](https://msdn.microsoft.com/library/aa394606) as the filter dialect.
