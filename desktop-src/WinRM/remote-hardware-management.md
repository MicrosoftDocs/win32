...
Original Equipment Manufacturers (OEMs) have developed a common architecture to address the need for hardware management. An important piece of this architecture is the [*baseboard management controller*](https://www.bing.com/search?q=*baseboard management controller*) (BMC). A BMC is a specialized device that monitors the state of the server computer. The BMC provides remote control of server hardware, retrieves status data, and receives notifications about critical errors and other hardware status changes. A script or application that is monitoring a remote server can obtain data from the server either [*in-band*](https://www.bing.com/search?q=*in-band*), through the remote operating system, or [*out-of-band*](https://www.bing.com/search?q=*out-of-band*), directly from the BMC.
