---
title: Hardware Counter Profiling
description: Write programs to isolate bottlenecks and improve performance by collecting data from hardware performance counters configured on the system. Capture thread profiling of cycle time and reasons for context switches.
ms.assetid: 7e88ec40-7a88-4aeb-ad0f-8f95144b2faf
ms.date: 05/31/2018
ms.topic: article
ms.author: windowssdkdev
ms.prod: windows
ms.technology: desktop
---

# Hardware Counter Profiling

## Purpose

Applications use the Hardware Counter Profiling (HCP) SDK to capture thread profiling data such as cycle time and the reasons for context switches. You can also use HCP to capture counter data for hardware performance counters that you have configured on the system.

## Developer audience

HCP is designed for C and C++ developers.

## Run-time requirements

HCP is included in the Windows 7 and Windows Server 2008 R2 operating systems.

For information about run-time requirements for a particular programming element, see the Requirements section of the reference page for that element.

## In this section



| Topic                                         | Description                                                         |
|-----------------------------------------------|---------------------------------------------------------------------|
| [HCP Reference](hcp-reference.md)<br/> | Reference information for the HCP programming interface.<br/> |



 

 

 





