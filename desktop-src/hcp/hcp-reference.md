...
You can enable profiling for one or more threads. To enable profiling for a thread, you must call the [**EnableThreadProfiling**](/windows/desktop/api/Winbase/nf-winbase-enablethreadprofiling) function from the thread itself—you cannot enable thread B from thread A.
...
After calling [**EnableThreadProfiling**](/windows/desktop/api/Winbase/nf-winbase-enablethreadprofiling) to enable profiling on the thread, you would call the [**ReadThreadProfilingData**](/windows/desktop/api/Winbase/nf-winbase-readthreadprofilingdata) function before and after areas of interest in your code (for example, before and after a function call) to help isolate bottlenecks or other issues in your code. Before exiting the thread, call the [**DisableThreadProfiling**](/windows/desktop/api/Winbase/nf-winbase-disablethreadprofiling) function to release system resources.
