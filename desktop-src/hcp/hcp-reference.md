...
You can enable profiling for one or more threads. To enable profiling for a thread, you must call the [**EnableThreadProfiling**](enablethreadprofiling.md) function from the thread itself—you cannot enable thread B from thread A.
...
After calling [**EnableThreadProfiling**](enablethreadprofiling.md) to enable profiling on the thread, you would call the [**ReadThreadProfilingData**](readthreadprofilingdata.md) function before and after areas of interest in your code (for example, before and after a function call) to help isolate bottlenecks or other issues in your code. Before exiting the thread, call the [**DisableThreadProfiling**](disablethreadprofiling.md) function to release system resources.
