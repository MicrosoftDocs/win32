...
Type of transfer job, such as BG\_JOB\_TYPE\_DOWNLOAD. For a list of transfer types, see the [**BG\_JOB\_TYPE**](bg-job-type.md) enumeration.
...
An [**IBackgroundCopyJob**](delivery_optimization-ibackgroundcopyjob) interface pointer that you use to modify the job's properties and specify the files to be transferred. To activate the job in the queue, call the [**IBackgroundCopyJob::Resume**](ibackgroundcopyjob-resume.md) method. Release *ppJob* when done.
...
|------------------------------------------------------------------------------------------|------------------------------------------------|
| <dl> <dt>****S\_OK****</dt> </dl> | Successfully generated the new job.<br/> |
...
Only the user who creates the job or a user with administrator privileges can [add files to the job](delivery_optimization-adding_files_to_a_job) and [change the job's properties](delivery_optimization-setting_and_retrieving_the_properties_of_a_job).
