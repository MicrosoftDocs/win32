...
Use the [**IBackgroundCopyJob**](delivery_optimization-ibackgroundcopyjob) interface to add files to the job, set the priority level of the job, determine the state of the job, and to start and stop the job.

To create a job, call the [**IBackgroundCopyManager::CreateJob**](ibackgroundcopymanager-createjob.md) method. To get an [**IBackgroundCopyJob**](delivery_optimization-ibackgroundcopyjob) interface pointer to an existing job, call the [**IBackgroundCopyManager::GetJob**](ibackgroundcopymanager-getjob.md) method.

## Members

The **IBackgroundCopyJob** interface inherits from the [**IUnknown**](https://msdn.microsoft.com/library/windows/desktop/ms680509) interface. **IBackgroundCopyJob** also has these types of members:
