...
3.  If the process did not call [**SetProcessWindowStation**](https://msdn.microsoft.com/en-us/library/ms686232(v=VS.85).aspx) and did not inherit a window station, the system attempts to open for MAXIMUM\_ALLOWED access and connect to a window station as follows:
    -   If a window station name was specified in the **lpDesktop** member of the [**STARTUPINFO**](https://docs.microsoft.com/windows/desktop/api/processthreadsapi/ns-processthreadsapi-startupinfoa) structure that was used when the process was created, the process connects to the specified window station.
