...
1.  If the process has called the [**SetProcessWindowStation**](/windows/desktop/api/Winuser/nf-winuser-setprocesswindowstation) function, it connects to the window station specified in that call.
2.  If the process did not call [**SetProcessWindowStation**](/windows/desktop/api/Winuser/nf-winuser-setprocesswindowstation), it connects to the window station inherited from the parent process.
3.  If the process did not call [**SetProcessWindowStation**](/windows/desktop/api/Winuser/nf-winuser-setprocesswindowstation) and did not inherit a window station, the system attempts to open for MAXIMUM\_ALLOWED access and connect to a window station as follows:
...
The window station assigned during this connection process cannot be closed by calling the [**CloseWindowStation**](/windows/desktop/api/Winuser/nf-winuser-closewindowstation) function.
