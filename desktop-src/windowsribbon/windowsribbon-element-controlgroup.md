...
<td>No<br/></td>
<td>Valid only when [<strong>Group</strong>](windowsribbon-element-group.md) is the parent element.<br/> Each <em>SequenceNumber</em> must be unique within a [<strong>Group</strong>](windowsribbon-element-group.md) element. The values for <em>SequenceNumber</em> should increase for each <strong>Group</strong> element, but do not need to be sequential. <br/> <br/>
