...
6.  Each page added by the property page objects will result in your callback function being called with the handle to the property page and the application-defined value. Your callback function must store each property page handle that is passed. When the [**CLSID\_DsPropertyPages**](guids-of-user-interface-elements.md) object's [**IShellPropSheetExt::AddPages**](_win32_ishellpropsheetext_win32_ishellpropsheetext_addpages_cpp) method returns, all pages will have been added via your callback function.
7.  Fill in a [**PROPSHEETHEADER**](_win32_propsheetheader_cpp) structure to display the property sheet. The **phpage** member receives a pointer to an array of page handles that were collected by your callback function. The **nPages** member receives the number of pages in the page handle array.
8.  Display the property sheet by calling the [**PropertySheet**](_win32_propertysheet_cpp) function.
...
2.  Convert each class identifier string into a **CLSID** using the [**CLSIDFromString**](_com_stringfromclsid) function.
...
5.  For each extension, in the order sorted above, call the extension's [**IShellExtInit::Initialize**](_win32_ishellextinit_win32_ishellextinit_initialize_cpp) with the same information described in Step 4 of the Hosting an Active Directory Users and Computers Property Sheet procedure.
6.  For each extension, in the order sorted above, call the extension's [**IShellPropSheetExt::AddPages**](_win32_ishellpropsheetext_win32_ishellpropsheetext_addpages_cpp) with the same information described in Step 5 of the Hosting an Active Directory Users and Computers Property Sheet procedure.
