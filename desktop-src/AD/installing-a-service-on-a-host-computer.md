...
1.  Calls the [**OpenSCManager**](https://msdn.microsoft.com/library/windows/desktop/ms684323) function to open a handle to the service control manager (SCM) on the local computer.
2.  Calls the [**CreateService**](https://msdn.microsoft.com/library/windows/desktop/ms682450) function to install the service in the SCM database. This call specifies the service's logon account and password, as well as the service's executable and other information about the service. **CreateService** fails if the specified logon account is invalid. However, **CreateService** does not check the validity of the password. It also does not verify that the account has the logon as a service right on the local computer. For more information, see [Granting Logon as Service Right on the Host Computer](granting-logon-as-service-right-on-the-host-computer.md).
