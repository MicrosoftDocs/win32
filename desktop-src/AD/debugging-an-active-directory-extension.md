...
You should now be able to set breakpoints and view debug output. If the extension does not appear to load, set a breakpoint in the extension's [**DllGetClassObject**](https://msdn.microsoft.com/en-us/library/ms680760(v=VS.85).aspx) function. If **DllGetClassObject** is not called, the extension is probably not registered correctly.
