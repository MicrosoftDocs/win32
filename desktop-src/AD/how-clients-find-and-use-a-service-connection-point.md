...
keywords: ["service connection points AD ,how clients find and use"]
---
...
This sample consists of two routines. The **GetGC** routine retrieves an [**IDirectorySearch**](https://msdn.microsoft.com/library/aa746362) pointer for a Global Catalog (GC). The **ScpLocate** routine uses the **IDirectorySearch** methods to search the GC.

The GC contains a partial replica of every object in the forest, but does not contain all of the SCP attributes that the client requires. First, the client must search the GC to find the SCP and retrieve its DN. Then the client uses the SCP's DN to bind to an [**IDirectoryObject**](https://msdn.microsoft.com/library/aa746355) pointer on the SCP. The client then calls the [**IDirectoryObject::GetObjectAttributes**](https://msdn.microsoft.com/library/aa746358) method to retrieve the rest of the attributes.
