...
The **CQPM\_GETPARAMETERS** message is sent to the [**CQPageProc**](/windows/win32/Cmnquery/nc-cmnquery-lpcqpageproc?branch=master) callback function of a query form extension page to retrieve data about the query performed by the page.
...
Pointer to a [**LPDSQUERYPARAMS**](/windows/win32/Dsquery/ns-dsquery-dsqueryparams?branch=master) value that receives data about the query performed by the page. If this parameter is **NULL**, the **DSQUERYPARAMS** structure must be allocated by the extension using the [**CoTaskMemAlloc**](_com_cotaskmemalloc) function.
