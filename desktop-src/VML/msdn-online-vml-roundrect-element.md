---
title: VML RoundRect Element
description: VML RoundRect Element
ms.assetid: 9ed36aa0-6cb1-4522-80c8-7fb81833da7d
ms.topic: article
ms.date: 05/31/2018
---

# VML RoundRect Element

This topic describes VML, a feature that is deprecated as of Windows Internet Explorer 9. Webpages and applications that rely on VML should be [migrated to SVG](https://go.microsoft.com/fwlink/p/?LinkID=236964) or other widely supported standards.

> [!Note]  
> As of December 2011, this topic has been archived. As a result, it is no longer actively maintained. For more information, see [Archived Content](https://docs.microsoft.com/previous-versions/windows/internet-explorer/ie-developer/). For information, recommendations, and guidance regarding the current version of Windows Internet Explorer, see [Internet Explorer Developer Center](https://go.microsoft.com/fwlink/p/?linkid=204313).

 

Predefined round rectangle shape.

**RoundRect** uses the [ArcSize](msdn-online-vml-arcsize-attribute.md) attribute.

**Remarks**

The following is the minimum code needed to produce a round rect.


```HTML
   <v:roundrect
   fillcolor="green"
   style="top:1;left:1;width:50;height:50">
   </v:roundrect>
```



[Show Me](https://samples.msdn.microsoft.com/workshop/samples/vml/shape/predef/t_roundrect.md)

 

 




