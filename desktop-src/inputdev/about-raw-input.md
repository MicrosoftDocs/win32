...
Through the **dwFlags** member of [**RAWINPUTDEVICE**](/windows/desktop/api/Winuser/nf-winuser-getrawinputdeviceinfoa), an application can select the devices to listen to and also those it wants to ignore. For example, an application can ask for input from all telephony devices except for answering machines. For sample code, see [Registering for Raw Input](using-raw-input.md#registering-for-raw-input).
