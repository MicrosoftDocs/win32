...
However, to develop an application or service provider that can handle DRM-protected content, you must request one or more certificates (each with a key) from the [Windows Media Licensing Page](http://go.microsoft.com/fwlink/p/?linkid=8076). Certificates are required for the following applications or objects:
