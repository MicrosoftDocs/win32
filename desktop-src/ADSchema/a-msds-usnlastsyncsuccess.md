---
title: ms-DS-USN-Last-Sync-Success attribute
description: The USN at which the last successful replication synchronization occurred.
ms.assetid: 445cfa3c-a638-4b30-bb5d-15212c72acbb
ms.tgt_platform: multiple
keywords:
- ms-DS-USN-Last-Sync-Success attribute AD Schema
- msDS-USNLastSyncSuccess attribute AD Schema
topic_type:
- apiref
api_name:
- ms-DS-USN-Last-Sync-Success
api_type:
- Schema
ms.topic: reference
ms.date: 05/31/2018
---

# ms-DS-USN-Last-Sync-Success attribute

The USN at which the last successful replication synchronization occurred.



|                   |                                      |
|-------------------|--------------------------------------|
| CN                | ms-DS-USN-Last-Sync-Success          |
| Ldap-Display-Name | msDS-USNLastSyncSuccess              |
| Size              | \-                                   |
| Update Privilege  | \-                                   |
| Update Frequency  | \-                                   |
| Attribute-Id      | 1.2.840.113556.1.4.2055              |
| System-Id-Guid    | 31f7b8b6-c9f8-4f2d-a37b-58a823030331 |
| Syntax            | [**Interval**](s-interval.md)       |



## Implementations

-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## Windows Server 2008 R2



|                        |                                                                                                                                  |
|------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                               |
| MAPI-Id                | \-                                                                                                                               |
| System-Only            | False                                                                                                                            |
| Is-Single-Valued       | True                                                                                                                             |
| Is Indexed             | False                                                                                                                            |
| In Global Catalog      | False                                                                                                                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                                     |
| Range-Lower            | \-                                                                                                                               |
| Range-Upper            | \-                                                                                                                               |
| Search-Flags           | 0x00000000                                                                                                                       |
| System-Flags           | 0x00000019                                                                                                                       |
| Classes used in        | [**Configuration**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> [**Domain-DNS**](c-domaindns.md)<br/> |



## Windows Server 2012



|                        |                                                                                                                                  |
|------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                               |
| MAPI-Id                | \-                                                                                                                               |
| System-Only            | False                                                                                                                            |
| Is-Single-Valued       | True                                                                                                                             |
| Is Indexed             | False                                                                                                                            |
| In Global Catalog      | False                                                                                                                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                                     |
| Range-Lower            | \-                                                                                                                               |
| Range-Upper            | \-                                                                                                                               |
| Search-Flags           | 0x00000000                                                                                                                       |
| System-Flags           | 0x00000019                                                                                                                       |
| Classes used in        | [**Configuration**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> [**Domain-DNS**](c-domaindns.md)<br/> |



 

 





