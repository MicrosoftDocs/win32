---
title: msSFU-30-Key-Values attribute
description: Contains internal data that the NIS server uses to store temporary data.
ms.assetid: 7c3bc936-f35c-456f-9f3e-7fe11ed1c98b
ms.tgt_platform: multiple
keywords:
- msSFU-30-Key-Values attribute AD Schema
- msSFU30KeyValues attribute AD Schema
topic_type:
- apiref
api_name:
- msSFU-30-Key-Values
api_type:
- Schema
ms.topic: article
ms.date: 05/31/2018
---

# msSFU-30-Key-Values attribute

Contains internal data that the NIS server uses to store temporary data.



|                   |                                      |
|-------------------|--------------------------------------|
| CN                | msSFU-30-Key-Values                  |
| Ldap-Display-Name | msSFU30KeyValues                     |
| Size              | \-                                   |
| Update Privilege  | \-                                   |
| Update Frequency  | \-                                   |
| Attribute-Id      | 1.2.840.113556.1.6.18.1.324          |
| System-Id-Guid    | 37830235-e5e9-46f2-922b-d8d44f03e7ae |
| Syntax            | [**String(IA5)**](s-string-ia5.md)  |



## Implementations

-   [**Windows Server 2003 R2**](#windows-server-2003-r2)
-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## Windows Server 2003 R2



|                        |                                                                                                                       |
|------------------------|-----------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                    |
| MAPI-Id                | \-                                                                                                                    |
| System-Only            | False                                                                                                                 |
| Is-Single-Valued       | False                                                                                                                 |
| Is Indexed             | False                                                                                                                 |
| In Global Catalog      | False                                                                                                                 |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                          |
| Range-Lower            | \-                                                                                                                    |
| Range-Upper            | \-                                                                                                                    |
| Search-Flags           | 0x00000000                                                                                                            |
| System-Flags           | 0x00000000                                                                                                            |
| Classes used in        | [**msSFU-30-Net-Id**](c-mssfu30netid.md)<br/> [**msSFU-30-Network-User**](c-mssfu30networkuser.md)<br/> |



## Windows Server 2008



|                        |                                                                                                                       |
|------------------------|-----------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                    |
| MAPI-Id                | \-                                                                                                                    |
| System-Only            | False                                                                                                                 |
| Is-Single-Valued       | False                                                                                                                 |
| Is Indexed             | False                                                                                                                 |
| In Global Catalog      | False                                                                                                                 |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                          |
| Range-Lower            | \-                                                                                                                    |
| Range-Upper            | \-                                                                                                                    |
| Search-Flags           | 0x00000000                                                                                                            |
| System-Flags           | 0x00000000                                                                                                            |
| Classes used in        | [**msSFU-30-Net-Id**](c-mssfu30netid.md)<br/> [**msSFU-30-Network-User**](c-mssfu30networkuser.md)<br/> |



## Windows Server 2008 R2



|                        |                                                                                                                       |
|------------------------|-----------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                    |
| MAPI-Id                | \-                                                                                                                    |
| System-Only            | False                                                                                                                 |
| Is-Single-Valued       | False                                                                                                                 |
| Is Indexed             | False                                                                                                                 |
| In Global Catalog      | False                                                                                                                 |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                          |
| Range-Lower            | \-                                                                                                                    |
| Range-Upper            | \-                                                                                                                    |
| Search-Flags           | 0x00000000                                                                                                            |
| System-Flags           | 0x00000000                                                                                                            |
| Classes used in        | [**msSFU-30-Net-Id**](c-mssfu30netid.md)<br/> [**msSFU-30-Network-User**](c-mssfu30networkuser.md)<br/> |



## Windows Server 2012



|                        |                                                                                                                       |
|------------------------|-----------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                    |
| MAPI-Id                | \-                                                                                                                    |
| System-Only            | False                                                                                                                 |
| Is-Single-Valued       | False                                                                                                                 |
| Is Indexed             | False                                                                                                                 |
| In Global Catalog      | False                                                                                                                 |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                          |
| Range-Lower            | \-                                                                                                                    |
| Range-Upper            | \-                                                                                                                    |
| Search-Flags           | 0x00000000                                                                                                            |
| System-Flags           | 0x00000000                                                                                                            |
| Classes used in        | [**msSFU-30-Net-Id**](c-mssfu30netid.md)<br/> [**msSFU-30-Network-User**](c-mssfu30networkuser.md)<br/> |



 

 





