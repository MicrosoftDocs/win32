---
title: Obj-Dist-Name attribute
description: Same as the Distinguished Name for an object. Used by Exchange.
audience: developer
author: REDMOND\\markl
manager: REDMOND\\mbaldwin
ms.assetid: '0dc2855c-2707-49d8-80e6-27f163a59bc8'
ms.prod: 'windows-server-dev'
ms.technology: 'active-directory-schema'
ms.tgt_platform: multiple
keywords: ["Obj-Dist-Name attribute AD Schema", "distinguishedName attribute AD Schema"]
topic_type:
- apiref
api_name:
- Obj-Dist-Name
api_type:
- Schema
---

# Obj-Dist-Name attribute

Same as the Distinguished Name for an object. Used by Exchange.



|                   |                                         |
|-------------------|-----------------------------------------|
| CN                | Obj-Dist-Name                           |
| Ldap-Display-Name | distinguishedName                       |
| Size              | \-                                      |
| Update Privilege  | This value is set by the system.        |
| Update Frequency  | Whenever an object is created or moved. |
| Attribute-Id      | 2.5.4.49                                |
| System-Id-Guid    | bf9679e4-0de6-11d0-a285-00aa003049e2    |
| Syntax            | [**Object(DS-DN)**](s-object-ds-dn.md) |



## Implementations

-   [**Windows 2000 Server**](#windows-2000-server)
-   [**Windows Server 2003**](#windows-server-2003)
-   [**ADAM**](#adam)
-   [**Windows Server 2003 R2**](#windows-server-2003-r2)
-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## Windows 2000 Server



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | 0x803C                          |
| System-Only            | True                            |
| Is-Single-Valued       | True                            |
| Is Indexed             | False                           |
| In Global Catalog      | True                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000008                      |
| System-Flags           | 0x00000013                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## Windows Server 2003



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | 0x803C                          |
| System-Only            | True                            |
| Is-Single-Valued       | True                            |
| Is Indexed             | False                           |
| In Global Catalog      | True                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000008                      |
| System-Flags           | 0x00000013                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## ADAM



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | 0x803C                          |
| System-Only            | True                            |
| Is-Single-Valued       | True                            |
| Is Indexed             | False                           |
| In Global Catalog      | True                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000008                      |
| System-Flags           | 0x00000013                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## Windows Server 2003 R2



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | 0x803C                          |
| System-Only            | True                            |
| Is-Single-Valued       | True                            |
| Is Indexed             | False                           |
| In Global Catalog      | True                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000008                      |
| System-Flags           | 0x00000013                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## Windows Server 2008



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | 0x803C                          |
| System-Only            | True                            |
| Is-Single-Valued       | True                            |
| Is Indexed             | False                           |
| In Global Catalog      | True                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000008                      |
| System-Flags           | 0x00000013                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## Windows Server 2008 R2



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | 0x803C                          |
| System-Only            | True                            |
| Is-Single-Valued       | True                            |
| Is Indexed             | False                           |
| In Global Catalog      | True                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000008                      |
| System-Flags           | 0x00000013                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## Windows Server 2012



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | 0x803C                          |
| System-Only            | True                            |
| Is-Single-Valued       | True                            |
| Is Indexed             | False                           |
| In Global Catalog      | True                            |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000008                      |
| System-Flags           | 0x00000013                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



 

 





