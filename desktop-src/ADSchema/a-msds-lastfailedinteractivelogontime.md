---
title: ms-DS-Last-Failed-Interactive-Logon-Time attribute
description: The time that an incorrect password was presented during a Ctrl+Alt+Delete logon.
audience: developer
author: REDMOND\\markl
manager: REDMOND\\mbaldwin
ms.assetid: 'ea52341b-4979-452d-b0d8-236acacb9875'
ms.prod: 'windows-server-dev'
ms.technology: 'active-directory-schema'
ms.tgt_platform: multiple
keywords: ["ms-DS-Last-Failed-Interactive-Logon-Time attribute AD Schema", "msDS-LastFailedInteractiveLogonTime attribute AD Schema"]
topic_type:
- apiref
api_name:
- ms-DS-Last-Failed-Interactive-Logon-Time
api_type:
- Schema
---

# ms-DS-Last-Failed-Interactive-Logon-Time attribute

The time that an incorrect password was presented during a Ctrl+Alt+Delete logon.



|                   |                                          |
|-------------------|------------------------------------------|
| CN                | ms-DS-Last-Failed-Interactive-Logon-Time |
| Ldap-Display-Name | msDS-LastFailedInteractiveLogonTime      |
| Size              | \-                                       |
| Update Privilege  | \-                                       |
| Update Frequency  | \-                                       |
| Attribute-Id      | 1.2.840.113556.1.4.1971                  |
| System-Id-Guid    | c7e7dafa-10c3-4b8b-9acd-54f11063742e     |
| Syntax            | [**Interval**](s-interval.md)           |



## Implementations

-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## Windows Server 2008



|                        |                                   |
|------------------------|-----------------------------------|
| Link-Id                | \-                                |
| MAPI-Id                | \-                                |
| System-Only            | True                              |
| Is-Single-Valued       | True                              |
| Is Indexed             | False                             |
| In Global Catalog      | False                             |
| NT-Security-Descriptor | O:BAG:BAD:S:                      |
| Range-Lower            | \-                                |
| Range-Upper            | \-                                |
| Search-Flags           | 0x00000000                        |
| System-Flags           | 0x00000010                        |
| Classes used in        | [**User**](c-user.md)<br/> |



## Windows Server 2008 R2



|                        |                                   |
|------------------------|-----------------------------------|
| Link-Id                | \-                                |
| MAPI-Id                | \-                                |
| System-Only            | True                              |
| Is-Single-Valued       | True                              |
| Is Indexed             | False                             |
| In Global Catalog      | False                             |
| NT-Security-Descriptor | O:BAG:BAD:S:                      |
| Range-Lower            | \-                                |
| Range-Upper            | \-                                |
| Search-Flags           | 0x00000000                        |
| System-Flags           | 0x00000010                        |
| Classes used in        | [**User**](c-user.md)<br/> |



## Windows Server 2012



|                        |                                   |
|------------------------|-----------------------------------|
| Link-Id                | \-                                |
| MAPI-Id                | \-                                |
| System-Only            | True                              |
| Is-Single-Valued       | True                              |
| Is Indexed             | False                             |
| In Global Catalog      | False                             |
| NT-Security-Descriptor | O:BAG:BAD:S:                      |
| Range-Lower            | \-                                |
| Range-Upper            | \-                                |
| Search-Flags           | 0x00000000                        |
| System-Flags           | 0x00000010                        |
| Classes used in        | [**User**](c-user.md)<br/> |



 

 





