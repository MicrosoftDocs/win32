---
title: ms-DS-Revealed-List-BL attribute
description: Backward link attribute for ms-DS-Revealed-List.
audience: developer
author: REDMOND\\markl
manager: REDMOND\\mbaldwin
ms.assetid: '12979dc0-674b-4102-a433-0d4ff6989248'
ms.prod: 'windows-server-dev'
ms.technology: 'active-directory-schema'
ms.tgt_platform: multiple
keywords: ["ms-DS-Revealed-List-BL attribute AD Schema", "msDS-RevealedListBL attribute AD Schema"]
topic_type:
- apiref
api_name:
- ms-DS-Revealed-List-BL
api_type:
- Schema
---

# ms-DS-Revealed-List-BL attribute

Backward link attribute for [**ms-DS-Revealed-List**](a-msds-revealedlist.md).



|                   |                                         |
|-------------------|-----------------------------------------|
| CN                | ms-DS-Revealed-List-BL                  |
| Ldap-Display-Name | msDS-RevealedListBL                     |
| Size              | \-                                      |
| Update Privilege  | \-                                      |
| Update Frequency  | \-                                      |
| Attribute-Id      | 1.2.840.113556.1.4.1975                 |
| System-Id-Guid    | aa1c88fd-b0f6-429f-b2ca-9d902266e808    |
| Syntax            | [**Object(DS-DN)**](s-object-ds-dn.md) |



## Implementations

-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## Windows Server 2008



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | \-                              |
| System-Only            | True                            |
| Is-Single-Valued       | False                           |
| Is Indexed             | False                           |
| In Global Catalog      | False                           |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000000                      |
| System-Flags           | 0x00000014                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## Windows Server 2008 R2



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | \-                              |
| System-Only            | True                            |
| Is-Single-Valued       | False                           |
| Is Indexed             | False                           |
| In Global Catalog      | False                           |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000000                      |
| System-Flags           | 0x00000014                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



## Windows Server 2012



|                        |                                 |
|------------------------|---------------------------------|
| Link-Id                | \-                              |
| MAPI-Id                | \-                              |
| System-Only            | True                            |
| Is-Single-Valued       | False                           |
| Is Indexed             | False                           |
| In Global Catalog      | False                           |
| NT-Security-Descriptor | O:BAG:BAD:S:                    |
| Range-Lower            | \-                              |
| Range-Upper            | \-                              |
| Search-Flags           | 0x00000000                      |
| System-Flags           | 0x00000014                      |
| Classes used in        | [**Top**](c-top.md)<br/> |



 

 





