---
title: MS-TS-ManagingLS2 attribute
description: Issuer name of the second terminal server per user CAL.
audience: developer
author: REDMOND\\markl
manager: REDMOND\\mbaldwin
ms.assetid: '7a44cb39-c5a3-415d-8990-3e0368730a23'
ms.prod: 'windows-server-dev'
ms.technology: 'active-directory-schema'
ms.tgt_platform: multiple
keywords: ["MS-TS-ManagingLS2 attribute AD Schema", "msTSManagingLS2 attribute AD Schema"]
topic_type:
- apiref
api_name:
- MS-TS-ManagingLS2
api_type:
- Schema
---

# MS-TS-ManagingLS2 attribute

Issuer name of the second terminal server per user CAL.



|                   |                                             |
|-------------------|---------------------------------------------|
| CN                | MS-TS-ManagingLS2                           |
| Ldap-Display-Name | msTSManagingLS2                             |
| Size              | \-                                          |
| Update Privilege  | \-                                          |
| Update Frequency  | \-                                          |
| Attribute-Id      | 1.2.840.113556.1.4.2002                     |
| System-Id-Guid    | 349f0757-51bd-4fc8-9d66-3eceea8a25be        |
| Syntax            | [**String(Unicode)**](s-string-unicode.md) |



## Implementations

-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## Windows Server 2008



|                        |              |
|------------------------|--------------|
| Link-Id                | \-           |
| MAPI-Id                | \-           |
| System-Only            | False        |
| Is-Single-Valued       | True         |
| Is Indexed             | True         |
| In Global Catalog      | False        |
| NT-Security-Descriptor | O:BAG:BAD:S: |
| Range-Lower            | 0            |
| Range-Upper            | 255          |
| Search-Flags           | 0x00000001   |
| System-Flags           | 0x00000010   |
| Classes used in        | \-           |



## Windows Server 2008 R2



|                        |              |
|------------------------|--------------|
| Link-Id                | \-           |
| MAPI-Id                | \-           |
| System-Only            | False        |
| Is-Single-Valued       | True         |
| Is Indexed             | True         |
| In Global Catalog      | False        |
| NT-Security-Descriptor | O:BAG:BAD:S: |
| Range-Lower            | 0            |
| Range-Upper            | 255          |
| Search-Flags           | 0x00000001   |
| System-Flags           | 0x00000010   |
| Classes used in        | \-           |



## Windows Server 2012



|                        |              |
|------------------------|--------------|
| Link-Id                | \-           |
| MAPI-Id                | \-           |
| System-Only            | False        |
| Is-Single-Valued       | True         |
| Is Indexed             | True         |
| In Global Catalog      | False        |
| NT-Security-Descriptor | O:BAG:BAD:S: |
| Range-Lower            | 0            |
| Range-Upper            | 255          |
| Search-Flags           | 0x00000001   |
| System-Flags           | 0x00000010   |
| Classes used in        | \-           |



 

 




