---
title: ms-TAPI-Unique-Identifier attribute
description: Provides the name of a TAPI multicast conference. It is the naming attribute of the Rt-Conference class.
ms.assetid: a8162af7-0169-4381-8edc-3dbbf178e8ed
ms.tgt_platform: multiple
keywords:
- ms-TAPI-Unique-Identifier attribute AD Schema
- msTAPI-uid attribute AD Schema
topic_type:
- apiref
api_name:
- ms-TAPI-Unique-Identifier
api_type:
- Schema
ms.topic: reference
ms.date: 05/31/2018
---

# ms-TAPI-Unique-Identifier attribute

Provides the name of a TAPI multicast conference. It is the naming attribute of the Rt-Conference class.



|                   |                                                                       |
|-------------------|-----------------------------------------------------------------------|
| CN                | ms-TAPI-Unique-Identifier                                             |
| Ldap-Display-Name | msTAPI-uid                                                            |
| Size              | Can be an arbitrary string, typically under 100 characters in length. |
| Update Privilege  | No special privileges required.                                       |
| Update Frequency  | Set once at the time of creating the Rt-Conference object.            |
| Attribute-Id      | 1.2.840.113556.1.4.1698                                               |
| System-Id-Guid    | 70a4e7ea-b3b9-4643-8918-e6dd2471bfd4                                  |
| Syntax            | [**String(Unicode)**](s-string-unicode.md)                           |



## Implementations

-   [**Windows Server 2003**](#windows-server-2003)
-   [**Windows Server 2003 R2**](#windows-server-2003-r2)
-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## Windows Server 2003



|                        |                                                                                                                             |
|------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                          |
| MAPI-Id                | \-                                                                                                                          |
| System-Only            | False                                                                                                                       |
| Is-Single-Valued       | True                                                                                                                        |
| Is Indexed             | False                                                                                                                       |
| In Global Catalog      | False                                                                                                                       |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                                |
| Range-Lower            | \-                                                                                                                          |
| Range-Upper            | \-                                                                                                                          |
| Search-Flags           | 0x00000000                                                                                                                  |
| System-Flags           | 0x00000010                                                                                                                  |
| Classes used in        | [**ms-TAPI-Rt-Conference**](c-mstapi-rtconference.md)<br/> [**ms-TAPI-Rt-Person**](c-mstapi-rtperson.md)<br/> |



## Windows Server 2003 R2



|                        |                                                                                                                             |
|------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                          |
| MAPI-Id                | \-                                                                                                                          |
| System-Only            | False                                                                                                                       |
| Is-Single-Valued       | True                                                                                                                        |
| Is Indexed             | False                                                                                                                       |
| In Global Catalog      | False                                                                                                                       |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                                |
| Range-Lower            | \-                                                                                                                          |
| Range-Upper            | \-                                                                                                                          |
| Search-Flags           | 0x00000000                                                                                                                  |
| System-Flags           | 0x00000010                                                                                                                  |
| Classes used in        | [**ms-TAPI-Rt-Conference**](c-mstapi-rtconference.md)<br/> [**ms-TAPI-Rt-Person**](c-mstapi-rtperson.md)<br/> |



## Windows Server 2008



|                        |                                                                                                                             |
|------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                          |
| MAPI-Id                | \-                                                                                                                          |
| System-Only            | False                                                                                                                       |
| Is-Single-Valued       | True                                                                                                                        |
| Is Indexed             | False                                                                                                                       |
| In Global Catalog      | False                                                                                                                       |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                                |
| Range-Lower            | \-                                                                                                                          |
| Range-Upper            | \-                                                                                                                          |
| Search-Flags           | 0x00000000                                                                                                                  |
| System-Flags           | 0x00000010                                                                                                                  |
| Classes used in        | [**ms-TAPI-Rt-Conference**](c-mstapi-rtconference.md)<br/> [**ms-TAPI-Rt-Person**](c-mstapi-rtperson.md)<br/> |



## Windows Server 2008 R2



|                        |                                                                                                                             |
|------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                          |
| MAPI-Id                | \-                                                                                                                          |
| System-Only            | False                                                                                                                       |
| Is-Single-Valued       | True                                                                                                                        |
| Is Indexed             | False                                                                                                                       |
| In Global Catalog      | False                                                                                                                       |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                                |
| Range-Lower            | \-                                                                                                                          |
| Range-Upper            | \-                                                                                                                          |
| Search-Flags           | 0x00000000                                                                                                                  |
| System-Flags           | 0x00000010                                                                                                                  |
| Classes used in        | [**ms-TAPI-Rt-Conference**](c-mstapi-rtconference.md)<br/> [**ms-TAPI-Rt-Person**](c-mstapi-rtperson.md)<br/> |



## Windows Server 2012



|                        |                                                                                                                             |
|------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| Link-Id                | \-                                                                                                                          |
| MAPI-Id                | \-                                                                                                                          |
| System-Only            | False                                                                                                                       |
| Is-Single-Valued       | True                                                                                                                        |
| Is Indexed             | False                                                                                                                       |
| In Global Catalog      | False                                                                                                                       |
| NT-Security-Descriptor | O:BAG:BAD:S:                                                                                                                |
| Range-Lower            | \-                                                                                                                          |
| Range-Upper            | \-                                                                                                                          |
| Search-Flags           | 0x00000000                                                                                                                  |
| System-Flags           | 0x00000010                                                                                                                  |
| Classes used in        | [**ms-TAPI-Rt-Conference**](c-mstapi-rtconference.md)<br/> [**ms-TAPI-Rt-Person**](c-mstapi-rtperson.md)<br/> |



 

 





