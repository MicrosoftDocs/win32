...
The alerter service must be running on the client computer when you call the **NetAlertRaise** function or the **NetAlertRaiseEx** function. If the service is not running, the functions fail with **ERROR\_FILE\_NOT\_FOUND**. The alerter service on the client calls the [**NetMessageBufferSend**](/windows/win32/Lmmsg/nf-lmmsg-netmessagebuffersend?branch=master) function to send information to recipients.
...
When you raise an alert with a call to the [**NetAlertRaise**](/windows/win32/Lmalert/nf-lmalert-netalertraise?branch=master) function, the message data should consist of one [**STD\_ALERT**](/windows/win32/Lmalert/ns-lmalert-_std_alert?branch=master) header structure, followed by additional fixed-length data that is alert-specific in one [**ADMIN\_OTHER\_INFO**](/windows/win32/Lmalert/ns-lmalert-_admin_other_info?branch=master), [**ERRLOG\_OTHER\_INFO**](/windows/win32/Lmalert/ns-lmalert-_errlog_other_info?branch=master), [**PRINT\_OTHER\_INFO**](/windows/win32/Lmalert/ns-lmalert-_print_other_info?branch=master), or [**USER\_OTHER\_INFO**](/windows/win32/Lmalert/ns-lmalert-_user_other_info?branch=master) structure. Additional variable-length data can follow the alert-specific structure. (Calls to the [**NetAlertRaiseEx**](/windows/win32/Lmalert/nf-lmalert-netalertraiseex?branch=master) function do not require a **STD\_ALERT** structure.) The calling application must allocate the memory for all structures and variable-length data, and free the memory after the call returns.
...
| Macro                                          | Description                                                                                               |
|------------------------------------------------|-----------------------------------------------------------------------------------------------------------|
| [**ALERT\_OTHER\_INFO**](/windows/win32/Lmalert/nf-lmalert-alert_other_info?branch=master) | Returns a pointer to the fixed-length data that follows the **STD\_ALERT** structure in an alert message. |
| [**ALERT\_VAR\_DATA**](/windows/win32/Lmalert/nf-lmalert-alert_var_data?branch=master)     | Returns a pointer to the variable-length data that follows the alert-specific data in an alert message.   |
