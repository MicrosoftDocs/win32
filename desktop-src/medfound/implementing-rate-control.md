...
1.  Implement the [**IMFGetService**](imfgetservice.md) interface.
2.  Support the **MF\_RATE\_CONTROL\_SERVICE** service. (See [Service Interfaces](service-interfaces.md).)
3.  Implement the [**IMFRateSupport**](imfratesupport.md) interface, which gets the playback rates supported by the object.
4.  Implement the [**IMFRateControl**](imfratecontrol.md) interface, which gets or sets the playback rate.

## Media Sources

If a media source supports rate control, it should implement both [**IMFRateSupport**](imfratesupport.md) and [**IMFRateControl**](imfratecontrol.md). Otherwise, the Media Session reports that the minimum and maximum playback rate is 1.0, regardless of what other components are in the pipeline.
...
The *fThin* parameter of the [**IMFRateControl::SetRate**](imfratecontrol-setrate.md) method indicates whether media source should *thin* the content. Thinning applies primarily to video streams. In thinned mode, the source drops delta frames and deliver only key frames. At very high playback rates, the source might skip some key frames (for example, deliver every other key frame).
...
When the media source completes a call to [**SetRate**](imfratecontrol-setrate.md), it sends the [MESourceRateChanged](mesourceratechanged.md) event.
...
If an MFT does not implement the [**IMFRateSupport**](imfratesupport.md) interface, the Media Session assumes the following:
...
If either of these conditions is not true, the MFT should implement [**IMFRateSupport**](imfratesupport.md) and [**IMFRateControl**](imfratecontrol.md).
...
If an MFT does not expose the [**IMFRateSupport**](imfratesupport.md) interface, the Media Session uses *thinning* for reverse playback, as follows:

-   The Media Session sends key frames to the MFT in the usual way, by calling [**IMFTransform::ProcessInput**](imftransform-processinput.md).
...
If the MFT implements [**IMFRateSupport**](imfratesupport.md), the Media Session uses this interface to discover whether the MFT supports non-thinned reverse playback. If the MFT supports non-thinned reverse playback, the Media Session delivers all of the samples, in reverse order, without dropping samples or flushing the MFT.

If an MFT supports non-thinned reverse playback, it should implement the [**IMFRateControl**](imfratecontrol.md) interface. The Media Session will use this interface to notify the MFT when reverse playback occurs. At that point, the MFT must be prepared for the time stamps to decrease and for delta frames to arrive in reverse order. A decoder will typically need to buffer samples until it has received an entire group of pictures (GOP), then decode the entire GOP and output the decoded frames in the correct (reverse) order.

## Media Sinks

If a media sink is *rateless*, the Media Session assumes that media sink can handle any playback rate. The media sink does not need to implement [**IMFRateSupport**](imfratesupport.md). (A rateless media sink returns the MEDIASINK\_RATELESS flag from the [**IMFMediaSink::GetCharacteristics**](imfmediasink-getcharacteristics.md) method.)

Otherwise, a media sink should implement [**IMFRateSupport**](imfratesupport.md) if it can handle playback rates other than 1.0.

Media sinks should not implement [**IMFRateControl**](imfratecontrol.md). When the playback rate changes, the presentation clock calls the media sink's [**IMFClockStateSink::OnClockSetRate**](imfclockstatesink-onclocksetrate.md) method.
