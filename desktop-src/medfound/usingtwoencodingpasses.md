...
Deliver the samples to the encoder DMO one at a time, as you would in a one-pass mode. When you process the input samples for your preprocessing pass, the calls to [**IMediaObject::ProcessInput**](dshow.imediaobject_processinput) or [**IMFTransform::ProcessInput**](imftransform-processinput.md) will return **S\_FALSE**, to indicate that no output is produced.
