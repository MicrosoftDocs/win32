...
Upon successful registration of Windows Media encoders with the system, an application can enumerate the encoders by calling [**MFTEnum**](mftenum.md). To search for the right encoder, you must specify the following:

-   The GUID that represents the category, which is either **MFT\_CATEGORY\_AUDIO\_ENCODER** or **MFT\_CATEGORY\_VIDEO\_ENCODER**.

-   The format to match. This is set in the [**MFT\_REGISTER\_TYPE\_INFO**](mft-register-type-info.md) structure that specifies the major type and subtype of the media type that the encoder will generate samples in. This structure is passed in the *pOutputType* parameter. For information about the supported types, see [Media Type GUIDs](media-type-guids.md).
