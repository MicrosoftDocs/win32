...
This interface is not intended for general application use. DirectShow decoder filters should use the [**IAMVideoAccelerator**](dshow.iamvideoaccelerator) interface, not **IDirect3DVideoDevice9**. The input pins of the Video Mixing Renderer (VMR) filter and the Overlay Mixer filter expose **IAMVideoAccelerator**.

## Members

The **IDirect3DVideoDevice9** interface inherits from the [**IUnknown**](com.iunknown) interface. **IDirect3DVideoDevice9** also has these types of members:
...
To get a pointer to this interface, call **QueryInterface** on an [**IDirect3DDevice9**](direct3d9.idirect3ddevice9) or [**IDirect3DDevice9Ex**](direct3d9.idirect3ddevice9ex) pointer.
