...
In Media Foundation, the *mutual exclusion* object that exposes the [**IMFASFMutualExclusion**](imfasfmutualexclusion.md) interface exists for every ASF Mutual Exclusion Object. The profile object holds reference to all mutual exclusion objects.
...
1.  Call [**IMFASFProfile::CreateMutualExclusion**](imfasfprofile-createmutualexclusion.md) to create an empty mutual exclusion object.
2.  Call [**IMFASFMutualExclusion::SetType**](imfasfmutualexclusion-settype.md) to set the criterion of the mutually exclusive stream.
...
3.  Call [**IMFASFMutualExclusion::AddRecord**](imfasfmutualexclusion-addrecord.md) to add a record to the mutual exclusion object.

    This method adds an empty record and assigns it a record index starting with zero.

4.  Call [**IMFASFMutualExclusion::AddStreamForRecord**](imfasfmutualexclusion-addstreamforrecord.md) to add the stream number to the record specified by the record index.

    Each record includes one or more streams. Every stream in a record is mutually exclusive of all streams in every other record. To get the number of streams in a record, call [**IMFASFMutualExclusion::GetStreamsForRecord**](imfasfmutualexclusion-getstreamsforrecord.md) by specifying the record index.

    To remove a stream from the record, call [**IMFASFMutualExclusion::RemoveStreamFromRecord**](imfasfmutualexclusion-removestreamfromrecord.md) and specify the record index and the stream number.

5.  Call [**IMFASFProfile::AddMutualExclusion**](imfasfprofile-addmutualexclusion.md) to add the configured mutual exclusion object to the profile.

## Enumerating Mutual Exclusion Objects in a Profile

If [**IMFASFProfile::AddMutualExclusion**](imfasfprofile-addmutualexclusion.md) succeeds, it assigns an index to the specified object, starting at zero.

To enumerate the mutual exclusion objects associated with a profile, call [**IMFASFProfile::GetMutualExclusionCount**](imfasfprofile-getmutualexclusioncount.md) and loop through the list by calling [**IMFASFProfile::GetMutualExclusion**](imfasfprofile-getmutualexclusion.md). Mutual exclusion indexes are sequential and range between 0 and one less than the number of streams retrieved by **GetMutualExclusionCount**.

A mutual exclusion object is removed from the profile by calling [**IMFASFProfile::RemoveMutualExclusion**](imfasfprofile-removemutualexclusion.md). The profile reassigns the mutual exclusion indexes so that they are sequential starting with zero. This overwrites previously stored indexes, which are no longer valid after calling this method. This releases the associated mutual exclusion stream records.

## Removing Records in a Mutual Exclusion Object

To remove a record from a mutual exclusion object, call [**IMFASFMutualExclusion::RemoveRecord**](imfasfmutualexclusion-removerecord.md). If this method succeeds, the mutual exclusion object indexes the remaining records so that they are sequential starting with zero. The application should enumerate the records to ensure the correct index for each record. To do so, call [**IMFASFMutualExclusion::GetRecordCount**](imfasfmutualexclusion-getrecordcount.md) and loop through the records by calling [**IMFASFMutualExclusion::GetStreamsForRecord**](imfasfmutualexclusion-getstreamsforrecord.md).
...
2.  Call [**IMFASFMutualExclusion::Clone**](imfasfmutualexclusion-clone.md) to clone the mutual exclusion object.

3.  Configure the cloned object as required.
4.  Call [**IMFASFProfile::RemoveMutualExclusion**](imfasfprofile-removemutualexclusion.md) to remove the old mutual exclusion object from the profile.

5.  Call [**IMFASFProfile::AddMutualExclusion**](imfasfprofile-addmutualexclusion.md) to add the updated mutual exclusion object to the profile.
