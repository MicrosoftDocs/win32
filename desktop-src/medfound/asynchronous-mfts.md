...
The original MFT design supports *in-band* events. An in-band event contains information about the data stream, such as information about a format change. The client sends in-band events to the MFT by calling [**IMFTransform::ProcessEvent**](imftransform-processevent.md). The MFT can send in-band events back to the client in the [**ProcessOutput**](imftransform-processoutput.md) method. (Specifically, events are conveyed in the **pEvents** member of the [**MFT\_OUTPUT\_DATA\_BUFFER**](mft-output-data-buffer.md) structure.)

An MFT sends out-of-band events through the [**IMFMediaEventGenerator**](imfmediaeventgenerator.md) interface as follows:

1.  The MFT implements the [**IMFMediaEventGenerator**](imfmediaeventgenerator.md) interface, as described in [Media Event Generators](media-event-generators.md).
2.  The client calls **IUnknown::QueryInterface** on the MFT for the [**IMFMediaEventGenerator**](imfmediaeventgenerator.md) interface. An asynchronous MFT must expose this interface. Synchronous MFTs should not expose this interface.
3.  The client calls [**IMFMediaEventGenerator::BeginGetEvent**](imfmediaeventgenerator-begingetevent.md) and [**IMFMediaEventGenerator::EndGetEvent**](imfmediaeventgenerator-endgetevent.md) to receive out-of-band events from the MFT.

## ProcessInput

The [**IMFTransform::ProcessInput**](imftransform-processinput.md) method is modified as follows:
...
3.  For each [METransformNeedInput](metransformneedinput.md) event, the client calls [**ProcessInput**](imftransform-processinput.md) for the specified stream.
4.  At the end of streaming, the client may call [**ProcessMessage**](imftransform-processmessage.md) with the [**MFT\_MESSAGE\_NOTIFY\_END\_OF\_STREAM**](mft-message-notify-end-of-stream.md) message.
...
-   The MFT should maintain a count of pending [METransformNeedInput](metransformneedinput.md) events. Any call to [**ProcessInput**](imftransform-processinput.md) that does not correspond to an METransformNeedInput event must return **MF\_E\_NOTACCEPTING**.
...
-   If [**ProcessInput**](imftransform-processinput.md) is called before [**MFT\_MESSAGE\_NOTIFY\_START\_OF\_STREAM**](mft-message-notify-start-of-stream.md) or after [**MFT\_MESSAGE\_NOTIFY\_END\_OF\_STREAM**](mft-message-notify-end-of-stream.md), the method must return **MF\_E\_NOTACCEPTING**.

## ProcessOutput

The [**IMFTransform::ProcessOutput**](imftransform-processoutput.md) method is modified as follows:

1.  Whenever the MFT has output, it sends an [METransformHaveOutput](metransformhaveoutput.md) event.
2.  For each [METransformHaveOutput](metransformhaveoutput.md) event, the client calls [**ProcessOutput**](imftransform-processoutput.md).

Implementation notes:

-   If the client calls [**ProcessOutput**](imftransform-processoutput.md) at any other time, the method returns **E\_UNEXPECTED**.
-   An asynchronous MFT should never return **MF\_E\_TRANSFORM\_NEED\_MORE\_INPUT** from the [**ProcessOutput**](imftransform-processoutput.md) method. If the MFT requires more input, it sends an [METransformNeedInput](metransformneedinput.md) event.
...
An asynchronous MFT must implement the [**IMFTransform::GetAttributes**](imftransform-getattributes.md) method to return a valid attribute store. The following attributes apply to asynchronous MFTs:
...
<dl> The client calls [**IMFTransform::GetAttributes**](imftransform-getattributes.md) on the MFT.  
...
Until the client unlocks the MFT, all [**IMFTransform**](imftransform.md) methods should return **MF\_E\_TRANSFORM\_ASYNC\_LOCKED**, with the following exceptions:

-   [**IMFTransform::GetAttributes**](imftransform-getattributes.md) (all asynchronous MFTs)
-   [**IMFTransform::GetInputAvailableType**](imftransform-getinputavailabletype.md) (all asynchronous MFTs)
-   [**IMFTransform::GetOutputCurrentType**](imftransform-getoutputcurrenttype.md) (encoders only)
-   [**IMFTransform::SetOutputType**](imftransform-setoutputtype.md) (encoders only)
-   [**IMFTransform::GetStreamCount**](imftransform-getstreamcount.md) (all asynchronous MFTs)
-   [**IMFTransform::GetStreamIDs**](imftransform-getstreamids.md) (all asynchronous MFTs)
...
Asynchronous MFTs must implement the [**IMFShutdown**](imfshutdown.md) interface.

-   [**Shutdown**](imfshutdown-shutdown.md): The MFT must shut down its event queue. If using the standard event queue, call [**IMFMediaEventQueue::Shutdown**](imfmediaeventqueue-shutdown.md). Optionally, the MFT may release other resources. The client must not use the MFT after calling **Shutdown**.
-   [**GetShutdownStatus**](imfshutdown-getshutdownstatus.md): After [**Shutdown**](imfshutdown-shutdown.md) has been called, the MFT should return the value **MFSHUTDOWN\_COMPLETED** in the *pStatus* parameter. It should not return the value **MFSHUTDOWN\_INITIATED**.

## Registration and Enumeration

To register an asynchronous MFT, call the [**MFTRegister**](mftregister.md) function and set the **MFT\_ENUM\_FLAG\_ASYNCMFT** flag in the *Flags* parameter. (Previously this flag was reserved.)

To enumerate asynchronous MFTs, call the [**MFTEnumEx**](mftenumex.md) function and set the **MFT\_ENUM\_FLAG\_ASYNCMFT** flag in the *Flags* parameter. For backward compatibility, the [**MFTEnum**](mftenum.md) function does not enumerate asynchronous MFTs. Otherwise, installing an asynchronous MFT on the user's computer could break existing applications.
