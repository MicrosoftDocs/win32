...
1.  Get a pointer to the encoder's property store by querying the encoder ([**IMFTransform**](/windows/win32/mftransform/nn-mftransform-imftransform?branch=master) interface) directly for the **IPropertyStore** interface.
2.  Call [**IMFASFContentInfo::GetEncodingConfigurationPropertyStore**](/windows/win32/wmcontainer/nf-wmcontainer-imfasfcontentinfo-getencodingconfigurationpropertystore?branch=master). To set stream-specific properties, specify the stream identifier in the *wStreamNumber* parameter; for file-level properties, pass 0. The *ppIStore* parameter receives a pointer to the **IPropertyStore** interface. The received property store is empty.
3.  Call **IPropertyStore::GetValue** on the encoder and get the property value by specifying the property key constants. For a complete list of encoding properties, see the [Codec Programming Reference](http://go.microsoft.com/fwlink/p/?linkid=111464).
...
The ASF media sink can be created by using an activation object by calling [**MFCreateASFMediaSinkActivate**](/windows/win32/wmcontainer/nf-wmcontainer-mfcreateasfmediasinkactivate?branch=master). The new media sink object is configured based on media sink-specific settings that can be set in the ContentInfo object's property store. The following table shows the ASF media sink property constants.
...
| [**MFPKEY\_ASFSTREAMSINK\_CORRECTED\_LEAKYBUCKET**](mfpkey-asfstreamsink-corrected-leakybucket-property.md) | This property must be set when the encoder decides which buffer window and bit rate to use. To set these values, use the [**IWMCodecLeakyBucket**](/windows/win32/wmcodecdsp/nn-wmcodecdsp-iwmcodecleakybucket?branch=master) interface. This must be set for each stream in the ASF file.                                                     |
