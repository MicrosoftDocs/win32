...
3.  The Topology Loader calls [**IMFTransform::ProcessMessage**](/windows/desktop/api/mftransform/nf-mftransform-imftransform-processmessage) with the [**MFT\_MESSAGE\_SET\_D3D\_MANAGER**](mft-message-set-d3d-manager.md) message. The *ulParam* parameter is an [**IUnknown**](https://msdn.microsoft.com/windows/desktop/33f1d79a-33fc-4ce5-a372-e08bda378332) pointer to the DXGI device manager. Query this pointer for the [**IMFDXGIDeviceManager**](/windows/desktop/api/mfobjects/nn-mfobjects-imfdxgidevicemanager) interface.
...
5.  To get a pointer to the Direct3D 11 device, call [**IMFDXGIDeviceManager::GetVideoService**](/windows/desktop/api/mfobjects/nf-mfobjects-imfdxgidevicemanager-getvideoservice). Pass in the device handle and the value **IID\_ID3D11Device**. The method returns a pointer to the [**ID3D11Device**](https://msdn.microsoft.com/windows/desktop/2f2559d9-1cd6-44f6-90e2-ee0f86e39f78) interface.
...
7.  Call [**ID3D11Device::GetImmediateContext**](https://msdn.microsoft.com/windows/desktop/0349f0b8-7696-4d72-bed4-d39b9ac90f6c) to get an [**ID3D11DeviceContext**](https://msdn.microsoft.com/windows/desktop/afb32c09-77f2-4c33-bd93-8dce92a2e45e) pointer.
8.  Call [**QueryInterface**](https://msdn.microsoft.com/windows/desktop/54d5ff80-18db-43f2-b636-f93ac053146d) on the [**ID3D11DeviceContext**](https://msdn.microsoft.com/windows/desktop/afb32c09-77f2-4c33-bd93-8dce92a2e45e) to get an [**ID3D11VideoContext**](/windows/desktop/api/d3d11/nn-d3d11-id3d11videocontext) pointer.
9.  It is recommended that you use multi-thread protection on the device context to prevent deadlock issues that can sometimes happen when you call [**ID3D11VideoContext::GetDecoderBuffer**](/windows/desktop/api/d3d11/nf-d3d11-id3d11videocontext-getdecoderbuffer) or [**ID3D11VideoContext::ReleaseDecoderBuffer**](/windows/desktop/api/d3d11/nf-d3d11-id3d11videocontext-releasedecoderbuffer). To set multi-thread protection, first call [**QueryInterface**](https://msdn.microsoft.com/windows/desktop/54d5ff80-18db-43f2-b636-f93ac053146d) on [**ID3D11Device**](https://msdn.microsoft.com/windows/desktop/2f2559d9-1cd6-44f6-90e2-ee0f86e39f78) to get an [**ID3D10Multithread**](https://msdn.microsoft.com/windows/desktop/ff4890fe-25a3-4515-b20d-45f68637e7b3) pointer. Then call [**ID3D10Multithread::SetMultithreadProtected**](https://msdn.microsoft.com/windows/desktop/6023a211-f493-4d2d-b9ff-1208c44d0d8f), passing in **true** for *bMTProtect*.
...
In the [**IMFTransform::GetOutputStreamInfo**](/windows/desktop/api/mftransform/nf-mftransform-imftransform-getoutputstreaminfo) method, set the **MFT\_OUTPUT\_STREAM\_PROVIDES\_SAMPLES** flag in the [**MFT\_OUTPUT\_STREAM\_INFO**](/windows/desktop/api/mftransform/ne-mftransform-_mft_output_stream_info_flags) structure. This flag notifies the Media Session that the MFT allocates its own output samples. To allocate the output samples, the MFT performs the following steps:

1.  Create a 2D texture array by calling [**ID3D11Device::CreateTexture2D**](https://msdn.microsoft.com/windows/desktop/69950ce7-9c8e-4f00-860d-e118e2bbc81a). In the [**D3D11\_TEXTURE2D\_DESC**](https://msdn.microsoft.com/windows/desktop/90c0f877-daf5-4b3d-9846-5bb414c55461) structure, set **ArraySize** equal to the number of surfaces that the decoder needs. This includes:
...
3.  For each surface in the texture array, create a media sample as follows:
    1.  Create a DXGI media buffer by calling the [**MFCreateDXGISurfaceBuffer**](/windows/desktop/api/mfapi/nf-mfapi-mfcreatedxgisurfacebuffer) function. Pass in the [**ID3D11Texture2D**](https://msdn.microsoft.com/windows/desktop/49cd6e21-6cb1-45ea-b83a-3c93f0560915) pointer and the offset for each element in the texture array. The function returns an [**IMFMediaBuffer**](/windows/desktop/api/mfobjects/nn-mfobjects-imfmediabuffer) pointer.
...
2.  Release all resources associated with the previous Direct3D 11 device, including the [**ID3D11VideoDecoder**](/windows/desktop/api/d3d11/nn-d3d11-id3d11videodecoder), [**ID3D11VideoContext**](/windows/desktop/api/d3d11/nn-d3d11-id3d11videocontext), [**ID3D11Texture2D**](https://msdn.microsoft.com/windows/desktop/49cd6e21-6cb1-45ea-b83a-3c93f0560915), and [**ID3D11VideoDecoderOutputView**](/windows/desktop/api/d3d11/nn-d3d11-id3d11videodecoderoutputview) interfaces.
...
2.  Query the media sample for the [**IMFTrackedSample**](/windows/desktop/api/evr/nn-mfidl-imftrackedsample) interface.
3.  Call [**IMFTrackedSample::SetAllocator**](/windows/desktop/api/evr/nf-mfidl-imftrackedsample-setallocator) and provide a pointer to the [**IMFAsyncCallback**](/windows/desktop/api/mfobjects/nn-mfobjects-imfasynccallback) interface. (The software decoder must implement this interface.) When the video renderer releases the sample, the callback will be invoked. Use this callback to keep track of which samples are currently available and which are in use.
