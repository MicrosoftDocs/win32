...
Each encoder or decoder object exposes the [**IMediaObject**](dshow.imediaobject) interface so that the object can be used as a DirectX Media Object (DMO) and the [**IMFTransform**](imftransform.md) interface so that the object can be used as a Media Foundation Transform (MFT).
