...
1.  Call [**IMFTransform::GetOutputAvailableType**](imftransform-getoutputavailabletype.md) to obtain an [**IMFMediaType**](imfmediatype.md) interface that represents the output type.
2.  Call [**IMFMediaType::GetRepresentation**](imfmediatype-getrepresentation.md) to get an [**AM\_MEDIA\_TYPE**](dshow.am_media_type) structure that contains information about the output type.
...
4.  Set the output type to your chosen type by calling [**IMFTransform::SetOutputType**](imftransform-setoutputtype.md).
...
5.  Set the output type to one of the types enumerated in step 4 by calling [**IMFTransform::SetOutputType**](imftransform-setoutputtype.md).

The following code enumerates all of the lossless output types for the Windows Media Audio encoder. The code prints the value of the audio format tag for each enumerated type. Because all of the enumerated types are lossless, all of those format tags have a value of 0x163. Assume that pIMT is a pointer to an [**IMFTransform**](imftransform.md) interface on a Windows Media Audio encoder object and that pStore is a pointer to an **IPropertyStore** interface on the same object. Also assume that hr is a variable of type **HRESULT** that was declared previously in the code.
