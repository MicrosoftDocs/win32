...
2.  Set this attribue on the [**IMFActivate**](/windows/win32/mfobjects/nn-mfobjects-imfactivate?branch=master) pointer by calling [**IMFAttributes::SetGUID**](/windows/win32/mfobjects/nf-mfobjects-imfattributes-setguid?branch=master). The value of the attribute is the CLSID of the application's custom video presenter.

If this attribute is set, the EVR calls **CoCreateInstance** with the specified CLSID to create the custom video presenter. The video presenter must expose the [**IMFVideoPresenter**](/windows/win32/evr/nn-evr-imfvideopresenter?branch=master) interface. The presenter is created as an in-process COM server.
