...
2.  Call [**MFGetService**](mfgetservice.md) on the media source to get a pointer to the [**IPropertyStore**](properties.IPropertyStore) interface. In the *guidService* parameter of **MFGetService**, specify the value **MF\_PROPERTY\_HANDLER\_SERVICE**. If the source does not support the **IPropertyStore** interface, **MFGetService** returns **MF\_E\_UNSUPPORTED\_SERVICE**.
