...
When setting this attribute on MPEG-4 sink through [**IMFMediaTypeHandler::SetCurrentMediaType**](/windows/win32/mfidl/nf-mfidl-imfmediatypehandler-setcurrentmediatype?branch=master) method, the data for the attribute MF\_MT\_MPEG4\_SAMPLE\_DESCRIPTION should not change after one or more samples have been sent to the sink on the corresponding stream's [**IMFStreamSink::ProcessSample**](/windows/win32/mfidl/nf-mfidl-imfstreamsink-processsample?branch=master) interface.
