...
In Media Foundation, asynchronous processing and callbacks are handled by [work queues](work-queues.md). Work queues always have multithreaded apartment (MTA) threads, so an application will have a simpler implementation if it runs on an MTA thread as well. Therefore, it is recommended to call [**CoInitializeEx**](https://msdn.microsoft.com/en-us/library/ms695279(v=VS.85).aspx) with the **COINIT\_MULTITHREADED** flag.
...
If you have an STA proxy to an MTA or free-threaded object, the object can be marshaled to an MTA proxy by using a work-queue callback. The [**CoCreateInstance**](https://msdn.microsoft.com/en-us/library/ms686615(v=VS.85).aspx) function can return either a raw pointer or an STA proxy, depending on the object model defined in the registry for that CLSID. If an STA proxy is returned, you must not pass the pointer to a Media Foundation API.
...
For more information about the global interface table, see [**IGlobalInterfaceTable**](https://msdn.microsoft.com/en-us/library/ms678517(v=VS.85).aspx).
...
Although not all Media Foundation objects are COM objects, all Media Foundation interfaces derive from [**IUnknown**](https://msdn.microsoft.com/en-us/library/ms680509(v=VS.85).aspx). Therefore, all Media Foundation objects must implement **IUnknown** according to COM specifications, including the rules for reference counting and [**QueryInterface**](https://msdn.microsoft.com/en-us/library/ms682521(v=VS.85).aspx). All reference counted objects should also ensure that [**DllCanUnloadNow**](https://msdn.microsoft.com/en-us/library/ms690368(v=VS.85).aspx) will not allow the module to be unloaded while the objects still persist.
