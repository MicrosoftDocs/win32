...
Profile flags that define the stream settings for the transcode topology. The flags are defined in the [**MF\_TRANSCODE\_ADJUST\_PROFILE\_FLAGS**](mf-transcode-adjust-profile-flags.md) enumeration.
...
An application can set this attribute at the container level on the transcode profile. If this attribute is set, the [**MFCreateTranscodeTopology**](mfcreatetranscodetopology.md) function changes the stream attributes during topology building, depending on the specified flag. For example, if the application specifies the **MF\_TRANSCODE\_ADJUST\_PROFILE\_DEFAULT** flag, the application-specified stream settings are used to create the profile.
