...
This property applies to the next frame that the encoder receives as input. For a Media Foundation transform (MFT), this is the next frame that is passed to the [**IMFTransform::ProcessInput**](imftransform-processinput.md) method. The setting resets to zero after the next frame.
