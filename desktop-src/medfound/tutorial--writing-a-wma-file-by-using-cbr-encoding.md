...
-   Source media type: Media type object, exposes [**IMFMediaType**](imfmediatype.md) interface, which describes the contents of the input file.
-   Audio profile: Profile object, exposes [**IMFASFProfile**](imfasfprofile.md) interface, which contains only audio streams of the output file.
-   Stream sample: Media sample, exposes [**IMFSample**](imfsample.md) interface, represents the input file's media data obtained from the encoder in a compressed state.
-   ContentInfo object: [ASF ContentInfo Object](asf-contentinfo-object.md), exposes [**IMFASFContentInfo**](imfasfcontentinfo.md) interface, which represents the ASF Header Object of the output file.
-   Data byte stream: Byte stream object, exposes [**IMFByteStream**](imfbytestream.md) interface, which represents the entire ASF Data Object portion of the output file.
-   Data packet: Media sample, exposes [**IMFSample**](imfsample.md) interface, generated by the [ASF Multiplexer](asf-multiplexer.md); represents an ASF data packet that will be written to the data byte stream.
-   Output byte stream: Byte stream object, exposes [**IMFByteStream**](imfbytestream.md) interface, which contains the contents of the output file.
...
For more information, see [**IMFByteStream::Write**](imfbytestream-write.md). The following code shows these helper functions:
...
In Media Foundation, encoders (expose the [**IMFTransform**](imftransform.md) interface) are implemented as [Media Foundation Transforms](media-foundation-transforms.md) (MFT).
...
1.  Call [**MFCreateASFProfile**](mfcreateasfprofile.md) to create an empty ASF profile object. The ASF profile exposes the [**IMFASFProfile**](imfasfprofile.md) interface. For more information, see [Creating and Configuring ASF Streams](creating-and-configuring-asf-streams.md).
2.  Get the encoded audio format from the `CWmaEncoder` object.
3.  Call [**IMFASFProfile::CreateStream**](imfasfprofile-createstream.md) to create a new stream for the ASF profile. Pass in a pointer to the [**IMFMediaType**](imfmediatype.md) interface, which represents the stream format.
4.  Call [**IMFASFStreamConfig::SetStreamNumber**](imfasfstreamconfig-setstreamnumber.md) to assign a stream identifier.
...
6.  Call [**IMFASFProfile::SetStream**](imfasfprofile-setstream.md) to add the new stream to the profile.
...
2.  Call [**IMFASFContentInfo::SetProfile**](imfasfcontentinfo-setprofile.md) to set the ASF profile.
...
1.  Call [**MFCreateASFMultiplexer**](mfcreateasfmultiplexer.md) to create the ASF multiplexer.
2.  Call [**IMFASFMultiplexer::Initialize**](imfasfmultiplexer-initialize.md) to initialize the multiplexer. Pass in a pointer to the ASF Content Info object, which was created in the previous section.
3.  Call [**IMFASFMultiplexer::SetFlags**](imfasfmultiplexer-setflags.md) to set the **MFASF\_MULTIPLEXER\_AUTOADJUST\_BITRATE** flag. When this setting is used, the multiplexer automatically adjusts the bit rate of the ASF content to match the characteristics of the streams being multiplexed.
...
1.  Call [**MFCreateTempFile**](mfcreatetempfile.md) to create a temporary byte stream to hold the ASF data packets.
...
4.  Call [**IMFASFMultiplexer::ProcessSample**](imfasfmultiplexer-processsample.md) to send the compressed audio samples to the ASF multiplexer for packetization.
...
7.  After all samples are sent to the multiplexer, call [**IMFASFMultiplexer::Flush**](imfasfmultiplexer-flush.md) and pull the remaining ASF packets from the multiplexer.
8.  Call [**IMFASFMultiplexer::End**](imfasfmultiplexer-end.md).
...
Next, write the ASF header to a media buffer by calling [**IMFASFContentInfo::GenerateHeader**](imfasfcontentinfo-generateheader.md). This method converts data stored in the ContentInfo object into binary data in ASF Header Object format. For more information, see [Generating a New ASF Header Object](generating-a-new-asf-header-object.md).
...
Now you can put the previous steps together into a complete application. Before using any of the Media Foundation objects, initialize the Media Foundation platform by calling [**MFStartup**](mfstartup.md). When you are done, call [**MFShutdown**](mfshutdown.md). For more information, see [Initializing Media Foundation](initializing-media-foundation.md).
