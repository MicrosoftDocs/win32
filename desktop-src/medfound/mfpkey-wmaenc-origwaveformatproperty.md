...
Specifies the [**WAVEFORMATEX**](dshow.waveformatex) structure describing the input audio content.
...
VT\_ARRAY \| VT\_UI1 ([**WAVEFORMATEX**](dshow.waveformatex) as an array of bytes)

## Remarks

When transcoding Windows Media Audio-based content to a lower bit rate, you can pass the [**WAVEFORMATEX**](dshow.waveformatex) structure of the content to the codec to enable the codec to optimize its algorithms. This feature, called smart recompression, provides better results than decompressing the content and then feeding the reconstructed PCM samples back through the codec.

When passing the [**WAVEFORMATEX**](dshow.waveformatex) structure, include any extra bytes at the end of the structure specified by **WAVEFORMATEX.cbSize**.
