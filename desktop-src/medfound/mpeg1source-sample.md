...
To use the media source, you can run the [BasicPlayback Sample](mf.basicplayback_sample). The source resolver will automatically load the media source if you select an MPEG-1 file for playback. (If an error occurs, make sure that you successfully registered the MPEG1Source DLL.)
