...
1.  Call [**MFGetService**](mfgetservice.md) to get the [**IMFRateControl**](imfratecontrol.md) interface from the [Media Session](media-session.md).
    > [!Note]  
    > Do not get the [**IMFRateControl**](imfratecontrol.md) interface from the media source. Always get the interface from the Media Session.

     

2.  Call [**IMFRateControl::SetRate**](imfratecontrol-setrate.md) to set the playback rate to zero. For more information about calling this method, see [How to Set the Playback Rate on the Media Session](how-to-set-the-playback-rate-on-the-media-session.md).
3.  Create a seek position in a **PROPVARIANT** by specifying the presentation time to seek to in a **MFTIME** type.
4.  Call [**IMFMediaSession::Start**](imfmediasession-start.md) with the seek position to start playback.
5.  When the scrub operation is complete, the Media Session sends an [MESessionScrubSampleComplete](mesessionscrubsamplecomplete.md) event. Wait for this event before calling [**Start**](imfmediasession-start.md) again for another scrubbing operation.
...
The application can perform frame stepping by setting the playback rate to zero and then passing a **PROPVARIANT** that is set to **VT\_EMPTY** in the call to [**IMFMediaSession::Start**](imfmediasession-start.md).
