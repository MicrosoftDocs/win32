...
-   Get a compatible media type from the Windows Media video encoder by setting the encoding properties and then calling [**IMFTransform::GetOutputAvailableType**](/windows/desktop/api/mftransform/nf-mftransform-imftransform-getoutputavailabletype). This method returns the partial type. Make sure you convert the partial type to a complete type by adding the following information:
    ...
    Because [**IWMCodecPrivateData::GetPrivateData**](https://msdn.microsoft.com/windows/desktop/2598e021-aa36-4df8-813e-eba79d30e7e2) checks the bit rate before returning the codec private data, make sure that you set the bit rate before getting the codec private data.
...
6.  Update the leaky bucket parameter for the audio stream.
    ...
    -   Buffer window: you can retrieve it by calling [**IWMCodecLeakyBucket::GetBufferSizeBits**](https://msdn.microsoft.com/windows/desktop/a7667fea-8bf9-4f87-bae1-6a32c00e753c).
...
2.  For each node, get a pointer to the stream sink in the node by calling [**IMFTopologyNode::GetObject**](/windows/desktop/api/mfidl/nf-mfidl-imftopologynode-getobject). Query for the [**IMFStreamSink**](/windows/desktop/api/mfidl/nn-mfidl-imfstreamsink) interface on the [**IUnknown**](https://msdn.microsoft.com/windows/desktop/33f1d79a-33fc-4ce5-a372-e08bda378332) pointer returned by **IMFTopologyNode::GetObject**.
...
5.  Query the encoder for the [**IPropertyStore**](https://www.bing.com/search?q=**IPropertyStore**) pointer to get the encoding property store from the encoder.
6.  Query the stream sink for the [**IPropertyStore**](https://www.bing.com/search?q=**IPropertyStore**) pointer to get the stream sink's property store.
7.  Call [**IPropertyStore::GetValue**](https://www.bing.com/search?q=**IPropertyStore::GetValue**) to get the required property values from the encoder's property store and copy them to the stream sink's property store by calling **IPropertyStore::SetValue**.
...
-   Open the ASF file in ASFViewer to see the structure of an ASF file. This tool can be downloaded from this [Microsoft website](http://go.microsoft.com/fwlink/p/?linkid=94626).
...
-   [**IWMCodecPrivateData::GetPrivateData**](https://msdn.microsoft.com/windows/desktop/2598e021-aa36-4df8-813e-eba79d30e7e2) returns HRESULT 0x80040203 "A syntax error occurred trying to evaluate a query string"
