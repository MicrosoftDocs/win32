...
<ul>
<li>An update offering multi-boot support via the [<strong>IFileSystemImage2</strong>](/windows/win32/imapi2fs/nn-imapi2fs-ifilesystemimage2?branch=master) interface was introduced in Windows Vista with Service Pack 1 (SP1) and Windows Server 2008.<br/></li>
...
-   On Windows Vista, version 1.0 no longer contains service or kernel-mode components. However, the [**IDiscRecorder2**](/windows/win32/imapi2/nn-imapi2-idiscrecorder2?branch=master) interface still uses the IOCTL\_CDROM\_EXCLUSIVE\_ACCESS and IOCTL\_SCSI\_PASS\_THROUGH\_DIRECT commands to manage access or communication to specific drive devices.
-   On Windows Vista, the version 1.0 interfaces now call the version 2.0 interfaces.
-   Included with Windows Vista with SP1 and Windows Server 2008, IMAPI vesion 2.0 offers multiboot support via the [**IFileSystemImage2**](/windows/win32/imapi2fs/nn-imapi2fs-ifilesystemimage2?branch=master) interface.
...
-   With the [Windows Feature Pack for Storage](http://go.microsoft.com/fwlink/p/?linkid=141659), burn verification options have been made available with a property accessed via [**IBurnVerification**](/windows/win32/imapi2/nn-imapi2-iburnverification?branch=master).
