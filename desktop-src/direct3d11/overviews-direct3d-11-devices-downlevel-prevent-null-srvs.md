...
To prevent the driver from receiving unwanted **NULL** SRVs, the applications must call [**ID3D11DeviceContext::PSSetShaderResources**](/windows/win32/D3D11/nf-d3d11-id3d11devicecontext-pssetshaderresources?branch=master) to unset all SRVs before each call to [**ID3D11DeviceContext::PSSetShader**](/windows/win32/D3D11/nf-d3d11-id3d11devicecontext-pssetshader?branch=master). However, if the applications do not destroy SRVs until the end of their code execution, they do not need to unset the SRVs.
