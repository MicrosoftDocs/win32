...
description: Depending on the tier of tiled resources support, mipmaps with certain dimensions dont follow the standard tile shapes and are considered to all be packed together with one another in a manner that is opaque to the application.
...
Dedicated APIs for copying tiles ([**ID3D11DeviceContext2::CopyTiles**](/windows/win32/D3D11_2/nf-d3d11_2-id3d11devicecontext2-copytiles?branch=master) and [**ID3D11DeviceContext2::UpdateTiles**](/windows/win32/D3D11_2/nf-d3d11_2-id3d11devicecontext2-updatetiles?branch=master)) can't access packed mips. Applications that want to copy data to and from packed mips can do so using all the non-tiled resource specific APIs for copying and rendering to surfaces.
