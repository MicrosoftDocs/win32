...
2.  Declare the location on the hull with the [domain location](https://docs.microsoft.com/windows/desktop/direct3dhlsl/sv-domainlocation) system value.
...
4.  Add user-defined code to compute the outputs; this makes up the body of the domain shader.
    ...
    The function is invoked once for each point generated by the fixed function tessellator. Since this example uses a quad patch, the input domain location ([SV\_DomainLocation](https://docs.microsoft.com/windows/desktop/direct3dhlsl/sv-domainlocation)) is a float2 (UV); a tri patch would have a float3 input location (UVW barycentric coordinates), and an isoline would have a float2 input domain location.
