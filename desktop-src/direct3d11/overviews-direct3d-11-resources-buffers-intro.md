...
keywords: ["constant buffer, what is", "vertex buffer, what is", "index buffer, what is"]
---
...
The following are the buffer resource types supported by Direct3D 11. All buffer types are encapsulated by the [**ID3D11Buffer**](id3d11buffer.md) interface.
...
-   Offset - the number of bytes from the start of the buffer to the data for the first vertex. You can specify the offset using the [**ID3D11DeviceContext::IASetVertexBuffers**](id3d11devicecontext-iasetvertexbuffers.md) method.
-   BaseVertexLocation - the number of bytes from the offset to the first vertex used by the appropriate draw call.

Before you create a vertex buffer, you need to define its layout by creating an [**ID3D11InputLayout**](id3d11inputlayout.md) interface; this is done by calling the [**ID3D11Device::CreateInputLayout**](id3d11device-createinputlayout.md) method. After the input-layout object is created, you can bind it to the input-assembler stage by calling the [**ID3D11DeviceContext::IASetInputLayout**](id3d11devicecontext-iasetinputlayout.md).

To create a vertex buffer, call [**ID3D11Device::CreateBuffer**](id3d11device-createbuffer.md).
...
-   Offset - the number of bytes from the base address of the index buffer. The offset is supplied to the [**ID3D11DeviceContext::IASetIndexBuffer**](id3d11devicecontext-iasetindexbuffer.md) method.
-   StartIndexLocation - specifies the first index buffer element from the base address and the offset provided in [**IASetIndexBuffer**](id3d11devicecontext-iasetindexbuffer.md). The start location is supplied to the [**ID3D11DeviceContext::DrawIndexed**](id3d11devicecontext-drawindexed.md) or [**ID3D11DeviceContext::DrawIndexedInstanced**](id3d11devicecontext-drawindexedinstanced.md) method and represents the first index to render.
-   IndexCount - the number of indices to render. The number is supplied to the [**DrawIndexed**](id3d11devicecontext-drawindexed.md) method
...
To create an index buffer, call [**ID3D11Device::CreateBuffer**](id3d11device-createbuffer.md).
...
Each element stores a 1-to-4 component constant, determined by the format of the data stored. To create a shader-constant buffer, call [**ID3D11Device::CreateBuffer**](id3d11device-createbuffer.md) and specify the **D3D11\_BIND\_CONSTANT\_BUFFER** member of the [**D3D11\_BIND\_FLAG**](d3d11-bind-flag.md) enumerated type.

A constant buffer can only use a single bind flag (**D3D11\_BIND\_CONSTANT\_BUFFER**), which cannot be combined with any other bind flag. To bind a shader-constant buffer to the pipeline, call one of the following methods: [**ID3D11DeviceContext::GSSetConstantBuffers**](id3d11devicecontext-gssetconstantbuffers.md), [**ID3D11DeviceContext::PSSetConstantBuffers**](id3d11devicecontext-pssetconstantbuffers.md), or [**ID3D11DeviceContext::VSSetConstantBuffers**](id3d11devicecontext-vssetconstantbuffers.md).
