...
The [**RegisterDeviceRemovedEvent**](id3d11device4-registerdeviceremovedevent.md), and [**UnregisterDeviceRemoved**](id3d11device4-unregisterdeviceremoved.md) methods are supported by a new interface, [**ID3D11Device4**](id3d11device4.md), to support receiving an asynchronous event notification when a Direct3D device has been removed.

## Multithreaded protection

To ensure that graphics commands in particular are executed in a specific order, the [**ID3D11Multithread**](id3d11multithread.md) interface has methods to turn multithread protection on and off, and methods to enter and leave critical code requiring this protection.

## Fences for multi-device synchronization and interop with Direct3D 12

The [**ID3D11Fence**](id3d11fence.md), [**ID3D11Device5**](id3d11device5.md) and [**ID3D11DeviceContext4**](id3d11devicecontext4.md) provide the same fence functionality as Direct3D 12 for Direct3D 11. Fences are used to synchronize multiple Direct3D11 devices, and for interop between Direct3D 11 and Direct3D 12. Fences are supported in the Windows 10 Creators Update.
...
Refer to the boolean setting in [**D3D11\_FEATURE\_DATA\_D3D11\_OPTIONS4**](d3d11-feature-data-d3d11-options4.md).
