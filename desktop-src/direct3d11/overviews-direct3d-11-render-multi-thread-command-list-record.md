...
1.  A command list must be created from a deferred context, which contains device state and rendering actions. Given a device, create a deferred context by calling [**ID3D11Device::CreateDeferredContext**](id3d11device-createdeferredcontext.md).
...
3.  Create/record a command list by calling [**ID3D11DeviceContext::FinishCommandList**](id3d11devicecontext-finishcommandlist.md) and passing a pointer to an uninitialized [**ID3D11CommandList**](id3d11commandlist.md) interface.
