...
1.  A command list must be created from a deferred context, which contains device state and rendering actions. Given a device, create a deferred context by calling [**ID3D11Device::CreateDeferredContext**](/windows/win32/D3D11/nf-d3d11-id3d11device-createdeferredcontext?branch=master).
...
3.  Create/record a command list by calling [**ID3D11DeviceContext::FinishCommandList**](/windows/win32/D3D11/nf-d3d11-id3d11devicecontext-finishcommandlist?branch=master) and passing a pointer to an uninitialized [**ID3D11CommandList**](/windows/win32/D3D11/nn-d3d11-id3d11commandlist?branch=master) interface.
