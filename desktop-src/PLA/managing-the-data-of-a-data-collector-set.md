...
To enable the data manager for a data collector set, set the [**IDataManager::Enabled**](/previous-versions/windows/desktop/api/Pla/nf-pla-idatamanager-get_enabled) property to VARIANT\_TRUE. The data manager is used to create reports, create cabinet files, and manage the disk resources used by the data collector set.

The data manager runs after the data collectors in the set complete. To have the data manager verify available disk space before the data collector set runs, set the [**IDataCollectorSet::CheckBeforeRunning**](/previous-versions/windows/desktop/api/Pla/nf-pla-idatamanager-get_checkbeforerunning) property to VARIANT\_TRUE.

To run the data manager manually, call the [**IDataManager::Run**](/previous-versions/windows/desktop/api/Pla/nf-pla-idatamanager-run) method and specify the steps to run.
