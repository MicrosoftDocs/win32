...
keywords: ["Federation Web Services for Windows", "WWSAPI", "WWS"]
---
...
-   Client-side: For obtaining the security token from the STS, one may use [**WsRequestSecurityToken**](wsrequestsecuritytoken.md) function. Alternatively, one may use a client side security token provider library such as CardSpace or LiveID, and then use their output to locally create a security token using [**WsCreateXmlSecurityToken**](wscreatexmlsecuritytoken.md). Either way, once the client has the security token, it may then create a channel to the service specifying [**WS\_XML\_TOKEN\_MESSAGE\_SECURITY\_BINDING**](ws-xml-token-message-security-binding.md) to present the token, together with a transport security binding such as [**WS\_SSL\_TRANSPORT\_SECURITY\_BINDING**](ws-ssl-transport-security-binding.md) to protect the channel.
-   Server-side: In a federation scenario with a security token service that issues SAML tokens, the server may use the [**WS\_SAML\_MESSAGE\_SECURITY\_BINDING**](ws-saml-message-security-binding.md), together with a transport security binding such as [**WS\_SSL\_TRANSPORT\_SECURITY\_BINDING**](ws-ssl-transport-security-binding.md) to protect the channel.
-   STS-side: Note that the STS is a Web Service application, and it can specify the security requirements for those requesting a security token from it using a [**security description**](ws-security-description.md) structure at listener creation time just as any other secure Web Service would. It may then parse incoming request message payloads to validate the token requests and send back issued tokens as reply message payloads. Currently, no features are provided to help these parsing and issuing steps.
...
For the client to make a successful [**WsRequestSecurityToken**](wsrequestsecuritytoken.md) call, it needs to know the parameters of that call (such as the required token type and claim types), the [**security description**](ws-security-description.md) requirements of the request channel to the STS, and the [endpoint address](endpoint-address.md) of the STS. The client application may use any of the following techniques for determining this information:
...
-   Dynamically discover this information at runtime using the [metadata import](metadata-import.md) feature with the [**WS\_ISSUED\_TOKEN\_MESSAGE\_SECURITY\_BINDING\_CONSTRAINT**](ws-issued-token-message-security-binding-constraint.md) structure.
