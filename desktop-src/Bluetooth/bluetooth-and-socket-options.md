...
Bluetooth for Windows supports the following socket options. Socket options are set and queried using the [**setsockopt**](https://msdn.microsoft.com/library/windows/desktop/ms740476) and [**getsockopt**](https://msdn.microsoft.com/library/windows/desktop/ms738544) functions, respectively. All of the following options can be used with the **setsockopt** function, but only the **SO\_BTH\_MTU** option is available for use with the **getsockopt** function.
...
For disconnected sockets, the **SO\_BTH\_AUTHENTICATE** specifies that authentication is required for a [**connect**](https://msdn.microsoft.com/library/windows/desktop/ms737625) or [**accept**](https://msdn.microsoft.com/library/windows/desktop/ms737526) operation to complete successfully. Setting this socket option actively initiates authentication during connection establishment, if the two Bluetooth devices were not previously authenticated. The user interface for passkey exchange, if necessary, is provided by the operating system outside the application context.
