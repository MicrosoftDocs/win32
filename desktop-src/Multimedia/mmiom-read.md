...
keywords: ["MMIOM_READ message Windows Multimedia"]
topic_type:
- apiref
api_name:
- MMIOM_READ
...
The **MMIOM\_READ** message is sent to an I/O procedure by the [**mmioRead**](mmioread.md) function to request that a specified number of bytes be read from an open file.
...
The I/O procedure is responsible for updating the **lDiskOffset** member of the [**MMIOINFO**](mmioinfo.md) structure to reflect the new file position after the read operation.
