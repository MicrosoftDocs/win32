...
The **MMIOM\_WRITEFLUSH** message is sent to an I/O procedure by the [**mmioWrite**](mmiowrite.md) function to request that data be written to an open file and that any internal buffers used by the I/O procedure be flushed to disk.
...
The I/O procedure is responsible for updating the **lDiskOffset** member of the [**MMIOINFO**](mmioinfo.md) structure to reflect the new file position after the write operation.
