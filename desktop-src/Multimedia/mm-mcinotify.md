...
When your application requests notification for a command, check the error return of the [**mciSendString**](/windows/desktop/api/Mmsystem/) or [**mciSendCommand**](/windows/desktop/api/Mmsystem/) functions. If these functions encounter an error and return a nonzero value, MCI will not set notification for the command.
