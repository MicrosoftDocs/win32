...
Most MCI applications use the [**mciSendString**](/windows/desktop/api/Mmsystem/) and [**mciSendCommand**](/windows/desktop/api/Mmsystem/) functions dozens of times. MCI provides some other useful functions that your application will use less frequently.

The device identifier required by most MCI commands is typically retrieved in a call to the [**open**](open.md) ([**MCI\_OPEN**](mci-open.md)) command. If you need a device identifier but do not want to open the device — for example, if you want to query the capabilities of the device before taking any other action — you can call the [**mciGetDeviceID**](/windows/desktop/api/Mmsystem/) function.

The [**mciGetCreatorTask**](/windows/desktop/api/Mmsystem/) function allows your application to use a device identifier to retrieve a handle to the task that created that identifier.

You can use the [**mciGetYieldProc**](/windows/desktop/api/Mmsystem/) and [**mciSetYieldProc**](/windows/desktop/api/Mmsystem/) functions to assign and retrieve the address of the callback function associated with the "wait" (MCI\_WAIT) flag.

The [**mciGetErrorString**](/windows/desktop/api/Mmsystem/) function retrieves a string that describes an MCI error value. Each string that MCI returns, whether data or an error description, is a maximum of 128 characters. Dialog box fields that are smaller than 128 characters will truncate the longer strings returned by MCI. For more information about these strings, see [MCIERR Return Values](mcierr-return-values.md).
...
| [**MCI\_MSF\_FRAME**](/windows/desktop/api/Mmsystem/)     | Retrieves the frames component from an MSF value.  |
