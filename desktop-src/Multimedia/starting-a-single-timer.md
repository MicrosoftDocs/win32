...
To start a single timer event, call the [**timeSetEvent**](/windows/desktop/api/TimeAPI/) function, specifying the amount of time before the callback occurs, the resolution, the address of the callback function (see [**TimeProc**](/windows/desktop/api/Mmsystem/)), and the user data to supply with the callback function. An application can use a function like the following to start a single timer event.
