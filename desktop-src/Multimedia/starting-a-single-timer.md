...
To start a single timer event, call the [**timeSetEvent**](https://msdn.microsoft.com/en-us/library/Dd757634(v=VS.85).aspx) function, specifying the amount of time before the callback occurs, the resolution, the address of the callback function (see [**TimeProc**](https://msdn.microsoft.com/en-us/library/Dd757631(v=VS.85).aspx)), and the user data to supply with the callback function. An application can use a function like the following to start a single timer event.
