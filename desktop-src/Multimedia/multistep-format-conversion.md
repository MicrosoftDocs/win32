...
To convert in two steps, use the [**acmFormatSuggest**](/windows/win32/Msacm/nf-msacm-acmformatsuggest?branch=master) function to find a PCM format that matches the ADPCM format. Then use two conversion streams to perform the conversion. For example, perform one conversion from 16-bit, 44-kHz stereo PCM to 16-bit, 11-kHz mono, then convert from 16-bit, 11-kHz mono to 11-kHz mono ADPCM.
