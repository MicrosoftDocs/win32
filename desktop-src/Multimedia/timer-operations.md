...
After you have established your application's timer resolution, you can start timer events by using the [**timeSetEvent**](https://msdn.microsoft.com/en-us/library/Dd757634(v=VS.85).aspx) function. This function returns a timer identifier that can be used to stop or identify timer events. One of the function's parameters is the address of a [**TimeProc**](https://msdn.microsoft.com/en-us/library/Dd757631(v=VS.85).aspx) callback function that is called when the timer event takes place.
...
You can cancel an active timer event at any time by using the [**timeKillEvent**](https://msdn.microsoft.com/en-us/library/Dd757630(v=VS.85).aspx) function. Be sure to cancel any outstanding timers before freeing the memory containing the callback function.
