...
keywords: ["multimedia timers,events", "timers,events", "timeSetEvent function", "timeKillEvent function", "starting timer events", "single timer events", "periodic timer events", "cancelling timer events"]
---

# Timer Event Operations

After you have established your application's timer resolution, you can start timer events by using the [**timeSetEvent**](timesetevent.md) function. This function returns a timer identifier that can be used to stop or identify timer events. One of the function's parameters is the address of a [**TimeProc**](timeproc.md) callback function that is called when the timer event takes place.
...
You can cancel an active timer event at any time by using the [**timeKillEvent**](timekillevent.md) function. Be sure to cancel any outstanding timers before freeing the memory containing the callback function.
