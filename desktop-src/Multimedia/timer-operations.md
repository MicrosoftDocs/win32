...
After you have established your application's timer resolution, you can start timer events by using the [**timeSetEvent**](/windows/win32/TimeAPI/?branch=master) function. This function returns a timer identifier that can be used to stop or identify timer events. One of the function's parameters is the address of a [**TimeProc**](/windows/win32/Mmsystem/?branch=master) callback function that is called when the timer event takes place.
...
You can cancel an active timer event at any time by using the [**timeKillEvent**](/windows/win32/TimeAPI/?branch=master) function. Be sure to cancel any outstanding timers before freeing the memory containing the callback function.
