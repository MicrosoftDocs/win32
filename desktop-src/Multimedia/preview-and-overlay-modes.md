...
keywords: ["WM_CAP_SET_PREVIEW message", "capPreview macro", "WM_CAP_SET_PREVIEWRATE message", "capPreviewRate macro", "WM_CAP_SET_SCALE message", "capPreviewScale macro"]
---
...
-   Use the [**WM\_CAP\_SET\_PREVIEW**](wm-cap-set-preview.md) message to enable or disable preview mode by sending the (or the [**capPreview**](cappreview.md) macro) to a capture window.
-   Use the [**WM\_CAP\_SET\_PREVIEWRATE**](wm-cap-set-previewrate.md) message (or the [**capPreviewRate**](cappreviewrate.md) macro) to set the rate at which frames are displayed in preview mode
-   Use the [**WM\_CAP\_SET\_SCALE**](wm-cap-set-scale.md) message (or the [**capPreviewScale**](cappreviewscale.md) macro) to enable or disable scaling of the preview video.
...
Overlay mode is a hardware function that displays the contents of the capture buffer on the monitor without using CPU resources. You can enable and disable overlay mode by sending the [**WM\_CAP\_SET\_OVERLAY**](wm-cap-set-overlay.md) message (or the [**capOverlay**](capoverlay.md) macro) to a capture window. Enabling overlay mode automatically disables preview mode.

You can also set the scroll position of the video frame within the client area of the capture window for preview mode or overlay mode by sending the [**WM\_CAP\_SET\_SCROLL**](wm-cap-set-scroll.md) message (or the [**capSetScrollPos**](capsetscrollpos.md) macro) to a capture window.
