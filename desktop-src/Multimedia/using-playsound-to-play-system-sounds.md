...
The [**PlaySound**](/windows/desktop/api/Mmsystem/) function will also play sounds referred to by a keyname in the registry. Users can assign their own sounds to system alerts and warnings, or to user actions, such as a mouse button click. Sounds that are associated with system alerts and warnings are called *sound events*.
...
The [**sndPlaySound**](/windows/desktop/api/Mmsystem/) function always searches the registry for a keyname matching *lpszSound* before attempting to load a file with this name. The [**PlaySound**](/windows/desktop/api/Mmsystem/) function accepts flags that specify the location of the sound.
