...
The [**IAVIStream**](/windows/win32/Vfw/nn-vfw-iavistream?branch=master) and [**IAVIFile**](/windows/win32/Vfw/nn-vfw-iavifile?branch=master) interfaces contain the methods used by file and stream handlers. The **PAVISTREAM** data type is a pointer to an AVI stream object (through the **IAVIStream** interface) and the **PAVIFILE** data type is a pointer to an AVI file object (through the **IAVIFile** interface).
