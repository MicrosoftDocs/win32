...
The following example uses the [**mmioDescend**](mmiodescend.md) function to search for the "FMT" chunk in the "RIFF" chunk of the previous example.
...
To search for a subchunk (that is, any chunk other than a "RIFF" or "LIST" chunk), identify its parent chunk in the *lpckParent* parameter of the [**mmioDescend**](mmiodescend.md) function.
...
If the search for a subchunk fails, the current file position is undefined. You can use the [**mmioSeek**](mmioseek.md) function and the **dwDataOffset** member of the [**MMCKINFO**](mmckinfo.md) structure describing the parent chunk to seek back to the beginning of the parent chunk, as in the following example:
