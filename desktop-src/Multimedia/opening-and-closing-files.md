...
keywords: ["AVIFileOpen function", "AVIFileRelease function"]
---

# Opening and Closing Files

An application must open an AVI file before reading or writing. To open an AVI file, use the [**AVIFileOpen**](avifileopen.md) function. **AVIFileOpen** returns the address of an AVI file interface that contains the handle of the open file and increments the reference count of the file.
...
You can increment the reference count of a file by using the [**AVIFileAddRef**](avifileaddref.md) function. For example, you might want to do this when passing a handle of the file interface to another application, or when you want to keep a file open while using a function that would normally close the file.

You can close a file by using the [**AVIFileRelease**](avifilerelease.md) function. The **AVIFileRelease** function decrements the reference count of an AVI file, saves changes made to the file, and when the reference count reaches zero, closes the file. Your applications should balance the reference count by including a call to **AVIFileRelease** for each use of [**AVIFileOpen**](avifileopen.md) and **AVIFileAddRef**.
