...
To see whether a device supports a particular format (standard or nonstandard), you can call the [**waveOutOpen**](waveoutopen.md) function with the WAVE\_FORMAT\_QUERY flag. The following example uses this technique to determine whether a waveform-audio device supports a specified format.
...
This technique for determining nonstandard format support also applies to waveform-audio input devices. The only difference is that the [**waveInOpen**](waveinopen.md) function is used in place of [**waveOutOpen**](waveoutopen.md) to query for format support.
