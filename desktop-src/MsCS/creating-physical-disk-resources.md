...
Applications may need to obtain access to a disk on a shared bus in order to secure data storage or satisfy [dependency](resource-dependencies.md) requirements for a [resource](resources.md). To make a disk usable as a [*cluster*](c-gly.md#-wolf-cluster-gly) resource, create a [*Physical Disk resource*](p-gly.md#-wolf-physical-disk-resource-gly) for it.

New Physical Disk resources should be created only for available [*cluster-capable disks*](c-gly.md#-wolf-cluster-capable-disk-gly). That is, disks that are accessible by all cluster nodes and that are not associated with an existing Physical Disk resource.
