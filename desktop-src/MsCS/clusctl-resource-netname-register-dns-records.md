...
Instructs the designated resource to re-register its DNS Host records with the DNS server associated with the designated node. Applications use this [control code](about-control-codes.md) as a parameter to the [**ClusterResourceControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusterresourcecontrol) function.
