...
keywords: ["CLUSCTL_RESOURCE_NETNAME_REGISTER_DNS_RECORDS control code Failover Cluster"]
topic_type:
- apiref
api_name:
...
Instructs the designated resource to re-register its DNS Host records with the DNS server associated with the designated node. Applications use this [control code](about-control-codes.md) as a parameter to the [**ClusterResourceControl**](clusterresourcecontrol.md) function.
