...
keywords: ["CLUSCTL_NETWORK_UNKNOWN control code Failover Cluster"]
topic_type:
- apiref
api_name:
...
This [control code](about-control-codes.md) verifies that control codes are being processed on the node where execution of the control is directed. Applications use this control code as a [**ClusterNetworkControl**](clusternetworkcontrol.md) parameter.
...
For complete parameter descriptions, see [**ClusterNetworkControl**](clusternetworkcontrol.md). This control code has no parameters associated with it.

<dl></dl>

## Return value

When an application uses CLUSCTL\_NETWORK\_UNKNOWN as a parameter for [**ClusterNetworkControl**](clusternetworkcontrol.md), **ClusterNetworkControl** always returns **ERROR\_SUCCESS**.
