...
Conditional. If your DLL handles [*quorum-capable resource*](q-gly.md#-wolf-quorum-capable-resource-gly) storage class resources, you must support the CLUSCTL\_RESOURCE\_TYPE\_STORAGE\_GET\_AVAILABLE\_DISKS control code. Create and return a value list formatted as described under *lpOutBuffer*. For more information, see [Creating a Value List](creating-value-lists.md) and [Implementing ResourceTypeControl](implementing-resourcetypecontrol.md).
