...
keywords: ["CLUSCTL_CLUSTER_GET_CLUSDB_TIMESTAMP control code Failover Cluster"]
topic_type:
- apiref
api_name:
...
Retrieves a cluster database timestamp for a cluster. Applications use this [control code](about-control-codes.md) as a [**ClusterControl**](clustercontrol.md) parameter.
...
For complete parameter descriptions, see [**ClusterControl**](clustercontrol.md). The following control code function parameter is specific to this control code.
...
When an application uses CLUSCTL\_CLUSTER\_GET\_CLUSDB\_TIMESTAMP as a parameter for [**ClusterControl**](clustercontrol.md), **ClusterControl** returns one of the following values:
