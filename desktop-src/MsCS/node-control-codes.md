...
The [Failover Cluster API](the-server-cluster-api.md) defines the following [*external control codes*](https://www.bing.com/search?q=*external control codes*) for [*nodes*](https://www.bing.com/search?q=*nodes*) (there are no [*internal control codes*](https://www.bing.com/search?q=*internal control codes*) defined for nodes):

Node control codes use the **CLUS\_OBJECT\_NODE** value of the [**CLUSTER\_CONTROL\_OBJECT**](/previous-versions/windows/desktop/api/ClusAPI/ne-clusapi-cluster_control_object) enumeration to indicate that the control code applies to cluster nodes. For more information about control codes, see [Control Code Architecture](control-code-architecture.md).

Node control codes are enumerated by the [**CLUSCTL\_NODE\_CODES**](/previous-versions/windows/desktop/api/ClusAPI/ne-clusapi-clusctl_node_codes) enumeration.
...
Retrieves the read/write [common properties](common-properties.md) for a [node](nodes.md). Applications use this [control code](about-control-codes.md) as a [**ClusterNodeControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusternodecontrol) parameter.
...
Retrieves the bitmap that represents the set of [nodes](nodes.md) that the current [Global Update Manager](https://www.bing.com/search?q=Global Update Manager) (GUM) lock owner is waiting for.
