...
Removes a [node](nodes.md) from the [*cluster*](https://www.bing.com/search?q=*cluster*).
...
An evicted node is no longer a member of the [*cluster*](https://www.bing.com/search?q=*cluster*). The [Cluster service](cluster-service.md) must be reinstalled on the node to reestablish the node's cluster membership.
