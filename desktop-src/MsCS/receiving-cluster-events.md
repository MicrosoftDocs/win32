...
An application or [resource DLL](resource-dlls.md) can monitor events occurring within the [*cluster*](https://www.bing.com/search?q=*cluster*) by creating a notification port. A notification port stores information about events as a queue of notifications (messages). Once a notification port has been created, an application typically spawns a thread to monitor the port, periodically retrieving event data and taking appropriate action in response to the events.
