...
Retrieves the read-only [private properties](private-properties.md) for a [resource](resources.md). Applications use this [control code](about-control-codes.md) as a [**ClusterResourceControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusterresourcecontrol) parameter, and [resource DLLs](resource-dlls.md) receive the control code as a [**ResourceControl**](/previous-versions/windows/desktop/api/ResApi/nc-resapi-presource_control_routine) parameter.
...
Required. Always support the CLUSCTL\_RESOURCE\_GET\_RO\_PRIVATE\_PROPERTIES control code in your implementation of [**ResourceControl**](/previous-versions/windows/desktop/api/ResApi/nc-resapi-presource_control_routine). Return a property list that includes all of the read-only properties specific to your resource. Remember to terminate the list with **CLUSPROP\_SYNTAX\_ENDMARK**, an enumerator from the [**CLUSTER\_PROPERTY\_SYNTAX**](/previous-versions/windows/desktop/api/ClusAPI/ne-clusapi-cluster_property_syntax) enumeration. For more information on working with property lists, see [Creating Property Lists](creating-property-lists.md).
