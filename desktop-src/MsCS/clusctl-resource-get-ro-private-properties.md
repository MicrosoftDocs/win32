...
keywords: ["CLUSCTL_RESOURCE_GET_RO_PRIVATE_PROPERTIES control code Failover Cluster"]
topic_type:
- apiref
api_name:
...
Retrieves the read-only [private properties](private-properties.md) for a [resource](resources.md). Applications use this [control code](about-control-codes.md) as a [**ClusterResourceControl**](clusterresourcecontrol.md) parameter, and [resource DLLs](resource-dlls.md) receive the control code as a [**ResourceControl**](resourcecontrol.md) parameter.
...
Required. Always support the CLUSCTL\_RESOURCE\_GET\_RO\_PRIVATE\_PROPERTIES control code in your implementation of [**ResourceControl**](resourcecontrol.md). Return a property list that includes all of the read-only properties specific to your resource. Remember to terminate the list with **CLUSPROP\_SYNTAX\_ENDMARK**, an enumerator from the [**CLUSTER\_PROPERTY\_SYNTAX**](cluster-property-syntax.md) enumeration. For more information on working with property lists, see [Creating Property Lists](creating-property-lists.md).
