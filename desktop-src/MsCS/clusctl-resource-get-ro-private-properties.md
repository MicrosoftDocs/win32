---
title: CLUSCTL\_RESOURCE\_GET\_RO\_PRIVATE\_PROPERTIES control code
description: Retrieves the read-only private properties for a resource.
audience: developer
author: REDMOND\\markl
manager: REDMOND\\markl
ms.assetid: c6cf84bd-ba14-4c66-9789-95cf3de996e6
ms.prod: windows-server-dev
ms.technology: failover-clustering
ms.tgt_platform: multiple
keywords:
- CLUSCTL_RESOURCE_GET_RO_PRIVATE_PROPERTIES control code Failover Cluster
topic_type:
- apiref
api_name:
...
# CLUSCTL\_RESOURCE\_GET\_RO\_PRIVATE\_PROPERTIES control code

Retrieves the read-only [private properties](private-properties.md) for a [resource](resources.md). Applications use this [control code](about-control-codes.md) as a [**ClusterResourceControl**](/windows/previous-versions/ClusAPI/nf-clusapi-clusterresourcecontrol?branch=master) parameter, and [resource DLLs](resource-dlls.md) receive the control code as a [**ResourceControl**](/windows/previous-versions/ResApi/nc-resapi-presource_control_routine?branch=master) parameter.
...
ClusterResourceControl( 
  ...
  CLUSCTL_RESOURCE_GET_RO_PRIVATE_PROPERTIES,        // this control code
...
On a successful return, points to a [property list](property-lists.md) containing the resource's read-only private properties.
...
Do not use the CLUSCTL\_RESOURCE\_GET\_RO\_PRIVATE\_PROPERTIES control code in any resource DLL entry point function. This control code can safely be called from a worker thread. For more information, see [Function Calls to Avoid in Resource DLLs](function-calls-to-avoid-in-resource-dlls.md).

ClusAPI.h defines the 32 bits of CLUSCTL\_RESOURCE\_GET\_RO\_PRIVATE\_PROPERTIES as follows (for more information, see [Control Code Architecture](control-code-architecture.md)).
...
Required. Always support the CLUSCTL\_RESOURCE\_GET\_RO\_PRIVATE\_PROPERTIES control code in your implementation of [**ResourceControl**](/windows/previous-versions/ResApi/nc-resapi-presource_control_routine?branch=master). Return a property list that includes all of the read-only properties specific to your resource. Remember to terminate the list with **CLUSPROP\_SYNTAX\_ENDMARK**, an enumerator from the [**CLUSTER\_PROPERTY\_SYNTAX**](/windows/previous-versions/ClusAPI/ne-clusapi-cluster_property_syntax?branch=master) enumeration. For more information on working with property lists, see [Creating Property Lists](creating-property-lists.md).

As a general guideline, the [Resource Monitor](resource-monitor.md) should handle all of the control codes for [common properties](common-properties.md), while your DLL should handle all control codes for [private properties](private-properties.md).
...
#ifndef _CLUSDOCEX_RESGETROPPROPS_CPP
#define _CLUSDOCEX_RESGETROPPROPS_CPP

LPVOID ClusDocEx_ResGetROPProperties
(
    IN HRESOURCE hRes,
    IN HNODE     hNode,
...
// Description:
// 
//     Retrieves the read-only private properties of a resource
//     using CLUSCTL_RESOURCE_GET_RO_PRIVATE_PROPERTIES.
//     
...
//    read-only private properties of the resource identified
//    by hRes.
// 
...
// ====================================================================
{
    ...
    DWORD  dwControlCode = CLUSCTL_RESOURCE_GET_RO_PRIVATE_PROPERTIES;
...
// end ClusDocEx_ResGetROPProperties
```
