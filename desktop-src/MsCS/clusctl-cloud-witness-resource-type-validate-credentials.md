...
description: Retrieves the token and credentials used to validate a cluster node's access to a storage account for a Cloud Witness resource.
audience: developer
...
keywords: ["CLUSCTL_CLOUD_WITNESS_RESOURCE_TYPE_VALIDATE_CREDENTIALS control code Failover Cluster"]
topic_type:
- apiref
api_name:
