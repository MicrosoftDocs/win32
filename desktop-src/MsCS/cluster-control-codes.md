...
The [Failover Cluster API](the-server-cluster-api.md) defines the following [*external control codes*](https://www.bing.com/search?q=*external control codes*) for [*clusters*](https://www.bing.com/search?q=*clusters*) (there are no [*internal control codes*](https://www.bing.com/search?q=*internal control codes*) defined for clusters):

Cluster control codes use the **CLUS\_OBJECT\_CLUSTER** value of the [**CLUSTER\_CONTROL\_OBJECT**](/previous-versions/windows/desktop/api/ClusAPI/ne-clusapi-cluster_control_object) enumeration to indicate that the control code applies to clusters. For more information about control codes, see [Control Code Architecture](control-code-architecture.md).

Cluster control codes are enumerated by the [**CLUSCTL\_CLUSTER\_CODES**](/previous-versions/windows/desktop/api/ClusAPI/ne-clusapi-clusctl_cluster_codes) enumeration.
...
TBD. Applications use this control code as a parameter to the [**ClusterControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clustercontrol) function.
...
Retrieves the fully qualified domain name (FQDN) of a [cluster](https://www.bing.com/search?q=cluster).
...
Retrieves the name of the current [Global Update Manager](https://www.bing.com/search?q=Global Update Manager) (GUM) lock owner for a [cluster](https://www.bing.com/search?q=cluster).
...
[CLUSCTL\_CLUSTER\_SET\_DNS\_DOMAIN](https://www.bing.com/search?q=CLUSCTL\_CLUSTER\_SET\_DNS\_DOMAIN)
</dt> <dd>
...
Verifies that a [property list](property-lists.md) is properly formatted. Applications use this [control code](about-control-codes.md) as a [**ClusterControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clustercontrol) parameter.
