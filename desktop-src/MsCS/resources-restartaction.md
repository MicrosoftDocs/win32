...
The data value for the **RestartAction** property can be set to the values of the [**CLUSTER\_RESOURCE\_RESTART\_ACTION**](/previous-versions/windows/desktop/api/ClusAPI/ne-clusapi-cluster_resource_restart_action) enumeration. The following table summarizes the possible values:
...
| **ClusterResourceRestartNoNotify**<br/> | 1<br/> | Restart the resource after a failure. If the resource exceeds its restart threshold within its restart period, do not attempt to [failover](failover.md) the [group](groups.md) to another [node](nodes.md) in the [*cluster*](https://www.bing.com/search?q=*cluster*).<br/> |
