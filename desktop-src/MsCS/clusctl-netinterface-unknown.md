...
keywords: ["CLUSCTL_NETINTERFACE_UNKNOWN control code Failover Cluster"]
topic_type:
- apiref
api_name:
...
This [control code](about-control-codes.md) verifies that the network interface control code verifies that control codes are being processed on the node where execution of the control is directed. Applications use this control code as a [**ClusterNetInterfaceControl**](clusternetinterfacecontrol.md) parameter.
...
For complete parameter descriptions, see [**ClusterNetInterfaceControl**](clusternetinterfacecontrol.md). This control code has no parameters associated with it.

<dl></dl>

## Return value

When an application uses CLUSCTL\_NETINTERFACE\_UNKNOWN as a parameter for [**ClusterNetInterfaceControl**](clusternetinterfacecontrol.md), **ClusterNetInterfaceControl** always returns **ERROR\_SUCCESS**.
