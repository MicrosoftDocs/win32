...
Before changing the cluster configuration, you can create a backup of the [cluster database](cluster-database.md) using the [Cluster API](cluster-api.md) function [**BackupClusterDatabase**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-backupclusterdatabase). This creates a snapshot of the cluster configuration which can later be restored with the [**RestoreClusterDatabase**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-restoreclusterdatabase) function.
