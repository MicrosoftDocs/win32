...
Retrieves the intrinsic characteristics of a [network](networks.md). Applications use this [control code](about-control-codes.md) as a [**ClusterNetworkControl**](/windows/previous-versions/ClusAPI/nf-clusapi-clusternetworkcontrol?branch=master) parameter.
...
On a successful return, contains a DWORD bitmask describing the network's characteristics enumerated from the [**CLUS\_CHARACTERISTICS**](/windows/previous-versions/ClusAPI/ne-clusapi-clus_characteristics?branch=master) enumeration. Only one bitmask is currently defined.
