...
For all string [properties](cluster-object-properties.md), the [*cluster*](https://www.bing.com/search?q=*cluster*) imposes no restrictions on the size of the string that you use. It is possible to store megabyte-sized strings as cluster property values. However, the registry API recommends that data over 2048 KB in size (2 MB) should be stored in an external file. Other factors may also limit string size:
...
In general, megabyte-size strings stored as properties will adversely impact performance and usability. [*Physical Disk resources*](https://www.bing.com/search?q=*Physical Disk resources*) should be used for large-scale data storage.
