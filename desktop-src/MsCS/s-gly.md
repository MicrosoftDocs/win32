...
The server side of the [*client-server model*](c-gly.md#-wolf-client-server-model-gly). The server waits for requests from its clients. When it receives a request, the server processes it and returns the requested information to the client.
...
See [*failover cluster*](f-gly.md#mscs-failover-cluster-gly).
...
See [*Failover Cluster API*](f-gly.md#mscs-failover-cluster-api-gly).
...
See [*cluster object*](c-gly.md#-wolf-cluster-object-gly).
...
Data or operation made available to network clients through a [*failover cluster instance*](f-gly.md#mscs-failover-cluster-instance-gly); not necessarily a service.
...
A disk shared between multiple owners simultaneously. [*Failover cluster*](f-gly.md#mscs-failover-cluster-gly) do not use shared disks. A [*cluster disk*](c-gly.md#-wolf-cluster-disk-gly) is owned by only one [*node*](n-gly.md#-wolf-node-gly) at any point in time.
...
Cluster organization in which some [*resources*](r-gly.md#-wolf-resource-gly) are accessible to all systems in the [*cluster*](c-gly.md#-wolf-cluster-gly).
...
A single hardware or software component that prevents a [*cluster*](c-gly.md#-wolf-cluster-gly) from functioning if the component fails. Administrators try to avoid single points of failure through redundancy and backup systems.
...
A data storage device that is accessible to all cluster [*nodes*](n-gly.md#-wolf-node-gly) and that is able to operate as a cluster [*resource*](r-gly.md#-wolf-resource-gly). An example of a storage class resource is the [Physical Disk](physical-disk.md) resource.
...
See [*disk striping*](d-gly.md#-wolf-disk-striping-gly).
...
See [*node*](n-gly.md#-wolf-node-gly).
