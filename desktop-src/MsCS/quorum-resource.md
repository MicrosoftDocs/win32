...
In every [*failover cluster*](f-gly.md#mscs-failover-cluster-gly), a single [resource](resources.md) is designated as the [*quorum resource*](q-gly.md#-wolf-quorum-resource-gly). The [Cluster service](cluster-service.md) uses the quorum resource to perform two fundamental tasks:
