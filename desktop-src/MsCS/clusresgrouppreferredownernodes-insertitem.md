...
Adding a node to a group's [**ClusResGroupPreferredOwnerNodes**](clusresgrouppreferredownernodes-collection.md) collection means that the node is listed as a [*preferred owner*](https://www.bing.com/search?q=*preferred owner*) node for the group. Use *lPosition* to specify an order of preference, with low values corresponding to highly preferred, high-priority nodes.
