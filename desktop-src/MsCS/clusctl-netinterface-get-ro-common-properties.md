---
title: CLUSCTL\_NETINTERFACE\_GET\_RO\_COMMON\_PROPERTIES control code
description: Retrieves the read-only common properties for a network interface.
audience: developer
...
Retrieves the read-only [common properties](common-properties.md) for a [network interface](network-interfaces.md). Applications use this [control code](about-control-codes.md) as a [**ClusterNetInterfaceControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusternetinterfacecontrol) parameter.
...
ClusterNetInterfaceControl( 
  ...
  CLUSCTL_NETINTERFACE_GET_RO_COMMON_PROPERTIES,  // this control code
...
On a successful return, points to a [property list](property-lists.md) containing the specified network interface's read-only [network interface common properties](network-interface-common-properties.md).
...
ClusAPI.h defines the 32 bits of CLUSCTL\_NETINTERFACE\_GET\_RO\_COMMON\_PROPERTIES as follows (for more information, see [Control Code Architecture](control-code-architecture.md)).
