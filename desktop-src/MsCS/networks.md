...
A [*failover cluster*](https://www.bing.com/search?q=*failover cluster*) defines a network as a connection between [network interfaces](network-interfaces.md) on the same subnet. A network performs one of the following roles in a [*cluster*](https://www.bing.com/search?q=*cluster*):
