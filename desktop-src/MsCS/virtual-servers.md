...
[*Failover clusters*](f-gly.md#mscs-failover-cluster-gly), however, allow the creation of failover cluster instances. A failover cluster instance is a [group](groups.md) that contains:
...
However, a failover cluster instance is not associated with a specific computer and can be [*failedover*](f-gly.md#-wolf-failover-gly) like any other group. If the [node](nodes.md) hosting the failover cluster instance fails, clients can still access the failover cluster instance's resources using the same server name.
