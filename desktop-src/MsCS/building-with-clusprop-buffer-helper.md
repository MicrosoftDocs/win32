...
To create a [value](value-lists.md) or [property list](property-lists.md) with [**CLUSPROP\_BUFFER\_HELPER**](/previous-versions/windows/desktop/api/ClusAPI/ns-clusapi-clusprop_buffer_helper), or to create any other kind of structure in a buffer, use the following procedure.
...
The following examples use [**CLUSPROP\_BUFFER\_HELPER**](/previous-versions/windows/desktop/api/ClusAPI/ns-clusapi-clusprop_buffer_helper) to generate a value list entry. A value list would consist of "stacks" of these entries terminated by **CLUSPROP\_SYNTAX\_ENDMARK**. A property list would consist of "stacks" of value lists (one value list per property) prefixed by the count of properties in the list.
