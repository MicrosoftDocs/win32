...
keywords:
- CLUSCTL_RESOURCE_VM_CANCEL_MIGRATION control code Failover Cluster
topic_type:
- apiref
api_name:
...
Cancels an ongoing live migration of a virtual machine (VM). The migration can be canceled only if the migration is in the "Setup" ([**MigrationState**](virtual-machines-migrationstate.md) is 1) or "Data Transfer" (**MigrationState** is 2) states and the resource control is called on the source node of the live migration. Applications use this control code as a parameter to the [**ClusterResourceControl**](/windows/previous-versions/ClusAPI/nf-clusapi-clusterresourcecontrol?branch=master) function, and [resource DLLs](resource-dlls.md) receive the control code as a parameter to the [*ResourceControl*](/windows/previous-versions/ResApi/nc-resapi-presource_control_routine?branch=master) callback function.
