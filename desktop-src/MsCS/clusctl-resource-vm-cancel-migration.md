...
Cancels an ongoing live migration of a virtual machine (VM). The migration can be canceled only if the migration is in the "Setup" ([**MigrationState**](virtual-machines-migrationstate.md) is 1) or "Data Transfer" (**MigrationState** is 2) states and the resource control is called on the source node of the live migration. Applications use this control code as a parameter to the [**ClusterResourceControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusterresourcecontrol) function, and [resource DLLs](resource-dlls.md) receive the control code as a parameter to the [*ResourceControl*](/previous-versions/windows/desktop/api/ResApi/nc-resapi-presource_control_routine) callback function.
