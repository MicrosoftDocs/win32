---
title: CLUSCTL\_GROUP\_SET\_COMMON\_PROPERTIES control code
description: Updates the read/write common properties for a group.
audience: developer
author: REDMOND\\markl
manager: REDMOND\\markl
ms.assetid: 854acb91-28df-431d-a6ed-ade4d319d41f
ms.prod: windows-server-dev
ms.technology: failover-clustering
ms.tgt_platform: multiple
keywords:
- CLUSCTL_GROUP_SET_COMMON_PROPERTIES control code Failover Cluster
topic_type:
- apiref
api_name:
...
# CLUSCTL\_GROUP\_SET\_COMMON\_PROPERTIES control code

Updates the read/write [common properties](common-properties.md) for a [group](groups.md). Applications use this [control code](about-control-codes.md) as a [**ClusterGroupControl**](/windows/previous-versions/ClusAPI/nf-clusapi-clustergroupcontrol?branch=master) parameter.


```C++
ClusterGroupControl( hGroup,                               // group handle
                     hHostNode,                            // optional node handle
                     CLUSCTL_GROUP_SET_COMMON_PROPERTIES,  // this control code
...
Pass a pointer to a [property list](property-lists.md) containing new values for one or more read/write [common group properties](group-common-properties.md).
...
If the **Priority** property for core and cluster shared volume (CSV) group is changed to a different value than what the current value is, the **ERROR\_INVALID\_PARAMETER** error code is returned. If that property is changed to the same value as the current value, the **ERROR\_SUCCESS** return code is returned. However, if that property is changed for any group other than the core and CSV group, the **ERROR\_SUCCESS** return code is returned regardless of whether new value matches the current value.

ClusAPI.h defines the 32 bits of CLUSCTL\_GROUP\_SET\_COMMON\_PROPERTIES as follows (for more information, see [Control Code Architecture](control-code-architecture.md)).
