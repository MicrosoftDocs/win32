...
keywords: ["entry point functions Failover Cluster ,implementing ResourceTypeControl", "ResourceTypeControl Failover Cluster ,implementing"]
---

# Implementing ResourceTypeControl

The [Cluster service](cluster-service.md) (through a [Resource Monitor](resource-monitor.md)) invokes the [**ResourceTypeControl**](resourcetypecontrol.md) entry point function when:

-   An application calls [**ClusterResourceTypeControl**](clusterresourcetypecontrol.md) to initiate a [control code](control-codes.md) operation.
...
To request that the Resource Monitor process a control code, return **ERROR\_INVALID\_FUNCTION**. You can return this value for a control code even if you include support for the code in [**ResourceTypeControl**](resourcetypecontrol.md), allowing you to augment the default functionality of the Resource Monitor.
