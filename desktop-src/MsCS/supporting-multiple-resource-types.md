...
For example, a single database application might be supported by a Database Engine resource type, a Transaction Engine resource type, and a Connection Manager resource type. Each type has its own set of [dependencies](resource-dependencies.md), properties, failure detectors, and [failover](failover.md) policies. However, the DLL that implements these types enforces the proper relationships between the modules and provides ways to share data. Thus the DLL might allow any number of Database Engine resources in the [*cluster*](https://www.bing.com/search?q=*cluster*), but only allow one Transaction Engine resource and one Connection Manager resource per cluster.
