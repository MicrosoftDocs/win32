...
keywords: ["CLUSCTL_CLUSTER_GET_COMMON_PROPERTIES control code Failover Cluster"]
topic_type:
- apiref
api_name:
...
Retrieves the read/write cluster [common properties](common-properties.md) for a cluster. Applications use this [control code](about-control-codes.md) as a [**ClusterControl**](clustercontrol.md) parameter.
...
For complete parameter descriptions, see [**ClusterControl**](clustercontrol.md). The following control code function parameter is specific to the CLUSCTL\_CLUSTER\_GET\_COMMON\_PROPERTIES control code.
...
When an application uses CLUSCTL\_CLUSTER\_GET\_COMMON\_PROPERTIES as a parameter for [**ClusterControl**](clustercontrol.md), **ClusterControl** returns one of the following values:
