...
Repairs the password for a security principal on a client based on the client's alternate computer name. Applications use this [control code](about-control-codes.md) as a parameter to the [**ClusterResourceControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusterresourcecontrol) function.
...
ClusAPI.h defines the 32 bits of CLUSCTL\_RESOURCE\_NETNAME\_REPAIR\_VCO (0x0100018D) as follows (for more information, see [Control Code Architecture](control-code-architecture.md)).
