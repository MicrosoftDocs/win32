...
keywords:
- CLUSCTL_RESOURCE_GET_REQUIRED_DEPENDENCIES control code Failover Cluster
topic_type:
- apiref
api_name:
...
Retrieves a list of all required [dependencies](resource-dependencies.md) for a [resource](resources.md). Applications use this [control code](about-control-codes.md) as a [**ClusterResourceControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusterresourcecontrol) parameter, and [resource DLLs](resource-dlls.md) receive the control code as a parameter to the [**ResourceControl**](/previous-versions/windows/desktop/api/ResApi/nc-resapi-presource_control_routine) function.
...
On a successful return, points to a [value list](value-lists.md) containing a [**CLUSPROP\_REQUIRED\_DEPENDENCY**](/previous-versions/windows/desktop/api/ClusAPI/ns-clusapi-clusprop_required_dependency) union for each dependency.
...
Conditional. If your resource has required dependencies, you must support the CLUSCTL\_RESOURCE\_GET\_REQUIRED\_DEPENDENCIES control code in your implementation of [**ResourceControl**](/previous-versions/windows/desktop/api/ResApi/nc-resapi-presource_control_routine). Return a value list containing one [**CLUSPROP\_REQUIRED\_DEPENDENCY**](/previous-versions/windows/desktop/api/ClusAPI/ns-clusapi-clusprop_required_dependency) union for each resource on which your resource is required to depend. Remember to terminate the list with **CLUSPROP\_SYNTAX\_ENDMARK**.

For example, if your resource has required dependencies on a [*storage class resource*](https://www.bing.com/search?q=*storage class resource*) and a [Network Name](network-name.md) resource, [**ResourceControl**](/previous-versions/windows/desktop/api/ResApi/nc-resapi-presource_control_routine) should return a value list with three entries:

-   A [**CLUSPROP\_REQUIRED\_DEPENDENCY**](/previous-versions/windows/desktop/api/ClusAPI/ns-clusapi-clusprop_required_dependency) union with the **ResClass** member set to **CLUS\_RESCLASS\_STORAGE**.
-   A [**CLUSPROP\_REQUIRED\_DEPENDENCY**](/previous-versions/windows/desktop/api/ClusAPI/ns-clusapi-clusprop_required_dependency) union with the **ResTypeName** member set to "Network Name".
-   A [**CLUSPROP\_VALUE**](/previous-versions/windows/desktop/api/ClusAPI/ns-clusapi-clusprop_value) structure with the Syntax member set to **CLUSPROP\_SYNTAX\_ENDMARK**.
