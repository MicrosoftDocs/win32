...
The cluster administrator can set, remove, or query a disk resource's maintenance mode through failover cluster cmdlets, or a calling application can place the resource in maintenance mode by using the [**ClusterResourceControl**](/previous-versions/windows/desktop/api/ClusAPI/nf-clusapi-clusterresourcecontrol) API function with either the [**CLUSCTL\_RESOURCE\_SET\_MAINTENANCE\_MODE**](clusctl-resource-set-maintenance-mode.md) or [**CLUSCTL\_RESOURCE\_QUERY\_MAINTENANCE\_MODE**](clusctl-resource-query-maintenance-mode.md) cluster resource control codes.
