...
Returns the flags set for a [*storage class resource*](https://www.bing.com/search?q=*storage class resource*) partition.
...
**Long** that receives the flags of the storage class resource, enumerated by the [**CLUSPROP\_PIFLAGS**](/previous-versions/windows/desktop/api/ClusAPI/ne-clusapi-clusprop_piflags) enumeration.
...
The partition should be used to store quorum files if no partition is specified in the [**SetClusterQuorumResource**](/previous-versions/windows/desktop/api/ClusAPI/nc-clusapi-pclusapi_set_cluster_quorum_resource) function. For [Physical Disk](physical-disk.md) resources, the smallest NTFS partition larger than 50MB automatically receives this flag.
